    ...
    media::EncryptionScheme>::Convert(const media::EncryptionScheme& input)
{
    ...
    mojo_encryption_scheme->pattern = media::mojom::Pattern::From(input.pattern());
    ...
    Convert(const scoped_refptr<media::DecoderBuffer>& input)
{
    ...
    if (input->decrypt_config()) {
        mojo_buffer->decrypt_config = media::mojom::DecryptConfig::From(*input->decrypt_config());
    }
    ...
    Convert(const media::AudioDecoderConfig& input)
{
    ...
    config->encryption_scheme = media::mojom::EncryptionScheme::From(input.encryption_scheme());
    return config;
}
    ...
    Convert(const scoped_refptr<media::VideoFrame>& input)
{
    ...
    frame->end_of_stream = input->metadata()->IsTrue(media::VideoFrameMetadata::END_OF_STREAM);
    ...
    CHECK_EQ(media::VideoFrame::STORAGE_MOJO_SHARED_BUFFER,
        input->storage_type());
    media::MojoSharedBufferVideoFrame* input_frame = static_cast<media::MojoSharedBufferVideoFrame*>(input.get());
    mojo::ScopedSharedBufferHandle duplicated_handle = input_frame->Handle().Clone();
