    ...
    std::unique_ptr<media::cast::ReceiverTimeOffsetEstimatorImpl> estimator)
{
    ...
    std::unique_ptr<base::DictionaryValue> stats = video_stats_subscriber->GetStats();
    std::string json;
...
int main(int argc, char** argv)
{
    ...
    int remote_port = 0;
    if (!base::StringToInt(cmd->GetSwitchValueASCII(kSwitchPort), &remote_port) || remote_port < 0 || remote_port > 65535) {
        remote_port = 2344;
    }
    LOG(INFO) << "Sending to " << remote_ip_address << ":" << remote_port
              << ".";

    media::cast::FrameSenderConfig audio_config = media::cast::GetDefaultAudioSenderConfig();
    media::cast::FrameSenderConfig video_config = media::cast::GetDefaultVideoSenderConfig();

    // Running transport on the main thread.
    // Setting up transport config.
    net::IPEndPoint remote_endpoint = CreateUDPAddress(remote_ip_address, static_cast<uint16_t>(remote_port));
    ...
    std::unique_ptr<media::cast::CastTransport> transport_sender = media::cast::CastTransport::Create(
    ...
    std::unique_ptr<media::cast::CastSender> cast_sender = media::cast::CastSender::Create(cast_environment, transport_sender.get());
    ...
    io_message_loop.task_runner()->PostTask(
        FROM_HERE, base::Bind(&media::cast::CastSender::InitializeAudio, base::Unretained(cast_sender.get()), audio_config, base::Bind(&QuitLoopOnInitializationResult)));
