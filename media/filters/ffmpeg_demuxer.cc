...
static base::Time ExtractTimelineOffset(AVFormatContext* format_context)
{
    if (strstr(format_context->iformat->name, "webm") || strstr(format_context->iformat->name, "matroska")) {
        ...
        if (entry != NULL && entry->value != NULL && base::Time::FromUTCString(entry->value, &timeline_offset)) {
...
static void RecordVideoCodecStats(const VideoDecoderConfig& video_config,
    ...
    MediaLog* media_log)
{
    media_log->RecordRapporWithSecurityOrigin("Media.OriginUrl.SRC.VideoCodec." + GetCodecName(video_config.codec()));
    ...
    const scoped_refptr<MediaLog>& media_log)
{
    ...
    if (stream->codec->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (!AVStreamToAudioDecoderConfig(stream, audio_config.get()) || !audio_config->IsValidConfig()) {
            MEDIA_LOG(ERROR, media_log) << "FFmpegDemuxer: failed creating audio stream";
            return nullptr;
        }
...
void FFmpegDemuxerStream::EnqueuePacket(ScopedAVPacket packet)
{
    ...
    if (last_packet_timestamp_ != kNoTimestamp) {
        ...
        if (fixup_negative_timestamps_ && buffer->timestamp() < last_packet_timestamp_) {
    ...
    int64_t filesize_in_bytes)
{
    ...
    for (size_t i = 0; i < format_context->nb_streams; ++i) {
        AVCodecContext* codec_parameters = format_context->streams[i]->codec;
    ...
    int result)
{
    ...
    if (internal && internal->packet_buffer && format_context->start_time != static_cast<int64_t>(AV_NOPTS_VALUE)) {
    ...
    for (size_t i = 0; i < format_context->nb_streams; ++i) {
        ...
        const AVCodecContext* codec_parameters = stream->codec;
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (codec_id == AV_CODEC_ID_HEVC) {
                ...
                if (stream->codec->format == AV_PIX_FMT_NONE)
                    stream->codec->format = AV_PIX_FMT_YUV420P;
            }
#endif
        ...
        std::unique_ptr<FFmpegDemuxerStream> demuxer_stream = FFmpegDemuxerStream::Create(this, stream, media_log_);
        ...
        if (strstr(format_context->iformat->name, "webm") || strstr(format_context->iformat->name, "matroska")) {
            ...
            track_id = static_cast<StreamParser::TrackId>(media_tracks->tracks().size() + 1);
            track_label = streams_[i]->GetMetadata("title");
        }

        if ((codec_type == AVMEDIA_TYPE_AUDIO && media_tracks->getAudioConfig(track_id).IsValidConfig()) || (codec_type == AVMEDIA_TYPE_VIDEO && media_tracks->getVideoConfig(track_id).IsValidConfig())) {
            MEDIA_LOG(INFO, media_log_)
                << GetDisplayName()
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            DCHECK(track_id_to_demux_stream_map_.find(media_track->id()) == track_id_to_demux_stream_map_.end());
    ...
    for (const auto& stream : streams_) {
        ...
        DCHECK(audio_stream);
        if (audio_stream->codec->codec_id == AV_CODEC_ID_OPUS || (strcmp(format_context->iformat->name, "ogg") == 0 && audio_stream->codec->codec_id == AV_CODEC_ID_VORBIS)) {
            for (size_t i = 0; i < streams_.size(); ++i) {
                ...
                if (streams_[i]->av_stream() == audio_stream && streams_[i]->start_time() < base::TimeDelta()) {
    ...
    base::TimeDelta max_duration)
{
    ...
    std::unique_ptr<MediaLogEvent> metadata_event = media_log_->CreateEvent(MediaLogEvent::PROPERTY_CHANGE);
    ...
    for (size_t i = 0; i < streams_.size(); ++i) {
        ...
        if (stream->type() == DemuxerStream::AUDIO) {
            ...
            const AVCodecContext* audio_parameters = avctx->streams[i]->codec;
            ...
            const AVCodecContext* video_parameters = video_av_stream->codec;
    ...
    bool enabled)
{
    ...
    for (const auto& stream : streams_) {
        if (!stream || stream->enabled() != enabled)
            continue;
        if (!lowest_start_time_stream || stream->start_time() < lowest_start_time_stream->start_time()) {
    ...
    base::TimeDelta seek_time)
{
    ...
    FFmpegDemuxerStream* lowest_start_time_enabled_stream = FindStreamWithLowestStartTimestamp(true);
    if (lowest_start_time_enabled_stream && lowest_start_time_enabled_stream->start_time() <= seek_time) {
    ...
    FFmpegDemuxerStream* lowest_start_time_disabled_stream = FindStreamWithLowestStartTimestamp(false);
    if (lowest_start_time_disabled_stream && lowest_start_time_disabled_stream->start_time() <= seek_time) {
    ...
    base::TimeDelta currTime)
{
    ...
    // that are enabled.
    for (const auto& stream : streams_) {
        if (stream && stream->type() == DemuxerStream::AUDIO && enabled_streams.find(stream.get()) == enabled_streams.end()) {
    ...
    base::TimeDelta currTime)
{
    ...
    for (const auto& stream : streams_) {
        if (stream && stream->type() == DemuxerStream::VIDEO && stream.get() != selected_stream) {
            DVLOG(1) << __func__ << ": disabling stream " << stream.get();
            stream->set_enabled(false, currTime);
        }
    }
...
void FFmpegDemuxer::ReadFrameIfNeeded()
{
    ...
    if (!blocking_thread_.IsRunning() || !StreamsHaveAvailableCapacity() || pending_read_ || !pending_seek_cb_.is_null()) {
        return;
    }
