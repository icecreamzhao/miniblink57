...
const int kAudioTrackEntryHeaderSize = kAudioTrackSizeOffset + kAudioTrackSizeWidth;
...
const int kVideoTrackEntryHeaderSize = kVideoTrackSizeOffset + kVideoTrackSizeWidth;
...
protected:
    ...
    {
        base::Closure open_cb = base::Bind(&ChunkDemuxerTest::DemuxerOpened, base::Unretained(this));
        ...
        int* size)
    {
        ...
        *size = ebml_header->data_size() + info->data_size() + kTracksHeaderSize + tracks_element_size;
        ...
        if (has_video) {
            ...
            if (is_video_encrypted) {
                ...
                WriteInt64(buf + kVideoTrackSizeOffset,
                    video_track_entry->data_size() + video_content_encodings->data_size() - kVideoTrackEntryHeaderSize);
        ...
        if (has_audio) {
            ...
            if (is_audio_encrypted) {
                ...
                WriteInt64(buf + kAudioTrackSizeOffset,
                    audio_track_entry->data_size() + audio_content_encodings->data_size() - kAudioTrackEntryHeaderSize);
        ...
        const std::string& codecs)
    {
        ...
        if (status == ChunkDemuxer::kOk)
            demuxer_->SetTracksWatcher(
                source_id, base::Bind(&ChunkDemuxerTest::InitSegmentReceived, base::Unretained(this)));
        return status;
    }
        ...
        int timecode, int block_count)
    {
        ...
        ASSERT_TRUE(AppendCluster(
            source_id, GenerateSingleStreamCluster(timecode, end_timecode, track_number, block_duration)));
    }
        ...
        std::vector<BlockInfo>* blocks)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); ++i) {
            ...
            if (track_number == kTextTrackNum || track_number == kAlternateTextTrackNum) {
            ...
            if (track_number == kAudioTrackNum || track_number == kAlternateAudioTrackNum)
        ...
        bool unknown_size)
    {
        ...
        for (size_t i = 0; i < blocks.size(); ++i) {
            ...
            if (blocks[i].duration) {
                if (blocks[i].track_number == kVideoTrackNum || blocks[i].track_number == kAlternateVideoTrackNum) {
    ...
    {
        PipelineStatus expected_status = (stream_flags != 0) ? PIPELINE_OK : CHUNK_DEMUXER_ERROR_APPEND_FAILED;
        ...
        if (is_audio_encrypted || is_video_encrypted) {
            ...
            int need_key_count = (is_audio_encrypted ? 1 : 0) + (is_video_encrypted ? 1 : 0);
            EXPECT_CALL(*this, OnEncryptedMediaInitData(EmeInitDataType::WEBM, std::vector<uint8_t>(kEncryptedMediaInitData, kEncryptedMediaInitData + arraysize(kEncryptedMediaInitData))))
    ...
    bool InitDemuxerWithConfigChangeData()
    {
        ...
        EXPECT_CALL(*this, InitSegmentReceivedMock(_));
        demuxer_->Initialize(
            &host_, CreateInitDoneCB(base::TimeDelta::FromMilliseconds(2744), PIPELINE_OK), true);
    ...
    void AddVideoBlockGroup(ClusterBuilder* cb,
        ...
        int flags)
    {
        const uint8_t* data = (flags & kWebMFlagKeyframe) != 0 ? kVP8Keyframe : kVP8Interframe;
        int size = (flags & kWebMFlagKeyframe) != 0 ? sizeof(kVP8Keyframe) : sizeof(kVP8Interframe);
    ...
    {
        ChunkDemuxerStream* stream = static_cast<ChunkDemuxerStream*>(demuxer_->GetStream(type));
    ...
    MOCK_METHOD2(ReadDone, void(DemuxerStream::Status status, const scoped_refptr<DecoderBuffer>&));
    ...
    {
        EXPECT_CALL(*this, ReadDone(DemuxerStream::kOk, HasTimestamp(timestamp_in_ms)));
        demuxer_->GetStream(type)->Read(base::Bind(
            &ChunkDemuxerTest::ReadDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
        ...
        int stream_flags)
    {
        ...
        // expect.
        for (size_t i = 0;
             (timestamps[i].audio_time_ms != kSkip || timestamps[i].video_time_ms != kSkip);
             i++) {
...
TEST_F(ChunkDemuxerTest, AudioVideoTrackIdsChange)
{
    ...
    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    ASSERT_TRUE(AppendInitSegment(HAS_AUDIO | HAS_VIDEO | USE_ALTERNATE_AUDIO_TRACK_ID | USE_ALTERNATE_VIDEO_TRACK_ID));
...
{
    EXPECT_CALL(*this, DemuxerOpened());
    demuxer_->Initialize(&host_, base::Bind(&ChunkDemuxerTest::DemuxerInitialized, base::Unretained(this)),
        true);
...
TEST_F(ChunkDemuxerTest, EndOfStreamWithPendingReads)
{
    ...
    EXPECT_CALL(host_, SetDuration(base::TimeDelta::FromMilliseconds(kVideoBlockDuration)));
    MarkEndOfStream(PIPELINE_OK);
...
TEST_F(ChunkDemuxerTest, SeekAudioAndVideoSources)
{
    ...
    ASSERT_TRUE(AppendCluster(
        audio_id, GenerateSingleStreamCluster(3000, 3092, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(
        video_id, GenerateSingleStreamCluster(3000, 3132, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, CodecIDsThatAreNotRFC6381Compliant)
{
    ...
    for (size_t i = 0; i < arraysize(codec_ids); ++i) {
        ChunkDemuxer::Status result = demuxer_->AddId("source_id", "audio/mp4", codec_ids[i]);
...
TEST_F(ChunkDemuxerTest, EndOfStreamDuringSeek)
{
    ...
    ASSERT_TRUE(AppendCluster(kDefaultSecondCluster()));
    EXPECT_CALL(host_, SetDuration(base::TimeDelta::FromMilliseconds(kDefaultSecondClusterEndTimestamp)));
    MarkEndOfStream(PIPELINE_OK);
...
TEST_F(ChunkDemuxerTest, EmitBuffersDuringAbort)
{
    ...
    Ranges<base::TimeDelta> range_before_abort = demuxer_->GetBufferedRanges(kSourceId);
    demuxer_->ResetParserState(kSourceId,
        append_window_start_for_next_append_,
        append_window_end_for_next_append_,
        &timestamp_offset_map_[kSourceId]);
    Ranges<base::TimeDelta> range_after_abort = demuxer_->GetBufferedRanges(kSourceId);
...
TEST_F(ChunkDemuxerTest, DurationChange)
{
    ...
    EXPECT_CALL(host_, SetDuration(base::TimeDelta::FromMilliseconds(kNewStreamDurationVideo)));
    ASSERT_TRUE(
    ...
    EXPECT_CALL(host_, SetDuration(base::TimeDelta::FromMilliseconds(kFinalStreamDuration)));
    ASSERT_TRUE(
        AppendCluster(GenerateCluster(kStreamDuration + kAudioBlockDuration,
...
TEST_F(ChunkDemuxerTest, DurationChangeTimestampOffset)
{
    ...
    EXPECT_CALL(host_, SetDuration(kDefaultDuration() + base::TimeDelta::FromMilliseconds(kVideoBlockDuration * 2)));
    ASSERT_TRUE(AppendCluster(GenerateCluster(0, 4)));
}
...
TEST_F(ChunkDemuxerTest, EndOfStreamTruncateDuration)
{
    ...
    EXPECT_CALL(host_, SetDuration(base::TimeDelta::FromMilliseconds(kDefaultFirstClusterEndTimestamp)));
    MarkEndOfStream(PIPELINE_OK);
}
...
TEST_F(ChunkDemuxerTest, AppendWindow_WebMFile_AudioOnly)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("bear-320x240-audio-only.webm");
    ExpectInitMediaLogs(HAS_AUDIO);
    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(2));
...
TEST_F(ChunkDemuxerTest, AppendWindow_AudioConfigUpdateRemovesPreroll)
{
    ...
    scoped_refptr<DecoderBuffer> buffer2 = ReadTestDataFile("bear-320x240-audio-only-48khz.webm");
    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    ...
    RelaxedKeyframe_RemoveInterruptsMuxedCodedFrameGroup_3)
{
    ...
    std::vector<MuxedStreamInfo> msi(2);
    msi[0] = MuxedStreamInfo(kAudioTrackNum, "0K 10K 20K 30K 40K 50K 60K 70K 80D10K");
    ...
    while (video_start < cluster->size() - 10) {
        if (cluster->data()[video_start] == 0xA3 && cluster->data()[video_start + 9] == 0x81) {
            found = true;
            break;
        }
        video_start++;
