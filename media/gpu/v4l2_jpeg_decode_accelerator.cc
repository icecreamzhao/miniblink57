...
V4L2JpegDecodeAccelerator::~V4L2JpegDecodeAccelerator()
{
    ...
    if (decoder_thread_.IsRunning()) {
        decoder_task_runner_->PostTask(
            FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::DestroyTask, base::Unretained(this)));
    ...
    Error error)
{
    child_task_runner_->PostTask(
        FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::NotifyError, weak_ptr_, bitstream_buffer_id, error));
}
...
bool V4L2JpegDecodeAccelerator::Initialize(Client* client)
{
    ...
    decoder_task_runner_->PostTask(
        FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::StartDevicePoll, base::Unretained(this)));
    ...
    const scoped_refptr<VideoFrame>& video_frame)
{
    ...
    decoder_task_runner_->PostTask(
        FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::DecodeTask, base::Unretained(this), base::Passed(&job_record)));
}
...
bool V4L2JpegDecodeAccelerator::ShouldRecreateInputBuffers()
{
    ...
    return (input_buffer_map_.empty() || (job_record->shm.size() + sizeof(kDefaultDhtSeg)) > input_buffer_map_.front().length[0]);
}
...
bool V4L2JpegDecodeAccelerator::CreateInputBuffers()
{
    ...
    for (size_t i = 0; i < input_buffer_map_.size(); ++i) {
        ...
        for (size_t j = 0; j < buffer.length; ++j) {
            void* address = device_->Mmap(NULL, planes[j].length, PROT_READ | PROT_WRITE,
...
bool V4L2JpegDecodeAccelerator::CreateOutputBuffers()
{
    ...
    VideoPixelFormat output_format = V4L2Device::V4L2PixFmtToVideoPixelFormat(output_buffer_pixelformat_);
    ...
    for (size_t i = 0; i < output_buffer_map_.size(); ++i) {
        ...
        for (size_t j = 0; j < buffer.length; ++j) {
            if (base::checked_cast<int64_t>(planes[j].length) < VideoFrame::PlaneSize(
                    output_format, j,
...
void V4L2JpegDecodeAccelerator::DevicePollTask()
{
    ...
    decoder_task_runner_->PostTask(
        FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::ServiceDeviceTask, base::Unretained(this), event_pending));
}
...
bool V4L2JpegDecodeAccelerator::DequeueSourceChangeEvent()
{
    ...
    if (device_->Ioctl(VIDIOC_DQEVENT, &ev) == 0) {
        if (ev.type == V4L2_EVENT_SOURCE_CHANGE) {
            ...
            if (ev.u.src_change.changes & (V4L2_EVENT_SRC_CH_RESOLUTION | V4L2_EVENT_SRC_CH_PIXELFORMAT)) {
                return true;
            }
...
void V4L2JpegDecodeAccelerator::ServiceDeviceTask(bool event_pending)
{
    ...
    if (!running_jobs_.empty()) {
        device_poll_task_runner_->PostTask(
            FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::DevicePollTask, base::Unretained(this)));
    }
...
void V4L2JpegDecodeAccelerator::EnqueueOutput()
{
    ...
    while (running_jobs_.size() > OutputBufferQueuedCount() && !free_output_buffers_.empty()) {
    ...
    const scoped_refptr<VideoFrame>& dst_frame)
{
    ...
    if (output_buffer_num_planes_ == 1) {
        ...
        VideoPixelFormat format = V4L2Device::V4L2PixFmtToVideoPixelFormat(output_buffer_pixelformat_);
        size_t src_size = VideoFrame::AllocationSize(format, output_buffer_coded_size_);
    ...
    } else if (output_buffer_pixelformat_ == V4L2_PIX_FMT_YUV420M || output_buffer_pixelformat_ == V4L2_PIX_FMT_YUV422M) {
...
void V4L2JpegDecodeAccelerator::Dequeue()
{
    ...
    while (!running_jobs_.empty() && OutputBufferQueuedCount() > 0) {
        ...
        if (dqbuf.flags & V4L2_BUF_FLAG_ERROR) {
            ...
            child_task_runner_->PostTask(
                FROM_HERE, base::Bind(&V4L2JpegDecodeAccelerator::VideoFrameReady, weak_ptr_, job_record->bitstream_buffer_id));
        }
    }
}
