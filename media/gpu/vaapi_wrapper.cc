    ...
    const base::Closure& report_error_to_uma_cb)
{
    ...
    scoped_refptr<VaapiWrapper> vaapi_wrapper = Create(mode, va_profile, report_error_to_uma_cb);
    return vaapi_wrapper;
}
...
VaapiWrapper::GetSupportedEncodeProfiles()
{
    ...
    std::vector<ProfileInfo> encode_profile_infos = profile_infos_.Get().GetSupportedProfileInfosForCodecMode(kEncode);
...
VaapiWrapper::GetSupportedDecodeProfiles()
{
    ...
    std::vector<ProfileInfo> decode_profile_infos = profile_infos_.Get().GetSupportedProfileInfosForCodecMode(kDecode);
    ...
    CodecMode mode)
{
    ...
    if (!profile_infos_.Get().IsProfileSupported(mode, va_profile) && va_profile == VAProfileH264Baseline) {
...
VaapiWrapper::GetSupportedProfileInfosForCodecModeInternal(CodecMode mode)
{
    ...
    VAEntrypoint entrypoint = (mode == kEncode ? VAEntrypointEncSlice : VAEntrypointVLD);
    ...
    VAEntrypoint entrypoint)
{
    ...
    if (num_supported_entrypoints < 0 || num_supported_entrypoints > max_entrypoints) {
    ...
    const std::vector<VAConfigAttrib>& required_attribs)
{
    ...
    for (size_t i = 0; i < required_attribs.size(); ++i) {
        if (attribs[i].type != required_attribs[i].type || (attribs[i].value & required_attribs[i].value) != required_attribs[i].value) {
    ...
    gfx::Size* resolution)
{
    ...
    VAStatus va_res = vaCreateConfig(va_display_, va_profile, entrypoint, &required_attribs[0],
    ...
    std::vector<VASurfaceID>* va_surfaces)
{
    ...
    VAStatus va_res = vaCreateSurfaces(va_display_, va_format, size.width(), size.height(),
    ...
    const scoped_refptr<ui::NativePixmap>& pixmap)
{
    ...
    va_attrib_extbuf.pixel_format = BufferFormatToVAFourCC(pixmap->GetBufferFormat());
    ...
    size_t num_planes = gfx::NumberOfPlanesForBufferFormat(pixmap->GetBufferFormat());
    ...
    scoped_refptr<ui::NativePixmap> target_pixmap)
{
    ...
    scoped_refptr<VASurface> processed_va_surface = CreateVASurfaceForPixmap(target_pixmap);
    ...
    void* buffer)
{
    ...
    VAEncMiscParameterBuffer* misc_param = reinterpret_cast<VAEncMiscParameterBuffer*>(data_ptr);
...
{
    base::AutoLock auto_lock(*va_lock_);
    VAStatus va_res = vaCreateBuffer(va_display_, va_context_id_, VAEncCodedBufferType, size, 1,
        NULL, buffer_id);
...
bool VaapiWrapper::Execute(VASurfaceID va_surface_id)
{
    ...
    if (pending_slice_bufs_.size() > 0) {
        va_res = vaRenderPicture(va_display_, va_context_id_, &pending_slice_bufs_[0],
    ...
    void** mem)
{
    ...
    va_res = vaCreateImage(va_display_, format, size.width(), size.height(), image);
...
VaapiWrapper::LazyProfileInfos::LazyProfileInfos()
{
    ...
    for (size_t i = 0; i < kCodecModeMax; ++i) {
        supported_profiles_[i] = vaapi_wrapper->GetSupportedProfileInfosForCodecModeInternal(
...
{
    return (major_version_ < major) || (major_version_ == major && minor_version_ < minor);
}
