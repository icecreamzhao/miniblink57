        ...
        static void RunTargetSamplingPeriodTest(int target_fps)
        {
            ...
            for (int content_fps = 1; content_fps <= 60; ++content_fps) {
                ...
                if (content_period >= target_sampling_period) {
                    ...
                    // rate.
                    const double absolute_diff = std::abs(1.0 / sampling_period.InSecondsF() - target_fps);
...
protected:
        ...
        const char* description)
    {
        ...
        for (std::vector<Event>::const_iterator i = events.begin();
            ...
            if (!has_detection_switched && was_detecting_before != sampler()->HasProposal()) {
                has_detection_switched = true;
            } else if (has_detection_switched && is_detecting_after != sampler()->HasProposal()) {
    ...
    void ExpectFrameDropRatioIsCorrect()
    {
        ...
        const double expected_sampling_ratio = GetParam().content_period.InSecondsF() / ComputeExpectedSamplingPeriod().InSecondsF();
...
TEST_P(AnimatedContentSamplerParameterizedTest, FrameTimestampsAreSmooth)
{
    ...
    const int64_t max_expected_repeats_per_frame = 1 + ComputeExpectedSamplingPeriod() / GetParam().vsync_interval;
    ...
    size_t stray_count_remaining = (frame_timestamps.size() - 1) - (highest_count + second_highest_count);
    ...
    for (size_t repeats = 1; repeats < display_counts.size() - 1; ++repeats) {
        if (display_counts[repeats] == highest_count) {
            ...
            ++repeats;
        } else if (second_highest_count > 0 && display_counts[repeats] == second_highest_count) {
    ...
    FrameTimestampsConvergeTowardsEventTimes)
{
    ...
    const base::TimeDelta total_error = events.back().second - last_frame_timestamp;
    const base::TimeDelta max_acceptable_error = GetParam().min_capture_period + base::TimeDelta::FromMilliseconds(max_drift_increment_millis);
