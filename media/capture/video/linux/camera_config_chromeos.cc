...
static const char kCameraCharacteristicsConfigFile[] = "/etc/camera/camera_characteristics.conf";
...
std::string CameraConfigChromeOS::GetUsbId(const std::string& device_id) const
{
    ...
    const std::string symlink = base::StringPrintf("/sys/class/video4linux/%s/device", file_name.c_str());
    ...
    for (const base::StringPiece& line : lines) {
        ...
        if (sub_keys.size() == 2 && sub_keys[1] == kLensFacing) {
            ...
            case LensFacing::FRONT:
                camera_id_to_facing_[camera_id] = VideoFacingMode::MEDIA_VIDEO_FACING_USER;
                break;
            case LensFacing::BACK:
                camera_id_to_facing_[camera_id] = VideoFacingMode::MEDIA_VIDEO_FACING_ENVIRONMENT;
                break;
            default:
