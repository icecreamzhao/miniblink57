    ...
    uint8_t* level_idc)
{
    ...
    uint32_t elem = 0;
    if (codec_id.size() != 11 || !base::HexStringToUInt(base::StringPiece(codec_id).substr(5), &elem)) {
    ...
    uint8_t* level_idc)
{
    ...
    const int kMaxHevcCodecIdLength = 5 + // 'hev1.' or 'hvc1.' prefix (5 chars)
    ...
    if (elem[1].size() > 0 && (elem[1][0] == 'A' || elem[1][0] == 'B' || elem[1][0] == 'C')) {
    ...
    if (!base::StringToUint(elem[1], &general_profile_idc) || general_profile_idc > 0x1f) {
    ...
    if (!base::StringToUint(elem[3], &general_level_idc) || general_level_idc > 0xff) {
...
VideoCodec StringToVideoCodec(const std::string& codec_id)
{
    ...
    if (codec_id == "vp8" || codec_id == "vp8.0")
        return kCodecVP8;
    if (ParseNewStyleVp9CodecID(codec_id, &profile, &level) || ParseLegacyVp9CodecID(codec_id, &profile, &level)) {
        return kCodecVP9;
    }
    if (codec_id == "theora")
        return kCodecTheora;
