...
{
    if (base::StartsWith(mime, "video", base::CompareCase::SENSITIVE) && is_secure && direction == MEDIA_CODEC_DECODER) {
...
MediaCodecStatus NdkMediaCodecBridge::GetOutputSize(gfx::Size* size)
{
    ...
    bool has_right = AMediaFormat_getInt32(format, kMediaFormatKeyCropRight, &right);
    bool has_bottom = AMediaFormat_getInt32(format, kMediaFormatKeyCropBottom, &bottom);
    ...
    base::TimeDelta presentation_time)
{
    ...
    media_status_t status = AMediaCodec_queueInputBuffer(media_codec_.get(), index, 0, data_size,
    ...
    base::TimeDelta presentation_time)
{
    ...
    if (data && !FillInputBuffer(index, data, data_size))
        return MEDIA_CODEC_ERROR;
    if (encryption_scheme.mode() != EncryptionScheme::CIPHER_MODE_AES_CTR || encryption_scheme.pattern().IsInEffect())
        return MEDIA_CODEC_ERROR;
    ...
    if (subsamples_size == 0) {
        ...
        for (int i = 0; i < subsamples_size; ++i) {
            ...
            if (subsamples[i].cypher_bytes > static_cast<int32_t>(std::numeric_limits<int32_t>::max())) {
                return MEDIA_CODEC_ERROR;
            }
    ...
    bool* key_frame)
{
    ...
    *presentation_time = base::TimeDelta::FromMicroseconds(buffer_info.presentationTimeUs);
    if (end_of_stream)
...
MediaCodecStatus NdkMediaCodecBridge::GetOutputBufferAddress(
    ...
    size_t* capacity)
{
    const uint8_t* src_data = AMediaCodec_getOutputBuffer(media_codec_.get(), index, capacity);
