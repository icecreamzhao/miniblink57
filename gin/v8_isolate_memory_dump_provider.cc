...
void V8IsolateMemoryDumpProvider::DumpHeapStatistics(
    base::trace_event::ProcessMemoryDump* process_memory_dump)
{
    std::string dump_base_name = base::StringPrintf("v8/isolate_%p", isolate_holder_->isolate());
    ...
    for (size_t space = 0; space < number_of_spaces; space++) {
        ...
        std::string space_dump_name = space_name_prefix + "/" + space_statistics.space_name();
    ...
    const size_t object_types = isolate_holder_->isolate()->NumberOfTrackedHeapObjectTypes();
    for (size_t type_index = 0; type_index < object_types; type_index++) {
        ...
        std::string dump_name = object_name_prefix + "/" + object_statistics.object_type();
    ...
    if (process_memory_dump->GetAllocatorDump(object_name_prefix + "/CODE_TYPE")) {
