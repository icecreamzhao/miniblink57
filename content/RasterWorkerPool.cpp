...
//             if (dependencies) {
//                 cc::TaskGraph::Edge edge(graph_.nodes.back().task.get(), node.task.get());
    ...
    const base::SimpleThread::Options& thread_options)
{
    ...
    while (threads_.size() < static_cast<size_t>(num_threads)) {
        base::DelegateSimpleThread* thread(new base::DelegateSimpleThread(
    ...
    base::TimeDelta delay)
{
    ...
    graph_.Reset();
//     for (const auto& graph_task : tasks_) {
//         cc::TaskGraph::Node node(graph_task.get(), 0, 0, 0);
//         graph_.nodes.push_back(node);
//     }
    DebugBreak();
