...
SkColor HSLToSkColor(const HSL& hsl, SkAlpha alpha)
{
    ...
    if (!saturation) {
        ...
        if (lightness < 0)
            ...
            light = ((int32_t)((lightness) * (1 << 16))) >> 8;
...
public:
        ...
        const SkScalar borderRadius) const
    {
        ...
        static sk_sp<SkShader> shader = SkGradientShader::MakeLinear(gradient_bounds, colors, NULL, 3, SkShader::kClamp_TileMode);
        SkPaint paint;
        paint.setAntiAlias(true);
        ...
        const blink::WebThemeEngine::ButtonExtraParams& button) const
    {
        ...
        sk_sp<SkShader> shader = SkGradientShader::MakeLinear(gradient_bounds, colors, NULL, 2, SkShader::kClamp_TileMode);
        paint.setStyle(SkPaint::kFill_Style);
        paint.setAntiAlias(true);
        paint.setShader(shader);
    ...
    {
        SkRect clip;
        return canvas->getClipBounds(&clip) && clip.intersect(SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(x + w), SkIntToScalar(y + h));
    }
