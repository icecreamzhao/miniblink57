...
//         }
        ...
        m_ofn.lpstrFilter = m_filterW.data();
    ...
    virtual void run() override
    {
        ...
        if (m_retVal) {
            ...
            if ((m_wrap->getInfo()->Flags & OFN_ALLOWMULTISELECT) && (::GetFileAttributesW(m_wrap->getFileNameBuf()) & FILE_ATTRIBUTE_DIRECTORY)) {
                ...
                selectedFilesRef.push_back(std::wstring(m_wrap->getFileNameBuf()));
            }
        }
...
static bool runFileChooserImpl(const blink::WebFileChooserParams& params, blink::WebFileChooserCompletion* completion, int webviewId, std::function<void(void)>* completionCallback)
{
    //////////////////////////////////////////////////////////////////////////
    ...
    std::function<void(void)>* callback = new std::function<void(void)>([wrap, saveAs, multiSelect, completion, webviewId, completionCallback] {
        BOOL retVal = FALSE;
        HWND hwndOwner = createHideWindow(); // ::GetActiveWindow();
        ...
        if (wrap->isDirectory()) {
            ...
            if (NULL != pil) {
                ::SHGetPathFromIDListW(pil, wrap->getInfo()->lpstrFile);//获取用户选择的文件路径
                retVal = TRUE;
            }
        } else {
