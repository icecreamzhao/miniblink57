...
std::unique_ptr<gfx::GpuMemoryBuffer> ChildGpuMemoryBufferManager::AllocateGpuMemoryBuffer(
    const gfx::Size& size,
    gfx::BufferFormat format,
    gfx::BufferUsage usage,
    gpu::SurfaceHandle surface_handle)
{
    ...
    std::unique_ptr<GpuMemoryBufferImpl> buffer(GpuMemoryBufferImpl::createFromHandle(handle, size, format, usage));
    if (!buffer)
        return nullptr;
