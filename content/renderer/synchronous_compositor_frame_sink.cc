...
namespace {
    ...
    public:
        SoftwareDevice(RenderViewHost* render_view)
            : canvas_(nullptr)
        ...
        void EndPaint() override
        {
            ...
            HDC hdc = ::GetDC(render_view_->GetPlatformHWND());
            skia::DrawToNativeContext(canvas_, hdc, damage_rect_.x(), damage_rect_.y(), &damage_rect_.ToRECT());
            ::ReleaseDC(render_view_->GetPlatformHWND(), hdc);
        }

        void FirePaintEvent(HDC hdc, const RECT* paintRect)
        {
            if (!canvas_ || gfx::Rect(*paintRect).IsEmpty())
                return;

            skia::DrawToNativeContext(canvas_, hdc, paintRect->left, paintRect->top, paintRect);
        }
    ...
    , render_view_(render_view)
{
    ...
    memory_policy_.priority_cutoff_when_visible = gpu::MemoryAllocation::CUTOFF_ALLOW_NICE_TO_HAVE;

    in_software_draw_ = true; // weolar
}
    ...
    cc::CompositorFrameSinkClient* sink_client)
{
    ...
    auto output_surface = base::MakeUnique<SoftwareOutputSurface>(base::MakeUnique<SoftwareDevice>(render_view_));
    software_output_surface_ = output_surface.get();

    shared_bitmap_manager_ = new WebSharedBitmapManager();
...
void SynchronousCompositorFrameSink::DidDrawCallback()
{
    ...
    cc::CompositorFrame frame)
{
    ...
    if (in_software_draw_) {
        sw_viewport_for_current_draw_ = frame.render_pass_list.back()->output_rect; // weolar

        if (current_sw_canvas_ && (current_sw_canvas_->imageInfo().width() != sw_viewport_for_current_draw_.width() || current_sw_canvas_->imageInfo().height() != sw_viewport_for_current_draw_.height())) {
            delete current_sw_canvas_;
            current_sw_canvas_ = nullptr;
        }
        if (!current_sw_canvas_)
            current_sw_canvas_ = skia::CreatePlatformCanvas(sw_viewport_for_current_draw_.width(), sw_viewport_for_current_draw_.height(), true);
        SoftwareDevice* device = (SoftwareDevice*)software_output_surface_->software_device();
        device->SetCanvas(current_sw_canvas_);
        ...
        display_->SetLocalFrameId(root_local_frame_id_, frame.metadata.device_scale_factor);
        ...
        // layer compositor.
        gfx::Size display_size(sw_viewport_for_current_draw_.right(), sw_viewport_for_current_draw_.bottom());
        display_->Resize(display_size);

        // The offset for the child frame relative to the origin of the canvas being
        // drawn into.
        gfx::Transform child_transform;
        child_transform.Translate(gfx::Vector2dF(sw_viewport_for_current_draw_.OffsetFromOrigin()));
        ...
        embed_render_pass->SetNew(1, gfx::Rect(display_size), gfx::Rect(display_size), gfx::Transform());
        embed_render_pass->has_transparent_background = false;

        // The RenderPass has a single SurfaceDrawQuad (and SharedQuadState for it).
        auto* shared_quad_state = embed_render_pass->CreateAndAppendSharedQuadState();
        auto* surface_quad = embed_render_pass->CreateAndAppendDrawQuad<cc::SurfaceDrawQuad>();
        shared_quad_state->SetAll(child_transform, child_size, gfx::Rect(child_size), gfx::Rect() /* clip_rect */, false /* is_clipped */, 1.f /* opacity */, SkBlendMode::kSrcOver, 0 /* sorting_context_id */);
        surface_quad->SetNew(shared_quad_state, gfx::Rect(child_size), gfx::Rect(child_size), cc::SurfaceId(kChildFrameSinkId, child_local_frame_id_));

        //child_factory_->SubmitCompositorFrame(child_local_frame_id_, std::move(frame), base::Bind(&NoOpDrawCallback));
        root_factory_->SubmitCompositorFrame(root_local_frame_id_, std::move(embed_frame), base::Bind(&SynchronousCompositorFrameSink::DidDrawCallback, base::Unretained(this)));
...
void SynchronousCompositorFrameSink::NotifyScreenStatus(int status)
{
    DCHECK(CalledOnValidThread());
    if (sync_client_)
        sync_client_->NotifyScreenStatus(status);
}
#endif
...
void SynchronousCompositorFrameSink::SetMemoryPolicy(size_t bytes_limit)
{
    ...
    bool became_non_zero = !memory_policy_.bytes_limit_when_visible && bytes_limit;
...
void SynchronousCompositorFrameSink::FirePaintEvent(HDC hdc, const RECT* paintRect)
{
    if (software_output_surface_ && software_output_surface_->software_device()) {
        SoftwareDevice* device = (SoftwareDevice*)software_output_surface_->software_device();
