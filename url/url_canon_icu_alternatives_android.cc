...
bool IDNToASCII(const base::char16* src, int src_len, CanonOutputW* output)
{
    ...
    base::android::ScopedJavaLocalRef<jstring> java_src = base::android::ConvertUTF16ToJavaString(
        env, base::StringPiece16(src, src_len));
    ScopedJavaLocalRef<jstring> java_result = android::Java_IDNStringUtil_idnToASCII(env, java_src.obj());
    // NULL indicates failure.
    if (java_result.is_null())
        return false;

    base::string16 utf16_result = base::android::ConvertJavaStringToUTF16(java_result);
