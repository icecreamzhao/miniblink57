...
{
    DCHECK(event.touchesLength || event.type() == WebInputEvent::TouchScrollStarted);
...
bool WebTouchEventTraits::IsTouchSequenceEnd(const WebTouchEvent& event)
{
    ...
    if (!event.touchesLength)
        return true;
    for (size_t i = 0; i < event.touchesLength; ++i) {
        if (event.touches[i].state != blink::WebTouchPoint::StateReleased && event.touches[i].state != blink::WebTouchPoint::StateCancelled)
            return false;
    }
    return true;
}
