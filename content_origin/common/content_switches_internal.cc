...
{
    const base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
    std::string v8_cache_options = command_line.GetSwitchValueASCII(switches::kV8CacheOptions);
...
bool IsUseZoomForDSFEnabled()
{
    ...
    bool enabled = (command_line->HasSwitch(switches::kEnableUseZoomForDSF) || use_zoom_for_dsf_enabled_by_default) && command_line->GetSwitchValueASCII(switches::kEnableUseZoomForDSF) != "false";
...
ProgressBarCompletion GetProgressBarCompletionPolicy()
{
    ...
    const base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
    std::string progress_bar_completion = command_line.GetSwitchValueASCII(switches::kProgressBarCompletion);
    ...
    if (progress_bar_completion == "resourcesBeforeDOMContentLoadedAndSameOriginIframes") {
