...
ui::PlatformCursor WebCursor::GetPlatformCursor()
{
    ...
    scale = std::min(scale, static_cast<float>(maximum_cursor_size_.height()) / bitmap.height());

    ui::ScaleAndRotateCursorBitmapAndHotpoint(scale, rotation_, &bitmap,
        &hotspot);

    platform_cursor_ = ui::CursorFactoryOzone::GetInstance()->CreateImageCursor(bitmap, hotspot);
    return platform_cursor_;
}

void WebCursor::SetDisplayInfo(const display::Display& display)
{
    if (rotation_ == display.rotation() && device_scale_factor_ == display.device_scale_factor() && maximum_cursor_size_ == display.maximum_cursor_size())
        return;
    ...
    if (maximum_cursor_size_.width() == 0 || maximum_cursor_size_.height() == 0)
        maximum_cursor_size_ = gfx::Size(kDefaultMaxCursorWidth, kDefaultMaxCursorHeight);
    if (platform_cursor_)
