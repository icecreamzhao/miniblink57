    ...
    url::Origin* p)
{
    ...
    std::string host;
    uint16_t port;
    if (!ReadParam(m, iter, &unique) || !ReadParam(m, iter, &scheme) || !ReadParam(m, iter, &host) || !ReadParam(m, iter, &port)) {
        *p = url::Origin();
        return false;
    }
    ...
    param_type* p)
{
    ...
    if (!ReadParam(m, iter, &bytes))
        return false;
    if (bytes.size() && bytes.size() != net::IPAddress::kIPv4AddressSize && bytes.size() != net::IPAddress::kIPv6AddressSize) {
        return false;
    }
