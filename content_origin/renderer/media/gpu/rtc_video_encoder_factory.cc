...
namespace {
        ...
        const media::VideoEncodeAccelerator::SupportedProfile& profile)
    {
        ...
        if (profile.profile >= media::VP8PROFILE_MIN && profile.profile <= media::VP8PROFILE_MAX) {
        ...
        } else if (profile.profile >= media::H264PROFILE_MIN && profile.profile <= media::H264PROFILE_MAX) {
            ...
            webrtc_h264_sw_enabled = base::FeatureList::IsEnabled(kWebRtcH264WithOpenH264FFmpeg);
#endif // BUILDFLAG(RTC_USE_H264) && !defined(MEDIA_DISABLE_FFMPEG)
            if (webrtc_h264_sw_enabled || base::FeatureList::IsEnabled(features::kWebRtcHWH264Encoding)) {
                codecs->push_back(cricket::WebRtcVideoEncoderFactory::VideoCodec(
...
{
    const media::VideoEncodeAccelerator::SupportedProfiles& profiles = gpu_factories_->GetVideoEncodeAcceleratorSupportedProfiles();
