...
{
    std::unique_ptr<media::Renderer> renderer = mojo_renderer_factory_->CreateRenderer(
        media_task_runner, worker_task_runner, audio_renderer_sink,
        video_renderer_sink, request_surface_cb);

    media::MojoRenderer* mojo_renderer = static_cast<media::MojoRenderer*>(renderer.release());

    media::ScopedStreamTextureWrapper stream_texture_wrapper = get_stream_texture_wrapper_cb_.Run();
