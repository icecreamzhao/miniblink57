...
TEST_F(DeviceOrientationEventPumpTest, UpdateRespectsOrientationThreshold)
{
    ...
    buffer()->data.alpha = 1 + DeviceOrientationEventPump::kOrientationThreshold / 2.0;
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        FROM_HERE, base::Bind(&DeviceOrientationEventPumpForTesting::FireEvent, base::Unretained(orientation_pump())));
    base::RunLoop().Run();
    ...
    buffer()->data.alpha = 1 + DeviceOrientationEventPump::kOrientationThreshold;
    listener()->set_did_change_device_orientation(false);
