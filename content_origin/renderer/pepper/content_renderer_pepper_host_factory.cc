...
namespace {
    ...
    {
        blink::WebPluginContainer* container = host->GetContainerForInstance(instance);
        if (!container)
            return false;

        GURL document_url = container->document().url();
        ContentRendererClient* content_renderer_client = GetContentClient()->renderer();
    ...
    const IPC::Message& message)
{
    ...
    PepperPluginInstanceImpl* instance_impl = host_->GetPluginInstanceImpl(instance);
    ...
    case PpapiHostMsg_Graphics2D_Create::ID: {
        ...
        ppapi::PPB_ImageData_Shared::ImageDataType image_type = ppapi::PPB_ImageData_Shared::PLATFORM;
#if defined(OS_WIN)
    ...
    if (message.type() == PpapiHostMsg_CameraDevice_Create::ID) {
        if (!GetPermissions().HasPermission(ppapi::PERMISSION_PRIVATE) && !CanUseCameraDeviceAPI(host_, instance))
            return nullptr;
