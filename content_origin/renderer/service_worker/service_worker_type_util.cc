...
namespace {
    ...
    public:
        explicit HeaderVisitor(ServiceWorkerHeaderMap* headers)
            : headers_(headers)
        {
        }
    ...
    {
        std::unique_ptr<ServiceWorkerHeaderMap> result = base::MakeUnique<ServiceWorkerHeaderMap>();
    ...
    {
        std::unique_ptr<ServiceWorkerHeaderList> result = base::MakeUnique<ServiceWorkerHeaderList>(web_headers.size());
    ...
    {
        std::unique_ptr<std::vector<GURL>> result = base::MakeUnique<std::vector<GURL>>(web_url_list.size());
