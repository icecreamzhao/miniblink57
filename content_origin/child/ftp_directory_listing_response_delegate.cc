    ...
    , loader_(loader)
{
    if (response.getExtraData()) {
        ...
        WebURLResponseExtraDataImpl* extra_data = static_cast<WebURLResponseExtraDataImpl*>(response.getExtraData());
...
void FtpDirectoryListingResponseDelegate::OnCompletedRequest()
{
    ...
    for (size_t i = 0; i < entries.size(); i++) {
        ...
        if (base::EqualsASCII(entry.name, ".") || base::EqualsASCII(entry.name, ".."))
            continue;
...
{
    net::UnescapeRule::Type unescape_rules = net::UnescapeRule::SPACES | net::UnescapeRule::URL_SPECIAL_CHARS_EXCEPT_PATH_SEPARATORS;
