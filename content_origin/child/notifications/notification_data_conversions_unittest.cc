...
TEST(NotificationDataConversionsTest, ToPlatformNotificationData)
{
    ...
    web_data.data = blink::WebVector<char>(kNotificationData, arraysize(kNotificationData));

    web_data.actions = blink::WebVector<blink::WebNotificationAction>(static_cast<size_t>(2));
    ...
    web_data.actions[0].placeholder = blink::WebString::fromUTF8(kAction1Placeholder);
...
TEST(NotificationDataConversionsTest, ToWebNotificationData)
{
    ...
    platform_data.actions[0].placeholder = base::NullableString16(base::ASCIIToUTF16(kAction1Placeholder), false);
...
TEST(NotificationDataConversionsTest, NotificationDataDirectionality)
{
    std::map<blink::WebNotificationData::Direction,
        PlatformNotificationData::Direction>
        mappings;

    mappings[blink::WebNotificationData::DirectionLeftToRight] = PlatformNotificationData::DIRECTION_LEFT_TO_RIGHT;
    mappings[blink::WebNotificationData::DirectionRightToLeft] = PlatformNotificationData::DIRECTION_RIGHT_TO_LEFT;
    mappings[blink::WebNotificationData::DirectionAuto] = PlatformNotificationData::DIRECTION_AUTO;
...
TEST(NotificationDataConversionsTest, TimeEdgeCaseValueBehaviour)
{
    {
        blink::WebNotificationData web_data;
        web_data.timestamp = static_cast<double>(std::numeric_limits<unsigned long long>::max());

        blink::WebNotificationData copied_data = ToWebNotificationData(ToPlatformNotificationData(web_data));
        EXPECT_NE(web_data.timestamp, copied_data.timestamp);
    }
    {
        blink::WebNotificationData web_data;
        web_data.timestamp = static_cast<double>(9211726771200000000ull); // January 1, 293878
