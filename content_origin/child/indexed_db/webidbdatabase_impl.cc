...
{
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Bind, base::Unretained(helper_), base::Passed(&database_info)));
}
...
void WebIDBDatabaseImpl::deleteObjectStore(long long transaction_id,
    long long object_store_id)
{
    io_runner_->PostTask(FROM_HERE, base::Bind(&IOThreadHelper::DeleteObjectStore, base::Unretained(helper_), transaction_id, object_store_id));
}
    ...
    blink::WebIDBTransactionMode mode)
{
    io_runner_->PostTask(
        ...
        base::Bind(&IOThreadHelper::CreateTransaction, base::Unretained(helper_),
            transaction_id, std::vector<int64_t>(object_store_ids.begin(), object_store_ids.end()),
            mode));
}
    ...
    const WebVector<int32_t>& observer_ids_to_remove)
{
    ...
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::RemoveObservers, base::Unretained(helper_), remove_observer_ids));
}
    ...
    WebIDBCallbacks* callbacks)
{
    ...
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Get, base::Unretained(helper_), transaction_id, object_store_id, index_id, IndexedDBKeyRangeBuilder::Build(key_range), key_only, base::Passed(&callbacks_impl)));
}
    ...
    const WebVector<WebIndexKeys>& index_keys)
{
    ...
    for (const WebBlobInfo& info : web_blob_info) {
        ...
        if (info.isFile()) {
            ...
            blob_info->file->last_modified = base::Time::FromDoubleT(info.lastModified());
        }
    ...
    WebIDBCallbacks* callbacks)
{
    ...
    auto callbacks_impl = base::MakeUnique<IndexedDBCallbacksImpl>(
        base::WrapUnique(callbacks), transaction_id, nullptr, io_runner_);
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Count, base::Unretained(helper_), transaction_id, object_store_id, index_id, IndexedDBKeyRangeBuilder::Build(key_range), base::Passed(&callbacks_impl)));
}
    ...
    WebIDBCallbacks* callbacks)
{
    ...
    auto callbacks_impl = base::MakeUnique<IndexedDBCallbacksImpl>(
        base::WrapUnique(callbacks), transaction_id, nullptr, io_runner_);
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Clear, base::Unretained(helper_), transaction_id, object_store_id, base::Passed(&callbacks_impl)));
}
...
{
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Abort, base::Unretained(helper_), transaction_id));
}

void WebIDBDatabaseImpl::commit(long long transaction_id)
{
    io_runner_->PostTask(
        FROM_HERE, base::Bind(&IOThreadHelper::Commit, base::Unretained(helper_), transaction_id));
}
