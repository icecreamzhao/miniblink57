...
{
    return std::tie(lhs.resource_context, lhs.http_cache, lhs.url_key) < std::tie(rhs.resource_context, rhs.http_cache, rhs.url_key);
}
    ...
    ResourceScheduler* scheduler)
{
    ...
    ResourceRequestInfoImpl* info = ResourceRequestInfoImpl::ForRequest(for_request);
    DCHECK(info);
    ...
    AsyncRevalidationKey async_revalidation_key(
        ...
        for_request->url());
    std::pair<AsyncRevalidationMap::iterator, bool> insert_result = in_progress_.insert(AsyncRevalidationMap::value_type(
    ...
    std::unique_ptr<net::URLRequest> new_request = request_context->CreateRequest(for_request->url(), net::IDLE, nullptr);
    ...
    int load_flags = for_request->load_flags() & (net::LOAD_DO_NOT_SAVE_COOKIES | net::LOAD_BYPASS_PROXY | net::LOAD_VERIFY_EV_CERT | net::LOAD_DO_NOT_SEND_COOKIES | net::LOAD_DO_NOT_SEND_AUTH_DATA | net::LOAD_MAYBE_USER_GESTURE | net::LOAD_DO_NOT_USE_EMBEDDED_IDENTITY);
    ...
    std::unique_ptr<ResourceThrottle> throttle = scheduler->ScheduleRequest(child_id, route_id, false, new_request.get());
    ...
    ResourceContext* resource_context)
{
    ...
    for (auto it = in_progress_.lower_bound(partial_key);
         it != in_progress_.end() && it->first.resource_context == resource_context;) {
...
{
    if (request.load_flags & (net::LOAD_BYPASS_CACHE | net::LOAD_DISABLE_CACHE | net::LOAD_VALIDATE_CACHE | net::LOAD_SKIP_CACHE_VALIDATION | net::LOAD_ONLY_FROM_CACHE | net::LOAD_IGNORE_LIMITS | net::LOAD_PREFETCH)) {
        return false;
    }
