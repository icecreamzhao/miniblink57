...
TEST(TemporaryFileStreamTest, Basic)
{
    ...
    int bytes_written = 0;
    scoped_refptr<net::IOBufferWithSize> buf = new net::IOBufferWithSize(kTestDataSize);
    memcpy(buf->data(), kTestData, kTestDataSize);
    scoped_refptr<net::DrainableIOBuffer> drainable = new net::DrainableIOBuffer(buf.get(), buf->size());
