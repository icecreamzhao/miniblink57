...
{
    const std::string javascript = "callAndExpectResolution({video: {mandatory: {minWidth: 320,"
...
{
    const std::string javascript = "callAndExpectResolution({video: {mandatory: {minWidth: 640,"
...
{
    const std::string javascript = "callAndDisableLocalVideo({video: true});";
    MakeTypicalPeerConnectionCall(javascript);
}
...
{
    const char* kJavascript = "callEmptyThenAddOneStreamAndRenegotiate({video: true, audio: true});";
...
{
    const char* kJavascript = "callAndRenegotiateToVideo({audio: true}, {audio: true, video:true});";
    MakeTypicalPeerConnectionCall(kJavascript);
}
