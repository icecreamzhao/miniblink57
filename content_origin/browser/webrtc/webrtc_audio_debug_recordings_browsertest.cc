    ...
    MAYBE_CallWithAudioDebugRecordings)
{
    ...
    base::FilePath input_audio_file = GetExpectedInputAudioFileName(base_file, render_process_id);
    ...
    DISABLED_TwoCallsWithAudioDebugRecordings)
{
    ...
    RenderProcessHost::iterator it = content::RenderProcessHost::AllHostsIterator();

    for (; !it.IsAtEnd(); it.Advance()) {
        base::ProcessId render_process_id = base::GetProcId(it.GetCurrentValue()->GetHandle());
        EXPECT_NE(base::kNullProcessId, render_process_id);

        // Verify that the expected AEC dump file exists and contains some data.
        base::FilePath aec_dump_file = GetExpectedAecDumpFileName(base_file, render_process_id);
