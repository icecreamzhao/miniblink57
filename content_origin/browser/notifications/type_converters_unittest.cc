...
TEST(NotificationTypeConvertersTest, TestConversion)
{
    ...
    second_action.placeholder = base::NullableString16(base::ASCIIToUTF16("Placeholder"), false);

    notification_data.actions.push_back(first_action);
    notification_data.actions.push_back(second_action);

    blink::mojom::NotificationPtr notification = blink::mojom::Notification::From(notification_data);
    PlatformNotificationData copied_notification_data = notification.To<PlatformNotificationData>();
    ...
    const PlatformNotificationAction& copied_first_action = copied_notification_data.actions[0];
    ...
    const PlatformNotificationAction& copied_second_action = copied_notification_data.actions[1];
