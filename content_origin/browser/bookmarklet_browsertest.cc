...
IN_PROC_BROWSER_TEST_F(BookmarkletTest, Redirect)
{
    ...
    NavigateToURL(shell(), GURL("javascript:location.href='data:text/plain,SUCCESS'"));
...
IN_PROC_BROWSER_TEST_F(BookmarkletTest, RedirectVoided)
{
    ...
    NavigateToURL(shell(), GURL("javascript:void(location.href='data:text/plain,SUCCESS')"));
    EXPECT_EQ("SUCCESS", GetBodyText());
}
