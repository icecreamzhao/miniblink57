    ...
    double temp_zoom_level)
{
    ...
    EXPECT_DOUBLE_EQ(host_zoom_level, host_zoom_map->GetZoomLevelForView(url, render_process_id, view_id));
    ...
    host_zoom_map->SetTemporaryZoomLevel(render_process_id, view_id,
        temp_zoom_level);
    EXPECT_DOUBLE_EQ(temp_zoom_level, host_zoom_map->GetZoomLevelForView(url, render_process_id, view_id));
...
IN_PROC_BROWSER_TEST_F(HostZoomMapImplBrowserTest, GetZoomForView_Host)
{
    ...
    HostZoomMap* host_zoom_map = HostZoomMap::GetForWebContents(shell()->web_contents());
