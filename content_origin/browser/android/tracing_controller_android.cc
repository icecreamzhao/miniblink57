...
{
    std::string categories = base::android::ConvertJavaStringToUTF8(env, jcategories);
    std::string options = base::android::ConvertJavaStringToUTF8(env, jtraceoptions);
    ...
    base::FilePath* file_path)
{
    ...
    ScopedJavaLocalRef<jstring> jfilename = Java_TracingControllerAndroid_generateTracingFilePath(env);
