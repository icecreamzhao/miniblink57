...
static void OnChildProcessStarted(JNIEnv*,
    ...
    jint handle)
{
    StartChildProcessCallback* callback = reinterpret_cast<StartChildProcessCallback*>(client_context);
    ...
    const StartChildProcessCallback& callback)
{
    ...
    for (size_t i = 0; i < file_count; ++i) {
        ...
        ScopedJavaLocalRef<jobject> j_file_info = Java_ChildProcessLauncher_makeFdInfo(env, id, fd, auto_close,
    ...
    jint secondary_id)
{
    ...
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE, base::Bind(&SetSurfacePeer, jsurface, pid, primary_id, secondary_id));
}
...
void CompleteScopedSurfaceRequest(JNIEnv* env,
    ...
    const JavaParamRef<jobject>& surface)
{
    base::UnguessableToken requestToken = base::android::UnguessableTokenAndroid::FromJavaUnguessableToken(env,
        token);
    if (!requestToken) {
...
base::android::ScopedJavaLocalRef<jobject> GetViewSurface(JNIEnv* env,
    ...
    jint surface_id)
{
    gl::ScopedJavaSurface surface_view = gpu::GpuSurfaceTracker::GetInstance()->AcquireJavaSurface(surface_id);
