...
protected:
    BrowserAccessibility* HitTestAndWaitForResult(const gfx::Point& point)
    {
        ...
        BrowserAccessibilityManager* manager = web_contents->GetRootBrowserAccessibilityManager();
        ...
        BrowserAccessibilityManager* target_manager = target_frame->browser_accessibility_manager();
        int hover_target_id = hover_waiter.event_target_id();
        BrowserAccessibility* hovered_node = target_manager->GetFromID(hover_target_id);
        return hovered_node;
    }
    ...
    BrowserAccessibility* CallCachingAsyncHitTest(const gfx::Point& point)
    {
        ...
        FrameTree* frame_tree = web_contents->GetFrameTree();
        BrowserAccessibilityManager* manager = web_contents->GetRootBrowserAccessibilityManager();
        gfx::Point screen_point = point + manager->GetViewBounds().OffsetFromOrigin();
    ...
    HitTestOutsideDocumentBoundsReturnsRoot)
{
    ...
    BrowserAccessibility* hovered_node = HitTestAndWaitForResult(gfx::Point(-1, -1));
