    ...
    const base::Callback<void(const std::string&)>& callback)
{
    ...
    ChromeBlobStorageContext* blob_storage_context = GetChromeBlobStorageContextForResourceContext(resource_context);

    std::unique_ptr<storage::BlobDataHandle> handle = blob_storage_context->context()->GetBlobDataFromPublicURL(url);
    if (!handle) {
    ...
    const std::vector<scoped_refptr<storage::BlobDataItem>>& items = data->items();
    ...
    JNIEnv* env, ScopedJavaLocalRef<jobject>& j_metadata)
{
    BrowserThread::PostTask(
        ...
        base::Bind(callback, base::TimeDelta::FromMilliseconds(Java_MediaMetadata_getDurationInMilliseconds(env, j_metadata)),
    ...
    const media::MediaResourceGetter::ExtractMediaMetadataCB& callback)
{
    ...
    ScopedJavaLocalRef<jstring> j_user_agent = ConvertUTF8ToJavaString(
        env, user_agent);
    ScopedJavaLocalRef<jobject> j_metadata = Java_MediaResourceGetter_extractMediaMetadata(
    ...
    const media::MediaResourceGetter::ExtractMediaMetadataCB& callback)
{
    ...
    ScopedJavaLocalRef<jobject> j_metadata = Java_MediaResourceGetter_extractMediaMetadataFromFd(
        env, fd, offset, size);
...
{
    DCHECK_CURRENTLY_ON(BrowserThread::IO);
    net::HttpTransactionFactory* factory = context_getter_->GetURLRequestContext()->http_transaction_factory();
    if (!factory)
        return net::AuthCredentials();

    net::HttpAuthCache* auth_cache = factory->GetSession()->http_auth_cache();
    if (!auth_cache)
        return net::AuthCredentials();

    net::HttpAuthCache::Entry* entry = auth_cache->LookupByPath(url.GetOrigin(), url.path());
    ...
    const media::MediaResourceGetter::GetCookieCB& callback)
{
    ...
    net::CookieStore* cookie_store = context_getter_->GetURLRequestContext()->cookie_store();
    if (!cookie_store) {
        callback.Run(std::string());
        return;
    }
    ...
    cookie_store->GetAllCookiesForURLAsync(
        url, base::Bind(&MediaResourceGetterTask::CheckPolicyForCookies, this, url, first_party_for_cookies, callback));
}
    ...
    const GURL& url, const GetPlatformPathCB& callback)
{
    ...
    GetPlatformPathCB cb = base::Bind(&MediaResourceGetterImpl::GetPlatformPathCallback,
        weak_factory_.GetWeakPtr(),
        callback);
