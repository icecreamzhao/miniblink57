...
{
    quota_reservation_ = file_system_context->CreateQuotaReservationOnFileTaskRunner(
    ...
    const storage::FileSystemURL& url)
{
    ...
    if (file_system_context_.get()) {
        base::File::Error error = file_system_context_->operation_runner()->SyncGetPlatformPath(
    ...
    std::unique_ptr<storage::OpenFileHandle> file_handle = quota_reservation_->GetOpenFileHandle(platform_file_path);
    std::pair<FileMap::iterator, bool> insert_result = files_.insert(std::make_pair(id, file_handle.get()));
...
{
    for (FileMap::iterator it = files_.begin(); it != files_.end(); ++it) {
        ppapi::FileGrowthMap::const_iterator growth_it = file_growths.find(it->first);
...
{
    if (file_system_context_.get() && !file_system_context_->default_file_task_runner()->RunsTasksOnCurrentThread()) {
