...
public:
        ...
        const scoped_refptr<media::VideoFrame>& frame) override
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::Bind(&VideoCaptureController::ReturnBuffer, base::Unretained(controller_), id, this, buffer_id, resource_utilization_));
    }
...
protected:
    ...
    void SetUp() override
    {
        ...
        auto consumer_feedback_observer = base::MakeUnique<MockConsumerFeedbackObserver>();
...
TEST_P(VideoCaptureControllerTest, NormalCaptureMultipleClients)
{
    ...
    session_100.requested_format = media::VideoCaptureFormat(gfx::Size(320, 240), 30, format);
    ...
    media::VideoCaptureDevice::Client::Buffer buffer = device_client_->ReserveOutputBuffer(
    ...
    media::VideoCaptureDevice::Client::Buffer buffer2 = device_client_->ReserveOutputBuffer(
        device_format.frame_size, device_format.pixel_format,
    ...
    media::VideoCaptureDevice::Client::Buffer buffer3 = device_client_->ReserveOutputBuffer(
        device_format.frame_size, device_format.pixel_format,
        device_format.pixel_storage, arbitrary_frame_feedback_id);
    ...
    media::VideoCaptureDevice::Client::Buffer buffer4 = device_client_->ReserveOutputBuffer(
        device_format.frame_size, device_format.pixel_format,
        device_format.pixel_storage, arbitrary_frame_feedback_id);
    {
...
TEST_F(VideoCaptureControllerTest, FrameFeedbackIsReportedForSequenceOfFrames)
{
    ...
    for (int frame_index = 0; frame_index < kTestFrameSequenceLength;
        ...
        const float stub_consumer_utilization = static_cast<float>(frame_index) / kTestFrameSequenceLength;
