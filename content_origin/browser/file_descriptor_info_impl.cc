    ...
    size_t i) const
{
    ...
    return (iter != ids_to_regions_.end()) ? iter->second : base::MemoryMappedFile::Region::kWholeFile;
}
...
base::ScopedFD FileDescriptorInfoImpl::ReleaseFD(base::PlatformFile file)
{
    ...
    base::ScopedFD fd;
    auto found = std::find(owned_descriptors_.begin(), owned_descriptors_.end(), file);
...
{
    std::unique_ptr<base::FileHandleMappingVector> result = base::MakeUnique<base::FileHandleMappingVector>(mapping_);
