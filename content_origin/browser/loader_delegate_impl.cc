...
namespace {
    ...
    {
        WebContentsImpl* web_contents = static_cast<WebContentsImpl*>(web_contents_getter.Run());
        if (web_contents)
    ...
    uint64_t upload_size)
{
    ...
    static_cast<WebContentsImpl*>(web_contents)->LoadStateChanged(url, load_state, upload_position, upload_size);
}
