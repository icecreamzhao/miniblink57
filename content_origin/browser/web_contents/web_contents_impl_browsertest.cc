...
#else
    static_cast<WebContentsImpl*>(shell->web_contents())->GetView()->SizeContents(size);
    ...
    MAYBE_GetSizeForNewRenderView)
{
    ...
    EXPECT_EQ(shell()->web_contents()->GetContainerBounds().size(),
        shell()->web_contents()->GetRenderWidgetHostView()->GetViewBounds().size());
...
#endif
    gfx::Size actual_size = shell()->web_contents()->GetRenderWidgetHostView()->GetViewBounds().size();
    ...
    AppendingFrameInWebUIDoesNotCrash)
{
    ...
    const char kJSCodeForAppendingFrame[] = "document.body.appendChild(document.createElement('iframe'));";
...
namespace {
    ...
    public:
        explicit WebDisplayModeDelegate(blink::WebDisplayMode mode)
            : mode_(mode)
        {
        }
...
public:
        ...
        , got_page_scale_update_(false)
    {
        ...
        ON_CALL(*this, OnPageScaleFactorChanged(::testing::_)).WillByDefault(::testing::InvokeWithoutArgs(this, &MockPageScaleObserver::GotPageScaleUpdate));
    }
...
IN_PROC_BROWSER_TEST_F(WebContentsImplBrowserTest, ViewSourceDirectNavigation)
{
    ...
    EXPECT_EQ(base::ASCIIToUTF16(std::string("view-source:") + kUrl.host() + ":" + kUrl.port() + kUrl.path()),
...
{
    const std::string kUrl = "view-source:chrome://" + std::string(kChromeUIGpuHost);
...
namespace {
    const char kDataUrlWarningPattern[] = "Upcoming versions will block content-initiated top frame navigations*";
...
{
    GURL web_ui_url(std::string(kChromeUIScheme) + "://" + std::string(kChromeUIGpuHost));
    ASSERT_TRUE(embedded_test_server()->Start());
    WebContents* base_web_contents = shell()->web_contents();
    ASSERT_TRUE(base_web_contents);
...
namespace {
    ...
    public:
        MOCK_METHOD5(OnFinishDownloadImage, void(int id, int status_code, const GURL& image_url, const std::vector<SkBitmap>& bitmap, const std::vector<gfx::Size>& sizes));
