...
bool ShellCrashReporterClient::GetCrashDumpLocation(base::FilePath* crash_dir)
{
    ...
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(
        ...
        return false;
    base::FilePath crash_directory = base::CommandLine::ForCurrentProcess()->GetSwitchValuePath(
        switches::kCrashDumpsDir);
#if defined(OS_WIN)
...
{
    return process_type == switches::kRendererProcess || process_type == switches::kPpapiPluginProcess || process_type == switches::kZygoteProcess || process_type == switches::kGpuProcess;
}
