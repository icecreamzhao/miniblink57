...
void LayoutTestPushMessagingService::SubscribeFromWorker(
    ...
    const PushMessagingService::RegisterCallback& callback)
{
    if (GetPermissionStatus(requesting_origin, options.user_visible_only) == blink::WebPushPermissionStatusGranted) {
    ...
    const UnregisterCallback& callback)
{
    ClearPushSubscriptionId(
        ...
        base::Bind(callback,
            service_worker_registration_id == subscribed_service_worker_registration_
                ? PUSH_UNREGISTRATION_STATUS_SUCCESS_UNREGISTERED
                : PUSH_UNREGISTRATION_STATUS_SUCCESS_WAS_NOT_REGISTERED));
    if (service_worker_registration_id == subscribed_service_worker_registration_) {
