...
_MD_Solaris_GetInterval(void);
#define _MD_GET_INTERVAL _MD_Solaris_GetInterval
NSPR_API(PRIntervalTime)
_MD_Solaris_TicksPerSecond(void);
...
struct _MDLock;
NSPR_API(void)
_MD_lock(struct _MDLock* md_lock);
#undef PROFILE_LOCKS
...
#define _PR_LOCK_HEAP() \
    if (_pr_heapLock)   \
    _MD_LOCK(&_pr_heapLock->md)
#define _PR_UNLOCK_HEAP() \
    if (_pr_heapLock)     \
    _MD_UNLOCK(&_pr_heapLock->md)
