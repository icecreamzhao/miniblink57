...
#define LL_INIT(hi, lo)            \
    {                              \
        PR_INT32(lo), PR_INT32(hi) \
    }
#else
#define LL_INIT(hi, lo)            \
    {                              \
        PR_INT32(hi), PR_INT32(lo) \
    }
#endif
...
#define LL_CMP(a, op, b) (((a).hi == (b).hi) ? ((a).lo op(b).lo) : ((PRInt32)(a).hi op(PRInt32)(b).hi))
#define LL_UCMP(a, op, b) (((a).hi == (b).hi) ? ((a).lo op(b).lo) : ((a).hi op(b).hi))
