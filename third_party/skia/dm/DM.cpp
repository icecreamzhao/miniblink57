...
static void push_src(const char* tag, ImplicitString options, Src* s)
{
    ...
    if (in_shard() && FLAGS_src.contains(tag) && !SkCommandLineFlags::ShouldSkip(FLAGS_match, src->name().c_str())) {
...
static void push_codec_srcs(Path path)
{
    ...
    for (CodecSrc::Mode mode : nativeModes) {
        for (CodecSrc::DstColorType colorType : colorTypes) {
            for (SkAlphaType alphaType : alphaModes) {
                ...
                if (CodecSrc::kCroppedScanline_Mode == mode && kPremul_SkAlphaType != alphaType) {
                    continue;
                }
                ...
                // different.
                if (supportsNativeScaling && CodecSrc::kNonNative8888_Always_DstColorType == colorType) {
                    // Native Scales
    ...
    for (SkAlphaType alphaType : alphaModes) {
        ...
        if (kWEBP_SkEncodedFormat != codec->getEncodedFormat() && kWBMP_SkEncodedFormat != codec->getEncodedFormat() && kUnpremul_SkAlphaType != alphaType) {
            push_image_gen_src(path, ImageGenSrc::kPlatform_Mode, alphaType, false);
        }
#elif defined(SK_BUILD_FOR_WIN)
        if (kWEBP_SkEncodedFormat != codec->getEncodedFormat() && kWBMP_SkEncodedFormat != codec->getEncodedFormat()) {
            push_image_gen_src(path, ImageGenSrc::kPlatform_Mode, alphaType, false);
        }
#endif
    }
...
static Sink* create_sink(const SkCommandLineConfig* config)
{
    ...
    if (gpu_supported()) {
        if (const SkCommandLineConfigGpu* gpuConfig = config->asConfigGpu()) {
            ...
            GrContextFactory::ContextOptions contextOptions = GrContextFactory::kNone_ContextOptions;
...
#define SINK(t, sink, ...)                \
    if (config->getBackend().equals(t)) { \
        return new sink(__VA_ARGS__);     \
    }
    ...
    const char* srcOptions, const char* name)
{
    for (int i = 0; i < FLAGS_blacklist.count() - 3; i += 4) {
        if (match(FLAGS_blacklist[i + 0], sink) && match(FLAGS_blacklist[i + 1], src) && match(FLAGS_blacklist[i + 2], srcOptions) && match(FLAGS_blacklist[i + 3], name)) {
            return true;
        }
    }
    return false;
...
struct Task {
    ...
    static void Run(const Task& task)
    {
        ...
        if (!FLAGS_dryRun) {
            ...
            if (!log.isEmpty()) {
                info("%s %s %s %s:\n%s\n", task.sink.tag.c_str(), task.src.tag.c_str(), task.src.options.c_str(), name.c_str(), log.c_str());
            }
            if (!err.isEmpty()) {
                if (err.isFatal()) {
            ...
            gDefinitelyThreadSafeWork.add([task, name, bitmap, data] {
                ...
                if (!FLAGS_readPath.isEmpty() && !gGold.contains(Gold(task.sink.tag, task.src.tag, task.src.options, name, md5))) {
...
int dm_main()
{
    ...
    for (auto& sink : gSinks)
        for (auto& src : gSrcs) {
            if (src->veto(sink->flags()) || is_blacklisted(sink.tag.c_str(), src.tag.c_str(), src.options.c_str(), src->name().c_str())) {
                SkAutoMutexAcquire lock(gMutex);
                gPending--;
                continue;
            }
    ...
    for (auto task : serial) {
        Task::Run(task);
    }
    for (auto test : gSerialTests) {
        run_test(test);
    }
    ...
    Reporter* reporter, GrContextFactory* factory)
{
    ...
    for (int typeInt = 0; typeInt < GrContextFactory::kContextTypeCnt; ++typeInt) {
        ...
        if (contextType == GrContextFactory::kGL_ContextType || contextType == GrContextFactory::kGLES_ContextType) {
