...
void Viewer::drawSlide(SkCanvas* canvas, bool inSplitScreen)
{
    ...
    if (fWindow->supportsContentRect()) {
        ...
        contentRect.fLeft += inSplitScreen ? (contentRect.fRight - contentRect.fLeft) * 0.5f : 0.0f;
...
void Viewer::updateUIState()
{
    ...
    double measurement = fMeasurements[(fCurrentMeasurement + (kMeasurementCount - 1)) % kMeasurementCount];
