...
void VulkanWindowContext::initializeContext(void* platformData, const DisplayParams& params)
{
    ...
    if (!(fBackendContext->fExtensions & kKHR_surface_GrVkExtensionFlag) || !(fBackendContext->fExtensions & kKHR_swapchain_GrVkExtensionFlag)) {
    ...
    const DisplayParams& params)
{
    ...
    SkASSERT(caps.supportedCompositeAlpha & (VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR | VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR));
    ...
    for (uint32_t i = 0; i < surfaceFormatCount; ++i) {
        ...
        if (GrVkFormatToPixelConfig(surfaceFormats[i].format, &config) && GrPixelConfigIsSRGB(config) == wantSRGB) {
...
void VulkanWindowContext::destroyContext()
{
    ...
    if (VK_NULL_HANDLE != fCommandPool) {
        GR_VK_CALL(fBackendContext->fInterface, DestroyCommandPool(fBackendContext->fDevice, fCommandPool, nullptr));
        fCommandPool = VK_NULL_HANDLE;
    }
...
sk_sp<SkSurface> VulkanWindowContext::getBackbufferSurface()
{
    ...
    VkPipelineStageFlags srcStageMask = (VK_IMAGE_LAYOUT_UNDEFINED == layout) ? VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT : VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    VkPipelineStageFlags dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    VkAccessFlags srcAccessMask = (VK_IMAGE_LAYOUT_UNDEFINED == layout) ? 0 : VK_ACCESS_MEMORY_READ_BIT;
