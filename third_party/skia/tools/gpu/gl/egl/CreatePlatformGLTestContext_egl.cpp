...
GrGLuint EGLGLTestContext::eglImageToExternalTexture(GrEGLImage image) const
{
    ...
    EGLImageTargetTexture2DProc glEGLImageTargetTexture2D = (EGLImageTargetTexture2DProc)eglGetProcAddress("glEGLImageTargetTexture2DOES");
...
static bool supports_egl_extension(EGLDisplay display, const char* extension)
{
    ...
    while (const char* match = strstr(extensionsStr, extension)) {
        ...
        if ((match == extensionsStr || match[-1] == ' ') && (match[extensionLength] == ' ' || match[extensionLength] == '\0')) {
            return true;
        }
...
bool EGLFenceSync::waitFence(SkPlatformGpuFence platformFence) const
{
    ...
    return EGL_CONDITION_SATISFIED_KHR == eglClientWaitSyncKHR(fDisplay, eglsync, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, EGL_FOREVER_KHR);
}
