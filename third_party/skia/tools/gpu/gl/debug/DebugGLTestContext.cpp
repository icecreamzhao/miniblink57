...
public:
        ...
        , fAbandoned(false)
    {
        for (int i = 0; i < kDefaultMaxTextureUnits; ++i) {
            fTextureUnits[i] = reinterpret_cast<GrTextureUnitObj*>(this->createObj(kTextureUnit_ObjTypes));
            fTextureUnits[i]->ref();
    ...
    {
        GrAlwaysAssert(target == GR_GL_TEXTURE_2D || target == GR_GL_TEXTURE_RECTANGLE || target == GR_GL_TEXTURE_EXTERNAL);
        ...
        GrGLvoid* pixels) override
    {
        ...
        if (componentSize >= alignment) {
            ...
            float fTemp = sk_float_ceil(componentSize * componentsPerPixel * pixelsInRow / static_cast<float>(alignment));
    ...
    GrGLvoid bindFramebuffer(GrGLenum target, GrGLuint frameBufferID) override
    {
        ...
        GrAlwaysAssert(GR_GL_FRAMEBUFFER == target || GR_GL_READ_FRAMEBUFFER == target || GR_GL_DRAW_FRAMEBUFFER);
    ...
    GrGLvoid deleteTextures(GrGLsizei n, const GrGLuint* textures) override
    {
        ...
        if (this->getFrameBuffer()) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (frameBuffer->getColor() && textures[i] == frameBuffer->getColor()->getID()) {
                    frameBuffer->setColor(nullptr);
                }
                if (frameBuffer->getDepth() && textures[i] == frameBuffer->getDepth()->getID()) {
                    frameBuffer->setDepth(nullptr);
                }
                if (frameBuffer->getStencil() && textures[i] == frameBuffer->getStencil()->getID()) {
    ...
    GrGLvoid deleteFramebuffers(GrGLsizei n, const GrGLuint* frameBuffers) override
    {
        ...
        if (this->getFrameBuffer()) {
            for (int i = 0; i < n; ++i) {
                ...
                if (frameBuffers[i] == this->getFrameBuffer()->getID()) {
    ...
    GrGLvoid deleteRenderbuffers(GrGLsizei n, const GrGLuint* renderBuffers) override
    {
        ...
        if (this->getRenderBuffer()) {
            for (int i = 0; i < n; ++i) {
                ...
                if (renderBuffers[i] == this->getRenderBuffer()->getID()) {
        ...
        if (this->getFrameBuffer()) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (frameBuffer->getColor() && renderBuffers[i] == frameBuffer->getColor()->getID()) {
                    frameBuffer->setColor(nullptr);
                }
                if (frameBuffer->getDepth() && renderBuffers[i] == frameBuffer->getDepth()->getID()) {
                    frameBuffer->setDepth(nullptr);
                }
                if (frameBuffer->getStencil() && renderBuffers[i] == frameBuffer->getStencil()->getID()) {
                    frameBuffer->setStencil(nullptr);
                }
            }
        }
        ...
        GrGLuint renderBufferID) override
    {
        ...
        GrAlwaysAssert(GR_GL_COLOR_ATTACHMENT0 == attachment || GR_GL_DEPTH_ATTACHMENT == attachment || GR_GL_STENCIL_ATTACHMENT == attachment);
