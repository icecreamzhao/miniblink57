...
static bool can_interchange_winding_and_even_odd_fill(const GrShape& shape)
{
    ...
    return strokeRecStyle == SkStrokeRec::kStroke_Style || strokeRecStyle == SkStrokeRec::kHairline_Style || (shape.style().isSimpleFill() && path.isConvex());
}
    ...
    const Key& keyA, const Key& keyB)
{
    ...
    if (SkPath::ConvertToNonInverseFillType(pathA.getFillType()) != SkPath::ConvertToNonInverseFillType(pathB.getFillType())) {
    ...
    if (a.style().hasNonDashPathEffect() == b.style().hasNonDashPathEffect() && a.style().isDashed() == b.style().isDashed()) {
        REPORTER_ASSERT(r, a.mayBeInverseFilledAfterStyling() == b.mayBeInverseFilledAfterStyling());
    }
...
static void test_basic(skiatest::Reporter* reporter, const GEO& geo)
{
    ...
    TestCase(geo, fill, reporter).compare(reporter, fillCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    TestCase(geo, stroke2RoundBevel, reporter).compare(reporter, stroke2RoundBevelCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    TestCase(geo, stroke2RoundBevelDash, reporter).compare(reporter, stroke2RoundBevelDashCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    TestCase(geo, stroke2RoundBevelAndFill, reporter).compare(reporter, stroke2RoundBevelAndFillCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    TestCase(geo, stroke2RoundBevelAndFillDash, reporter).compare(reporter, stroke2RoundBevelAndFillDashCase, TestCase::kAllSame_ComparisonExpecation);
...
static void test_scale(skiatest::Reporter* reporter, const GEO& geo)
{
    ...
    if (strokeAndFillCase1.appliedFullStyleShape().asRRect(&rrect, nullptr, nullptr, nullptr)) {
        ...
        REPORTER_ASSERT(reporter,
            strokeAndFillCase1.baseShape().asRRect(&rrect, nullptr, nullptr, nullptr) && rrect.isRect());
...
void test_path_effect_makes_rrect(skiatest::Reporter* reporter, const GEO& geo)
{
    ...
    REPORTER_ASSERT(reporter, geoPECase.appliedPathEffectShape().asRRect(&rrect, nullptr, nullptr, nullptr));
    ...
    REPORTER_ASSERT(reporter, geoPECase.appliedFullStyleShape().asRRect(&rrect, nullptr, nullptr, nullptr));
    REPORTER_ASSERT(reporter, rrect == RRectPathEffect::RRect());
    ...
    REPORTER_ASSERT(reporter, geoPEStrokeCase.appliedPathEffectShape().asRRect(&rrect, nullptr, nullptr, nullptr));
    REPORTER_ASSERT(reporter, rrect == RRectPathEffect::RRect());
    REPORTER_ASSERT(reporter, geoPEStrokeCase.appliedPathEffectKey() == rrectFillCase.baseKey());

    REPORTER_ASSERT(reporter, !geoPEStrokeCase.appliedFullStyleShape().asRRect(&rrect, nullptr, nullptr, nullptr));
    REPORTER_ASSERT(reporter, geoPEStrokeCase.appliedFullStyleKey() == rrectStrokeCase.appliedFullStyleKey());
}
...
void test_rrect(skiatest::Reporter* r, const SkRRect& rrect)
{
    ...
    auto index = [](bool inverted,
                     ...
                     Style style,
                     bool dash) -> int {
        return inverted * (2 * 8 * kStyleCnt * 2) + dir * (8 * kStyleCnt * 2) + start * (kStyleCnt * 2) + style * (2) + dash;
    };
            ...
            for (unsigned start = 0; start < 8; ++start) {
                for (Style style : { kFill, kStroke, kHairline, kStrokeAndFill }) {
                    for (bool dash : { false, true }) {
                        ...
                        shapes[index(inverted, dir, start, style, dash)] = GrShape(rrect, dir, start, SkToBool(inverted),
    ...
    REPORTER_ASSERT(r, exampleInvFillCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, queryInverted);

    REPORTER_ASSERT(r, exampleStrokeAndFillCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, !queryInverted);

    REPORTER_ASSERT(r, exampleInvStrokeAndFillCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, queryInverted);

    REPORTER_ASSERT(r, exampleHairlineCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, !queryInverted);

    REPORTER_ASSERT(r, exampleInvHairlineCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, queryInverted);
    ...
    REPORTER_ASSERT(r, exampleInvStrokeCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
    REPORTER_ASSERT(r, queryRR == rrect);
    REPORTER_ASSERT(r, SkPath::kCW_Direction == queryDir);
    REPORTER_ASSERT(r, 0 == queryStart);
    REPORTER_ASSERT(r, queryInverted);
            ...
            for (unsigned start = 0; start < 8; ++start) {
                for (bool dash : { false, true }) {
                    ...
                    if (dash) {
                        ...
                        REPORTER_ASSERT(r, strokeCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
                        REPORTER_ASSERT(r, queryRR == rrect);
                        REPORTER_ASSERT(r, queryDir == dir);
                        REPORTER_ASSERT(r, queryStart == expectedStart);
                        REPORTER_ASSERT(r, !queryInverted);
                        REPORTER_ASSERT(r, hairlineCase.asRRect(&queryRR, &queryDir, &queryStart, &queryInverted));
                        REPORTER_ASSERT(r, queryRR == rrect);
                        REPORTER_ASSERT(r, queryDir == dir);
                        REPORTER_ASSERT(r, queryStart == expectedStart);
                        REPORTER_ASSERT(r, !queryInverted);
...
void test_lines(skiatest::Reporter* r)
{
    ...
    strokeAB.compare(r, canonicalizeAsAB ? dashAB : dashBA,
        TestCase::kSameUpToPE_ComparisonExpecation);
    REPORTER_ASSERT(r, strokeAB.baseShape().asLine(pts, &inverted) && !inverted && pts[0] == canonicalPts[0] && pts[1] == canonicalPts[1]);
    REPORTER_ASSERT(r, hairlineAB.baseShape().asLine(pts, &inverted) && !inverted && pts[0] == canonicalPts[0] && pts[1] == canonicalPts[1]);
    REPORTER_ASSERT(r, dashAB.baseShape().asLine(pts, &inverted) && !inverted && pts[0] == kA && pts[1] == kB);
    REPORTER_ASSERT(r, dashBA.baseShape().asLine(pts, &inverted) && !inverted && pts[0] == kB && pts[1] == kA);
    ...
    REPORTER_ASSERT(r, strokeInvAB.baseShape().asLine(pts, &inverted) && inverted && pts[0] == canonicalPts[0] && pts[1] == canonicalPts[1]);
    REPORTER_ASSERT(r, hairlineInvAB.baseShape().asLine(pts, &inverted) && inverted && pts[0] == canonicalPts[0] && pts[1] == canonicalPts[1]);
    // Dashing ignores inverse.
    REPORTER_ASSERT(r, dashInvAB.baseShape().asLine(pts, &inverted) && !inverted && pts[0] == kA && pts[1] == kB);
}
...
DEF_TEST(GrShape, reporter)
{
    ...
    for (auto testPath : paths) {
        for (bool inverseFill : { false, true }) {
            ...
            test_make_hairline_path_effect(reporter, path, testPath.fIsRRectForStroke || testPath.fIsLine);
        }
    }
    ...
    for (auto testPath : paths) {
        ...
        SkRRect rrect;
        REPORTER_ASSERT(reporter, testPath.fIsRRectForFill == fillPathCase.baseShape().asRRect(&rrect, nullptr, nullptr, nullptr));
        ...
        REPORTER_ASSERT(reporter, testPath.fIsRRectForStroke == strokePathCase.baseShape().asRRect(&rrect, nullptr, nullptr, nullptr));
