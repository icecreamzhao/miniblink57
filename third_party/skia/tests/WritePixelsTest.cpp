...
static bool check_pixel(SkPMColor a, SkPMColor b, bool didPremulConversion)
{
    ...
    return aA == bA && SkAbs32(aR - bR) <= 1 && SkAbs32(aG - bG) <= 1 && SkAbs32(aB - bB) <= 1;
}
    ...
    int writeX, int writeY)
{
    ...
    if (canvasInfo.width() != DEV_W || canvasInfo.height() != DEV_H || canvasInfo.colorType() != kN32_SkColorType) {
        return false;
    }
            ...
            if (writeRect.contains(cx, cy)) {
                ...
                if (!check_pixel(bmpPMColor, canvasPixel, mul)) {
                    ERRORF(reporter, "Expected canvas pixel at %d, %d to be 0x%08x, got 0x%08x. "
                                     "Write performed premul: %d",
                        cx, cy, bmpPMColor, canvasPixel, mul);
                    return false;
                }
            } else {
                ...
                if (canvasPixel != testColor) {
                    ERRORF(reporter, "Canvas pixel outside write rect at %d, %d changed."
                                     " Should be 0x%08x, got 0x%08x. ",
                        cx, cy, testColor, canvasPixel);
                    return false;
                }
            }
        }
    ...
    for (size_t r = 0; r < SK_ARRAY_COUNT(testRects); ++r) {
        ...
        for (int tightBmp = 0; tightBmp < 2; ++tightBmp) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gSrcConfigs); ++c) {
                ...
                fill_canvas(&canvas);
                SkBitmap bmp;
                REPORTER_ASSERT(reporter, setup_bitmap(&bmp, ct, at, rect.width(), rect.height(), SkToBool(tightBmp)));
                ...
                REPORTER_ASSERT(reporter, check_write(reporter, &canvas, bmp, rect.fLeft, rect.fTop));
