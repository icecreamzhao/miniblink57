    ...
    float error)
{
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        if (outputComponent < SkScalarFloorToInt(lower * 255.f) || outputComponent > SkScalarCeilToInt(upper * 255.f)) {
            return false;
        }
    }
    return true;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SRGBReadWritePixels, reporter, ctxInfo)
{
    ...
    if (context->caps()->isConfigRenderable(desc.fConfig, false) && context->caps()->isConfigTexturable(desc.fConfig)) {
