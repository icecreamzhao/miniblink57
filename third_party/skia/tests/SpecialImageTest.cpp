    ...
    int offset, int size)
{
    ...
    REPORTER_ASSERT(reporter, SK_ColorRED == bm.getColor(kSmallerSize + kPad - 1, kSmallerSize + kPad - 1));
    REPORTER_ASSERT(reporter, SK_ColorBLUE == bm.getColor(kSmallerSize + kPad, kSmallerSize + kPad));
    ...
    }
    {
        ...
        REPORTER_ASSERT(reporter, peekTextureSucceeds == !!tightSurf->getTextureHandle(SkSurface::kDiscardWrite_BackendHandleAccess));
        SkPixmap tmpPixmap;
    ...
    const sk_sp<SkSpecialImage>& gpuBacked)
{
    ...
    REPORTER_ASSERT(reporter, gpuBacked->subset().width() == orig->subset().width() && gpuBacked->subset().height() == orig->subset().height());
}
