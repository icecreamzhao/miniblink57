...
static void test_xform(skiatest::Reporter* r, const sk_sp<SkGammas>& gammas)
{
    ...
    std::unique_ptr<SkColorSpaceXform> xform = ColorSpaceXformTest::CreateDefaultXform(gammas, srcToDst, gammas);
    ...
    for (int i = 0; i < width; i++) {
        REPORTER_ASSERT(r, almost_equal(((srcPixels[i] >> 0) & 0xFF), SkGetPackedR32(dstPixels[i])));
        REPORTER_ASSERT(r, almost_equal(((srcPixels[i] >> 8) & 0xFF), SkGetPackedG32(dstPixels[i])));
        REPORTER_ASSERT(r, almost_equal(((srcPixels[i] >> 16) & 0xFF), SkGetPackedB32(dstPixels[i])));
        REPORTER_ASSERT(r, almost_equal(((srcPixels[i] >> 24) & 0xFF), SkGetPackedA32(dstPixels[i])));
    }
}
