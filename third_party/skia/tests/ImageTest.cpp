...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkImage_newTextureImage, reporter, contextInfo)
{
    ...
    GrContextFactory::ContextType otherContextType = GrContextFactory::NativeContextTypeForBackend(testContext->backend());
...
public:
    EmptyGenerator()
        : SkImageGenerator(SkImageInfo::MakeN32Premul(0, 0))
    {
    }
};
...
static void test_read_pixels(skiatest::Reporter* reporter, SkImage* image)
{
    ...
    sk_memset32(pixels, notExpected, w * h);
    REPORTER_ASSERT(reporter, image->readPixels(info, pixels, rowBytes, image->width() - w, image->height() - h));
    REPORTER_ASSERT(reporter, has_pixels(pixels, w * h, expected));
    ...
    sk_memset32(pixels, notExpected, w * h);
    REPORTER_ASSERT(reporter, image->readPixels(info, pixels, rowBytes, image->width() - 1, image->height() - 1));
...
static void test_legacy_bitmap(skiatest::Reporter* reporter, const SkImage* image, SkImage::LegacyBitmapMode mode)
{
    ...
    SkBitmap tmp;
    REPORTER_ASSERT(reporter, bitmap.extractSubset(&tmp, SkIRect::MakeWH(image->width() / 2, image->height() / 2)));
...
static void test_peek(skiatest::Reporter* reporter, SkImage* image, bool expectPeekSuccess)
{
    ...
    if (success) {
        ...
        REPORTER_ASSERT(reporter, kPremul_SkAlphaType == info.alphaType() || kOpaque_SkAlphaType == info.alphaType());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DeferredTextureImage, reporter, ctxInfo)
{
    ...
    GrContextFactory otherFactory;
    ContextInfo otherContextInfo = otherFactory.getContextInfo(GrContextFactory::kNativeGL_ContextType);
    ...
    struct {
        ...
        { create_image_large, { { SkMatrix::I(), kNone_SkFilterQuality, 4 }, { SkMatrix::I(), kMedium_SkFilterQuality, 4 } },
            kMedium_SkFilterQuality, 16, true },
        // Create an image with multiple prescale levels, make sure we chose the minimum scale.
        { create_image_large, { { SkMatrix::I(), kMedium_SkFilterQuality, 5 }, { SkMatrix::I(), kMedium_SkFilterQuality, 4 } },
            kMedium_SkFilterQuality, 16, true },
    };
