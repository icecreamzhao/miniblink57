...
{
    for (int i = 0; i < 4; i++) {
        if (SkScalarAbs(c0[i].fX - c1[i].fX) > tol || SkScalarAbs(c0[i].fY - c1[i].fY) > tol) {
            PrintCurve("c0", c0);
            PrintCurve("c1", c1);
            return false;
        }
...
DEF_TEST(ClipCubic, reporter)
{
    ...
    REPORTER_ASSERT(reporter, CurvesAreEqual(clipped, SetCurve(0, 0, 2, 3, 1, 10, 4, 12, shouldbe), tol));
    ...
    success = clipper.clipCubic(crv, clipped);
    REPORTER_ASSERT(reporter, success == true);
    REPORTER_ASSERT(reporter, CurvesAreEqual(clipped, SetCurve(0.5126125216f, 1, 1.841195941f, 4.337081432f, 1.297019958f, 10.19801331f, 4, 12, shouldbe), tol));
    ...
    success = clipper.clipCubic(crv, clipped);
    REPORTER_ASSERT(reporter, success == true);
    REPORTER_ASSERT(reporter, CurvesAreEqual(clipped, SetCurve(00.8412352204f, 2, 1.767683744f, 5.400758266f, 1.55052948f, 10.36701965f, 4, 12, shouldbe), tol));
    ...
    success = clipper.clipCubic(crv, clipped);
    REPORTER_ASSERT(reporter, success == true);
    REPORTER_ASSERT(reporter, CurvesAreEqual(clipped, SetCurve(0, 0, 1.742904663f, 2.614356995f, 1.207521796f, 8.266430855f, 3.026495695f, 11, shouldbe), tol));
    ...
    success = clipper.clipCubic(crv, clipped);
    REPORTER_ASSERT(reporter, success == true);
    REPORTER_ASSERT(reporter, CurvesAreEqual(clipped, SetCurve(0, 0, 1.551193237f, 2.326789856f, 1.297736168f, 7.059780121f, 2.505550385f, 10, shouldbe), tol));
