    ...
    bool renderTarget, bool linearTiling)
{
    ...
    if (linearTiling) {
        if (!caps->isConfigTexurableLinearly(config) || (renderTarget && !caps->isConfigRenderableLinearly(config, false))) {
            canCreate = false;
        }
    }
    ...
    if (tex0) {
        ...
        REPORTER_ASSERT(reporter, does_full_buffer_contain_correct_color(srcBuffer, dstBuffer, config, kWidth, kHeight));
        ...
        REPORTER_ASSERT(reporter, does_full_buffer_contain_correct_color(srcBuffer, dstBuffer, config, 10, 2));
    ...
    if (tex1) {
        ...
        REPORTER_ASSERT(reporter, does_full_buffer_contain_correct_color(srcBuffer, dstBuffer, config, 4, 5));
