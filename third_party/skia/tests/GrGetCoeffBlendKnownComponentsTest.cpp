...
DEF_TEST(GrGetCoeffBlendKnownComponents, reporter)
{
    ...
    for (int i = 0; i < SkXfermode::kLastCoeffMode; ++i) {
        ...
        for (int j = 0; j < 1000; ++j) {
            ...
            if (SkAbs32(GrColorUnpackA(baselineColor) - GrColorUnpackA(outColor)) > 1 || SkAbs32(GrColorUnpackR(baselineColor) - GrColorUnpackR(outColor)) > 1 || SkAbs32(GrColorUnpackG(baselineColor) - GrColorUnpackG(outColor)) > 1 || SkAbs32(GrColorUnpackB(baselineColor) - GrColorUnpackB(outColor)) > 1) {
    ...
    GrGetCoeffBlendKnownComponents(kZero_GrBlendCoeff, kZero_GrBlendCoeff,
        ...
        0xFFFFFFFF, kNone_GrColorComponentFlags,
        &outColor, &outFlags);
    REPORTER_ASSERT(reporter, GrColor_TRANSPARENT_BLACK == outColor && kRGBA_GrColorComponentFlags == outFlags);
    ...
    REPORTER_ASSERT(reporter,
        (kG_GrColorComponentFlag | kB_GrColorComponentFlag | kA_GrColorComponentFlag) == outFlags && (outColor & 0xFFFFFF00) == 0xFF808000);
}
