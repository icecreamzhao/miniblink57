    ...
    const SkMD5::Digest* goodDigest)
{
...

    {
        ...
        if (info.alphaType() == kOpaque_SkAlphaType) {
            ...
            REPORTER_ASSERT(r, expectedResult == codec->getPixels(info565, bm565.getPixels(), bm565.rowBytes()));
    ...
    if (codec->getInfo().colorType() == kGray_8_SkColorType) {
        ...
        REPORTER_ASSERT(r, expectedResult == codec->getPixels(grayInfo, grayBm.getPixels(), grayBm.rowBytes()));
    ...
    bool supportsIncomplete = true)
{
    ...
    if (supportsScanlineDecoding) {
        ...
        REPORTER_ASSERT(r, codec->getPixels(bm.info(), bm.getPixels(), bm.rowBytes()) == expectedResult);
        REPORTER_ASSERT(r, codec->getScanlines(bm.getAddr(0, 0), 1, 0) == 0);
        REPORTER_ASSERT(r, codec->skipScanlines(1) == 0);
...
static void test_invalid_stream(skiatest::Reporter* r, const void* stream, size_t len)
{
    ...
    SkAndroidCodec* androidCodec = SkAndroidCodec::NewFromStream(new SkMemoryStream(stream, len, false));
...
static void test_dimensions(skiatest::Reporter* r, const char path[])
{
    ...
    for (int sampleSize = 1; sampleSize < 32; sampleSize++) {
        ...
        SkCodec::Result result = codec->getAndroidPixels(scaledInfo, pixels.get(), rowBytes, &options);
        ...
        bool readChunk(const char tag[], const void* data, size_t length) override
        {
            for (size_t i = 0; i < SK_ARRAY_COUNT(gUnknowns); ++i) {
                if (!strcmp(tag, (const char*)gUnknowns[i].name)) {
                    ...
                    REPORTER_ASSERT(fReporter, !strcmp((const char*)data, (const char*)gUnknowns[i].data));
                    return true;
                }
            }
