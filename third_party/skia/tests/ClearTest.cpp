...
{
    SkDEBUGCODE(uint32_t oldID = 0;) if (*dc)
    {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ClearBatch, reporter, ctxInfo)
{
    ...
    if (!check_rect(drawContext.get(), outerLeftEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerTopEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerRightEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerBottomEdge, kColor1, &actualValue, &failX, &failY)) {
        ERRORF(reporter, "Expected 0x%08x but got 0x%08x at (%d, %d).", kColor1, actualValue,
            failX, failY);
    }
    ...
    if (!check_rect(drawContext.get(), innerLeftEdge, kColor2, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), innerTopEdge, kColor2, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), innerRightEdge, kColor2, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), innerBottomEdge, kColor2, &actualValue, &failX, &failY)) {
        ERRORF(reporter, "Expected 0x%08x but got 0x%08x at (%d, %d).", kColor2, actualValue,
            failX, failY);
    }
    if (!check_rect(drawContext.get(), outerLeftEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerTopEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerRightEdge, kColor1, &actualValue, &failX, &failY) || !check_rect(drawContext.get(), outerBottomEdge, kColor1, &actualValue, &failX, &failY)) {
