...
bool SkBmpCodec::ReadHeader(SkStream* stream, bool inIco, SkCodec** codecOut)
{
    ...
    if (!inIco) {
        ...
        if (stream->read(hBuffer.get(), kBmpHeaderBytesPlusFour) != kBmpHeaderBytesPlusFour) {
    ...
    case kBitMasks_BmpCompressionMethod:
        ...
        case kInfoV1_BmpHeaderType: {
            ...
            if (stream->read(mBuffer.get(), kBmpMaskBytes) != kBmpMaskBytes) {
        ...
        case kInfoV5_BmpHeaderType:
            ...
            if (kInfoV2_BmpHeaderType == headerType || (kInfoV3_BmpHeaderType == headerType && !inIco)) {
                break;
            }
    ...
    case kStandard_BmpInputFormat: {
        ...
        SkEncodedInfo::Alpha alpha = inIco ? SkEncodedInfo::kBinary_Alpha : SkEncodedInfo::kOpaque_Alpha;
        bool isOpaque = true;
