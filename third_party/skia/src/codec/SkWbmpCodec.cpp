    ...
    int* rowsDecoded)
{
    ...
    if (!valid_color_type(info.colorType()) || !valid_alpha(info.alphaType(), this->getInfo().alphaType())) {
    ...
    const Options& options, SkPMColor inputColorTable[], int* inputColorCount)
{
    ...
    if (!valid_color_type(dstInfo.colorType()) || !valid_alpha(dstInfo.alphaType(), this->getInfo().alphaType())) {
        return kInvalidConversion;
    }
