...
bool SkGifCodec::IsGif(const void* buf, size_t bytesRead)
{
    if (bytesRead >= GIF_STAMP_LEN) {
        if (memcmp(GIF_STAMP, buf, GIF_STAMP_LEN) == 0 || memcmp(GIF87_STAMP, buf, GIF_STAMP_LEN) == 0 || memcmp(GIF89_STAMP, buf, GIF_STAMP_LEN) == 0) {
            return true;
        }
    }
    return false;
...
bool SkGifCodec::ReadHeader(SkStream* stream, SkCodec** codecOut, GifFileType** gifOut)
{
    ...
    if (nullptr != codecOut) {
        ...
        SkEncodedInfo::Alpha alpha = (transIndex < 256) ? SkEncodedInfo::kBinary_Alpha : SkEncodedInfo::kOpaque_Alpha;
        ...
        case EXTENSION_RECORD_TYPE:
            ...
            while (nullptr != extData) {
                ...
                if (AddExtensionBlock(&saveExt, extData[0],
                    ...
                    == GIF_ERROR) {
#else
                if (GIF_ERROR == GifAddExtensionBlock(&saveExt.ExtensionBlockCount, &saveExt.ExtensionBlocks, extFunction, extData[0], &extData[1])) {
#endif
...
bool SkGifCodec::GetDimensions(GifFileType* gif, SkISize* size, SkIRect* frameRect)
{
    ...
    // in fact positive.
    if (frameLeft < 0 || frameTop < 0 || frameWidth < 0 || frameHeight < 0 || width <= 0 || height <= 0) {
        return false;
    }
...
int SkGifCodec::onOutputScanline(int inputScanline) const
{
    if (fGif->Image.Interlace) {
        ...
        return get_output_row_interlaced(inputScanline - fFrameRect.top(), fFrameRect.height()) + fFrameRect.top();
    }
    return inputScanline;
}
