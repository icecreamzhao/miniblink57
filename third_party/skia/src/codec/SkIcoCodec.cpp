...
bool SkIcoCodec::IsIco(const void* buffer, size_t bytesRead)
{
    ...
    return bytesRead >= sizeof(icoSig) && (!memcmp(buffer, icoSig, sizeof(icoSig)) || !memcmp(buffer, curSig, sizeof(curSig)));
}
...
SkCodec* SkIcoCodec::NewFromStream(SkStream* stream)
{
    ...
    if (inputStream.get()->read(dirBuffer.get(), kIcoDirectoryBytes) != kIcoDirectoryBytes) {
    ...
    if (inputStream.get()->read(entryBuffer.get(), numImages * kIcoDirEntryBytes) != numImages * kIcoDirEntryBytes) {
...
SkCodec::SkScanlineOrder SkIcoCodec::onGetScanlineOrder() const
{
    ...
    return fCurrScanlineCodec ? fCurrScanlineCodec->getScanlineOrder() : INHERITED::onGetScanlineOrder();
}
