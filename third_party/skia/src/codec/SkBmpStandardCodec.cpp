    ...
    const Options& opts)
{
    ...
    if (fInIco && fIsOpaque) {
        ...
        const size_t bytesToSkip = remainingScanlines * this->srcRowBytes() + startScanline * fAndMaskRowBytes;
    ...
    void* dst, size_t dstRowBytes)
{
    ...
    // being an SkPMColor.
    SkASSERT(kRGBA_8888_SkColorType == dstInfo.colorType() || kBGRA_8888_SkColorType == dstInfo.colorType());
    ...
    for (int y = 0; y < dstInfo.height(); y++) {
        ...
        SkPMColor* dstRow = SkTAddOffset<SkPMColor>(dstPtr, row * dstRowBytes);
