...
{
    return ((tilesInArea.v + tilesInTask.v - 1) / tilesInTask.v) * ((tilesInArea.h + tilesInTask.h - 1) / tilesInTask.h);
}
...
public:
    explicit SkDngHost(dng_memory_allocator* allocater)
        : dng_host(allocater)
    {
    }
...
public:
    ...
    {
        size_t newSize;
        if (!safe_add_to_size_t(this->bytesWritten(), size, &newSize) || newSize > kMaxStreamSize) {
...
public:
    ...
    SkMemoryStream* transferBuffer(size_t offset, size_t size) override
    {
        ...
        if (offset > fStreamBuffer.bytesWritten()) {
            ...
            if (alreadyBuffered > 0 && !fStreamBuffer.read(data->writable_data(), offset, alreadyBuffered)) {
                return nullptr;
            }
...
public:
    ...
    ::piex::Error GetData(const size_t offset, const size_t length,
        uint8* data) override
    {
        return fStream->read(static_cast<void*>(data), offset, length) ? ::piex::Error::kOk : ::piex::Error::kFail;
    }
...
public:
    ...
    {
        size_t sum;
        if (!safe_add_to_size_t(static_cast<uint64>(count), offset, &sum) || !fStream->read(data, static_cast<size_t>(offset), static_cast<size_t>(count))) {
            ThrowReadFile();
        }
    }
...
SkCodec* SkRawCodec::NewFromStream(SkStream* stream)
{
    ...
    if (::piex::IsRaw(&piexStream)) {
        ...
        if (error == ::piex::Error::kOk && imageData.preview.length > 0 && imageData.preview.format == ::piex::Image::kJpegCompressed) {
            // transferBuffer() is destructive to the rawStream. Abandon the rawStream after this
            // function call.
            // FIXME: one may avoid the copy of memoryStream and use the buffered rawStream.
            SkMemoryStream* memoryStream = rawStream->transferBuffer(imageData.preview.offset, imageData.preview.length);
    ...
    int* rowsDecoded)
{
    ...
    if (imageSize.h / width > maxDiffRatio || imageSize.h < width || imageSize.v / height > maxDiffRatio || imageSize.v < height) {
