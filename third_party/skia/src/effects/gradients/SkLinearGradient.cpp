    ...
    : INHERITED(shader, ctx)
{
    ...
    if ((shader.getGradFlags() & SkGradientShader::kInterpolateColorsInPremul_Flag) || shader.colorsAreOpaque()) {
...
// Only use the Sk4f impl when known to be fast.
    ...
    if (SkShader::kClamp_TileMode == linearGradient.fTileMode && kLinear_MatrixClass == fDstToIndexClass) {
...
SkPMColor trunc_from_255(const Sk4f& x)
{
    SkPMColor c;
...
template <bool apply_alpha>
void fill(SkPMColor dst[], int count,
...
template <bool apply_alpha>
void fill(SkPMColor dst[], int count, const Sk4f& c4)
{
...
template <bool apply_alpha>
void ramp(SkPMColor dstC[], int n, const Sk4f& c, const Sk4f& dc,
