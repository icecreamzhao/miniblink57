...
void ramp<DstType::S32, ApplyPremul::False>(const Sk4f& c, const Sk4f& dc, SkPMColor dst[], int n)
{
    ...
    if (n & 2) {
        DstTraits<DstType::S32, ApplyPremul::False>::store(Sk4f(c4x.r[0], c4x.g[0], c4x.b[0], c4x.a[0]), dst++);
        DstTraits<DstType::S32, ApplyPremul::False>::store(Sk4f(c4x.r[1], c4x.g[1], c4x.b[1], c4x.a[1]), dst++);
    }

    if (n & 1) {
        DstTraits<DstType::S32, ApplyPremul::False>::store(Sk4f(c4x.r[n & 2], c4x.g[n & 2], c4x.b[n & 2], c4x.a[n & 2]), dst);
    }
}
    ...
    BlitState* state)
{
    ...
    if (mode != SkXfermode::kSrc_Mode && !(mode == SkXfermode::kSrcOver_Mode && (fFlags & kOpaqueAlpha_Flag))) {
        return false;
    }
        ...
        int count)
{
    ...
    const LinearGradient4fContext* ctx = static_cast<const LinearGradient4fContext*>(state->fCtx);
