    ...
    int halfKernelSize, const float* summedHalfKernelTable)
{
    ...
    for (int i = 0; i < numSteps; ++i, x += 1.f) {
        ...
        if (y < 0) {
            ...
            float yFrac = y - yInt;
            results[i] = (1.f - yFrac) * summedHalfKernelTable[yInt] + yFrac * summedHalfKernelTable[yInt + 1];
        }
    }
}
    ...
    float* textureRadius)
{
    ...
    profileTextureWidth = (profileTextureWidth >= 1024) ? 1024 : next_pow2_16bits(profileTextureWidth);
