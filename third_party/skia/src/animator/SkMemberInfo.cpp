...
size_t SkMemberInfo::GetSize(SkDisplayTypes type)
{ // size of simple types only
    size_t byteSize;
    switch (type) {
    case SkType_ARGB:
...
{
    SkOperand* storage = untypedStorage ? (SkOperand*)untypedStorage : arrayStorage ? arrayStorage->begin() : nullptr;
    if (storage)
    ...
    if (fType == SkType_MemberProperty) {
        ...
        SkASSERT(scriptValue.fType == SkType_Int || scriptValue.fType == SkType_ARGB || scriptValue.fType == SkType_Array);
        SkASSERT(scriptValue.fType != SkType_Array || (array != nullptr && array->getType() == SkType_Int));
        ...
        for (int index = 0; index < numberOfColors; index++) {
            SkColor color = scriptValue.fType == SkType_Array ? (SkColor)array->begin()[index].fS32 : (SkColor)scriptValue.fOperand.fS32;
