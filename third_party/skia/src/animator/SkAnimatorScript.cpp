...
SkAnimatorScript::SkAnimatorScript(SkAnimateMaker& maker, SkDisplayable* working, SkDisplayTypes type)
    : SkScriptEngine(SkScriptEngine::ToOpType(type))
    , fMaker(maker)
    , fParent(nullptr)
    , fWorking(working)
{
    ...
    void* eng, SkScriptValue* value)
{
    ...
    value->fType = SkType_Int;
    value->fOperand.fS32 = type == SkType_Float ? SkScalarIsNaN(scalar) == false && SkScalarAbs(scalar) != SK_ScalarInfinity : type == SkType_Int;
    return true;
}
