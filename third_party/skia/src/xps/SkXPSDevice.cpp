...
static HRESULT subset_typeface(SkXPSDevice::TypefaceUse* current)
{
    ...
    if (ttcCount > 0) {
        ...
        SkSFNTHeader::TableDirectoryEntry* tableDirectory = SkTAfter<SkSFNTHeader::TableDirectoryEntry>(sfntHeader);
    ...
    IXpsOMTileBrush** xpsBrush)
{
    ...
    const size_t size = SK_ARRAY_COUNT(L"/Documents/1/Resources/Images/" L_GUID_ID L".png");
    wchar_t buffer[size];
    wchar_t id[GUID_ID_LEN];
    HR(this->createId(id, GUID_ID_LEN));
    ...
    if (SkShader::kClamp_TileMode != xy[0] && SkShader::kClamp_TileMode != xy[1]) {
...
static bool rect_must_be_pathed(const SkPaint& paint, const SkMatrix& matrix)
{
    ...
    return paint.getPathEffect() || paint.getMaskFilter() || paint.getRasterizer() || (stroke && ((matrix.hasPerspective() && !zeroWidth) || SkPaint::kMiter_Join != paint.getStrokeJoin() || (SkPaint::kMiter_Join == paint.getStrokeJoin() && paint.getStrokeMiter() < SK_ScalarSqrt2)));
}
    ...
    IXpsOMGeometryFigure** xpsQuad)
{
    ...
    FLOAT segmentData[6] = {
        SkScalarToFLOAT(points[1].fX),
        SkScalarToFLOAT(points[1].fY),
        SkScalarToFLOAT(points[2].fX),
        SkScalarToFLOAT(points[2].fY),
        SkScalarToFLOAT(points[3].fX),
        SkScalarToFLOAT(points[3].fY),
    };
    ...
    const SkPaint& paint)
{
    ...
    if (d.fRC->isEmpty() || (paint.getAlpha() == 0 && paint.getXfermode() == nullptr)) {
        return;
    }
    ...
    BOOL stroke, BOOL fill, const SkPath& path)
{
    ...
    while ((verb = iter.next(points)) != SkPath::kDone_Verb) {
        ...
        case SkPath::kLine_Verb:
            if (iter.isCloseLine())
                break; //ignore the line, auto-closed
    ...
    bool pathIsMutable)
{
    ...
    if (filter) {
        ...
        SkASSERT(SkPaint::kFill_Style == paint->getStyle() || (SkPaint::kStroke_Style == paint->getStyle() && 0 == paint->getStrokeWidth()));
    ...
    TypefaceUse** typefaceUse)
{
    ...
    const size_t size = SK_ARRAY_COUNT(L"/Resources/Fonts/" L_GUID_ID L".odttf");
    wchar_t buffer[size];
    wchar_t id[GUID_ID_LEN];
    HR(this->createId(id, GUID_ID_LEN));
    ...
    bool isTTC = (data && fontData->getLength() >= sizeof(SkTTCFHeader) && ((SkTTCFHeader*)data)->ttcTag == SkTTCFHeader::TAG);
