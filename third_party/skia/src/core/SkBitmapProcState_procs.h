    ...
    uint32_t a10, uint32_t a11)
{
    ...
    int xy = x * y >> 3;
    return a00 * (32 - 2 * y - 2 * x + xy) + a01 * (2 * x - xy) + a10 * (2 * y - xy) + a11 * xy;
}
    ...
    uint32_t a10, uint32_t a11)
{
    ...
    int xy = x * y >> 4;
    uint32_t result = a00 * (16 - y - x + xy) + a01 * (x - xy) + a10 * (y - xy) + a11 * xy;
    ...
    U8CPU a10, U8CPU a11)
{
    ...
    int xy = x * y;
    unsigned result = a00 * (256 - 16 * y - 16 * x + xy) + a01 * (16 * x - xy) + a10 * (16 * y - xy) + a11 * xy;
...
#define FILTER_PROC(x, y, a, b, c, d, dst) NAME_WRAP(Filter_32_opaque) \
(x, y, a, b, c, d, dst)

#define MAKENAME(suffix) NAME_WRAP(S32_opaque_D32##suffix)
#define SRCTYPE SkPMColor
#define CHECKSTATE(state)                                \
    SkASSERT(4 == state.fPixmap.info().bytesPerPixel()); \
...
#undef FILTER_PROC
#define FILTER_PROC(x, y, a, b, c, d, dst) NAME_WRAP(Filter_32_alpha) \
(x, y, a, b, c, d, dst, alphaScale)
...
#define CHECKSTATE(state)                                        \
    SkASSERT(kRGB_565_SkColorType == state.fPixmap.colorType()); \
    SkASSERT(state.fAlphaScale == 256)
...
#define CHECKSTATE(state)                                        \
    SkASSERT(kIndex_8_SkColorType == state.fPixmap.colorType()); \
    SkASSERT(state.fAlphaScale == 256)
...
#define PREAMBLE(state)                      \
    unsigned alphaScale = state.fAlphaScale; \
...
#define CHECKSTATE(state)                                          \
    SkASSERT(kARGB_4444_SkColorType == state.fPixmap.colorType()); \
    SkASSERT(state.fAlphaScale == 256)
