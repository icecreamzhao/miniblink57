    ...
    SkAdvancedTypefaceMetrics::WidthRange* range)
{
    ...
    for (int i = expectedAdvanceCount - 1; i >= 0; --i) {
        if (range->fAdvance[i] != kDontCareAdvance && range->fAdvance[i] != kInvalidAdvance && range->fAdvance[i] != 0) {
    ...
    SkAdvancedTypefaceMetrics::GetAdvance getAdvance)
{
    ...
    if (subsetGlyphIDs) {
        ...
        lastIndex = static_cast<int>(subsetGlyphIDs[subsetGlyphIDsLength - 1]) + 1;
    }
    ...
    for (int gId = firstIndex; gId <= lastIndex; gId++) {
        ...
        if (gId < lastIndex) {
            ...
            SkASSERT(!subsetGlyphIDs || (subsetIndex < subsetGlyphIDsLength && static_cast<uint32_t>(gId) <= subsetGlyphIDs[subsetIndex]));
            if (!subsetGlyphIDs || (subsetIndex < subsetGlyphIDsLength && static_cast<uint32_t>(gId) == subsetGlyphIDs[subsetIndex])) {
        ...
        if (advance == lastAdvance) {
            ...
            wildCardsInRun++;
            trailingWildCards++;
        } else if (curRange.fAdvance.count() == repeatedAdvances + 1 + wildCardsInRun) { // All in run.
            if (lastAdvance == 0) {
            ...
            if (lastAdvance == 0 && repeatedAdvances + 1 + wildCardsInRun >= 4) {
                ...
                curRange = WidthRange(gId);
                trailingWildCards = 0;
            } else if (lastAdvance != 0 && (repeatedAdvances + 1 >= 3 || (repeatedAdvances + 1 >= 2 && wildCardsInRun >= 3))) {
                FinishRange(&curRange,
                    gId - repeatedAdvances - wildCardsInRun - 2,
                    WidthRange::kRange);
                (void)fGlyphWidths.emplace_back(std::move(curRange));
                curRange = WidthRange(gId - repeatedAdvances - wildCardsInRun - 1);
