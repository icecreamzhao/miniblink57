...
template <SkColorType, SkGammaType>
class PixelGetter;
...
public:
    ...
    void VECTORCALL pointListFew(int n, Sk4s xs, Sk4s ys) override
    {
        ...
        if (n >= 1)
            fNext->blendPixel(px0);
        if (n >= 2)
            fNext->blendPixel(px1);
        if (n >= 3)
            fNext->blendPixel(px2);
    }
...
public:
    ...
    void VECTORCALL pointListFew(int n, Sk4s xs, Sk4s ys) override
    {
        ...
        if (n >= 1)
            fNext->blendPixel(bilerpPixel(0));
        if (n >= 2)
            fNext->blendPixel(bilerpPixel(1));
        if (n >= 3)
            fNext->blendPixel(bilerpPixel(2));
    }
