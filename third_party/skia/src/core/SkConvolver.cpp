    ...
    bool useSimdIfPossible)
{
    ...
    int filterOffset, filterLength;
    const SkConvolutionFilter1D::ConvolutionFixed* filterValues = filterY.FilterForValue(0, &filterOffset, &filterLength);
    ...
    int rowBufferHeight = maxYFilterSize + (convolveProcs.fConvolve4RowsHorizontally ? 4 : 0);
    ...
    filterX.FilterForValue(filterX.numValues() - 1, &lastFilterOffset,
        &lastFilterLength);
    int avoidSimdRows = 1 + convolveProcs.fExtraHorizontalReads / (lastFilterOffset + lastFilterLength);
    ...
    for (int outY = 0; outY < numOutputRows; outY++) {
        ...
        while (nextXRow < filterOffset + filterLength) {
            if (convolveProcs.fConvolve4RowsHorizontally && nextXRow + 3 < lastFilterOffset + lastFilterLength - avoidSimdRows) {
                ...
                if (convolveProcs.fConvolveHorizontally && nextXRow < lastFilterOffset + lastFilterLength - avoidSimdRows) {
        ...
        unsigned char* const* rowsToConvolve = rowBuffer.GetRowAddresses(&firstRowInCircularBuffer);

        // Now compute the start of the subset of those rows that the filter
        // needs.
        unsigned char* const* firstRowForFilter = &rowsToConvolve[filterOffset - firstRowInCircularBuffer];
