...
static bool geometric_contains(const SkRect& outer, const SkRect& inner)
{
    ...
    return inner.fRight <= outer.fRight && inner.fLeft >= outer.fLeft && inner.fBottom <= outer.fBottom && inner.fTop >= outer.fTop;
}
...
{
    return lt_90(pts[1], pts[0], pts[3]) && lt_90(pts[2], pts[0], pts[3]) && lt_90(pts[1], pts[3], pts[0]) && lt_90(pts[2], pts[3], pts[0]);
}
