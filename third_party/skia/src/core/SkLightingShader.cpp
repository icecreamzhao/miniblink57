...
private:
    ...
    {
        const LightingFP& lightingFP = proc.cast<LightingFP>();
        return fDiffDeviceTransform == lightingFP.fDiffDeviceTransform && fDiffuseTextureAccess == lightingFP.fDiffuseTextureAccess && fLightDir == lightingFP.fLightDir && fLightColor == lightingFP.fLightColor && fAmbientColor == lightingFP.fAmbientColor;
    }
    ...
    SkSourceGammaTreatment gammaTreatment) const
{
    ...
    SkASSERT(fDiffuseMap.width() == fNormalMap.width() && fDiffuseMap.height() == fNormalMap.height());
    SkMatrix diffM;
...
sk_sp<SkShader> SkLightingShader::Make(const SkBitmap& diffuse, const SkBitmap& normal,
    ...
    const SkMatrix* diffLocalM, const SkMatrix* normLocalM)
{
    if (diffuse.isNull() || SkBitmapProcShader::BitmapIsTooBig(diffuse) || normal.isNull() || SkBitmapProcShader::BitmapIsTooBig(normal) || diffuse.width() != normal.width() || diffuse.height() != normal.height()) {
        return nullptr;
    }
