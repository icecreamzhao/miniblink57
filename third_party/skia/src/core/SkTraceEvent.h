...
#define INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE() \
    *INTERNAL_TRACE_EVENT_UID(category_group_enabled) & (SkEventTracer::kEnabledForRecording_CategoryGroupEnabledFlags | SkEventTracer::kEnabledForEventCallback_CategoryGroupEnabledFlags)
...
#define INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO_CUSTOM_VARIABLES(                             \
    ...
    category_group_enabled = reinterpret_cast<const uint8_t*>(TRACE_EVENT_API_ATOMIC_LOAD(   \
        atomic));                                                                            \
...
namespace tracing_internals {
    ...
    public:
        ...
        public:
            ...
            explicit DontMangle(uint64_t id)
                : data_(id)
            {
            }
            explicit DontMangle(unsigned int id)
                : data_(id)
            {
            }
            explicit DontMangle(unsigned short id)
                : data_(id)
            {
            }
            explicit DontMangle(unsigned char id)
                : data_(id)
            {
            }
        ...
        class ForceMangle {
        public:
            explicit ForceMangle(uint64_t id)
                : data_(id)
            {
            }
            explicit ForceMangle(unsigned int id)
                : data_(id)
            {
            }
            explicit ForceMangle(unsigned short id)
                : data_(id)
            {
            }
            explicit ForceMangle(unsigned char id)
                : data_(id)
            {
            }
        ...
        TraceID(ForceMangle id, unsigned char* flags)
            : data_(id.data())
        {
            *flags |= TRACE_EVENT_FLAG_MANGLE_ID;
        }
        TraceID(DontMangle id, unsigned char* flags)
            : data_(id.data())
        {
        }
