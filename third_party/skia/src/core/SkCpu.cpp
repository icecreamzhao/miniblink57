...
static uint32_t read_cpu_features()
{
    ...
    if (abcd[2] & (1 << 9)) {
        features |= SkCpu::SSSE3;
    }
    if (abcd[2] & (1 << 19)) {
        features |= SkCpu::SSE41;
    }
    if (abcd[2] & (1 << 20)) {
        features |= SkCpu::SSE42;
    }
...
#elif defined(SK_CPU_ARM32) && defined(SK_BUILD_FOR_ANDROID) && !defined(SK_BUILD_FOR_ANDROID_FRAMEWORK)
...
static uint32_t read_cpu_features()
{
    ...
    if (android_features & ANDROID_CPU_ARM_FEATURE_NEON_FMA) {
        features |= SkCpu::NEON_FMA;
    }
    if (android_features & ANDROID_CPU_ARM_FEATURE_VFP_FP16) {
        features |= SkCpu::VFP_FP16;
    }
    return features;
}
