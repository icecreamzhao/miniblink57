...
struct ColorSpaceHeader {
     ...
     */
    static const uint8_t kMatrix_Flag = 1 << 0;
     ...
     */
    static const uint8_t kICC_Flag = 1 << 1;
...
sk_sp<SkData> SkColorSpace::serialize() const
{
    ...
    case kAdobeRGB_Named: {
        ...
        *((ColorSpaceHeader*)data->writable_data()) = ColorSpaceHeader::Pack(k0_Version, fNamed, fGammaNamed, 0);
        return data;
    }
    default:
        break;
    ...
    case kLinear_GammaNamed: {
        sk_sp<SkData> data = SkData::MakeUninitialized(sizeof(ColorSpaceHeader) + 12 * sizeof(float));
    ...
    sk_sp<SkData> data = SkData::MakeUninitialized(sizeof(ColorSpaceHeader) + sizeof(uint32_t) + SkAlign4(profileSize));
    void* dataPtr = data->writable_data();
...
bool SkColorSpace::Equals(const SkColorSpace* src, const SkColorSpace* dst)
{
    ...
    if (srcData || dstData) {
        if (srcData && dstData) {
            return srcData->size() == dstData->size() && 0 == memcmp(srcData->data(), dstData->data(), srcData->size());
        }
