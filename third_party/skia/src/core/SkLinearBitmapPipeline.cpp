...
public:
    SrcFPPixel(float postAlpha)
        : fPostAlpha { postAlpha }
    {
    }
    SrcFPPixel(const SrcFPPixel& Blender)
        : fPostAlpha(Blender.fPostAlpha)
    {
    }
    ...
    const SkImageInfo& dstInfo)
{
    ...
    if (matrixMask & ~SkMatrix::kTranslate_Mask) {
        return false;
    }
    if (filterQuality != SkFilterQuality::kNone_SkFilterQuality) {
        return false;
    }
    if (finalAlpha != 1.0f) {
        return false;
