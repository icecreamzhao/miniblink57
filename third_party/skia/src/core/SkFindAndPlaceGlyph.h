...
struct SkMaxSizeOf<H, Ts...> {
    static const size_t value = sizeof(H) >= SkMaxSizeOf<Ts...>::value ? sizeof(H) : SkMaxSizeOf<Ts...>::value;
};
...
#define alignof __alignof
    ...
    public:
        UtfNGlyphFinder(SkGlyphCache* cache)
            : fCache(cache)
        {
    ...
    public:
        Utf8GlyphFinder(SkGlyphCache* cache)
            : UtfNGlyphFinder(cache)
        {
        }
    ...
    public:
        Utf16GlyphFinder(SkGlyphCache* cache)
            : UtfNGlyphFinder(cache)
        {
        }
    ...
    public:
        Utf32GlyphFinder(SkGlyphCache* cache)
            : UtfNGlyphFinder(cache)
        {
        }
    ...
    public:
        GlyphIdGlyphFinder(SkGlyphCache* cache)
            : fCache(cache)
        {
            SkASSERT(cache != nullptr);
        }
    ...
    typedef PolymorphicVariant<
        MapperInterface, TranslationMapper, XScaleMapper, GeneralMapper>
        Mapper;
...
#undef kSubpixelRounding
            ...
            const char** text, SkPoint position, ProcessOneGlyph&& processOneGlyph) override
        {
            ...
            if (kTextAlignment != SkPaint::kLeft_Align) {
                ...
                if (metricGlyph.fWidth <= 0) {
                    ...
                    *text = tempText;
                    return finalPosition + SkPoint { SkFloatToScalar(metricGlyph.fAdvanceX), SkFloatToScalar(metricGlyph.fAdvanceY) };
                }
            ...
            const SkGlyph& renderGlyph = fGlyphFinder->lookupGlyphXY(text, lookupPosition.fX, lookupPosition.fY);
            ...
            return finalPosition + SkPoint { SkFloatToScalar(renderGlyph.fAdvanceX), SkFloatToScalar(renderGlyph.fAdvanceY) };
        }
    ...
    public:
            ...
            const char** text, SkPoint position, ProcessOneGlyph&& processOneGlyph) override
        {
            ...
            return finalPosition + SkPoint { SkFloatToScalar(glyph.fAdvanceX), SkFloatToScalar(glyph.fAdvanceY) };
        }
    ...
    SkGlyphCache* cache, ProcessOneGlyph&& processOneGlyph)
{
    ...
    while (text < stop) {
        current = findAndPosition->findAndPositionGlyph(
