...
SkLua::SkLua(const char termCode[])
    : fTermCode(termCode)
    , fWeOwnL(true)
{
    fL = luaL_newstate();
...
#define CHECK_SETFIELD(key)            \
    do                                 \
        if (key)                       \
            lua_setfield(fL, -2, key); \
    while (0)
...
static int ldocument_beginPage(lua_State* L)
{
    ...
    push_ref(L, get_ref<SkDocument>(L, 1)->beginPage(lua2scalar(L, 2), lua2scalar(L, 3), contentPtr));
    return 1;
}
