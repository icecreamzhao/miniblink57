    ...
    const SkScalar stretchX, int width)
{
    ...
    verts->set(bounds.fRight, vy);
    verts++;
    texs->set(SkIntToScalar(width), ty);
    texs++;
}
    ...
    const SkPaint* paint)
{
    ...
    SkAutoMalloc storage(vCount * sizeof(SkPoint) * 2 + indexCount * sizeof(uint16_t));
    ...
    const SkPaint* paint)
{
    ...
    if (dstX[1] > dstX[2]) {
        dstX[1] = dstX[0] + (dstX[3] - dstX[0]) * SkIntToScalar(margins.fLeft) / (SkIntToScalar(margins.fLeft) + SkIntToScalar(margins.fRight));
        dstX[2] = dstX[1];
    }

    if (dstY[1] > dstY[2]) {
        dstY[1] = dstY[0] + (dstY[3] - dstY[0]) * SkIntToScalar(margins.fTop) / (SkIntToScalar(margins.fTop) + SkIntToScalar(margins.fBottom));
        dstY[2] = dstY[1];
    }
    ...
    const SkPaint* paint)
{
    ...
    if (false /* is our canvas backed by a gpu?*/) {
        ...
        if (xDivs[0] > xDivs[1]) {
            xDivs[0] = bitmap.width() * margins.fLeft / (margins.fLeft + margins.fRight);
            xDivs[1] = xDivs[0];
        }
        if (yDivs[0] > yDivs[1]) {
            yDivs[0] = bitmap.height() * margins.fTop / (margins.fTop + margins.fBottom);
            yDivs[1] = yDivs[0];
        }
