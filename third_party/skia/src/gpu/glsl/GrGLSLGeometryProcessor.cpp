    ...
    TransformsOut* tout)
{
    ...
    for (int i = 0; i < tin.count(); i++) {
        ...
        for (int t = 0; t < coordTransforms.count(); t++) {
            ...
            varyingType = SkToBool(SkMatrix::kPerspective_Mask & type) ? kVec3f_GrSLType : kVec2f_GrSLType;
            GrSLPrecision precision = coordTransforms[t]->precision();

            const char* uniName;
            fInstalledTransforms[i][t].fHandle = uniformHandler->addUniform(kVertex_GrShaderFlag,
