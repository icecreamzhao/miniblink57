    ...
    const CopyParams& copyParams)
{
    ...
    if (!caps->isConfigRenderable(rtDesc.fConfig, false)) {
        ...
        } else if (kRGB_GrColorComponentFlags == (kRGB_GrColorComponentFlags & GrPixelConfigComponentMask(rtDesc.fConfig))) {
    ...
    if (copyParams.fFilter != GrTextureParams::kNone_FilterMode && subset && (subset->width() != copyParams.fWidth || subset->height() != copyParams.fHeight)) {
        SkRect domain;
    ...
    , fOriginal(original)
{
    ...
    if (contentArea.fLeft > 0 || contentArea.fTop > 0 || contentArea.fRight < original->width() || contentArea.fBottom < original->height()) {
    ...
    SkRect* domainRect)
{
    ...
    SkASSERT(!textureContentArea || (!textureContentArea->contains(SkIRect::MakeWH(texW, texH)) && SkRect::Make(*textureContentArea).contains(constraintRect)));
    ...
    if (restrictFilterToRect) {
        ...
        if (coordsLimitedToConstraintRect) {
            ...
            if (textureContentArea->fLeft > 0 && textureContentArea->fLeft + filterHalfWidth > constraintRect.fLeft) {
                domainRect->fLeft = textureContentArea->fLeft + kDomainInset;
                needContentAreaConstraint = true;
            }
            if (textureContentArea->fTop > 0 && textureContentArea->fTop + filterHalfWidth > constraintRect.fTop) {
                domainRect->fTop = textureContentArea->fTop + kDomainInset;
                needContentAreaConstraint = true;
            }
            if (textureContentArea->fRight < texW && textureContentArea->fRight - filterHalfWidth < constraintRect.fRight) {
                domainRect->fRight = textureContentArea->fRight - kDomainInset;
                needContentAreaConstraint = true;
            }
            if (textureContentArea->fBottom < texH && textureContentArea->fBottom - filterHalfWidth < constraintRect.fBottom) {
    ...
    const GrTextureParams::FilterMode* filterOrNullForBicubic)
{
    ...
    if (filterOrNullForBicubic) {
        ...
        if (kDomain_DomainMode == domainMode) {
            ...
            static const SkShader::TileMode kClampClamp[] = { SkShader::kClamp_TileMode, SkShader::kClamp_TileMode };
    ...
    SkSourceGammaTreatment gammaTreatment)
{
    ...
    DomainMode domainMode = determine_domain_mode(*constraintRect, filterConstraint, coordsLimitedToConstraintRect,
    ...
    if (kTightCopy_DomainMode == domainMode) {
        ...
        SkASSERT(filterOrNullForBicubic && GrTextureParams::kMipMap_FilterMode == *filterOrNullForBicubic);
        static const GrTextureParams::FilterMode kBilerp = GrTextureParams::kBilerp_FilterMode;
        domainMode = determine_domain_mode(*constraintRect, filterConstraint, coordsLimitedToConstraintRect,
            texture->width(), texture->height(),
            contentArea, &kBilerp, &domain);
        SkASSERT(kTightCopy_DomainMode != domainMode);
    }
    SkASSERT(kNoDomain_DomainMode == domainMode || (domain.fLeft <= domain.fRight && domain.fTop <= domain.fBottom));
    ...
    SkSourceGammaTreatment gammaTreatment)
{
    ...
    if (filterOrNullForBicubic && GrTextureParams::kMipMap_FilterMode == *filterOrNullForBicubic && kYes_FilterConstraint == filterConstraint) {
