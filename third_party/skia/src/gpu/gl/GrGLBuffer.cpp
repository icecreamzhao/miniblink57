...
{
    bool cpuBacked = gpu->glCaps().useNonVBOVertexAndIndexDynamicData() && GrBufferTypeIsVertexOrIndex(intendedType) && kDynamic_GrAccessPattern == accessPattern;
    ...
    , fHasAttachedToTexture(false)
{
    if (this->isCPUBacked()) {
        ...
        if (fBufferID) {
            ...
            GL_ALLOC_CALL(gpu->glInterface(), BufferData(target, (GrGLsizeiptr)fSizeInBytes, data, fUsage));
...
void GrGLBuffer::onMap()
{
    ...
    case GrGLCaps::kMapBufferRange_MapBufferType: {
        ...
        GL_CALL_RET(fMapPtr, MapBufferRange(target, 0, fSizeInBytes, readOnly ? GR_GL_MAP_READ_BIT : writeAccess));
        break;
    }
    case GrGLCaps::kChromium_MapBufferType: {
        ...
        GL_CALL_RET(fMapPtr, MapBufferSubData(target, 0, fSizeInBytes, readOnly ? GR_GL_READ_ONLY : GR_GL_WRITE_ONLY));
        break;
    }
    }
    fGLSizeInBytes = fSizeInBytes;
