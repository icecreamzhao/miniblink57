...
void GrGLProgramBuilder::bindProgramResourceLocations(GrGLuint programID)
{
    ...
    if (!fGpu->glCaps().shaderCaps()->pathRenderingSupport() || !fGpu->glPathRendering()->shouldBindFragmentInputs()) {
        return;
    }
...
void GrGLProgramBuilder::resolveProgramResourceLocations(GrGLuint programID)
{
    ...
    for (int i = 0; i < count; ++i) {
        GrGLint location;
        GL_CALL_RET(location, GetProgramResourceLocation(programID, GR_GL_FRAGMENT_INPUT, fVaryingHandler.fPathProcVaryingInfos[i].fVariable.c_str()));
