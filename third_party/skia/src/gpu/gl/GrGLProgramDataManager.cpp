...
#define ASSERT_ARRAY_UPLOAD_IN_BOUNDS(UNI, COUNT) \
    SkASSERT((COUNT) <= (UNI).fArrayCount || (1 == (COUNT) && GrGLSLShaderVar::kNonArray == (UNI).fArrayCount))
    ...
    , fProgramID(programID)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        SkASSERT(GrGLSLShaderVar::kNonArray == builderUniform.fVariable.getArrayCount() || builderUniform.fVariable.getArrayCount() > 0);
        SkDEBUGCODE(
    ...
    for (int i = 0; i < count; i++) {
        ...
        SkASSERT(GrGLSLShaderVar::kNonArray == builderPathProcVarying.fVariable.getArrayCount() || builderPathProcVarying.fVariable.getArrayCount() > 0);
        SkDEBUGCODE(
...
void GrGLProgramDataManager::set1f(UniformHandle u, float v0) const
{
    ...
    SkASSERT(GrGLSLShaderVar::kNonArray == uni.fArrayCount);
    SkDEBUGCODE(this->printUnused(uni);) if (kUnusedUniform != uni.fFSLocation)
    {
    ...
    const SkMatrix& matrix) const
{
    ...
    SkASSERT((components == 2 && fragmentInput.fType == kVec2f_GrSLType) || (components == 3 && fragmentInput.fType == kVec3f_GrSLType));
