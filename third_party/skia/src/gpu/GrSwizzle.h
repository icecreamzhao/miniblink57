...
private:
    ...
    {
        return (8 * idx) == GrColor_SHIFT_R ? 'r' : (8 * idx) == GrColor_SHIFT_G ? 'g' : (8 * idx) == GrColor_SHIFT_B ? 'b' : 'a';
    }
    ...
    explicit GrSwizzle(const char* str)
    {
        ...
        fSwiz[3] = str[3];
        fSwiz[4] = 0;
        fKey = SkToU8(CharToIdx(fSwiz[0]) | (CharToIdx(fSwiz[1]) << 2) | (CharToIdx(fSwiz[2]) << 4) | (CharToIdx(fSwiz[3]) << 6));
    }
