    ...
    , fFragmentUniformsDirty(false)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        SkASSERT(GrGLSLShaderVar::kNonArray == uniformInfo.fVariable.getArrayCount() || uniformInfo.fVariable.getArrayCount() > 0);
        SkDEBUGCODE(
            uniform.fArrayCount = uniformInfo.fVariable.getArrayCount();
            uniform.fType = uniformInfo.fVariable.getType(););
        uniform.fBinding = (kVertex_GrShaderFlag == uniformInfo.fVisibility) ? GrVkUniformHandler::kVertexBinding
    ...
    const float v[]) const
{
    ...
    SkASSERT(arrayCount <= uni.fArrayCount || (1 == arrayCount && GrGLSLShaderVar::kNonArray == uni.fArrayCount));
...
template <int N>
inline void GrVkPipelineStateDataManager::setMatrices(UniformHandle u,
    int arrayCount,
