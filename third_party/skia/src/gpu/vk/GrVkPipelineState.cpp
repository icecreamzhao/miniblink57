    ...
    const GrPipeline& pipeline)
{
    ...
    if (fVertexUniformBuffer.get() || fFragmentUniformBuffer.get()) {
        if (fDataManager.uploadUniformBuffers(gpu, fVertexUniformBuffer, fFragmentUniformBuffer) || VK_NULL_HANDLE == fDescriptorSets[GrVkUniformHandler::kUniformBufferDescSet]) {
...
void GrVkPipelineState::writeUniformBuffers(const GrVkGpu* gpu)
{
    ...
    if (uniformBindingUpdateCount) {
        GR_VK_CALL(gpu->vkInterface(), UpdateDescriptorSets(gpu->device(), uniformBindingUpdateCount, &descriptorWrites[firstUniformWrite], 0, nullptr));
    }
}
    ...
    VkDescriptorSet* ds)
{
    ...
    GR_VK_CALL_ERRCHECK(gpu->vkInterface(), AllocateDescriptorSets(gpu->device(), &dsAllocateInfo, ds));
}

void GrVkPipelineState::DescriptorPoolManager::freeGPUResources(const GrVkGpu* gpu)
{
    if (fDescLayout) {
        GR_VK_CALL(gpu->vkInterface(), DestroyDescriptorSetLayout(gpu->device(), fDescLayout, nullptr));
