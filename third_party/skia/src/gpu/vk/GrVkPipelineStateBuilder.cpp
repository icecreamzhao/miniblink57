    ...
    VkPipelineShaderStageCreateInfo* stageInfo)
{
    ...
    VkResult err = GR_VK_CALL(gpu->vkInterface(), CreateShaderModule(gpu->device(), &moduleCreateInfo, nullptr, shaderModule));
    if (!gpu->vkCaps().canUseGLSLForShaderModule()) {
    ...
    const GrVkPipelineState::Desc& desc)
{
    ...
    for (uint32_t i = 0; i < numSamplers; ++i) {
        const GrVkGLSLSampler& sampler = static_cast<const GrVkGLSLSampler&>(fUniformHandler.getSampler(i));
    ...
    GR_VK_CALL_ERRCHECK(fGpu->vkInterface(), CreatePipelineLayout(fGpu->device(), &layoutCreateInfo, nullptr, &pipelineLayout));
    ...
    if (!pipeline) {
        GR_VK_CALL(fGpu->vkInterface(), DestroyPipelineLayout(fGpu->device(), pipelineLayout, nullptr));
