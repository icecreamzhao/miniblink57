    ...
    bool stencilOnly)
{
    ...
    for (int p = 0; p < passCount; ++p) {
        if (lastPassIsBounds && (p == passCount - 1)) {
            ...
            const SkMatrix& viewM = (reverse && viewMatrix.hasPerspective()) ? SkMatrix::I() : viewMatrix;
...
bool GrDefaultPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    return !args.fAntiAlias && (args.fShape->style().isSimpleFill() || IsStrokeHairlineOrEquivalent(args.fShape->style(), *args.fViewMatrix, nullptr));
}
