...
static int is_degen_quad_or_conic(const SkPoint p[3], SkScalar* dsqd)
{
    ...
    static const SkScalar gDegenerateToLineTolSqd = SkScalarMul(gDegenerateToLineTol, gDegenerateToLineTol);

    if (p[0].distanceToSqd(p[1]) < gDegenerateToLineTolSqd || p[1].distanceToSqd(p[2]) < gDegenerateToLineTolSqd) {
        return 1;
    }
    ...
    for (;;) {
        ...
        case SkPath::kCubic_Verb:
            ...
            if (SkIRect::Intersects(devClipBounds, ibounds)) {
                ...
                if (persp) {
                    SkScalar tolScale = GrPathUtils::scaleToleranceToSrc(SK_Scalar1, m, path.getBounds());
    ...
    SkPoint* result)
{
    ...
    SkScalar wInv = SkScalarMul(normA.fX, normB.fY) - SkScalarMul(normA.fY, normB.fX);
...
bool GrAAHairLinePathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    if (SkPath::kLine_SegmentMask == args.fShape->segmentMask() || args.fShaderCaps->shaderDerivativeSupport()) {
        return true;
    }
...
private:
    ...
    bool onCombineIfPossible(GrBatch* t, const GrCaps& caps) override
    {
        ...
        if (this->viewMatrix().hasPerspective() && !this->viewMatrix().cheapEqualTo(that->viewMatrix())) {
            return false;
        }
...
void AAHairlineBatch::onPrepareDraws(Target* target) const
{
    ...
    if (lineCount) {
        sk_sp<GrGeometryProcessor> lineGP;
        {
            ...
            LocalCoords localCoords(this->usesLocalCoords() ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type);
