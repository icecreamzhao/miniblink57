...
private:
        ...
        SkScalar scale) const
    {
        ...
        scaledBounds.fRight = scaledBounds.fLeft + SkIntToScalar(devPathBounds.width() - 2 * SK_DistanceFieldInset);
        scaledBounds.fBottom = scaledBounds.fTop + SkIntToScalar(devPathBounds.height() - 2 * SK_DistanceFieldInset);
    ...
    void flush(GrVertexBatch::Target* target, FlushInfo* flushInfo) const
    {
        if (flushInfo->fInstancesToFlush) {
            GrMesh mesh;
            int maxInstancesPerDraw = static_cast<int>(flushInfo->fIndexBuffer->gpuMemorySize() / sizeof(uint16_t) / 6);
...
DRAW_BATCH_TEST_DEFINE(AADistanceFieldPathBatch)
{
    ...
    if (context->uniqueID() != gTestStruct.fContextID) {
        ...
        gTestStruct.reset();
        gTestStruct.fAtlas = context->resourceProvider()->createAtlas(kAlpha_8_GrPixelConfig,
            ATLAS_TEXTURE_WIDTH, ATLAS_TEXTURE_HEIGHT,
            NUM_PLOTS_X, NUM_PLOTS_Y,
