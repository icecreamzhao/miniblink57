...
void GrAtlasTextBatch::onPrepareDraws(Target* target) const
{
    ...
    if (this->usesDistanceFields()) {
        flushInfo.fGeometryProcessor = this->setupDfProcessor(this->viewMatrix(), fFilteredColor, this->color(), texture);
    } else {
...
{
    GrMesh mesh;
    int maxGlyphsPerDraw = static_cast<int>(flushInfo->fIndexBuffer->gpuMemorySize() / sizeof(uint16_t) / 6);
    ...
    GrTexture* texture) const
{
    ...
    if (isLCD) {
        ...
        float blueCorrection = fDistanceAdjustTable->getAdjustment(
            ...
            fUseGammaCorrectDistanceTable);
        GrDistanceFieldLCDTextGeoProc::DistanceAdjust widthAdjust = GrDistanceFieldLCDTextGeoProc::DistanceAdjust::Make(redCorrection,
            greenCorrection,
            blueCorrection);
