...
bool GrAAConvexTessellator::tessellate(const SkMatrix& m, const SkPath& path)
{
    ...
    if (fStrokeWidth >= 0.0f && fInitialRing.numPts() > 2) {
        ...
        if (this->createInsetRings(fInitialRing, 0.0f, coverage, strokeDepth, coverage,
                &insetStrokeRing)) {
            Ring* insetAARing;
            this->createInsetRings(*insetStrokeRing, strokeDepth, coverage, strokeDepth + kAntialiasingRadius * 2, 0.0f, &insetAARing);
        }
    } else {
        Ring* insetAARing;
    ...
    SkScalar targetDepth, SkScalar targetCoverage)
{
    ...
    SkScalar result = (depth - initialDepth) / (targetDepth - initialDepth) * (targetCoverage - initialCoverage) + initialCoverage;
