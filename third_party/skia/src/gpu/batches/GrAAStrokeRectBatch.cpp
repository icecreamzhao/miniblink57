    ...
    bool coverageIgnored)
{
    ...
    LocalCoords localCoords(usesLocalCoords ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type);
...
void AAStrokeRectBatch::onPrepareDraws(Target* target) const
{
    ...
    SkASSERT(canTweakAlphaForCoverage ? vertexStride == sizeof(GrDefaultGeoProcFactory::PositionColorAttr) : vertexStride == sizeof(GrDefaultGeoProcFactory::PositionColorCoverageAttr));
    ...
    bool tweakAlphaForCoverage) const
{
    ...
    SkPoint* fan3Pos = reinterpret_cast<SkPoint*>(verts + (2 * outerVertexNum + innerVertexNum) * vertexStride);
    ...
    if (!degenerate) {
        ...
        inset = SK_ScalarHalf * SkMinScalar(inset, SkTMax(devOutside.height(), devOutsideAssist.height()));
    }
#else
    SkScalar inset;
    if (!degenerate) {
    ...
    if (miterStroke) {
        ...
        SkPoint* fan1AssistPos = reinterpret_cast<SkPoint*>(verts + (outerVertexNum + 4) * vertexStride);
        // outermost
        set_inset_fan(fan0Pos, vertexStride, devOutside, -SK_ScalarHalf, -SK_ScalarHalf);
...
inline static bool is_miter(const SkStrokeRec& stroke)
{
    ...
    if ((stroke.getWidth() > 0) && (stroke.getJoin() != SkPaint::kMiter_Join || stroke.getMiter() < SK_ScalarSqrt2)) {
        return false;
    }
    return true;
}
    ...
    const SkRect& devInside)
{
    SkASSERT(!devOutside.isEmpty())
        SkASSERT(!devInside.isEmpty())
            AAStrokeRectBatch* batch
        = AAStrokeRectBatch::Create(viewMatrix, true);
...
DRAW_BATCH_TEST_DEFINE(AAStrokeRectBatch)
{
    ...
    SkRect rect = random->nextBool() ? SkRect::MakeXYWH(10, 10, 50, 40) : SkRect::MakeXYWH(6, 7, 0, 0);
