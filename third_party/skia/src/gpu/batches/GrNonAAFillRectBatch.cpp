    ...
    const SkMatrix* localMatrix)
{
    ...
    if (viewMatrix.hasPerspective()) {
        LocalCoords localCoords(hasExplicitLocalCoords ? LocalCoords::kHasExplicit_Type : LocalCoords::kUsePosition_Type,
            localMatrix);
    ...
    const GrQuad* localQuad)
{
    ...
    if (localQuad) {
        ...
        for (int i = 0; i < NonAAFillRectBatchBase::kVertsPerInstance; i++) {
            SkPoint* coords = reinterpret_cast<SkPoint*>(vertices + kLocalOffset + i * vertexStride);
...
public:
        ...
        const GrXPOverridesForBatch& overrides)
    {
        ...
        return mine.fViewMatrix.cheapEqualTo(theirs.fViewMatrix) && mine.fHasLocalRect == theirs.fHasLocalRect && (!mine.fHasLocalMatrix || mine.fLocalMatrix.cheapEqualTo(theirs.fLocalMatrix));
    }
        ...
        const GrXPOverridesForBatch& overrides)
    {
        ...
        SkASSERT(geo.fHasLocalRect ? gp->getVertexStride() == sizeof(GrDefaultGeoProcFactory::PositionColorLocalCoordAttr) : gp->getVertexStride() == sizeof(GrDefaultGeoProcFactory::PositionColorAttr));
        return gp;
    }
...
bool Append(GrBatch* origBatch,
    ...
    const SkMatrix* localMatrix)
{
    bool usePerspective = viewMatrix.hasPerspective() || (localMatrix && localMatrix->hasPerspective());
    ...
    if (!usePerspective) {
        ...
        if (!geo.fViewMatrix.cheapEqualTo(viewMatrix) || geo.fHasLocalRect != SkToBool(localRect) || geo.fHasLocalMatrix != SkToBool(localMatrix) || (geo.fHasLocalMatrix && !geo.fLocalMatrix.cheapEqualTo(*localMatrix))) {
            return false;
        }
