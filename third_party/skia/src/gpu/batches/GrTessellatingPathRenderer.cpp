...
public:
    explicit PathInvalidator(const GrUniqueKey& key)
        : fMsg(key)
    {
    }
...
bool GrTessellatingPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const
{
    ...
    return !args.fShape->style().applies() && args.fShape->style().isSimpleFill() && !args.fAntiAlias && args.fShape->hasUnstyledKey() && !args.fShape->knownToBeConvex();
}
...
private:
    ...
    void onPrepareDraws(Target* target) const override
    {
        sk_sp<GrGeometryProcessor> gp;
        {
            ...
            Color color(fColor);
            LocalCoords localCoords(fPipelineInfo.readsLocalCoords() ? LocalCoords::kUsePosition_Type : LocalCoords::kUnused_Type);
