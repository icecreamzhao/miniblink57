    ...
    SkPathPriv::FirstDirection* dir)
{
    ...
    SkScalar det2x2 = SkScalarMul(m.get(SkMatrix::kMScaleX), m.get(SkMatrix::kMScaleY)) - SkScalarMul(m.get(SkMatrix::kMSkewX), m.get(SkMatrix::kMSkewY));
    if (det2x2 < 0) {
...
sk_sp<GrGeometryProcessor> QuadEdgeEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    return d->fCaps->shaderCaps()->shaderDerivativeSupport() ? QuadEdgeEffect::Make(GrRandomColor(d->fRandom),
...
{
    return (args.fShaderCaps->shaderDerivativeSupport() && args.fAntiAlias && args.fShape->style().isSimpleFill() && !args.fShape->inverseFilled() && args.fShape->knownToBeConvex());
}
    ...
    bool tweakAlphaForCoverage)
{
    ...
    for (int i = 0; i < tess.numPts(); ++i) {
        if (tweakAlphaForCoverage) {
            ...
            *reinterpret_cast<float*>(verts + i * vertexStride + sizeof(GrColor)) = tess.coverage(i);
        }
    }
