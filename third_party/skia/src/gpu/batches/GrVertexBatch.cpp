...
void GrVertexBatch::onDraw(GrBatchFlushState* state)
{
    ...
    for (int currDrawIdx = 0; currDrawIdx < fQueuedDraws.count(); ++currDrawIdx) {
        ...
        while (currUploadIdx < fInlineUploads.count() && fInlineUploads[currUploadIdx].fUploadBeforeToken == drawToken) {
...
void GrVertexBatch::Target::draw(const GrGeometryProcessor* gp, const GrMesh& mesh)
{
    ...
    if (!batch->fQueuedDraws.empty()) {
        ...
        if (lastDraw.fGeometryProcessor == gp && (batch->fInlineUploads.empty() || batch->fInlineUploads.back().fUploadBeforeToken != this->nextDrawToken())) {
            ++lastDraw.fMeshCnt;
            return;
        }
    }
