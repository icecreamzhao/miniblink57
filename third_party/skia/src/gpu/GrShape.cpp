...
GrShape::GrShape(const GrShape& that)
    : fStyle(that.fStyle)
{
...
GrShape::GrShape(const GrShape& parent, GrStyle::Apply apply, SkScalar scale)
{
    ...
    // stroke of a rect).
    if (!parent.style().applies() || (GrStyle::Apply::kPathEffectOnly == apply && !parent.style().pathEffect())) {
    ...
    if (pe) {
        ...
        if (!parent.fStyle.applyPathEffectToPath(&this->path(), &strokeRec, *srcForPathEffect,
            ...
            if (parent.fType == Type::kRRect && (parent.fRRectData.fDir != kDefaultRRectDir || parent.fRRectData.fStart != kDefaultRRectStart)) {
...
void GrShape::attemptToSimplifyPath()
{
    ...
    if (Type::kPath != fType) {
        ...
        if (!this->style().hasNonDashPathEffect()) {
            if (this->style().strokeRec().getStyle() == SkStrokeRec::kStroke_Style || this->style().strokeRec().getStyle() == SkStrokeRec::kHairline_Style) {
