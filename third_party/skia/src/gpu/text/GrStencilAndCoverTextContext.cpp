...
static void delete_hash_map_entry(const Key&, Val* val)
{
    SkASSERT(*val);
    delete *val;
}
...
template <typename T>
static void delete_hash_table_entry(T* val)
{
    SkASSERT(*val);
    delete *val;
}
    ...
    , fLastDrawnGlyphsID(SK_InvalidUniqueID)
{
    ...
    if (!fFont.getPathEffect() && !fStyle.isDashed()) {
        ...
        fUsingRawGlyphPaths = SK_Scalar1 == fFont.getTextScaleX() && 0 == fFont.getTextSkewX() && !fFont.isFakeBoldText() && !fFont.isVerticalText();
    } else {
