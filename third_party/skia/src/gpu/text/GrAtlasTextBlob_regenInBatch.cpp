    ...
    SkScalar transY) const
{
    ...
    for (int glyphIdx = 0; glyphIdx < glyphCount; glyphIdx++) {
        ...
        if (regenTexCoords) {
            ...
            if (!fontCache->hasGlyph(glyph) && !strike->addGlyphToAtlas(target, glyph, lazyCache->get(), info->maskFormat())) {
                helper->flush();
    ...
    if (regenTexCoords) {
        ...
        info->setAtlasGeneration(brokenRun ? GrBatchAtlas::kInvalidAtlasGeneration : fontCache->atlasGeneration(info->maskFormat()));
    }
}
