...
GrAtlasTextBlob* GrAtlasTextBlob::Create(GrMemoryPool* pool, int glyphCount, int runCount)
{
    ...
    size_t size = sizeof(GrAtlasTextBlob) + verticesCount + glyphCount * sizeof(GrGlyph**) + sizeof(GrAtlasTextBlob::Run) * runCount;
    ...
    const SkMatrix* viewMatrix)
{
    ...
    SkAutoDescriptor* desc = run->fOverrideDescriptor.get() ? run->fOverrideDescriptor.get() : &run->fDescriptor;
    ...
    const SkMatrix& viewMatrix, SkScalar x, SkScalar y)
{
    ...
    if (fKey.fCanonicalColor == SK_ColorTRANSPARENT && fPaintColor != color) {
        return true;
    }
    ...
    if (fKey.fHasBlur && (fBlurRec.fSigma != blurRec.fSigma || fBlurRec.fStyle != blurRec.fStyle || fBlurRec.fQuality != blurRec.fQuality)) {
        return true;
    }

    // Similarly, we only cache one version for each style
    if (fKey.fStyle != SkPaint::kFill_Style && (fStrokeInfo.fFrameWidth != paint.getStrokeWidth() || fStrokeInfo.fMiterLimit != paint.getStrokeMiter() || fStrokeInfo.fJoin != paint.getStrokeJoin())) {
        return true;
    }
    ...
    if (this->hasBitmap()) {
        if (fInitialViewMatrix.getScaleX() != viewMatrix.getScaleX() || fInitialViewMatrix.getScaleY() != viewMatrix.getScaleY() || fInitialViewMatrix.getSkewX() != viewMatrix.getSkewX() || fInitialViewMatrix.getSkewY() != viewMatrix.getSkewY()) {
            return true;
        }
        ...
        SkScalar transY = viewMatrix.getTranslateY() + viewMatrix.getSkewY() * (x - fInitialX) + viewMatrix.getScaleY() * (y - fInitialY) - fInitialViewMatrix.getTranslateY();
...
inline void GrAtlasTextBlob::flushRun(GrDrawContext* dc, GrPipelineBuilder* pipelineBuilder,
...
{
    if (applyVM) {
        *transX = newViewMatrix.getTranslateX() + newViewMatrix.getScaleX() * (newX - currentX) + newViewMatrix.getSkewX() * (newY - currentY) - currentViewMatrix.getTranslateX();

        *transY = newViewMatrix.getTranslateY() + newViewMatrix.getSkewY() * (newX - currentX) + newViewMatrix.getScaleY() * (newY - currentY) - currentViewMatrix.getTranslateY();
    } else {
...
void GrAtlasTextBlob::AssertEqual(const GrAtlasTextBlob& l, const GrAtlasTextBlob& r)
{
    ...
    for (int i = 0; i < l.fRunCount; i++) {
        ...
        if (lRun.fOverrideDescriptor.get()) {
            ...
            SkASSERT_RELEASE(*lRun.fOverrideDescriptor->getDesc() == *rRun.fOverrideDescriptor->getDesc());
        } else {
        ...
        for (int j = 0; j < lRun.fSubRunInfo.count(); j++) {
            ...
            if (lSubRun.strike()) {
                ...
                SkASSERT_RELEASE(GrBatchTextStrike::GetKey(*lSubRun.strike()) == GrBatchTextStrike::GetKey(*rSubRun.strike()));
