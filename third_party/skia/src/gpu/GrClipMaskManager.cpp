    ...
    const GrReducedClip::ElementList& elements)
{
    ...
    for (GrReducedClip::ElementList::Iter iter(elements.headIter()); iter.get(); iter.next()) {
        ...
        bool needsStencil = invert || SkRegion::kIntersect_Op == op || SkRegion::kReverseDifference_Op == op;
...
#define DISABLE_DEV_BOUNDS_FOR_CLIP_REDUCTION 0
    ...
    if (elements.count() <= kMaxAnalyticElements) {
        ...
        if (disallowAnalyticAA && !drawContext->numColorSamples()) {
            ...
            // is multisampled.
            disallowAnalyticAA = pipelineBuilder.isHWAntialias() || pipelineBuilder.hasUserStencilSettings();
        }
        sk_sp<GrFragmentProcessor> clipFP;
        if (elements.isEmpty() || (requiresAA && get_analytic_clip_processor(elements, disallowAnalyticAA, clipToRTOffset, devBounds, &clipFP))) {
    ...
    const SkIPoint& clipSpaceToStencilOffset)
{
    ...
    if (stencilAttachment->mustRenderClip(elementsGenID, clipSpaceIBounds, clipSpaceToStencilOffset)) {
        ...
        for (GrReducedClip::ElementList::Iter iter(elements.headIter()); iter.get(); iter.next()) {
            ...
            bool canRenderDirectToStencil = GrPathRenderer::kNoRestriction_StencilSupport == stencilSupport;
            bool drawDirectToClip; // Given the renderer, the element,
                // fill rule, and set operation should
                // we render the element directly to
                // stencil bit used for clipping.
            GrUserStencilSettings const* const* stencilPasses = GrStencilSettings::GetClipPasses(op, canRenderDirectToStencil, fillInverted,
                &drawDirectToClip);
