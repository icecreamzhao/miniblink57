...
void GrStencilSettings::reset(const GrStencilSettings& that)
{
    ...
    if (!this->isTwoSided()) {
        ...
        GR_STATIC_ASSERT(sizeof(Face) == offsetof(GrStencilSettings, fBack) - offsetof(GrStencilSettings, fFront));
    }
}
    ...
    int numStencilBits)
{
    ...
    SkDEBUGCODE(GrUserStencilOp otherOp = SkTMin(user.fPassOp, user.fFailOp);) if (maxOp <= kLastUserOnlyStencilOp)
    {
        // Ops that only modify user bits.
    ...
    else if (maxOp <= kLastClipOnlyStencilOp)
    {
        // Ops that only modify the clip bit.
        fWriteMask = clipBit;
        SkASSERT(GrUserStencilOp::kKeep == otherOp || (otherOp > kLastUserOnlyStencilOp && otherOp <= kLastClipOnlyStencilOp));
    }
    else
    {
