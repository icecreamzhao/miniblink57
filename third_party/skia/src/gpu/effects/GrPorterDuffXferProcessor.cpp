...
public:
    ...
    struct get_properties : std::integral_constant<Properties, static_cast<Properties>(
                                                                   ...
                                                                   (GR_BLEND_MODIFIES_DST(BlendEquation, SrcCoeff, DstCoeff) ? kModifiesDst_Property : 0) |

                                                                   (GR_BLEND_COEFFS_USE_DST_COLOR(SrcCoeff, DstCoeff) ? kUsesDstColor_Property : 0) |

                                                                   ((PrimaryOut >= kModulate_OutputType && GR_BLEND_COEFFS_USE_SRC_COLOR(SrcCoeff, DstCoeff)) || (SecondaryOut >= kModulate_OutputType && GR_BLEND_COEFF_REFS_SRC2(DstCoeff)) ? kUsesInputColor_Property : 0) | // We assert later that SrcCoeff doesn't ref src2.

                                                                   (kModulate_OutputType == PrimaryOut && kNone_OutputType == SecondaryOut && GR_BLEND_CAN_TWEAK_ALPHA_FOR_COVERAGE(BlendEquation, SrcCoeff, DstCoeff) ? kCanTweakAlphaForCoverage_Property : 0))> {

        // The provided formula should already be optimized.
        GR_STATIC_ASSERT((kNone_OutputType == PrimaryOut) == !GR_BLEND_COEFFS_USE_SRC_COLOR(SrcCoeff, DstCoeff));
...
public:
    PorterDuffXferProcessor(BlendFormula blendFormula)
...
public:
    static void GenKey(const GrProcessor& processor, GrProcessorKeyBuilder* b)
    {
        ...
        b->add32(xp.getBlendFormula().fPrimaryOutputType | (xp.getBlendFormula().fSecondaryOutputType << 3));
    ...
    const GrCaps& caps) const
{
    ...
    if (!fBlendFormula.modifiesDst()) {
        ...
        optFlags |= (GrXferProcessor::kIgnoreColor_OptFlag | GrXferProcessor::kIgnoreCoverage_OptFlag | GrXferProcessor::kCanTweakAlphaForCoverage_OptFlag);
    } else {
        ...
        if (optimizations.fColorPOI.allStagesMultiplyInput() && fBlendFormula.canTweakAlphaForCoverage() && !optimizations.fCoveragePOI.isFourChannelOutput()) {
    ...
    const DstTexture* dstTexture) const
{
    ...
    if (optimizations.fCoveragePOI.isFourChannelOutput()) {
        if (SkXfermode::kSrcOver_Mode == fXfermode && kRGBA_GrColorComponentFlags == optimizations.fColorPOI.validFlags() && !caps.shaderCaps()->dualSourceBlendingSupport() && !caps.shaderCaps()->dstReadInShaderSupport()) {
    ...
    const GrPipelineOptimizations& optimizations) const
{
    ...
    // the XP.
    if (optimizations.fCoveragePOI.isFourChannelOutput()) {
        if (SkXfermode::kSrcOver_Mode == fXfermode && kRGBA_GrColorComponentFlags == optimizations.fColorPOI.validFlags() && !caps.shaderCaps()->dstReadInShaderSupport()) {
            return false;
        }
    ...
    const GrXferProcessor::DstTexture* dstTexture)
{
    ...
    if (!optimizations.fCoveragePOI.isFourChannelOutput() && !(optimizations.fCoveragePOI.isSolidWhite() && !hasMixedSamples && optimizations.fColorPOI.isOpaque())) {
    ...
    BlendFormula blendFormula;
    if (optimizations.fCoveragePOI.isFourChannelOutput()) {
        if (kRGBA_GrColorComponentFlags == optimizations.fColorPOI.validFlags() && !caps.shaderCaps()->dualSourceBlendingSupport() && !caps.shaderCaps()->dstReadInShaderSupport()) {
...
bool GrPorterDuffXPFactory::SrcOverWillNeedDstTexture(const GrCaps& caps,
    const GrPipelineOptimizations& optimizations)
{
    if (caps.shaderCaps()->dstReadInShaderSupport() || caps.shaderCaps()->dualSourceBlendingSupport()) {
        return false;
    }
    ...
    // into the XP.
    if (optimizations.fCoveragePOI.isFourChannelOutput()) {
        if (kRGBA_GrColorComponentFlags == optimizations.fColorPOI.validFlags() && !caps.shaderCaps()->dstReadInShaderSupport()) {
            return false;
        }
    ...
    return get_blend_formula(optimizations.fColorPOI, optimizations.fCoveragePOI,
        kHasMixedSamples, SkXfermode::kSrcOver_Mode)
        .hasSecondaryOutput();
}
