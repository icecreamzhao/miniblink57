    ...
    uint32_t flags)
{
    ...
    if (Apply::kPathEffectAndStrokeRec == apply && style.strokeRec().needToApply()) {
        ...
        key[i++] = style.strokeRec().getStyle() | style.strokeRec().getJoin() << kJoinShift | cap << kCapShift;
    ...
    SkScalar resScale) const
{
    ...
    if (strokeRec.needToApply()) {
        ...
        *style = SkStrokeRec::kFill_InitStyle;
        ...
        SkASSERT(SkStrokeRec::kFill_Style == strokeRec.getStyle() || SkStrokeRec::kHairline_Style == strokeRec.getStyle());
