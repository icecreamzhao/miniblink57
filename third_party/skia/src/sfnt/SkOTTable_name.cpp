...
bool SkOTTableName::Iterator::next(SkOTTableName::Iterator::Record& record)
{
    ...
    if (SkOTTableName::format_1 == fName.format && languageID >= 0x8000) {
        ...
        const SkOTTableName::Format1Ext* format1ext = SkTAfter<const SkOTTableName::Format1Ext>(nameRecords, nameRecordsCount);

        if (languageTagRecordIndex < SkEndian_SwapBE16(format1ext->langTagCount)) {
            const SkOTTableName::Format1Ext::LangTagRecord* languageTagRecord = SkTAfter<const SkOTTableName::Format1Ext::LangTagRecord>(format1ext);
