    ...
    int count, int x, int y)
{
    ...
    *xy++ = ClampX_ClampY_pack_filter(fy, maxY, s.fFilterOneY);
    ...
    if (dx > 0 && (unsigned)(fx >> 16) <= maxX && (unsigned)((fx + dx * (count - 1)) >> 16) < maxX) {
        if (count >= 4) {
            ...
            while (count >= 4) {
                ...
                wide_out = _mm_or_si128(wide_out, _mm_add_epi32(_mm_srai_epi32(wide_fx, 16), wide_1));
    ...
    uint32_t xy[], int count, int x, int y)
{
    ...
    *xy++ = SkClampMax(mapper.intY(), maxY);
    ...
    // test if we don't need to apply the tile proc
    if ((unsigned)(fx >> 16) <= maxX && (unsigned)((fx + dx * (count - 1)) >> 16) <= maxX) {
    ...
    uint32_t xy[], int count, int x, int y)
{
    ...
    if (count >= 4 && (maxX <= 0xFFFF)) {
        while (((size_t)xy & 0x0F) != 0) {
            *xy++ = (SkClampMax(fy >> 16, maxY) << 16) | SkClampMax(fx >> 16, maxX);
            fx += dx;
            fy += dy;
            count--;
        }
