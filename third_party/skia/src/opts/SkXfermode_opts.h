...
#define XFERMODE(Xfermode)                               \
    struct Xfermode {                                    \
        Sk4f operator()(const Sk4f&, const Sk4f&) const; \
    };                                                   \
...
XFERMODE_AA(Plus)
{ // [ clamp( (1-AA)D + (AA)(S+D) ) == clamp(D + AA*S) ]
...
#define CASE(Xfermode)                   \
    case SkXfermode::k##Xfermode##_Mode: \
        return new Sk4pxXfermode<Xfermode>(rec, mode)
        CASE(Clear);
        CASE(Src);
        CASE(Dst);
        CASE(SrcOver);
...
#define CASE(Xfermode)                   \
    case SkXfermode::k##Xfermode##_Mode: \
        return new Sk4fXfermode<Xfermode>(rec, mode)
        CASE(ColorDodge);
        CASE(ColorBurn);
        CASE(SoftLight);
#undef CASE
