...
protected:
        ...
        bool drawText)
    {
        ...
        this->setWHZ(SkScalarCeilToInt(bounds.right()), drawText ? SkScalarRoundToInt(scale * 3 / 2) : SkScalarRoundToInt(scale),
    ...
    public:
        int fIndex;
        MyClick(SkView* target, int index)
            : Click(target)
            , fIndex(index)
        {
        }
    ...
    bool onClick(Click* click) override
    {
        ...
        if (index < (int)SK_ARRAY_COUNT(fPts)) {
            ...
            gDebugStrokerError = SkTMax(FLT_EPSILON, MapScreenYtoValue(click->fICurr.fY, fErrorControl, kStrokerErrorMin, kStrokerErrorMax));
            gDebugStrokerErrorSet = true;
        }
#endif
        else if (index == (int)SK_ARRAY_COUNT(fPts) + 4) {
            fWidth = SkTMax(FLT_EPSILON, MapScreenYtoValue(click->fICurr.fY, fWidthControl, kWidthMin, kWidthMax));
