...
public:
    SimpleShapesGM(bool antialias)
        : INHERITED("simpleshapes", antialias)
    {
    }
...
public:
    InnerShapesGM(bool antialias)
        : INHERITED("innershapes", antialias)
    {
    }
...
private:
    void drawShapes(SkCanvas* canvas) const override
    {
        ...
        for (int i = 0; i < fShapes.count(); i++) {
            ...
            float s = 0.95f * SkTMin(outer.rect().width() / inner.rect().width(), outer.rect().height() / inner.rect().height());
            SkMatrix innerXform;
