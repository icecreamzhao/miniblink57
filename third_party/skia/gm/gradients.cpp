...
public:
    GradientsLocalPerspectiveGM(bool dither)
        : fDither(dither)
    {
        this->setBGColor(sk_tool_utils::color_to_565(0xFFDDDDDD));
    }
...
protected:
    SkString onShortName()
    {
        return SkString(fDither ? "gradients_local_perspective" : "gradients_local_perspective_nodither");
    }
...
public:
    GradientsViewPerspectiveGM(bool dither)
        : INHERITED(dither)
    {
    }
...
public:
    GradientsDegenrate2PointGM(bool dither)
        : fDither(dither)
    {
    }
...
public:
    ClampedGradientsGM(bool dither)
        : fDither(dither)
    {
    }
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        const SkColor sweep_colors[] = { 0xFFFF0000, 0xFFFFFF00, 0xFF00FF00, 0xFF00FFFF, 0xFF0000FF, 0xFFFF00FF, 0xFFFF0000 };
...
static void draw_many_stops(SkCanvas* canvas, uint32_t flags)
{
    ...
    for (unsigned i = 0; i < kStopCount; i++) {
        switch (i % 5) {
        case 0:
            colors[i] = SK_ColorRED;
            break;
        case 1:
            colors[i] = SK_ColorGREEN;
            break;
        case 2:
            colors[i] = SK_ColorGREEN;
            break;
        case 3:
            colors[i] = SK_ColorBLUE;
            break;
        case 4:
            colors[i] = SK_ColorRED;
            break;
