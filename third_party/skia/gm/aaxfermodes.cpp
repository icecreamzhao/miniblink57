...
protected:
    ...
    {
        return SkISize::Make(2 * kMargin + 2 * kXfermodeTypeSpacing - (kXfermodeTypeSpacing - (kLabelSpacing + 2 * kPaintSpacing)),
            2 * kMargin + kTitleSpacing + kSubtitleSpacing + (1 + SkXfermode::kLastCoeffMode) * kShapeSpacing);
    }
    ...
    {
        SkRect clipRect = { -kShapeSize * 11 / 16, -kShapeSize * 11 / 16, kShapeSize * 11 / 16, kShapeSize * 11 / 16 };
        ...
        for (size_t xfermodeSet = 0; xfermodeSet < 2; xfermodeSet++) {
            ...
            if (kShape_Pass == drawingPass) {
                ...
                canvas->drawText("Src Opaque", sizeof("Src Opaque") - 1,
                    kLabelSpacing + kShapeTypeSpacing * 1.5f + kShapeSpacing / 2 + kPaintSpacing, kSubtitleSpacing / 2 + fLabelPaint.getTextSize() / 3,
                    fLabelPaint);
            }
