...
public:
    ...
    void validate() const;
    template <typename T>
    void set(const char* confname,
        T value,
        bool warnIfNotFound = true);

private:
    template <typename T>
    friend class SkRTConf;

    void registerConf(SkRTConfBase* conf);

    template <typename T>
    bool parse(const char* name, T* value);
...
template <>
inline void SkRTConf<bool>::doPrint(char* s) const
{
    char tmp[30];
...
template <>
inline void SkRTConf<int>::doPrint(char* s) const
{
    char tmp[30];
...
template <>
inline void SkRTConf<unsigned int>::doPrint(char* s) const
{
    char tmp[30];
...
template <>
inline void SkRTConf<float>::doPrint(char* s) const
{
    char tmp[30];
...
template <>
inline void SkRTConf<double>::doPrint(char* s) const
{
    char tmp[30];
    sprintf(tmp, "%6.6f # [%6.6f]", fValue, fDefault);
    sprintf(s, "%-30.30s", tmp);
...
template <>
inline void SkRTConf<const char*>::doPrint(char* s) const
{
    char tmp[30];
