...
public:
    ...
    static sk_sp<SkImageFilter> MakeN(sk_sp<SkImageFilter> filters[], int count, const SkBlendMode modes[],
        const CropRect* cropRect = nullptr)
    {
        if (0 == count)
            return nullptr;
        SkXfermode::Mode* xModes = (SkXfermode::Mode*)malloc(count);
        for (int i = 0; i < count; ++i) {
            xModes[i] = SkXfermode::SkBlendModeToSkXfermodeMode(modes[i]);
        }
