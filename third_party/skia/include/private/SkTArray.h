...
*/
template <typename T, bool MEM_COPY = false>
class SkTArray {
public:
    /**
    ...
    void swap(SkTArray* that)
    {
        ...
        if (this->fPreAllocMemArray != this->fItemArray && that->fPreAllocMemArray != that->fItemArray) {
...
protected:
    ...
    void init(int count, void* preAllocStorage, int preAllocOrReserveCount)
    {
        ...
        fCount = count;
        fReserveCount = (preAllocOrReserveCount > 0) ? preAllocOrReserveCount : gMIN_ALLOC_COUNT;
        fPreAllocMemArray = preAllocStorage;
        if (fReserveCount >= fCount && preAllocStorage) {
