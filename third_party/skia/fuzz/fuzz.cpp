...
int fuzz_img(SkData* bytes, uint8_t scale, uint8_t mode)
{
    ...
    case 1: { //kScanline_Mode
        if (SkCodec::kSuccess != codec->startScanlineDecode(decodeInfo, NULL, colorPtr, colorCountPtr)) {
...
Fuzz::Fuzz(SkData* bytes)
    : fBytes(SkSafeRef(bytes))
    , fNextByte(0)
{
}
