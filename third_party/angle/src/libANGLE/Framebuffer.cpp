...
const FramebufferAttachment* Framebuffer::Data::getColorAttachment(size_t colorAttachment) const
{
    ...
    return mColorAttachments[colorAttachment].isAttached() ? &mColorAttachments[colorAttachment] : nullptr;
}
...
const FramebufferAttachment* Framebuffer::Data::getDepthStencilAttachment() const
{
    ...
    if (mDepthAttachment.isAttached() && mStencilAttachment.isAttached() && mDepthAttachment.type() == mStencilAttachment.type() && mDepthAttachment.id() == mStencilAttachment.id()) {
        return &mDepthAttachment;
    }
...
const FramebufferAttachment* Framebuffer::getDrawBuffer(size_t drawBuffer) const
{
    ...
    if (mData.mDrawBufferStates[drawBuffer] != GL_NONE) {
        ...
        ASSERT(mData.mDrawBufferStates[drawBuffer] == GL_COLOR_ATTACHMENT0 + drawBuffer || (drawBuffer == 0 && mData.mDrawBufferStates[drawBuffer] == GL_BACK));
...
{
    ASSERT(buffer == GL_BACK || buffer == GL_NONE || (buffer >= GL_COLOR_ATTACHMENT0 && (buffer - GL_COLOR_ATTACHMENT0) < mData.mColorAttachments.size()));
