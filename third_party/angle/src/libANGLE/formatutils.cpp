...
static bool FloatRenderableSupport(GLuint clientVersion, const Extensions& extensions)
{
    ...
    return FloatSupport(clientVersion, extensions) && (extensions.colorBufferFloat || clientVersion == 2);
}
...
{
    // We don't expose colorBufferFloat in ES2, but we silently support rendering to float.
    return FloatSupportRG(clientVersion, extensions) && (extensions.colorBufferFloat || clientVersion == 2);
}
