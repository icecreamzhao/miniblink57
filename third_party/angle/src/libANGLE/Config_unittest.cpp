...
TEST(ConfigSetTest, Sorting_BitSizes)
{
    ...
    for (size_t i = 1; i < filteredConfigs.size(); i++) {
        ...
        size_t prevBitCount = prevConfig.redSize + prevConfig.greenSize + prevConfig.blueSize + prevConfig.alphaSize;

        const egl::Config& curConfig = *filteredConfigs[i];
        size_t curBitCount = curConfig.redSize + curConfig.greenSize + curConfig.blueSize + curConfig.alphaSize;
