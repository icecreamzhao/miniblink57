...
egl::Error DisplayGLX::initialize(egl::Display* display)
{
    ...
    bool isOpenGLES = eglAttributes.get(EGL_PLATFORM_ANGLE_TYPE_ANGLE, EGL_PLATFORM_ANGLE_TYPE_DEFAULT_ANGLE) == EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE;
    ...
    std::string rendererString = reinterpret_cast<const char*>(mFunctionsGL->getString(GL_RENDERER));
    ...
    glx::Context* context)
{
    ...
    EGLint requestedDisplayType = eglAttributes.get(EGL_PLATFORM_ANGLE_TYPE_ANGLE, EGL_PLATFORM_ANGLE_TYPE_DEFAULT_ANGLE);
    ...
    if (static_cast<EGLint>(requestedVersion.major) != EGL_DONT_CARE && static_cast<EGLint>(requestedVersion.minor) != EGL_DONT_CARE) {
        if (!(profileMask & GLX_CONTEXT_ES2_PROFILE_BIT_EXT) && requestedVersion >= gl::Version(3, 2)) {
    ...
    if (requestedDisplayType != EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE) {
        for (auto& version : desktopVersionsFrom3_2) {
            egl::Error error = createContextAttribs(config, version, GLX_CONTEXT_CORE_PROFILE_BIT_ARB, context);
            if (!error.isError()) {
                return error;
            }
        }
    ...
    if (requestedDisplayType != EGL_PLATFORM_ANGLE_TYPE_OPENGL_ANGLE) {
        for (auto& version : esVersionsFrom2_0) {
            egl::Error error = createContextAttribs(config, version, GLX_CONTEXT_ES2_PROFILE_BIT_EXT, context);
            if (!error.isError()) {
                return error;
            }
        }
...
egl::ConfigSet DisplayGLX::generateConfigs() const
{
    ...
    int contextSampleBuffers = mHasMultisample ? getGLXFBConfigAttrib(mContextConfig, GLX_SAMPLE_BUFFERS) : 0;
    ...
    for (int i = 0; i < glxConfigCount; i++) {
        ...
        if (config.redSize != contextRedSize || config.greenSize != contextGreenSize || config.blueSize != contextBlueSize || config.alphaSize != contextAlphaSize) {
            continue;
        }
        ...
        // configuration.
        bool hasSameDepthStencil = config.depthSize == contextDepthSize && config.stencilSize == contextStencilSize;
        ...
        int sampleBuffers = mHasMultisample ? getGLXFBConfigAttrib(glxConfig, GLX_SAMPLE_BUFFERS) : 0;
        ...
        if (samples != contextSamples || sampleBuffers != contextSampleBuffers || accumRedSize != contextAccumRedSize || accumGreenSize != contextAccumGreenSize || accumBlueSize != contextAccumBlueSize || accumAlphaSize != contextAccumAlphaSize) {
            continue;
        }
        ...
        config.surfaceType = 0 | (glxDrawable & GLX_WINDOW_BIT ? EGL_WINDOW_BIT : 0) | (glxDrawable & GLX_PBUFFER_BIT ? EGL_PBUFFER_BIT : 0) | (glxDrawable & GLX_PIXMAP_BIT ? EGL_PIXMAP_BIT : 0);
