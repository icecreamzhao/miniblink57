    ...
    , mSkipDrawCalls(false)
{
    ...
    mHasDebugOutput = mFunctions->isAtLeastGL(gl::Version(4, 3)) || mFunctions->hasGLExtension("GL_KHR_debug") || mFunctions->isAtLeastGLES(gl::Version(3, 2)) || mFunctions->hasGLESExtension("GL_KHR_debug");
#ifndef NDEBUG
    if (mHasDebugOutput) {
    ...
    const gl::IndexRange& indexRange)
{
    ...
    gl::Error error = mStateManager->setDrawElementsState(data, count, type, indices, 0, &drawIndexPointer);
    if (error.isError()) {
        return error;
    }
...
std::string RendererGL::getRendererDescription() const
{
    ...
    if (mFunctions->standard == STANDARD_GL_DESKTOP) {
        ...
        if ((mFunctions->profile & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) != 0 || (mFunctions->isAtLeastGL(gl::Version(3, 2)) && mFunctions->profile == 0)) {
