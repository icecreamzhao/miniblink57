    ...
    size_t* outMaxAttributeDataSize) const
{
    ...
    *outMaxAttributeDataSize = 0;
    ...
    for (auto idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask)) {
        ...
        *outStreamingDataSize += typeSize * ComputeVertexAttributeElementCount(attrib, indexRange.vertexCount(), instanceCount);
    ...
    const gl::IndexRange& indexRange) const
{
    ...
    while (unmapResult != GL_TRUE && --unmapRetryAttempts > 0) {
        ...
        for (auto idx : angle::IterateBitSet(mAttributesNeedStreaming & activeAttributesMask)) {
            ...
            const size_t streamedVertexCount = ComputeVertexAttributeElementCount(attrib, indexRange.vertexCount(), instanceCount);
            ...
            if (destStride == sourceStride) {
                ...
                for (size_t vertexIdx = 0; vertexIdx < streamedVertexCount; vertexIdx++) {
                    ...
                    const uint8_t* in = inputPointer + sourceStride * (vertexIdx + indexRange.start);
...
void VertexArrayGL::syncState(const VertexArray::DirtyBits& dirtyBits)
{
    for (unsigned long dirtyBit : angle::IterateBitSet(dirtyBits)) {
        ...
        } else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_ENABLED && dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_ENABLED) {
            size_t attribIndex = static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_ENABLED;
            updateAttribEnabled(attribIndex);
        } else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_POINTER && dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_POINTER) {
            size_t attribIndex = static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_POINTER;
            updateAttribPointer(attribIndex);
        } else if (dirtyBit >= VertexArray::DIRTY_BIT_ATTRIB_0_DIVISOR && dirtyBit < VertexArray::DIRTY_BIT_ATTRIB_MAX_DIVISOR) {
            size_t attribIndex = static_cast<size_t>(dirtyBit) - VertexArray::DIRTY_BIT_ATTRIB_0_DIVISOR;
            const VertexAttribute& attrib = mData.getVertexAttribute(attribIndex);
