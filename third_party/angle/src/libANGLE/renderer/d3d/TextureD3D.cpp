    ...
    bool forceRelease)
{
    ...
    if (mTexStorage) {
        ...
        if ((level >= storageLevels && storageLevels != 0) || size.width != storageWidth || size.height != storageHeight || internalformat != storageFormat) // Discard mismatched storage
        {
...
{
    return (mTexStorage && index.type == GL_TEXTURE_2D && index.mipIndex >= 0 && index.mipIndex < mTexStorage->getLevelCount());
}
    ...
    const gl::Framebuffer* source)
{
    ...
    // so we should use the non-rendering copy path.
    if (!canCreateRenderTargetForImage(index) || mRenderer->getWorkarounds().zeroMaxLodWorkaround) {
        gl::Error error = mImageArray[faceIndex][level]->copyFromFramebuffer(destOffset, sourceArea, source);
        if (error.isError()) {
            return error;
        }
...
bool TextureD3D_Cube::isCubeComplete() const
{
    ...
    for (int faceIndex = 1; faceIndex < 6; faceIndex++) {
        ...
        if (faceBaseImage.getWidth() != baseWidth || faceBaseImage.getHeight() != baseHeight || faceBaseImage.getInternalFormat() != baseFormat) {
            return false;
        }
    }
...
{
    return (mTexStorage && gl::IsCubeMapTextureTarget(index.type) && index.mipIndex >= 0 && index.mipIndex < mTexStorage->getLevelCount());
}
...
gl::Error TextureD3D_3D::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents& size)
{
    ...
    // TODO(geofflang): Verify storage creation had no errors
    bool renderTarget = IsRenderTargetUsage(mUsage);
    TextureStorage* storage = mRenderer->createTextureStorage3D(internalFormat, renderTarget, size.width, size.height,
...
void TextureD3D_3D::redefineImage(GLint level, GLenum internalformat, const gl::Extents& size)
{
    ...
    if (mTexStorage) {
        ...
        if ((level >= storageLevels && storageLevels != 0) || size.width != storageWidth || size.height != storageHeight || size.depth != storageDepth || internalformat != storageFormat) // Discard mismatched storage
        {
            for (int i = 0; i < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; i++) {
                mImageArray[i]->markDirty();
            }
...
{
    return (mTexStorage && index.type == GL_TEXTURE_3D && index.mipIndex >= 0 && index.mipIndex < mTexStorage->getLevelCount());
}
...
{
    ASSERT(index.mipIndex < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS);
    ASSERT((index.layerIndex == 0 && mLayerCounts[index.mipIndex] == 0) || index.layerIndex < mLayerCounts[index.mipIndex]);
    ...
    const uint8_t* pixels)
{
    ...
    GLsizei inputDepthPitch = formatInfo.computeDepthPitch(GL_UNSIGNED_BYTE, size.width, size.height, 1, 0, 0);
    ...
    const gl::PixelUnpackState& unpack, size_t imageSize, const uint8_t* pixels)
{
    ...
    GLsizei inputDepthPitch = formatInfo.computeDepthPitch(GL_UNSIGNED_BYTE, area.width, area.height, 1, 0, 0);
...
gl::Error TextureD3D_2DArray::setStorage(GLenum target, size_t levels, GLenum internalFormat, const gl::Extents& size)
{
    ...
    // TODO(geofflang): Verify storage creation had no errors
    bool renderTarget = IsRenderTargetUsage(mUsage);
    TextureStorage* storage = mRenderer->createTextureStorage2DArray(internalFormat, renderTarget, size.width,
