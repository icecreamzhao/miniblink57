...
namespace gl_d3d9 {
    ...
    {
        return (red ? D3DCOLORWRITEENABLE_RED : 0) | (green ? D3DCOLORWRITEENABLE_GREEN : 0) | (blue ? D3DCOLORWRITEENABLE_BLUE : 0) | (alpha ? D3DCOLORWRITEENABLE_ALPHA : 0);
    }
...
namespace d3d9_gl {
        ...
        UINT adapter, D3DFORMAT adapterFormat)
    {
        ...
        if (d3dFormatInfo.texFormat != D3DFMT_UNKNOWN) {
            if (formatInfo.depthBits > 0 || formatInfo.stencilBits > 0) {
                ...
                textureCaps.texturable = SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, 0, D3DRTYPE_TEXTURE, d3dFormatInfo.texFormat)) && SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, adapterFormat, 0, D3DRTYPE_CUBETEXTURE, d3dFormatInfo.texFormat));
            }
        ...
        gl::Limitations* limitations)
    {
        ...
        if (deviceCaps.VertexShaderVersion >= D3DVS_VERSION(3, 0) && SUCCEEDED(d3d9->CheckDeviceFormat(adapter, deviceType, currentDisplayMode.Format, D3DUSAGE_QUERY_VERTEXTEXTURE, D3DRTYPE_TEXTURE, D3DFMT_R16F))) {
        ...
        if (SUCCEEDED(d3d9->GetAdapterIdentifier(adapter, 0, &adapterId))) {
            ...
            extensions->textureNPOT = !(deviceCaps.TextureCaps & D3DPTEXTURECAPS_POW2) && !(deviceCaps.TextureCaps & D3DPTEXTURECAPS_CUBEMAP_POW2) && !(deviceCaps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL) && !(!isWindowsVistaOrGreater() && adapterId.VendorId == VENDOR_ID_AMD);
...
namespace d3d9 {
    ...
    void MakeValidSize(bool isImage, D3DFORMAT format, GLsizei* requestWidth, GLsizei* requestHeight, int* levelOffset)
    {
        ...
        if (isImage || *requestWidth < static_cast<GLsizei>(d3dFormatInfo.blockWidth) || *requestHeight < static_cast<GLsizei>(d3dFormatInfo.blockHeight)) {
