...
gl::Error Image9::copyLockableSurfaces(IDirect3DSurface9* dest, IDirect3DSurface9* source)
{
    ...
    ASSERT(bytes <= static_cast<unsigned int>(sourceLock.Pitch) && bytes <= static_cast<unsigned int>(destLock.Pitch));
...
bool Image9::redefine(GLenum target, GLenum internalformat, const gl::Extents& size, bool forceRelease)
{
    ...
    if (mWidth != size.width || mHeight != size.height || mDepth != size.depth || mInternalFormat != internalformat || forceRelease) {
        mWidth = size.width;
        mHeight = size.height;
        mDepth = size.depth;
...
gl::Error Image9::setManagedSurfaceCube(TextureStorage* storage, int face, int level)
{
    IDirect3DSurface9* surface = NULL;
    TextureStorage9* storage9 = GetAs<TextureStorage9>(storage);
    gl::Error error = storage9->getSurfaceLevel(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level, false, &surface);
    if (error.isError()) {
        return error;
    }
    return setManagedSurface(surface);
