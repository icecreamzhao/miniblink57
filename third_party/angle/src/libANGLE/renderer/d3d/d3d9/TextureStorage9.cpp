...
gl::Error TextureStorage9_EGLImage::copyToStorage(TextureStorage* destStorage)
{
    ...
    error = mRenderer->copyToRenderTarget(destSurface, sourceRenderTarget9->getSurface(), isManaged());
    if (error.isError()) {
        SafeRelease(destSurface);
        return error;
    }
...
gl::Error TextureStorage9_Cube::copyToStorage(TextureStorage* destStorage)
{
    ...
    for (int f = 0; f < CUBE_FACE_COUNT; f++) {
        for (int i = 0; i < levels; i++) {
            IDirect3DSurface9* srcSurf = NULL;
            gl::Error error = getSurfaceLevel(GL_TEXTURE_CUBE_MAP_POSITIVE_X + f, i, false, &srcSurf);
            if (error.isError()) {
                return error;
            }
