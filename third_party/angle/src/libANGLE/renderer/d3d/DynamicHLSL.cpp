...
{
    std::string varyingSemantic = GetVaryingSemantic(mRenderer->getMajorShaderModel(), varyingPacking.usesPointSize());
    ...
    const std::vector<sh::Attribute>& shaderAttributes) const
{
    ...
    for (size_t attributeIndex = 0; attributeIndex < shaderAttributes.size(); ++attributeIndex) {
        ...
        if (!shaderAttribute.name.empty()) {
            ...
            VertexFormatType vertexFormatType = inputIndex < inputLayout.size() ? inputLayout[inputIndex] : VERTEX_FORMAT_INVALID;
            ...
            if (IsMatrixType(shaderAttribute.type)) {
                ...
                if (shaderAttribute.name == "gl_InstanceID") {
                    ...
                    structStream << "    " << HLSLComponentTypeString(componentType, VariableComponentCount(shaderAttribute.type));
                }
            }
            ...
            if (IsMatrixType(shaderAttribute.type) || (mRenderer->getVertexConversionType(vertexFormatType) & VERTEX_CONVERT_GPU) != 0) {
    ...
    std::string* vertexHLSL) const
{
    ...
    bool useInstancedPointSpriteEmulation = programMetadata.usesPointSize() && mRenderer->getWorkarounds().useInstancedPointSpriteEmulation;
    ...
    if (shaderModel >= 4 && mRenderer->getShaderModelSuffix() == "") {
        ...
        if (programMetadata.usesViewScale() && (shaderModel >= 4 && mRenderer->getShaderModelSuffix() != "")) {
            vertexStream << "    output.dx_Position.y = dx_ViewScale.y * (gl_Position.y * "
    ...
    std::vector<PixelShaderOutputVariable>* outPixelShaderKey)
{
    ...
    const unsigned int numRenderTargets = (broadcast || metadata.usesMultipleFragmentOuts() ? data.caps->maxDrawBuffers : 1);
    ...
    if (metadata.getMajorShaderVersion() < 300) {
        ...
        const auto& shaderOutputVars = metadata.getFragmentShader()->getData().getActiveOutputVariables();
