...
namespace {
    bool NeedsOffscreenTexture(Renderer11* renderer, NativeWindow nativeWindow, EGLint orientation)
    {
        ...
        return orientation != EGL_SURFACE_ORIENTATION_INVERT_Y_ANGLE && !(renderer->presentPathFastEnabled() && nativeWindow.getNativeWindow());
    }
} // anonymous namespace
...
EGLint SwapChain11::resetOffscreenColorBuffer(int backbufferWidth, int backbufferHeight)
{
    ...
    if (mAppCreatedShareHandle) {
        ...
        if (offscreenTextureDesc.Width != (UINT)backbufferWidth || offscreenTextureDesc.Height != (UINT)backbufferHeight || offscreenTextureDesc.Format != backbufferFormatInfo.texFormat || offscreenTextureDesc.MipLevels != 1 || offscreenTextureDesc.ArraySize != 1) {
...
EGLint SwapChain11::resetOffscreenDepthBuffer(int backbufferWidth, int backbufferHeight)
{
    ...
    if (mDepthBufferFormat != GL_NONE) {
        ...
        ID3D11Device* device = mRenderer->getDevice();
        HRESULT result = device->CreateTexture2D(&depthStencilTextureDesc, NULL, &mDepthStencilTexture);
        if (FAILED(result)) {
