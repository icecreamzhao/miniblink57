...
{
    return swizzleRed == other.swizzleRed && swizzleGreen == other.swizzleGreen && swizzleBlue == other.swizzleBlue && swizzleAlpha == other.swizzleAlpha;
}
    ...
    const gl::Box& copyArea)
{
    ...
    bool fullCopy = copyArea.x == 0 && copyArea.y == 0 && copyArea.z == 0 && copyArea.width == texSize.width && copyArea.height == texSize.height && copyArea.depth == texSize.depth;
    ...
    const uint8_t* pixelData)
{
    ...
    const UINT srcRowPitch = internalFormatInfo.computeRowPitch(type, width, unpack.alignment, unpack.rowLength);
...
gl::Error TextureStorage11_2D::getRenderTarget(const gl::ImageIndex& index, RenderTargetD3D** outRT)
{
    ...
    if (mUseLevelZeroTexture) {
        if (!mLevelZeroRenderTarget) {
            ...
            mLevelZeroRenderTarget = new TextureRenderTarget11(rtv, mLevelZeroTexture, nullptr, mInternalFormat,
    ...
    ASSERT(mDepthStencilFormat != DXGI_FORMAT_UNKNOWN);
    ...
    Image11* incomingImage)
{
    ...
    if ((0 <= level && level < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS)) {
        if (0 <= layerTarget && layerTarget < CUBE_FACE_COUNT) {
            // No need to let the old Image recover its data, if it is also the incoming Image.
            if (mAssociatedImages[layerTarget][level] != nullptr && mAssociatedImages[layerTarget][level] != incomingImage) {
    ...
    if (!mRenderTarget[faceIndex][level]) {
        ...
        if (mUseLevelZeroTexture) {
            if (!mLevelZeroRenderTarget[faceIndex]) {
                ...
                mLevelZeroRenderTarget[faceIndex] = new TextureRenderTarget11(rtv, mLevelZeroTexture, nullptr, mInternalFormat,
                    getLevelWidth(level), getLevelHeight(level), 1, 0);
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN) {
            ...
            mRenderTarget[faceIndex][level] = new TextureRenderTarget11(rtv, texture, srv, mInternalFormat, getLevelWidth(level),
            ...
            mRenderTarget[faceIndex][level] = new TextureRenderTarget11(dsv, texture, srv, mInternalFormat, getLevelWidth(level),
                getLevelHeight(level), 1, 0);
    ...
    if (!index.hasLayer()) {
        ...
        *outRT = mLevelRenderTargets[mipLevel];
        ...
        if (mLevelLayerRenderTargets.find(key) == mLevelLayerRenderTargets.end()) {
            ...
            mLevelLayerRenderTargets[key] = new TextureRenderTarget11(rtv, texture, srv, mInternalFormat,
    ...
    RenderTargetD3D** outRT)
{
    ...
    if (mRenderTargets.find(key) == mRenderTargets.end()) {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN) {
            ...
            mRenderTargets[key] = new TextureRenderTarget11(rtv, texture, srv, mInternalFormat,
                getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1, 0);
