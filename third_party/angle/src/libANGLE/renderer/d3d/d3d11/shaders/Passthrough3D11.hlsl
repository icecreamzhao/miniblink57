...
[maxvertexcount(3)] void GS_Passthrough3D(triangle VS_OUTPUT input[3], inout TriangleStream<GS_OUTPUT> outputStream) {
    GS_OUTPUT output;
...
float4 PS_PassthroughRGBA3D(GS_OUTPUT input)
    : SV_TARGET0
{
    return TextureF.Sample(Sampler, input.TexCoord).rgba;
}

uint4 PS_PassthroughRGBA3DUI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
...
int4 PS_PassthroughRGBA3DI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
...
float4 PS_PassthroughRGB3D(GS_OUTPUT input)
    : SV_TARGET0
{
    return float4(TextureF.Sample(Sampler, input.TexCoord).rgb, 1.0f);
}

uint4 PS_PassthroughRGB3DUI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureUI.GetDimensions(size.x, size.y, size.z);
...
int4 PS_PassthroughRGB3DI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureI.GetDimensions(size.x, size.y, size.z);
...
float4 PS_PassthroughRG3D(GS_OUTPUT input)
    : SV_TARGET0
{
    return float4(TextureF.Sample(Sampler, input.TexCoord).rg, 0.0f, 1.0f);
}

uint4 PS_PassthroughRG3DUI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureUI.GetDimensions(size.x, size.y, size.z);
...
int4 PS_PassthroughRG3DI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureI.GetDimensions(size.x, size.y, size.z);
...
float4 PS_PassthroughR3D(GS_OUTPUT input)
    : SV_TARGET0
{
    return float4(TextureF.Sample(Sampler, input.TexCoord).r, 0.0f, 0.0f, 1.0f);
}

uint4 PS_PassthroughR3DUI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureUI.GetDimensions(size.x, size.y, size.z);
...
int4 PS_PassthroughR3DI(GS_OUTPUT input)
    : SV_TARGET0
{
    uint3 size;
    TextureI.GetDimensions(size.x, size.y, size.z);
...
float4 PS_PassthroughLum3D(GS_OUTPUT input)
    : SV_TARGET0
{
    return float4(TextureF.Sample(Sampler, input.TexCoord).rrr, 1.0f);
}

float4 PS_PassthroughLumAlpha3D(GS_OUTPUT input)
    : SV_TARGET0
{
