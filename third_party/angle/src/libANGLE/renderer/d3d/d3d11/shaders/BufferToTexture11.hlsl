...
void VS_BufferToTexture(in uint vertexID
                        : SV_VertexID, out VS_OUTPUT outVertex)
{
    ComputePositionAndIndex(vertexID, outVertex);
}

[maxvertexcount(1)] void GS_BufferToTexture(point VS_OUTPUT inVertex[1], inout PointStream<GS_OUTPUT> outStream) {
    GS_OUTPUT outVertex;
...
float4 PS_BufferToTexture_4F(in float4 inPosition
                             : SV_Position, in uint inIndex
                             : TEXCOORD0)
    : SV_Target
{
...
int4 PS_BufferToTexture_4I(in float4 inPosition
                           : SV_Position, in uint inIndex
                           : TEXCOORD0)
    : SV_Target
{
...
uint4 PS_BufferToTexture_4UI(in float4 inPosition
                             : SV_Position, in uint inIndex
                             : TEXCOORD0)
    : SV_Target
{
