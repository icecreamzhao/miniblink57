...
gl::Error Query11::testQuery(QueryState* queryState)
{
    if (!queryState->finished) {
        ...
        case GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT: {
            ...
            UINT64 numPixels = 0;
            HRESULT result = context->GetData(queryState->query, &numPixels, sizeof(numPixels), 0);
            if (FAILED(result)) {
        ...
        case GL_TIME_ELAPSED_EXT: {
            ...
            HRESULT result = context->GetData(queryState->query, &timeStats, sizeof(timeStats), 0);
            if (FAILED(result)) {
            ...
            if (result == S_OK) {
                UINT64 beginTime = 0;
                HRESULT beginRes = context->GetData(queryState->beginTimestamp, &beginTime, sizeof(UINT64), 0);
                if (FAILED(beginRes)) {
                    return gl::Error(
                        GL_OUT_OF_MEMORY,
                ...
                UINT64 endTime = 0;
                HRESULT endRes = context->GetData(queryState->endTimestamp, &endTime, sizeof(UINT64), 0);
                if (FAILED(endRes)) {
                    return gl::Error(
                        GL_OUT_OF_MEMORY,
