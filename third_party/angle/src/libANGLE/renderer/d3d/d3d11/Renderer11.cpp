...
egl::ConfigSet Renderer11::generateConfigs() const
{
    ...
    const EGLint optimalSurfaceOrientation = mPresentPathFastEnabled ? 0 : EGL_SURFACE_ORIENTATION_INVERT_Y_ANGLE;
...
gl::Error Renderer11::generateSwizzle(gl::Texture* texture)
{
    if (texture) {
        ...
        if (texStorage) {
            ...
            error = storage11->generateSwizzles(textureState.swizzleRed, textureState.swizzleGreen,
...
gl::Error Renderer11::setTexture(gl::SamplerType type, int index, gl::Texture* texture)
{
    ...
    ASSERT((type == gl::SAMPLER_PIXEL && static_cast<unsigned int>(index) < getRendererCaps().maxTextureImageUnits) || (type == gl::SAMPLER_VERTEX && static_cast<unsigned int>(index) < getRendererCaps().maxVertexTextureImageUnits));
    ...
    const std::vector<GLint>& fragmentUniformBuffers)
{
    for (size_t uniformBufferIndex = 0; uniformBufferIndex < vertexUniformBuffers.size(); uniformBufferIndex++) {
        ...
        const OffsetBindingPointer<gl::Buffer>& uniformBuffer = data.state->getIndexedUniformBuffer(binding);
        ...
        if (uniformBuffer.get() != nullptr) {
            ...
            if (mCurrentConstantBufferVS[uniformBufferIndex] != bufferStorage->getSerial() || mCurrentConstantBufferVSOffset[uniformBufferIndex] != uniformBufferOffset || mCurrentConstantBufferVSSize[uniformBufferIndex] != uniformBufferSize) {
                if (mRenderer11DeviceCaps.supportsConstantBufferOffsets && uniformBufferSize != 0) {
                    ...
                    mDeviceContext1->VSSetConstantBuffers1(
                        getReservedVertexUniformBuffers() + static_cast<unsigned int>(uniformBufferIndex),
    ...
    for (size_t uniformBufferIndex = 0; uniformBufferIndex < fragmentUniformBuffers.size(); uniformBufferIndex++) {
        ...
        if (uniformBuffer.get() != nullptr) {
            ...
            if (mCurrentConstantBufferPS[uniformBufferIndex] != bufferStorage->getSerial() || mCurrentConstantBufferPSOffset[uniformBufferIndex] != uniformBufferOffset || mCurrentConstantBufferPSSize[uniformBufferIndex] != uniformBufferSize) {
                if (mRenderer11DeviceCaps.supportsConstantBufferOffsets && uniformBufferSize != 0) {
                    UINT firstConstant = 0, numConstants = 0;
                    CalculateConstantBufferParams(uniformBufferOffset, uniformBufferSize, &firstConstant, &numConstants);
                    mDeviceContext1->PSSetConstantBuffers1(
                        getReservedFragmentUniformBuffers() + static_cast<unsigned int>(uniformBufferIndex),
                        1, &constantBuffer, &firstConstant, &numConstants);
                } else {
...
gl::Error Renderer11::updateState(const gl::Data& data, GLenum drawMode)
{
    ...
    const bool presentPathFastActive = UsePresentPathFast(this, framebufferObject->getFirstColorbuffer());
    ...
    if (requiresUpdate || numXFBBindings != mAppliedNumXFBBindings) {
        ...
        for (size_t i = 0; i < numXFBBindings; ++i) {
            ...
            if (binding.get() != nullptr) {
                ...
                mCurrentD3DOffsets[i] = (mAppliedTFBuffers[i] != d3dBuffer || mAppliedTFOffsets[i] != binding.getOffset()) ? static_cast<UINT>(binding.getOffset()) : -1;
    ...
    GLsizei instances)
{
    ...
    if (programD3D->usesGeometryShader(mode) && data.state->isTransformFeedbackActiveUnpaused()) {
        ...
        error = programD3D->getGeometryExecutableForPrimitiveType(data, mode, &geometryExe, nullptr);
        if (error.isError()) {
            return error;
        }
    ...
    bool useInstancedPointSpriteEmulation = programD3D->usesPointSize() && getWorkarounds().useInstancedPointSpriteEmulation;
    ...
    int instances)
{
    ...
    unsigned int spaceNeeded = static_cast<unsigned int>(sizeof(GLuint) * mScratchIndexDataBuffer.size());
    ...
    if (mAppliedIB != d3dIndexBuffer || mAppliedIBFormat != indexFormat || mAppliedIBOffset != offset) {
    ...
    int instances)
{
    ...
    const unsigned int spaceNeeded = static_cast<unsigned int>(mScratchIndexDataBuffer.size() * sizeof(unsigned int));
...
gl::Error Renderer11::applyShadersImpl(const gl::Data& data, GLenum drawMode)
{
    ...
    error = programD3D->getGeometryExecutableForPrimitiveType(data, drawMode, &geometryExe, nullptr);
    if (error.isError()) {
        return error;
    }
    ...
    const std::vector<D3DUniform*>& uniformArray)
{
    ...
    if (!mDriverConstantBufferVS) {
        ...
        HRESULT result = mDevice->CreateBuffer(&constantBufferDescription, nullptr, &mDriverConstantBufferVS);
        ASSERT(SUCCEEDED(result));
        if (FAILED(result)) {
    ...
    if (!mDriverConstantBufferPS) {
        ...
        HRESULT result = mDevice->CreateBuffer(&constantBufferDescription, nullptr, &mDriverConstantBufferPS);
        ASSERT(SUCCEEDED(result));
        if (FAILED(result)) {
    ...
    size_t samplerMetadataReferencedBytesVS = sizeof(SamplerMetadataD3D11::dx_SamplerMetadata) * programD3D.getUsedSamplerRange(gl::SAMPLER_VERTEX);
    ...
    size_t samplerMetadataReferencedBytesPS = sizeof(SamplerMetadataD3D11::dx_SamplerMetadata) * programD3D.getUsedSamplerRange(gl::SAMPLER_PIXEL);
    ...
    GLenum internalFormat)
{
    ...
    if (needInternalFormatBits && mSamplerMetadata[samplerIndex].parameters[1] != internalFormatBits) {
    ...
    ID3D11Buffer* driverConstantBuffer)
{
    ...
    if (memcmp(appliedConstants, &constants, sizeof(TShaderConstants)) != 0 || samplerMetadata->isDirty()) {
        memcpy(appliedConstants, &constants, sizeof(TShaderConstants));

        D3D11_MAPPED_SUBRESOURCE mapping = { 0 };
        HRESULT result = mDeviceContext->Map(driverConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapping);
        ASSERT(SUCCEEDED(result));
        UNUSED_ASSERTION_VARIABLE(result);
...
d3d11::ANGLED3D11DeviceType Renderer11::getDeviceType() const
{
    ...
    if ((mRequestedDriverType == D3D_DRIVER_TYPE_SOFTWARE) || (mRequestedDriverType == D3D_DRIVER_TYPE_REFERENCE) || (mRequestedDriverType == D3D_DRIVER_TYPE_NULL)) {
...
gl::Error Renderer11::createRenderTarget(int width, int height, GLenum format, GLsizei samples, RenderTargetD3D** outRT)
{
    ...
    if (width > 0 && height > 0) {
        ...
        desc.BindFlags = (bindRTV ? D3D11_BIND_RENDER_TARGET : 0) | (bindDSV ? D3D11_BIND_DEPTH_STENCIL : 0) | (bindSRV ? D3D11_BIND_SHADER_RESOURCE : 0);
    ...
    uint8_t* pixelsOut)
{
    ...
    if (sourceFormatInfo.format == params.format && sourceFormatInfo.type == params.type) {
        ...
        if (fastCopyFunc) {
            ...
            static_assert(sizeof(temp) >= sizeof(gl::ColorF) && sizeof(temp) >= sizeof(gl::ColorUI) && sizeof(temp) >= sizeof(gl::ColorI),
    ...
    bool stencilBlit)
{
    ...
    auto readToDrawX = [&drawRectIn, &readRectIn](int readOffset) {
        double readToDrawScale = static_cast<double>(drawRectIn.width) / static_cast<double>(readRectIn.width);
    ...
    auto readToDrawY = [&drawRectIn, &readRectIn](int readOffset) {
        double readToDrawScale = static_cast<double>(drawRectIn.height) / static_cast<double>(readRectIn.height);
        return static_cast<int>(round(static_cast<double>(readOffset) * readToDrawScale));
    };
    if (readRect.y < 0) {
    ...
    colorMask.red = (srcFormatInfo.redBits > 0) && (destFormatInfo.redBits == 0) && (dxgiFormatInfo.redBits > 0);
    colorMask.green = (srcFormatInfo.greenBits > 0) && (destFormatInfo.greenBits == 0) && (dxgiFormatInfo.greenBits > 0);
    colorMask.blue = (srcFormatInfo.blueBits > 0) && (destFormatInfo.blueBits == 0) && (dxgiFormatInfo.blueBits > 0);
    colorMask.alpha = (srcFormatInfo.alphaBits > 0) && (destFormatInfo.alphaBits == 0) && (dxgiFormatInfo.alphaBits > 0);
    ...
    bool wholeBufferCopy = !scissorNeeded && !colorMaskingNeeded && readRect.x == 0 && readRect.width == readSize.width && readRect.y == 0 && readRect.height == readSize.height && drawRect.x == 0 && drawRect.width == drawSize.width && drawRect.y == 0 && drawRect.height == drawSize.height;

    bool stretchRequired = readRect.width != drawRect.width || readRect.height != drawRect.height;

    bool flipRequired = readRect.width < 0 || readRect.height < 0 || drawRect.width < 0 || drawRect.height < 0;

    bool outOfBounds = readRect.x < 0 || readRect.x + readRect.width > readSize.width || readRect.y < 0 || readRect.y + readRect.height > readSize.height || drawRect.x < 0 || drawRect.x + drawRect.width > drawSize.width || drawRect.y < 0 || drawRect.y + drawRect.height > drawSize.height;
    ...
    if (readRenderTarget11->getDXGIFormat() == drawRenderTarget11->getDXGIFormat() && !stretchRequired && !outOfBounds && !flipRequired && !partialDSBlit && !colorMaskingNeeded && (!(depthBlit || stencilBlit) || wholeBufferCopy)) {
