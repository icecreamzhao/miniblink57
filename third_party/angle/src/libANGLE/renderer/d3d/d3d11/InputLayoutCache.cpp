    ...
    GLsizei numIndicesPerInstance)
{
    ...
    if (mFeatureLevel <= D3D_FEATURE_LEVEL_9_3 && !unsortedAttributes.empty()) {
        if (mSortedAttributes[0]->divisor > 0) {
            Optional<size_t> firstNonInstancedIndex = FindFirstNonInstanced(mSortedAttributes, unsortedAttributes.size());
    ...
    for (size_t attribIndex = 0; attribIndex < (gl::MAX_VERTEX_ATTRIBS - reservedBuffers);
        ...
        if (attribIndex < unsortedAttributes.size() && attrib.active) {
            ...
            if (bufferStorage == nullptr) {
                ...
                if (indexInfo->srcIndexData.srcBuffer != nullptr) {
                    ...
                    ptrdiff_t offset = reinterpret_cast<ptrdiff_t>(indexInfo->srcIndexData.srcIndices);
        ...
        if (buffer != mCurrentBuffers[bufferIndex] || vertexStride != mCurrentVertexStrides[bufferIndex] || vertexOffset != mCurrentVertexOffsets[bufferIndex]) {
            dirtyBuffers = true;
...
gl::Error InputLayoutCache::updateVertexOffsetsForPointSpritesEmulation(GLsizei emulatedInstanceId)
{
    ...
    for (size_t attribIndex = 0; attribIndex < mUnsortedAttributesCount; ++attribIndex) {
        ...
        if (attrib.active && attrib.divisor > 0) {
            mCurrentVertexOffsets[bufferIndex] = attrib.offset + (attrib.stride * (emulatedInstanceId / attrib.divisor));
        }
    }
    ...
    GLsizei numIndicesPerInstance)
{
    ...
    for (size_t attribIndex = 0; attribIndex < attribCount; ++attribIndex) {
        ...
        gl::VertexFormatType vertexFormatType = gl::GetVertexFormatType(*attrib.attribute, attrib.currentValueType);

        // Record the type of the associated vertex shader vector in our key
        // This will prevent mismatched vertex shaders from using the same input layout
        GLenum glslElementType = GetGLSLAttributeType(shaderAttributes, semanticToLocation[sortedIndex]);
    ...
    if (layout.numAttributes > 0 || layout.flags != 0) {
        ...
        if (layoutMapIt != mLayoutMap.end()) {
            ...
            gl::Error error = createInputLayout(sortedAttributes, sortedSemanticIndices, attribCount, mode,
    ...
    ID3D11InputLayout** inputLayoutOut)
{
    ...
    for (size_t attribIndex = 0; attribIndex < attribCount; ++attribIndex) {
        ...
        D3D11_INPUT_CLASSIFICATION inputClass = attrib.divisor > 0 ? D3D11_INPUT_PER_INSTANCE_DATA : D3D11_INPUT_PER_VERTEX_DATA;

        const auto& vertexFormatType = gl::GetVertexFormatType(*attrib.attribute, attrib.currentValueType);
    ...
    HRESULT result = mDevice->CreateInputLayout(inputElements.data(), inputElementCount, shader11->getFunction(),
