...
gl::Error Blit11::initResources()
{
    ...
    vbDesc.ByteWidth = static_cast<unsigned int>(std::max(sizeof(d3d11::PositionLayerTexCoord3DVertex),
    ...
    const gl::Rectangle* scissor, bool stencilOnly)
{
    ...
    if (stencilOnly) {
        ...
        ASSERT(dxgiFormatInfo.stencilBits % 8 == 0 && dxgiFormatInfo.depthBits % 8 == 0);
    }
    ...
    bool wholeRowCopy = sourceArea.width == clippedDestArea.width && sourceArea.x >= 0 && sourceArea.x + sourceArea.width <= sourceSize.width && copySize == pixelSize;
    ...
    for (int y = clippedDestArea.y; y < clippedDestArea.y + clippedDestArea.height; y++) {
        ...
        if (wholeRowCopy) {
            void* sourceRow = reinterpret_cast<char*>(sourceMapping.pData) + readRow * sourceMapping.RowPitch + sourceArea.x * pixelSize;

            void* destRow = reinterpret_cast<char*>(destMapping.pData) + writeRow * destMapping.RowPitch + destArea.x * pixelSize;
            ...
            for (int x = clippedDestArea.x; x < clippedDestArea.x + clippedDestArea.width; x++) {
                ...
                void* sourcePixel = reinterpret_cast<char*>(sourceMapping.pData) + readRow * sourceMapping.RowPitch + readColumn * pixelSize + copyOffset;

                void* destPixel = reinterpret_cast<char*>(destMapping.pData) + writeRow * destMapping.RowPitch + writeColumn * pixelSize + copyOffset;
