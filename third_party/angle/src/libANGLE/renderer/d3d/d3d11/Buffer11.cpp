...
gl::Error Buffer11::mapRange(size_t offset, size_t length, GLbitfield access, GLvoid** mapPtr)
{
    ...
    if (latestStorage && (latestStorage->getUsage() == BUFFER_USAGE_PIXEL_PACK || latestStorage->getUsage() == BUFFER_USAGE_STAGING)) {
...
ID3D11ShaderResourceView* Buffer11::getSRV(DXGI_FORMAT srvFormat)
{
    ...
    bufferSRVDesc.Buffer.ElementWidth = static_cast<unsigned int>(mSize) / dxgiFormatInfo.pixelBytes;
...
void Buffer11::updateBufferStorage(BufferStorage* storage, size_t sourceOffset, size_t storageSize)
{
    ...
    if (latestBuffer && latestBuffer->getDataRevision() > storage->getDataRevision()) {
        ...
        if (latestBuffer->getUsage() != BUFFER_USAGE_STAGING && storage->getUsage() != BUFFER_USAGE_STAGING && (!latestBuffer->isMappable() || !storage->isMappable())) {
    ...
    size_t destOffset)
{
    ...
    if (source->getUsage() == BUFFER_USAGE_PIXEL_PACK || source->getUsage() == BUFFER_USAGE_SYSTEM_MEMORY) {
    ...
    unsigned int bufferSize)
{
    ...
    case BUFFER_USAGE_UNIFORM:
        ...
        bufferDesc->ByteWidth = std::min<UINT>(bufferDesc->ByteWidth,
...
ID3D11Buffer* Buffer11::EmulatedIndexedStorage::getNativeStorage()
{
    if (!mNativeStorage) {
        ...
        unsigned int expandedDataSize = static_cast<unsigned int>((mIndexInfo.srcCount * mAttributeStride) + mAttributeOffset);
    ...
    const PackPixelsParams& params)
{
    ...
    if (!mStagingTexture.getResource() || mStagingTexture.getFormat() != srcTexture.getFormat() || mStagingTexture.getExtents() != srcTextureSize) {
        auto textureOrError = CreateStagingTexture(srcTexture.getTextureType(), srcTexture.getFormat(),
