...
{
    if (mWidth != size.width || mHeight != size.height || mInternalFormat != internalformat || forceRelease) {
...
gl::Error Image11::loadCompressedData(const gl::Box& area, const void* input)
{
    ...
    uint8_t* offsetMappedData = reinterpret_cast<uint8_t*>(mappedImage.pData) + ((area.y / outputBlockHeight) * mappedImage.RowPitch + (area.x / outputBlockWidth) * outputPixelSize + area.z * mappedImage.DepthPitch);
    ...
    UINT sourceSubResource)
{
    ...
    bool needResolve = (textureHelper.getTextureType() == GL_TEXTURE_2D && textureHelper.getSampleCount() > 1);
