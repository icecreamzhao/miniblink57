...
static int ElementsInBuffer(const gl::VertexAttribute& attrib, unsigned int size)
{
    ...
    return (size - attrib.offset % stride + (stride - static_cast<GLsizei>(ComputeVertexAttributeTypeSize(attrib)))) / stride;
}
...
void VertexDataManager::hintUnmapAllResources(const std::vector<gl::VertexAttribute>& vertexAttributes)
{
    ...
    for (const TranslatedAttribute* translated : mActiveEnabledAttributes) {
        ...
        StaticVertexBufferInterface* staticBuffer = storage
    ...
    GLsizei instances)
{
    ...
    for (size_t attribIndex = 0; attribIndex < vertexAttributes.size(); ++attribIndex) {
        if (program->isAttribLocationActive(attribIndex)) {
            ...
            translated->currentValueType = state.getVertexAttribCurrentValue(static_cast<unsigned int>(attribIndex)).Type;
    ...
    GLsizei instances) const
{
    ...
    StaticVertexBufferInterface* staticBuffer = bufferImpl ? bufferImpl->getStaticVertexBuffer(attrib, D3D_BUFFER_CREATE_IF_NECESSARY)
                                                           : NULL;
    ...
    if (!vertexBuffer->directStoragePossible(attrib, translatedAttrib.currentValueType)) {
        if (staticBuffer) {
            if (staticBuffer->getBufferSize() == 0) {
                int totalCount = ElementsInBuffer(attrib, static_cast<unsigned int>(bufferImpl->getSize()));
            ...
            ASSERT(!bufferImpl || ElementsInBuffer(attrib, static_cast<unsigned int>(bufferImpl->getSize())) >= static_cast<int>(totalCount));
    ...
    GLsizei instances)
{
    ...
    StaticVertexBufferInterface* staticBuffer = storage ? storage->getStaticVertexBuffer(attrib, D3D_BUFFER_DO_NOT_CREATE) : NULL;
    VertexBufferInterface* vertexBuffer = staticBuffer ? staticBuffer : static_cast<VertexBufferInterface*>(mStreamingBuffer);
    ...
    if (staticBuffer) {
        ...
        if (!staticBuffer->lookupAttribute(attrib, &streamOffset)) {
            ...
            int totalCount = ElementsInBuffer(attrib, static_cast<unsigned int>(storage->getSize()));
            int startIndex = static_cast<int>(attrib.offset) / static_cast<int>(ComputeVertexAttributeStride(attrib));
        ...
        unsigned int firstElementOffset = (static_cast<unsigned int>(attrib.offset) / static_cast<unsigned int>(ComputeVertexAttributeStride(attrib))) * outputElementSize;
