...
namespace {
    ...
    ClearParameters GetClearParameters(const gl::State& state, GLbitfield mask)
    {
        ...
        if (mask & GL_STENCIL_BUFFER_BIT) {
            if (framebufferObject->getStencilbuffer() != NULL && framebufferObject->getStencilbuffer()->getStencilSize() > 0) {
...
gl::Error FramebufferD3D::readPixels(const gl::State& state, const gl::Rectangle& area, GLenum format, GLenum type, GLvoid* pixels) const
{
    ...
    GLuint outputPitch = sizedFormatInfo.computeRowPitch(type, area.width, packState.alignment, packState.rowLength);
    ...
    GLbitfield mask, GLenum filter, const gl::Framebuffer* sourceFramebuffer)
{
    ...
    if ((mask & GL_COLOR_BUFFER_BIT) && sourceFramebuffer->getReadColorbuffer() != nullptr && mData.getFirstColorAttachment() != nullptr) {
        blitRenderTarget = true;
    }

    bool blitStencil = false;
    if ((mask & GL_STENCIL_BUFFER_BIT) && sourceFramebuffer->getStencilbuffer() != nullptr && mData.getStencilAttachment() != nullptr) {
        blitStencil = true;
    }

    bool blitDepth = false;
    if ((mask & GL_DEPTH_BUFFER_BIT) && sourceFramebuffer->getDepthbuffer() != nullptr && mData.getDepthAttachment() != nullptr) {
        blitDepth = true;
    }
...
bool FramebufferD3D::checkStatus() const
{
    ...
    if (mData.getDepthAttachment() != nullptr && mData.getStencilAttachment() != nullptr && mData.getDepthStencilAttachment() == nullptr) {
        return false;
    }
    ...
    for (size_t colorAttachment = 0; colorAttachment < colorAttachments.size(); colorAttachment++) {
        ...
        if (attachment.isAttached()) {
            for (size_t prevColorAttachment = 0; prevColorAttachment < colorAttachment; prevColorAttachment++) {
                ...
                if (prevAttachment.isAttached() && (attachment.id() == prevAttachment.id() && attachment.type() == prevAttachment.type())) {
                    return false;
                }
            }
        }
...
void FramebufferD3D::syncState(const gl::Framebuffer::DirtyBits& dirtyBits)
{
    ...
    for (auto dirtyBit : angle::IterateBitSet(dirtyBits)) {
        if ((dirtyBit >= gl::Framebuffer::DIRTY_BIT_COLOR_ATTACHMENT_0 && dirtyBit < gl::Framebuffer::DIRTY_BIT_COLOR_ATTACHMENT_MAX) || dirtyBit == gl::Framebuffer::DIRTY_BIT_DRAW_BUFFERS) {
            invalidateColorAttachmentCache = true;
        }
    }
