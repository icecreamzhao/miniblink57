    ...
    const uint8_t* sourceData)
{
    ...
    if (!IsUnsignedAdditionSafe(mWritePosition, alignedSpaceRequired) || alignedSpaceRequired < spaceRequired) {
...
gl::Error VertexBufferInterface::reserveVertexSpace(const gl::VertexAttribute& attrib, GLsizei count, GLsizei instances)
{
    ...
    // Protect against integer overflow
    if (!IsUnsignedAdditionSafe(mReservedSpace, alignedRequiredSpace) || alignedRequiredSpace < requiredSpace) {
    ...
    GLenum currentValueType) const
{
    ...
    bool isAligned = (static_cast<size_t>(ComputeVertexAttributeStride(attrib)) % alignment == 0) && (static_cast<size_t>(attrib.offset) % alignment == 0);
...
bool StaticVertexBufferInterface::lookupAttribute(const gl::VertexAttribute& attrib, unsigned int* outStreamOffset)
{
    for (unsigned int element = 0; element < mCache.size(); element++) {
        ...
        if (mCache[element].type == attrib.type && mCache[element].size == attrib.size && mCache[element].stride == attribStride && mCache[element].normalized == attrib.normalized && mCache[element].pureInteger == attrib.pureInteger) {
