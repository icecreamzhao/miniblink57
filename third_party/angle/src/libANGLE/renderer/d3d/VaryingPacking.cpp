...
VaryingPacking::BuiltinVarying::BuiltinVarying()
    : enabled(false)
    , index(0)
    , systemValue(false)
{
...
VaryingPacking::VaryingPacking(GLuint maxVaryingVectors)
    : mRegisterMap(maxVaryingVectors)
    , mBuiltinInfo(SHADER_TYPE_MAX)
{
}
...
bool VaryingPacking::packVarying(const PackedVarying& packedVarying)
{
    ...
    for (unsigned int column = 1; column < 4; ++column) {
        if (bestContiguousSpace[column] >= varyingRows && (bestContiguousSpace[bestColumn] < varyingRows || totalSpace[column] < totalSpace[bestColumn])) {
            bestColumn = column;
        }
    }
