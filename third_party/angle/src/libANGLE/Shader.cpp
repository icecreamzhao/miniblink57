...
void Shader::compile(Compiler* compiler)
{
    ...
    int additionalOptions = mImplementation->prepareSourceAndReturnOptions(&sourceStream, &sourcePath);
    ...
    bool result = ShCompile(compilerHandle, &sourceCStrings[0], sourceCStrings.size(), compileOptions);
    ...
    if (mData.mShaderType == GL_VERTEX_SHADER) {
        ...
        mData.mActiveOutputVariables = GetActiveShaderVariables(ShGetOutputVariables(compilerHandle));
    }
