...
GLint Program::Data::getUniformLocation(const std::string& name) const
{
    ...
    for (size_t location = 0; location < mUniformLocations.size(); ++location) {
        ...
        if (uniform.name == baseName) {
            if ((uniform.isArray() && uniformLocation.element == subscript) || (subscript == GL_INVALID_INDEX)) {
...
GLint Program::getActiveUniformi(GLuint index, GLenum pname) const
{
    ...
    case GL_UNIFORM_TYPE:
        return static_cast<GLint>(uniform.type);
    case GL_UNIFORM_SIZE:
        return static_cast<GLint>(uniform.elementCount());
    case GL_UNIFORM_NAME_LENGTH:
        return static_cast<GLint>(uniform.name.size() + 1 + (uniform.isArray() ? 3 : 0));
...
{
    ASSERT(uniformBlockIndex < mData.mUniformBlocks.size()); // index must be smaller than getActiveUniformBlockCount()
...
void Program::getActiveUniformBlockiv(GLuint uniformBlockIndex, GLenum pname, GLint* params) const
{
    ...
    case GL_UNIFORM_BLOCK_NAME_LENGTH:
        *params = static_cast<GLint>(uniformBlock.name.size() + 1 + (uniformBlock.isArray ? 3 : 0));
        break;
...
GLuint Program::getUniformBlockIndex(const std::string& name) const
{
    ...
    for (unsigned int blockIndex = 0; blockIndex < numUniformBlocks; blockIndex++) {
        ...
        if (uniformBlock.name == baseName) {
            const bool arrayElementZero = (subscript == GL_INVALID_INDEX && (!uniformBlock.isArray || uniformBlock.arrayElement == 0));
...
bool Program::linkUniformBlocks(InfoLog& infoLog, const Caps& caps)
{
    ...
    for (const sh::InterfaceBlock& fragmentInterfaceBlock : fragmentInterfaceBlocks) {
        ...
        // Note: shared and std140 layouts are always considered active
        if (fragmentInterfaceBlock.staticUse || fragmentInterfaceBlock.layout != sh::BLOCKLAYOUT_PACKED) {
    ...
    const sh::ShaderVariable& fragmentVariable, bool validatePrecision)
{
    ...
    for (unsigned int memberIndex = 0; memberIndex < numMembers; memberIndex++) {
        ...
        const std::string memberName = variableName.substr(0, variableName.length() - 1) + "." + vertexMember.name + "'";
    ...
    const Caps& caps) const
{
    ...
    for (const std::string& tfVaryingName : mData.mTransformFeedbackVaryingNames) {
        ...
        for (const sh::Varying* varying : varyings) {
            if (tfVaryingName == varying->name) {
                ...
                if (mData.mTransformFeedbackBufferMode == GL_SEPARATE_ATTRIBS && componentCount > caps.maxTransformFeedbackSeparateComponents) {
    ...
    if (mData.mTransformFeedbackBufferMode == GL_INTERLEAVED_ATTRIBS && totalComponents > caps.maxTransformFeedbackInterleavedComponents) {
...
void Program::linkOutputVariables()
{
    ...
    for (unsigned int outputVariableIndex = 0; outputVariableIndex < shaderOutputVars.size();
        ...
        for (unsigned int elementIndex = 0; elementIndex < outputVariable.elementCount();
            ...
            mData.mOutputVariables[location] = VariableLocation(outputVariable.name, element, outputVariableIndex);
        }
    }
}
...
bool Program::flattenUniformsAndCheckCaps(const Caps& caps, InfoLog& infoLog)
{
    ...
    mSamplerUniformRange.end = mSamplerUniformRange.start + static_cast<unsigned int>(samplerUniforms.size());
    ...
    std::vector<LinkedUniform>* samplerUniforms)
{
    ...
    vectorAndSamplerCount.vectorCount = (isSampler ? 0 : (VariableRegisterCount(uniform.type) * elementCount));
    ...
    for (const VarT& field : fields) {
        ...
        if (field.isStruct()) {
            for (unsigned int arrayElement = 0; arrayElement < field.elementCount(); arrayElement++) {
                const std::string uniformElementName = fullName + (field.isArray() ? ArrayString(arrayElement) : "");
