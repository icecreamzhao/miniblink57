...
TIntermBinary::TIntermBinary(const TIntermBinary& node)
    : TIntermOperator(node)
    , mAddIndexClamp(node.mAddIndexClamp)
{
...
TIntermSelection::TIntermSelection(const TIntermSelection& node)
    : TIntermTyped(node)
{
...
bool TIntermBinary::promote(TInfoSink& infoSink)
{
    ...
    case EOpInitialize:
        ...
        ASSERT((mLeft->getNominalSize() == mRight->getNominalSize()) && (mLeft->getSecondarySize() == mRight->getSecondarySize()));
        break;
    case EOpAdd:
    case EOpSub:
    case EOpDiv:
    ...
    case EOpBitwiseXorAssign:
    case EOpBitwiseOrAssign:
        if ((mLeft->isMatrix() && mRight->isVector()) || (mLeft->isVector() && mRight->isMatrix())) {
            return false;
        }

        // Are the sizes compatible?
        if (mLeft->getNominalSize() != mRight->getNominalSize() || mLeft->getSecondarySize() != mRight->getSecondarySize()) {
            ...
            // vector either.
            if (!mRight->isScalar() && (isAssignment() || mOp == EOpBitShiftLeft || mOp == EOpBitShiftRight))
                return false;
        }
...
TConstantUnion* TIntermConstantUnion::foldBinary(TOperator op, TIntermConstantUnion* rightNode, TInfoSink& infoSink)
{
    ...
    case EOpMatrixTimesMatrix: {
        if (getType().getBasicType() != EbtFloat || rightNode->getBasicType() != EbtFloat) {
            ...
            for (int column = 0; column < resultCols; column++) {
                ...
                for (int i = 0; i < leftCols; i++) {
                    resultArray[resultRows * column + row].setFConst(
                        resultArray[resultRows * column + row].getFConst() + leftArray[i * leftRows + row].getFConst() * rightArray[column * rightRows + i].getFConst());
                }
            }
        }
    } break;
    ...
    case EOpMatrixTimesVector: {
        ...
        for (int matrixRow = 0; matrixRow < matrixRows; matrixRow++) {
            ...
            for (int col = 0; col < matrixCols; col++) {
                resultArray[matrixRow].setFConst(resultArray[matrixRow].getFConst() + leftArray[col * matrixRows + matrixRow].getFConst() * rightArray[col].getFConst());
            }
        }
    } break;
    ...
    case EOpVectorTimesMatrix: {
        ...
        for (int matrixCol = 0; matrixCol < matrixCols; matrixCol++) {
            ...
            for (int matrixRow = 0; matrixRow < matrixRows; matrixRow++) {
                resultArray[matrixCol].setFConst(resultArray[matrixCol].getFConst() + leftArray[matrixRow].getFConst() * rightArray[matrixCol * matrixRows + matrixRow].getFConst());
            }
        }
    } break;
...
TConstantUnion* TIntermConstantUnion::foldUnaryWithDifferentReturnType(TOperator op, TInfoSink& infoSink)
{
    ...
    case EOpTranspose:
        if (getType().getBasicType() == EbtFloat) {
            resultArray = new TConstantUnion[objectSize];
            angle::Matrix<float> result = GetMatrix(operandArray, getType().getNominalSize(), getType().getSecondarySize()).transpose();
    ...
    if (paramsCount == 2) {
        ...
        case EOpReflect:
            if (basicType == EbtFloat) {
                ...
                for (size_t i = 0; i < maxObjectSize; i++) {
                    float result = unionArrays[0][i].getFConst() - 2.0f * dotProduct * unionArrays[1][i].getFConst();
                    resultArray[i].setFConst(result);
                }
            } else
                UNREACHABLE();
        ...
        case EOpMul:
            if (basicType == EbtFloat && (*sequence)[0]->getAsTyped()->isMatrix() && (*sequence)[1]->getAsTyped()->isMatrix()) {
                // Perform component-wise matrix multiplication.
                resultArray = new TConstantUnion[maxObjectSize];
                int size = (*sequence)[0]->getAsTyped()->getNominalSize();
                angle::Matrix<float> result = GetMatrix(unionArrays[0], size).compMult(GetMatrix(unionArrays[1], size));
                SetUnionArrayFromMatrix(result, resultArray);
            } else
                UNREACHABLE();
            break;
        ...
        case EOpOuterProduct:
            if (basicType == EbtFloat) {
                ...
                angle::Matrix<float> result = GetMatrix(unionArrays[0], 1, static_cast<int>(numCols))
            ...
            if (basicType == EbtFloat) {
                ...
                for (size_t i = 0; i < maxObjectSize; i++) {
                    ...
                    if (k < 0.0f)
                        ...
                        resultArray[i].setFConst(eta * unionArrays[0][i].getFConst() - (eta * dotProduct + sqrtf(k)) * unionArrays[1][i].getFConst());
                }
            } else
                UNREACHABLE();
            break;
...
void TIntermTraverser::updateTree()
{
    for (size_t ii = 0; ii < mInsertions.size(); ++ii) {
        ...
        if (!insertion.insertionsBefore.empty()) {
            bool inserted = insertion.parent->insertChildNodes(insertion.position, insertion.insertionsBefore);
            ASSERT(inserted);
            UNUSED_ASSERTION_VARIABLE(inserted);
        }
    }
