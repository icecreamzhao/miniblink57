...
bool isSingleStatement(TIntermNode* node)
{
    if (const TIntermAggregate* aggregate = node->getAsAggregate()) {
        return (aggregate->getOp() != EOpFunction) && (aggregate->getOp() != EOpSequence);
...
{
    TString preString = useEmulatedFunction ? BuiltInFunctionEmulator::GetEmulatedFunctionName(preStr) : preStr;
...
bool TOutputGLSLBase::visitAggregate(Visit visit, TIntermAggregate* node)
{
    ...
    case EOpFunction: {
        ...
        TIntermAggregate* body = ++seqIter != sequence.end() ? (*seqIter)->getAsAggregate() : NULL;
        visitCodeBlock(body);
        decrementDepth();
...
bool TOutputGLSLBase::visitLoop(Visit visit, TIntermLoop* node)
{
    ...
    if (loopType == ELoopFor) // for loop
    {
        if (!node->getUnrollFlag()) {
            ...
            TIntermSymbol* indexSymbol = (*declSeq)[0]->getAsBinaryNode()->getLeft()->getAsSymbolNode();
