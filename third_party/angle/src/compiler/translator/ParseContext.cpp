    ...
    TType* type)
{
    *type = function.getReturnType();
    ...
    if (!type->isMatrix() || !matrixInMatrix) {
        if ((op != EOpConstructStruct && size != 1 && size < type->getObjectSize()) || (op == EOpConstructStruct && size < type->getObjectSize())) {
...
bool TParseContext::parameterSamplerErrorCheck(const TSourceLoc& line,
    ...
    const TType& type)
{
    if ((qualifier == EvqOut || qualifier == EvqInOut) && type.getBasicType() != EbtStruct && IsSampler(type.getBasicType())) {
...
{
    if ((type.qualifier == EvqAttribute) || (type.qualifier == EvqVertexIn) || (type.qualifier == EvqConst && mShaderVersion < 300)) {
    ...
    const TSourceLoc& identifierLocation)
{
    ...
    if (publicType.qualifier != EvqUniform && samplerErrorCheck(identifierLocation, publicType, "samplers must be uniform")) {
        return true;
    }
    ...
    const TSymbol* symbol)
{
    ...
    if (!symbol) {
        ...
        if (symbolTable.findBuiltIn(variable->getName(), mShaderVersion) && !variable->getExtension().empty() && extensionErrorCheck(location, variable->getExtension())) {
            recover();
        }
    ...
    const TPublicType& typeSpecifier)
{
    ...
    if (mShaderVersion < 300) {
        ...
        if ((qualifier == EvqVaryingIn || qualifier == EvqVaryingOut) && (typeSpecifier.type == EbtBool || typeSpecifier.type == EbtInt)) {
            error(typeSpecifier.line, "cannot be bool or int", getQualifierString(qualifier));
            recover();
        }
    } else {
    ...
    const TSourceLoc& qualifierLocation)
{
    ...
    // restrictions.
    bool typeContainsIntegers = (type.type == EbtInt || type.type == EbtUInt || type.isStructureContainingType(EbtInt) || type.isStructureContainingType(EbtUInt));
    ...
    const TSymbol* symbol)
{
    ...
    if (!symbol) {
        ...
        TIntermSymbol* intermSymbol = intermediate.addSymbol(variable->getUniqueId(), *identifier, type, identifierLoc);
    ...
    const TSourceLoc& location)
{
    ...
    // multiple times.
    TFunction* symbolTableFunction = static_cast<TFunction*>(symbolTable.find(function.getMangledName(), getShaderVersion()));
    ...
    TIntermTyped* indexExpression)
{
    ...
    if (indexConstantUnion) {
        ...
        if (baseConstantUnion) {
            if (baseExpression->isArray()) {
                ...
                indexedExpression = addConstArrayNode(index, baseConstantUnion, location, outOfRangeIndexIsError);
                ...
                indexedExpression = addConstVectorNode(fields, baseConstantUnion, location, outOfRangeIndexIsError);
            } else if (baseExpression->isMatrix()) {
                // constant folding for matrix indexing
                indexedExpression = addConstMatrixNode(index, baseConstantUnion, location, outOfRangeIndexIsError);
            }
        } else {
            int safeIndex = -1;
    ...
    if (baseExpression->getType().getQualifier() == EvqConst && indexExpression->getType().getQualifier() == EvqConst) {
    ...
    const TSourceLoc& loc)
{
    ...
    if ((switchType != EbtInt && switchType != EbtUInt) || init->isMatrix() || init->isArray() || init->isVector()) {
...
TIntermCase* TParseContext::addCase(TIntermTyped* condition, const TSourceLoc& loc)
{
    ...
    if ((condition->getBasicType() != EbtInt && condition->getBasicType() != EbtUInt) || condition->isMatrix() || condition->isArray() || condition->isVector()) {
    ...
    const TType* funcReturnType)
{
    ...
    switch (op) {
    case EOpLogicalNot:
        if (child->getBasicType() != EbtBool || child->isMatrix() || child->isArray() || child->isVector()) {
            return nullptr;
        }
        break;
    case EOpBitwiseNot:
        if ((child->getBasicType() != EbtInt && child->getBasicType() != EbtUInt) || child->isMatrix() || child->isArray()) {
            return nullptr;
        }
        break;
    ...
    case EOpNegative:
    case EOpPositive:
        if (child->getBasicType() == EbtStruct || child->getBasicType() == EbtBool || child->isArray()) {
            return nullptr;
        }
    ...
    const TSourceLoc& loc)
{
    ...
    case EOpNotEqual:
        ...
        if ((mShaderVersion < 300 || op == EOpAssign || op == EOpInitialize) && left->getType().isStructureContainingSamplers()) {
...
void TParseContext::checkTextureOffsetConst(TIntermAggregate* functionCall)
{
    ...
    if (name.compare(0, 16, "texelFetchOffset") == 0 || name.compare(0, 16, "textureLodOffset") == 0 || name.compare(0, 20, "textureProjLodOffset") == 0 || name.compare(0, 17, "textureGradOffset") == 0 || name.compare(0, 21, "textureProjGradOffset") == 0) {
        offset = arguments->back();
    } else if (name.compare(0, 13, "textureOffset") == 0 || name.compare(0, 17, "textureProjOffset") == 0) {
    ...
    *fatalError = false;
    ...
    if (thisNode != nullptr) {
        ...
        if (fnCandidate) {
            ...
            // A declared function.
            //
            if (builtIn && !fnCandidate->getExtension().empty() && extensionErrorCheck(loc, fnCandidate->getExtension())) {
                recover();
            }
