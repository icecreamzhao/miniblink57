    ...
    if (IsVarying(symbol->getQualifier())) {
        ...
        case EvqFragDepthEXT:
            if (!mFragDepthEXTAdded) {
                ...
                info.type = GL_FLOAT;
                info.arraySize = 0;
                info.precision = GLVariablePrecision(static_cast<const TVariable*>(
...
bool CollectVariables::visitAggregate(Visit, TIntermAggregate* node)
{
    ...
    case EOpDeclaration: {
        ...
        if (typedNode.getBasicType() == EbtInterfaceBlock) {
            ...
            visitChildren = false;
        } else if (qualifier == EvqAttribute || qualifier == EvqVertexIn || qualifier == EvqFragmentOut || qualifier == EvqUniform || IsVarying(qualifier)) {
            switch (qualifier) {
            case EvqAttribute:
            case EvqVertexIn:
