...
public:
    TScopedPoolAllocator(TPoolAllocator* allocator)
...
bool TCompiler::shouldRunLoopAndIndexingValidation(int compileOptions) const
{
    ...
    return (IsWebGLBasedSpec(shaderSpec) && shaderVersion == 100) || (compileOptions & SH_VALIDATE_LOOP_INDEXING);
}

bool TCompiler::Init(const ShBuiltInResources& resources)
{
    shaderVersion = 100;
    maxUniformVectors = (shaderType == GL_VERTEX_SHADER) ? resources.MaxVertexUniformVectors : resources.MaxFragmentUniformVectors;
    ...
    const int compileOptions)
{
    ...
    if (success) {
        ...
        if (success && shaderType == GL_VERTEX_SHADER && ((compileOptions & SH_INIT_GL_POSITION) || (outputType == SH_GLSL_COMPATIBILITY_OUTPUT)))
        ...
        if (success && shouldCollectVariables(compileOptions)) {
            ...
            if (success && shaderType == GL_VERTEX_SHADER && (compileOptions & SH_INIT_VARYINGS_WITHOUT_STATIC_USE))
