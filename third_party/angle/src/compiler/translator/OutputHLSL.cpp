...
void OutputHLSL::header(TInfoSinkBase& out, const BuiltInFunctionEmulator* builtInFunctionEmulator)
{
    ...
    for (TextureFunctionSet::const_iterator textureFunction = mUsesTexture.begin(); textureFunction != mUsesTexture.end(); textureFunction++) {
        ...
        if (textureFunction->method == TextureFunction::BIAS || textureFunction->method == TextureFunction::LOD0BIAS) {
            out << ", float bias";
        }
        ...
        if (textureFunction->method == TextureFunction::SIZE) {
            ...
            if (IsSampler2D(textureFunction->sampler) || IsSamplerCube(textureFunction->sampler)) {
                if (IsSamplerArray(textureFunction->sampler) || (IsIntegerSampler(textureFunction->sampler) && IsSamplerCube(textureFunction->sampler))) {
            ...
            case EbtSamplerCube:
                ...
                break;
            case EbtSampler2DArray:
                out << "    return int3(width, height, layers);";
                break;
            case EbtISampler2D:
                out << "    return int2(width, height);";
                break;
            ...
            } else if (IsIntegerSampler(textureFunction->sampler) && textureFunction->method != TextureFunction::FETCH) {
                if (IsSampler2D(textureFunction->sampler)) {
                    if (IsSamplerArray(textureFunction->sampler)) {
                        ...
                        if (textureFunction->method == TextureFunction::LOD0) {
                            ...
                            if (textureFunction->method == TextureFunction::IMPLICIT || textureFunction->method == TextureFunction::BIAS) {
                                out << "    float2 tSized = float2(t.x * width, t.y * height);\n"
            ...
            if (IsIntegerSampler(textureFunction->sampler) || textureFunction->method == TextureFunction::FETCH) {
                switch (hlslCoords) {
                case 2:
                    out << "int3(";
                    break;
...
bool OutputHLSL::visitAggregate(Visit visit, TIntermAggregate* node)
{
    ...
    case EOpDeclaration:
        if (visit == PreVisit) {
            ...
            if (variable && (variable->getQualifier() == EvqTemporary || variable->getQualifier() == EvqGlobal || variable->getQualifier() == EvqConst)) {
    ...
    case EOpFunctionCall: {
        ...
        if (node->isUserDefined()) {
            ...
            } else if (name == "texture2DLod" || name == "textureCubeLod" || name == "textureLod" || name == "texture2DLodEXT" || name == "textureCubeLodEXT") {
        ...
        for (TIntermSequence::iterator arg = arguments->begin(); arg != arguments->end(); arg++) {
            if (mOutputType == SH_HLSL_4_0_FL9_3_OUTPUT && IsSampler((*arg)->getAsTyped()->getBasicType())) {
                out << "texture_";
                (*arg)->traverse(this);
                out << ", sampler_";
            }
...
bool OutputHLSL::visitLoop(Visit visit, TIntermLoop* node)
{
    ...
    mInsideDiscontinuousLoop = mInsideDiscontinuousLoop || mCurrentFunctionMetadata->mDiscontinuousLoops.count(node) > 0;
...
TString OutputHLSL::argumentString(const TIntermSymbol* symbol)
{
    ...
    if (IsSampler(type.getBasicType())) {
        ...
        if (mOutputType == SH_HLSL_4_0_FL9_3_OUTPUT) {
            return QualifierString(qualifier) + " " + TextureString(type.getBasicType()) + " texture_" + nameStr + ArrayString(type) + ", " + QualifierString(qualifier) + " " + SamplerString(type.getBasicType()) + " sampler_" + nameStr + ArrayString(type);
        }
    }
...
bool OutputHLSL::canWriteAsHLSLLiteral(TIntermTyped* expression)
{
    ...
    if (expression->getQualifier() != EvqConst || !expression->getAsAggregate() || !expression->getAsAggregate()->isConstructor()) {
        return false;
    }
