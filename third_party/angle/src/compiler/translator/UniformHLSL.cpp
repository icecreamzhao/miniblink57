    ...
    const ReferencedSymbols& referencedUniforms)
{
    ...
    for (auto& uniformIt : referencedUniforms) {
        ...
        if (outputType == SH_HLSL_4_1_OUTPUT && IsSampler(type.getBasicType())) {
            ...
            // permitted.
            const TString& typeName = ((structure && !structure->name().empty()) ? QualifiedStructNameString(*structure, false, false) : TypeString(type));
...
TString UniformHLSL::interfaceBlockString(const TInterfaceBlock& interfaceBlock, unsigned int registerIndex, unsigned int arrayIndex)
{
    ...
    if (interfaceBlock.hasInstanceName()) {
        hlsl += "    " + InterfaceBlockStructName(interfaceBlock) + " " + interfaceBlockInstanceString(interfaceBlock, arrayIndex) + ";\n";
    } else {
