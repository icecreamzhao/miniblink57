    ...
    if (cond->getAsConstantUnion()) {
        if (cond->getAsConstantUnion()->getBConst(0) == true) {
            return nodePair.node1 ? setAggregateOperator(
                       nodePair.node1, EOpSequence, nodePair.node1->getLine())
                                  : NULL;
        } else {
            return nodePair.node2 ? setAggregateOperator(
                       nodePair.node2, EOpSequence, nodePair.node2->getLine())
                                  : NULL;
        }
    }
    ...
    int shaderVersion)
{
    ...
    if (shaderVersion >= 300 || left->getQualifier() != EvqConst || right->getQualifier() != EvqConst) {
    ...
    const TSourceLoc& line)
{
    ...
    if (cond->getQualifier() == EvqConst && trueBlock->getQualifier() == EvqConst && falseBlock->getQualifier() == EvqConst) {
