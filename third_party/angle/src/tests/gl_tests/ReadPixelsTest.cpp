...
protected:
    ...
    void SetUp() override
    {
        ...
        const char* vertexShaderSrc = "attribute vec4 aTest; attribute vec2 aPosition; varying vec4 vTest;\n"
                                      "void main()\n"
                                      "{\n"
                                      "    vTest        = aTest;\n"
...
public:
    ...
    void verifyPBO(GLint level, GLint layer)
    {
        ...
        void* mapPointer = glMapBufferRange(GL_PIXEL_PACK_BUFFER, 0, sizeof(angle::GLColor), GL_MAP_READ_BIT);
