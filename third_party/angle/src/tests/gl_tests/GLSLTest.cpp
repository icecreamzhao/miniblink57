...
{
    const std::string& fragmentShaderSourceVec = "precision mediump float;\n"
    ...
    const std::string& fragmentShaderSourceMat = "precision mediump float;\n"
    ...
    const std::string& fragmentShaderSourceArray = "precision mediump float;\n"
...
TEST_P(GLSLTest, LoopIndexingValidation)
{
    ...
    if ((GetParam() == ES2_D3D11_FL9_3() || GetParam() == ES2_D3D9())) {
        if (compileResult != 0) {
            ...
            if (infoLog.find("Loop index cannot be compared with non-constant expression") == std::string::npos) {
...
{
    if (GetParam().eglParameters.renderer == EGL_PLATFORM_ANGLE_TYPE_OPENGL_ANGLE || GetParam().eglParameters.renderer == EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE) {
        std::cout << "Test disabled on OpenGL." << std::endl;
        return;
    }
