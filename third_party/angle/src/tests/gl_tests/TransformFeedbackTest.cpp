...
TEST_P(TransformFeedbackTest, VertexOnly)
{
    ...
    GLvoid* mappedBuffer = glMapBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 0, sizeof(float) * 6, GL_MAP_READ_BIT);
...
TEST_P(TransformFeedbackTest, PackingBug)
{
    ...
    const GLvoid* mapPointer = glMapBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 0, sizeof(Vector2) * 2 * 6, GL_MAP_READ_BIT);
