...
protected:
    DebugTest()
        : mDebugExtensionAvailable(false)
    {
        setWindowWidth(128);
        setWindowHeight(128);
        setConfigRedBits(8);
    ...
    const void* userParam)
{
    ...
    std::vector<Message>* messages = static_cast<std::vector<Message>*>(const_cast<void*>(userParam));
...
TEST_P(DebugTest, InsertMessage)
{
    ...
    GLuint ret = glGetDebugMessageLogKHR(1, static_cast<GLsizei>(messageBuf.size()), &sourceBuf, &typeBuf,
...
TEST_P(DebugTest, InsertMessageMultiple)
{
    ...
    for (size_t i = 0; i < messageCount; i++) {
        ...
        GLsizei lengthBuf = 0;
        std::vector<char> messageBuf(messageLength + 1);
        GLuint ret = glGetDebugMessageLogKHR(1, static_cast<GLsizei>(messageBuf.size()), &sourceBuf,
