...
protected:
    ...
    {
        std::string infoLog;
        bool compilationSuccess = compileTestShader(GL_FRAGMENT_SHADER, SH_GLES2_SPEC, SH_HLSL_4_1_OUTPUT, shaderString,
    ...
    void expect(const char* patterns[], size_t count)
    {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            for (size_t j = 0; j < ArraySize(badPatterns); j++) {
                ...
                if (pattern != badPattern && mTranslatedSource.find(badPattern, mCurrentPosition) < position) {
