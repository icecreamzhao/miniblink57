...
{
    const char* str = "#if 1 1\n"
                      "#endif\n";
    ASSERT_TRUE(mPreprocessor.init(1, &str, 0));

    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_CONDITIONAL_UNEXPECTED_TOKEN, pp::SourceLocation(0, 1), "1"));
...
TEST_F(IfTest, ExtraIdentifierExpression)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_CONDITIONAL_UNEXPECTED_TOKEN, pp::SourceLocation(0, 2), "1"));
...
TEST_F(IfTest, UnterminatedDefinedInMacro)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_INVALID_EXPRESSION, pp::SourceLocation(0, 2), "syntax error"));
...
{
    const char* str = "#define foo defined(bar\n"
                      "#if foo\n"
                      "#endif\n";
    ASSERT_TRUE(mPreprocessor.init(1, &str, 0));
