...
TEST_F(DefineTest, ReservedUnderScore1)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_WARNING_MACRO_NAME_RESERVED, pp::SourceLocation(0, 1), "__foo"));
...
TEST_F(DefineTest, ReservedUnderScore2)
{
    ...
    EXPECT_CALL(mDiagnostics, print(pp::Diagnostics::PP_WARNING_MACRO_NAME_RESERVED, pp::SourceLocation(0, 1), "foo__bar"));
...
{
    const char* input = "#define foo defined(bar)\n"
                        "foo\n";
    const char* expected = "\n"
                           "defined(bar)\n";
    preprocess(input, expected);
