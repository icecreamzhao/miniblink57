...
{
    // Some sanity check.
    static_assert(static_cast<unsigned int>(kNumberOfExactMatchTokens) == sizeof(kTokenData) / sizeof(kTokenData[0]), "sanity check");
    static_assert(static_cast<unsigned int>(kNumberOfErrors) == sizeof(kErrorMessage) / sizeof(kErrorMessage[0]), "sanity check");
}
    ...
    const GPUTestBotConfig& bot_config) const
{
    for (size_t i = 0; i < entries_.size(); ++i) {
        if (NamesMatching(entries_[i].test_name, test_name) && bot_config.Matches(entries_[i].test_config))
    ...
    GPUTestConfig* config, int32 token, size_t line_number)
{
    ...
    case kConfigIntel:
    case kConfigVMWare: {
        uint32 gpu_vendor = static_cast<uint32>(kTokenData[token].flag);
    ...
    size_t line_number)
{
    DCHECK(config);
    uint32 device_id = 0;
    if (config->gpu_device_id() != 0 || !base::HexStringToUInt(gpu_device_id, &device_id) || device_id == 0) {
...
bool GPUTestExpectationsParser::DetectConflictsBetweenEntries()
{
    ...
    for (size_t i = 0; i < entries_.size(); ++i) {
        for (size_t j = i + 1; j < entries_.size(); ++j) {
            if (entries_[i].test_name == entries_[j].test_name && entries_[i].test_config.OverlapsWith(entries_[j].test_config)) {
