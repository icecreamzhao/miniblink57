...
CollectInfoResult CollectPCIVideoCardInfo(GPUInfo* gpu_info)
{
    ...
    for (pci_dev* device = access->devices;
        ...
        if (!primary_gpu_identified) {
            ...
            if (gpu_info->gpu.vendor_id == kVendorIDIntel && gpu.vendor_id != kVendorIDIntel) {
    ...
    if (gpu_info->secondary_gpus.size() == 1 && gpu_info->secondary_gpus[0].vendor_id == kVendorIDIntel) {
