...
GLColor::GLColor(GLubyte r, GLubyte g, GLubyte b, GLubyte a)
    : R(r)
    , G(g)
    , B(b)
    , A(a)
...
{
    mEGLWindow = new EGLWindow(GetParam().majorVersion, GetParam().minorVersion, GetParam().eglParameters);
}
...
void ANGLETest::checkD3D11SDKLayersMessages()
{
    ...
    if (mIgnoreD3D11SDKLayersWarnings || mEGLWindow->getPlatform().renderer != EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE || mEGLWindow->getDisplay() == EGL_NO_DISPLAY) {
        return;
    }

    const char* extensionString = static_cast<const char*>(eglQueryString(mEGLWindow->getDisplay(), EGL_EXTENSIONS));
    ...
    HRESULT hr = d3d11Device->QueryInterface(__uuidof(infoQueue), reinterpret_cast<void**>(&infoQueue));
    if (SUCCEEDED(hr)) {
        UINT64 numStoredD3DDebugMessages = infoQueue->GetNumStoredMessagesAllowedByRetrievalFilter();
...
{
    std::string rendererString(reinterpret_cast<const char*>(glGetString(GL_RENDERER)));
    return (rendererString.find("AMD") != std::string::npos) || (rendererString.find("ATI") != std::string::npos);
}
