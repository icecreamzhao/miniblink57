...
protected:
    ...
    void initializeEGL(bool usePresentPathFast)
    {
        ...
        mDisplay = eglGetPlatformDisplayEXT(EGL_PLATFORM_ANGLE_ANGLE, EGL_DEFAULT_DISPLAY, displayAttribs);
    ...
    void createPbufferFromClientBufferSurface()
    {
        ...
        const char* extensionString = static_cast<const char*>(eglQueryString(mDisplay, EGL_EXTENSIONS));
        ...
        IDXGIResource* dxgiResource = DynamicCastComObject<IDXGIResource>(mOffscreenSurfaceD3D11Texture);
    ...
    void drawQuadUsingGL()
    {
        ...
        const std::string vertexShaderSource = SHADER_SOURCE(precision highp float; attribute vec4 position; varying vec2 texcoord;
        ...
        const std::string fragmentShaderSource2D = SHADER_SOURCE(precision highp float; uniform sampler2D tex; varying vec2 texcoord;
