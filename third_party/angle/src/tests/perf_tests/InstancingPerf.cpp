...
InstancingPerfBenchmark::InstancingPerfBenchmark()
    : ANGLERenderTest("InstancingPerf", GetParam())
    , mProgram(0)
    , mNumPoints(75000)
{
...
void InstancingPerfBenchmark::initializeBenchmark()
{
    ...
    const std::string vs = "attribute vec2 aPosition;\n"
    ...
    float fov = static_cast<float>(getWindow()->getWidth()) / static_cast<float>(getWindow()->getHeight());
