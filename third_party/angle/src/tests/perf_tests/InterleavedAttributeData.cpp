...
InterleavedAttributeDataBenchmark::InterleavedAttributeDataBenchmark()
    : ANGLERenderTest("InterleavedAttributeData", GetParam())
    , mPointSpriteProgram(0)
{
}
...
void InterleavedAttributeDataBenchmark::initializeBenchmark()
{
    ...
    const std::string vs = "attribute vec4 aPosition;"
    ...
    for (size_t i = 0; i < ArraySize(mPositionColorBuffer); i++) {
        ...
        for (unsigned int j = 0; j < params.numSprites; j++) {
            float pointSpriteX = (static_cast<float>(rand() % getWindow()->getWidth()) / getWindow()->getWidth()) * 2.0f - 1.0f;
            float pointSpriteY = (static_cast<float>(rand() % getWindow()->getHeight()) / getWindow()->getHeight()) * 2.0f - 1.0f;
