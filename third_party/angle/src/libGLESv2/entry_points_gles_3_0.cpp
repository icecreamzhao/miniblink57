...
void GL_APIENTRY DrawRangeElements(GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type, const GLvoid* indices)
{
    ...
    if (context) {
        ...
        Error error = context->drawRangeElements(mode, start, end, count, type, indices, indexRange);
        if (error.isError()) {
...
void GL_APIENTRY CopyTexSubImage3D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLint x, GLint y, GLsizei width, GLsizei height)
{
    ...
    Context* context = GetValidGlobalContext();
    if (context) {
        if (!context->skipValidation() && !ValidateCopyTexSubImage3D(context, target, level, xoffset, yoffset, zoffset, x, y, width, height)) {
            return;
        }
...
void GL_APIENTRY BlitFramebuffer(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter)
{
    ...
    Context* context = GetValidGlobalContext();
    if (context) {
        if (!context->skipValidation() && !ValidateBlitFramebuffer(context, srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter)) {
            return;
        }
...
void GL_APIENTRY ClearBufferfi(GLenum buffer, GLint drawbuffer, GLfloat depth, GLint stencil)
{
    ...
    Context* context = GetValidGlobalContext();
    if (context) {
        if (!context->skipValidation() && !ValidateClearBufferfi(context, buffer, drawbuffer, depth, stencil)) {
            return;
        }
...
void GL_APIENTRY CopyBufferSubData(GLenum readTarget, GLenum writeTarget, GLintptr readOffset, GLintptr writeOffset, GLsizeiptr size)
{
    ...
    if (context) {
        ...
        if (readOffset < 0 || writeOffset < 0 || size < 0 || static_cast<unsigned int>(readOffset + size) > readBuffer->getSize() || static_cast<unsigned int>(writeOffset + size) > writeBuffer->getSize()) {
            context->recordError(Error(GL_INVALID_VALUE));
            return;
        }
...
void GL_APIENTRY DrawElementsInstanced(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices, GLsizei instanceCount)
{
    ...
    if (context) {
        ...
        Error error = context->drawElementsInstanced(mode, count, type, indices, instanceCount, indexRange);
        if (error.isError()) {
            context->recordError(error);
            return;
        }
...
void GL_APIENTRY GetSynciv(GLsync sync, GLenum pname, GLsizei bufSize, GLsizei* length, GLint* values)
{
    ...
    if (context) {
        ...
        switch (pname) {
        case GL_OBJECT_TYPE:
            values[0] = static_cast<GLint>(GL_SYNC_FENCE);
            break;
        case GL_SYNC_CONDITION:
            values[0] = static_cast<GLint>(fenceSync->getCondition());
            break;
        case GL_SYNC_FLAGS:
            values[0] = static_cast<GLint>(fenceSync->getFlags());
            break;
...
void GL_APIENTRY InvalidateFramebuffer(GLenum target, GLsizei numAttachments, const GLenum* attachments)
{
    ...
    Context* context = GetValidGlobalContext();
    if (context) {
        if (!context->skipValidation() && !ValidateInvalidateFramebuffer(context, target, numAttachments, attachments)) {
            return;
        }
