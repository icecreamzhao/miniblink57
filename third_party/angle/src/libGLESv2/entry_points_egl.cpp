...
EGLBoolean EGLAPIENTRY MakeCurrent(EGLDisplay dpy, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    if (readSurface) {
        Error readCompatError = ValidateCompatibleConfigs(display, readSurface->getConfig(), readSurface,
    ...
    if (draw != read) {
        ...
        if (drawSurface) {
            Error drawCompatError = ValidateCompatibleConfigs(display, drawSurface->getConfig(), drawSurface,
...
#define INSERT_PROC_ADDRESS(ns, proc) \
    map[#ns #proc] = reinterpret_cast<__eglMustCastToProperFunctionPointerType>(ns::proc)
        ...
        map["glFenceSync"] = reinterpret_cast<__eglMustCastToProperFunctionPointerType>(gl::FenceSync_);
