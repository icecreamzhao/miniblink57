...
bool X11Window::initialize(const std::string& name, size_t width, size_t height)
{
...

    {
        ...
        attributes.event_mask = StructureNotifyMask | PointerMotionMask | ButtonPressMask | ButtonReleaseMask | FocusChangeMask | EnterWindowMask | LeaveWindowMask | KeyPressMask | KeyReleaseMask;
...
void X11Window::processEvent(const XEvent& xEvent)
{
    ...
    case ClientMessage:
        if (xEvent.xclient.message_type == WM_PROTOCOLS && static_cast<Atom>(xEvent.xclient.data.l[0]) == WM_DELETE_WINDOW) {
            Event event;
