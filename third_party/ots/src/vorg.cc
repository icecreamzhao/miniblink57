...
bool ots_vorg_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    uint16_t num_recs;
    if (!table.ReadU16(&vorg->major_version) || !table.ReadU16(&vorg->minor_version) || !table.ReadS16(&vorg->default_vert_origin_y) || !table.ReadU16(&num_recs)) {
    ...
    for (unsigned i = 0; i < num_recs; ++i) {
        ...
        if (!table.ReadU16(&rec.glyph_index) || !table.ReadS16(&rec.vert_origin_y)) {
...
bool ots_vorg_serialise(OTSStream* out, Font* font)
{
    ...
    if (num_metrics != vorg->metrics.size() || !out->WriteU16(vorg->major_version) || !out->WriteU16(vorg->minor_version) || !out->WriteS16(vorg->default_vert_origin_y) || !out->WriteU16(num_metrics)) {
    ...
    for (uint16_t i = 0; i < num_metrics; ++i) {
        ...
        if (!out->WriteU16(rec.glyph_index) || !out->WriteS16(rec.vert_origin_y)) {
