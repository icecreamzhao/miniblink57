    ...
    size_t length, const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&glyph_count)) {
        return OTS_FAILURE_MSG("Failed to read gdef header");
    }
    const unsigned attach_points_end = 2 * static_cast<unsigned>(glyph_count) + 4;
    if (attach_points_end > std::numeric_limits<uint16_t>::max()) {
        return OTS_FAILURE_MSG("Bad glyph count in gdef");
    }
    if (offset_coverage == 0 || offset_coverage >= length || offset_coverage < attach_points_end) {
    ...
    for (unsigned i = 0; i < glyph_count; ++i) {
        ...
        if (attach_points[i] >= length || attach_points[i] < attach_points_end) {
    ...
    size_t length, const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&lig_glyph_count)) {
        return OTS_FAILURE_MSG("Can't read caret structure");
    }
    const unsigned lig_glyphs_end = 2 * static_cast<unsigned>(lig_glyph_count) + 4;
    if (lig_glyphs_end > std::numeric_limits<uint16_t>::max()) {
        return OTS_FAILURE_MSG("Bad caret structure");
    }
    if (offset_coverage == 0 || offset_coverage >= length || offset_coverage < lig_glyphs_end) {
    ...
    size_t length, const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&mark_set_count)) {
    ...
    for (unsigned i = 0; i < mark_set_count; ++i) {
        ...
        if (offset_coverage >= length || offset_coverage < mark_sets_end) {
...
bool ots_gdef_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&offset_glyph_class_def) || !table.ReadU16(&offset_attach_list) || !table.ReadU16(&offset_lig_caret_list) || !table.ReadU16(&offset_mark_attach_class_def)) {
        return OTS_FAILURE_MSG("Incomplete table");
    }
    ...
    if (offset_glyph_class_def) {
        if (offset_glyph_class_def >= length || offset_glyph_class_def < gdef_header_end) {
    ...
    if (offset_attach_list) {
        if (offset_attach_list >= length || offset_attach_list < gdef_header_end) {
    ...
    if (offset_lig_caret_list) {
        if (offset_lig_caret_list >= length || offset_lig_caret_list < gdef_header_end) {
    ...
    if (offset_mark_attach_class_def) {
        if (offset_mark_attach_class_def >= length || offset_mark_attach_class_def < gdef_header_end) {
    ...
    if (offset_mark_glyph_sets_def) {
        if (offset_mark_glyph_sets_def >= length || offset_mark_glyph_sets_def < gdef_header_end) {
