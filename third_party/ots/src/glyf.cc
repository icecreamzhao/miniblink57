    ...
    uint32_t* new_size)
{
    ...
    if ((font->maxp->version_1) && (font->maxp->max_size_glyf_instructions < bytecode_length)) {
    ...
    if (gly_length < (gly_header_length + bytecode_length + flags_count_physical + xy_coordinates_length)) {
        return OTS_FAILURE_MSG("Glyph too short %d", gly_length);
    }

    if (gly_length - (gly_header_length + bytecode_length + flags_count_physical + xy_coordinates_length) > 3) {
...
bool ots_glyf_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    for (unsigned i = 0; i < num_glyphs; ++i) {
        ...
        if (!table.ReadS16(&num_contours) || !table.ReadS16(&xmin) || !table.ReadS16(&ymin) || !table.ReadS16(&xmax) || !table.ReadS16(&ymax)) {
