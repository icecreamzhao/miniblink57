    ...
    bool* out_is_operator)
{
    ...
    *out_is_operator = false;
    ...
    if (v <= 11) {
        ...
        *out_number = ((static_cast<int32_t>(v) - 247) * 256) + static_cast<int32_t>(w) + 108;
    } else if (v <= 254) {
        uint8_t w = 0;
        if (!char_string->ReadU8(&w)) {
            return OTS_FAILURE();
        }
        *out_number = -((static_cast<int32_t>(v) - 251) * 256) - static_cast<int32_t>(w) - 108;
    } else if (v == 255) {
    ...
    size_t* in_out_num_stems)
{
    ...
    case ots::kCallSubr:
    case ots::kCallGSubr: {
        const ots::CFFIndex& subrs_index = (op == ots::kCallSubr ? local_subrs_index : global_subrs_index);
        ...
        const size_t length = subrs_index.offsets[subr_number + 1] - subrs_index.offsets[subr_number];
    ...
    case ots::kVVCurveTo:
        ...
        if (((stack_size % 4) != 0) && (((stack_size - 1) % 4) != 0)) {
            return OTS_FAILURE();
        }
        while (!argument_stack->empty())
            argument_stack->pop();
    ...
    case ots::kHVCurveTo: {
        ...
        if (((stack_size - 4) % 8) == 0) {
            ...
            successful = true;
        } else if ((stack_size >= 5) && ((stack_size - 5) % 8) == 0) {
            // dx1 dx2 dy2 dy3 {dya dxb dyb dxc dxd dxe dye dyf}* dxf
            successful = true;
        } else if ((stack_size >= 8) && ((stack_size - 8) % 8) == 0) {
            // {dxa dxb dyb dyc dyd dxe dye dxf}+
            successful = true;
        } else if ((stack_size >= 9) && ((stack_size - 9) % 8) == 0) {
    ...
    const ots::CFFIndex** out_local_subrs_to_use)
{
    *out_local_subrs_to_use = NULL;
    ...
    if ((fd_select.size() > 0) && (!local_subrs_per_font.empty())) {
        // Look up FDArray index for the glyph.
        std::map<uint16_t, uint8_t>::const_iterator iter = fd_select.find(glyph_index);
    ...
    Buffer* cff_table)
{
    ...
    for (unsigned i = 1; i < char_strings_index.offsets.size(); ++i) {
        ...
        // for the |i|-th glyph.
        const size_t length = char_strings_index.offsets[i] - char_strings_index.offsets[i - 1];
        if (length > kMaxCharStringLength) {
            return OTS_FAILURE();
        }
