...
bool ParseIndex(ots::Buffer* table, ots::CFFIndex* index)
{
    ...
    if ((index->off_size == 0) || (index->off_size > 4)) {
        return OTS_FAILURE();
    }
    ...
    std::vector<std::pair<uint32_t, DICT_OPERAND_TYPE> >* operands)
{
    ...
    switch (b0) {
    case 28: // shortint
        if (!table->ReadU8(&b1) || !table->ReadU8(&b2)) {
            return OTS_FAILURE();
        }
    ...
    case 29: // longint
        if (!table->ReadU8(&b1) || !table->ReadU8(&b2) || !table->ReadU8(&b3) || !table->ReadU8(&b4)) {
            return OTS_FAILURE();
        }
        operands->push_back(std::make_pair(
