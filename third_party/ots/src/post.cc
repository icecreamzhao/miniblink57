...
bool ots_post_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU32(&post->version) || !table.ReadU32(&post->italic_angle) || !table.ReadS16(&post->underline) || !table.ReadS16(&post->underline_thickness) || !table.ReadU32(&post->is_fixed_pitch)) {
...
bool ots_post_serialise(OTSStream* out, Font* font)
{
    ...
    if (!out->WriteU32(post->version) || !out->WriteU32(post->italic_angle) || !out->WriteS16(post->underline) || !out->WriteS16(post->underline_thickness) || !out->WriteU32(post->is_fixed_pitch) || !out->WriteU32(0) || !out->WriteU32(0) || !out->WriteU32(0) || !out->WriteU32(0)) {
    ...
    const uint16_t num_indexes = static_cast<uint16_t>(post->glyph_name_index.size());
    if (num_indexes != post->glyph_name_index.size() || !out->WriteU16(num_indexes)) {
    ...
    for (unsigned i = 0; i < post->names.size(); ++i) {
        ...
        if (string_length != s.size() || !out->Write(&string_length, 1)) {
