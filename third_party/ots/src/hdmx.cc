...
bool ots_hdmx_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    int16_t num_recs;
    if (!table.ReadU16(&hdmx->version) || !table.ReadS16(&num_recs) || !table.ReadS32(&hdmx->size_device_record)) {
    ...
    for (int i = 0; i < num_recs; ++i) {
        ...
        if (!table.ReadU8(&rec.pixel_size) || !table.ReadU8(&rec.max_width)) {
            return OTS_FAILURE_MSG("Failed to read hdmx record %d", i);
        }
        if ((i != 0) && (rec.pixel_size <= last_pixel_size)) {
        ...
        if ((hdmx->pad_len > 0) && !table.Skip(hdmx->pad_len)) {
...
bool ots_hdmx_serialise(OTSStream* out, Font* font)
{
    ...
    if (hdmx->records.size() > static_cast<size_t>(std::numeric_limits<int16_t>::max()) || !out->WriteU16(hdmx->version) || !out->WriteS16(num_recs) || !out->WriteS32(hdmx->size_device_record)) {
    ...
    for (int16_t i = 0; i < num_recs; ++i) {
        ...
        if (!out->Write(&rec.pixel_size, 1) || !out->Write(&rec.max_width, 1) || !out->Write(&rec.widths[0], rec.widths.size())) {
            return OTS_FAILURE_MSG("Failed to write hdmx record %d", i);
        }
        if ((hdmx->pad_len > 0) && !out->Write((const uint8_t*)"\x00\x00\x00", hdmx->pad_len)) {
