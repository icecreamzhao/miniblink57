    ...
    const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.Skip(4)) {
    ...
    if (format == 2) {
        ...
        if (!subtable.ReadU16(&offset_x_device) || !subtable.ReadU16(&offset_y_device)) {
    ...
    const uint16_t class_count)
{
    ...
    for (unsigned i = 0; i < mark_count; ++i) {
        ...
        if (!subtable.ReadU16(&class_value) || !subtable.ReadU16(&offset_mark_anchor)) {
            return OTS_FAILURE_MSG("Can't read mark table %d", i);
        }
        // |class_value| may take arbitrary values including 0 here so we don't
        // check the value.
        if (offset_mark_anchor < mark_records_end || offset_mark_anchor >= length) {
    ...
    const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&value_format)) {
    ...
    const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&offset_class_def1) || !subtable.ReadU16(&offset_class_def2) || !subtable.ReadU16(&class1_count) || !subtable.ReadU16(&class2_count)) {
    ...
    for (unsigned i = 0; i < class1_count; ++i) {
        ...
        for (unsigned j = 0; j < class2_count; ++j) {
            if (value_format1 && !ParseValueRecord(font, &subtable, data, length, value_format1)) {
                return OTS_FAILURE_MSG("Failed to parse value record 1 %d and %d", j, i);
            }
            if (value_format2 && !ParseValueRecord(font, &subtable, data, length, value_format2)) {
    ...
    if (offset_class_def1 < subtable.offset() || offset_class_def1 >= length || offset_class_def2 < subtable.offset() || offset_class_def2 >= length) {
    ...
    const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&value_format1) || !subtable.ReadU16(&value_format2)) {
    ...
    const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&entry_exit_count)) {
    ...
    const unsigned entry_exit_records_end = 2 * static_cast<unsigned>(entry_exit_count) + 6;
    ...
    for (unsigned i = 0; i < entry_exit_count; ++i) {
        ...
        if (!subtable.ReadU16(&offset_entry_anchor) || !subtable.ReadU16(&offset_exit_anchor)) {
            return OTS_FAILURE_MSG("Can't read entry exit record %d", i);
        }
        // These offsets could be NULL.
        if (offset_entry_anchor) {
            if (offset_entry_anchor < entry_exit_records_end || offset_entry_anchor >= length) {
        ...
        if (offset_exit_anchor) {
            if (offset_exit_anchor < entry_exit_records_end || offset_exit_anchor >= length) {
    ...
    const uint16_t class_count)
{
    ...
    const unsigned anchor_array_end = 2 * static_cast<unsigned>(record_count) * static_cast<unsigned>(class_count) + 2;
    ...
    const GPOS_TYPE type)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage1) || !subtable.ReadU16(&offset_coverage2) || !subtable.ReadU16(&class_count) || !subtable.ReadU16(&offset_mark_array) || !subtable.ReadU16(&offset_type_specific_array)) {
    ...
    if (offset_type_specific_array < header_end || offset_type_specific_array >= length) {
        return OTS_FAILURE_MSG("Bad type specific array offset %d", offset_type_specific_array);
    }
    if (type == GPOS_TYPE_MARK_TO_BASE_ATTACHMENT || type == GPOS_TYPE_MARK_TO_MARK_ATTACHMENT) {
...
bool ots_gpos_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU32(&version) || !table.ReadU16(&offset_script_list) || !table.ReadU16(&offset_feature_list) || !table.ReadU16(&offset_lookup_list)) {
