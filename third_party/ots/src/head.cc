...
bool ots_head_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    uint32_t version = 0;
    if (!table.ReadU32(&version) || !table.ReadU32(&head->revision)) {
    ...
    if (head->ppem < 16 || head->ppem > 16384) {
  ...
  if ((head->ppem - 1) & head->ppem) {
    ...
    if (!table.ReadR64(&head->created) || !table.ReadR64(&head->modified)) {
        return OTS_FAILURE_MSG("Can't read font dates");
    }

    if (!table.ReadS16(&head->xmin) || !table.ReadS16(&head->ymin) || !table.ReadS16(&head->xmax) || !table.ReadS16(&head->ymax)) {
    ...
    if (!table.ReadS16(&glyph_data_format) || glyph_data_format) {
...
bool ots_head_serialise(OTSStream* out, Font* font)
{
    ...
    if (!out->WriteU32(0x00010000) || !out->WriteU32(head->revision) || !out->WriteU32(0) || // check sum not filled in yet
        !out->WriteU32(0x5F0F3CF5) || !out->WriteU16(head->flags) || !out->WriteU16(head->ppem) || !out->WriteR64(head->created) || !out->WriteR64(head->modified) || !out->WriteS16(head->xmin) || !out->WriteS16(head->ymin) || !out->WriteS16(head->xmax) || !out->WriteS16(head->ymax) || !out->WriteU16(head->mac_style) || !out->WriteU16(head->min_ppem) || !out->WriteS16(2) || !out->WriteS16(head->index_to_loc_format) || !out->WriteS16(0)) {
