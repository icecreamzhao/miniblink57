...
bool ots_vdmx_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&vdmx->version) || !table.ReadU16(&vdmx->num_recs) || !table.ReadU16(&vdmx->num_ratios)) {
    ...
    for (unsigned i = 0; i < vdmx->num_ratios; ++i) {
        ...
        if (!table.ReadU8(&rec.charset) || !table.ReadU8(&rec.x_ratio) || !table.ReadU8(&rec.y_start_ratio) || !table.ReadU8(&rec.y_end_ratio)) {
        ...
        if ((i < vdmx->num_ratios - 1u) && (rec.x_ratio == 0) && (rec.y_start_ratio == 0) && (rec.y_end_ratio == 0)) {
    ...
    for (unsigned i = 0; i < vdmx->num_recs; ++i) {
        ...
        if (!table.ReadU16(&group.recs) || !table.ReadU8(&group.startsz) || !table.ReadU8(&group.endsz)) {
        ...
        for (unsigned j = 0; j < group.recs; ++j) {
            ...
            if (!table.ReadU16(&vt.y_pel_height) || !table.ReadS16(&vt.y_max) || !table.ReadS16(&vt.y_min)) {
...
bool ots_vdmx_serialise(OTSStream* out, Font* font)
{
    ...
    if (!out->WriteU16(vdmx->version) || !out->WriteU16(vdmx->num_recs) || !out->WriteU16(vdmx->num_ratios)) {
    ...
    for (unsigned i = 0; i < vdmx->rat_ranges.size(); ++i) {
        ...
        if (!out->Write(&rec.charset, 1) || !out->Write(&rec.x_ratio, 1) || !out->Write(&rec.y_start_ratio, 1) || !out->Write(&rec.y_end_ratio, 1)) {
    ...
    for (unsigned i = 0; i < vdmx->groups.size(); ++i) {
        ...
        if (!out->WriteU16(group.recs) || !out->Write(&group.startsz, 1) || !out->Write(&group.endsz, 1)) {
        ...
        for (unsigned j = 0; j < group.entries.size(); ++j) {
            ...
            if (!out->WriteU16(vt.y_pel_height) || !out->WriteS16(vt.y_max) || !out->WriteS16(vt.y_min)) {
