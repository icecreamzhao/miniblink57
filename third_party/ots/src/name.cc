...
bool ots_name_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    const char* string_base = reinterpret_cast<const char*>(data) + string_offset;
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (!table.ReadU16(&rec.platform_id) || !table.ReadU16(&rec.encoding_id) || !table.ReadU16(&rec.language_id) || !table.ReadU16(&rec.name_id) || !table.ReadU16(&name_length) || !table.ReadU16(&name_offset)) {
        ...
        const unsigned name_end = static_cast<unsigned>(string_offset) + name_offset + name_length;
        if (name_end > length) {
            continue;
        }
    ...
    if (format == 1) {
        ...
        for (unsigned i = 0; i < lang_tag_count; ++i) {
            ...
            const unsigned tag_end = static_cast<unsigned>(string_offset) + tag_offset + tag_length;
            if (tag_end > length) {
...
bool ots_name_serialise(OTSStream* out, Font* font)
{
    ...
    if (!out->WriteU16(format) || !out->WriteU16(name_count) || !out->WriteU16(static_cast<uint16_t>(string_offset))) {
    ...
    for (std::vector<NameRecord>::const_iterator name_iter = name->names.begin();
        ...
        if (string_data.size() + rec.text.size() > std::numeric_limits<uint16_t>::max() || !out->WriteU16(rec.platform_id) || !out->WriteU16(rec.encoding_id) || !out->WriteU16(rec.language_id) || !out->WriteU16(rec.name_id) || !out->WriteU16(static_cast<uint16_t>(rec.text.size())) || !out->WriteU16(static_cast<uint16_t>(string_data.size()))) {
    ...
    if (format == 1) {
        ...
        for (std::vector<std::string>::const_iterator tag_iter = name->lang_tags.begin();
             tag_iter != name->lang_tags.end(); ++tag_iter) {
            if (string_data.size() + tag_iter->size() > std::numeric_limits<uint16_t>::max() || !out->WriteU16(static_cast<uint16_t>(tag_iter->size())) || !out->WriteU16(static_cast<uint16_t>(string_data.size()))) {
