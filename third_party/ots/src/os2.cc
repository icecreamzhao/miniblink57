...
#undef SET_TO_ZERO
    ...
    //     "bXHeight",
    //   };
    static std::vector<std::string> panose_strings; // for USING_VC6RT
    if (panose_strings.empty()) {
        panose_strings.push_back("bFamilyType");
        panose_strings.push_back("bSerifStyle");
        panose_strings.push_back("bWeight");
        panose_strings.push_back("bProportion");
        panose_strings.push_back("bContrast");
        panose_strings.push_back("bStrokeVariation");
        panose_strings.push_back("bArmStyle");
        panose_strings.push_back("bLetterform");
        panose_strings.push_back("bMidline");
        panose_strings.push_back("bXHeight");
    }
    ...
    if (!table.ReadU32(&os2->unicode_range_1) || !table.ReadU32(&os2->unicode_range_2) || !table.ReadU32(&os2->unicode_range_3) || !table.ReadU32(&os2->unicode_range_4) || !table.ReadU32(&os2->vendor_id) || !table.ReadU16(&os2->selection) || !table.ReadU16(&os2->first_char_index) || !table.ReadU16(&os2->last_char_index) || !table.ReadS16(&os2->typo_ascender) || !table.ReadS16(&os2->typo_descender) || !table.ReadS16(&os2->typo_linegap) || !table.ReadU16(&os2->win_ascent) || !table.ReadU16(&os2->win_descent)) {
    ...
    if ((os2->selection & 0x1) && !(font->head->mac_style & 0x2)) {
        OTS_WARNING("adjusting Mac style (italic)");
        font->head->mac_style |= 0x2;
    }
    if ((os2->selection & 0x2) && !(font->head->mac_style & 0x4)) {
    ...
    if ((os2->selection & 0x40) && (font->head->mac_style & 0x3)) {
        OTS_WARNING("adjusting Mac style (regular)");
        font->head->mac_style &= 0xfffcu;
    }

    if ((os2->version < 4) && (os2->selection & 0x300)) {
    ...
    if (!table.ReadU32(&os2->code_page_range_1) || !table.ReadU32(&os2->code_page_range_2)) {
    ...
    if (!table.ReadS16(&os2->x_height) || !table.ReadS16(&os2->cap_height) || !table.ReadU16(&os2->default_char) || !table.ReadU16(&os2->break_char) || !table.ReadU16(&os2->max_context)) {
    ...
    if (!table.ReadU16(&os2->lower_optical_pointsize) || !table.ReadU16(&os2->upper_optical_pointsize)) {
...
bool ots_os2_serialise(OTSStream* out, Font* font)
{
    ...
    if (!out->WriteU32(os2->code_page_range_1) || !out->WriteU32(os2->code_page_range_2)) {
    ...
    if (!out->WriteS16(os2->x_height) || !out->WriteS16(os2->cap_height) || !out->WriteU16(os2->default_char) || !out->WriteU16(os2->break_char) || !out->WriteU16(os2->max_context)) {
    ...
    if (!out->WriteU16(os2->lower_optical_pointsize) || !out->WriteU16(os2->upper_optical_pointsize)) {
