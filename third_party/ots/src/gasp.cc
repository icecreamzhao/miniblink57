...
bool ots_gasp_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&gasp->version) || !table.ReadU16(&num_ranges)) {
    ...
    for (unsigned i = 0; i < num_ranges; ++i) {
        uint16_t max_ppem = 0;
        uint16_t behavior = 0;
        if (!table.ReadU16(&max_ppem) || !table.ReadU16(&behavior)) {
...
bool ots_gasp_serialise(OTSStream* out, Font* font)
{
    ...
    if (num_ranges != gasp->gasp_ranges.size() || !out->WriteU16(gasp->version) || !out->WriteU16(num_ranges)) {
        return OTS_FAILURE_MSG("failed to write gasp header");
    }

    for (uint16_t i = 0; i < num_ranges; ++i) {
        if (!out->WriteU16(gasp->gasp_ranges[i].first) || !out->WriteU16(gasp->gasp_ranges[i].second)) {
