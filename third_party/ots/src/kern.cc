...
bool ots_kern_parse(Font* font, const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&kern->version) || !table.ReadU16(&num_tables)) {
    ...
    for (unsigned i = 0; i < num_tables; ++i) {
        ...
        if (!table.ReadU16(&subtable.version) || !table.ReadU16(&sub_length)) {
        ...
        if (!table.ReadU16(&num_pairs) || !table.ReadU16(&subtable.search_range) || !table.ReadU16(&subtable.entry_selector) || !table.ReadU16(&subtable.range_shift)) {
        ...
        const uint16_t expected_range_shift = kFormat0PairSize * num_pairs - subtable.search_range;
        ...
        for (unsigned j = 0; j < num_pairs; ++j) {
            ...
            if (!table.ReadU16(&kerning_pair.left) || !table.ReadU16(&kerning_pair.right) || !table.ReadS16(&kerning_pair.value)) {
...
bool ots_kern_serialise(OTSStream* out, Font* font)
{
    ...
    if (num_subtables != kern->subtables.size() || !out->WriteU16(kern->version) || !out->WriteU16(num_subtables)) {
        return OTS_FAILURE_MSG("Can't write kern table header");
    }

    for (uint16_t i = 0; i < num_subtables; ++i) {
        const size_t length = 14 + (6 * kern->subtables[i].pairs.size());
        if (length > std::numeric_limits<uint16_t>::max() || !out->WriteU16(kern->subtables[i].version) || !out->WriteU16(static_cast<uint16_t>(length)) || !out->WriteU16(kern->subtables[i].coverage) || !out->WriteU16(static_cast<uint16_t>(kern->subtables[i].pairs.size())) || !out->WriteU16(kern->subtables[i].search_range) || !out->WriteU16(kern->subtables[i].entry_selector) || !out->WriteU16(kern->subtables[i].range_shift)) {
            return OTS_FAILURE_MSG("Failed to write kern subtable %d", i);
        }
        for (unsigned j = 0; j < kern->subtables[i].pairs.size(); ++j) {
            if (!out->WriteU16(kern->subtables[i].pairs[j].left) || !out->WriteU16(kern->subtables[i].pairs[j].right) || !out->WriteS16(kern->subtables[i].pairs[j].value)) {
