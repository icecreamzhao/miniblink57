    ...
    uint8_t* dst, size_t dst_size, size_t* glyph_size)
{
    ...
    unsigned int flag_offset = kEndPtsOfContoursOffset + 2 * n_contours + 2 + instruction_length;
    int last_flag = -1;
    int repeat_count = 0;
    int last_x = 0;
    int last_y = 0;
    ...
    size_t glyf_buf_length)
{
    ...
    for (unsigned int i = 0; i < n_glyphs; ++i) {
        if (buf[i >> 3] & (0x80 >> (i & 7))) {
            ...
            if (glyf_buf_length < 2 + 10 || loca_offset > glyf_buf_length - 2 - 10) {
                return OTS_FAILURE();
            }
    ...
    uint8_t* loca_buf, size_t loca_size)
{
    ...
    if (!file.ReadU16(&num_glyphs) || !file.ReadU16(&index_format)) {
        return OTS_FAILURE();
    }
    ...
    for (unsigned int i = 0; i < num_glyphs; ++i) {
        ...
        if (n_contours == 0xffff) {
            ...
            const uint8_t* triplet_buf = glyph_stream.buffer() + glyph_stream.offset();
            ...
            const uint32_t header_and_endpts_contours_size = kEndPtsOfContoursOffset + 2 * n_contours;
            ...
            // No integer overflow here (instruction_size < 2^16).
            if (glyf_dst_size - header_and_endpts_contours_size < instruction_size + 2) {
                return OTS_FAILURE();
            }
    ...
    uint8_t* dst, size_t dst_length)
{
    if (tag == TAG('g', 'l', 'y', 'f')) {
        ...
        if (static_cast<uint64_t>(glyf_table->dst_offset) + glyf_table->dst_length > dst_length) {
            return OTS_FAILURE();
        }
        if (static_cast<uint64_t>(loca_table->dst_offset) + loca_table->dst_length > dst_length) {
            return OTS_FAILURE();
        }
...
uint32_t ComputeChecksum(const uint8_t* buf, size_t size)
{
    ...
    for (size_t i = 0; i < size; i += 4) {
        ...
        checksum += (buf[i] << 24) | (buf[i + 1] << 16) | (buf[i + 2] << 8) | buf[i + 3];
    }
    return checksum;
}
...
bool FixChecksums(const std::vector<Table>& tables, uint8_t* dst)
{
    ...
    if (head_table == NULL || head_table->dst_length < kCheckSumAdjustmentOffset + 4) {
        return OTS_FAILURE();
    }
    ...
    size_t num_tables)
{
    for (size_t i = 0; i < num_tables; ++i) {
        ...
        if (tag == TAG('g', 'l', 'y', 'f') || tag == TAG('l', 'o', 'c', 'a')) {
        ...
        if (transform_length > 1024 * 1024 * 1024 || dst_length > 1024 * 1024 * 1024) {
            return OTS_FAILURE();
        }
        table->tag = tag;
        table->flags = flags;
...
size_t ComputeWOFF2FinalSize(const uint8_t* data, size_t length)
{
    ...
    if (!file.Skip(16) || !file.ReadU32(&total_length)) {
        return 0;
    }
    return total_length;
}
    ...
    const uint8_t* data, size_t length)
{
    ...
    uint32_t signature;
    uint32_t flavor;
    if (!file.ReadU32(&signature) || signature != kWoff2Signature || !file.ReadU32(&flavor)) {
        return OTS_FAILURE();
    }
    ...
    uint64_t dst_offset = kSfntHeaderSize + kSfntEntrySize * static_cast<uint64_t>(num_tables);
    ...
    for (uint16_t i = 0; i < num_tables; ++i) {
        ...
        if ((flags & kWoff2FlagsTransform) == 0) {
            ...
            if (static_cast<uint64_t>(table->dst_offset) + transform_length > result_length) {
                return OTS_FAILURE();
            }
