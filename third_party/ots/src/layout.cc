    ...
    const uint16_t num_features)
{
    ...
    if (!subtable->ReadU16(&offset_lookup_order) || !subtable->ReadU16(&req_feature_index) || !subtable->ReadU16(&feature_count)) {
    ...
    if (req_feature_index != kNoRequiredFeatureIndexDefined && req_feature_index >= num_features) {
    ...
    const uint32_t tag, const uint16_t num_features)
{
    ...
    if (!subtable.ReadU16(&offset_default_lang_sys) || !subtable.ReadU16(&lang_sys_count)) {
    ...
    const unsigned lang_sys_record_end = 6 * static_cast<unsigned>(lang_sys_count) + 4;
    ...
    for (unsigned i = 0; i < lang_sys_count; ++i) {
        if (!subtable.ReadU32(&lang_sys_records[i].tag) || !subtable.ReadU16(&lang_sys_records[i].offset)) {
        ...
        if (lang_sys_records[i].offset < lang_sys_record_end || lang_sys_records[i].offset >= length) {
    ...
    const uint16_t num_lookups)
{
    ...
    if (!subtable.ReadU16(&offset_feature_params) || !subtable.ReadU16(&lookup_count)) {
        return OTS_FAILURE_MSG("Failed to read feature table header");
    }

    const unsigned feature_table_end = 2 * static_cast<unsigned>(lookup_count) + 4;
    ...
    if (offset_feature_params != 0 && (offset_feature_params < feature_table_end || offset_feature_params >= length)) {
    ...
    const ots::LookupSubtableParser* parser)
{
    ...
    if (!subtable.ReadU16(&lookup_type) || !subtable.ReadU16(&lookup_flag) || !subtable.ReadU16(&subtable_count)) {
    ...
    // Check lookup flags.
    if ((lookup_flag & kGdefRequiredFlags) && (!font->gdef || !font->gdef->has_glyph_class_def)) {
        return OTS_FAILURE_MSG("Lookup flags require GDEF table, "
                               "but none was found: %d",
            lookup_flag);
    }
    if ((lookup_flag & kMarkAttachmentTypeMask) && (!font->gdef || !font->gdef->has_mark_attachment_class_def)) {
        return OTS_FAILURE_MSG("Lookup flags ask for mark attachment, "
                               "but there is no GDEF table or it has no "
                               "mark attachment classes: %d",
            lookup_flag);
    }
    ...
    const unsigned lookup_table_end = 2 * static_cast<unsigned>(subtable_count) + (use_mark_filtering_set ? 8 : 6);
    ...
    for (unsigned i = 0; i < subtable_count; ++i) {
        ...
        if (offset_subtable < lookup_table_end || offset_subtable >= length) {
    ...
    if (use_mark_filtering_set) {
        ...
        if (font->gdef->num_mark_glyph_sets == 0 || mark_filtering_set >= font->gdef->num_mark_glyph_sets) {
    ...
    const uint16_t num_classes)
{
    ...
    for (unsigned i = 0; i < range_count; ++i) {
        ...
        uint16_t end = 0;
        uint16_t class_value = 0;
        if (!subtable.ReadU16(&start) || !subtable.ReadU16(&end) || !subtable.ReadU16(&class_value)) {
    ...
    const uint16_t expected_num_glyphs)
{
    ...
    for (unsigned i = 0; i < range_count; ++i) {
        ...
        if (!subtable.ReadU16(&start) || !subtable.ReadU16(&end) || !subtable.ReadU16(&start_coverage_index)) {
    ...
    if (expected_num_glyphs && expected_num_glyphs != last_start_coverage_index) {
    ...
    const uint16_t num_lookups)
{
    ...
    if (!subtable->ReadU16(&sequence_index) || !subtable->ReadU16(&lookup_list_index)) {
    ...
    const uint16_t num_lookups)
{
    ...
    uint16_t glyph_count = 0;
    uint16_t lookup_count = 0;
    if (!subtable.ReadU16(&glyph_count) || !subtable.ReadU16(&lookup_count)) {
    ...
    const uint16_t num_lookups)
{
    ...
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&rule_set_count)) {
        return OTS_FAILURE_MSG("Failed to read header of context format 1");
    }

    const unsigned rule_set_end = static_cast<unsigned>(6) + rule_set_count * 2;
    ...
    const uint16_t num_lookups)
{
    ...
    const unsigned class_rule_end = 2 * static_cast<unsigned>(class_rule_count) + 2;
    ...
    const uint16_t num_lookups)
{
    ...
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&offset_class_def) || !subtable.ReadU16(&class_set_cnt)) {
    ...
    const uint16_t num_lookups)
{
    ...
    uint16_t lookup_count = 0;
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&glyph_count) || !subtable.ReadU16(&lookup_count)) {
    ...
    const unsigned lookup_record_end = 2 * static_cast<unsigned>(glyph_count) + 4 * static_cast<unsigned>(lookup_count) + 6;
    ...
    const uint16_t num_lookups)
{
    ...
    const unsigned chain_rule_end = 2 * static_cast<unsigned>(chain_rule_count) + 2;
    ...
    const uint16_t num_lookups)
{
    ...
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&chain_rule_set_count)) {
        return OTS_FAILURE_MSG("Failed to read header of chain context format 1");
    }

    const unsigned chain_rule_set_end = 2 * static_cast<unsigned>(chain_rule_set_count) + 6;
    ...
    for (unsigned i = 0; i < chain_rule_set_count; ++i) {
        ...
        if (offset_chain_rule_set < chain_rule_set_end || offset_chain_rule_set >= length) {
    ...
    const uint16_t num_lookups)
{
    ...
    const unsigned chain_class_rule_end = 2 * static_cast<unsigned>(chain_class_rule_count) + 2;
    ...
    for (unsigned i = 0; i < chain_class_rule_count; ++i) {
        ...
        if (offset_chain_class_rule < chain_class_rule_end || offset_chain_class_rule >= length) {
    ...
    const uint16_t num_lookups)
{
    ...
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&offset_backtrack_class_def) || !subtable.ReadU16(&offset_input_class_def) || !subtable.ReadU16(&offset_lookahead_class_def) || !subtable.ReadU16(&chain_class_set_count)) {
        return OTS_FAILURE_MSG("Failed to read header of chain context format 2");
    }

    const unsigned chain_class_set_end = 2 * static_cast<unsigned>(chain_class_set_count) + 12;
    ...
    if (offset_backtrack_class_def) {
        if (offset_backtrack_class_def < chain_class_set_end || offset_backtrack_class_def >= length) {
    ...
    if (offset_input_class_def < chain_class_set_end || offset_input_class_def >= length) {
    ...
    if (offset_lookahead_class_def) {
        if (offset_lookahead_class_def < chain_class_set_end || offset_lookahead_class_def >= length) {
    ...
    for (unsigned i = 0; i < chain_class_set_count; ++i) {
        ...
        if (offset_chain_class_set) {
            if (offset_chain_class_set < chain_class_set_end || offset_chain_class_set >= length) {
    ...
    const uint16_t num_lookups)
{
    ...
    uint16_t backtrack_count = 0;
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&backtrack_count)) {
    ...
    const unsigned lookup_record_end = 2 * (static_cast<unsigned>(backtrack_count) + static_cast<unsigned>(input_count) + static_cast<unsigned>(lookahead_count)) + 4 * static_cast<unsigned>(lookup_count) + 10;
    if (lookup_record_end > std::numeric_limits<uint16_t>::max()) {
        return OTS_FAILURE_MSG("Bad end of lookup record %d in chain context format 3", lookup_record_end);
    }
    for (unsigned i = 0; i < backtrack_count; ++i) {
        if (offsets_backtrack[i] < lookup_record_end || offsets_backtrack[i] >= length) {
    ...
    for (unsigned i = 0; i < lookahead_count; ++i) {
        if (offsets_lookahead[i] < lookup_record_end || offsets_lookahead[i] >= length) {
    ...
    const uint16_t num_features)
{
    ...
    const unsigned script_record_end = 6 * static_cast<unsigned>(script_count) + 2;
    ...
    for (unsigned i = 0; i < script_count; ++i) {
        ScriptRecord record;
        if (!subtable.ReadU32(&record.tag) || !subtable.ReadU16(&record.offset)) {
    ...
    uint16_t* num_features)
{
    ...
    const unsigned feature_record_end = 6 * static_cast<unsigned>(feature_count) + 2;
    ...
    uint32_t last_tag = 0;
    for (unsigned i = 0; i < feature_count; ++i) {
        if (!subtable.ReadU32(&feature_records[i].tag) || !subtable.ReadU16(&feature_records[i].offset)) {
        ...
        if (feature_records[i].offset < feature_record_end || feature_records[i].offset >= length) {
    ...
    uint16_t* num_lookups)
{
    ...
    const unsigned lookup_end = 2 * static_cast<unsigned>(*num_lookups) + 2;
    ...
    const uint8_t* data, size_t length)
{
    ...
    if (!subtable.ReadU16(&start_size) || !subtable.ReadU16(&end_size) || !subtable.ReadU16(&delta_format)) {
    ...
    const unsigned num_units = (end_size - start_size) / (1 << (4 - delta_format)) + 1;
    ...
    const LookupSubtableParser* parser)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&lookup_type) || !subtable.ReadU32(&offset_extension)) {
    ...
    if (lookup_type < 1 || lookup_type > parser->num_types || lookup_type == parser->extension_type) {
        return OTS_FAILURE_MSG("Bad lookup type %d in extension table", lookup_type);
    }

    const unsigned format_end = static_cast<unsigned>(8);
    if (offset_extension < format_end || offset_extension >= length) {
