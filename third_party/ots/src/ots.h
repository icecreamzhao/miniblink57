...
public:
    ...
    bool Read(uint8_t* buf, size_t n_bytes)
    {
        ...
        if ((offset_ + n_bytes > length_) || (offset_ > length_ - n_bytes)) {
            return OTS_FAILURE();
        }
        if (buf) {
    ...
    bool ReadU24(uint32_t* value)
    {
        ...
        *value = static_cast<uint32_t>(buffer_[offset_]) << 16 | static_cast<uint32_t>(buffer_[offset_ + 1]) << 8 | static_cast<uint32_t>(buffer_[offset_ + 2]);
        offset_ += 3;
        return true;
    }
