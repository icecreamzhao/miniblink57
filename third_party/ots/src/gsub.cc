    ...
    const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage)) {
    ...
    const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&sequence_count)) {
    ...
    const uint16_t num_glyphs = font->maxp->num_glyphs;
    const unsigned sequence_end = static_cast<unsigned>(6) + sequence_count * 2;
    ...
    const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&alternate_set_count)) {
    ...
    const uint16_t num_glyphs = font->maxp->num_glyphs;
    const unsigned alternate_set_end = static_cast<unsigned>(6) + alternate_set_count * 2;
    ...
    for (unsigned i = 0; i < alternate_set_count; ++i) {
        ...
        if (offset_alternate_set < alternate_set_end || offset_alternate_set >= length) {
    ...
    const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&lig_glyph) || !subtable.ReadU16(&comp_count)) {
    ...
    const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&lig_set_count)) {
    ...
    const uint16_t num_glyphs = font->maxp->num_glyphs;
    const unsigned ligature_set_end = static_cast<unsigned>(6) + lig_set_count * 2;
    ...
    for (unsigned i = 0; i < lig_set_count; ++i) {
        ...
        if (offset_ligature_set < ligature_set_end || offset_ligature_set >= length) {
    ...
    const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    const unsigned substitute_end = static_cast<unsigned>(10) + (backtrack_glyph_count + lookahead_glyph_count + glyph_count) * 2;
    ...
    for (unsigned i = 0; i < backtrack_glyph_count; ++i) {
        if (offsets_backtrack[i] < substitute_end || offsets_backtrack[i] >= length) {
    ...
    for (unsigned i = 0; i < lookahead_glyph_count; ++i) {
        if (offsets_lookahead[i] < substitute_end || offsets_lookahead[i] >= length) {
