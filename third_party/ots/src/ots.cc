    ...
    uint32_t offset = 0)
{
    ...
    if (!file.ReadU16(&font->num_tables) || !file.ReadU16(&font->search_range) || !file.ReadU16(&font->entry_selector) || !file.ReadU16(&font->range_shift)) {
    ...
    const uint16_t expected_range_shift = 16 * font->num_tables - font->search_range;
    ...
    for (unsigned i = 0; i < font->num_tables; ++i) {
        OpenTypeTable table;
        if (!file.ReadU32(&table.tag) || !file.ReadU32(&table.chksum) || !file.ReadU32(&table.offset) || !file.ReadU32(&table.length)) {
    ...
    uint32_t index)
{
    ...
    if (index == static_cast<uint32_t>(-1)) {
        if (!output->WriteU32(ttc_tag) || !output->WriteU32(0x00010000) || !output->WriteU32(num_fonts) || !output->Seek((3 + num_fonts) * 4)) {
        ...
        for (unsigned i = 0; i < num_fonts; i++) {
            ...
            if (!output->Seek((3 + i) * 4) || !output->WriteU32(out_offset) || !output->Seek(out_offset)) {
                return OTS_FAILURE_MSG_HDR("Error writing output");
            }
    ...
    ots::OTSStream* output, const uint8_t* data, size_t length)
{
    ...
    if (!file.ReadU32(&meta_offset) || !file.ReadU32(&meta_length) || !file.ReadU32(&meta_length_orig)) {
    ...
    if (!file.ReadU32(&priv_offset) || !file.ReadU32(&priv_length)) {
    ...
    for (unsigned i = 0; i < font->num_tables; ++i) {
        OpenTypeTable table;
        if (!file.ReadU32(&table.tag) || !file.ReadU32(&table.offset) || !file.ReadU32(&table.length) || !file.ReadU32(&table.uncompressed_length) || !file.ReadU32(&table.chksum)) {
            return OTS_FAILURE_MSG_HDR("error reading table directory");
        }
    ...
    if (tables[last_index].offset >= length || length - tables[last_index].offset < tables[last_index].length) {
    ...
    uint64_t block_end = ots::Round4(
        static_cast<uint64_t>(tables[last_index].offset) + static_cast<uint64_t>(tables[last_index].length));
    ...
    if (meta_offset) {
        ...
        block_end = ots::Round4(static_cast<uint64_t>(meta_offset) + static_cast<uint64_t>(meta_length));
        if (block_end > std::numeric_limits<uint32_t>::max()) {
    ...
    if (priv_offset) {
        ...
        block_end = ots::Round4(static_cast<uint64_t>(priv_offset) + static_cast<uint64_t>(priv_length));
        if (block_end > std::numeric_limits<uint32_t>::max()) {
    ...
    ots::Buffer& file)
{
    ...
    for (unsigned i = 0;; ++i) {
        ...
        if (ot == header->tables.end()) {
            ...
            if (action == ots::TABLE_ACTION_SANITIZE && !table_parsers[i].parse(font, table_data, table_length)) {
                return OTS_FAILURE();
            }
...
#define PASSTHRU_TABLE(tag_) (table_map.find(tag_) != table_map.end() && GetTableAction(header, tag_) == ots::TABLE_ACTION_PASSTHRU)
            // We don't sanitise bitmap table, but don't reject bitmap-only fonts if
            // we keep the tables.
            if (!PASSTHRU_TABLE(OTS_TAG('C', 'B', 'D', 'T')) || !PASSTHRU_TABLE(OTS_TAG('C', 'B', 'L', 'C'))) {
...
#undef PASSTHRU_TABLE
    ...
    for (std::map<uint32_t, OpenTypeTable>::const_iterator it = table_map.begin();
        ...
        if (action == ots::TABLE_ACTION_PASSTHRU) {
            ...
            for (unsigned i = 0; table_parsers[i].parse != NULL; ++i) {
                if (table_parsers[i].tag == it->first && table_parsers[i].should_serialise(font)) {
                    num_output_tables++;
                    break;
                }
            }
    ...
    if (!output->WriteU32(font->version) || !output->WriteU16(num_output_tables) || !output->WriteU16(output_search_range) || !output->WriteU16(max_pow2) || !output->WriteU16((num_output_tables << 4) - output_search_range)) {
        return OTS_FAILURE_MSG_HDR("error writing output");
    }
    ...
    for (unsigned i = 0; i < out_tables.size(); ++i) {
        if (!output->WriteU32(out_tables[i].tag) || !output->WriteU32(out_tables[i].chksum) || !output->WriteU32(out_tables[i].offset) || !output->WriteU32(out_tables[i].length)) {
            return OTS_FAILURE_MSG_HDR("error writing output");
        }
...
bool IsValidVersionTag(uint32_t tag)
{
    return tag == 0x000010000 ||
        ...
        tag == OTS_TAG('t', 'r', 'u', 'e') || tag == OTS_TAG('t', 'y', 'p', '1');
}
