    ...
    , is_directory_(false)
{
    ...
    if (file_path_.IsAbsolute() || base::StartsWith(file_name_in_zip, "/", base::CompareCase::INSENSITIVE_ASCII))
        is_unsafe_ = true;
    ...
    const base::FilePath& output_file_path) const
{
    ...
    if (success && current_entry_info()->last_modified() != base::Time::UnixEpoch()) {
