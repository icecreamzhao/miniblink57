...
TEST_F(ZipReaderTest, OpenFromString)
{
    ...
    const char kTestData[] = "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00\xa4\x66\x24\x41\x13\xe8"
...
TEST_F(ZipReaderTest, ExtractCurrentEntryToString)
{
    ...
    base::FilePath test_zip_file = test_data_dir_.AppendASCII("test_mismatch_size.zip");
...
protected:
    void SetUp() override
    {
        ...
        file_.Initialize(temp_file_path_, (base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_TEMPORARY | base::File::FLAG_DELETE_ON_CLOSE));
