...
protected:
    ...
    void TestUnzipFile(const base::FilePath& path, bool expect_hidden_files)
    {
        ...
        while (!next_path.value().empty()) {
            if (next_path.value().find(FILE_PATH_LITERAL(".svn")) == base::FilePath::StringType::npos) {
                EXPECT_EQ(zip_contents_.count(next_path), 1U) << "Couldn't find " << next_path.value();
                count++;
            }
...
TEST_F(ZipTest, ZipNonASCIIDir)
{
    ...
    base::FilePath src_dir_russian = temp_dir.path().Append(base::FilePath::FromUTF8Unsafe(
...
TEST_F(ZipTest, UnzipFilesWithIncorrectSize)
{
    ...
    base::FilePath test_zip_file = test_data_folder.AppendASCII("test_mismatch_size.zip");
