...
//     }
// 
//     inline void operator= (const UnicodeString& text) const
//     {
//         return;
//     }
// 
//     inline UnicodeString operator= (const wchar_t* text)
//     {
...
//     }
// 
//     static UnicodeString fromUTF8(const char* utf8)
//     {
//         return UnicodeString();
//     }
// 
...
//     }
// 
//     inline unsigned char caseCompare(const UnicodeString &text, unsigned int options) const
//     {
//         return 0;
//     }
// };
...
void ConstructName(wchar_t* name_part, std::string* name, int32_t name_id) {
  switch (name_id) {
    case NameId::kFullFontName:
      *name = fromWideToUtf8(name_part);
      break;
...
bool HasName(const char* font_name, Font* font) {
  std::string font_string = fromUTF8(font_name);
  if (0 == font_string.size())
    return false;
  std::string regular_suffix = fromUTF8(" Regular");
  std::string alt_font_string = font_string;
  alt_font_string += regular_suffix;

  typedef std::map<int32_t, std::string> NameMap;
  NameMap names;
  ...
  for (int32_t i = 0; i < name_table->NameCount(); ++i) {
    switch (name_table->NameId(i)) {
      ...
      case NameId::kWWSFamilyName:
      case NameId::kWWSSubfamilyName: {
          wchar_t* name_part = name_table->Name(i);
        if (name_part == NULL) {
          continue;
        }
  ...
  if (!names.empty()) {
    for (NameMap::iterator i = names.begin(), e = names.end(); i != e; ++i) {
      if (caseCompare(i->second, font_string) || caseCompare(i->second, alt_font_string) ) {
        return true;
      }
    }
  }
...
bool ResolveCompositeGlyphs(GlyphTable* glyph_table,
  ...
  int id_remaining = 0;
  glyph_id_remaining.insert(id_remaining);  // Always include glyph id 0.
