...
//     memset(result, 0, sizeof(wchar_t) * 12);
...
//   return NULL;
  ...
  if (0 != strcmp(encoding, "UTF-16BE") || 0 != ((name_bytes->size()) % 2)) {
    memcpy(output_buffer, ptr, name_bytes->size());
  } else { // 不管是不是，强制当UTF-16BE转 // weolar
    for (size_t i = 0; i < name_bytes->size(); i += 2) {
      output_buffer[i] = ptr[i + 1];
      output_buffer[i + 1] = ptr[i];
    }
  }
