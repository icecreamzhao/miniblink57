...
public:
    ...
    using CacheWithResponsesCallbacks = WebCallbacks<const WebVector<WebServiceWorkerResponse>&,
        WebServiceWorkerCacheError>;
    using CacheWithRequestsCallbacks = WebCallbacks<const WebVector<WebServiceWorkerRequest>&,
        WebServiceWorkerCacheError>;
    ...
    struct BatchOperation {
        BatchOperation()
            : operationType(OperationTypeUndefined)
        {
        }
    ...
    virtual void dispatchBatch(std::unique_ptr<CacheBatchCallbacks>,
        const WebVector<BatchOperation>&)
        = 0;
};
