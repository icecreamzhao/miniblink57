...
private:
    ...
    bool shouldShrink() const
    {
        ...
        // expensive.
        return m_keyCount * m_minLoad < m_tableSize && m_tableSize > KeyTraits::minimumTableSize && Allocator::isAllocationAllowed();
    }
...
#endif
{
    static_assert(Allocator::isGarbageCollected || (!IsPointerToGarbageCollectedType<Key>::value && !IsPointerToGarbageCollectedType<Value>::value),
    ...
    Allocator>::reserveCapacityForSize(unsigned newSize)
{
    ...
    if (newCapacity > capacity()) {
        RELEASE_ASSERT(!static_cast<int>(
            newCapacity >> 31)); // HashTable capacity should not overflow 32bit int.
    ...
    allocateTable(unsigned size)
{
    ...
    static_assert(Allocator::isGarbageCollected || ((!AllowsOnlyPlacementNew<KeyType>::value || !IsTraceable<KeyType>::value) && (!AllowsOnlyPlacementNew<ValueType>::value || !IsTraceable<ValueType>::value)),
