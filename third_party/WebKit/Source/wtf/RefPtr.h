...
public:
    ...
    ALWAYS_INLINE explicit RefPtr(T& ref)
        : m_ptr(&ref)
    {
        m_ptr->ref();
    }
...
public:
    ALWAYS_INLINE RefPtrValuePeeker(T* p)
        : m_ptr(p)
    {
    }
    ALWAYS_INLINE RefPtrValuePeeker(std::nullptr_t)
        : m_ptr(nullptr)
    {
    }
    template <typename U>
    RefPtrValuePeeker(const RefPtr<U>& p)
        : m_ptr(p.get())
    {
    }
    template <typename U>
    RefPtrValuePeeker(const PassRefPtr<U>& p)
        : m_ptr(p.get())
    {
    }
