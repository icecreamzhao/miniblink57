    ...
    CustomElementRegistry* registry)
{
    ...
    v8::Local<v8::String> name = V8HiddenValue::customElementsRegistryMap(scriptState->isolate());
    v8::Local<v8::Object> wrapper = ToV8(registry, scriptState).As<v8::Object>();
    v8::Local<v8::Value> map = V8HiddenValue::getHiddenValue(scriptState, wrapper, name);
    if (map.IsEmpty()) {
...
{
    v8::Local<v8::Map> map = ensureCustomElementRegistryMap(scriptState, registry);
    ...
    const HashSet<AtomicString>& observedAttributes)
{
    ...
    v8::Local<v8::Value> nameValue = v8String(scriptState->isolate(), descriptor.name());
    v8::Local<v8::Map> map = ensureCustomElementRegistryMap(scriptState, registry);
    ...
    const QualifiedName& tagName)
{
    ...
    Element* element = nullptr;
    {
        ...
        bool isImportDocument = document.importsController() && document.importsController()->master() != document;
        if (isImportDocument) {
    ...
    v8::Local<v8::Value> argv[])
{
    ...
    ExecutionContext* executionContext = m_scriptState->getExecutionContext();
    v8::Local<v8::Value> elementHandle = ToV8(element, m_scriptState->context()->Global(), isolate);
