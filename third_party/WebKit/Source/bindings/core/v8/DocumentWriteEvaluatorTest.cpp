...
namespace {
    ...
    public:
        DocumentWriteEvaluatorTest()
            : m_evaluator(new DocumentWriteEvaluator("/path/",
                "www.example.com",
                "http:",
                "userAgent"))
        {
...
{
    String written = m_evaluator->evaluateAndEmitWrittenSource("var a = 2;");
    EXPECT_EQ("", written);
}
...
TEST_F(DocumentWriteEvaluatorTest, MultipleScripts)
{
    ...
    String written2 = m_evaluator->evaluateAndEmitWrittenSource("write('Hello');");
