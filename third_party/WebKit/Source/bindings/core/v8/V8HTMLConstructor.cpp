    ...
    const HTMLElementType elementInterfaceName)
{
    ...
    if (!RuntimeEnabledFeatures::customElementsV1Enabled() || !scriptState->world().isMainWorld()) {
    ...
    v8::Local<v8::Function> activeFunctionObject = scriptState->perContextData()->constructorForType(
    ...
    // steps.
    ScriptCustomElementDefinition* definition = ScriptCustomElementDefinition::forConstructor(scriptState, registry,
        newTarget);
    if (!definition) {
        V8ThrowException::throwTypeError(isolate, "Illegal constructor");
        return;
    ...
    if (!prototype->IsObject()) {
        if (V8PerContextData* perContextData = V8PerContextData::from(
            ...
            prototype = perContextData->prototypeForType(&V8HTMLElement::wrapperTypeInfo);
        } else {
            V8ThrowException::throwError(isolate, "The context has been destroyed");
            return;
        }
