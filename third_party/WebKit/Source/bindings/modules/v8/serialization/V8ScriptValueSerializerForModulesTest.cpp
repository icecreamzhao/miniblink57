...
namespace {
        ...
        V8TestingScope& scope)
    {
        ...
        RefPtr<SerializedScriptValue> serializedScriptValue = V8ScriptValueSerializerForModules(scriptState)
    ...
    T subtleCryptoSync(ScriptState* scriptState, PMF func, Args&&... args)
    {
        ...
        (Platform::current()->crypto()->*func)(
            ...
            toWebCryptoResult(scriptState, WTF::bind([](T* out, T result) {
                *out = result;
                testing::exitRunLoop();
            },
    ...
    TEST(V8ScriptValueSerializerForModulesTest, RoundTripCryptoKeyAES)
    {
        ...
        CryptoKey* key = syncGenerateKey(scriptState, algorithm, true,
        ...
        WebVector<uint8_t> keyRaw = syncExportKey(scriptState, WebCryptoKeyFormatRaw, key->key());
        WebVector<uint8_t> newKeyRaw = syncExportKey(scriptState, WebCryptoKeyFormatRaw, newKey->key());
        ...
        WebVector<uint8_t> ciphertext = syncEncrypt(scriptState, encryptAlgorithm, key->key(), plaintext);
        WebVector<uint8_t> newPlaintext = syncDecrypt(scriptState, encryptAlgorithm, newKey->key(), ciphertext);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, DecodeCryptoKeyAES)
    {
        ...
        RefPtr<SerializedScriptValue> input = serializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4b, 0x01, 0x01, 0x10, 0x04,
        ...
        WebCryptoAlgorithm encryptAlgorithm(
            WebCryptoAlgorithmIdAesCbc, WTF::makeUnique<WebCryptoAesCbcParams>(iv));
        WebVector<uint8_t> plaintext = syncDecrypt(scriptState, encryptAlgorithm, newKey->key(), ciphertext);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, RoundTripCryptoKeyHMAC)
    {
        ...
        CryptoKey* key = syncGenerateKey(scriptState, generateKeyAlgorithm, true,
        ...
        WebVector<uint8_t> signature = syncSign(scriptState, algorithm, key->key(), message);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, RoundTripCryptoKeyRSAHashed)
    {
        ...
        std::tie(publicKey, privateKey) = syncGenerateKeyPair(scriptState, generateKeyAlgorithm, true,
            WebCryptoKeyUsageSign | WebCryptoKeyUsageVerify);
        ...
        WebVector<uint8_t> keyRaw = syncExportKey(scriptState, WebCryptoKeyFormatPkcs8, privateKey->key());
        WebVector<uint8_t> newKeyRaw = syncExportKey(scriptState, WebCryptoKeyFormatPkcs8, newPrivateKey->key());
        EXPECT_THAT(newKeyRaw, ElementsAreArray(keyRaw));
        ...
        WebVector<uint8_t> signature = syncSign(scriptState, algorithm, newPrivateKey->key(), message);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, RoundTripCryptoKeyNoParams)
    {
        ...
        WebVector<uint8_t> bitsRaw = syncDeriveBits(scriptState, algorithm, key->key(), 16);
        WebVector<uint8_t> newBitsRaw = syncDeriveBits(scriptState, algorithm, newKey->key(), 16);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, DecodeCryptoKeyNoParams)
    {
        ...
        RefPtr<SerializedScriptValue> input = serializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4b, 0x06, 0x11, 0xa0, 0x02,
        ...
        WebCryptoAlgorithm algorithm(WebCryptoAlgorithmIdPbkdf2, std::move(params));
        WebVector<uint8_t> bitsRaw = syncDeriveBits(scriptState, algorithm, newKey->key(), 32);
    ...
    TEST(V8ScriptValueSerializerForModulesTest, DecodeCryptoKeyInvalid)
    {
        ...
        EXPECT_TRUE(
            V8ScriptValueDeserializerForModules(
                scriptState, serializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4b, 0x04, 0x0d, 0x01, 0x80, 0x08, 0x03, 0x01 }))
                .deserialize()
                ->IsNull());
    }
