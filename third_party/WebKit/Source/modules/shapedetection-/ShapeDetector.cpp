...
namespace {
        ...
        int size)
    {
        ...
        mojo::ScopedSharedBufferHandle sharedBufferHandle = mojo::SharedBufferHandle::Create(size);
        ...
        const mojo::ScopedSharedBufferMapping mappedBuffer = sharedBufferHandle->Map(size);
    ...
    ImageData* imageData)
{
    ...
    mojo::ScopedSharedBufferHandle sharedBufferHandle = getSharedBufferOnData(resolver, data, allocationSize.ValueOrDefault(0));
    if (!sharedBufferHandle->is_valid())
        return promise;
    ...
    const HTMLImageElement* img)
{
    ...
    const SkImageInfo skiaInfo = SkImageInfo::MakeN32(image->width(), image->height(), image->alphaType());
    ...
    const mojo::ScopedSharedBufferMapping mappedBuffer = sharedBufferHandle->Map(allocationSize);
