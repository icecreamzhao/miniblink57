...
enum BitmapOpacity { OpaqueBitmap,
    TransparentBitmap };
...
#define TEST_OVERDRAW_SETUP(EXPECTED_OVERDRAWS)                                                                                             \
    std::unique_ptr<MockImageBufferSurfaceForOverwriteTesting> mockSurface = WTF::wrapUnique(new MockImageBufferSurfaceForOverwriteTesting( \
...
public:
    enum FallbackExpectation { ExpectFallback,
        ExpectNoFallback };
...
TEST_F(CanvasRenderingContext2DTest, detectOverdrawWithDrawImage)
{
    ...
    TEST_OVERDRAW_1(
        1, drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0, 10, 10, 0, 0, 10, 10, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_1(
        1, drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0, 1, 1, 0, 0, 10, 10, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_2(
    ...
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_1(
        0, drawImage(canvasElement().getExecutionContext(), &m_alphaBitmap, 0, 0, 10, 10, 0, 0, 10, 10, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_2(
        0, setGlobalAlpha(0.5f),
    ...
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_1(
        0, drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0, 10, 10, 1, 0, 10, 10, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_1(
        0, drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0, 10, 10, 0, 0, 9, 9, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_1(
        1, drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0, 10, 10, 0, 0, 11, 11, exceptionState));
    EXPECT_FALSE(exceptionState.hadException());
    TEST_OVERDRAW_2(
        1, translate(-1, 0),
        drawImage(canvasElement().getExecutionContext(), &m_opaqueBitmap, 0, 0,
...
{
    createContext(NonOpaque);
    std::unique_ptr<RecordingImageBufferSurface> surface = WTF::wrapUnique(new RecordingImageBufferSurface(
        IntSize(10, 10),
...
TEST_F(CanvasRenderingContext2DTest, NoLayerPromotionUnderImageSizeRatioLimit)
{
    ...
    std::unique_ptr<UnacceleratedImageBufferSurface> sourceSurface = WTF::makeUnique<UnacceleratedImageBufferSurface>(sourceSize, NonOpaque);
...
TEST_F(CanvasRenderingContext2DTest, GPUMemoryUpdateForAcceleratedCanvas)
{
    ...
    std::unique_ptr<FakeAcceleratedImageBufferSurfaceForTesting>
        fakeAccelerateSurface = WTF::wrapUnique(new FakeAcceleratedImageBufferSurfaceForTesting(
            IntSize(10, 10), NonOpaque));
    ...
    std::unique_ptr<FakeAcceleratedImageBufferSurfaceForTesting>
        fakeAccelerateSurface2 = WTF::wrapUnique(new FakeAcceleratedImageBufferSurfaceForTesting(
...
TEST_F(CanvasRenderingContext2DTest, GetImageDataDisablesAcceleration)
{
    ...
    RefPtr<Canvas2DLayerBridge> bridge = makeBridge(std::move(contextProvider), size,
...
TEST_F(CanvasRenderingContext2DTest, TextureUploadHeuristics)
{
    ...
    for (int testVariant = 0; testVariant < TestVariantCount; testVariant++) {
        ...
        int srcSize = std::sqrt(static_cast<float>(ExpensiveCanvasHeuristicParameters::
    ...
    LegacyColorSpaceUsesGlobalTargetColorBehavior)
{
    ...
    // srgb)
    sk_sp<SkColorSpace> savedGlobalTargetColorSpace = ColorBehavior::globalTargetColorSpace();
    ColorBehavior::setGlobalTargetColorSpaceForTesting(
        SkColorSpace::MakeNamed(SkColorSpace::kAdobeRGB_Named));
    bool savedColorCorrectRenderingEnabled = RuntimeEnabledFeatures::colorCorrectRenderingEnabled();
    ...
    LinearRGBColorSpaceUsesTransformToLinearSRGBColorBehavior)
{
    ...
    ColorBehavior behavior = context2d()->drawImageColorBehavior();
    sk_sp<SkColorSpace> linearSrgbColorSpace = SkColorSpace::MakeNamed(SkColorSpace::kSRGBLinear_Named);
    EXPECT_TRUE(behavior.isTransformToTargetColorSpace());
    ...
    const ColorSpaceConversion& colorSpaceConversion)
{
    ...
    bool flag = (colorSpaceConversion != ColorSpaceConversion::DEFAULT_NOT_COLOR_CORRECTED);
...
{
    bool experimentalCanvasFeaturesRuntimeFlag = RuntimeEnabledFeatures::experimentalCanvasFeaturesEnabled();
    bool colorCorrectRenderingRuntimeFlag = RuntimeEnabledFeatures::colorCorrectRenderingEnabled();
    bool colorCorrectRenderingDefaultModeRuntimeFlag = RuntimeEnabledFeatures::colorCorrectRenderingDefaultModeEnabled();
    ...
    NonThrowableExceptionState exceptionState;
    uint8_t* srcPixel = context->getImageData(5, 5, 1, 1, exceptionState)->data()->data();
    // Swizzle if needed
    ...
    SkColorSpaceXform::ColorFormat colorFormat32 = (colorType == kBGRA_8888_SkColorType)
        ? SkColorSpaceXform::ColorFormat::kBGRA_8888_ColorFormat
        : SkColorSpaceXform::ColorFormat::kRGBA_8888_ColorFormat;
    SkColorSpaceXform::ColorFormat colorFormat = colorFormat32;
    sk_sp<SkColorSpace> srcRGBColorSpace = SkColorSpace::MakeNamed(SkColorSpace::kSRGB_Named);
    ...
    for (uint8_t i = static_cast<uint8_t>(
        ...
        ColorSpaceConversion colorSpaceConversion = static_cast<ColorSpaceConversion>(i);
        ImageBitmapOptions options = prepareBitmapOptionsAndSetRuntimeFlags(colorSpaceConversion);
        ...
        SkImage* convertedImage = imageBitmap->bitmapImage()
        ...
        // is done correctly.
        std::unique_ptr<SkColorSpaceXform> colorSpaceXform = SkColorSpaceXform::New(srcRGBColorSpace.get(), colorSpace.get());
