...
BluetoothRemoteGATTCharacteristic::BluetoothRemoteGATTCharacteristic(
    ...
    , m_device(device)
{
    m_properties = BluetoothCharacteristicProperties::create(m_characteristic->properties);
}
    ...
    const Optional<Vector<uint8_t>>& value)
{
    ...
    if (result == mojom::blink::WebBluetoothResult::SUCCESS) {
        DCHECK(value);
        DOMDataView* domDataView = BluetoothRemoteGATTUtils::ConvertWTFVectorToDataView(value.value());
    ...
    const DOMArrayPiece& value)
{
    ...
    if (value.byteLength() > 512)
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(InvalidModificationError, "Value can't exceed 512 bytes."));
...
{
    String descriptor = BluetoothUUID::getDescriptor(descriptorUUID, exceptionState);
