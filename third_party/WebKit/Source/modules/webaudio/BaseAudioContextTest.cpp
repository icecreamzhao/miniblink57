...
namespace {
    ...
    private:
        explicit MockCrossOriginFrameLoaderClient(Frame* parent)
            : m_parent(parent)
        {
        }
...
TEST_F(BaseAudioContextTest, AutoplayMetrics_NoRestriction)
{
    ...
    BaseAudioContext* audioContext = BaseAudioContext::create(document(), ASSERT_NO_EXCEPTION);
...
TEST_F(BaseAudioContextTest, AutoplayMetrics_CreateNoGesture)
{
    ...
    BaseAudioContext* audioContext = BaseAudioContext::create(childDocument(), ASSERT_NO_EXCEPTION);
    recordAutoplayStatus(audioContext);
