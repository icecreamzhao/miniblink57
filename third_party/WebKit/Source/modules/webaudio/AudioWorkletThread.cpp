...
void AudioWorkletThread::collectAllGarbage()
{
    ...
    WorkletThreadHolder<AudioWorkletThread>* workletThreadHolder = WorkletThreadHolder<AudioWorkletThread>::getInstance();
    if (!workletThreadHolder)
        return;
    workletThreadHolder->thread()->backingThread().postTask(
        BLINK_FROM_HERE, crossThreadBind(&collectAllGarbageOnAudioWorkletThread, crossThreadUnretained(&doneEvent)));
    doneEvent.wait();
}
