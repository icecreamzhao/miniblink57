    ...
    AXObjectCacheImpl& cache) const
{
    ...
    std::unique_ptr<AXNode> parentNodeObject = buildProtocolAXObject(*parentAXObject, nullptr, true, nodes, cache);
    std::unique_ptr<protocol::Array<AXNodeId>> childIds = protocol::Array<AXNodeId>::create();
...
{
    AccessibilityRole role = axObject.roleValue();
    std::unique_ptr<AXNode> nodeObject = AXNode::create()
    ...
    AXObjectCacheImpl& cache) const
{
    ...
    if (&axObject != inspectedAXObject || (inspectedAXObject && !inspectedAXObject->accessibilityIsIgnored())) {
...
void InspectorAccessibilityAgent::addChildren(
    ...
    AXObjectCacheImpl& cache) const
{
    if (inspectedAXObject && inspectedAXObject->accessibilityIsIgnored() && &axObject == inspectedAXObject->parentObjectUnignored()) {
