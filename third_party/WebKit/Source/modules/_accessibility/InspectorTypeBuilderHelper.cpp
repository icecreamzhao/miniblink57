    ...
    String* name = nullptr)
{
    ...
    if (!backendNodeId)
        return nullptr;
    std::unique_ptr<AXRelatedNode> relatedNode = AXRelatedNode::create().setBackendDOMNodeId(backendNodeId).build();
    ...
    const String& valueType)
{
    ...
    return AXValue::create()
        .setType(valueType)
        .setRelatedNodes(std::move(relatedNodes))
        .build();
}
    ...
    const String& valueType)
{
    ...
    return AXValue::create()
        .setType(valueType)
        .setRelatedNodes(std::move(frontendRelatedNodes))
        .build();
}
...
std::unique_ptr<AXValueSource> createValueSource(NameSource& nameSource)
{
    ...
    if (!nameSource.relatedObjects.isEmpty()) {
        if (nameSource.attribute == aria_labelledbyAttr || nameSource.attribute == aria_labeledbyAttr) {
            std::unique_ptr<AXValue> attributeValue = createRelatedNodeListValue(
                nameSource.relatedObjects, AXValueTypeEnum::IdrefList);
            if (!nameSource.attributeValue.isNull())
                attributeValue->setValue(protocol::StringValue::create(
                    nameSource.attributeValue.getString()));
            valueSource->setAttributeValue(std::move(attributeValue));
        } else if (nameSource.attribute == QualifiedName::null()) {
            valueSource->setNativeSourceValue(createRelatedNodeListValue(
    ...
    if (!nameSource.text.isNull())
        valueSource->setValue(
            createValue(nameSource.text, AXValueTypeEnum::ComputedString));
