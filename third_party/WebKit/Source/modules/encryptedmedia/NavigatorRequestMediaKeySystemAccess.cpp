...
namespace {
        ...
        const WebMediaKeySystemConfiguration& config)
    {
        ...
        if (!config.audioCapabilities.isEmpty()) {
            areAllAudioCodecsEmpty = std::find_if(config.audioCapabilities.begin(),
        ...
        if (!config.videoCapabilities.isEmpty()) {
            areAllVideoCodecsEmpty = std::find_if(config.videoCapabilities.begin(),
    ...
    const HeapVector<MediaKeySystemConfiguration>& supportedConfigurations)
{
    ...
    if (keySystem != "org.w3.clearkey") {
        ...
        if (!document->settings() || !document->settings()->getEncryptedMediaEnabled()) {
    ...
    if (!supportedConfigurations.size()) {
        return ScriptPromise::reject(
            scriptState, V8ThrowException::createTypeError(scriptState->isolate(), "The supportedConfigurations parameter is empty."));
    }
    ...
    MediaKeySystemAccessInitializer* initializer = new MediaKeySystemAccessInitializer(scriptState, keySystem,
