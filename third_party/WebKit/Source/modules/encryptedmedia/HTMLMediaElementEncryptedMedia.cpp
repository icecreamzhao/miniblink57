...
ScriptPromise SetMediaKeysHandler::create(ScriptState* scriptState,
...
SetMediaKeysHandler::SetMediaKeysHandler(ScriptState* scriptState,
    HTMLMediaElement& element,
    MediaKeys* mediaKeys)
...
void SetMediaKeysHandler::clearExistingMediaKeys()
{
    ...
    if (thisElement.m_mediaKeys) {
        ...
        if (mediaPlayer) {
            ...
            std::unique_ptr<SuccessCallback> successCallback = WTF::bind(
            ...
            ContentDecryptionModuleResult* result = new SetContentDecryptionModuleResult(std::move(successCallback),
...
void SetMediaKeysHandler::setNewMediaKeys()
{
    ...
    if (m_newMediaKeys) {
        ...
        if (m_element->webMediaPlayer()) {
            ...
            ContentDecryptionModuleResult* result = new SetContentDecryptionModuleResult(std::move(successCallback),
                std::move(failureCallback));
            m_element->webMediaPlayer()->setContentDecryptionModule(
                m_newMediaKeys->contentDecryptionModule(), result->result());
...
HTMLMediaElementEncryptedMedia& HTMLMediaElementEncryptedMedia::from(
    HTMLMediaElement& element)
{
    HTMLMediaElementEncryptedMedia* supplement = static_cast<HTMLMediaElementEncryptedMedia*>(
...
static Event* createEncryptedEvent(WebEncryptedMediaInitDataType initDataType,
    const unsigned char* initData,
    unsigned initDataLength)
{
    MediaEncryptedEventInit initializer;
    initializer.setInitDataType(
        EncryptedMediaUtils::convertFromInitDataType(initDataType));
    ...
    unsigned initDataLength)
{
    ...
    Event* event;
    if (m_mediaElement->isMediaDataCORSSameOrigin(
            m_mediaElement->getExecutionContext()->getSecurityOrigin())) {
        ...
        event = createEncryptedEvent(WebEncryptedMediaInitDataType::Unknown,
            nullptr, 0);
    }
