...
namespace {
        ...
        ExceptionState& exceptionState)
    {
        ...
        const bool isSenderId = length > 0 && length < kMaxApplicationServerKeyLength && (std::find_if_not(input, input + length, &WTF::isASCIIDigit<unsigned char>) == input + length);
    ...
    ExceptionState& exceptionState)
{
    ...
    if (options.hasApplicationServerKey())
        webOptions.applicationServerKey = bufferSourceToString(options.applicationServerKey(), exceptionState);
    return webOptions;
}
