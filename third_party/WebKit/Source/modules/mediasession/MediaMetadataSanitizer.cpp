...
namespace {
    ...
    bool checkMediaImageSrcSanity(const KURL& src, ExecutionContext* context)
    {
        ...
        if (!src.protocolIs(url::kHttpScheme) && !src.protocolIs(url::kHttpsScheme) && !src.protocolIs(url::kDataScheme) && !src.protocolIs(url::kBlobScheme)) {
            context->addConsoleMessage(ConsoleMessage::create(
                JSMessageSource, WarningMessageLevel,
                "MediaImage src can only be of http/https/data/blob scheme: " + src.getString()));
            return false;
        }
    ...
    ExecutionContext* context)
{
    ...
    for (const MediaImage& image : metadata->artwork()) {
        blink::mojom::blink::MediaImagePtr mojoImage = sanitizeMediaImageAndConvertToMojo(image, context);
