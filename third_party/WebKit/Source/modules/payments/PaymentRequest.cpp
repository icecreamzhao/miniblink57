...
struct TypeConverter<PaymentShippingOptionPtr, blink::PaymentShippingOption> {
    ...
    {
        PaymentShippingOptionPtr output = payments::mojom::blink::PaymentShippingOption::New();
        output->id = input.id();
        output->label = input.label();
        output->amount = PaymentCurrencyAmount::From(input.amount());
...
namespace {
        ...
        ExceptionState& exceptionState)
    {
        ...
        if (androidPay.hasMinGooglePlayServicesVersion()) {
            bool ok = false;
            int minGooglePlayServicesVersion = androidPay.minGooglePlayServicesVersion().toIntStrict(&ok);
            if (ok) {
        ...
        if (androidPay.hasPaymentMethodTokenizationParameters()) {
            const blink::AndroidPayTokenization& tokenization = androidPay.paymentMethodTokenizationParameters();
            output->tokenization_type = payments::mojom::blink::AndroidPayTokenization::UNSPECIFIED;
            ...
            if (tokenization.hasParameters()) {
                const Vector<String>& keys = tokenization.parameters().getPropertyNames(exceptionState);
                if (exceptionState.hadException())
                    return;
                String value;
        ...
        ExceptionState& exceptionState)
    {
        ...
        output->stringified_data = v8StringToWebCoreString<String>(value, DoNotExternalize);
        ...
        if (RuntimeEnabledFeatures::paymentRequestBasicCardEnabled() && supportedMethods.contains("basic-card")) {
        ...
        ExceptionState& exceptionState)
    {
        ...
        for (const PaymentDetailsModifier& modifier : input) {
            ...
            output.back()->method_data = payments::mojom::blink::PaymentMethodData::New();
    ...
    bool allowedToUsePaymentRequest(const Frame* frame)
    {
        ...
        if (!frame->isMainFrame()) {
            if (allowedToUsePaymentRequest(frame->tree().parent())) {
                return (frame->owner() && frame->owner()->allowPaymentRequest()) || frame->tree().parent()->securityContext()->getSecurityOrigin()->isSameSchemeHostPortAndSuborigin(frame->securityContext()->getSecurityOrigin());
            }
        }
...
ScriptPromise PaymentRequest::show(ScriptState* scriptState)
{
    ...
    if (!scriptState->domWindow() || !scriptState->domWindow()->frame())
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(InvalidStateError, "Cannot show the payment request"));
...
{
    if (!m_paymentProvider.is_bound() || m_canMakePaymentResolver || !scriptState->contextIsValid()) {
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(InvalidStateError, "Cannot query payment request"));
    }
    ...
    PaymentComplete result)
{
    if (m_completeResolver)
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(InvalidStateError, "Already called complete() once"));
    ...
    const ScriptValue& detailsScriptValue)
{
    ...
    PaymentDetailsPtr validatedDetails = payments::mojom::blink::PaymentDetails::New();
...
void PaymentRequest::OnPaymentResponse(PaymentResponsePtr response)
{
    ...
    if ((m_options.requestPayerName() && response->payer_name.isEmpty()) || (m_options.requestPayerEmail() && response->payer_email.isEmpty()) || (m_options.requestPayerPhone() && response->payer_phone.isEmpty()) || (!m_options.requestPayerName() && !response->payer_name.isNull()) || (!m_options.requestPayerEmail() && !response->payer_email.isNull()) || (!m_options.requestPayerPhone() && !response->payer_phone.isNull())) {
        m_showResolver->reject(DOMException::create(SyntaxError));
        clearResolversAndCloseMojoConnection();
        return;
    }
