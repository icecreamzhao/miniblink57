    ...
    GLuint length)
{
    ...
    if (scoped.isLost()) {
        DOMException* exception = DOMException::create(InvalidStateError, "context lost");
    ...
    if (!mappedData) {
        DOMException* exception = DOMException::create(InvalidStateError, "Out of memory");
        resolver->reject(exception);
        return promise;
    }
...
void WebGLGetBufferSubDataAsyncCallback::destroy()
{
    ...
    DOMException* exception = DOMException::create(InvalidStateError, "Context lost or destroyed");
