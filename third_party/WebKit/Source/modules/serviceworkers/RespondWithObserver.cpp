...
namespace {
        ...
        const KURL& requestURL)
    {
        ...
        case WebServiceWorkerResponseErrorResponseTypeOpaque:
            errorMessage = errorMessage + "an \"opaque\" response was used for a request whose type "
                                          "is not no-cors";
            break;
        ...
        case WebServiceWorkerResponseErrorBodyUsed:
            errorMessage = errorMessage + "a Response whose \"bodyUsed\" is \"true\" cannot be used "
        ...
        case WebServiceWorkerResponseErrorResponseTypeOpaqueRedirect:
            errorMessage = errorMessage + "an \"opaqueredirect\" type response was used for a "
                                          "request whose redirect mode is not \"manual\".";
            break;
        case WebServiceWorkerResponseErrorBodyLocked:
            errorMessage = errorMessage + "a Response whose \"body\" is locked cannot be used to "
                                          "respond to a request.";
            break;
        case WebServiceWorkerResponseErrorNoForeignFetchResponse:
            errorMessage = errorMessage + "an object that was not a ForeignFetchResponse was passed "
                                          "to respondWith().";
            break;
        ...
        case WebServiceWorkerResponseErrorRedirectedResponseForNotFollowRequest:
            errorMessage = errorMessage + "a redirected response was used for a request whose "
    ...
    {
        String errorMessage = "In Chrome 59, the navigation to \"" + requestURL.getString() + "\" " + "will result in a network error, because FetchEvent.respondWith() was " + "called with a redirected response. See https://crbug.com/658249. The " + "url list of the response was: [\"" + responseURLList[0].getString() + "\"";
        for (size_t i = 1; i < responseURLList.size(); ++i) {
            errorMessage = errorMessage + ", \"" + responseURLList[i].getString() + "\"";
        }
        return errorMessage + "]";
    }
...
void RespondWithObserver::responseWasFulfilled(const ScriptValue& value)
{
    ...
    if (m_redirectMode != WebURLRequest::FetchRedirectModeManual && responseType == FetchResponseData::OpaqueRedirectType) {
        responseWasRejected(
            WebServiceWorkerResponseErrorResponseTypeOpaqueRedirect);
        return;
    }
    if (m_redirectMode != WebURLRequest::FetchRedirectModeFollow && response->redirected()) {
