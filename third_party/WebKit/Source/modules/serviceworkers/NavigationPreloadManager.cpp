...
ScriptPromise NavigationPreloadManager::setHeaderValue(ScriptState* scriptState,
    const String& value)
{
    ServiceWorkerContainerClient* client = ServiceWorkerContainerClient::from(m_registration->getExecutionContext());
    ...
    if (!isValidHTTPHeaderValue(value)) {
        return ScriptPromise::reject(
            scriptState, V8ThrowException::createTypeError(scriptState->isolate(), "The string provided to setHeaderValue ('" + value + "') is not a valid HTTP header field value."));
    }
