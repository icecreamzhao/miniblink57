    ...
    std::unique_ptr<WebDataConsumerHandle> dataConsumeHandle)
{
    ...
    FetchResponseData* responseData = FetchResponseData::createWithBuffer(
        new BodyStreamBuffer(scriptState, new BytesConsumerForDataConsumerHandle(scriptState->getExecutionContext(), std::move(dataConsumeHandle))));
    ...
    FetchResponseData* taintedResponse = responseData->createBasicFilteredResponse();
