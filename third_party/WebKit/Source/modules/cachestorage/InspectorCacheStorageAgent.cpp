...
namespace {
    ...
    public:
        ...
        void onSuccess(const WebVector<WebServiceWorkerRequest>& requests) override
        {
            ...
            RefPtr<ResponsesAccumulator> accumulator = adoptRef(new ResponsesAccumulator(requests.size(), m_params,
                std::move(m_callback)));
    ...
    public:
        ...
        void onSuccess(std::unique_ptr<WebServiceWorkerCache> cache) override
        {
            auto deleteRequest = WTF::makeUnique<DeleteCacheEntry>(std::move(m_callback));
    ...
    std::unique_ptr<RequestEntriesCallback> callback)
{
    String cacheName;
    std::unique_ptr<WebServiceWorkerCacheStorage> cache;
    Response response = assertCacheStorageAndNameForId(cacheId, &cacheName, cache);
    if (!response.isSuccess()) {
        callback->sendFailure(response);
        return;
    }
