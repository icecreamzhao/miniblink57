...
namespace {
    ...
    MATCHER_P2(MemEq,
        p,
        len,
        std::string("pointing to memory") + (negation ? " not" : "") + " equal to \"" + std::string(p, len) + "\" (length=" + PrintToString(len) + ")")
    {
    ...
    MATCHER_P(KURLEq,
        urlString,
        std::string(negation ? "doesn't equal" : "equals") + " to \"" + urlString + "\"")
    {
        ...
        sendBinaryInVectorNonLatin1UTF8Continuation)
    {
        ...
        {
            InSequence s;
            EXPECT_CALL(*handle(), send(false, WebSocketHandle::MessageTypeBinary, MemEq("\xe7\x8b\x90\xe7\x8b\x90\xe7\x8b\x90\xe7"
