...
namespace {
    ...
    public:
        ...
        MockDataConsumerHandle()
            : m_proxy(new MockReaderProxy)
        {
        }
    ...
    private:
        ...
        public:
            explicit Reader(MockReaderProxy* proxy)
                : m_proxy(proxy)
            {
            }
    ...
    TEST_F(BytesConsumerForDataConsumerHandleTest, drainAsBlobDataHandle)
    {
        ...
        std::unique_ptr<MockDataConsumerHandle> handle = WTF::wrapUnique(new MockDataConsumerHandle);
