...
void PresentationConnection::handleMessageQueue()
{
    ...
    while (!m_messages.isEmpty() && !m_blobLoader) {
        ...
        case MessageTypeArrayBuffer:
            client->sendArrayBuffer(m_url, m_id, static_cast<const uint8_t*>(message->arrayBuffer->data()),
...
{
    if (m_state != WebPresentationConnectionState::Connecting && m_state != WebPresentationConnectionState::Connected) {
        return;
    }
    WebPresentationClient* client = presentationClient(getExecutionContext());
    if (client)
