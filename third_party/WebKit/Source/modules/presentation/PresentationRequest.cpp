...
namespace {
    ...
    ScriptPromise rejectWithSandBoxException(ScriptState* scriptState)
    {
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(SecurityError, "The document is sandboxed and lacks "
    ...
    ExceptionState& exceptionState)
{
    ...
    for (size_t i = 0; i < urls.size(); ++i) {
        ...
        if (!parsedUrl.isValid() || !(parsedUrl.protocolIsInHTTPFamily() || parsedUrl.protocolIs("cast"))) {
        ...
        if (MixedContentChecker::isMixedContent(
            ...
            exceptionState.throwSecurityError(
                "Presentation of an insecure document [" + urls[i] + "] is prohibited from a secure context.");
            return nullptr;
        }
...
ScriptPromise PresentationRequest::start(ScriptState* scriptState)
{
    ...
    bool isUserGestureRequired = !contextSettings || contextSettings->getPresentationRequiresUserGesture();
    ...
    const String& id)
{
    ...
    PresentationController* controller = presentationController(getExecutionContext());
    DCHECK(controller);

    PresentationConnection* existingConnection = controller->findExistingConnection(m_urls, id);
    if (existingConnection) {
        client->joinSession(
            m_urls, id, WTF::makeUnique<ExistingPresentationConnectionCallbacks>(resolver, existingConnection));
    } else {
        client->joinSession(
            m_urls, id,
