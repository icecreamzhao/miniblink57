            ...
            if (options.hasAudioBitsPerSecond() || options.hasBitsPerSecond()) {
                if (audioBps > kLargestAutoAllocatedOpusBitRate) {
                    context->addConsoleMessage(ConsoleMessage::create(
                        ...
                        "Clamping calculated audio bitrate (" + String::number(audioBps) + "bps) to the maximum (" + String::number(kLargestAutoAllocatedOpusBitRate) + "bps)"));
                ...
                if (audioBps < kSmallestPossibleOpusBitRate) {
                    context->addConsoleMessage(ConsoleMessage::create(
                        JSMessageSource, WarningMessageLevel,
                        "Clamping calculated audio bitrate (" + String::number(audioBps) + "bps) to the minimum (" + String::number(kSmallestPossibleOpusBitRate) + "bps)"));
        ...
        if (useVideo) {
            ...
            if (options.hasVideoBitsPerSecond() || options.hasBitsPerSecond()) {
                if (videoBps < kSmallestPossibleVpxBitRate) {
                    context->addConsoleMessage(ConsoleMessage::create(
                        JSMessageSource, WarningMessageLevel,
                        "Clamping calculated video bitrate (" + String::number(videoBps) + "bps) to the minimum (" + String::number(kSmallestPossibleVpxBitRate) + "bps)"));
          ...
          &MediaRecorder::dispatchScheduledEvent))
{
    ...
    m_recorderHandler = WTF::wrapUnique(Platform::current()->createMediaRecorderHandler());
    ...
    if (!m_recorderHandler->initialize(
        ...
        exceptionState.throwDOMException(
            NotSupportedError,
            "Failed to initialize native MediaRecorder the type provided (" + m_mimeType + ") is not supported.");
        return;
    }
    m_stopped = false;
}
