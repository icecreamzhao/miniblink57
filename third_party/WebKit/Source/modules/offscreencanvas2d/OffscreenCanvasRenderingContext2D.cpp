...
ImageBitmap* OffscreenCanvasRenderingContext2D::transferToImageBitmap(
    ScriptState* scriptState)
{
    UseCounter::Feature feature = UseCounter::OffscreenCanvasTransferToImageBitmap2D;
    UseCounter::count(scriptState->getExecutionContext(), feature);
    ...
    SnapshotReason reason)
{
    if (!imageBuffer())
        return nullptr;
    sk_sp<SkImage> snapshot = m_imageBuffer->newSkImageSnapshot(PreferNoAcceleration, reason);
    ImageData* imageData = nullptr;
    if (snapshot) {
        imageData = ImageData::create(offscreenCanvas()->size());
        SkImageInfo imageInfo = SkImageInfo::Make(this->width(), this->height(), kRGBA_8888_SkColorType,
