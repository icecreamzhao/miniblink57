    ...
    RemotePlaybackAvailabilityCallback* callback)
{
    ...
    do {
        id = getExecutionContext()->circularSequentialID();
                  ...
                  .add(id, TraceWrapperMember<RemotePlaybackAvailabilityCallback>(this, callback))
                  .isNewEntry);
...
ScriptPromise RemotePlayback::prompt(ScriptState* scriptState)
{
    ...
    if (m_availability == WebRemotePlaybackAvailability::SourceNotSupported || m_availability == WebRemotePlaybackAvailability::SourceNotCompatible) {
        resolver->reject(DOMException::create(
            NotSupportedError,
...
{
    return hasEventListeners() || !m_availabilityCallbacks.isEmpty() || m_promptPromiseResolver;
}
...
void RemotePlayback::stateChanged(WebRemotePlaybackState state)
{
    ...
    if (m_promptPromiseResolver) {
        ...
        // prompt() succeeded.
        if (m_state != WebRemotePlaybackState::Connected && state == WebRemotePlaybackState::Disconnected) {
            m_promptPromiseResolver->reject(DOMException::create(
                AbortError, "Failed to connect to the remote device."));
        } else {
            DCHECK((m_state == WebRemotePlaybackState::Disconnected && state == WebRemotePlaybackState::Connecting) || (m_state == WebRemotePlaybackState::Connected && state == WebRemotePlaybackState::Disconnected));
