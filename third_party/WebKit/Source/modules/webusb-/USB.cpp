    ...
    if (!m_deviceManager) {
        ...
        if (!executionContext->isSecureContext(errorMessage)) {
            ...
            m_deviceManager->GetDevices(
                nullptr, convertToBaseCallback(WTF::bind(&USB::onGetDevices, wrapPersistent(this), wrapPersistent(resolver))));
        }
    }
    return promise;
}
    ...
    const USBDeviceRequestOptions& options)
{
    ...
    if (!executionContext->isSecureContext(errorMessage)) {
        ...
        m_chooserService->GetPermission(
            std::move(filters), convertToBaseCallback(WTF::bind(&USB::onGetPermission, wrapPersistent(this), wrapPersistent(resolver))));
    }
    return promise;
}
