...
namespace {
    ...
    const char kFilterNameTooLong[] = "A 'name' or 'namePrefix' longer than 240 bytes results in no devices "
    ...
    mojom::blink::WebBluetoothDevicePtr device)
{
    ...
    if (result == mojom::blink::WebBluetoothResult::SUCCESS) {
        BluetoothDevice* bluetoothDevice = getBluetoothDeviceRepresentingDevice(std::move(device), resolver);
...
{
    BluetoothRemoteGATTCharacteristic* characteristic = m_activeCharacteristics.get(characteristicInstanceId);
    if (characteristic)
