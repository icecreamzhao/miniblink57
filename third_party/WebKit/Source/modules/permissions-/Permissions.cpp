...
namespace {
    ...
    {
        PermissionDescriptor permission = NativeValueTraits<PermissionDescriptor>::nativeValue(
        ...
        if (name == "push") {
            PushPermissionDescriptor pushPermission = NativeValueTraits<PushPermissionDescriptor>::nativeValue(
                scriptState->isolate(), rawPermission.v8Value(), exceptionState);
            if (exceptionState.hadException()) {
                exceptionState.throwTypeError(exceptionState.message());
                return nullptr;
        ...
        if (name == "midi") {
            MidiPermissionDescriptor midiPermission = NativeValueTraits<MidiPermissionDescriptor>::nativeValue(
                scriptState->isolate(), rawPermission.v8Value(), exceptionState);
    ...
    const Dictionary& rawPermission)
{
    ExceptionState exceptionState(scriptState->isolate(),
        ...
        "query");
    PermissionDescriptorPtr descriptor = parsePermission(scriptState, rawPermission, exceptionState);
    if (exceptionState.hadException())
    ...
    const Dictionary& rawPermission)
{
    ...
    if (!service)
        return ScriptPromise::rejectWithDOMException(
            scriptState, DOMException::create(InvalidStateError, "In its current state, the global "
...
{
    if (!m_service && connectToPermissionService(executionContext, mojo::MakeRequest(&m_service)))
...
{
    if (!resolver->getExecutionContext() || resolver->getExecutionContext()->isContextDestroyed())
        return;
    resolver->resolve(
