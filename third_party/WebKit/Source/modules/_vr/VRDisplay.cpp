    ...
    public:
        ...
        void handleEvent(double highResTimeMs) override
        {
            ...
            m_vrDisplay->serviceScriptedAnimations(
                doc->loader()->timing().pseudoWallTimeToMonotonicTime(highResTimeMs / 1000.0));
        }
    ...
    const HeapVector<VRLayer>& layers)
{
    ...
    if ((m_layer.leftBounds().size() != 0 && m_layer.leftBounds().size() != 4) || (m_layer.rightBounds().size() != 0 && m_layer.rightBounds().size() != 4)) {
        forceExitPresent();
        DOMException* exception = DOMException::create(
            InvalidStateError,
    ...
    if (!m_pendingPresentResolvers.isEmpty()) {
        ...
        m_pendingPresentResolvers.append(resolver);
        m_display->RequestPresent(secureContext, convertToBaseCallback(WTF::bind(&VRDisplay::onPresentComplete, wrapPersistent(this))));
    } else {
        updateLayerBounds();
        resolver->resolve();
...
ScriptPromise VRDisplay::exitPresent(ScriptState* scriptState)
{
    ...
    if (!m_display) {
        DOMException* exception = DOMException::create(InvalidStateError, "VRService is not available.");
        resolver->reject(exception);
        return promise;
    }
...
void VRDisplay::updateLayerBounds()
{
    ...
    device::mojom::blink::VRLayerBoundsPtr leftBounds = device::mojom::blink::VRLayerBounds::New();
    device::mojom::blink::VRLayerBoundsPtr rightBounds = device::mojom::blink::VRLayerBounds::New();
