...
struct TypeConverter<NFCRecordPtr, blink::NFCRecord> {
    static NFCRecordPtr Convert(const blink::NFCRecord& record)
    {
        ...
        auto recordData = mojo::ConvertTo<WTF::Optional<WTF::Vector<uint8_t>>>(record.data());
...
namespace {
    ...
    bool isValidJSONRecord(const NFCRecord& record)
    {
        ...
        if (record.hasMediaType() && !record.mediaType().startsWith(kJsonMimePrefix, TextCaseASCIIInsensitive))
            return false;
    ...
    bool isValidNFCRecord(const NFCRecord& record)
    {
        ...
        if (!record.hasData() && (type != device::nfc::mojom::blink::NFCRecordType::EMPTY)) {
            return false;
        }
        ...
        const device::nfc::mojom::blink::NFCRecordPtr& record)
    {
        ...
        case device::nfc::mojom::blink::NFCRecordType::JSON: {
            ...
            if (record->record_type == device::nfc::mojom::blink::NFCRecordType::JSON) {
    ...
    const NFCPushOptions& options)
{
    ...
    device::nfc::mojom::blink::NFCMessagePtr message = device::nfc::mojom::blink::NFCMessage::From(pushMessage);
    if (!message)
    ...
    if (getNFCMessageSize(message) > device::nfc::mojom::blink::NFCMessage::kMaxSize) {
        return ScriptPromise::rejectWithDOMException(
