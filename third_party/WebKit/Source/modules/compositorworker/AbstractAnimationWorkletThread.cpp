...
void AbstractAnimationWorkletThread::collectAllGarbage()
{
    ...
    WorkletThreadHolder<AbstractAnimationWorkletThread>* workletThreadHolder = WorkletThreadHolder<AbstractAnimationWorkletThread>::getInstance();
    if (!workletThreadHolder)
        return;
    workletThreadHolder->thread()->backingThread().postTask(
        BLINK_FROM_HERE, crossThreadBind(&collectAllGarbageOnThread, crossThreadUnretained(&doneEvent)));
    doneEvent.wait();
}
