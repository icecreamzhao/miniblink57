    ...
    , m_lastUpdateTimestamp(0.0)
{
    ...
    if (!toDocument(executionContext)->domWindow()->frame() || !toDocument(executionContext)->frame()->isMainFrame()) {
...
{
    if (m_state == Sensor::SensorState::Activated || m_state == Sensor::SensorState::Activating)
        stopListening();
}
...
void Sensor::startListening()
{
    ...
    if (!m_configuration) {
        ...
        DCHECK(m_configuration->frequency > 0 && m_configuration->frequency <= m_sensorProxy->maximumFrequency());
    }
    ...
    const String& unsanitizedMessage)
{
    ...
    if (getExecutionContext()) {
        auto error = DOMException::create(code, sanitizedMessage, unsanitizedMessage);
        getExecutionContext()->postTask(
            TaskType::Sensor, BLINK_FROM_HERE,
