    ...
    SensorClientRequest clientRequest)
{
    ...
    m_sharedBuffer = m_sharedBufferHandle->MapAtOffset(kReadBufferSize, params->buffer_offset);
    ...
    auto errorCallback = WTF::bind(&SensorProxy::handleSensorError, wrapWeakPersistent(this),
...
{
    DCHECK(isInitialized());
    const ReadingBuffer* buffer = static_cast<const ReadingBuffer*>(m_sharedBuffer.get());
