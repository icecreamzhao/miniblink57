...
namespace {
    ...
    Vector<v8::Local<v8::Value>> WebScriptExecutor::execute(LocalFrame* frame)
    {
        ...
        if (m_worldID) {
            ...
            v8::Local<v8::Value> scriptValue = frame->script().executeScriptInMainWorldAndReturnValue(
                m_sources.front());
    ...
    Vector<v8::Local<v8::Value>> V8FunctionExecutor::execute(LocalFrame* frame)
    {
            ...
            args.push_back(m_args.Get(i));
        {
            ...
            if (m_gestureToken) {
                gestureIndicator = WTF::wrapUnique(new UserGestureIndicator(m_gestureToken.release()));
            }
...
void SuspendableScriptExecutor::executeAndDestroySelf()
{
    ...
    Vector<v8::Local<v8::Value>> results = m_executor->execute(toDocument(getExecutionContext())->frame());
