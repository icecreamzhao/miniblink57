...
protected:
    void SetUp() override
    {
        const char* const kHTMLString = "<a id='link' href='foo.com' style='font-size:20pt'>Text to select "
                                        "foobar</a>"
...
protected:
    ...
    void SetUp() override
    {
        const char* const kHTMLString = "<div id='empty_div' style='width: 100px; height: 100px;'></div>"
    ...
    void checkMouseClicks(Element& element, bool doubleClickEvent)
    {
        struct ScopedListenersCleaner {
            ScopedListenersCleaner(Element* element)
                : m_element(element)
            {
            }
