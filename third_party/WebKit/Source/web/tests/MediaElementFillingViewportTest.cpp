...
protected:
    ...
    {
        std::unique_ptr<SimRequest> mainResource = WTF::wrapUnique(new SimRequest("https://example.com/", "text/html"));
...
TEST_F(MediaElementFillingViewportTest, MostlyFillingViewport)
{
    ...
    HTMLMediaElement* element = toElement<HTMLMediaElement>(document().getElementById("video"));
