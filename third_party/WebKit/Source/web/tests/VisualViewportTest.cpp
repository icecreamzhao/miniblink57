...
TEST_P(VisualViewportTest, TestResizeAtFullyScrolledPreservesViewportLocation)
{
    ...
    IntPoint expectedLocation = frameView.getScrollableArea()->visibleContentRect().location();
...
TEST_P(VisualViewportTest, TestWebViewResizedBeforeAttachment)
{
    initializeWithDesktopSettings();
    FrameView& frameView = *webViewImpl()->mainFrameImpl()->frameView();
    GraphicsLayer* rootGraphicsLayer = frameView.layoutViewItem().compositor()->rootGraphicsLayer();
...
MATCHER_P2(ContextMenuAtLocation,
    x,
    y,
    std::string(negation ? "is" : "isn't") + " at expected location [" + PrintToString(x) + ", " + PrintToString(y) + "]")
{
...
TEST_P(VisualViewportTest, TestBrowserControlsAdjustmentWithScale)
{
    ...
    frameView.layoutViewportScrollableArea()->scrollBy(ScrollOffset(10000, 10000),
        UserScroll);
    ScrollOffset expected = expectedMaxFrameViewScrollOffset(visualViewport, frameView);
...
TEST_P(VisualViewportTest, TestBrowserControlsAdjustmentAndResize)
{
    ...
    EXPECT_SIZE_EQ(
        ...
        visualViewport.visibleRect().size());
    EXPECT_SIZE_EQ(IntSize(1000, layoutViewportHeight - browserControlsHeight / minPageScale),
    ...
    ScrollOffset frameViewExpected = expectedMaxFrameViewScrollOffset(visualViewport, frameView);
    ...
    EXPECT_SIZE_EQ(
        totalExpected,
        visualViewport.getScrollOffset() + frameView.layoutViewportScrollableArea()->getScrollOffset());
}
...
TEST_P(VisualViewportTest, TestBrowserControlsShrinkAdjustmentAndResize)
{
    ...
    ScrollOffset frameViewExpected(
        0, contentHeight - (layoutViewportHeight - browserControlsHeight / minPageScale));
    ScrollOffset visualViewportExpected = ScrollOffset(
        750, (layoutViewportHeight - browserControlsHeight / minPageScale - visualViewport.visibleRect().height()));
...
{
    bool originalInertTopControls = RuntimeEnabledFeatures::inertTopControlsEnabled();
