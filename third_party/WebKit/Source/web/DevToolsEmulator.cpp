...
{
    if (m_deviceMetricsEnabled && m_emulationParams.viewSize == params.viewSize && m_emulationParams.screenPosition == params.screenPosition && m_emulationParams.deviceScaleFactor == params.deviceScaleFactor && m_emulationParams.offset == params.offset && m_emulationParams.scale == params.scale) {
        return;
    }
    if (m_emulationParams.deviceScaleFactor != params.deviceScaleFactor || !m_deviceMetricsEnabled)
...
bool DevToolsEmulator::handleInputEvent(const WebInputEvent& inputEvent)
{
    ...
    if (isPinch && m_touchEventEmulationEnabled) {
        ...
        if (scaledEvent.type() == WebInputEvent::GesturePinchBegin) {
            ...
            m_lastPinchAnchorDip = WTF::wrapUnique(new IntPoint(flooredIntPoint(gesturePosition)));
            m_lastPinchAnchorDip->scale(pageScaleFactor, pageScaleFactor);
        }
        if (scaledEvent.type() == WebInputEvent::GesturePinchUpdate && m_lastPinchAnchorCss) {
