    ...
    SkFontStyle fontStyle)
{
    ...
    if (!family.length() || !Platform::current()->sandboxSupport()) {
        ...
        const int sizeAndStyle = (((int)textSize) << 2) | (((int)isBold) << 1) | (((int)isItalic) << 0);
    ...
    if (result.useSubpixelPositioning == FontRenderStyle::NoPreference || LayoutTestSupport::isRunningLayoutTest())
