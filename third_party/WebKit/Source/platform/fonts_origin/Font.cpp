...
bool Font::operator==(const Font& other) const
{
    ...
    return first == second && m_fontDescription == other.m_fontDescription && (m_fontFallbackList ? m_fontFallbackList->fontSelectorVersion() : 0) == (other.m_fontFallbackList ? other.m_fontFallbackList->fontSelectorVersion() : 0) && (m_fontFallbackList ? m_fontFallbackList->generation() : 0) == (other.m_fontFallbackList ? other.m_fontFallbackList->generation() : 0);
}
...
namespace {
    ...
    public:
        ...
        std::pair<sk_sp<SkTextBlob>, BlobRotation> next()
        {
            ...
            while (m_index < m_endIndex) {
                ...
                while (m_index < m_endIndex && m_buffer.fontDataAt(m_index) == fontData && !m_buffer.isSkipInkException(m_index))
                    m_index++;
            ...
            m_blobCount++;
            sk_sp<SkTextBlob> skBlob;
            skBlob.reset((SkTextBlob*)m_builder.build());
            return std::make_pair(skBlob, currentRotation);
        }
    ...
    private:
            ...
            const SimpleFontData* fontData)
        {
            ...
            const SkTextBlobBuilder::RunBuffer& buffer = m_hasVerticalOffsets ? m_builder.allocRunPos(paint, count)
            ...
            if (m_rotation == NoRotation) {
                ...
                const float verticalBaselineXOffset = fontData->getFontMetrics().floatAscent() - fontData->getFontMetrics().floatAscent(IdeographicBaseline);
