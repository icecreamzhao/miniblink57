    ...
    SmallCapsIterator::SmallCapsBehavior sourceTextCase)
{
    ...
    if (m_fontSupport == FontSupport::Fallback) {
        ...
        if (m_requestedCaps == FontDescription::FontVariantCaps::PetiteCaps || (m_requestedCaps == FontDescription::FontVariantCaps::Unicase && sourceTextCase == SmallCapsIterator::SmallCapsSameCase))
...
bool OpenTypeCapsSupport::needsRunCaseSplitting()
{
    ...
    return m_fontSupport != FontSupport::Full && m_requestedCaps != FontDescription::TitlingCaps;
}
    ...
    SmallCapsIterator::SmallCapsBehavior runCase)
{
    ...
    if (m_fontSupport == FontSupport::None) {
        if (runCase == SmallCapsIterator::SmallCapsUppercaseNeeded && (m_capsSynthesis == CapsSynthesis::LowerToSmallCaps || m_capsSynthesis == CapsSynthesis::BothToSmallCaps))
            return true;

        if (runCase == SmallCapsIterator::SmallCapsSameCase && (m_capsSynthesis == CapsSynthesis::UpperToSmallCaps || m_capsSynthesis == CapsSynthesis::BothToSmallCaps)) {
            return true;
        }
    }
    ...
    SmallCapsIterator::SmallCapsBehavior runCase)
{
    ...
    case SmallCapsIterator::SmallCapsSameCase:
        caseMapIntend = m_fontSupport == FontSupport::Fallback && (m_capsSynthesis == CapsSynthesis::BothToSmallCaps || m_capsSynthesis == CapsSynthesis::UpperToSmallCaps)
            ? CaseMapIntend::LowerCase
            : CaseMapIntend::KeepSameCase;
        break;
    case SmallCapsIterator::SmallCapsUppercaseNeeded:
        caseMapIntend = m_fontSupport != FontSupport::Fallback && (m_capsSynthesis == CapsSynthesis::LowerToSmallCaps || m_capsSynthesis == CapsSynthesis::BothToSmallCaps)
...
void OpenTypeCapsSupport::determineFontSupport(hb_script_t script)
{
    ...
    case FontDescription::AllSmallCaps:
        if (!(supportsOpenTypeFeature(script, HB_TAG('s', 'm', 'c', 'p')) && supportsOpenTypeFeature(script, HB_TAG('c', '2', 's', 'c')))) {
            m_fontSupport = FontSupport::None;
    ...
    case FontDescription::AllPetiteCaps:
        if (!(supportsOpenTypeFeature(script, HB_TAG('p', 'c', 'a', 'p')) && supportsOpenTypeFeature(script, HB_TAG('c', '2', 'p', 'c')))) {
            if (supportsOpenTypeFeature(script, HB_TAG('s', 'm', 'c', 'p')) && supportsOpenTypeFeature(script, HB_TAG('c', '2', 's', 'c'))) {
                m_fontSupport = FontSupport::Fallback;
            } else {
                m_fontSupport = FontSupport::None;
