...
void WebURLLoaderMockFactoryImpl::serveAsynchronousRequests()
{
    ...
    while (!pending_loaders_.isEmpty()) {
        ...
        while (response.httpStatusCode() >= 300 && response.httpStatusCode() < 400) {
...
void WebURLLoaderMockFactoryImpl::LoadRequest(const WebURLRequest& request,
    ...
    WebData* data)
{
    URLToErrorMap::const_iterator error_iter = url_to_error_info_.find(request.url());
    if (error_iter != url_to_error_info_.end())
        *error = error_iter->value;

    URLToResponseMap::const_iterator iter = url_to_response_info_.find(request.url());
