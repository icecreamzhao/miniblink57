...
{
    if (value) {
        m_paintChunksRasterInvalidationTrackingMap = WTF::wrapUnique(new RasterInvalidationTrackingMap<const PaintChunk>);
    } else {
...
void PaintController::removeLastDisplayItem()
{
    ...
    if (it != m_newDisplayItemIndicesByClient.end()) {
        ...
        if (!indices.isEmpty() && indices.back() == (m_newDisplayItemList.size() - 1))
            indices.pop_back();
    }
#endif
    ...
    if (!m_firstPainted) {
        for (const auto& item : m_newDisplayItemList) {
            if (item.isDrawing() &&
                ...
                // background.
                item.getType() != DisplayItem::kDocumentBackground && item.drawsContent()) {
                m_firstPainted = true;
                break;
            }
        }
...
{
    DCHECK(m_newDisplayItemList.isEmpty());
    DrawingDisplayItem& displayItem = m_currentPaintArtifact.getDisplayItemList()
...
#ifndef NDEBUG
    const SkPicture* newPicture = newItem.isDrawing()
        ? static_cast<const DrawingDisplayItem&>(newItem).picture()
        : nullptr;
    const SkPicture* oldPicture = oldItem && oldItem->isDrawing()
...
void PaintController::checkUnderInvalidation()
{
    ...
    DisplayItem* oldItem = oldItemIndex < m_currentPaintArtifact.getDisplayItemList().size()
    ...
    WTFLogAlways(
        ...
        m_newDisplayItemList
            .subsequenceAsJSON(
                ...
                showPictures ? (DisplayItemList::JsonOptions::ShowPictures | DisplayItemList::JsonOptions::ShowClientDebugName)
