    ...
    bool& success)
{
    ...
    FloatRect lcaVisualRect = localToAncestorVisualRectInternal(rect, sourceState, lcaState, success);
    if (!success)
    ...
    const TransformPaintPropertyNode* destinationTransformNode)
{
    ...
    const TransformPaintPropertyNode* lcaTransform = lowestCommonAncestor(sourceTransformNode, destinationTransformNode);
    DCHECK(lcaTransform);

    FloatRect lcaRect = localToAncestorRect(rect, sourceTransformNode, lcaTransform);
    ...
    bool& success)
{
    ...
    const auto clipRect = localToAncestorClipRectInternal(localState, ancestorState, success);
    ...
    const TransformPaintPropertyNode* localTransformNode)
{
    ...
    const auto& transformMatrix = localToAncestorMatrix(localTransformNode, ancestorTransformNode);
...
{
    bool success = false;
    FloatRect result = localToAncestorClipRectInternal(localState, ancestorState, success);
    DCHECK(success);
    return result;
}
    ...
    bool& success)
{
    ...
    PrecomputedDataForAncestor& precomputedData = getPrecomputedDataForAncestor(ancestorState.transform());
    ...
    bool& success)
{
    ...
    PrecomputedDataForAncestor& precomputedData = getPrecomputedDataForAncestor(ancestorTransformNode);
