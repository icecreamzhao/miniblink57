...
PropertyTreeState::InnermostNode PropertyTreeState::innermostNode() const
{
    ...
    // repeatedly.
    bool clipTransformStrictAncestorOfTransform = m_clip->localTransformSpace() != m_transform.get() && isAncestorOf<TransformPaintPropertyNode>(m_clip->localTransformSpace(), m_transform.get());
    bool effectTransformStrictAncestorOfTransform = m_effect->localTransformSpace() != m_transform.get() && isAncestorOf<TransformPaintPropertyNode>(m_effect->localTransformSpace(), m_transform.get());

    if (!m_transform->isRoot() && clipTransformStrictAncestorOfTransform && effectTransformStrictAncestorOfTransform)
        return Transform;

    bool clipAncestorOfEffect = isAncestorOf<ClipPaintPropertyNode>(m_clip.get(), m_effect->outputClip());
