    ...
    const DisplayItemClient& client)
{
    ...
    if (!RuntimeEnabledFeatures::slimmingPaintV2Enabled() && lastDisplayItem && secondToLastDisplayItem && lastDisplayItem->drawsContent() && secondToLastDisplayItem->getType() == DisplayItem::kBeginCompositing) {
        FloatRect cullRect(
