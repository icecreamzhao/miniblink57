...
void PropertyTreeManager::setupRootClipNode()
{
    ...
    m_propertyTrees.layer_id_to_clip_node_index[clipNode.owning_layer_id] = clipNode.id;
...
void PropertyTreeManager::setupRootEffectNode()
{
    ...
    cc::EffectNode& effectNode = *effectTree.Node(effectTree.Insert(cc::EffectNode(), kInvalidNodeId));
    ...
    m_propertyTrees.layer_id_to_effect_node_index[effectNode.owning_layer_id] = effectNode.id;
...
void PropertyTreeManager::setupRootScrollNode()
{
    ...
    m_propertyTrees.layer_id_to_scroll_node_index[scrollNode.owning_layer_id] = scrollNode.id;
    ...
    const ClipPaintPropertyNode* clipNode)
{
    ...
    m_propertyTrees.layer_id_to_clip_node_index[compositorNode.owning_layer_id] = id;

    // TODO(jbroman): Don't discard rounded corners.
    compositorNode.clip = clipNode->clipRect().rect();
    compositorNode.transform_id = ensureCompositorTransformNode(clipNode->localTransformSpace());
    ...
    const ScrollPaintPropertyNode* scrollNode)
{
    ...
    m_propertyTrees
        .layer_id_to_scroll_node_index[compositorNode.owning_layer_id]
        = id;
    ...
    compositorNode.transform_id = ensureCompositorTransformNode(scrollNode->scrollOffsetTranslation());
...
{
    const EffectPaintPropertyNode* ancestor = GeometryMapper::lowestCommonAncestor(currentEffectNode(), &nextEffect);
    ...
    const EffectPaintPropertyNode* nextEffect)
{
    ...
    if (!nextEffect->filter().isEmpty() || nextEffect->blendMode() != SkBlendMode::kSrcOver)
        effectNode.has_render_surface = true;
