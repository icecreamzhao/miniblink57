...
public:
    ...
    {
        RasterInvalidationTracking* tracking = ccLayersRasterInvalidationTrackingMap().find(m_ccPictureLayer.get());
        if (!tracking)
            return;
        ...
        RasterInvalidationInfo* rasterInvalidationInfo)
    {
        ...
        RasterInvalidationTracking& tracking = ccLayersRasterInvalidationTrackingMap().add(m_ccPictureLayer.get());
...
namespace {
    ...
    enum EndDisplayItemType { EndTransform,
        EndClip,
        EndEffect };
        ...
        GeometryMapper& geometryMapper)
    {
...
#ifdef DCHECK_IS_ON
        const TransformPaintPropertyNode* transformNode = localState.clip()->localTransformSpace();
        if (transformNode != ancestorState.transform()) {
            const TransformationMatrix& localToAncestorMatrix = geometryMapper.localToAncestorMatrix(transformNode,
        ...
        FloatRect combinedClip = geometryMapper.localToAncestorClipRect(localState, ancestorState);
        ...
        GeometryMapper& geometryMapper)
    {
        ...
        for (Vector<PropertyTreeState>::const_reverse_iterator pairedState = pairedStates.rbegin();
            ...
            case PropertyTreeState::Clip: {
                ...
                hasClip = true;
#ifdef DCHECK_IS_ON
                if (pairedState->clip()->localTransformSpace() != pairedState->transform()) {
                    const TransformationMatrix& localTransformMatrix = pairedState->effect()->localTransformSpace()->matrix();
            ...
            case PropertyTreeState::Effect: {
                ...
                FloatRect clipRect = pairedState->effect()->outputClip()->clipRect().rect();
                // TODO(chrishtr): specify origin of the filter.
                FloatPoint filterOrigin;
                if (pairedState->effect()->localTransformSpace() != pairedState->transform()) {
                    const TransformPaintPropertyNode* transformNode = pairedState->effect()->localTransformSpace();
                    const TransformationMatrix& localToAncestorMatrix = geometryMapper.localToAncestorMatrix(transformNode,
    ...
    static void recordPairedEndDisplayItems(
        ...
        cc::DisplayItemList* ccList)
    {
        for (Vector<EndDisplayItemType>::const_reverse_iterator endType = endDisplayItemTypes.rbegin();
    ...
    GeometryMapper& geometryMapper)
{
    ...
    scoped_refptr<cc::DisplayItemList> ccList = cc::DisplayItemList::Create(settings);
    ...
    GeometryMapper& geometryMapper)
{
    ...
    std::unique_ptr<ContentLayerClientImpl> contentLayerClient = clientForPaintChunk(firstPaintChunk, paintArtifact);
    ...
    for (const auto& paintChunk : pendingLayer.paintChunks) {
        RasterInvalidationTracking* rasterTracking = trackingMap ? trackingMap->find(paintChunk) : nullptr;
        DCHECK(!rasterTracking || rasterTracking->trackedRasterInvalidations.size() == paintChunk->rasterInvalidationRects.size());
        ...
        if (storeDebugInfo) {
            contentLayerClient->addPaintChunkDebugData(
                paintArtifact.getDisplayItemList().subsequenceAsJSON(
                    ...
                    DisplayItemList::SkipNonDrawings | DisplayItemList::ShownOnlyDisplayItemTypes));
        }
    ...
    const PendingLayer& candidatePendingLayer)
{
    ...
    for (const PropertyTreeState* currentState = &newChunk.properties.propertyTreeState;
    ...
    GeometryMapper& geometryMapper)
{
    ...
    FloatRect paintChunkScreenVisualRect = geometryMapper.localToAncestorVisualRect(
        paintChunk.bounds, paintChunk.properties.propertyTreeState,
        rootPropertyTreeState);

    FloatRect pendingLayerScreenVisualRect = geometryMapper.localToAncestorVisualRect(
    ...
    GeometryMapper& geometryMapper)
{
    ...
    for (const PaintChunk& paintChunk : paintArtifact.paintChunks()) {
        bool createNew = true;
        for (Vector<PendingLayer>::reverse_iterator candidatePendingLayer = pendingLayers.rbegin();
    ...
    bool storeDebugInfo)
{
    ...
    layerTree->property_trees()->sequence_number = PropertyTreeManager::kPropertyTreeSequenceNumber;
