...
bool AcceleratedStaticBitmapImage::isValid()
{
    ...
    if (sharedContextId != SharedGpuContext::kNoSharedContext && sharedContextId != SharedGpuContext::contextId()) {
...
void AcceleratedStaticBitmapImage::createImageFromMailboxIfNeeded()
{
    ...
    if (m_textureHolder->isSkiaTextureHolder())
        return;
    m_textureHolder = WTF::wrapUnique(new SkiaTextureHolder(std::move(m_textureHolder)));
}
...
void AcceleratedStaticBitmapImage::ensureMailbox()
{
    ...
    m_textureHolder = WTF::wrapUnique(new MailboxTextureHolder(std::move(m_textureHolder)));
}
