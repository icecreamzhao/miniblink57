        ...
        RefPtr<StaticBitmapImage> image)
{
    ...
    SkImageInfo info = SkImageInfo::Make(
        m_width, m_height, kN32_SkColorType,
        image->isPremultiplied() ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
    RefPtr<ArrayBuffer> dstBuffer = ArrayBuffer::createOrNull(m_width * m_height, info.bytesPerPixel());
    ...
    GLenum format = (kN32_SkColorType == kRGBA_8888_SkColorType) ? GL_RGBA : GL_BGRA_EXT;
    ...
    resource.mailbox_holder = gpu::MailboxHolder(mailbox, syncToken, GL_TEXTURE_2D);
...
{
    image->ensureMailbox();
    resource.mailbox_holder = gpu::MailboxHolder(image->mailbox(), image->syncToken(), GL_TEXTURE_2D);
    resource.read_lock_fences_enabled = false;
    resource.is_software = false;
...
namespace {
    ...
    {
        DCHECK(isMainThread());
        OffscreenCanvasPlaceholder* placeholderCanvas = OffscreenCanvasPlaceholder::getPlaceholderById(placeholderCanvasId);
...
)
{
    ...
    if (image->isTextureBacked()) {
        if (Platform::current()->isGPUCompositingEnabled() && !isWebGLSoftwareRendering) {
    ...
    // it is returned.
    image->transfer();
    RefPtr<WebTaskRunner> dispatcherTaskRunner = Platform::current()->currentThread()->getWebTaskRunner();
        ...
        if (isMainThread()) {
            DEFINE_STATIC_LOCAL(
                ...
                ("Blink.Canvas.OffscreenCommit.GPUCanvasSoftwareCompositingMain", 0,
                    10000000, 50));
            commitGPUCanvasSoftwareCompositingMainTimer.count(elapsedTime * 1000000.0);
        } else {
            DEFINE_THREAD_SAFE_STATIC_LOCAL(
                ...
                new CustomCountHistogram("Blink.Canvas.OffscreenCommit."
                    ...
                    0, 10000000, 50));
            commitGPUCanvasSoftwareCompositingWorkerTimer.count(elapsedTime * 1000000.0);
        }
        break;
        ...
        if (isMainThread()) {
            DEFINE_STATIC_LOCAL(
                ...
                ("Blink.Canvas.OffscreenCommit.SoftwareCanvasGPUCompositingMain", 0,
                    10000000, 50));
            commitSoftwareCanvasGPUCompositingMainTimer.count(elapsedTime * 1000000.0);
        } else {
            DEFINE_THREAD_SAFE_STATIC_LOCAL(
                ...
                new CustomCountHistogram("Blink.Canvas.OffscreenCommit."
                    ...
                    0, 10000000, 50));
            commitSoftwareCanvasGPUCompositingWorkerTimer.count(elapsedTime * 1000000.0);
        }
        break;
            ...
            DEFINE_STATIC_LOCAL(CustomCountHistogram,
                ...
                ("Blink.Canvas.OffscreenCommit."
                 "SoftwareCanvasSoftwareCompositingMain",
                    0, 10000000, 50));
            commitSoftwareCanvasSoftwareCompositingMainTimer.count(elapsedTime * 1000000.0);
        } else {
            DEFINE_THREAD_SAFE_STATIC_LOCAL(
                ...
                new CustomCountHistogram("Blink.Canvas.OffscreenCommit."
                    ...
                    0, 10000000, 50));
            commitSoftwareCanvasSoftwareCompositingWorkerTimer.count(elapsedTime * 1000000.0);
        }
        break;
...
{
    m_syntheticBeginFrameTask = Platform::current()
                                    ->currentThread()
                                    ->getWebTaskRunner()
    ...
    const cc::BeginFrameArgs& beginFrameArgs)
{
    if (!client())
        return;
    unsigned framesInFlight = m_cachedImages.size() + m_sharedBitmaps.size() + m_cachedTextureIds.size();
