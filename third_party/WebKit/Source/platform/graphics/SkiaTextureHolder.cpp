    ...
    std::unique_ptr<TextureHolder> textureHolder)
{
    ...
    GLuint sharedContextTextureId = sharedGL->CreateAndConsumeTextureCHROMIUM(GL_TEXTURE_2D, mailbox.name);
...
void SkiaTextureHolder::releaseImageThreadSafe()
{
    ...
    if (textureThreadTaskRunner() && m_image && wasTransferred() && SharedGpuContext::isValid()) {
        gpu::gles2::GLES2Interface* sharedGL = SharedGpuContext::gl();
