...
{
    m_refFactory = new OffscreenCanvasSurfaceReferenceFactory(m_weakFactory.GetWeakPtr());
    ...
    const cc::SurfaceInfo& surfaceInfo)
{
    if (!m_currentSurfaceId.is_valid() && surfaceInfo.id().is_valid()) {
        ...
        scoped_refptr<cc::SurfaceLayer> surfaceLayer = cc::SurfaceLayer::Create(m_refFactory);
        ...
        // SurfaceLayer
        m_currentSurfaceId = surfaceInfo.id();
        cc::SurfaceLayer* surfaceLayer = static_cast<cc::SurfaceLayer*>(m_CCLayer.get());
        surfaceLayer->SetSurfaceInfo(surfaceInfo);
    }
