...
public:
    Buffer(size_t bufferSize, const char* typeName)
    {
        ...
        m_begin = m_end = static_cast<char*>(WTF::Partitions::bufferMalloc(m_capacity, typeName));
...
{
    return sizeof(*this) + capacityInBytes() + m_elements.capacity() * sizeof(m_elements[0]);
}
    ...
    const char* typeName)
{
    ...
    std::unique_ptr<Buffer> newBuffer = WTF::makeUnique<Buffer>(bufferSize, typeName);
