...
#include "platform/text/SimpleTextBreakIterator.h"
...
// #include <unicode/rbbi.h>
// #include <unicode/ubrk.h>
#include "SimpleTextBreakIterator.h"
>>>>>>> miniblink49
...
{                                                                                                    \
    static iteratorType* breakIter = 0;                                                              \
    if (!breakIter)                                                                                  \
        breakIter = new iteratorType();                                                              \
    breakIter->reset(string, length);                                                                \
    return breakIter;                                                                                \
}                                                                                                    \
                                                                                                     \
...
{                                                                                                    \
    ...
    return nullptr;                                                                                  \
                                                                                                     ...
                                                                                                     \
TextBreakIterator* breakType##BreakIterator(const String& string, int start, int length)             \
{                                                                                                    \
    if (string.isNull()|| string.isEmpty() || start < 0 || length < 0)                               \
        return 0;                                                                                    \
    if (string.length() <= (unsigned)(start + length))                                               \
        return 0;                                                                                    \
    if (string.is8Bit())                                                                             \
        return breakType##BreakIterator(string.characters8() + start, length);                       \
    return breakType##BreakIterator(string.characters16() + start, length);                          \
}

DEFINE_BREAK_ITERATOR(word, WordBreakIterator)
DEFINE_BREAK_ITERATOR(char, CharBreakIterator)
DEFINE_BREAK_ITERATOR(line, LineBreakIterator)
DEFINE_BREAK_ITERATOR(sentence, SentenceBreakIterator)
...
TextBreakIterator* acquireLineBreakIterator(const LChar* string, int length, const AtomicString& locale, const UChar* priorContext, unsigned priorContextLength)
{
    ...
    return iterator;
#endif // MINIBLINK_NOT_IMPLEMENTED
    return lineBreakIterator(string, length);
}
...
<<<<<<< HEAD
    ...
    static const char* const kRules = "$CR      = [\\p{Grapheme_Cluster_Break = CR}];"
