...
{
    if (text.length() < minimumPrefixLength + minimumSuffixLength || beforeIndex <= minimumPrefixLength)
        return 0;

    std::vector<uint8_t> result = hyphenate(text);
    beforeIndex = std::min<size_t>(beforeIndex, text.length() - minimumSuffixLength);
