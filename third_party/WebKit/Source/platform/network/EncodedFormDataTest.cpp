...
namespace {
    ...
    TEST_F(EncodedFormDataTest, DeepCopy)
    {
        ...
        EXPECT_EQ(KURL(KURL(), String("ws://localhost/")),
            copyElements[3].m_fileSystemURL);
        ...
        for (size_t i = 0; i < 4; ++i) {
            if (copyElements[i].m_filename.impl()) {
                EXPECT_NE(originalElements[i].m_filename.impl(),
                    copyElements[i].m_filename.impl());
                EXPECT_TRUE(copyElements[i].m_filename.impl()->hasOneRef());
            }

            if (copyElements[i].m_blobUUID.impl()) {
                EXPECT_NE(originalElements[i].m_blobUUID.impl(),
                    copyElements[i].m_blobUUID.impl());
                EXPECT_TRUE(copyElements[i].m_blobUUID.impl()->hasOneRef());
            }

            if (copyElements[i].m_fileSystemURL.getString().impl()) {
                EXPECT_NE(originalElements[i].m_fileSystemURL.getString().impl(),
                    copyElements[i].m_fileSystemURL.getString().impl());
                EXPECT_TRUE(
                    copyElements[i].m_fileSystemURL.getString().impl()->hasOneRef());
            }
