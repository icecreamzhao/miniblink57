...
namespace scheduler {
    ...
    namespace {
            ...
            base::TimeTicks deadline)
        {
            if ((*run_count + 1) < max_idle_task_reposts) {
                idle_task_runner->PostIdleTask(
                    FROM_HERE, base::Bind(&RepostingIdleTestTask, base::Unretained(idle_task_runner), run_count));
            }
            (*run_count)++;
        }
            ...
            base::TimeTicks deadline)
        {
            if ((*run_count + 1) < max_idle_task_reposts) {
                idle_task_runner->PostIdleTask(
                    FROM_HERE, base::Bind(&RepostingUpdateClockIdleTestTask, base::Unretained(idle_task_runner), run_count, clock, advance_time, deadlines));
            }
    ...
    TEST_F(RendererSchedulerImplTest, TestRentrantTask)
    {
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(AppendToVectorReentrantTask, base::RetainedRef(default_task_runner_), &run_order, &count, 5));
        RunUntilIdle();
    ...
    {
        int run_count = 0;
        base::TimeTicks expected_deadline = clock_->NowTicks() + base::TimeDelta::FromMilliseconds(2300);
    ...
    TEST_F(RendererSchedulerImplTest, TestRepostingIdleTask)
    {
        ...
        idle_task_runner_->PostIdleTask(
            FROM_HERE, base::Bind(&RepostingIdleTestTask, base::RetainedRef(idle_task_runner_), &run_count));
        EnableIdleTasks();
        RunUntilIdle();
        EXPECT_EQ(1, run_count);
        ...
        TestCompositorPolicy_CompositorHandlesInput_LongGestureDuration)
    {
        ...
        base::TimeTicks loop_end_time = clock_->NowTicks() + base::TimeDelta::FromMilliseconds(UserModel::kMedianGestureDurationMillis * 2);
    ...
    TEST_F(RendererSchedulerImplTest, TestIsHighPriorityWorkAnticipated)
    {
        ...
        scheduler_->SetHasVisibleRenderWidgetWithTouchHandler(true);
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&AnticipationTestTask, scheduler_.get(), SimulateInputType::None, &is_anticipated_before, &is_anticipated_after));
        RunUntilIdle();
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&AnticipationTestTask, scheduler_.get(), SimulateInputType::TouchStart, &is_anticipated_before, &is_anticipated_after));
        bool dummy;
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&AnticipationTestTask, scheduler_.get(), SimulateInputType::TouchEnd, &dummy, &dummy));
        default_task_runner_->PostTask(
            FROM_HERE,
    ...
    TEST_F(RendererSchedulerImplTest, TestShouldYield)
    {
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&PostingYieldingTestTask, scheduler_.get(), base::RetainedRef(default_task_runner_), false, &should_yield_before, &should_yield_after));
        RunUntilIdle();
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&PostingYieldingTestTask, scheduler_.get(), base::RetainedRef(compositor_task_runner_), false, &should_yield_before, &should_yield_after));
        RunUntilIdle();
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&PostingYieldingTestTask, scheduler_.get(), base::RetainedRef(compositor_task_runner_), true, &should_yield_before, &should_yield_after));
        RunUntilIdle();
        ...
        NonNestableIdleTaskDoesntExecuteInNestedLoop)
    {
        ...
        EXPECT_THAT(order, testing::ElementsAre(std::string("1"), std::string("2"), std::string("4"), std::string("5"), std::string("3")));
    }
    ...
    TEST_F(RendererSchedulerImplTest, TestLongIdlePeriodRepeating)
    {
        ...
        EXPECT_THAT(
            ...
            testing::ElementsAre(
                ...
                clock_before + (2 * idle_task_runtime) + maximum_idle_period_duration()));
    ...
    TEST_F(RendererSchedulerImplTest, TestRendererHiddenIdlePeriod)
    {
        ...
        idle_task_runner_->PostIdleTask(
            FROM_HERE, base::Bind(&RepostingIdleTestTask, base::RetainedRef(idle_task_runner_), &run_count));
        clock_->Advance(end_idle_when_hidden_delay() + base::TimeDelta::FromMilliseconds(10));
        RunUntilIdle();
        EXPECT_EQ(2, run_count);
    }
    ...
    TEST_F(RendererSchedulerImplTest, TestRendererBackgroundedTimerSuspension)
    {
        ...
        now = base::TimeTicks() + suspend_timers_when_backgrounded_delay() + base::TimeDelta::FromMilliseconds(10);
        run_order.clear();
        clock_->SetNowTicks(now);
        RunUntilIdle();
    ...
    TEST_F(RendererSchedulerImplTest, ModeratelyExpensiveTimer_NotBlocked)
    {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            compositor_task_runner_->PostTask(
                FROM_HERE, base::Bind(&RendererSchedulerImplTest::SimulateMainThreadInputHandlingCompositorTask, base::Unretained(this), base::TimeDelta::FromMilliseconds(8)));
            timer_task_runner_->PostTask(
                FROM_HERE, base::Bind(&RendererSchedulerImplTest::SimulateTimerTask, base::Unretained(this), base::TimeDelta::FromMilliseconds(4)));
            ...
            base::TimeDelta time_till_next_frame = EstimatedNextFrameBegin() - clock_->NowTicks();
        ...
        FourtyMsTimer_NotBlocked_CompositorScrolling)
    {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            timer_task_runner_->PostTask(
                FROM_HERE, base::Bind(&RendererSchedulerImplTest::SimulateTimerTask, base::Unretained(this), base::TimeDelta::FromMilliseconds(40)));
        ...
        ExpensiveTimer_NotBlocked_UseCase_MAIN_THREAD_CUSTOM_INPUT_HANDLING)
    {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            compositor_task_runner_->PostTask(
                FROM_HERE, base::Bind(&RendererSchedulerImplTest::SimulateMainThreadInputHandlingCompositorTask, base::Unretained(this), base::TimeDelta::FromMilliseconds(8)));
            timer_task_runner_->PostTask(
                FROM_HERE, base::Bind(&RendererSchedulerImplTest::SimulateTimerTask, base::Unretained(this), base::TimeDelta::FromMilliseconds(10)));
    ...
    namespace {
            ...
            scoped_refptr<base::SingleThreadTaskRunner> timer_queue)
        {
            ...
            if (++(*count) < 500) {
                timer_queue->PostTask(FROM_HERE, base::Bind(SlowCountingTask, count, clock, task_duration, timer_queue));
            }
        }
    }
        ...
        SYNCHRONIZED_GESTURE_TimerTaskThrottling_task_expensive)
    {
        ...
        base::TimeTicks first_throttled_run_time = TaskQueueThrottler::AlignedThrottledRunTime(clock_->NowTicks());
        ...
        scheduler_->TimerTaskRunner()->PostTask(
            FROM_HERE, base::Bind(SlowCountingTask, &count, clock_.get(), 7, scheduler_->TimerTaskRunner()));
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            bool throttled_task_expected_to_have_run = (clock_->NowTicks() > first_throttled_run_time);
        ...
        SYNCHRONIZED_GESTURE_TimerTaskThrottling_TimersSuspended)
    {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            bool expect_queue_enabled = (i == 0) || (clock_->NowTicks() > first_throttled_run_time);
            if (suspended)
        ...
        SYNCHRONIZED_GESTURE_TimerTaskThrottling_task_not_expensive)
    {
        ...
        scheduler_->TimerTaskRunner()->PostTask(
            FROM_HERE, base::Bind(SlowCountingTask, &count, clock_.get(), 6, scheduler_->TimerTaskRunner()));
    ...
    TEST_F(RendererSchedulerImplTest, UnthrottledTaskRunner)
    {
        ...
        // task runner.
        SimulateCompositorGestureStart(TouchEventPolicy::SEND_TOUCH_START);
        scoped_refptr<TaskQueue> unthrottled_task_runner = scheduler_->NewUnthrottledTaskRunner(TaskQueue::QueueType::UNTHROTTLED);

        size_t timer_count = 0;
        size_t unthrottled_count = 0;
        scheduler_->TimerTaskRunner()->PostTask(
            FROM_HERE, base::Bind(SlowCountingTask, &timer_count, clock_.get(), 7, scheduler_->TimerTaskRunner()));
        unthrottled_task_runner->PostTask(
            FROM_HERE, base::Bind(SlowCountingTask, &unthrottled_count, clock_.get(), 7, unthrottled_task_runner));
        scheduler_->SuspendTimerQueue();
    ...
    TEST_F(RendererSchedulerImplTest, EnableVirtualTime)
    {
        ...
        scoped_refptr<TaskQueue> loading_tq = scheduler_->NewLoadingTaskRunner(TaskQueue::QueueType::TEST);
        scoped_refptr<TaskQueue> timer_tq = scheduler_->NewTimerTaskRunner(TaskQueue::QueueType::TEST);
        scoped_refptr<TaskQueue> unthrottled_tq = scheduler_->NewUnthrottledTaskRunner(TaskQueue::QueueType::TEST);
    ...
    TEST_F(RendererSchedulerImplTest, Tracing)
    {
        ...
        std::unique_ptr<WebFrameSchedulerImpl> web_frame_scheduler = web_view_scheduler1->createWebFrameSchedulerImpl(nullptr);
        ...
        TaskQueueThrottler::TimeBudgetPool* time_budget_pool = scheduler_->task_queue_throttler()->CreateTimeBudgetPool(
        ...
        std::unique_ptr<base::trace_event::ConvertableToTraceFormat> value = scheduler_->AsValue(base::TimeTicks());
        EXPECT_TRUE(value);
    }
        ...
        DefaultTimerTasksAreThrottledWhenBackgrounded)
    {
        ...
        mock_task_runner_->RunUntilTime(base::TimeTicks() + base::TimeDelta::FromMilliseconds(1100));

        EXPECT_THAT(run_times, testing::ElementsAre(base::TimeTicks() + base::TimeDelta::FromSeconds(1)));
        run_times.clear();
        ...
        mock_task_runner_->RunUntilTime(base::TimeTicks() + base::TimeDelta::FromMilliseconds(1500));

        EXPECT_THAT(run_times,
            testing::ElementsAre(base::TimeTicks() + base::TimeDelta::FromMilliseconds(1300)));
    }
