...
namespace scheduler {
    ...
    namespace {
            ...
            WebViewSchedulerImpl::WebViewSchedulerSettings* settings)
        {
            ...
            double initial_budget = settings->expensiveBackgroundThrottlingInitialBudget();
    ...
    WebViewSchedulerImpl::WebViewSchedulerImpl(
        ...
        , settings_(settings)
    {
        if (kMinimalBackgroundThrottlingDurationToReport.is_zero()) {
            kMinimalBackgroundThrottlingDurationToReport = base::TimeDelta::FromSeconds(3);
            kDefaultMaxBackgroundBudgetLevel = base::TimeDelta::FromSeconds(3);
            kDefaultMaxBackgroundThrottlingDelay = base::TimeDelta::FromMinutes(1);
            kDefaultInitialBackgroundBudget = base::TimeDelta::FromSeconds(1);
            kBackgroundThrottlingGracePeriod = base::TimeDelta::FromSeconds(10);
        }
    ...
    void WebViewSchedulerImpl::ApplyVirtualTimePolicy()
    {
        ...
        // first load arrives.
        setAllowVirtualTimeToAdvance(pending_loads_.size() == 0 && background_parser_count_ == 0 && have_seen_loading_task_);
    }
    ...
    void WebViewSchedulerImpl::MaybeInitializeBackgroundTimeBudgetPool()
    {
        ...
        background_time_budget_pool_ = renderer_scheduler_->task_queue_throttler()->CreateTimeBudgetPool(
