...
namespace scheduler {
    namespace {
        ...
        {
            return base::TimeTicks() + base::TimeDelta::FromSecondsD(monotonicTimeInSeconds);
        }
        ...
        , weak_factory_(this)
    {
        ...
        default_loading_task_runner_ = NewLoadingTaskRunner(TaskQueue::QueueType::DEFAULT_LOADING);
        default_timer_task_runner_ = NewTimerTaskRunner(TaskQueue::QueueType::DEFAULT_TIMER);
        ...
        TaskQueue::QueueType queue_type)
    {
        ...
        if (MainThreadOnly().current_policy.loading_queue_policy.time_domain_type == TimeDomainType::THROTTLED) {
        ...
        TaskQueue::QueueType queue_type)
    {
        ...
        if (MainThreadOnly().current_policy.timer_queue_policy.time_domain_type == TimeDomainType::THROTTLED) {
        ...
        const scoped_refptr<TaskQueue>& task_queue)
    {
        ...
        } else if (timer_task_runners_.find(task_queue) != timer_task_runners_.end()) {
            task_queue->RemoveTaskObserver(&MainThreadOnly().timer_task_cost_estimator);
            timer_task_runners_.erase(task_queue);
        } else if (unthrottled_task_runners_.find(task_queue) != unthrottled_task_runners_.end()) {
    ...
    void RendererSchedulerImpl::SetAllRenderWidgetsHidden(bool hidden)
    {
        ...
        if (hidden) {
            ...
            // hidden.
            base::TimeDelta end_idle_when_hidden_delay = base::TimeDelta::FromMilliseconds(kEndIdleWhenHiddenDelayMillis);
    ...
    {
        helper_.CheckOnValidThread();
        if (has_visible_render_widget_with_touch_handler == MainThreadOnly().has_visible_render_widget_with_touch_handler)
            return;

        MainThreadOnly().has_visible_render_widget_with_touch_handler = has_visible_render_widget_with_touch_handler;
    ...
    void RendererSchedulerImpl::OnRendererBackgrounded()
    {
        ...
        base::TimeDelta suspend_timers_when_backgrounded_delay = base::TimeDelta::FromMilliseconds(
    ...
    void RendererSchedulerImpl::OnAudioStateChanged()
    {
        ...
        MainThreadOnly().last_audio_state_change = helper_.scheduler_tqm_delegate()->NowTicks();
        ...
        const blink::WebInputEvent& web_input_event)
    {
        ...
        if ((web_input_event.type() == blink::WebInputEvent::MouseDown || web_input_event.type() == blink::WebInputEvent::MouseMove) && (web_input_event.modifiers() & blink::WebInputEvent::LeftButtonDown)) {
            return true;
        }
        ...
        if (blink::WebInputEvent::isMouseEventType(web_input_event.type()) || blink::WebInputEvent::isKeyboardEventType(web_input_event.type())) {
            return false;
        }
        return true;
    }
    ...
    void RendererSchedulerImpl::DidAnimateForInputOnCompositorThread()
    {
        ...
        base::AutoLock lock(any_thread_lock_);
        AnyThread().fling_compositor_escalation_deadline = helper_.scheduler_tqm_delegate()->NowTicks() + base::TimeDelta::FromMilliseconds(kFlingEscalationLimitMillis);
    }
        ...
        InputEventState input_event_state)
    {
        ...
        UseCase previous_use_case = ComputeCurrentUseCase(now, &unused_policy_duration);
        bool was_awaiting_touch_start_response = AnyThread().awaiting_touch_start_response;
        ...
        case blink::WebInputEvent::TouchMove:
            ...
            // state.
            if (AnyThread().awaiting_touch_start_response && CompositorThreadOnly().last_input_type == blink::WebInputEvent::TouchMove) {
        ...
        case blink::WebInputEvent::GestureScrollUpdate:
            ...
            AnyThread().last_gesture_was_compositor_driven = input_event_state == InputEventState::EVENT_CONSUMED_BY_COMPOSITOR;
            AnyThread().awaiting_touch_start_response = false;
        ...
        case blink::WebInputEvent::MouseWheel:
            ...
            AnyThread().default_gesture_prevented = !AnyThread().last_gesture_was_compositor_driven;
            break;
        ...
        if (use_case != previous_use_case || was_awaiting_touch_start_response != AnyThread().awaiting_touch_start_response) {
        ...
        WebInputEventResult result)
    {
        ...
        if (ShouldPrioritizeInputEvent(web_input_event)) {
            ...
            // detected.
            if (AnyThread().awaiting_touch_start_response && result == WebInputEventResult::HandledApplication) {
                AnyThread().awaiting_touch_start_response = false;
                AnyThread().default_gesture_prevented = true;
    ...
    bool RendererSchedulerImpl::IsHighPriorityWorkAnticipated()
    {
        ...
        return MainThreadOnly().touchstart_expected_soon || use_case == UseCase::TOUCHSTART || use_case == UseCase::MAIN_THREAD_GESTURE || use_case == UseCase::MAIN_THREAD_CUSTOM_INPUT_HANDLING || use_case == UseCase::SYNCHRONIZED_GESTURE;
    }
    ...
    bool RendererSchedulerImpl::ShouldYieldForHighPriorityWork()
    {
        ...
        case UseCase::SYNCHRONIZED_GESTURE:
            return compositor_task_runner_->HasPendingImmediateWork() || MainThreadOnly().touchstart_expected_soon;
        ...
        const base::Closure& callback)
    {
        ...
        IdleTaskRunner()->PostIdleTask(
            FROM_HERE, base::Bind(&RendererSchedulerImpl::EndIdlePeriodForTesting, weak_factory_.GetWeakPtr(), callback));
        idle_helper_.EnableLongIdlePeriod();
    }
    ...
    void RendererSchedulerImpl::UpdatePolicyLocked(UpdateType update_type)
    {
        ...
        base::TimeDelta longest_jank_free_task_duration = EstimateLongestJankFreeTaskDuration();
        MainThreadOnly().longest_jank_free_task_duration = longest_jank_free_task_duration;

        bool loading_tasks_seem_expensive = false;
        bool timer_tasks_seem_expensive = false;
        loading_tasks_seem_expensive = MainThreadOnly().loading_task_cost_estimator.expected_task_duration() > longest_jank_free_task_duration;
        timer_tasks_seem_expensive = MainThreadOnly().timer_task_cost_estimator.expected_task_duration() > longest_jank_free_task_duration;
        ...
        if (new_policy_duration.is_zero() || (touchstart_expected_flag_valid_for_duration > base::TimeDelta() && new_policy_duration > touchstart_expected_flag_valid_for_duration)) {
        ...
        // work.
        if (MainThreadOnly().last_audio_state_change && !MainThreadOnly().is_audio_playing) {
            base::TimeTicks audio_will_expire = MainThreadOnly().last_audio_state_change.value() + kThrottlingDelayAfterAudioIsPlayed;
            ...
            if (audio_will_expire_after > base::TimeDelta()) {
                if (new_policy_duration.is_zero()) {
                    ...
                    new_policy_duration = std::min(new_policy_duration, audio_will_expire_after);
                }
            }
        }
        ...
        bool main_thread_compositing_is_fast = MainThreadOnly().idle_time_estimator.GetExpectedIdleDuration(
                                                   MainThreadOnly().compositor_frame_interval)
            > MainThreadOnly().compositor_frame_interval * kFastCompositingIdleTimeThreshold;
        ...
        case UseCase::COMPOSITOR_GESTURE:
            if (touchstart_expected_soon) {
                ...
                new_policy.compositor_queue_policy.priority = TaskQueue::BEST_EFFORT_PRIORITY;
            }
            break;

        case UseCase::SYNCHRONIZED_GESTURE:
            new_policy.compositor_queue_policy.priority = main_thread_compositing_is_fast ? TaskQueue::HIGH_PRIORITY
        ...
        case UseCase::NONE:
            ...
            // driven gesture.
            if (touchstart_expected_soon && AnyThread().last_gesture_was_compositor_driven) {
                new_policy.rail_mode = v8::PERFORMANCE_RESPONSE;
                expensive_task_policy = ExpensiveTaskPolicy::BLOCK;
            }
            break;
        ...
        if (expensive_task_policy == ExpensiveTaskPolicy::BLOCK && (!MainThreadOnly().have_seen_a_begin_main_frame || MainThreadOnly().navigation_task_expected_count > 0)) {
            expensive_task_policy = ExpensiveTaskPolicy::RUN;
        }
        ...
        case ExpensiveTaskPolicy::THROTTLE:
            if (loading_tasks_seem_expensive) {
                new_policy.loading_queue_policy.time_domain_type = TimeDomainType::THROTTLED;
            }
            if (timer_tasks_seem_expensive) {
        ...
        if (MainThreadOnly().timer_queue_suspend_count != 0 || MainThreadOnly().timer_queue_suspended_when_backgrounded) {
            new_policy.timer_queue_policy.is_enabled = false;
        ...
        if (MainThreadOnly().use_virtual_time) {
            new_policy.compositor_queue_policy.time_domain_type = TimeDomainType::VIRTUAL;
        ...
        new_policy.should_disable_throttling = ShouldDisableThrottlingBecauseOfAudio(now) || MainThreadOnly().use_virtual_time;
        ...
        MainThreadOnly().have_reported_blocking_intervention_in_current_policy = false;
        ...
        if (MainThreadOnly().rail_mode_observer && new_policy.rail_mode != MainThreadOnly().current_policy.rail_mode) {
            MainThreadOnly().rail_mode_observer->OnRAILModeChanged(
                new_policy.rail_mode);
        }

        if (new_policy.should_disable_throttling != MainThreadOnly().current_policy.should_disable_throttling) {
    ...
    {
        if (task_queue_enabled_voter && old_task_queue_policy.is_enabled != new_task_queue_policy.is_enabled) {
        ...
        if (old_task_queue_policy.time_domain_type != new_task_queue_policy.time_domain_type) {
            ...
            } else if (new_task_queue_policy.time_domain_type == TimeDomainType::THROTTLED) {
                task_queue_throttler_->IncreaseThrottleRefCount(task_queue);
            } else if (new_task_queue_policy.time_domain_type == TimeDomainType::VIRTUAL) {
        ...
        base::TimeDelta* expected_use_case_duration) const
    {
        ...
        if (AnyThread().fling_compositor_escalation_deadline > now && !AnyThread().awaiting_touch_start_response) {
            *expected_use_case_duration = AnyThread().fling_compositor_escalation_deadline - now;
            return UseCase::COMPOSITOR_GESTURE;
        }
        // Above all else we want to be responsive to user input.
        *expected_use_case_duration = AnyThread().user_model.TimeLeftInUserGesture(now);
        ...
        // content on the page.
        if (AnyThread().waiting_for_meaningful_paint && !AnyThread().have_seen_input_since_navigation) {
        ...
        base::TimeDelta* next_long_idle_period_delay_out)
    {
        ...
        if (MainThreadOnly().current_use_case == UseCase::TOUCHSTART) {
            ...
            *next_long_idle_period_delay_out = std::max(base::TimeDelta(),
    ...
    {
        base::AutoLock lock(any_thread_lock_);
        AnyThread().last_idle_period_end_time = helper_.scheduler_tqm_delegate()->NowTicks();
        ...
        WebScheduler::NavigatingFrameType type)
    {
        ...
        if (type == blink::WebScheduler::NavigatingFrameType::kMainFrame && MainThreadOnly().navigation_task_expected_count > 0) {
            MainThreadOnly().navigation_task_expected_count--;
            UpdatePolicy();
        }
    }
    ...
    {
        DCHECK(!MainThreadOnly().renderer_backgrounded || (MainThreadOnly().renderer_backgrounded && !MainThreadOnly().renderer_suspended));
        if (!MainThreadOnly().timer_queue_suspended_when_backgrounded)
            return;
    ...
    bool RendererSchedulerImpl::MainThreadSeemsUnresponsive()
    {
        ...
        estimated_queueing_time = queueing_time_estimator.EstimateQueueingTimeIncludingCurrentTask(now);

        bool main_thread_seems_unresponsive = estimated_queueing_time > main_thread_responsiveness_threshold_;
    ...
    void RendererSchedulerImpl::RemoveWebViewScheduler(
        WebViewSchedulerImpl* web_view_scheduler)
    {
        DCHECK(MainThreadOnly().web_view_schedulers.find(web_view_scheduler) != MainThreadOnly().web_view_schedulers.end());
    ...
    {
        if (MainThreadOnly().current_use_case == UseCase::TOUCHSTART || MainThreadOnly().longest_jank_free_task_duration < base::TimeDelta::FromMilliseconds(kRailsResponseTimeMillis) || MainThreadOnly().timer_queue_suspend_count || MainThreadOnly().timer_queue_suspended_when_backgrounded) {
            return;
        }
        if (!MainThreadOnly().timer_tasks_seem_expensive && !MainThreadOnly().loading_tasks_seem_expensive) {
            return;
        }
        if (!MainThreadOnly().have_reported_blocking_intervention_in_current_policy) {
            MainThreadOnly().have_reported_blocking_intervention_in_current_policy = true;
        ...
        if (!MainThreadOnly().have_reported_blocking_intervention_since_navigation) {
            ...
            MainThreadOnly().have_reported_blocking_intervention_since_navigation = true;
        ...
        base::TimeTicks now)
    {
        ...
        return MainThreadOnly().last_audio_state_change.value() + kThrottlingDelayAfterAudioIsPlayed > now;
    }
