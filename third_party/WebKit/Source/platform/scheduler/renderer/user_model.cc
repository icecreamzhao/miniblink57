...
namespace scheduler {
        ...
        const base::TimeTicks now)
    {
        ...
        if (type == blink::WebInputEvent::TouchStart || type == blink::WebInputEvent::GestureScrollBegin || type == blink::WebInputEvent::GesturePinchBegin) {
            ...
            if (!is_gesture_active_) {
                ...
                if (!last_continuous_gesture_time_.is_null()) {
                    base::TimeDelta time_since_last_gesture = now - last_continuous_gesture_time_;
                    UMA_HISTOGRAM_MEDIUM_TIMES(
        ...
        if (type == blink::WebInputEvent::GestureScrollBegin || type == blink::WebInputEvent::GestureScrollEnd || type == blink::WebInputEvent::GestureScrollUpdate || type == blink::WebInputEvent::GestureFlingStart || type == blink::WebInputEvent::GestureFlingCancel || type == blink::WebInputEvent::GesturePinchBegin || type == blink::WebInputEvent::GesturePinchEnd || type == blink::WebInputEvent::GesturePinchUpdate) {
        ...
        if (type == blink::WebInputEvent::GestureScrollEnd || type == blink::WebInputEvent::GesturePinchEnd || type == blink::WebInputEvent::GestureFlingStart || type == blink::WebInputEvent::TouchEnd) {
            // Only update stats once per gesture.
            if (is_gesture_active_) {
    ...
    {
        base::TimeDelta escalated_priority_duration = base::TimeDelta::FromMilliseconds(kGestureEstimationLimitMillis);

        // If the input event is still pending, go into input prioritized policy and
        // check again later.
        if (pending_input_event_count_ > 0)
            return escalated_priority_duration;
        if (last_input_signal_time_.is_null() || last_input_signal_time_ + escalated_priority_duration < now) {
        ...
        base::TimeDelta* prediction_valid_duration)
    {
        ...
        is_gesture_expected_ = IsGestureExpectedSoonImpl(now, prediction_valid_duration);
        ...
        if (was_gesture_expected && !is_gesture_expected_ && last_gesture_expected_start_time_ > last_gesture_start_time_) {
        ...
        base::TimeDelta* prediction_valid_duration) const
    {
        if (is_gesture_active_) {
            if (IsGestureExpectedToContinue(now, prediction_valid_duration)) {
                ...
                // gesture soon.
                *prediction_valid_duration = base::TimeDelta::FromMilliseconds(kExpectSubsequentGestureMillis);
                return true;
            }
        } else {
            // If we've have a finished a gesture then a subsequent gesture is deemed
            // likely.
            base::TimeDelta expect_subsequent_gesture_for = base::TimeDelta::FromMilliseconds(kExpectSubsequentGestureMillis);
            if (last_continuous_gesture_time_.is_null() || last_continuous_gesture_time_ + expect_subsequent_gesture_for <= now) {
                return false;
            }
            *prediction_valid_duration = last_continuous_gesture_time_ + expect_subsequent_gesture_for - now;
            return true;
        }
    }
        ...
        base::TimeDelta* prediction_valid_duration) const
    {
        ...
        base::TimeDelta median_gesture_duration = base::TimeDelta::FromMilliseconds(kMedianGestureDurationMillis);
        base::TimeTicks expected_gesture_end_time = last_gesture_start_time_ + median_gesture_duration;
