...
namespace scheduler {
    ...
    void WebFrameSchedulerImpl::RemoveTimerQueueFromBackgroundTimeBudgetPool()
    {
        ...
        TaskQueueThrottler::TimeBudgetPool* time_budget_pool = parent_web_view_scheduler_->BackgroundTimeBudgetPool();
    ...
    RefPtr<blink::WebTaskRunner> WebFrameSchedulerImpl::loadingTaskRunner()
    {
        ...
        if (!loading_web_task_runner_) {
            ...
            loading_queue_enabled_voter_ = loading_task_queue_->CreateQueueEnabledVoter();
    ...
    RefPtr<blink::WebTaskRunner> WebFrameSchedulerImpl::unthrottledTaskRunner()
    {
        ...
        if (!unthrottled_web_task_runner_) {
            ...
            unthrottled_web_task_runner_ = WebTaskRunnerImpl::create(unthrottled_task_queue_);
        }
    ...
    bool WebFrameSchedulerImpl::ShouldThrottleTimers() const
    {
        if (page_throttled_)
            return true;
        return RuntimeEnabledFeatures::timerThrottlingForHiddenFramesEnabled() && !frame_visible_ && cross_origin_;
    }
