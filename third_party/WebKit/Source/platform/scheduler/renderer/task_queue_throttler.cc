...
namespace scheduler {
    ...
    void TaskQueueThrottler::TimeBudgetPool::AddQueue(base::TimeTicks now,
        TaskQueue* queue)
    {
        std::pair<TaskQueueMap::iterator, bool> insert_result = task_queue_throttler_->queue_details_.insert(
        ...
        TaskQueue* queue)
    {
        ...
        DCHECK(find_it != task_queue_throttler_->queue_details_.end() && find_it->second.time_budget_pool == this);
        ...
        base::TimeTicks end_time)
    {
        ...
        if (is_enabled_) {
            ...
            if (!reporting_callback_.is_null() && old_budget_level.InSecondsF() > 0 && current_budget_level_.InSecondsF() < 0) {
    ...
    void TaskQueueThrottler::TimeBudgetPool::EnforceBudgetLevelRestrictions()
    {
        if (max_budget_level_) {
            current_budget_level_ = std::min(current_budget_level_, max_budget_level_.value());
        }
        if (max_throttling_duration_) {
            // Current budget level may be negative.
            current_budget_level_ = std::max(current_budget_level_,
        ...
        , weak_factory_(this)
    {
        ...
        forward_immediate_work_callback_ = base::Bind(&TaskQueueThrottler::OnTimeDomainHasImmediateWork,
    ...
    void TaskQueueThrottler::IncreaseThrottleRefCount(TaskQueue* task_queue)
    {
        ...
        std::pair<TaskQueueMap::iterator, bool> insert_result = queue_details_.insert(std::make_pair(task_queue, Metadata()));
    ...
    void TaskQueueThrottler::DecreaseThrottleRefCount(TaskQueue* task_queue)
    {
        ...
        if (iter == queue_details_.end() || --iter->second.throttling_ref_count != 0) {
            return;
        }
    ...
    void TaskQueueThrottler::OnTimeDomainHasDelayedWork(TaskQueue* queue)
    {
        ...
        base::Optional<base::TimeTicks> next_scheduled_delayed_task = NextTaskRunTime(&lazy_now, queue);
    ...
    void TaskQueueThrottler::PumpThrottledTasks()
    {
        ...
        for (const TaskQueueMap::value_type& map_entry : queue_details_) {
            ...
            base::TimeTicks next_allowed_run_time = GetNextAllowedRunTime(lazy_now.Now(), task_queue);
            base::Optional<base::TimeTicks> next_desired_run_time = NextTaskRunTime(&lazy_now, task_queue);

            if (next_desired_run_time && next_allowed_run_time > next_desired_run_time.value()) {
                TRACE_EVENT1(
                    tracing_category_,
                ...
                next_scheduled_delayed_task = Min(next_scheduled_delayed_task, next_allowed_run_time);

                continue;
            }

            next_scheduled_delayed_task = Min(next_scheduled_delayed_task, task_queue->GetNextScheduledWakeUp());
        ...
        base::TimeTicks unthrottled_runtime)
    {
        ...
        return unthrottled_runtime + one_second - ((unthrottled_runtime - base::TimeTicks()) % one_second);
    }
        ...
        base::TimeTicks unaligned_runtime)
    {
        ...
        base::TimeTicks runtime = AlignedThrottledRunTime(std::max(now, unaligned_runtime));
        DCHECK_LE(now, runtime);

        // If there is a pending call to PumpThrottledTasks and it's sooner than
        // |runtime| then return.
        if (pending_pump_throttled_tasks_runtime_ && runtime >= pending_pump_throttled_tasks_runtime_.value()) {
            return;
        }
    ...
    TaskQueueThrottler::TimeBudgetPool* TaskQueueThrottler::CreateTimeBudgetPool(
        ...
        base::Optional<base::TimeDelta> max_throttling_duration)
    {
        TimeBudgetPool* time_budget_pool = new TimeBudgetPool(name, this, tick_clock_->NowTicks(), max_budget_level,
    ...
    {
        LazyNow lazy_now(now);
        base::Optional<base::TimeTicks> next_run_time = Max(NextTaskRunTime(&lazy_now, queue), next_possible_run_time);
