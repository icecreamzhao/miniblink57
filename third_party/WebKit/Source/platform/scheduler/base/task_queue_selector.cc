...
// chromeos bots without it :(
            ...
            WorkQueue** out_work_queue) const
        {
            ...
            if (task_queue_selector_->immediate_starvation_count_ >= kMaxDelayedStarvationTasks) {
            ...
            bool* out_chose_delayed_over_immediate)
        {
            ...
            if (max_priority > TaskQueue::CONTROL_PRIORITY && ChooseOldestWithPriority(TaskQueue::CONTROL_PRIORITY, out_chose_delayed_over_immediate, out_work_queue)) {
                return true;
            }

            // Select from the normal priority queue if we are starving it.
            if (max_priority > TaskQueue::NORMAL_PRIORITY && task_queue_selector_->high_priority_starvation_count_ >= kMaxHighPriorityStarvationTasks && ChooseOldestWithPriority(TaskQueue::NORMAL_PRIORITY, out_chose_delayed_over_immediate, out_work_queue)) {
                return true;
            }
        ...
        {
            bool contains_delayed_work_queue = delayed_work_queue_sets_.ContainsWorkQueueForTest(
            ...
            const WorkQueue& chosen_enabled_queue)
        {
            ...
            TaskQueue::QueuePriority max_priority = NextPriority(chosen_enabled_queue.task_queue()->GetQueuePriority());
            ...
            if (chosen_blocked_queue->task_queue()->GetQueuePriority() < chosen_enabled_queue.task_queue()->GetQueuePriority()) {
                task_queue_selector_observer_->OnTriedToSelectBlockedWorkQueue(
                    chosen_blocked_queue);
                return;
            }
        ...
        bool TaskQueueSelector::EnabledWorkQueuesEmpty() const
        {
            ...
            for (TaskQueue::QueuePriority priority = TaskQueue::CONTROL_PRIORITY;
                ...
                if (!enabled_selector_.delayed_work_queue_sets()->IsSetEmpty(priority) || !enabled_selector_.immediate_work_queue_sets()->IsSetEmpty(priority)) {
                    return false;
                }
            }
            return true;
