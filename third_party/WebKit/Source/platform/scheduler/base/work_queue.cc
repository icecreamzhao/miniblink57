...
namespace scheduler {
    namespace internal {
        ...
        bool WorkQueue::InsertFence(EnqueueOrder fence)
        {
            ...
            if (work_queue_sets_ && !work_queue_.empty() && was_blocked_by_fence && !BlockedByFence()) {
                work_queue_sets_->OnTaskPushedToEmptyQueue(this);
                return true;
            }
        ...
        bool WorkQueue::ShouldRunBefore(const WorkQueue* other_queue) const
        {
            ...
            bool have_other_task = other_queue->GetFrontTaskEnqueueOrder(&other_enqueue_order);
            DCHECK(have_task);
