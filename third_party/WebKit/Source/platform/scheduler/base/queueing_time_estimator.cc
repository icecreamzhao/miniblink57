...
namespace scheduler {
    ...
    namespace {
            ...
            base::TimeTicks window_end)
        {
            ...
            base::TimeTicks task_in_window_start_time = std::max(task_start, window_start);
            base::TimeTicks task_in_window_end_time = std::min(task_end, window_end);
            DCHECK(task_in_window_end_time <= task_in_window_end_time);

            double probability_of_this_task = static_cast<double>((task_in_window_end_time - task_in_window_start_time)
                                                                      .InMicroseconds())
                / (window_end - window_start).InMicroseconds();

            base::TimeDelta expected_queueing_duration_within_task = ((task_end - task_in_window_start_time) + (task_end - task_in_window_end_time)) / 2;

            return base::TimeDelta::FromMillisecondsD(
                probability_of_this_task * expected_queueing_duration_within_task.InMillisecondsF());
        }
