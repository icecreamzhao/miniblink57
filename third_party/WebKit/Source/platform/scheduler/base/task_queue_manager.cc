...
namespace scheduler {
        ...
        , weak_factory_(this)
    {
        ...
        from_main_thread_immediate_do_work_closure_ = base::Bind(&TaskQueueManager::DoWork, weak_factory_.GetWeakPtr(),
            base::TimeTicks(), true);
        from_other_thread_immediate_do_work_closure_ = base::Bind(&TaskQueueManager::DoWork, weak_factory_.GetWeakPtr(),
        ...
        const TaskQueue::Spec& spec)
    {
        ...
        TimeDomain* time_domain = spec.time_domain ? spec.time_domain : real_time_domain_.get();
        ...
        const tracked_objects::Location& from_here)
    {
        ...
        if (on_main_thread) {
            ...
            delegate_->PostTask(from_here, from_other_thread_immediate_do_work_closure_);
        }
    }
        ...
        base::TimeDelta delay)
    {
        ...
        if (main_thread_pending_wakeups_.find(base::TimeTicks()) != main_thread_pending_wakeups_.end()) {
            return;
        }
        // De-duplicate DoWork posts.
        base::TimeTicks run_time = now + delay;
        if (!main_thread_pending_wakeups_.empty() && *main_thread_pending_wakeups_.begin() <= run_time) {
            return;
        }
        main_thread_pending_wakeups_.insert(run_time);
        delegate_->PostDelayedTask(
            from_here, base::Bind(&TaskQueueManager::DoWork, weak_factory_.GetWeakPtr(), run_time, true),
            delay);
    }
    ...
    void TaskQueueManager::DoWork(base::TimeTicks run_time, bool from_main_thread)
    {
        ...
        // continuation task.
        base::Optional<base::TimeDelta> next_delay = ComputeDelayTillNextTask(&lazy_now);
        ...
        LazyNow* lazy_now)
    {
        ...
        for (TimeDomain* time_domain : time_domains_) {
            base::Optional<base::TimeDelta> continuation = time_domain->DelayTillNextTask(lazy_now);
            if (!continuation)
                continue;
        ...
        base::TimeTicks* time_after_task)
    {
        ...
        internal::TaskQueueImpl::Task pending_task = work_queue->TakeTaskFromWorkQueue();
        ...
        if (queue->GetShouldNotifyObservers()) {
            ...
            bool notify_time_observers = !delegate_->IsNested() && task_time_observers_.might_have_observers();
        ...
        internal::TaskQueueImpl* prev_executing_task_queue = currently_executing_task_queue_;
        ...
        const internal::TaskQueueImpl* queue)
    {
        ...
        if (!pending_task.delayed_run_time.is_null()) {
            RecordDelayedTaskLateness(delegate_->NowTicks() - pending_task.delayed_run_time);
        } else if (!pending_task.time_posted.is_null()) {
            RecordImmediateTaskQueueingDuration(tracked_objects::TrackedTime::Now() - pending_task.time_posted);
        }
    }
