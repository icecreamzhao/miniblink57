...
namespace scheduler {
    ...
    namespace internal {
            ...
            TaskType task_type)
        {
            ...
            EnqueueOrder sequence_number = any_thread().task_queue_manager->GetNextSequenceNumber();
            ...
            TaskType task_type)
        {
            ...
            if (base::PlatformThread::CurrentId() == thread_id_) {
                ...
                EnqueueOrder sequence_number = main_thread_only().task_queue_manager->GetNextSequenceNumber();
            ...
            Task pending_task, base::TimeTicks now)
        {
            ...
            // is requested.
            if (main_thread_only().delayed_incoming_queue.top().delayed_run_time == delayed_run_time) {
        ...
        void TaskQueueImpl::PushOntoDelayedIncomingQueueLocked(Task pending_task)
        {
            ...
            int thread_hop_task_sequence_number = any_thread().task_queue_manager->GetNextSequenceNumber();
            PushOntoImmediateIncomingQueueLocked(
                FROM_HERE,
        ...
        {
            if (!main_thread_only().delayed_work_queue->Empty() || !main_thread_only().delayed_incoming_queue.empty() || !main_thread_only().immediate_work_queue->Empty()) {
                return false;
            }
        ...
        bool TaskQueueImpl::HasPendingImmediateWork() const
        {
            ...
            if (!main_thread_only().delayed_work_queue->Empty() || !main_thread_only().immediate_work_queue->Empty()) {
                return true;
            }

            // Tasks on |delayed_incoming_queue| that could run now, count as
            // immediate work.
            if (!main_thread_only().delayed_incoming_queue.empty() && main_thread_only().delayed_incoming_queue.top().delayed_run_time <= main_thread_only().time_domain->CreateLazyNow().Now()) {
                return true;
            }
        ...
        void TaskQueueImpl::WakeUpForDelayedWork(LazyNow* lazy_now)
        {
            ...
            while (!main_thread_only().delayed_incoming_queue.empty()) {
                Task& task = const_cast<Task&>(main_thread_only().delayed_incoming_queue.top());
        ...
        void TaskQueueImpl::TraceQueueSize(bool is_locked) const
        {
            ...
            TRACE_COUNTER1(disabled_by_default_tracing_category_, GetName(),
                any_thread().immediate_incoming_queue.size() + main_thread_only().immediate_work_queue->Size() + main_thread_only().delayed_work_queue->Size() + main_thread_only().delayed_incoming_queue.size());
            if (!is_locked)
        ...
        void TaskQueueImpl::AsValueInto(base::trace_event::TracedValue* state) const
        {
            ...
            state->SetString(
                "task_queue_id",
                base::StringPrintf("%" PRIx64, static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this))));
            ...
            if (!main_thread_only().delayed_incoming_queue.empty()) {
                base::TimeDelta delay_to_next_task = (main_thread_only().delayed_incoming_queue.top().delayed_run_time - main_thread_only().time_domain->CreateLazyNow().Now());
        ...
        void TaskQueueImpl::InsertFence(TaskQueue::InsertFencePosition position)
        {
            ...
            main_thread_only().current_fence = position == TaskQueue::InsertFencePosition::NOW
                ? main_thread_only().task_queue_manager->GetNextSequenceNumber()
            ...
            if (!task_unblocked && previous_fence && previous_fence < main_thread_only().current_fence) {
                base::AutoLock lock(any_thread_lock_);
                if (!any_thread().immediate_incoming_queue.empty() && any_thread().immediate_incoming_queue.front().enqueue_order() > previous_fence && any_thread().immediate_incoming_queue.front().enqueue_order() < main_thread_only().current_fence) {
                    task_unblocked = true;
                }
            }
        ...
        void TaskQueueImpl::RemoveFence()
        {
            ...
            if (!task_unblocked && previous_fence) {
                base::AutoLock lock(any_thread_lock_);
                if (!any_thread().immediate_incoming_queue.empty() && any_thread().immediate_incoming_queue.front().enqueue_order() > previous_fence) {
                    task_unblocked = true;
                }
            }
        ...
        bool TaskQueueImpl::BlockedByFence() const
        {
            ...
            if (!main_thread_only().immediate_work_queue->BlockedByFence() || !main_thread_only().delayed_work_queue->BlockedByFence()) {
                return false;
            }
            ...
            return any_thread().immediate_incoming_queue.front().enqueue_order() > main_thread_only().current_fence;
        }
            ...
            base::trace_event::TracedValue* state)
        {
            ...
            std::priority_queue<Task>* mutable_queue = const_cast<std::priority_queue<Task>*>(&queue);
        ...
        {
            return main_thread_only().is_enabled_refcount == main_thread_only().voter_refcount;
        }
        ...
        void TaskQueueImpl::SweepCanceledDelayedTasks(base::TimeTicks now)
        {
            ...
            base::TimeTicks first_task_runtime = main_thread_only().delayed_incoming_queue.top().delayed_run_time;

            // TODO(alexclarke): Let this remove all tasks once the DoWork refactor has
            // landed.
            std::priority_queue<Task> remaining_tasks;
            while (!main_thread_only().delayed_incoming_queue.empty()) {
                if (!main_thread_only().delayed_incoming_queue.top().task.IsCancelled() || main_thread_only().delayed_incoming_queue.top().delayed_run_time == first_task_runtime) {
