...
namespace scheduler {
        ...
        NowCalledMinimumNumberOfTimesToComputeTaskDurations)
    {
        ...
        TestCountUsesTimeSource* test_count_uses_time_source = new TestCountUsesTimeSource();
    ...
    TEST_F(TaskQueueManagerTest, NowNotCalledForNestedTasks)
    {
        ...
        runners_[0]->PostTask(
            FROM_HERE, base::Bind(&PostFromNestedRunloop, message_loop_.get(), base::RetainedRef(runners_[0]), base::Unretained(&tasks_to_post_from_nested_loop)));
    ...
    TEST_F(TaskQueueManagerTest, DenyRunning_BeforePosting)
    {
        ...
        std::vector<EnqueueOrder> run_order;
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = runners_[0]->CreateQueueEnabledVoter();
    ...
    {
        (*run_count)++;
        runner->PostTask(FROM_HERE, Bind(&RePostingTestTask, base::Unretained(runner.get()), run_count));
    }
    ...
    TEST_F(TaskQueueManagerTest, DeleteTaskQueueManagerInsideATask)
    {
        ...
        runners_[0]->PostTask(
            FROM_HERE, base::Bind(&TaskQueueManagerTest::DeleteTaskQueueManager, base::Unretained(this)));
    ...
    TEST_F(TaskQueueManagerTest, GetAndClearSystemIsQuiescentBit)
    {
        ...
        scoped_refptr<internal::TaskQueueImpl> queue0 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST)
                                                                                   .SetShouldMonitorQuiescence(true));
        scoped_refptr<internal::TaskQueueImpl> queue1 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST)
                                                                                   .SetShouldMonitorQuiescence(true));
        scoped_refptr<internal::TaskQueueImpl> queue2 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST)
    ...
    TEST_F(TaskQueueManagerTest, ImmediateAndDelayedTaskInterleaving)
    {
        ...
        for (int i = 0; i < 9; i++) {
            runners_[0]->PostTask(FROM_HERE, base::Bind(&ExpensiveTestTask, i, now_src_.get(), &run_order));
        }
    ...
    TEST_F(TaskQueueManagerTest, QuitWhileNested)
    {
        ...
        runners_[0]->PostTask(FROM_HERE, base::Bind(&PostAndQuitFromNestedRunloop, base::Unretained(&run_loop), base::RetainedRef(runners_[0]), base::Unretained(&was_nested)));
    ...
    TEST_F(TaskQueueManagerTest, NewTaskQueues)
    {
        ...
        scoped_refptr<internal::TaskQueueImpl> queue1 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST));
        scoped_refptr<internal::TaskQueueImpl> queue2 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST));
        scoped_refptr<internal::TaskQueueImpl> queue3 = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST));
    ...
    TEST_F(TaskQueueManagerTest, OnUnregisterTaskQueue)
    {
        ...
        scoped_refptr<internal::TaskQueueImpl> task_queue = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST));
    ...
    TEST_F(TaskQueueManagerTest, OnTriedToExecuteBlockedTask)
    {
        ...
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = task_queue->CreateQueueEnabledVoter();
        ...
        DelayedTasksDontBadlyStarveNonDelayedWork_SameQueue)
    {
        ...
        LinearTask linear_immediate_task(runners_[0], base::TimeDelta(),
            now_src_.get());
        base::Callback<bool()> should_exit = base::Bind(ShouldExit, &quadratic_delayed_task, &linear_immediate_task);
        ...
        double ratio = static_cast<double>(linear_immediate_task.count()) / static_cast<double>(quadratic_delayed_task.count());
    ...
    TEST_F(TaskQueueManagerTest, ImmediateWorkCanStarveDelayedTasks_SameQueue)
    {
        ...
        LinearTask linear_delayed_task(
            runners_[0], base::TimeDelta::FromMilliseconds(10), now_src_.get());
        base::Callback<bool()> should_exit = base::Bind(&ShouldExit, &quadratic_immediate_task, &linear_delayed_task);
        ...
        double ratio = static_cast<double>(linear_delayed_task.count()) / static_cast<double>(quadratic_immediate_task.count());
    ...
    TEST_F(TaskQueueManagerTest, CurrentlyExecutingTaskQueue_TaskRunning)
    {
        ...
        std::vector<TaskQueue*> task_sources;
        queue0->PostTask(FROM_HERE, base::Bind(&CurrentlyExecutingTaskQueueTestTask, manager_.get(), &task_sources));
        queue1->PostTask(FROM_HERE, base::Bind(&CurrentlyExecutingTaskQueueTestTask, manager_.get(), &task_sources));
        test_task_runner_->RunUntilIdle();
    ...
    TEST_F(TaskQueueManagerTest, CurrentlyExecutingTaskQueue_NestedLoop)
    {
        ...
        queue0->PostTask(
            FROM_HERE, base::Bind(&RunloopCurrentlyExecutingTaskQueueTestTask, message_loop_.get(), manager_.get(), &task_sources, &tasks_to_post_from_nested_loop));
    ...
    TEST_F(TaskQueueManagerTestWithTracing, BlameContextAttribution)
    {
        ...
        Query q = Query::EventPhaseIs(TRACE_EVENT_PHASE_ENTER_CONTEXT) || Query::EventPhaseIs(TRACE_EVENT_PHASE_LEAVE_CONTEXT);
    ...
    TEST_F(TaskQueueManagerTest, NoWakeUpsForCanceledDelayedTasks)
    {
        ...
        runners_[0]->PostDelayedTask(
            FROM_HERE, base::Bind(&CancelableTask::RecordTimeTask, task1.weak_factory_.GetWeakPtr(), &run_times),
            delay1);
        runners_[0]->PostDelayedTask(
            FROM_HERE, base::Bind(&CancelableTask::RecordTimeTask, task2.weak_factory_.GetWeakPtr(), &run_times),
            delay2);
        runners_[0]->PostDelayedTask(
            FROM_HERE, base::Bind(&CancelableTask::RecordTimeTask, task3.weak_factory_.GetWeakPtr(), &run_times),
            delay3);
        runners_[0]->PostDelayedTask(
            FROM_HERE, base::Bind(&CancelableTask::RecordTimeTask, task4.weak_factory_.GetWeakPtr(), &run_times),
            delay4);
    ...
    TEST_F(TaskQueueManagerTest, TaskQueueVoters)
    {
        ...
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter1 = runners_[0]->CreateQueueEnabledVoter();
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter2 = runners_[0]->CreateQueueEnabledVoter();
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter3 = runners_[0]->CreateQueueEnabledVoter();
        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter4 = runners_[0]->CreateQueueEnabledVoter();
    ...
    TEST_F(TaskQueueManagerTest, UnregisterQueueBeforeEnabledVoterDeleted)
    {
        ...
        scoped_refptr<internal::TaskQueueImpl> queue = manager_->NewTaskQueue(TaskQueue::Spec(TaskQueue::QueueType::TEST));

        std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = queue->CreateQueueEnabledVoter();
