...
namespace scheduler {
    namespace internal {
        ...
        public:
            ...
            std::vector<size_t> PopTasks()
            {
                ...
                while (selector_.SelectWorkQueueToService(&chosen_work_queue)) {
                    size_t chosen_queue_index = queue_to_index_map_.find(chosen_work_queue->task_queue())->second;
            ...
            {
                TaskQueueImpl::QueueEnabledVoterImpl* voter_impl = static_cast<TaskQueueImpl::QueueEnabledVoterImpl*>(voter);
        ...
        protected:
            void SetUp() final
            {
                ...
                for (size_t i = 0; i < kTaskQueueCount; i++) {
                    scoped_refptr<TaskQueueImpl> task_queue = make_scoped_refptr(new TaskQueueImpl(
        ...
        {
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter1 = task_queues_[1]->CreateQueueEnabledVoter();
        ...
        TEST_F(TaskQueueSelectorTest, TestDisableEnable)
        {
            ...
            size_t queue_order[] = { 0, 1, 2, 3, 4 };
            PushTasks(queue_order, 5);
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter2 = task_queues_[2]->CreateQueueEnabledVoter();
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter4 = task_queues_[4]->CreateQueueEnabledVoter();
        ...
        TEST_F(TaskQueueSelectorTest, TestEmptyQueues)
        {
            ...
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter0 = task_queues_[0]->CreateQueueEnabledVoter();
        ...
        TEST_F(TaskQueueSelectorTest, TestObserverWithTwoBlockedQueues)
        {
            ...
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = task_queue->CreateQueueEnabledVoter();
            std::unique_ptr<TaskQueue::QueueEnabledVoter> voter2 = task_queue2->CreateQueueEnabledVoter();
