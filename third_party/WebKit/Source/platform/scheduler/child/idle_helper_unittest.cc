...
namespace scheduler {
            ...
            base::TimeDelta delay)
        {
            if (num_repeats > 1) {
                task_runner->PostDelayedTask(
                    FROM_HERE, base::Bind(&RepeatingTask, base::Unretained(task_runner), num_repeats - 1, delay),
                    delay);
            }
        }
    ...
    protected:
        ...
        {
            EXPECT_STREQ(expected, IdleHelper::IdlePeriodStateToString(idle_helper_->SchedulerIdlePeriodState()));
        }
    ...
    TEST_F(IdleHelperTest, TestLongIdlePeriodRestartWaitsIfNotMaxDeadline)
    {
        ...
        base::TimeTicks expected_deadline(clock_->NowTicks() + pending_task_delay + maximum_idle_period_duration() + retry_enable_long_idle_period_delay());
    ...
    void TestCanExceedIdleDeadlineIfRequiredTask(IdleHelperForTest* idle_helper,
        ...
        base::TimeTicks deadline)
    {
        *can_exceed_idle_deadline_out = idle_helper->CanExceedIdleDeadlineIfRequired();
        (*run_count)++;
    }
        ...
        LongIdlePeriodStartsAfterQuiescence)
    {
        ...
        // a row.
        base::TimeTicks expected_deadline = clock_->NowTicks() + base::TimeDelta::FromMilliseconds(5 * kQuiescenceDelayMs + kLongIdlePeriodMs);
        base::TimeTicks deadline_in_task;
        idle_task_runner_->PostIdleTask(
            FROM_HERE, base::Bind(&IdleTestTask, &run_count, &deadline_in_task));
        ...
        QuescienceCheckedForAfterLongIdlePeriodEnds)
    {
        ...
        int run_count = 0;
        base::TimeTicks deadline_in_task;
        base::TimeTicks expected_deadline = clock_->NowTicks() + base::TimeDelta::FromMilliseconds(kQuiescenceDelayMs + kLongIdlePeriodMs);
        idle_task_runner_->PostIdleTask(
            FROM_HERE, base::Bind(&IdleTestTask, &run_count, &deadline_in_task));
        idle_helper_->EnableLongIdlePeriod();
        RunUntilIdle();
