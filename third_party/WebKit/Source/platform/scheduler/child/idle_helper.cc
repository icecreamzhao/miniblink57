...
namespace scheduler {
        ...
        base::TimeDelta* next_long_idle_period_delay_out)
    {
        ...
        base::TimeDelta max_long_idle_period_duration = base::TimeDelta::FromMilliseconds(kMaximumIdlePeriodMillis);
        ...
        if (long_idle_period_duration >= base::TimeDelta::FromMilliseconds(kMinimumIdlePeriodDurationMillis)) {
    ...
    bool IdleHelper::ShouldWaitForQuiescence()
    {
        ...
        if (required_quiescence_duration_before_long_idle_period_ == base::TimeDelta()) {
            return false;
        }
    ...
    void IdleHelper::EnableLongIdlePeriod()
    {
        ...
        IdlePeriodState new_idle_period_state = ComputeNewLongIdlePeriodState(now, &next_long_idle_period_delay);
        ...
        base::TimeTicks idle_period_deadline)
    {
        ...
        if (idle_period_duration < base::TimeDelta::FromMilliseconds(kMinimumIdlePeriodDurationMillis)) {
    ...
    void IdleHelper::DidProcessTask(const base::PendingTask& pending_task)
    {
        ...
        if (IsInIdlePeriod(state_.idle_period_state()) && state_.idle_period_state() != IdlePeriodState::IN_LONG_IDLE_PERIOD_PAUSED && helper_->scheduler_tqm_delegate()->NowTicks() >= state_.idle_period_deadline()) {
            ...
            if (IsInLongIdlePeriod(state_.idle_period_state())) {
                ...
                DCHECK(IdlePeriodState::IN_SHORT_IDLE_PERIOD == state_.idle_period_state());
                EndIdlePeriod();
            }
        }
    }
    ...
    void IdleHelper::UpdateLongIdlePeriodStateAfterIdleTask()
    {
        ...
        if (!idle_queue_->HasPendingImmediateWork()) {
            ...
            base::TimeDelta next_long_idle_period_delay;
            if (state_.idle_period_state() == IdlePeriodState::IN_LONG_IDLE_PERIOD_WITH_MAX_DEADLINE) {
                ...
                next_long_idle_period_delay = std::max(
                    base::TimeDelta(), state_.idle_period_deadline() - helper_->scheduler_tqm_delegate()->NowTicks());
            }
    ...
    void IdleHelper::OnIdleTaskPostedOnMainThread()
    {
        ...
        if (is_shutdown_)
            return;
        if (state_.idle_period_state() == IdlePeriodState::IN_LONG_IDLE_PERIOD_PAUSED) {
    ...
    {
        return state == IdlePeriodState::IN_LONG_IDLE_PERIOD || state == IdlePeriodState::IN_LONG_IDLE_PERIOD_WITH_MAX_DEADLINE || state == IdlePeriodState::IN_LONG_IDLE_PERIOD_PAUSED;
    }
    ...
    bool IdleHelper::CanExceedIdleDeadlineIfRequired() const
    {
        ...
        helper_->CheckOnValidThread();
        return state_.idle_period_state() == IdlePeriodState::IN_LONG_IDLE_PERIOD_WITH_MAX_DEADLINE;
    }
        ...
        base::TimeTicks optional_now)
    {
        ...
        } else if (!IsInIdlePeriod(new_state) && IsInIdlePeriod(old_idle_period_state)) {
        ...
        base::TimeTicks now)
    {
        ...
        if (idle_period_trace_event_started_ && running_idle_task_for_tracing_ && !new_running_idle_task) {
            running_idle_task_for_tracing_ = false;
        ...
        if (IsInIdlePeriod(new_state)) {
            ...
            } else if (IsInLongIdlePeriod(new_state) && new_state != IdlePeriodState::IN_LONG_IDLE_PERIOD_PAUSED) {
                TRACE_EVENT_ASYNC_STEP_INTO0(tracing_category_, idle_period_tracing_name_,
                    this, "LongIdlePeriod");
