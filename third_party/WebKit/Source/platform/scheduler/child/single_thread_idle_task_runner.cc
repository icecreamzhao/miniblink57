...
namespace scheduler {
    ...
    SingleThreadIdleTaskRunner::SingleThreadIdleTaskRunner(
        ...
        , weak_factory_(this)
    {
        DCHECK(!idle_priority_task_runner_ || idle_priority_task_runner_->RunsTasksOnCurrentThread());
        ...
        const IdleTask& idle_task)
    {
        ...
        idle_priority_task_runner_->PostTask(
            from_here, base::Bind(&SingleThreadIdleTaskRunner::RunTask, weak_scheduler_ptr_, idle_task));
    }
        ...
        const IdleTask& idle_task)
    {
        ...
        delayed_idle_tasks_.insert(std::make_pair(
            first_run_time,
            std::make_pair(from_here, base::Bind(&SingleThreadIdleTaskRunner::RunTask, weak_scheduler_ptr_, idle_task))));
    }
    ...
    void SingleThreadIdleTaskRunner::EnqueueReadyDelayedIdleTasks()
    {
        ...
        while (!delayed_idle_tasks_.empty() && delayed_idle_tasks_.begin()->first <= now) {
            idle_priority_task_runner_->PostTask(
