...
namespace scheduler {
    ...
    void WebThreadBase::addTaskObserver(TaskObserver* observer)
    {
        ...
        std::pair<TaskObserverMap::iterator, bool> result = task_observer_map_.insert(std::make_pair(observer, nullptr));
        if (result.second)
        ...
        IdleTask* idle_task)
    {
        GetIdleTaskRunner()->PostIdleTask(
            location, base::Bind(&WebThreadBase::RunWebThreadIdleTask, base::Passed(base::WrapUnique(idle_task))));
    }
