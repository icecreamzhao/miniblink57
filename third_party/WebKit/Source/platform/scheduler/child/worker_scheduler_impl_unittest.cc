...
namespace scheduler {
    ...
    TEST_F(WorkerSchedulerImplTest, TestIdleTaskWhenIsNotQuiescent)
    {
        ...
        default_task_runner_->PostTask(
            FROM_HERE, base::Bind(&RecordTimelineTask, base::Unretained(&timeline), base::Unretained(clock_.get())));
        RunUntilIdle();
    ...
    TEST_F(WorkerSchedulerImplTest, TestLongIdlePeriodTimeline)
    {
        ...
        // idle period.
        base::TimeTicks idle_period_deadline = scheduler_->CurrentIdleTaskDeadlineForTesting();
        clock_->Advance(maximum_idle_period_duration());
        RunUntilIdle();

        base::TimeTicks new_idle_period_deadline = scheduler_->CurrentIdleTaskDeadlineForTesting();
        ...
        // the idle task run.
        default_task_runner_->PostDelayedTask(
            FROM_HERE, base::Bind(&PostIdleTask, base::Unretained(&timeline), base::Unretained(clock_.get()), base::Unretained(idle_task_runner_.get())),
