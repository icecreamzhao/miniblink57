...
template <
    typename T,
    bool mainThreadOnly = WTF::IsSubclass<typename std::remove_const<T>::type, Node>::value || WTF::IsSubclass<typename std::remove_const<T>::type, NodeList>::value>
...
struct ThreadingTrait<HashMap<Key, Value, T, U, V, HeapAllocator>> {
    STATIC_ONLY(ThreadingTrait);
    static const ThreadAffinity Affinity = (ThreadingTrait<Key>::Affinity == MainThreadOnly) && (ThreadingTrait<Value>::Affinity == MainThreadOnly)
        ? MainThreadOnly
        : AnyThread;
};
...
struct ThreadingTrait<WTF::KeyValuePair<First, Second>> {
    STATIC_ONLY(ThreadingTrait);
    static const ThreadAffinity Affinity = (ThreadingTrait<First>::Affinity == MainThreadOnly) && (ThreadingTrait<Second>::Affinity == MainThreadOnly)
        ? MainThreadOnly
        : AnyThread;
};
