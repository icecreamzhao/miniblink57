...
TEST(HeapCompactTest, SparseBitmapBasic)
{
    ...
    SparseHeapBitmap* start = bitmap->hasRange(base + 2 * unitPointer, doubleChunk);
    EXPECT_TRUE(!!start);
...
TEST(HeapCompactTest, SparseBitmapBuild)
{
    ...
    SparseHeapBitmap* start = bitmap->hasRange(base - doubleChunk - 2 * unitPointer, 4 * unitPointer);
    EXPECT_TRUE(!!start);
