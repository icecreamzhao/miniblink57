...
public:
        ...
        void* callbackData)
    {
        ...
        m_fixupCallbacks.add(reference, std::pair<void*, MovingObjectCallback>(callbackData, callback));
    }
    ...
    void relocateInteriorFixups(Address from, Address to, size_t size)
    {
        ...
        for (size_t offset = 0; offset < size; offset += sizeof(void*)) {
            if (!range->isSet(from + offset))
                continue;
            MovableReference* slot = reinterpret_cast<MovableReference*>(from + offset);
            auto it = m_interiorFixups.find(slot);
    ...
    void relocate(Address from, Address to)
    {
        ...
        if (interior != m_interiorFixups.end()) {
            MovableReference* slotLocation = reinterpret_cast<MovableReference*>(interior->value);
            if (!slotLocation) {
        ...
        if (UNLIKELY(*slot != from)) {
            ...
            DCHECK(
                slotPage->isLargeObjectPage() || (HeapCompact::isCompactableArena(slotPage->arena()->arenaIndex()) && !m_relocatablePages.contains(slotPage)));
#endif
            return;
        }
        *slot = to;
    ...
    BlinkGC::GCReason reason)
{
    ...
    if (reason != BlinkGC::IdleGC && reason != BlinkGC::PreciseGC && reason != BlinkGC::ForcedGC)
        return false;
    ...
    return true;
#else
    return s_forceCompactionGC || (m_gcCountSinceLastCompaction > kGCCountSinceLastCompactionThreshold && m_freeListSize > kFreeListSizeThreshold);
#endif
#endif
}
...
void HeapCompact::updateHeapResidency(ThreadState* threadState)
{
    ...
    for (int i = BlinkGC::Vector1ArenaIndex; i <= BlinkGC::HashTableArenaIndex;
         ++i) {
        NormalPageArena* arena = static_cast<NormalPageArena*>(threadState->arena(i));
