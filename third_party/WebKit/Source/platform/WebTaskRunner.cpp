...
using RunnerMethodType = void (blink::TaskHandle::Runner::*)(const blink::TaskHandle&);
...
TaskHandle::TaskHandle(RefPtr<Runner> runner)
...
TaskHandle WebTaskRunner::postCancellableTask(const WebTraceLocation& location, std::unique_ptr<WTF::Closure> task)
{
    ...
    postTask(location, WTF::bind(&TaskHandle::Runner::run, runner->asWeakPtr(), TaskHandle(runner)));
    ...
    long long delayMs)
{
    DCHECK(runsTasksOnCurrentThread());
    RefPtr<TaskHandle::Runner> runner = adoptRef(new TaskHandle::Runner(std::move(task)));
    postDelayedTask(location, WTF::bind(&TaskHandle::Runner::run, runner->asWeakPtr(), TaskHandle(runner)),
        delayMs);
    return TaskHandle(runner);
}
