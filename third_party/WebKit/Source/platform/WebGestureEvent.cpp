...
WebFloatSize WebGestureEvent::tapAreaInRootFrame() const
{
    ...
    } else if (m_type == WebInputEvent::GestureLongPress || m_type == WebInputEvent::GestureLongTap) {
        return WebFloatSize(data.longPress.width / m_frameScale,
            data.longPress.height / m_frameScale);
    } else if (m_type == WebInputEvent::GestureTap || m_type == WebInputEvent::GestureTapUnconfirmed) {
