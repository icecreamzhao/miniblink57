    ...
    const ColorBehavior& colorBehavior)
{
    ...
    const size_t maxDecodedBytes = Platform::current() ? Platform::current()->maxDecodedImageBytes()
...
#if ENABLE_WML
    if (!decoder && WBMPImageDecoder::matchesBMPSignature(data->getAsSkData()))
        decoder.reset(new WBMPImageDecoder(alphaOption, colorBehavior, maxDecodedBytes));
#endif
...
size_t ImageDecoder::clearCacheExceptFrame(size_t clearExceptFrame)
{
    ...
    if (clearExceptFrame < m_frameBufferCache.size()) {
        ...
        if (!frameStatusSufficientForSuccessors(clearExceptFrame) || frame.getDisposalMethod() == ImageFrame::DisposeOverwritePrevious)
    ...
    while (clearExceptFrame2 < m_frameBufferCache.size() && !frameStatusSufficientForSuccessors(clearExceptFrame2)) {
        clearExceptFrame2 = m_frameBufferCache[clearExceptFrame2].requiredPreviousFrameIndex();
    }
...
Vector<size_t> ImageDecoder::findFramesToDecode(size_t index) const
{
    ...
    } while (index != kNotFound && m_frameBufferCache[index].getStatus() != ImageFrame::FrameComplete);
...
void ImageDecoder::correctAlphaWhenFrameBufferSawNoAlpha(size_t index)
{
    ...
    if (buffer.originalFrameRect().contains(IntRect(IntPoint(), size()))) {
        ...
        DCHECK(prevBuffer->getDisposalMethod() != ImageFrame::DisposeOverwritePrevious);
...
bool ImageDecoder::initFrameBuffer(size_t frameIndex)
{
    ...
    if (requiredPreviousFrameIndex == kNotFound) {
        ...
        ImageFrame* const prevBuffer = &m_frameBufferCache[requiredPreviousFrameIndex];
        ...
        if ((!canReusePreviousFrameBuffer(frameIndex) || !buffer->takeBitmapDataIfWritable(prevBuffer)) && !buffer->copyBitmapData(*prevBuffer))
            return setFailed();
