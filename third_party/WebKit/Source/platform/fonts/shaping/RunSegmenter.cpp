...
RunSegmenter::RunSegmenter(const UChar* buffer, unsigned bufferSize, FontOrientation runOrientation)
    ...
    , m_scriptRunIterator(WTF::wrapUnique(new ScriptRunIterator(buffer, bufferSize)))
    , m_orientationIterator(runOrientation == FontOrientation::VerticalMixed ? WTF::wrapUnique(new OrientationIterator(buffer, bufferSize, runOrientation)) : nullptr)
    , m_symbolsIterator(WTF::wrapUnique(new SymbolsIterator(buffer, bufferSize)))
    , m_lastSplit(0)
