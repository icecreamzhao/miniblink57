    ...
    float initialAdvance, unsigned from, unsigned to, unsigned runOffset)
{
    ...
    FloatPoint glyphCenter = emphasisData->fontData->boundsForGlyph(emphasisData->glyph).center();
    ...
    uint16_t clusterStart = static_cast<uint16_t>(direction == TextDirection::kRtl
    ...
    TextDirection dir) const
{
    ...
    for (unsigned i = 0; i < m_results.size(); ++i) {
        const auto& wordResult = isLeftToRightDirection(dir) ? m_results[i] : m_results[m_results.size() - 1 - i];
    ...
    TextDirection direction, float totalWidth) const
{
    ...
    for (const RefPtr<const ShapeResult> result : m_results) {
        ...
        for (unsigned index = 0; index < runCount; index++) {
            unsigned runIndex = direction == TextDirection::kRtl ? runCount - 1 - index : index;
