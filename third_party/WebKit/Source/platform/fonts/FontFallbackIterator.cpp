...
const PassRefPtr<FontDataForRangeSet> FontFallbackIterator::next(const Vector<UChar32>& hintList)
{
    ...
    if (m_fallbackStage == FallbackPriorityFonts) {
        ...
        RefPtr<SimpleFontData> fontData = fallbackPriorityFont(hintList[0]);
        FontDataForRangeSet* rangeSet = new FontDataForRangeSet(fontData);
        RefPtr<FontDataForRangeSet> fallbackPriorityFontRange = adoptRef(rangeSet);
        if (fallbackPriorityFontRange->hasFontData())
            return fallbackPriorityFontRange.release();

        RefPtr<FontDataForRangeSet> result = next(hintList);
        return result;
    }
