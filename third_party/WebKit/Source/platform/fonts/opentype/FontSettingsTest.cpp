...
{
    RefPtr<FontVariationSettings> variationSettings = FontVariationSettings::create();
...
{
    RefPtr<FontVariationSettings> oneAxisA = makeFontVariationSettings({ FontVariationAxis { "a   ", 0 } });
    RefPtr<FontVariationSettings> oneAxisB = makeFontVariationSettings({ FontVariationAxis { "b   ", 0 } });
    RefPtr<FontVariationSettings> twoAxes = makeFontVariationSettings(
        { FontVariationAxis { "a   ", 0 }, FontVariationAxis { "b   ", 0 } });
    RefPtr<FontVariationSettings> twoAxesDifferentValue = makeFontVariationSettings(
        { FontVariationAxis { "a   ", 0 }, FontVariationAxis { "b   ", 1 } });

    RefPtr<FontVariationSettings> emptyVariationSettings = FontVariationSettings::create();
