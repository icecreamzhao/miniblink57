...
{
    RefPtr<scheduler::FakeWebTaskRunner> taskRunner = adoptRef(new scheduler::FakeWebTaskRunner);
...
#pragma GCC diagnostic pop
    ...
    bool isActive = false;
    handle = taskRunner->postCancellableTask(
        BLINK_FROM_HERE, WTF::bind(&getIsActive, WTF::unretained(&isActive), WTF::unretained(&handle)));
    EXPECT_TRUE(handle.isActive());
    taskRunner->runUntilIdle();
...
TEST(WebTaskRunnerTest, CancellationCheckerTest)
{
    ...
    handle = taskRunner->postCancellableTask(
        BLINK_FROM_HERE, WTF::bind(&CancellationTestHelper::incrementCounter, helper.createWeakPtr()));
