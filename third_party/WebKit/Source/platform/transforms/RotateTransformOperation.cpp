    ...
    const TransformOperation& other) const
{
    if (!isSameType(other))
        return false;
    const RotateAroundOriginTransformOperation& otherRotate = toRotateAroundOriginTransformOperation(other);
    const Rotation& otherRotation = otherRotate.m_rotation;
    return m_rotation.axis == otherRotation.axis && m_rotation.angle == otherRotation.angle && m_originX == otherRotate.m_originX && m_originY == otherRotate.m_originY;
}
    ...
    bool blendToIdentity)
{
    ...
    const RotateAroundOriginTransformOperation& fromRotate = toRotateAroundOriginTransformOperation(*from);
    return RotateAroundOriginTransformOperation::create(
