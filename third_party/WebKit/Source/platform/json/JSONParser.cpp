...
namespace {
        ...
        StringBuilder* output)
    {
        while (start < end) {
            ...
            case 'v':
                ...
                break;
            case 'u':
                c = (hexToInt(*start) << 12) + (hexToInt(*(start + 1)) << 8) + (hexToInt(*(start + 2)) << 4) + hexToInt(*(start + 3));
                start += 4;
                break;
            default:
                return false;
        ...
        int maxDepth)
    {
        ...
        case ArrayBegin: {
            ...
            while (token != ArrayEnd) {
                std::unique_ptr<JSONValue> arrayNode = buildValue(start, end, &tokenEnd, maxDepth - 1);
                if (!arrayNode)
                    return nullptr;
        ...
        case ObjectBegin: {
            ...
            while (token != ObjectEnd) {
                ...
                std::unique_ptr<JSONValue> value = buildValue(start, end, &tokenEnd, maxDepth - 1);
                if (!value)
                    return nullptr;
        ...
        int maxDepth)
    {
        ...
        const CharType* tokenEnd;
        std::unique_ptr<JSONValue> value = buildValue(start, end, &tokenEnd, maxDepth);
