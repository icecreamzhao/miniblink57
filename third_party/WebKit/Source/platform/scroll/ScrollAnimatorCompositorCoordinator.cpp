...
void ScrollAnimatorCompositorCoordinator::updateImplOnlyCompositorAnimations()
{
    ...
    CompositorAnimationHost* host = getScrollableArea()->compositorAnimationHost();
    if (layer && host) {
        ...
        if (!m_implOnlyAnimationAdjustment.isZero()) {
            host->adjustImplOnlyScrollOffsetAnimation(
                elementId, gfx::Vector2dF(m_implOnlyAnimationAdjustment.width(), m_implOnlyAnimationAdjustment.height()));
        }
