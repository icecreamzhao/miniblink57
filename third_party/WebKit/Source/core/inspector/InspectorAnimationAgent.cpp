    ...
    bool isTransition)
{
    ...
    if (isTransition) {
        ...
        Vector<RefPtr<Keyframe>> keyframes = KeyframeEffectModelBase::normalizedKeyframesForInspector(
            model->getFrames());
        if (keyframes.size() == 3) {
...
buildObjectForAnimationKeyframes(const KeyframeEffectReadOnly* effect)
{
    if (!effect || !effect->model() || !effect->model()->isKeyframeEffectModel())
        return nullptr;
    const KeyframeEffectModelBase* model = toKeyframeEffectModelBase(effect->model());
    Vector<RefPtr<Keyframe>> normalizedKeyframes = KeyframeEffectModelBase::normalizedKeyframesForInspector(
        model->getFrames());
...
{
    const Element* element = toKeyframeEffectReadOnly(animation.effect())->target();
    ...
    std::unique_ptr<protocol::Animation::AnimationEffect> animationEffectObject = buildObjectForAnimationEffect(
        toKeyframeEffectReadOnly(animation.effect()),
        animationType == AnimationType::CSSTransition);
    ...
    blink::Animation* animation)
{
    ...
    if (!m_idToAnimationClone.get(id)) {
        KeyframeEffectReadOnly* oldEffect = toKeyframeEffectReadOnly(animation->effect());
    ...
    double delay)
{
    ...
    if (type == AnimationType::CSSTransition) {
        ...
        AnimationEffectTiming* timing = toAnimationEffectTiming(animation->effect()->timing());
        UnrestrictedDoubleOrString unrestrictedDuration;
    ...
    blink::Animation::AnimationPlayState newPlayState)
{
    ...
    if ((newPlayState == blink::Animation::Running || newPlayState == blink::Animation::Finished) && !m_idToAnimation.contains(animationId))
