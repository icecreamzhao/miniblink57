...
void InspectorLogAgent::consoleMessageAdded(ConsoleMessage* message)
{
    ...
    std::unique_ptr<protocol::Log::LogEntry> entry = protocol::Log::LogEntry::create()
    ...
    std::unique_ptr<v8_inspector::protocol::Runtime::API::StackTrace> stackTrace = message->location()->buildInspectorObject();
    if (stackTrace)
    ...
    std::unique_ptr<protocol::Array<ViolationSetting>> settings)
{
    ...
    for (size_t i = 0; i < settings->length(); ++i) {
        PerformanceMonitor::Violation violation = parseViolation(settings->get(i)->getName());
...
{
    String messageText = String::format("Forced reflow while executing JavaScript took %ldms",
