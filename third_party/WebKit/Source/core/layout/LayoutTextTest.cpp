...
TEST_F(LayoutTextTest, WidthZeroFromZeroLength)
{
    ...
    ASSERT_EQ(0, getBasicText()->width(0u, 0u, LayoutUnit(), TextDirection::kLtr, false));
}

TEST_F(LayoutTextTest, WidthMaxFromZeroLength)
{
    setBasicBody(kTacoText);
    ASSERT_EQ(0, getBasicText()->width(std::numeric_limits<unsigned>::max(), 0u, LayoutUnit(), TextDirection::kLtr, false));
}
...
{
    setBasicBody(kTacoText);
    ASSERT_EQ(0, getBasicText()->width(std::numeric_limits<unsigned>::max(), std::numeric_limits<unsigned>::max(), LayoutUnit(), TextDirection::kLtr, false));
}
...
{
    setBasicBody("x");
    ASSERT_EQ(0u, getBasicText()->width(1u, 1u, LayoutUnit(), TextDirection::kLtr, false));
}
...
TEST_F(LayoutTextTest, WidthLengthBeyondLength)
{
    ...
    // Width may vary by platform and we just want to make sure it's something
    // roughly reasonable.
    const float width = getBasicText()->width(0u, 2u, LayoutUnit(), TextDirection::kLtr, false);
    ASSERT_GE(width, 4.f);
