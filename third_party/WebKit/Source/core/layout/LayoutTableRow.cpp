...
LayoutBox::PaginationBreakability LayoutTableRow::getPaginationBreakability()
    const
{
    PaginationBreakability breakability = LayoutTableBoxComponent::getPaginationBreakability();
...
bool LayoutTableRow::isFirstRowInSectionAfterHeader() const
{
    ...
    return header && table()->sectionAbove(section()) == header && header->getPaginationBreakability() != AllowAnyBreaks;
}
