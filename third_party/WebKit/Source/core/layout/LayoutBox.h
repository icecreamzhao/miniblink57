...
enum SizeType { MainOrPreferredSize,
    MinSize,
    MaxSize };
...
enum MarginDirection { BlockDirection,
    InlineDirection };
enum BackgroundRectType { BackgroundClipRect,
    BackgroundKnownOpaqueRect };

enum ShouldComputePreferred { ComputeActual,
    ComputePreferred };
...
public:
    ...
    {
        return LayoutUnit(borderLeft() + (shouldPlaceBlockDirectionScrollbarOnLogicalLeft() ? verticalScrollbarWidth() : 0));
    }
    ...
    enum PageBoundaryRule { AssociateWithFormerPage,
    ...
    {
        return hasOverflowClip() && (style()->overflowY() == EOverflow::Auto || style()->overflowY() == EOverflow::PagedY || style()->overflowY() == EOverflow::Overlay);
    }
    DISABLE_CFI_PERF bool hasAutoHorizontalScrollbar() const
    {
        return hasOverflowClip() && (style()->overflowX() == EOverflow::Auto || style()->overflowX() == EOverflow::Overlay);
    }
    ...
    {
        return scrollsOverflowX() && pixelSnappedScrollWidth() != pixelSnappedClientWidth();
    }
    bool hasScrollableOverflowY() const
    {
        return scrollsOverflowY() && pixelSnappedScrollHeight() != pixelSnappedClientHeight();
    }
    virtual bool scrollsOverflowX() const
    {
        return hasOverflowClip() && (style()->overflowX() == EOverflow::Scroll || hasAutoHorizontalScrollbar());
    }
    virtual bool scrollsOverflowY() const
    {
        return hasOverflowClip() && (style()->overflowY() == EOverflow::Scroll || hasAutoVerticalScrollbar());
    }
    ...
    {
        return !parent() || parent()->style()->getWritingMode() != style()->getWritingMode();
    }
    bool isOrthogonalWritingModeRoot() const
    {
        return parent() && parent()->isHorizontalWritingMode() != isHorizontalWritingMode();
    }
    ...
    {
        return !isInline() && !isFloatingOrOutOfFlowPositioned() && parent() && parent()->isDeprecatedFlexibleBox();
    }
    ...
    {
        return !isInline() && !isFloatingOrOutOfFlowPositioned() && parent() && parent()->isFlexibleBox();
    }
    ...
    {
        return m_overflow && !borderBoxRect().contains(m_overflow->selfVisualOverflowRect());
    }
...
{
    return breakValue == BreakColumn || breakValue == BreakLeft || breakValue == BreakPage || breakValue == BreakRecto || breakValue == BreakRight || breakValue == BreakVerso;
}
