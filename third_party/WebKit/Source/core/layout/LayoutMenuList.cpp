...
void LayoutMenuList::adjustInnerStyle()
{
    ...
    if (m_optionStyle) {
        if ((m_optionStyle->direction() != innerStyle.direction() || m_optionStyle->getUnicodeBidi() != innerStyle.getUnicodeBidi()))
...
void LayoutMenuList::updateInnerBlockHeight()
{
    ...
    DCHECK(fontData);
    m_innerBlockHeight = (fontData ? fontData->getFontMetrics().height() : 0) + m_innerBlock->borderAndPaddingHeight();
}
    ...
    const LayoutPoint& additionalOffset) const
{
    ...
    LayoutRect innerBox(
        additionalOffset + m_innerBlock->location() + LayoutSize(m_innerBlock->paddingLeft(), m_innerBlock->paddingTop()),
    ...
    LayoutUnit& maxLogicalWidth) const
{
    ...
    maxLogicalWidth = std::max(m_optionsWidth,
