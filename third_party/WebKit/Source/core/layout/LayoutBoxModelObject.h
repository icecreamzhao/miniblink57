...
enum LinePositionMode { PositionOnContainingLine,
...
public:
    ...
    {
        return computedCSSPaddingStart() + computedCSSPaddingEnd() + borderStart() + borderEnd();
    }
    LayoutUnit collapsedBorderAndCSSPaddingLogicalHeight() const
    {
        return computedCSSPaddingBefore() + computedCSSPaddingAfter() + borderBefore() + borderAfter();
    }
