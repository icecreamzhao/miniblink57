...
{
    m_targetRect = LayoutRect(toLayoutBoxModelObject(target())->borderBoundingBox());
}

void IntersectionGeometry::initializeRootRect()
{
    if (m_root->isLayoutView()) {
        m_rootRect = LayoutRect(toLayoutView(m_root)->frameView()->visibleContentRect());
        ...
        m_rootRect = LayoutRect(toLayoutBoxModelObject(m_root)->borderBoundingBox());
    }
    applyRootMargin();
}
...
void IntersectionGeometry::mapTargetRectToTargetFrameCoordinates()
{
    ...
    LayoutSize scrollPosition = LayoutSize(targetDocument.view()->getScrollOffset());
