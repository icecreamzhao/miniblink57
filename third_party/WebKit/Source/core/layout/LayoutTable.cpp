...
void LayoutTable::updateLogicalWidth()
{
    ...
    LayoutUnit containerWidthInInlineDirection = hasPerpendicularContainingBlock
    ...
    if ((styleMaxLogicalWidth.isSpecified() && !styleMaxLogicalWidth.isNegative()) || styleMaxLogicalWidth.isIntrinsic()) {
        LayoutUnit computedMaxLogicalWidth = convertStyleLogicalWidthToComputedWidth(styleMaxLogicalWidth,
            availableLogicalWidth);
        setLogicalWidth(
    ...
    if ((styleMinLogicalWidth.isSpecified() && !styleMinLogicalWidth.isNegative()) || styleMinLogicalWidth.isIntrinsic()) {
        LayoutUnit computedMinLogicalWidth = convertStyleLogicalWidthToComputedWidth(styleMinLogicalWidth,
            availableLogicalWidth);
        setLogicalWidth(
    ...
    LayoutUnit availableWidth) const
{
    ...
    if (isCSSTable && styleLogicalWidth.isSpecified() && styleLogicalWidth.isPositive() && style()->boxSizing() == EBoxSizing::kContentBox)
    ...
    const Length& styleLogicalHeight) const
{
    ...
    if (styleLogicalHeight.isFixed()) {
        ...
        // browsers allow.
        if (isHTMLTableElement(node()) || style()->boxSizing() == EBoxSizing::kBorderBox) {
...

    {
        ...
        if (LayoutTableSection* section = header()) {
            ...
            if (state.isPaginated()) {
                ...
                if (sectionLogicalHeight < section->pageLogicalHeightForOffset(section->logicalTop()) && section->getPaginationBreakability() != AllowAnyBreaks) {
        ...
        if (topSection) {
            totalSectionLogicalHeight = bottomSection->logicalBottom() - topSection->logicalTop();
        }

        if (!state.isPaginated() || !crossesPageBoundary(tableBoxLogicalTop, computedLogicalHeight)) {
...
void LayoutTable::subtractCaptionRect(LayoutRect& rect) const
{
    for (unsigned i = 0; i < m_captions.size(); i++) {
        ...
        bool captionIsBefore = (m_captions[i]->style()->captionSide() != ECaptionSide::kBottom) ^ style()->isFlippedBlocksWritingMode();
...
void LayoutTable::computePreferredLogicalWidths()
{
    ...
    if (styleToUse.logicalMinWidth().isFixed() && styleToUse.logicalMinWidth().value() > 0) {
        m_maxPreferredLogicalWidth = std::max(
            m_maxPreferredLogicalWidth, adjustContentBoxLogicalWidthForBoxSizing(styleToUse.logicalMinWidth().value()));
        m_minPreferredLogicalWidth = std::max(
            m_minPreferredLogicalWidth, adjustContentBoxLogicalWidthForBoxSizing(styleToUse.logicalMinWidth().value()));
    }
    ...
    if (styleToUse.logicalMaxWidth().isFixed()) {
        ...
        m_maxPreferredLogicalWidth = std::min(
            m_maxPreferredLogicalWidth, adjustContentBoxLogicalWidthForBoxSizing(styleToUse.logicalMaxWidth().value()));
