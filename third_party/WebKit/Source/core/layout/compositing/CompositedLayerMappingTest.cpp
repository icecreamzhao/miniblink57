...
TEST_P(CompositedLayerMappingTest, ClippingMaskLayer)
{
    ...
    GraphicsLayer* graphicsLayer = toLayoutBoxModelObject(videoElement->layoutObject())
                                       ->layer()
...
TEST_P(CompositedLayerMappingTest, InterestRectChangeOnViewportScroll)
{
    ...
    document().view()->updateAllLifecyclePhases();
    GraphicsLayer* rootScrollingLayer = document().layoutViewItem().layer()->graphicsLayerBacking();
...
TEST_P(CompositedLayerMappingTest, InterestRectChangeOnScroll)
{
    ...
    GraphicsLayer* scrollingLayer = scroller->layoutBox()->layer()->graphicsLayerBacking();
    ...
    ScrollingContentsAndForegroundLayerPaintingPhase)
{
    ...
    CompositedLayerMapping* mapping = toLayoutBlock(getLayoutObjectByElementId("container"))
                                          ->layer()
    ...
    EXPECT_EQ(
        static_cast<GraphicsLayerPaintingPhase>(
            GraphicsLayerPaintOverflowContents | GraphicsLayerPaintCompositedScroll | GraphicsLayerPaintForeground),
        mapping->scrollingContentsLayer()->paintingPhase());
...
TEST_P(CompositedLayerMappingTest, RootScrollerAncestorsNotClipped)
{
    ...
    TopDocumentRootScrollerController& rootScrollerController = document().frameHost()->globalRootScrollerController();
    ...
    CompositedLayerMapping* mapping = toLayoutBlock(getLayoutObjectByElementId("container"))
                                          ->layer()
                                          ->compositedLayerMapping();
    CompositedLayerMapping* mapping2 = toLayoutBlock(getLayoutObjectByElementId("container2"))
                                           ->layer()
                                           ->compositedLayerMapping();
    CompositedLayerMapping* mapping3 = toLayoutBlock(getLayoutObjectByElementId("container3"))
                                           ->layer()
                                           ->compositedLayerMapping();
    ...
    ScrollingLayerWithPerspectivePositionedCorrectly)
{
    ...
    CompositedLayerMapping* mapping = toLayoutBlock(getLayoutObjectByElementId("scroller"))
                                          ->layer()
                                          ->compositedLayerMapping();

    CompositedLayerMapping* mapping2 = toLayoutBlock(getLayoutObjectByElementId("scroller2"))
                                           ->layer()
                                           ->compositedLayerMapping();
...
TEST_P(CompositedLayerMappingTest, AncestorClippingMaskLayerUpdates)
{
    ...
    PaintLayer* ancestorPaintLayer = toLayoutBoxModelObject(ancestor->layoutObject())->layer();
    ASSERT_TRUE(ancestorPaintLayer);

    CompositedLayerMapping* ancestorMapping = ancestorPaintLayer->compositedLayerMapping();
    ASSERT_FALSE(ancestorMapping);

    Element* child = document().getElementById("child");
    ASSERT_TRUE(child);
    PaintLayer* childPaintLayer = toLayoutBoxModelObject(child->layoutObject())->layer();
    ...
    CompositedLayerMapping* childMapping = childPaintLayer->compositedLayerMapping();
...
TEST_P(CompositedLayerMappingTest, AncestorClippingMaskLayerSiblingUpdates)
{
    ...
    ASSERT_TRUE(child1);
    PaintLayer* child1PaintLayer = toLayoutBoxModelObject(child1->layoutObject())->layer();
    ASSERT_TRUE(child1PaintLayer);
    CompositedLayerMapping* child1Mapping = child1PaintLayer->compositedLayerMapping();
    ASSERT_FALSE(child1Mapping);

    Element* child2 = document().getElementById("child2");
    ASSERT_TRUE(child2);
    PaintLayer* child2PaintLayer = toLayoutBoxModelObject(child2->layoutObject())->layer();
    ASSERT_TRUE(child2PaintLayer);
    CompositedLayerMapping* child2Mapping = child2PaintLayer->compositedLayerMapping();
...
TEST_P(CompositedLayerMappingTest, AncestorClippingMaskLayerGrandchildUpdates)
{
    ...
    PaintLayer* grandchildPaintLayer = toLayoutBoxModelObject(grandchild->layoutObject())->layer();
    ASSERT_TRUE(grandchildPaintLayer);
    CompositedLayerMapping* grandchildMapping = grandchildPaintLayer->compositedLayerMapping();
    ...
    childMapping = childPaintLayer->compositedLayerMapping();
    ASSERT_FALSE(childMapping);
    grandchildPaintLayer = toLayoutBoxModelObject(grandchild->layoutObject())->layer();
    ASSERT_TRUE(grandchildPaintLayer);
...
TEST_P(CompositedLayerMappingTest, StickyPositionContentOffset)
{
    ...
    CompositedLayerMapping* sticky1 = toLayoutBlock(getLayoutObjectByElementId("sticky1"))
                                          ->layer()
                                          ->compositedLayerMapping();
    CompositedLayerMapping* sticky2 = toLayoutBlock(getLayoutObjectByElementId("sticky2"))
                                          ->layer()
                                          ->compositedLayerMapping();
    ...
    ASSERT_TRUE(sticky1);
    WebLayerStickyPositionConstraint constraint1 = sticky1->mainGraphicsLayer()
                                                       ->contentLayer()
                                                       ->layer()
    ...
    ASSERT_TRUE(sticky2);
    WebLayerStickyPositionConstraint constraint2 = sticky2->mainGraphicsLayer()
                                                       ->contentLayer()
                                                       ->layer()
                                                       ->stickyPositionConstraint();
...
TEST_P(CompositedLayerMappingTest, StickyPositionTableCellContentOffset)
{
    ...
    CompositedLayerMapping* sticky = toLayoutBlock(getLayoutObjectByElementId("sticky"))
                                         ->layer()
                                         ->compositedLayerMapping();

    ASSERT_TRUE(sticky);
    WebLayerStickyPositionConstraint constraint = sticky->mainGraphicsLayer()
                                                      ->contentLayer()
                                                      ->layer()
                                                      ->stickyPositionConstraint();
