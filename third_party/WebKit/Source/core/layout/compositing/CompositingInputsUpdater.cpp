    ...
    const PaintLayer* layer)
{
    ...
    while (current) {
        ...
        DCHECK(!current->hasClipRelatedProperty() || current->styleRef().containsPaint() || (current->isBox() && toLayoutBox(current)->hasControlClip()));
    }
    ASSERT_NOT_REACHED();
    return nullptr;
}
    ...
    AncestorInfo info)
{
    ...
    if (info.lastOverflowClipLayer && layer->needsCompositingInputsUpdate() && layer->layoutObject()->style()->position() == StickyPosition) {
        if (info.lastOverflowClipLayer != previousOverflowLayer && !RuntimeEnabledFeatures::rootLayerScrollingEnabled()) {
    ...
    if (updateType == ForceUpdate) {
        ...
        if (!layer->isRootLayer()) {
            if (!RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
                properties.unclippedAbsoluteBoundingBox = enclosingIntRect(m_geometryMap.absoluteRect(
                ...
                IntRect clipRect = pixelSnappedIntRect(layer->clipper()
                                                           .backgroundClipRect(ClipRectsContext(
                                                               m_rootLayer, AbsoluteClipRects))
                                                           .rect());
                properties.clippedAbsoluteBoundingBox = properties.unclippedAbsoluteBoundingBox;
            ...
            if (info.hasAncestorWithClipRelatedProperty) {
                ...
                const bool parentHasClipRelatedProperty = parentLayerOnClippingContainerChain->layoutObject()
                                                              ->hasClipRelatedProperty();
                properties.clippingContainer = parentHasClipRelatedProperty
                ...
                if (layer->layoutObject()->isOutOfFlowPositioned() && !layer->subtreeIsInvisible()) {
                    const PaintLayer* clippingLayer = properties.clippingContainer
            ...
            if (info.lastScrollingAncestor) {
                ...
                if (layer->stackingNode()->isStacked() && properties.ancestorScrollingLayer && !info.ancestorStackingContext->layoutObject()->isDescendantOf(properties.ancestorScrollingLayer->layoutObject()))
