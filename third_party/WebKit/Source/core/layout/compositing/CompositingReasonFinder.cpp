    ...
    LayoutObject* layoutObject) const
{
    ...
    DCHECK((layoutObject->isTransparent() || layoutObject->hasMask() || layoutObject->hasFilterInducingProperty() || style.hasBlendMode()) == layoutObject->createsGroup());
    ...
    const LayoutObject& layoutObject)
{
    ...
    // support them.
    return layoutObject.hasTransformRelatedProperty() && layoutObject.styleRef().has3DTransform();
}
...
{
    if (style.subtreeWillChangeContents()) {
        return style.isRunningOpacityAnimationOnCompositor() || style.isRunningFilterAnimationOnCompositor() || style.isRunningBackdropFilterAnimationOnCompositor();
    }

    return style.hasCurrentOpacityAnimation() || style.hasCurrentFilterAnimation() || style.hasCurrentBackdropFilterAnimation();
}
...
bool CompositingReasonFinder::requiresCompositingForScrollDependentPosition(
    const PaintLayer* layer) const
{
    if (layer->layoutObject()->style()->position() != FixedPosition && layer->layoutObject()->style()->position() != StickyPosition)
        return false;

    if (!(m_compositingTriggers & ViewportConstrainedPositionedTrigger) && (!RuntimeEnabledFeatures::compositeOpaqueFixedPositionEnabled() || !layer->backgroundIsKnownToBeOpaqueInRect(LayoutRect(layer->boundingBoxForCompositing())) || layer->compositesWithTransform() || layer->compositesWithOpacity())) {
        return false;
    }
