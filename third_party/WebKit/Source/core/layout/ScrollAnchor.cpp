    ...
    const ScrollableArea* scroller)
{
    ...
    if (layoutObject->isBox()) {
        ...
        if (!layoutObject->hasOverflowClip()) {
            ...
            LayoutUnit maxHeight = std::max(localBounds.height(),
...
void ScrollAnchor::notifyBeforeLayout()
{
    ...
    float blockDirectionScrollOffset = scrollerLayoutBox(m_scroller)->isHorizontalWritingMode()
        ? scrollOffset.height()
        : scrollOffset.width();
    ...
    ScrollableArea* owningScroller = m_scroller->isRootFrameViewport()
...
{
    LayoutObject* layoutObject = m_anchorObject ? m_anchorObject : scrollerLayoutBox(m_scroller);
