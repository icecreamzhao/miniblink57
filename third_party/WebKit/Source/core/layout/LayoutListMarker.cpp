...
void LayoutListMarker::computePreferredLogicalWidths()
{
    ...
    if (isImage()) {
        LayoutSize imageSize(imageBulletSize());
        m_minPreferredLogicalWidth = m_maxPreferredLogicalWidth = style()->isHorizontalWritingMode() ? imageSize.width()
                                                                                                     : imageSize.height();
    ...
    case ListStyleCategory::None:
        break;
    case ListStyleCategory::Symbol:
        logicalWidth = LayoutUnit((fontData->getFontMetrics().ascent() * 2 / 3 + 1) / 2 + 2);
        break;
    case ListStyleCategory::Language:
...
{
    return m_listItem->notInList() || style()->listStylePosition() == EListStylePosition::kInside;
}
