...
static void makeCapitalized(String* string, UChar previous)
{
    ...
    for (endOfWord = boundary->next(); endOfWord != TextBreakDone;
        ...
        if (startOfWord) // Ignore first char of previous string
            result.append((UChar32)(input[startOfWord - 1] == noBreakSpaceCharacter
    ...
    const ComputedStyle* oldStyle)
{
    ...
    ETextTransform oldTransform = oldStyle ? oldStyle->textTransform() : ETextTransform::kNone;
    ...
    ShouldAffinityBeDownstream shouldAffinityBeDownstream)
{
    ...
    int textStartOffset = box->getLineLayoutItem().isText()
    ...
    ShouldAffinityBeDownstream shouldAffinityBeDownstream)
{
    ...
    if (positionIsAtStartOfBox == box->isLeftToRightDirection()) {
        ...
        if ((prevBox && prevBox->bidiLevel() == box->bidiLevel()) || box->getLineLayoutItem().containingBlock().style()->direction() == box->direction()) // FIXME: left on 12CBA
    ...
    if ((nextBox && nextBox->bidiLevel() == box->bidiLevel()) || box->getLineLayoutItem().containingBlock().style()->direction() == box->direction())
        return createPositionWithAffinityForBox(box, box->caretRightmostOffset(),
            shouldAffinityBeDownstream);
    ...
    LayoutUnit* extraWidthToEndOfLine)
{
    ...
    if (box->getLineLayoutItem().style(box->isFirstLineStyle())->direction() == TextDirection::kLtr) {
    ...
    const ComputedStyle* styleToUse = caretBox->getLineLayoutItem().style(caretBox->isFirstLineStyle());
    ...
    FloatRect& glyphBounds)
{
    ...
    Hyphenation* hyphenation = styleToUse.autoWrap() ? styleToUse.getHyphenation() : nullptr;
    ...
    if ((is8Bit() && textDirection == TextDirection::kLtr) || isOverride(styleToUse.getUnicodeBidi())) {
        run = 0;
    } else {
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (wordLen) {
            ...
            if (isSpace && (f.getFontDescription().getTypesettingFeatures() & Kerning)) {
                const unsigned textDirectionIndex = static_cast<unsigned>(textDirection);
                DCHECK_GE(textDirectionIndex, 0U);
                DCHECK_LE(textDirectionIndex, 1U);
                if (!cachedWordTrailingSpaceWidth[textDirectionIndex])
                    cachedWordTrailingSpaceWidth[textDirectionIndex] = f.width(constructTextRun(f, &spaceCharacter, 1, styleToUse,
                                                                           textDirection))
                        + wordSpacing;
                wordTrailingSpaceWidth = cachedWordTrailingSpaceWidth[textDirectionIndex];
            }
            ...
            if (w > maxWordWidth) {
                DCHECK(hyphenation);
                int suffixStart;
                float maxFragmentWidth = maxWordFragmentWidth(this, styleToUse, f, textDirection,
                    *hyphenation, i, wordLen, suffixStart);
                if (suffixStart) {
                    float suffixWidth;
                    if (wordTrailingSpaceWidth && isSpace)
                        suffixWidth = widthFromFont(f, i + suffixStart, wordLen - suffixStart + 1,
                                          leadWidth, currMaxWidth, textDirection,
                                          &fallbackFonts, &glyphBounds)
                            - wordTrailingSpaceWidth;
                    else
        ...
        if (!style()->preserveNewline() && !from && len == textLength()) {
            if (fallbackFonts) {
                ...
                if (preferredLogicalWidthsDirty() || !m_knownToHaveNoOverflowAndNoFallbackFonts)
                    ...
                    *glyphBounds = FloatRect(0, -fontData->getFontMetrics().floatAscent(),
