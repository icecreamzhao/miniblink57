...
void LayoutObject::addChild(LayoutObject* newChild, LayoutObject* beforeChild)
{
    ...
    if (newChild->isText() && newChild->style()->textTransform() == ETextTransform::kCapitalize)
...
{
    if (hasClip() || hasOverflowClip() || hasClipPath() || style()->containsPaint())
        return true;
...
LayoutBlock* LayoutObject::containingBlock(AncestorSkipInfo* skipInfo) const
{
    ...
    if (isColumnSpanAll()) {
        ...
        while (object && ((object->isInline() && !object->isAtomicInlineLevel()) || !object->isLayoutBlock())) {
            if (skipInfo)
                skipInfo->update(*object);
            object = object->parent();
        }
...
const LayoutBoxModelObject* LayoutObject::enclosingCompositedContainer() const
{
    ...
    if (PaintLayer* paintingLayer = this->paintingLayer()) {
        if (PaintLayer* compositingLayer = paintingLayer
    ...
    VisualRectFlags visualRectFlags) const
{
    ...
    if (LayoutObject* parent = this->parent()) {
        if (parent->isBox()) {
            ...
            if (parent != ancestor && !parentBox->mapScrollingContentsRectToBoxSpace(rect, visualRectFlags))
                return false;
        }
    ...
    StyleDifference diff) const
{
    ...
    if (diff.textDecorationOrColorChanged() && !diff.needsPaintInvalidation()) {
        if (style()->hasBorder() || style()->hasOutline() || style()->hasBackgroundRelatedColorReferencingCurrentColor()
            ...
            || (isLayoutBlock() && toLayoutBlock(this)->hasCaret()) || (isSVG() && style()->svgStyle().isFillColorCurrentColor()) || (isSVG() && style()->svgStyle().isStrokeColorCurrentColor()) || isListMarker())
...
void LayoutObject::setStyle(PassRefPtr<ComputedStyle> style)
{
    ...
    StyleImage* oldBoxReflectMaskImage = oldStyle && oldStyle->boxReflect()
    ...
    if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled() && !isText() && (diff.transformChanged() || diff.opacityChanged() || diff.zIndexChanged() || diff.filterChanged() || diff.backdropFilterChanged() || diff.cssClipChanged())) {
        ...
        if (RuntimeEnabledFeatures::slimmingPaintV2Enabled() && !shouldDoFullPaintInvalidation())
    ...
    const ComputedStyle& newStyle)
{
    if (m_style) {
        ...
        bool visibilityChanged = m_style->visibility() != newStyle.visibility() || m_style->zIndex() != newStyle.zIndex() || m_style->isStackingContext() != newStyle.isStackingContext();
        ...
        s_affectsParentBlock = isFloatingOrOutOfFlowPositioned() && (!newStyle.isFloating() && !newStyle.hasOutOfFlowPosition()) && parent() && (parent()->isLayoutBlockFlow() || parent()->isLayoutInline());
    ...
    const ComputedStyle* oldStyle)
{
    ...
    if (diff.needsPaintInvalidation() && oldStyle) {
        if (resolveColor(*oldStyle, CSSPropertyBackgroundColor) != resolveColor(CSSPropertyBackgroundColor) || oldStyle->backgroundLayers() != styleRef().backgroundLayers())
...
void LayoutObject::propagateStyleToAnonymousChildren()
{
    ...
    for (LayoutObject* child = slowFirstChild(); child;
        ...
        RefPtr<ComputedStyle> newStyle = ComputedStyle::createAnonymousStyleWithDisplay(
...
{
    const WritingMode oldWritingMode = newChild->mutableStyleRef().getWritingMode();
    ...
    MapCoordinatesFlags mode) const
{
    ...
    // them.
    bool preserve3D = mode & UseTransforms && ((container->style()->preserves3D() && !container->isText()) || (style()->preserves3D() && !isText()));
    ...
    if (skipInfo.ancestorSkipped()) {
        ...
        if (ancestor->style()->position() != FixedPosition && container->isLayoutView() && styleRef().position() == FixedPosition) {
    ...
    MapCoordinatesFlags mode) const
{
    ...
    if (isLayoutFlowThread()) {
        ...
        transformState.move(
            visualPoint - toLayoutFlowThread(this)->visualPointToFlowThreadPoint(visualPoint));
    }

    if (applyContainerFlip) {
        IntPoint centerPoint = roundedIntPoint(transformState.mappedPoint());
        transformState.move(
            centerPoint - toLayoutBox(container)->flipForWritingMode(LayoutPoint(centerPoint)));
    }
    ...
    const LayoutObject* layoutObject)
{
    ...
    if (layoutObject->document().settings() && layoutObject->document().settings()->getShouldRespectImageOrientation())
        return RespectImageOrientation;
...
void LayoutObject::insertedIntoTree()
{
    ...
    // can't be used
    if (parent()->style()->visibility() != EVisibility::kVisible && style()->visibility() == EVisibility::kVisible && !hasLayer()) {
        if (!layer)
            layer = parent()->enclosingLayer();
        if (layer)
            layer->dirtyVisibleContentStatus();
...
enum FindReferencingScrollAnchorsBehavior { DontClear,
    Clear };
    ...
    ComputedStyle* style)
{
    ...
    if (layoutObjectForFirstLineStyle->behavesLikeBlockContainer()) {
        if (const LayoutBlock* firstLineBlock = toLayoutBlock(layoutObjectForFirstLineStyle)
    ...
    } else if (!layoutObjectForFirstLineStyle->isAnonymous() && layoutObjectForFirstLineStyle->isLayoutInline() && !layoutObjectForFirstLineStyle->node()->isFirstLetterPseudoElement()) {
    ...
    const ComputedStyle* ownStyle) const
{
    ...
    if (pseudoStyleRequest.pseudoId == PseudoIdFirstLineInherited) {
        RefPtr<ComputedStyle> result = document().ensureStyleResolver().styleForElement(element, parentStyle,
...
Element* LayoutObject::offsetParent(const Element* base) const
{
    ...
    for (LayoutObject* ancestor = parent(); ancestor;
        ...
        // (N and M are depths).
        if (base && (node->isClosedShadowHiddenFrom(*base) || (node->isInShadowTree() && node->containingShadowRoot()->type() == ShadowRootType::UserAgent))) {
...
#if DCHECK_IS_ON()
    DCHECK(!shouldCheckForPaintInvalidationRegardlessOfPaintInvalidationState() || paintInvalidationStateIsDirty());
#endif
...
void LayoutObject::setChildNeedsLayout(MarkingBehavior markParents,
    SubtreeLayoutScope* layouter)
{
#if DCHECK_IS_ON()
    DCHECK(!isSetNeedsLayoutForbidden());
