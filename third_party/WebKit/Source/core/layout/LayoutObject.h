...
enum CursorDirective { SetCursorBasedOnStyle,
    SetCursor,
    DoNotSetCursor };

enum HitTestFilter { HitTestAll,
    HitTestSelf,
    HitTestDescendants };
...
enum ScheduleRelayoutBehavior { ScheduleRelayout,
...
public:
    ...
    {
        return isTableCell() || isLayoutTableCol() || isTableCaption() || isTableRow() || isTableSection();
    }
    ...
    {
        return !isSVG() || (isSVGContainer() && !isSVGHiddenContainer()) || isSVGShape() || isSVGImage() || isSVGText();
    }
    ...
    bool isAnonymousBlock() const
    {
        ...
        return isAnonymous() && (style()->display() == EDisplay::Block || style()->display() == EDisplay::WebkitBox) && style()->styleType() == PseudoIdNone && isLayoutBlock() && !isListMarker() && !isLayoutFlowThread() && !isLayoutMultiColumnSet() && !isLayoutFullScreen() && !isLayoutFullScreenPlaceholder();
    }
    ...
    {
        return m_bitfields.selfNeedsLayout() || m_bitfields.normalChildNeedsLayout() || m_bitfields.posChildNeedsLayout() || m_bitfields.needsSimplifiedNormalFlowLayout() || m_bitfields.needsPositionedMovementLayout();
    }
    ...
    {
        return m_bitfields.selfNeedsOverflowRecalcAfterStyleChange() || m_bitfields.childNeedsOverflowRecalcAfterStyleChange();
    }
    ...
    public:
        AncestorSkipInfo(const LayoutObject* ancestor, bool checkForFilters = false)
            : m_ancestor(ancestor)
            , m_checkForFilters(checkForFilters)
        {
        }
    ...
    {
        return m_style->canContainAbsolutePositionObjects() || canContainFixedPositionObjects();
    }
    bool canContainFixedPositionObjects() const
    {
        return isLayoutView() || isSVGForeignObject() || (isLayoutBlock() && m_style->canContainFixedPositionObjects());
    }
   ...
   * of line -
    ...
    {
        return style()->visibility() == EVisibility::kVisible && (request.ignorePointerEventsNone() || style()->pointerEvents() != EPointerEvents::kNone) && !isInert();
    }
    ...
    {
        return isTransparent() || hasMask() || hasFilterInducingProperty() || style()->hasBlendMode();
    }
    ...
    {
        return paintInvalidationState.hasForcedSubtreeInvalidationFlags() || shouldCheckForPaintInvalidationRegardlessOfPaintInvalidationState();
    }
...
protected:
    ...
    {
        return backgroundChangedSinceLastPaintInvalidation() || shouldCheckForPaintInvalidationRegardlessOfPaintInvalidationState();
    }
#endif
...
public:                                    \
    ...
    public:
        ...
        bool isInFlowPositioned() const
        {
            return m_positionedState == IsRelativelyPositioned || m_positionedState == IsStickyPositioned;
        }
