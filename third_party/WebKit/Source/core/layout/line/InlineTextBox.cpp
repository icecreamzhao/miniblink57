...
SelectionState InlineTextBox::getSelectionState() const
{
    ...
    if (state == SelectionStart || state == SelectionEnd || state == SelectionBoth) {
        ...
        int endOfLineAdjustmentForCSSLineBreak = getLineLayoutItem().style()->getLineBreak() == LineBreakAfterWhiteSpace
            ? -1
            : 0;
        ...
        if (start && end)
            ...
            state = SelectionEnd;
        else if ((state == SelectionEnd || startPos < m_start) && (state == SelectionStart || endPos > lastSelectable))
...
bool InlineTextBox::hasWrappedSelectionNewline() const
{
    ...
    return (state == SelectionStart || state == SelectionInside)
        ...
        && ((!isLeftToRightDirection() && root().firstSelectedBox() == this) || (isLeftToRightDirection() && root().lastSelectedBox() == this));
}
...
static Color backgroundColor(LayoutObject* render, bool& isImageCleared, bool& hasBackgroundImage)
{
    if (!render)
        return Color::white;
    ...
    do {
        if (render->style()) {
            if (!isImageCleared) {
                isImageCleared = render->style()->isBackgroundImageCleared();
            }
            //style里面的hasBackgroundImage判断不准确，取到image的size，或确定image是Generated，才可以确定存在背景图片
            if ((!hasBackgroundImage) && render->mutableStyle()->accessBackgroundLayers().image()) {
                LayoutSize imgsize = render->mutableStyle()->accessBackgroundLayers().image()->imageSize(*render, 1.0f, LayoutSize());
                hasBackgroundImage = (imgsize.width() > 0) && (imgsize.height() > 0);
                if (!hasBackgroundImage)
                    hasBackgroundImage = render->mutableStyle()->accessBackgroundLayers().image()->isGeneratedImage();
            }
        }
        if (render->style() && render->style()->backgroundColorForFitScreen() != Color::transparent)
            return render->style()->backgroundColorForFitScreen();
    ...
    StringBuilder* charactersWithHyphen) const
{
    ...
    RELEASE_ASSERT(startPos <= string.length() && length <= string.length() - startPos);
