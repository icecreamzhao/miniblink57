...
LayoutUnit RootInlineBox::lineHeight() const
{
    return boxModelObject().lineHeight(
        isFirstLineStyle(), isHorizontal() ? HorizontalLine : VerticalLine,
        PositionOfInteriorLineBoxes);
}
...
LayoutUnit RootInlineBox::placeEllipsis(const AtomicString& ellipsisStr,
    bool ltr,
    ...
    LayoutUnit ellipsisWidth)
{
    ...
    EllipsisBox* ellipsisBox = new EllipsisBox(getLineLayoutItem(), ellipsisStr, this, ellipsisWidth,
    ...
    LayoutUnit position = placeEllipsisBox(ltr, blockLeftEdge, blockRightEdge, ellipsisWidth,
...
LayoutUnit RootInlineBox::placeEllipsisBox(bool ltr,
    ...
    bool& foundBox)
{
    LayoutUnit result = InlineFlowBox::placeEllipsisBox(ltr, blockLeftEdge, blockRightEdge,
...
LayoutUnit RootInlineBox::beforeAnnotationsAdjustment() const
{
    ...
    if (!getLineLayoutItem().style()->isFlippedLinesWritingMode()) {
        ...
        LayoutUnit highestAllowedPosition = prevRootBox()
...
SelectionState RootInlineBox::getSelectionState() const
{
    ...
    for (InlineBox* box = firstLeafChild(); box; box = box->nextLeafChild()) {
        ...
        if ((boxState == SelectionStart && state == SelectionEnd) || (boxState == SelectionEnd && state == SelectionStart)) {
            state = SelectionBoth;
...
LayoutUnit RootInlineBox::selectionBottom() const
{
    ...
    if (m_hasAnnotationsAfter)
        selectionBottom += !getLineLayoutItem().style()->isFlippedLinesWritingMode()
            ? computeUnderAnnotationAdjustment(m_lineBottom)
            : computeOverAnnotationAdjustment(m_lineBottom);
...
{
    return BidiStatus(
        static_cast<WTF::Unicode::CharDirection>(m_lineBreakBidiStatusEor),
    ...
    const BidiStatus& status)
{
    ...
    SECURITY_DCHECK(!obj || obj.isText() || !(obj.isLayoutInline() && obj.isBox() && !LineLayoutBox(obj).inlineBoxWrapper()));
    ...
    VerticalPositionCache& verticalPositionCache)
{
    ...
    if (verticalAlign == EVerticalAlign::Top || verticalAlign == EVerticalAlign::Bottom)
        return LayoutUnit();

    LineLayoutItem parent = boxModel.parent();
    if (parent.isLayoutInline() && parent.style()->verticalAlign() != EVerticalAlign::Top && parent.style()->verticalAlign() != EVerticalAlign::Bottom)
    ...
    if (verticalAlign != EVerticalAlign::Baseline) {
        ...
        if (verticalAlign == EVerticalAlign::Sub) {
            ...
            verticalPosition = LayoutUnit(
                (verticalPosition - LayoutUnit(fontMetrics.xHeight() / 2) - boxModel.lineHeight(firstLine, lineDirection) / 2 + boxModel.baselinePosition(baselineType(), firstLine, lineDirection))
                    .round());
            ...
            verticalPosition -= valueForLength(
                boxModel.style()->getVerticalAlignLength(), lineHeight);
        }
    }

    // Store the cached value.
    if (isLayoutInline && !firstLine)
        verticalPositionCache.set(boxModel, baselineType(),
