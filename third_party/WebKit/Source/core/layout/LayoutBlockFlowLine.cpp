    ...
    InlineBox* childBox)
{
    ...
    do {
        SECURITY_DCHECK(lineLayoutItem.isLayoutInline() || lineLayoutItem.isEqual(this));
    ...
    const LineInfo& lineInfo)
{
    ...
    // the line.
    bool isLogicallyLastRunWrapped = bidiRuns.logicallyLastRun()->m_lineLayoutItem && bidiRuns.logicallyLastRun()->m_lineLayoutItem.isText()
    ...
    if (isLeftToRightDirection) {
        if (totalLogicalWidth > availableLogicalWidth && trailingSpaceRun)
            trailingSpaceRun->m_box->setLogicalWidth(std::max(
                LayoutUnit(), trailingSpaceRun->m_box->logicalWidth() - totalLogicalWidth + availableLogicalWidth));
        return;
    }
    ...
    LayoutUnit availableLogicalWidth)
{
    ...
    if (totalLogicalWidth > availableLogicalWidth && trailingSpaceRun) {
        trailingSpaceRun->m_box->setLogicalWidth(
            std::max(LayoutUnit(), trailingSpaceRun->m_box->logicalWidth() - totalLogicalWidth + availableLogicalWidth));
        totalLogicalWidth -= trailingSpaceRun->m_box->logicalWidth();
    } else {
        logicalLeft += availableLogicalWidth - totalLogicalWidth;
    }
    ...
    LayoutUnit availableLogicalWidth)
{
    ...
    if (trailingSpaceRun) {
        totalLogicalWidth -= trailingSpaceRun->m_box->logicalWidth();
        trailingSpaceWidth = std::min(trailingSpaceRun->m_box->logicalWidth(),
    ...
    if (isLeftToRightDirection)
        ...
        logicalLeft += totalLogicalWidth > availableLogicalWidth
            ...
            : (availableLogicalWidth - totalLogicalWidth) / 2 - trailingSpaceWidth;
}
    ...
    size_t& wordMeasurementsIndex)
{
    ...
    if (canUseCachedWordMeasurements) {
        ...
        for (size_t size = wordMeasurements.size();
            ...
            if (kerningIsEnabled && lastEndOffset == run->m_stop) {
                ...
                measuredWidth += layoutText.width(wordMeasurement.startOffset, wordLength, xPos,
            ...
            if (!wordMeasurement.fallbackFonts.isEmpty()) {
                ...
                for (HashSet<const SimpleFontData*>::const_iterator it = wordMeasurement.fallbackFonts.begin();
                     it != end; ++it)
    ...
    if (!fallbackFonts.isEmpty()) {
        ...
        GlyphOverflowAndFallbackFontsMap::ValueType* it = textBoxDataMap
    ...
    unsigned expansionOpportunityCount)
{
    ...
    if (rootInlineBox && rootInlineBox->getLineLayoutItem().style()->getUnicodeBidi() == UnicodeBidi::kPlaintext)
    ...
    const WordMeasurements& wordMeasurements)
{
    ...
    for (; r; r = r->next()) {
        ...
        if (r->m_lineLayoutItem.isText()) {
            ...
            if (textAlign == ETextAlign::kJustify && r != trailingSpaceRun && textJustify != TextJustifyNone) {
                if (!isAfterExpansion)
    ...
    const BidiStatus& cleanLineBidiStatus)
{
    ...
    while (!endOfLine.atEnd()) {
        ...
        if (layoutState.lineInfo().isEmpty()) {
            ...
            VisualDirectionOverride override = (styleToUse.rtlOrdering() == EOrder::kVisual
                    ? (styleToUse.direction() == TextDirection::kLtr
                            ? VisualLeftToRightOverride
                            : VisualRightToLeftOverride)
                    : NoVisualOverride);
            if (isNewUBAParagraph && styleToUse.getUnicodeBidi() == UnicodeBidi::kPlaintext && !resolver.context()->parent()) {
                    ...
                    if (paginationStrutFromDeletedLine) {
                        ...
                        if (adjustment) {
                            LayoutUnit oldLineWidth = availableLogicalWidthForLine(
                                oldLogicalHeight, layoutState.lineInfo().isFirstLine() ? IndentText : DoNotIndentText);
            ...
            for (RootInlineBox* line = layoutState.endLine(); line;
                ...
                if (Vector<LayoutBox*>* cleanLineFloats = line->floatsPtr()) {
                    for (auto* box : *cleanLineFloats) {
                        ...
                        LayoutUnit logicalTop = logicalTopForChild(*box) - marginBeforeForChild(*box) + delta;
    ...
    bool endOfInline)
{
    ...
    if (endOfInline) {
        return getBPMWidth(child.marginEnd(), childStyle.marginEnd()) + getBPMWidth(child.paddingEnd(), childStyle.paddingEnd()) + child.borderEnd();
    }
    return getBPMWidth(child.marginStart(), childStyle.marginStart()) + getBPMWidth(child.paddingStart(), childStyle.paddingStart()) + child.borderStart();
}
    ...
    LayoutObject* trailingSpaceChild)
{
    if (trailingSpaceChild && trailingSpaceChild->isText()) {
        ...
        TextRun run = constructTextRun(font, &trailingWhitespaceChar, 1, text->styleRef(),
    ...
    while (LayoutObject* child = childIterator.next()) {
        ...
        if (!child->isBR()) {
            ...
            if (!child->isLayoutInline() && !child->isText()) {
                ...
                if (child->isFloating()) {
                    const ComputedStyle& childStyle = child->styleRef();
                    clearPreviousFloat = (prevFloat && ((prevFloat->styleRef().floating() == EFloat::kLeft && (childStyle.clear() & ClearLeft)) || (prevFloat->styleRef().floating() == EFloat::kRight && (childStyle.clear() & ClearRight))));
                    prevFloat = child;
                } else {
    ...
    LayoutUnit afterEdge)
{
    ...
    if (lastRootBox()) {
        ...
        if (!style()->isFlippedLinesWritingMode())
            lastLineAnnotationsAdjustment = lastRootBox()
        ...
        while (line != curr) {
            if (Vector<LayoutBox*>* cleanLineFloats = line->floatsPtr()) {
                for (auto* box : *cleanLineFloats) {
                    ...
                    ASSERT(!floatingObject->originatingLine());
                    floatingObject->setOriginatingLine(line);
                    LayoutUnit logicalTop = logicalTopForChild(*box) - marginBeforeForChild(*box);
                    placeNewFloats(logicalTop);
    ...
    BidiStatus& cleanLineBidiStatus)
{
    ...
    cleanLineStart = InlineIterator(LineLayoutItem(this), LineLayoutItem(prev->lineBreakObj()),
        prev->lineBreakPos());
...
void LayoutBlockFlow::checkLinesForTextOverflow()
{
    ...
    if (!ellipsisWidth) {
        ...
        if (font.primaryFont()->glyphForCharacter(horizontalEllipsisCharacter)) {
            ...
            ellipsisWidth = font.width(constructTextRun(font, &horizontalEllipsisCharacter, 1,
