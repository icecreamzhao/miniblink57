    ...
    const ComputedStyle* oldStyle)
{
    ...
    bool newPreserves = style() ? style()->whiteSpace() == EWhiteSpace::kPre : false;
    bool oldPreserves = oldStyle ? oldStyle->whiteSpace() == EWhiteSpace::kPre : false;
    ...
    const LayoutPoint& point)
{
    ...
    DCHECK(fontData);
    float baseline = fontData ? fontData->getFontMetrics().floatAscent() / m_scalingFactor : 0;
    ...
    bool& lastCharacterWasWhiteSpace)
{
    ...
    const float cachedFontHeight = fontData->getFontMetrics().floatHeight() / m_scalingFactor;
