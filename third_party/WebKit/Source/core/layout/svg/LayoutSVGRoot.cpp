...
bool LayoutSVGRoot::isEmbeddedThroughFrameContainingSVGDocument() const
{
    ...
    if (frame->ownerLayoutItem().isNull() || !frame->ownerLayoutItem().isEmbeddedObject())
        return false;
...
bool LayoutSVGRoot::shouldApplyViewportClip() const
{
    ...
    return style()->overflowX() == EOverflow::Hidden || style()->overflowX() == EOverflow::Auto || style()->overflowX() == EOverflow::Scroll || this->isDocumentElement();
}
...
LayoutRect LayoutSVGRoot::localVisualRect() const
{
    ...
    if (style()->visibility() != EVisibility::kVisible && !enclosingLayer()->hasVisibleContent())
        return LayoutRect();
    ...
    if (m_hasBoxDecorationBackground || hasOverflowModel()) {
        ...
        LayoutRect decoratedVisualRect = unionRect(localSelectionRect(), visualOverflowRect());
            ...
            for (LayoutObject* child = lastChild(); child;
                ...
                if (child->nodeAtFloatPoint(result, localPoint, hitTestAction)) {
                    ...
                    if (result.addNodeToListBasedTestResult(
                            child->node(), locationInContainer)
                        == StopHitTesting)
                        return true;
                }
            }
