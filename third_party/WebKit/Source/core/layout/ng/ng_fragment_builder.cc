    ...
    const NGLogicalOffset& child_offset)
{
    ...
    const Vector<NGStaticPosition>& oof_positions = physical_fragment->OutOfFlowPositions();
    size_t oof_index = 0;
    ...
    NGLogicalOffset child_offset)
{
    ...
    NGStaticPosition child_position = NGStaticPosition::Create(writing_mode_, direction_, NGPhysicalOffset());
    out_of_flow_candidate_placements_.push_back(
    ...
    Vector<NGStaticPosition>* descendant_positions)
{
    ...
    for (auto& oof_node : out_of_flow_descendant_candidates_) {
        OutOfFlowPlacement oof_placement = out_of_flow_candidate_placements_[placement_index++];

        NGPhysicalOffset child_offset = oof_placement.child_offset.ConvertToPhysical(
        ...
        builder_relative_position.offset = child_offset + oof_placement.descendant_position.offset;
