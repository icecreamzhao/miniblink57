...
namespace {
    ...
    TEST(NGUnitsTest, ConvertPhysicalStrutToLogical)
    {
        ...
        NGBoxStrut logical = physical.ConvertToLogical(kHorizontalTopBottom, TextDirection::kLtr);
        EXPECT_EQ(left, logical.inline_start);
        EXPECT_EQ(top, logical.block_start);

        logical = physical.ConvertToLogical(kHorizontalTopBottom, TextDirection::kRtl);
