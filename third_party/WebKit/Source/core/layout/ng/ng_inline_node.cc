...
void NGInlineNode::CollectInlines(LayoutObject* start, LayoutObject* last)
{
    ...
    is_bidi_enabled_ = !text_content_.isEmpty() && !(text_content_.is8Bit() && !builder.HasBidiControls());
}
...
void NGInlineNode::SegmentText()
{
    ...
    for (unsigned start = 0; start < text_content_.length();) {
        ...
        item_index = NGLayoutInlineItem::SetBidiLevel(items_, item_index, end, level);
        start = end;
    }
    ...
    NGLineBuilder* line_builder)
{
    ...
    // inline.
    NGConstraintSpace* child_constraint_space = NGConstraintSpaceBuilder(constraint_space->WritingMode())
...
{
    if (!next_sibling_) {
        LayoutObject* next_sibling = last_inline_ ? last_inline_->nextSibling() : nullptr;
    ...
    Vector<unsigned, 32>* text_offsets_out)
{
    ...
    for (unsigned i = 0; i < items_.size(); i++) {
        ...
        if (next_text != current_text) {
            if (current_text && current_text->textLength() != item.StartOffset() - current_offset) {
    ...
    if (current_text && current_text->textLength() != text_content_.length() - current_offset) {
