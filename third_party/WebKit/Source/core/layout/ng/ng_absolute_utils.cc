...
namespace {
    ...
    bool AbsoluteHorizontalNeedsEstimate(const ComputedStyle& style)
    {
        ...
        return width.isIntrinsic() || (width.isAuto() && (style.left().isAuto() || style.right().isAuto()));
    }

    bool AbsoluteVerticalNeedsEstimate(const ComputedStyle& style)
    {
        Length height = style.height();
        return height.isIntrinsic() || (height.isAuto() && (style.top().isAuto() || style.bottom().isAuto()));
    }
        ...
        NGAbsolutePhysicalPosition* position)
    {
        ...
        NGPhysicalSize percentage_physical = percentage_logical.ConvertToPhysical(space.WritingMode());
        LayoutUnit border_left(style.borderLeftWidth());
        LayoutUnit border_right(style.borderRightWidth());
        LayoutUnit padding_left = valueForLength(style.paddingLeft(), percentage_logical.inline_size);
        LayoutUnit padding_right = valueForLength(style.paddingRight(), percentage_logical.inline_size);
        Optional<LayoutUnit> margin_left;
        if (!style.marginLeft().isAuto())
            margin_left = valueForLength(style.marginLeft(), percentage_logical.inline_size);
        Optional<LayoutUnit> margin_right;
        if (!style.marginRight().isAuto())
            margin_right = valueForLength(style.marginRight(), percentage_logical.inline_size);
        ...
        LayoutUnit border_padding = border_left + border_right + padding_left + padding_right;
        ...
        if (!style.width().isAuto()) {
            if (space.WritingMode() == kHorizontalTopBottom) {
                ...
                LayoutUnit computed_width = child_minmax.has_value() ? child_minmax->max_content : LayoutUnit();
                width = ResolveBlockLength(space, style, style.width(), computed_width,
                    LengthResolveType::kContentSize);
            }
        }

        NGPhysicalSize container_size = space.AvailableSize().ConvertToPhysical(space.WritingMode());
        ...
        NGAbsolutePhysicalPosition* position)
    {
        ...
        LayoutUnit padding_top = valueForLength(style.paddingTop(), percentage_logical.inline_size);
        LayoutUnit padding_bottom = valueForLength(style.paddingBottom(), percentage_logical.inline_size);
        Optional<LayoutUnit> margin_top;
        if (!style.marginTop().isAuto())
            margin_top = valueForLength(style.marginTop(), percentage_logical.inline_size);
        Optional<LayoutUnit> margin_bottom;
        if (!style.marginBottom().isAuto())
            margin_bottom = valueForLength(style.marginBottom(), percentage_logical.inline_size);
        ...
        LayoutUnit border_padding = border_top + border_bottom + padding_top + padding_bottom;

        Optional<LayoutUnit> height;
        if (!style.height().isAuto()) {
            if (space.WritingMode() == kHorizontalTopBottom) {
                LayoutUnit computed_height = child_minmax.has_value() ? child_minmax->max_content : LayoutUnit();
    ...
    const ComputedStyle& child_style)
{
    ...
    bool contains_absolute = container_style.canContainAbsolutePositionObjects() || contains_fixed;

    return (contains_absolute && position == AbsolutePosition) || (contains_fixed && position == FixedPosition);
}
