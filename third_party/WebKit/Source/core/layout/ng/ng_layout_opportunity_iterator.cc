...
namespace {
        ...
        const NGLogicalRect& exclusion)
    {
        ...
        LayoutUnit bottom_opportunity_block_size = parent_opportunity.BlockEndOffset() - exclusion.BlockEndOffset();
        ...
        const NGLogicalRect& exclusion)
    {
        ...
        LayoutUnit left_opportunity_inline_size = exclusion.InlineStartOffset() - parent_opportunity.InlineStartOffset();
        NGEdge node_edge = { /* start */ parent_opportunity.InlineStartOffset(),
            /* end */ exclusion.InlineStartOffset() };

        if (left_opportunity_inline_size > 0 && IsOverlapping(parent_node->exclusion_edge, node_edge)) {
            NGLayoutOpportunity opportunity;
            opportunity.offset.inline_offset = parent_opportunity.InlineStartOffset();
        ...
        const NGLogicalRect& exclusion)
    {
        ...
        LayoutUnit right_opportunity_inline_size = parent_opportunity.InlineEndOffset() - exclusion.InlineEndOffset();
        if (right_opportunity_inline_size > 0 && IsOverlapping(parent_node->exclusion_edge, node_edge)) {
            NGLayoutOpportunity opportunity;
    ...
    NGLayoutOpportunity GetTopSpace(const NGLayoutOpportunity& parent_opportunity,
        const NGLogicalRect& exclusion)
    {
        LayoutUnit top_opportunity_block_size = exclusion.BlockStartOffset() - parent_opportunity.BlockStartOffset();
        ...
        NGLayoutOpportunities& opportunities)
    {
        ...
        NGLayoutOpportunity top_layout_opp = GetTopSpace(node->opportunity, exclusion->rect);
    ...
    {
        LayoutUnit inline_size = leader_point.inline_offset - origin_point.inline_offset;
    ...
    : constraint_space_(space)
{
    ...
    NGLogicalOffset origin_point = opt_origin_point ? opt_origin_point.value() : NGLogicalOffset();
    NGLayoutOpportunity initial_opportunity = CreateLayoutOpportunityFromConstraintSpace(*space, origin_point);
    opportunity_tree_root_ = new NGLayoutOpportunityTreeNode(initial_opportunity);

    if (opt_leader_point) {
        const NGExclusion leader_exclusion = ToLeaderExclusion(origin_point, opt_leader_point.value());
