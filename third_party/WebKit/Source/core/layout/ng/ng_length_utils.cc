    ...
    const ComputedStyle& style)
{
    ...
    // correctness.
    return constraint_space.IsShrinkToFit() || style.logicalWidth().isIntrinsic() || style.logicalMinWidth().isIntrinsic() || style.logicalMaxWidth().isIntrinsic();
}

bool NeedMinAndMaxContentSizesForContentContribution(
    const ComputedStyle& style)
{
    return style.logicalWidth().isIntrinsicOrAuto() || style.logicalMinWidth().isIntrinsic() || style.logicalMaxWidth().isIntrinsic();
}
    ...
    LengthResolveType type)
{
    ...
    if (type != LengthResolveType::kMarginBorderPaddingSize) {
        border_and_padding = ComputeBorders(style) + ComputePadding(constraint_space, style);
    }
    switch (length.type()) {
    case Auto:
    case FillAvailable: {
    ...
    case Fixed:
    case Calculated: {
        LayoutUnit percentage_resolution_size = constraint_space.PercentageResolutionSize().inline_size;
    ...
    case FitContent: {
        ...
        if (length.isMinContent()) {
            ...
            NGBoxStrut margins = ComputeMargins(
                constraint_space, style,
                FromPlatformWritingMode(style.getWritingMode()), style.direction());
            LayoutUnit fill_available = std::max(LayoutUnit(), available_size - margins.InlineSum() - border_and_padding.InlineSum());
    ...
    LengthResolveType type)
{
    ...
    if (length.isPercentOrCalc() && constraint_space.PercentageResolutionSize().block_size == NGSizeIndefinite)
        return content_size;
    ...
    case Fixed:
    case Calculated: {
        LayoutUnit percentage_resolution_size = constraint_space.PercentageResolutionSize().block_size;
        LayoutUnit value = valueForLength(length, percentage_resolution_size);
        if (style.boxSizing() == EBoxSizing::kContentBox) {
    ...
    const WTF::Optional<MinAndMaxContentSizes>& min_and_max)
{
    ...
    if (inline_size.isAuto()) {
        ...
        NGBoxStrut border_and_padding = ComputeBorders(style) + ComputePadding(*space, style);
        computed_sizes.min_content = min_and_max->min_content + border_and_padding.InlineSum();
        computed_sizes.max_content = min_and_max->max_content + border_and_padding.InlineSum();
    } else {
        computed_sizes.min_content = computed_sizes.max_content = ResolveInlineLength(*space, style, min_and_max, inline_size,
    ...
    LayoutUnit min = ResolveInlineLength(*space, style, min_and_max, style.logicalMinWidth(),
    ...
    NGBoxStrut margins = ComputeMargins(*space, style, writing_mode, style.direction());
    ...
    LayoutUnit content_size)
{
    ...
    LayoutUnit extent = ResolveBlockLength(constraint_space, style, style.logicalHeight(),
    ...
    if (!max_length.isMaxSizeNone()) {
        LayoutUnit max = ResolveBlockLength(constraint_space, style, max_length, content_size,
            LengthResolveType::kMaxSize);
        extent = std::min(extent, max);
    }

    LayoutUnit min = ResolveBlockLength(constraint_space, style, style.logicalMinHeight(),
    ...
    LayoutUnit available_size)
{
    ...
    int count_from_width = ((available_size + used_gap) / (computed_size + used_gap)).toInt();
    ...
    const ComputedStyle& style)
{
    ...
    LayoutUnit computed_size = style.hasAutoColumnWidth()
        ? NGSizeIndefinite
    ...
    NGBoxStrut* margins)
{
    ...
    const LayoutUnit available_space = constraint_space.AvailableSize().inline_size - used_space;
