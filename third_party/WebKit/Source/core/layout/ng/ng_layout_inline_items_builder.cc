...
static bool IsAmbiguosEastAsianWidthWide(const ComputedStyle* style)
{
    ...
    return script == USCRIPT_KATAKANA_OR_HIRAGANA || script == USCRIPT_SIMPLIFIED_HAN || script == USCRIPT_TRADITIONAL_HAN;
}
    ...
    const ComputedStyle* after_style)
{
    ...
    return (!before.is8Bit() || !after.is8Bit()) && ShouldRemoveNewlineSlow(before, before_style, after, after_index, after_style);
}
    ...
    LayoutObject* layout_object)
{
    ...
    bool preserve_newline = ComputedStyle::preserveNewline(whitespace) && !is_svgtext_;
...
void NGLayoutInlineItemsBuilder::Append(UChar character,
    ...
    LayoutObject* layout_object)
{
    DCHECK(character != spaceCharacter && character != tabulationCharacter && character != newlineCharacter && character != zeroWidthSpaceCharacter);
    if (has_pending_newline_)
        ProcessPendingNewline(emptyString(), nullptr);
...
void NGLayoutInlineItemsBuilder::EnterBlock(const ComputedStyle* style)
{
    ...
    if (style->getUnicodeBidi() == UnicodeBidi::kBidiOverride || style->getUnicodeBidi() == UnicodeBidi::kIsolateOverride) {
