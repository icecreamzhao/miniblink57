...
bool NGBlockNode::ComputeMinAndMaxContentSizes(MinAndMaxContentSizes* sizes)
{
    ...
    NGConstraintSpace* constraint_space = NGConstraintSpaceBuilder(
    ...
    NGLayoutCoordinator* minmax_coordinator = new NGLayoutCoordinator(this, constraint_space);
    ...
    constraint_space = NGConstraintSpaceBuilder(
        FromPlatformWritingMode(Style()->getWritingMode()))
                           .SetTextDirection(Style()->direction())
...
{
    if (!next_sibling_) {
        LayoutObject* next_sibling = layout_box_ ? layout_box_->nextSibling() : nullptr;
...
bool NGBlockNode::CanUseNewLayout()
{
    ...
    if (!layout_box_->isLayoutBlockFlow())
        return false;
    return RuntimeEnabledFeatures::layoutNGInlineEnabled() || !HasInlineChildren();
}
    ...
    const NGConstraintSpace& constraint_space)
{
    ...
    NGBoxStrut border_and_padding = ComputeBorders(*Style()) + ComputePadding(constraint_space, *Style());
    LayoutUnit intrinsic_logical_height = layout_box_->style()->isHorizontalWritingMode()
    ...
    const NGConstraintSpace& constraint_space)
{
    ...
    if (constraint_space.IsFixedSizeInline()) {
        layout_box_->setOverrideLogicalContentWidth(
            constraint_space.AvailableSize().inline_size - layout_box_->borderAndPaddingLogicalWidth());
    }
    if (constraint_space.IsFixedSizeBlock()) {
        layout_box_->setOverrideLogicalContentHeight(
            constraint_space.AvailableSize().block_size - layout_box_->borderAndPaddingLogicalHeight());
    }
