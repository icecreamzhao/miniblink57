...
namespace {
        ...
        LayoutUnit* content_size)
    {
        ...
        LayoutUnit left_block_end_offset = left_exclusion ? left_exclusion->rect.BlockEndOffset() : *content_size;
        LayoutUnit right_block_end_offset = right_exclusion ? right_exclusion->rect.BlockEndOffset() : *content_size;
        ...
        const ComputedStyle& style)
    {
        ...
        if (style.specifiesColumns() || style.containsPaint() || style.containsLayout())
            return true;
        ...
        if (display == EDisplay::Grid || display == EDisplay::Flex || display == EDisplay::WebkitBox)
            return true;
    ...
    MinAndMaxContentSizes* sizes)
{
    ...
    for (NGBlockNode* node = first_child_; node; node = node->NextSibling()) {
        ...
        MinAndMaxContentSizes child_sizes = ComputeMinAndMaxContentContribution(*node->Style(), child_minmax);
    ...
    NGLayoutAlgorithm** algorithm_out)
{
    ...
    border_and_padding_ = ComputeBorders(Style()) + ComputePadding(ConstraintSpace(), Style());

    LayoutUnit inline_size = ComputeInlineSizeForFragment(ConstraintSpace(), Style(), sizes);
    LayoutUnit adjusted_inline_size = inline_size - border_and_padding_.InlineSum();
    // TODO(layout-ng): For quirks mode, should we pass blockSize instead of
    // -1?
    LayoutUnit block_size = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), NGSizeIndefinite);
    ...
    if (Style().specifiesColumns()) {
        ...
        adjusted_inline_size = ResolveUsedColumnInlineSize(adjusted_inline_size, Style());
        LayoutUnit inline_progression = adjusted_inline_size + ResolveUsedColumnGap(Style());
        fragmentainer_mapper_ = new NGColumnMapper(inline_progression, adjusted_block_size);
    }
    ...
    while (current_child_) {
        ...
        DCHECK(!ConstraintSpace().HasBlockFragmentation() || SpaceAvailableForCurrentChild() > LayoutUnit());
    ...
    block_size = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), content_size_);
...
{
    LayoutUnit block_size = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), content_size_);
...
LayoutUnit NGBlockLayoutAlgorithm::NextBreakOffset() const
{
    ...
    return PreviousBreakOffset() + ConstraintSpace().FragmentainerSpaceAvailable();
}
    ...
    const NGBoxFragment& fragment)
{
    ...
    NGMarginStrut curr_margin_strut = is_zero_height_box ? prev_child_margin_strut_ : fragment.MarginStrut();

    // Calculate borders and padding for the current child.
    NGBoxStrut border_and_padding = ComputeBorders(CurrentChildStyle()) + ComputePadding(ConstraintSpace(), CurrentChildStyle());
    ...
    if (CurrentChildStyle().logicalHeight().isAuto() && !border_and_padding.block_end) {
...
{
    const NGBoxStrut collapsed_margins = CollapseMargins(child_margins, toNGBoxFragment(fragment));

    AdjustToClearance(ConstraintSpace(), CurrentChildStyle(), &content_size_);

    LayoutUnit inline_offset = border_and_padding_.inline_start + child_margins.inline_start;
    ...
    max_inline_size_ = std::max(
        max_inline_size_, fragment.InlineSize() + child_margins.InlineSum() + border_and_padding_.InlineSum());
    return NGLogicalOffset(inline_offset, block_offset);
}
...
NGBlockLayoutAlgorithm::CreateConstraintSpaceForCurrentChild() const
{
    ...
    bool shrink_to_fit = CurrentChildStyle().display() == EDisplay::InlineBlock || CurrentChildStyle().isFloating();
    DCHECK(current_child_);
    space_builder_
