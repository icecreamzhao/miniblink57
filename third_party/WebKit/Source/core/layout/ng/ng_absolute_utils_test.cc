...
#define NGAuto LayoutUnit(-1)
    ...
    TEST_F(NGAbsoluteUtilsTest, Horizontal)
    {
        ...
        LayoutUnit width = container_size_.inline_size - left - margin_left - right - margin_right;
        ...
        p = ComputePartialAbsoluteWithChildInlineSize(
            *ltr_space_, *style_, static_position, estimated_inline);
        LayoutUnit marginSpace = (container_size_.inline_size - left - right - p.size.width) / 2;
        ...
        SetHorizontalStyle(NGAuto, margin_left, width - border_left - border_right - padding_left - padding_right,
            margin_right, right);
        EXPECT_EQ(AbsoluteNeedsChildInlineSize(*style_), false);
        estimated_inline.reset();
        p = ComputePartialAbsoluteWithChildInlineSize(
    ...
    TEST_F(NGAbsoluteUtilsTest, Vertical)
    {
        ...
        LayoutUnit height = container_size_.block_size - top - margin_top - bottom - margin_bottom;
        ...
        ComputeFullAbsoluteWithChildBlockSize(*ltr_space_, *style_, static_position,
            auto_height, &p);
        LayoutUnit margin_space = (container_size_.block_size - top - height - bottom) / 2;
