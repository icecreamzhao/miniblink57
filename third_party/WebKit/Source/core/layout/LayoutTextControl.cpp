    ...
    LogicalExtentComputedValues& computedValues) const
{
    ...
    if (LayoutBox* innerEditorBox = innerEditor->layoutBox()) {
        ...
        if (style()->overflowInlineDirection() == EOverflow::Scroll || (style()->overflowInlineDirection() == EOverflow::Auto && innerEditor->layoutObject()->style()->overflowWrap() == NormalOverflowWrap))
    ...
    const AtomicString& family)
{
    ...
    if (metrics.hasZeroWidth() && fontData->avgCharWidth() > metrics.zeroWidth() * 1.7)
        return false;
...
int LayoutTextControl::firstLineBoxBaseline() const
{
    ...
    LayoutBlock* innerEditorLayoutObject = toLayoutBlock(innerEditor->layoutObject());
    const SimpleFontData* fontData = innerEditorLayoutObject->style(true)->font().primaryFont();
    DCHECK(fontData);
    if (!fontData)
        return -1;
