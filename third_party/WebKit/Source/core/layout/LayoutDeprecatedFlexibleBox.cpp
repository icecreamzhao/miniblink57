...
public:
    ...
    LayoutBox* next()
    {
        do {
            if (!m_currentChild) {
                ...
                if (!m_ordinalIteration) {
                    ...
                    m_currentOrdinal = m_forward ? m_sortedOrdinalValues[m_ordinalIteration - 1]
                                                 : m_sortedOrdinalValues[m_sortedOrdinalValues.size() - m_ordinalIteration];
                }
        ...
        } while (!m_currentChild || (!m_currentChild->isAnonymous() && m_currentChild->style()->boxOrdinalGroup() != m_currentOrdinal));
...
static bool childDoesNotAffectWidthOrFlexing(LayoutObject* child)
{
    ...
    return child->isOutOfFlowPositioned() || child->style()->visibility() == EVisibility::kCollapse;
}
...
void LayoutDeprecatedFlexibleBox::layoutBlock(bool relayoutChildren)
{
...

    {
        ...
        if (previousSize != size() || (parent()->isDeprecatedFlexibleBox() && parent()->style()->boxOrient() == HORIZONTAL && parent()->style()->boxAlign() == BSTRETCH))
...
void LayoutDeprecatedFlexibleBox::layoutHorizontalBox(bool relayoutChildren)
{
            ...
            for (unsigned i = start; i <= end && remainingSpace; i++) {
                ...
                do {
                    ...
                    for (LayoutBox* child = iterator.first(); child;
                        ...
                        if (allowedFlex) {
                            LayoutUnit projectedFlex = (allowedFlex == LayoutUnit::max())
                                ? allowedFlex
                                : LayoutUnit(allowedFlex * (totalFlex / child->style()->boxFlex()));
                            spaceAvailableThisPass = expanding ? std::min(spaceAvailableThisPass, projectedFlex)
    ...
    if (remainingSpace > 0 && ((style()->isLeftToRightDirection() && style()->boxPack() != BoxPackStart) || (!style()->isLeftToRightDirection() && style()->boxPack() != BoxPackEnd))) {
                        ...
                        if (allowedFlex) {
                            LayoutUnit projectedFlex = (allowedFlex == LayoutUnit::max())
                                ...
                                : static_cast<LayoutUnit>(
                                    allowedFlex * (totalFlex / child->style()->boxFlex()));
                            spaceAvailableThisPass = expanding ? std::min(spaceAvailableThisPass, projectedFlex)
                                                               : std::max(spaceAvailableThisPass, projectedFlex);
                        }
                    }
                    ...
                    for (LayoutBox* child = iterator.first();
                        ...
                        if (allowedChildFlex(child, expanding, i)) {
                            LayoutUnit spaceAdd = static_cast<LayoutUnit>(
                                spaceAvailableThisPass * (child->style()->boxFlex() / totalFlex));
                            if (spaceAdd) {
    ...
    bool relayoutChildren)
{
    ...
    for (LayoutBox* child = iterator.first(); child; child = iterator.next()) {
        ...
        child->clearOverrideSize();
        if (relayoutChildren || (child->isAtomicInlineLevel() && (child->style()->width().isPercentOrCalc() || child->style()->height().isPercentOrCalc())) || (child->style()->height().isAuto() && child->isLayoutBlock())) {
            child->setChildNeedsLayout(MarkOnlyThis);
    ...
    int numVisibleLines = lineClamp.isPercentage()
    ...
    for (LayoutBox* child = iterator.first(); child; child = iterator.next()) {
        ...
        float totalWidth = font.width(constructTextRun(font, &horizontalEllipsisCharacter, 1,
...
void LayoutDeprecatedFlexibleBox::clearLineClamp()
{
    ...
    for (LayoutBox* child = iterator.first(); child; child = iterator.next()) {
        ...
        child->clearOverrideSize();
        if ((child->isAtomicInlineLevel() && (child->style()->width().isPercentOrCalc() || child->style()->height().isPercentOrCalc())) || (child->style()->height().isAuto() && child->isLayoutBlock())) {
