    ...
    for (LayoutObject* child = m_table->children()->firstChild(); child;
        ...
        if (child->isLayoutTableCol()) {
            ...
            for (unsigned i = 0; i < numRows; i++) {
                ...
                if (cell->colSpan() == 1) {
                    columnLayout.minLogicalWidth = std::max<int>(cell->minPreferredLogicalWidth().toInt(),
                    ...
                    case Fixed:
                        ...
                        if (cellLogicalWidth.isPositive() && !columnLayout.logicalWidth.isPercentOrCalc()) {
                            int logicalWidth = cell->adjustBorderBoxLogicalWidthForBoxSizing(
                                                       cellLogicalWidth.value())
                                                   .toInt();
                            if (columnLayout.logicalWidth.isFixed()) {
                                // Nav/IE weirdness
                                if ((logicalWidth > columnLayout.logicalWidth.value()) || ((columnLayout.logicalWidth.value() == logicalWidth) && (maxContributor == cell))) {
                    ...
                    columnLayout.minLogicalWidth = std::max<int>(columnLayout.minLogicalWidth,
...
static bool shouldScaleColumnsForSelf(LayoutTable* table)
{
    ...
    while (true) {
        ...
        while (!cb->isLayoutView() && !cb->isTableCell() && cb->style()->width().isAuto() && !cb->isOutOfFlowPositioned())
            cb = cb->containingBlock();

        // TODO(dgrogan): Should the second clause check for isFixed() instead?
        if (!cb->isTableCell() || (!cb->style()->width().isAuto() && !cb->style()->width().isPercentOrCalc()))
            return true;
    ...
    for (size_t i = 0; i < m_layoutStruct.size(); ++i) {
        ...
        if (scaleColumnsForSelf) {
            if (m_layoutStruct[i].effectiveLogicalWidth.isPercentOrCalc()) {
                float percent = std::min(static_cast<float>(
...
int TableLayoutAlgorithmAuto::calcEffectiveLogicalWidth()
{
    ...
    for (size_t i = 0; i < m_spanCells.size(); ++i) {
        ...
        if (cellLogicalWidth.isPercentOrCalc()) {
            if (totalPercent > cellLogicalWidth.percent() || allColsArePercent) {
                ...
                maxLogicalWidth = std::max(maxLogicalWidth,
                    static_cast<int>(
                        std::max(spanMaxLogicalWidth, cellMaxLogicalWidth) * 100 / cellLogicalWidth.percent()));
            ...
            if (allColsAreFixed) {
                for (unsigned pos = effCol; fixedWidth > 0 && pos < lastCol; ++pos) {
                    int cellLogicalWidth = std::max(
                        ...
                        static_cast<int>(cellMinLogicalWidth * m_layoutStruct[pos].logicalWidth.value() / fixedWidth));
                ...
                for (unsigned pos = effCol; pos < lastCol; ++pos) {
                    ...
                    float percent = m_layoutStruct[pos].logicalWidth.isPercentOrCalc()
                    ...
                    m_layoutStruct[pos].effectiveMinLogicalWidth = std::max(m_layoutStruct[pos].effectiveMinLogicalWidth,
                ...
                for (unsigned pos = effCol;
                    ...
                    if (m_layoutStruct[pos].logicalWidth.isFixed() && haveAuto && fixedWidth <= cellMinLogicalWidth) {
                        int colMinLogicalWidth = std::max<int>(m_layoutStruct[pos].effectiveMinLogicalWidth,
                ...
                for (unsigned pos = effCol;
                     remainingMaxLogicalWidth >= 0 && pos < lastCol && remainingMinLogicalWidth < cellMinLogicalWidth;
                     ++pos) {
                    if (!(m_layoutStruct[pos].logicalWidth.isFixed() && haveAuto && fixedWidth <= cellMinLogicalWidth)) {
                        int colMinLogicalWidth = std::max<int>(
                            m_layoutStruct[pos].effectiveMinLogicalWidth,
                            static_cast<int>(remainingMaxLogicalWidth
                                    ? cellMinLogicalWidth * static_cast<float>(m_layoutStruct[pos].effectiveMaxLogicalWidth) / remainingMaxLogicalWidth
                                    : cellMinLogicalWidth));
                        colMinLogicalWidth = std::min<int>(
                            m_layoutStruct[pos].effectiveMinLogicalWidth + (cellMinLogicalWidth - remainingMinLogicalWidth),
                            colMinLogicalWidth);
                        remainingMaxLogicalWidth -= m_layoutStruct[pos].effectiveMaxLogicalWidth;
                        remainingMinLogicalWidth -= m_layoutStruct[pos].effectiveMinLogicalWidth;
                        cellMinLogicalWidth -= colMinLogicalWidth;
        ...
        if (!cellLogicalWidth.isPercentOrCalc()) {
            if (cellMaxLogicalWidth > spanMaxLogicalWidth) {
                for (unsigned pos = effCol; spanMaxLogicalWidth >= 0 && pos < lastCol;
                    ...
                    int colMaxLogicalWidth = std::max(
                        ...
                        static_cast<int>(
                            spanMaxLogicalWidth
                                ? cellMaxLogicalWidth * static_cast<float>(m_layoutStruct[pos].effectiveMaxLogicalWidth) / spanMaxLogicalWidth
                                : cellMaxLogicalWidth));
            ...
            for (unsigned pos = effCol; pos < lastCol; ++pos)
                m_layoutStruct[pos].maxLogicalWidth = std::max(m_layoutStruct[pos].maxLogicalWidth,
        ...
        if (totalPercent > 100) {
            ...
            for (unsigned i = nEffCols; i;) {
                ...
                if (m_layoutStruct[i].effectiveLogicalWidth.isPercentOrCalc()) {
                    ...
                    // handled in mozilla.
                    excess -= reduction;
                    int newLogicalWidth = std::max<int>(m_layoutStruct[i].effectiveMinLogicalWidth,
    ...
    Total total)
{
    ...
    for (int i = startToEnd ? 0 : nEffCols - 1;
        ...
        int cellLogicalWidth = (distributionMode == InitialWidth)
            ? max<int>(m_layoutStruct[i].computedLogicalWidth, newWidth)
            : newWidth;
        available -= cellLogicalWidth;
        total -= factor;
        m_layoutStruct[i].computedLogicalWidth = (distributionMode == InitialWidth)
            ? cellLogicalWidth
            : m_layoutStruct[i].computedLogicalWidth + cellLogicalWidth;
