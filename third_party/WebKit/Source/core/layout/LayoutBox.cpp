...
PaintLayerType LayoutBox::layerTypeRequired() const
{
    ...
    // coerced to auto.
    if (isPositioned() || createsGroup() || hasClipPath() || hasTransformRelatedProperty() || style()->hasCompositorProxy() || hasHiddenBackface() || hasReflection() || style()->specifiesColumns() || style()->isStackingContext() || style()->shouldCompositeForCurrentAnimations())
    ...
    const ComputedStyle& newStyle)
{
    ...
    if (oldStyle) {
        ...
        // substantially.
        if ((diff.needsPaintInvalidation() || diff.needsLayout()) && node() && (isHTMLHtmlElement(*node()) || isHTMLBodyElement(*node()))) {
    ...
    const ComputedStyle* oldStyle)
{
    ...
    if (hasOverflowClip() && oldStyle && oldStyle->effectiveZoom() != newStyle.effectiveZoom()) {
        ...
        if (oldOffset.width() || oldOffset.height()) {
            ScrollOffset newOffset = oldOffset.scaledBy(newStyle.effectiveZoom() / oldStyle->effectiveZoom());
    ...
    if (isOutOfFlowPositioned() && parent() && parent()->styleRef().isDisplayFlexibleOrGridBox())
        clearOverrideSize();
    ...
    if (oldStyle) {
        ...
        if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled() && shouldClipOverflow()) {
            ...
            if (!oldStyle->border().sizeEquals(newStyle.border()) || !oldStyle->border().radiiEqual(newStyle.border()))
                setNeedsPaintPropertyUpdate();
        }
    }

    if (diff.transformChanged()) {
        if (ScrollingCoordinator* scrollingCoordinator = document().frame()->page()->scrollingCoordinator())
    ...
    const ComputedStyle* oldStyle)
{
    ...
    float oldShapeImageThreshold = oldStyle ? oldStyle->shapeImageThreshold()
    ...
    const ComputedStyle* oldStyle)
{
    ...
    if (oldStyle->gridColumnStart() == style()->gridColumnStart() && oldStyle->gridColumnEnd() == style()->gridColumnEnd() && oldStyle->gridRowStart() == style()->gridRowStart() && oldStyle->gridRowEnd() == style()->gridRowEnd() && oldStyle->order() == style()->order() && oldStyle->hasOutOfFlowPosition() == style()->hasOutOfFlowPosition())
        return;
    ...
    FrameView* frameView)
{
    if (ownerElement && isHTMLFrameElementBase(*ownerElement)) {
        ...
        if (Page* page = frameView->frame().page()) {
            return page->autoscrollController().autoscrollInProgress() && frameElementBase->scrollingMode() == ScrollbarAlwaysOff;
        }
    }
    return false;
}
    ...
    bool makeVisibleInVisualViewport)
{
    ...
    // scroll the parent.
    if (style()->position() == FixedPosition && containerForFixedPosition() == view()) {
        return;
    }
    ...
    LayoutBlock* cb) const
{
    ...
    if (!styleToUse.logicalMaxWidth().isMaxSizeNone())
        logicalWidth = std::min(logicalWidth,
        ...
        do {
            ...
            if (rectType == BackgroundKnownOpaqueRect) {
                ...
                if (const StyleImage* image = cur->image()) {
                    if ((cur->repeatX() == RepeatFill || cur->repeatX() == RoundFill) && (cur->repeatY() == RepeatFill || cur->repeatY() == RoundFill) && image->knownToBeOpaque(*this)) {
            ...
            if (rectType == BackgroundClipRect && currentClip == ContentFillBox && cur->attachment() == LocalBackgroundAttachment) {
                currentClip = PaddingFillBox;
            }
...
{
    while (
        layoutObject && !(layoutObject->isBox() && toLayoutBox(layoutObject)->canAutoscroll())) {
        // Do not start autoscroll when the node is inside a fixed-position element.
        if (layoutObject->isBox() && toLayoutBox(layoutObject)->hasLayer() && toLayoutBox(layoutObject)->layer()->sticksToViewport()) {
            return nullptr;
        }
...
void LayoutBox::scrollByRecursively(const ScrollOffset& delta)
{
    ...
    if (hasOverflowClip() && !restrictedByLineClamp) {
        ...
        // scroll to try.
        ScrollOffset remainingScrollOffset = newScrollOffset - scrollableArea->getScrollOffset();
...
IntSize LayoutBox::originAdjustmentForScrollbars() const
{
    ...
    if (hasFlippedBlocksWritingMode() || (isHorizontalWritingMode() && shouldPlaceBlockDirectionScrollbarOnLogicalLeft())) {
...
IntSize LayoutBox::scrolledContentOffset() const
{
    ...
    PaintLayerScrollableArea* scrollableArea = getScrollableArea();
    IntSize result = scrollableArea->scrollOffsetInt() + originAdjustmentForScrollbars();
    ...
    HitTestAction action)
{
    ...
    if (shouldHitTestSelf && hasOverflowClip() && hitTestOverflowControl(result, locationInContainer, adjustedLocation))
        return true;
    ...
    if (style()->hasBorderRadius() && hitTestClippedOutByBorder(locationInContainer, adjustedLocation))
        return false;
    ...
    if (shouldHitTestSelf && visibleToHitTestRequest(result.hitTestRequest())) {
        ...
        if (locationInContainer.intersects(boundsRect)) {
            updateHitTestResult(result,
                flipForWritingMode(locationInContainer.point() - toLayoutSize(adjustedLocation)));
...
static bool isCandidateForOpaquenessTest(const LayoutBox& childBox)
{
    ...
    if (childStyle.position() != StaticPosition && childBox.containingBlock() != childBox.parent())
        return false;
    if (childStyle.visibility() != EVisibility::kVisible || childStyle.shapeOutside())
        return false;
...
void LayoutBox::imageChanged(WrappedImagePtr image, const IntRect*)
{
    ...
    // images.
    if ((styleRef().borderImage().image() && styleRef().borderImage().image()->data() == image) || (styleRef().maskBoxImage().image() && styleRef().maskBoxImage().image()->data() == image) || (styleRef().boxReflect() && styleRef().boxReflect()->mask().image() && styleRef().boxReflect()->mask().image()->data() == image)) {
        setShouldDoFullPaintInvalidation();
    } else {
...
{
    //   String out = String::format("LayoutBox::sizeChanged: %d\n", m_frameRect.width().toInt());
    //   OutputDebugStringA(out.utf8().data());
    //   if (1000 == m_frameRect.width().toInt())
    //       OutputDebugStringA("");
...
bool LayoutBox::intersectsVisibleViewport() const
{
    ...
    while (!layoutView->frame()->ownerLayoutItem().isNull())
        layoutView = LayoutAPIShim::layoutObjectFrom(layoutView->frame()->ownerLayoutItem())
                         ->view();
...
void LayoutBox::ensureIsReadyForPaintInvalidation()
{
    ...
    if (fullPaintInvalidationReason() != PaintInvalidationDelayedFull || !intersectsVisibleViewport())
        return;
...
LayoutRect LayoutBox::clipRect(const LayoutPoint& location) const
{
    ...
    if (!style()->clipBottom().isAuto())
        clipRect.contract(LayoutUnit(),
            size().height() - valueForLength(style()->clipBottom(), size().height()));
    ...
    const LayoutBlockFlow* cb) const
{
    ...
    LayoutUnit endOffsetForLine = cb->endOffsetForLine(logicalTopPosition, DoNotIndentText, logicalHeight);
    ...
    LayoutUnit width = cb->availableLogicalWidthForLine(
                           logicalTopPosition, DoNotIndentText, logicalHeight)
        - std::max(LayoutUnit(), childMarginStart) - std::max(LayoutUnit(), childMarginEnd);
...
{
    if (hasNonCompositedScrollbars() || getSelectionState() != SelectionNone || hasBoxDecorationBackground() || styleRef().hasBoxDecorations() || styleRef().hasVisualOverflowingEffect())
        return false;
    ...
    VisualRectFlags visualRectFlags) const
{
    ...
    if (container->isBox() && container != ancestor && !toLayoutBox(container)->mapScrollingContentsRectToBoxSpace(rect, visualRectFlags))
        return false;
...
static bool needUseMaxWidth(LayoutBox* rbox)
{
    if ((!rbox) || (!rbox->isFloating()))
        return false;

    LayoutBlock* r = NULL;
    if (rbox->isLayoutBlock())
        r = static_cast<LayoutBlock*>(rbox);
    if (!r)
        return false;

    Element* e = static_cast<Element*>(r->node());
    if (!e)
        return false;

    if (e->isDefaultInlineNode())
        return true;

    int count = 0;
    if (r->firstChild()) {
        if (r->firstChild()->isAnonymous())
            return false;
        Node* child = r->firstChild()->node();
        while (child) {
            if ((!child->isTextNode()) && (child->layoutObject() && child->layoutObject()->style() && EDisplay::None != child->layoutObject()->style()->display())) {
                Element* ce = static_cast<Element*>(child);
                if (ce && ce->isDefaultInlineNode()) {
                    ++count;
                    if (count > 1)
                        return false;
                } else
...
void LayoutBox::dealWidthForFitScreen()
{
    LayoutBlock* cb = containingBlock();
    if ((!cb) || !(document().page()))
        return;

    if (node() && node()->isTextControlInnerElement()) {
        return;
    }
    ...
    LayoutUnit maxAvailableWidth = cb->size().width() - cb->borderAndPaddingWidth() - marginLeft() - marginRight();

    LayoutUnit defaultMaxWidth(document().page()->getDefaultMaxWidth());
    if (maxAvailableWidth > defaultMaxWidth)
        maxAvailableWidth = defaultMaxWidth - cb->borderAndPaddingWidth() - marginLeft() - marginRight();

    if (maxAvailableWidth <= 0)
        return;

    if (isInline() || isLayoutImage() || (style() && (style()->hasBackgroundImage() && !style()->isBackgroundImageCleared()) && style()->width().isFixed())) { //\u89e3\u51b3manhua2\u5b50\u9875\u9762\u6392\u7248\u9519\u4e71\u7684\u95ee\u9898
        if (size().width() > maxAvailableWidth) {
    ...
    if (isFloating()) {
        //\u89e3\u51b3www.17173.com\u5728\u9002\u5e94\u5c4f\u5e55\u60c5\u51b5\u4e0bcrash!
        if (node() && !node()->isElementNode())
            return;

        if (style()->floating() == EFloat::kLeft
            && previousSibling()
            && previousSibling()->style()
            && previousSibling()->style()->floating() == EFloat::kLeft) {
            if (previousSibling()->isBox()) {
                LayoutBox* pre = static_cast<LayoutBox*>(previousSibling());
                if (pre) {
                    LayoutUnit preWidth = pre->size().width();
                    LayoutUnit newAvailableWidth = maxAvailableWidth - preWidth;
                    maxAvailableWidth = newAvailableWidth > size().width() ? newAvailableWidth : maxAvailableWidth;
                }
            }
        }
        ...
        if ((e && e->isDefaultInlineNode()) || (style() && style()->width().isFixed() && style()->width().value() < 150) || needUseMaxWidth(this)) {
            LayoutUnit w = maxPreferredLogicalWidth(); // + borderAndPaddingWidth() + marginLeft() + marginRight();
            w = w > size().width() ? w : size().width();
            w = w < maxAvailableWidth ? w : maxAvailableWidth;
            setWidth(w);
            return;
        }
    }
...
static bool checkIsInSpecialSiteImageListScope(const LayoutBox* box)
{
    Node* node = box->node();
    if (!node)
        return false;
...
#if TENCENT_FITSCREEN
    if (document().needFitScreenLayout() && !checkIsInSpecialSiteImageListScope(this))
        dealWidthForFitScreen();
#endif
}
    ...
    LogicalExtentComputedValues& computedValues) const
{
    ...
    if (isInline() && !isInlineBlockOrInlineTable()) {
        ...
        if (treatAsReplaced)
            computedValues.m_extent = std::max(LayoutUnit(floatValueForLength(logicalWidthLength, 0)) + borderAndPaddingLogicalWidth(),
    ...
    if (treatAsReplaced) {
        ...
        LayoutUnit preferredWidth = computeLogicalWidthUsing(MainOrPreferredSize, styleToUse.logicalWidth(),
    ...
    if (!hasPerpendicularContainingBlock && containerLogicalWidth && containerLogicalWidth != (computedValues.m_extent + computedValues.m_margins.m_start + computedValues.m_margins.m_end) && !isFloating() && !isInline() && !cb->isFlexibleBoxIncludingDeprecated() && !cb->isLayoutGrid()) {
    ...
    const LayoutBlock* cb) const
{
    ...
    if (shrinkToAvoidFloats() && cb->isLayoutBlockFlow() && toLayoutBlockFlow(cb)->containsFloats())
        logicalWidthResult = std::min(
            logicalWidthResult, shrinkLogicalWidthToAvoidFloats(marginStart, marginEnd, toLayoutBlockFlow(cb)));
...
{
    return node() && (isHTMLInputElement(*node()) || isHTMLSelectElement(*node()) || isHTMLButtonElement(*node()) || isHTMLTextAreaElement(*node()) || (isHTMLLegendElement(*node()) && !style()->hasOutOfFlowPosition()));
}
    ...
    Length marginEndLength) const
{
    ...
    if (marginBoxWidth < availableWidth) {
        ...
        if ((marginStartLength.isAuto() && marginEndLength.isAuto()) || (!marginStartLength.isAuto() && !marginEndLength.isAuto() && containingBlockStyle.textAlign() == ETextAlign::kWebkitCenter)) {
            // Other browsers center the margin box for align=center elements so we
            // match them here.
            LayoutUnit centeredMarginBoxStart = std::max(
                LayoutUnit(),
                (availableWidth - childWidth - marginStartWidth - marginEndWidth) / 2);
#ifdef TENCENT_FITSCREEN
            if (document().needFitScreenLayout() && !checkIsInSpecialSiteImageListScope(this))
                marginStart = LayoutUnit();
            else
#endif
        ...
        if ((!containingBlockStyle.isLeftToRightDirection() && containingBlockStyle.textAlign() == ETextAlign::kWebkitLeft) || (containingBlockStyle.isLeftToRightDirection() && containingBlockStyle.textAlign() == ETextAlign::kWebkitRight)) {
    ...
    LogicalExtentComputedValues& computedValues) const
{
    ...
    if (isOutOfFlowPositioned()) {
        ...
        MarginDirection flowDirection = isHorizontalWritingMode() != cb->isHorizontalWritingMode()
            ? InlineDirection
            : BlockDirection;
        ...
        if (h.isAuto() && inHorizontalBox && toLayoutDeprecatedFlexibleBox(parent())->isStretchingChildren()) {
            h = Length(parentBox()->contentLogicalHeight() - marginBefore() - marginAfter() - borderAndPaddingLogicalHeight(),
                Fixed);
    ...
    bool paginatedContentNeedsBaseHeight = document().printing() && h.isPercentOrCalc() && (isDocumentElement() || (isBody() && heightForDocumentElement(document()).isPercentOrCalc())) && !isInline();
    ...
    const Length& height) const
{
    ...
    if (isHorizontalWritingMode() != cb->isHorizontalWritingMode()) {
        ...
        if (!skippedAutoHeightContainingBlock) {
            ...
            if (!cb->hasOverrideLogicalContentHeight()) {
                ...
                if (style()->overflowY() != EOverflow::Visible && style()->overflowY() != EOverflow::Hidden && !shouldBeConsideredAsReplaced() && (!cell->style()->logicalHeight().isAuto() || !cell->table()->style()->logicalHeight().isAuto()))
                    return LayoutUnit();
                return LayoutUnit(-1);
            }
    ...
    bool subtractBorderAndPadding = isTable() || (cb->isTableCell() && !skippedAutoHeightContainingBlock && cb->hasOverrideLogicalContentHeight() && style()->boxSizing() == EBoxSizing::kContentBox);
...
LayoutUnit LayoutBox::computeReplacedLogicalWidthRespectingMinMaxWidth(
    ...
    ShouldComputePreferred shouldComputePreferred) const
{
    LayoutUnit minLogicalWidth = (shouldComputePreferred == ComputePreferred && style()->logicalMinWidth().isPercentOrCalc())
        ? logicalWidth
        : computeReplacedLogicalWidthUsing(
            MinSize, style()->logicalMinWidth());
    LayoutUnit maxLogicalWidth = (shouldComputePreferred == ComputePreferred && style()->logicalMaxWidth().isPercentOrCalc()) || style()->logicalMaxWidth().isMaxSizeNone()
        ? logicalWidth
        : computeReplacedLogicalWidthUsing(MaxSize,
            style()->logicalMaxWidth());
#ifdef TENCENT_FITSCREEN
    if (isImage() && mutableStyle() && mutableStyle()->getFitScreenLayout() && intrinsicSize().width() > 0) {
        //make sure img's width is smaller than intrinsic width
        maxLogicalWidth = std::min(maxLogicalWidth, intrinsicSize().width());
    }
#endif
    ...
    case Calculated: {
        ...
        if (logicalWidth.isIntrinsic())
            return computeIntrinsicLogicalWidthUsing(
                ...
                - borderAndPaddingLogicalWidth();
        if (cw > 0 || (!cw && (containerLogicalWidth.isFixed() || containerLogicalWidth.isPercentOrCalc())))
...
#ifdef TENCENT_FITSCREEN
    LayoutSize is = intrinsicSize();
    if (isImage() && mutableStyle() && mutableStyle()->getFitScreenLayout() && 0 != size().width().toInt() && 0 != size().height().toInt() && is.height() > 0 && is.width() > 0) {
        //height is smaller than intrinsicHeight
        LayoutUnit intrinsicHeight = size().width() > 0 && size().width() < is.width() ? is.height() * size().width() / is.width() : is.height();
        maxLogicalHeight = std::min(maxLogicalHeight, intrinsicHeight);
    }
#endif
    ...
    const Length& logicalHeight) const
{
    ...
    case Calculated: {
        ...
        if (cb->isLayoutBlock()) {
            ...
            if (block->isFlexItem())
                stretchedHeight = toLayoutFlexibleBox(block->parent())
        ...
        if (isOutOfFlowPositioned()) {
            ...
            while (cb && !cb->isLayoutView() && (cb->style()->logicalHeight().isAuto() || cb->style()->logicalHeight().isPercentOrCalc())) {
                if (cb->isTableCell()) {
            ...
            if (curr->isBox()) {
                ...
                if (curr->isInFlowPositioned()) {
                    if (!curr->style()->logicalLeft().isAuto())
                        staticPosition += valueForLength(curr->style()->logicalLeft(),
                            curr->containingBlock()->availableWidth());
                    else
                        staticPosition -= valueForLength(curr->style()->logicalRight(),
                            curr->containingBlock()->availableWidth());
                }
            }
        }
    ...
    LayoutUnit containerLogicalWidth)
{
    ...
    if (containerBlock->isHorizontalWritingMode() != child->isHorizontalWritingMode() && containerBlock->style()->isFlippedBlocksWritingMode()) {
        ...
        logicalLeftPos += (child->isHorizontalWritingMode() ? containerBlock->borderRight()
                                                            : containerBlock->borderBottom());
    } else {
        logicalLeftPos += (child->isHorizontalWritingMode() ? containerBlock->borderLeft()
    ...
    LogicalExtentComputedValues& computedValues) const
{
    ...
    if (widthSizeType == MinSize && logicalWidth.isAuto())
        ...
        logicalWidthValue = computeIntrinsicLogicalWidthUsing(logicalWidth, containerLogicalWidth,
                                bordersPlusPadding)
            - bordersPlusPadding;
    else
    ...
    LayoutUnit containerLogicalHeight)
{
    ...
    // along this axis, then we need to flip the coordinate.  This can only happen
    // if the containing block is both a flipped mode and perpendicular to us.
    if ((child->style()->isFlippedBlocksWritingMode() && child->isHorizontalWritingMode() != containerBlock->isHorizontalWritingMode()) || (child->style()->isFlippedBlocksWritingMode() != containerBlock->style()->isFlippedBlocksWritingMode() && child->isHorizontalWritingMode() == containerBlock->isHorizontalWritingMode()))
    ...
    if (!logicalTopIsAuto && !logicalHeightIsAuto && !logicalBottomIsAuto) {
        ...
        if (logicalTopIsAuto && logicalHeightIsAuto && !logicalBottomIsAuto) {
            ...
            logicalHeightValue = std::max(LayoutUnit(),
                availableSpace - (logicalTopValue + valueForLength(logicalBottom, containerLogicalHeight)));
        } else if (!logicalTopIsAuto && !logicalHeightIsAuto && logicalBottomIsAuto) {
    ...
    LayoutUnit* extraWidthToEndOfLine)
{
    ...
    LayoutUnit fontHeight = LayoutUnit(fontData ? fontData->getFontMetrics().height() : 0);
...
PositionWithAffinity LayoutBox::positionForPoint(const LayoutPoint& point)
{
    ...
    for (LayoutObject* layoutObject = firstChild; layoutObject;
        ...
        if ((!layoutObject->slowFirstChild() && !layoutObject->isInline() && !layoutObject->isLayoutBlockFlow()) || layoutObject->style()->visibility() != EVisibility::kVisible)
            continue;
...
{
    return shouldBeConsideredAsReplaced() || hasOverflowClip() || isHR() || isLegend() || isWritingModeRoot() || isFlexItemIncludingDeprecated() || style()->containsPaint() || style()->containsLayout();
}
...
void LayoutBox::updateFragmentationInfoForChild(LayoutBox& child)
{
    ...
    LayoutUnit spaceLeft = pageRemainingLogicalHeightForOffset(logicalTop, AssociateWithLatterPage);
    ...
    SubtreeLayoutScope& layoutScope)
{
    ...
    if (layoutState->paginationStateChanged() || (layoutState->isPaginated() && childNeedsRelayoutForPagination(child)))
...
void LayoutBox::addLayoutOverflow(const LayoutRect& rect)
{
    ...
    if (!m_overflow) {
        m_overflow = WTF::wrapUnique(new BoxOverflowModel(clientBox, borderBoxRect()));
    }
...
void LayoutBox::addSelfVisualOverflow(const LayoutRect& rect)
{
    ...
    if (!m_overflow) {
        m_overflow = WTF::wrapUnique(new BoxOverflowModel(noOverflowRect(), borderBox));
    }
...
bool LayoutBox::hasUnsplittableScrollingOverflow() const
{
    ...
    return !style()->logicalHeight().isIntrinsicOrAuto() || (!style()->logicalMaxHeight().isIntrinsicOrAuto() && !style()->logicalMaxHeight().isMaxSizeNone() && (!style()->logicalMaxHeight().isPercentOrCalc() || percentageLogicalHeightIsResolvable())) || (!style()->logicalMinHeight().isIntrinsicOrAuto() && style()->logicalMinHeight().isPositive() && (!style()->logicalMinHeight().isPercentOrCalc() || percentageLogicalHeightIsResolvable()));
}
...
LayoutBox::PaginationBreakability LayoutBox::getPaginationBreakability() const
{
    ...
    if (isAtomicInlineLevel() || hasUnsplittableScrollingOverflow() || (parent() && isWritingModeRoot()) || (isOutOfFlowPositioned() && style()->position() == FixedPosition))
        return ForbidBreaks;
    ...
    const ComputedStyle& parentStyle) const
{
    ...
    // along that axis.
    if (isFlippedBlocksWritingMode(style()->getWritingMode()) || isFlippedBlocksWritingMode(parentStyle.getWritingMode()))
...
LayoutRect LayoutBox::noOverflowRect() const
{
    ...
    LayoutUnit left(
        borderLeft() + (shouldPlaceBlockDirectionScrollbarOnLogicalLeft() ? scrollBarWidth : 0));
    ...
    const LayoutPoint& point) const
{
    ...
    // the right place.
    return LayoutPoint(point.x() + size().width() - child->size().width() - (2 * child->location().x()),
        point.y());
}
...
{
    return style()->logicalWidth().isPercentOrCalc() || style()->logicalMinWidth().isPercentOrCalc() || style()->logicalMaxWidth().isPercentOrCalc();
}

bool LayoutBox::hasRelativeLogicalHeight() const
{
    return style()->logicalHeight().isPercentOrCalc() || style()->logicalMinHeight().isPercentOrCalc() || style()->logicalMaxHeight().isPercentOrCalc();
}
    ...
    const FillLayer& layer)
{
    ...
    if (sizeType == SizeLength) {
        // TODO(alancutter): Make this work correctly for calc lengths.
        if (layer.sizeLength().height().isPercentOrCalc() && !layer.sizeLength().height().isZero())
            return true;
    ...
    const FillLayer& layer)
{
    ...
    if (sizeType == SizeLength) {
        // TODO(alancutter): Make this work correctly for calc lengths.
        if (layer.sizeLength().width().isPercentOrCalc() && !layer.sizeLength().width().isZero())
            return true;
    ...
    LayoutUnit logicalHeight) const
{
    if (!pageLogicalHeightForOffset(offset))
        return false;
    return pageRemainingLogicalHeightForOffset(offset, AssociateWithLatterPage) < logicalHeight;
}
...
LayoutPoint LayoutBox::location() const
{
    return m_frameRect.location();
}

LayoutSize LayoutBox::locationOffset() const
{
...
IntSize LayoutBox::pixelSnappedSize() const
{
    return m_frameRect.pixelSnappedSize();
}
...
LayoutRect LayoutBox::frameRect() const
{
    return m_frameRect;
}
