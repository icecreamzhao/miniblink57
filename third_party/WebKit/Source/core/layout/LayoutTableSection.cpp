    ...
    const LayoutTableCell* cell)
{
    ...
    if (logicalHeight.isPositive()) {
        ...
        case Percent:
            ...
            if (!(cRowLogicalHeight.isPercentOrCalc()) || (cRowLogicalHeight.isPercent() && cRowLogicalHeight.percent() < logicalHeight.percent()))
                row.logicalHeight = logicalHeight;
            break;
        case Fixed:
            if (cRowLogicalHeight.type() < Percent || (cRowLogicalHeight.isFixed() && cRowLogicalHeight.value() < logicalHeight.value()))
                row.logicalHeight = logicalHeight;
            break;
        default:
            break;
...
void LayoutTableSection::addCell(LayoutTableCell* cell, LayoutTableRow* row)
{
    ...
    // <TR><TD colspan="2">5
    // </TABLE>
    while (m_cCol < numCols(insertionRow) && (cellAt(insertionRow, m_cCol).hasCells() || cellAt(insertionRow, m_cCol).inColSpan))
        m_cCol++;
    ...
    int& baselineDescent)
{
    ...
    if (baselinePosition > cell->borderBefore() + (cell->paddingBefore() - cell->intrinsicPaddingBefore())) {
        ...
        if (cell->rowSpan() == 1) {
            baselineDescent = std::max(baselineDescent,
    ...
    for (unsigned r = 0; r < m_grid.size(); r++) {
        ...
        if (m_grid[r].logicalHeight.isSpecified()) {
            ...
            m_rowPos[r + 1] = std::max(
                m_rowPos[r] + minimumValueForLength(m_grid[r].logicalHeight, LayoutUnit()).round(),
                0);
        } else {
        ...
        for (unsigned c = 0; c < totalCols; c++) {
            ...
            for (unsigned i = 0; i < current.cells.size(); i++) {
                ...
                if (r < indexOfFirstStretchableRow || (state.isPaginated() && crossesPageBoundary(LayoutUnit(m_rowPos[r]), LayoutUnit(cell->logicalHeightForRowSizing())))) {
                    ...
                    indexOfFirstStretchableRow = std::max(indexOfFirstStretchableRow, rowIndexBelowCell);
        ...
        if (r < indexOfFirstStretchableRow && m_grid[r].rowLayoutObject) {
            ...
            // layout instead.
            m_rowPos[r + 1] = m_rowPos[r] + m_grid[r].rowLayoutObject->logicalHeight().toInt();
        }
    ...
    LayoutObject* cellDescendant)
{
    if (!cell.style()->logicalHeight().isSpecified())
        return false;
    if (cellDescendant->style()->overflowY() == EOverflow::Visible || cellDescendant->style()->overflowY() == EOverflow::Hidden)
        return true;
    return cellDescendant->isBox() && toLayoutBox(cellDescendant)->shouldBeConsideredAsReplaced();
}
...
void LayoutTableSection::layoutRows()
{
    ...
    for (unsigned r = 0; r < totalRows; r++) {
        ...
        for (unsigned c = 0; c < numCols(r); c++) {
            ...
            if (state.isPaginated() && crossesPageBoundary(LayoutUnit(rowLogicalTop), LayoutUnit(rHeight)))
    ...
    unsigned nEffCols)
{
    ...
    unsigned maxAllowedOverflowingCellsCount = totalCellsCount < gMinTableSizeToUseFastPaintPathWithOverflowingCell
        ? 0
...
int LayoutTableSection::firstLineBoxBaseline() const
{
    ...
    for (size_t i = 0; i < firstRow.size(); ++i) {
        ...
        if (cell)
            firstLineBaseline = std::max<int>(firstLineBaseline,
                (cell->logicalTop() + cell->borderBefore() + cell->paddingBefore() + cell->contentLogicalHeight())
                    .toInt());
    }
    ...
    unsigned effectiveColumn) const
{
    ...
    if (!style()->isLeftToRightDirection())
        cellLocation.setX(LayoutUnit(
            table()->effectiveColumnPositions()[table()->numEffectiveColumns()] - table()->effectiveColumnPositions()[table()->absoluteColumnToEffectiveColumn(cell->absoluteColumnIndex() + cell->colSpan())] + horizontalBorderSpacing));
    else
        cellLocation.setX(
            LayoutUnit(table()->effectiveColumnPositions()[effectiveColumn] + horizontalBorderSpacing));
    ...
    int rowHeight)
{
    ...
    bool flexAllChildren = cell.style()->logicalHeight().isSpecified() || (!table()->style()->logicalHeight().isAuto() && rowHeight != cell.logicalHeight());

    for (LayoutObject* child = cell.firstChild(); child;
         child = child->nextSibling()) {
        if (!child->isText() && child->style()->logicalHeight().isPercentOrCalc() && (flexAllChildren || shouldFlexCellChild(cell, child)) && (!child->isTable() || toLayoutTable(child)->hasSections())) {
    ...
    const LayoutTableRow& rowObject) const
{
    ...
    for (unsigned colIndex = 0; colIndex < cols; colIndex++) {
        ...
        unsigned rowSpan = cell->rowSpan();
        if (rowSpan == 1) {
            logicalHeight = std::max(logicalHeight, cell->logicalHeightForRowSizing());
            continue;
        }
        unsigned rowIndexForCell = cell->rowIndex();
        if (rowIndex == m_grid.size() - 1 || (rowSpan > 1 && rowIndex - rowIndexForCell == rowSpan - 1)) {
            // This is the last row of the rowspanned cell. Add extra height if
            // needed.
            if (LayoutTableRow* firstRowForCell = m_grid[rowIndexForCell].rowLayoutObject) {
                int minLogicalHeight = cell->logicalHeightForRowSizing();
                // Subtract space provided by previous rows.
                minLogicalHeight -= rowObject.logicalTop().toInt() - firstRowForCell->logicalTop().toInt();
    ...
    if (m_grid[rowIndex].logicalHeight.isSpecified()) {
        LayoutUnit specifiedLogicalHeight = minimumValueForLength(m_grid[rowIndex].logicalHeight, LayoutUnit());
    ...
    SubtreeLayoutScope& layouter)
{
    ...
    int paginationStrut = paginationStrutForRow(&rowObject, rowObject.logicalTop());
    bool rowIsAtTopOfColumn = false;
    LayoutUnit offsetFromTopOfPage;
    if (!paginationStrut) {
        LayoutUnit pageLogicalHeight = pageLogicalHeightForOffset(rowObject.logicalTop());
        if (pageLogicalHeight && table()->header() && table()->rowOffsetFromRepeatingHeader()) {
            offsetFromTopOfPage = pageLogicalHeight - pageRemainingLogicalHeightForOffset(rowObject.logicalTop(), AssociateWithLatterPage);
            rowIsAtTopOfColumn = !offsetFromTopOfPage || offsetFromTopOfPage <= table()->vBorderSpacing();
        }
