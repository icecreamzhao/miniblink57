...
#if TENCENT_FITSCREEN
    , m_bFloatMarked(false)
    , m_bSetOverflowHidden(false)
    , m_bSetClearBoth(false)
#endif
{
    ...
    const ComputedStyle& newStyle)
{
    ...
    if (oldStyle && parent()) {
        ...
        if ((oldStyleContainsFixedPosition && !newStyleContainsFixedPosition) || (oldStyleContainsAbsolutePosition && !newStyleContainsAbsolutePosition)) {
...
enum LogicalExtent { LogicalWidth,
    LogicalHeight };
    ...
    LogicalExtent logicalExtent)
{
    if (newStyle.isHorizontalWritingMode() == (logicalExtent == LogicalWidth)) {
        return oldStyle.borderLeftWidth() != newStyle.borderLeftWidth() || oldStyle.borderRightWidth() != newStyle.borderRightWidth() || oldStyle.paddingLeft() != newStyle.paddingLeft() || oldStyle.paddingRight() != newStyle.paddingRight();
    }

    return oldStyle.borderTopWidth() != newStyle.borderTopWidth() || oldStyle.borderBottomWidth() != newStyle.borderBottomWidth() || oldStyle.paddingTop() != newStyle.paddingTop() || oldStyle.paddingBottom() != newStyle.paddingBottom();
}
    ...
    LayoutObject* beforeDescendant)
{
    ...
    if (beforeDescendantContainer->isAnonymousBlock()
        ...
        // blocks, not tables:
        || beforeDescendantContainer->isLayoutFullScreen() || beforeDescendantContainer->isLayoutFullScreenPlaceholder()) {
...
#if TENCENT_FITSCREEN
//if newChild is float, mark the parents, and set overflow to hidden
void LayoutBlock::floatMarkParent(LayoutObject* newChild)
{
    if (!newChild)
        return;
    ...
    // mark
    LayoutBlock* p = this;
    while (p && !p->m_bFloatMarked) {
        p->m_bFloatMarked = true;

        if (!(p->parent() && p->parent()->isLayoutBlock()))
            break;
        p = toLayoutBlock(p->parent());
    }

    if (node() && node()->isElementNode()) {
        Element* pElement = static_cast<Element*>(node());
        if (!pElement)
            return;
        if (style() && style()->overflowX() != EOverflow::Hidden) {
            if (/*tencent::gFitScreenLayoutModel
          && document()
          && */
                document().needFitScreenLayout()) {
                RefPtr<ComputedStyle> newStyle = pElement->styleForLayoutObject();
                newStyle->setOverflowX(EOverflow::Hidden);
...
//if newChild isn't float, and parents is marked, consume the mark, set newChild's clear to both
void LayoutBlock::consumeFloatMark(LayoutObject* newChild)
{
    ...
    if (newChild
        ...
        &&*/
            document().needFitScreenLayout())
            newChild->mutableStyle()->setClear(EClear::ClearBoth);
        if (newChild->isLayoutBlock()) {
            toLayoutBlock(newChild)->setClearBoth(true);
        }

        //unmark
        LayoutBlock* p = this;
        while (p && p->m_bFloatMarked) {
            p->m_bFloatMarked = false;
...
void LayoutBlock::addChild(LayoutObject* newChild, LayoutObject* beforeChild)
{
    ...
    if (newChild->isFloating()) {
        floatMarkParent(newChild);
        if (beforeChild)
            consumeFloatMark(beforeChild);
    } else if (m_bFloatMarked && (!beforeChild)) {
        consumeFloatMark(newChild);
    } else if (beforeChild && beforeChild->previousSibling() && beforeChild->previousSibling()->isFloating()) {
        consumeFloatMark(newChild);
    }
#endif
...
void LayoutBlock::layout()
{
    ...
    bool needsScrollAnchoring = hasOverflowClip() && getScrollableArea()->shouldPerformScrollAnchoring();
...
bool LayoutBlock::widthAvailableToChildrenHasChanged()
{
    ...
    widthAvailableToChildrenHasChanged |= style()->boxSizing() == EBoxSizing::kBorderBox && needsPreferredWidthsRecalculation() && view()->layoutState()->containingBlockLogicalWidthChanged();
...
{
    return isInlineBlockOrInlineTable() || isFloatingOrOutOfFlowPositioned() || hasOverflowClip() || isFlexItemIncludingDeprecated() || style()->specifiesColumns() || isLayoutFlowThread() || isTableCell() || isTableCaption() || isFieldset() || isWritingModeRoot() || isDocumentElement() || isColumnSpanAll() || isGridItem() || style()->containsPaint() || style()->containsLayout() || isSVGForeignObject();
}
    ...
    LayoutBox& child)
{
    ...
    bool hasRelativeLogicalHeight = child.hasRelativeLogicalHeight() || (child.isAnonymous() && this->hasRelativeLogicalHeight()) || child.stretchesToViewport();
    if (relayoutChildren || (hasRelativeLogicalHeight && !isLayoutView()) || (m_heightAvailableToChildrenChanged && changeInAvailableLogicalHeightAffectsChild(this, child))) {
...

    {
        ...
        if (posChildNeedsLayout() || needsPositionedMovementLayout() || canContainFixedPosObjects)
            layoutPositionedObjects(
                false, needsPositionedMovementLayout() ? ForcedLayoutAfterContainingBlockMoved : (!posChildNeedsLayout() && canContainFixedPosObjects ? LayoutOnlyFixedPositionedObjects : DefaultLayout));
...
bool LayoutBlock::isSelectionRoot() const
{
    ...
    if (isBody() || isDocumentElement() || hasOverflowClip() || isPositioned() || isFloating() || isTableCell() || isInlineBlockOrInlineTable() || hasTransformRelatedProperty() || hasReflection() || hasMask() || isWritingModeRoot() || isLayoutFlowThread() || isFlexItemIncludingDeprecated())
        return true;
...
{
    if (visibleToHitTestRequest(result.hitTestRequest()) && isPointInOverflowControl(result, locationInContainer.point(), adjustedLocation)) {
        updateHitTestResult(
            result, locationInContainer.point() - toLayoutSize(adjustedLocation));
        // FIXME: isPointInOverflowControl() doesn't handle rect-based tests yet.
        if (result.addNodeToListBasedTestResult(
                nodeForHitTest(), locationInContainer)
            == StopHitTesting)
            return true;
    }
    return false;
    ...
    LineLayoutBox child)
{
    ...
    return !ancestor || !ancestor->parent() || (ancestor->hasLayer() && ancestor->parent()->isLayoutView()) || hasEditableStyle(*ancestor->nonPseudoNode()) == hasEditableStyle(*child.nonPseudoNode());
}
...
{
    return box->size().height() && box->style()->visibility() == EVisibility::kVisible && !box->isFloatingOrOutOfFlowPositioned() && !box->isLayoutFlowThread();
}
...
PositionWithAffinity LayoutBlock::positionForPoint(const LayoutPoint& point)
{
    ...
    if (lastCandidateBox) {
        if (pointInLogicalContents.y() > logicalTopForChild(*lastCandidateBox) || (!blocksAreFlipped && pointInLogicalContents.y() == logicalTopForChild(*lastCandidateBox)))
...
void LayoutBlock::computePreferredLogicalWidths()
{
    ...
    if (!isTableCell() && styleToUse.logicalWidth().isFixed() && styleToUse.logicalWidth().value() >= 0 && !(isDeprecatedFlexItem() && !styleToUse.logicalWidth().intValue())
#if TENCENT_FITSCREEN
        && !(document().needFitScreenLayout() && (!isLayoutImage()) && (!styleToUse.hasBackgroundImage() || styleToUse.isBackgroundImageCleared()))
#endif
    )
        m_minPreferredLogicalWidth = m_maxPreferredLogicalWidth = adjustContentBoxLogicalWidthForBoxSizing(
    ...
    if (styleToUse.logicalMinWidth().isFixed() && styleToUse.logicalMinWidth().value() > 0) {
        m_maxPreferredLogicalWidth = std::max(m_maxPreferredLogicalWidth,
            adjustContentBoxLogicalWidthForBoxSizing(
                LayoutUnit(styleToUse.logicalMinWidth().value())));
        m_minPreferredLogicalWidth = std::max(m_minPreferredLogicalWidth,
            adjustContentBoxLogicalWidthForBoxSizing(
                LayoutUnit(styleToUse.logicalMinWidth().value())));
    }

    if (styleToUse.logicalMaxWidth().isFixed()) {
        m_maxPreferredLogicalWidth = std::min(m_maxPreferredLogicalWidth,
            adjustContentBoxLogicalWidthForBoxSizing(
                LayoutUnit(styleToUse.logicalMaxWidth().value())));
        m_minPreferredLogicalWidth = std::min(m_minPreferredLogicalWidth,
            adjustContentBoxLogicalWidthForBoxSizing(
                LayoutUnit(styleToUse.logicalMaxWidth().value())));
    }
    ...
    while (child) {
        ...
        if (!child->isFloating()) {
            if (child->isBox() && toLayoutBox(child)->avoidsFloats()) {
                ...
                LayoutUnit maxRight = marginLogicalRight > 0
    ...
    LinePositionMode linePositionMode) const
{
    ...
    if (isInline() && linePositionMode == PositionOnContainingLine) {
        ...
        if (isDeprecatedFlexibleBox()) {
            ...
            LayoutUnit bottomOfContent = direction == HorizontalLine
                ? size().height() - borderBottom() - paddingBottom() - horizontalScrollbarHeight()
                : size().width() - borderLeft() - paddingLeft() - verticalScrollbarWidth();
...
const LayoutBlock* LayoutBlock::enclosingFirstLineStyleBlock() const
{
    ...
    while (true) {
        ...
        if (firstLineBlock->isAtomicInlineLevel() || firstLineBlock->isFloatingOrOutOfFlowPositioned() || !parentBlock || !parentBlock->behavesLikeBlockContainer())
            break;
...
LayoutBlockFlow* LayoutBlock::nearestInnerBlockWithFirstLine()
{
    ...
    for (LayoutObject* child = firstChild();
         child && !child->isFloatingOrOutOfFlowPositioned() && child->isLayoutBlockFlow();
...
bool LayoutBlock::hasCursorCaret() const
{
    ...
    return frame->selection().caretLayoutObject() == this && frame->selection().hasEditableStyle();
}
    ...
    EDisplay display)
{
    ...
    RefPtr<ComputedStyle> newStyle = ComputedStyle::createAnonymousStyleWithDisplay(parent->styleRef(),
        newDisplay);
...
void LayoutBlock::checkPositionedObjectsNeedLayout()
{
    ...
    if (TrackedLayoutBoxListHashSet* positionedDescendantSet = positionedObjects()) {
        ...
        for (TrackedLayoutBoxListHashSet::const_iterator it = positionedDescendantSet->begin();
             it != end; ++it) {
...
LayoutUnit LayoutBlock::availableLogicalHeightForPercentageComputation() const
{
    ...
    if (isFlexItem())
        stretchedFlexHeight = toLayoutFlexibleBox(parent())
