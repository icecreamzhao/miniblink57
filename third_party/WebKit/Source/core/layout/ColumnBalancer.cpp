...
void ColumnBalancer::traverseChildren(const LayoutObject& object)
{
    ...
    for (const LayoutObject* child = object.slowFirstChild(); child;
        ...
        if (childBox.isFloating()) {
            ...
            logicalHeight = std::max(logicalHeight, childBox.logicalHeight() + marginAfter);
        ...
        if (m_flowThreadOffset + logicalTop + logicalHeight <= logicalTopInFlowThread()) {
        ...
        // table section.
        LayoutUnit offsetForThisChild = childBox.isTableRow() ? LayoutUnit() : logicalTop;
        ...
        if (childBox.getPaginationBreakability() != LayoutBox::ForbidBreaks && (!childBox.isLayoutBlockFlow() || !toLayoutBlockFlow(childBox).multiColumnFlowThread())) {
    ...
    EBreak previousBreakAfterValue)
{
    ...
    if (isLogicalTopWithinBounds(flowThreadOffset() - box.paginationStrut())) {
        ...
        } else if (isFirstAfterBreak(flowThreadOffset()) && m_lastBreakSeen != flowThreadOffset()) {
    ...
    if (box.getPaginationBreakability() != LayoutBox::AllowAnyBreaks) {
        m_tallestUnbreakableLogicalHeight = std::max(m_tallestUnbreakableLogicalHeight, childLogicalHeight);
        return;
    }
...
void InitialColumnHeightFinder::examineLine(const RootInlineBox& line)
{
    ...
    ASSERT(
        isFirstAfterBreak(lineTopInFlowThread) || !line.paginationStrut() || !isLogicalTopWithinBounds(lineTopInFlowThread - line.paginationStrut()));
    if (isFirstAfterBreak(lineTopInFlowThread) && m_lastBreakSeen != lineTopInFlowThread) {
...
{
    unsigned stopBeforeColumn = groupAtOffset(offsetInFlowThread)
    ...
    LayoutUnit endOffsetInFlowThread)
{
    ...
    if (!m_contentRuns.isEmpty() && endOffsetInFlowThread <= m_contentRuns.back().breakOffset())
        return;
    ...
    if (m_contentRuns.size() >= columnSet().usedColumnCount()) {
        ...
        if (!flowThread->enclosingFragmentationContext() || columnSet().newFragmentainerGroupsAllowed())
            return;
    }
    ...
    EBreak previousBreakAfterValue)
{
    ...
    if (breakability != LayoutBox::ForbidBreaks) {
        ...
        if (isFirstAfterBreak(flowThreadOffset()) || group.columnLogicalTopForOffset(flowThreadOffset()) != group.columnLogicalTopForOffset(bottomInFlowThread)) {
    ...
    if (!flowThread || flowThread->isLayoutPagedFlowThread())
        return;
    for (const LayoutMultiColumnSet* columnSet = flowThread->firstMultiColumnSet();
    ...
    LayoutUnit childLogicalHeight)
{
    ...
    recordSpaceShortage(logicalOffsetFromCurrentColumn + childLogicalHeight - m_pendingStrut);
    m_pendingStrut = LayoutUnit::min();
}
