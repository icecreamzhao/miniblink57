    ...
    CoordinateSpaceConversion mode) const
{
    ...
    if (LayoutMultiColumnFlowThread* enclosingFlowThread = flowThread->enclosingFlowThread()) {
        ...
        LayoutSize enclosingTranslationOrigin = enclosingFlowThread->flowThreadTranslationAtOffset(
    ...
    const LayoutRect& boundingBoxInFlowThread) const
{
    ...
    LayoutUnit boundingBoxLogicalBottom = isHorizontalWritingMode ? flippedBoundingBoxInFlowThread.maxY()
...
LayoutUnit MultiColumnFragmentainerGroup::calculateMaxColumnHeight() const
{
    ...
    if (FragmentationContext* enclosingFragmentationContext = flowThread->enclosingFragmentationContext()) {
        if (enclosingFragmentationContext->isFragmentainerLogicalHeightKnown()) {
            ...
            LayoutUnit remainingOuterLogicalHeight = enclosingFragmentationContext->remainingLogicalHeightAt(
                blockOffsetInEnclosingFragmentationContext());
    ...
    LayoutBox::PageBoundaryRule pageBoundaryRule) const
{
    ...
    if (!m_columnHeight)
        return 0;
    unsigned columnIndex = ((offsetInFlowThread - m_logicalTopInFlowThread) / m_columnHeight)
                               .floor();
    if (pageBoundaryRule == LayoutBox::AssociateWithFormerPage && columnIndex > 0 && logicalTopInFlowThreadAt(columnIndex) == offsetInFlowThread) {
    ...
    const LayoutPoint& visualPoint) const
{
    ...
    bool isHorizontalWritingMode = m_columnSet.isHorizontalWritingMode();
    LayoutUnit columnLengthInColumnProgressionDirection = isColumnProgressionInline ? m_columnSet.pageLogicalWidth()
                                                                                    : logicalHeight();
    LayoutUnit offsetInColumnProgressionDirection = isHorizontalWritingMode == isColumnProgressionInline ? visualPoint.x()
                                                                                                         : visualPoint.y();
