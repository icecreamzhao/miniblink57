...
enum SkipEmptySectionsValue { DoNotSkipEmptySections,
    SkipEmptySections };
...
public:
    ...
    unsigned absoluteColumnToEffectiveColumn(unsigned absoluteColumnIndex) const
    {
        ...
        for (unsigned c = m_noCellColspanAtLeast;
             effectiveColumn < numColumns && c + m_effectiveColumns[effectiveColumn].span - 1 < absoluteColumnIndex;
             ++effectiveColumn)
    ...
    enum WhatToMarkAllCells { MarkDirtyOnly,
