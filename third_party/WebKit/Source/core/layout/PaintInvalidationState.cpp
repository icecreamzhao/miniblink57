...
static bool supportsCachedOffsets(const LayoutObject& object)
{
    ...
    // for all children.)
    return !(object.hasTransformRelatedProperty() && !object.isPaintInvalidationContainer()) && !object.hasFilterInducingProperty() && !object.isLayoutFlowThread() && !object.isLayoutMultiColumnSpannerPlaceholder() && !object.styleRef().isFlippedBlocksWritingMode() && !(object.isLayoutBlock() && object.isSVG());
}
...
// RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled().
    ...
    if (currentObject.isPaintInvalidationContainer()) {
    ...
    } else if (currentObject.isFloating() && !currentObject.parent()->isLayoutBlock()) {
        // See LayoutObject::paintingLayer() for specialty of floating objects.
        m_paintInvalidationContainer = &currentObject.containerForPaintInvalidation();
        m_cachedOffsetsEnabled = false;
        ...
        currentObject.hasLayer() && m_paintInvalidationContainer != m_paintInvalidationContainerForStackedContents) {
    ...
    if (currentObject == m_paintInvalidationContainer) {
        ...
        if (currentObject != m_paintInvalidationContainerForStackedContents) {
            ...
            m_forcedSubtreeInvalidationFlags = 0;
            if (currentObject != m_containerForAbsolutePosition && m_cachedOffsetsForAbsolutePositionEnabled && m_cachedOffsetsEnabled) {
    ...
    const LayoutObject& child) const
{
    ...
    if (child.isColumnSpanAll() || (child.isFloating() && !m_currentObject.isLayoutBlock()))
    ...
    const PaintInvalidationState& parentState)
{
    ...
    if (position == AbsolutePosition) {
        ...
        if (container.isInFlowPositioned() && container.isLayoutInline())
            m_paintOffset += toLayoutInline(container).offsetForInFlowPositionedInline(
...
void PaintInvalidationState::updateForChildren(PaintInvalidationReason reason)
{
    ...
    case PaintInvalidationSVGResourceChange:
        m_forcedSubtreeInvalidationFlags |= PaintInvalidatorContext::ForcedSubtreeSVGResourceChange;
        break;
    default:
        break;
    }
    ...
    if (box.isLayoutView()) {
        if (!RuntimeEnabledFeatures::rootLayerScrollingEnabled()) {
            if (box != m_paintInvalidationContainer) {
                m_paintOffset -= LayoutSize(toLayoutView(box).frameView()->getScrollOffset());
