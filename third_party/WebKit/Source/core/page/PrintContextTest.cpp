...
public:
    MockPrintContext(LocalFrame* frame)
        : PrintContext(frame)
    {
    }
...
public:
    ...
    MockCanvas()
        : SkCanvas(kPageWidth, kPageHeight)
    {
    }
...
protected:
    ...
    void printSinglePage(SkCanvas& canvas)
    {
        ...
        {
            DrawingRecorder recorder(context, *document().layoutView(),
...
TEST_F(PrintContextTest, LinkTarget)
{
    MockCanvas canvas;
    setBodyInnerHTML(
        absoluteBlockHtmlForLink(50, 60, 70, 80, "http://www.google.com") + absoluteBlockHtmlForLink(150, 160, 170, 180, "http://www.google.com#fragment"));
...
{
    MockCanvas canvas;
    setBodyInnerHTML("<div style='padding-top: 50px'>" + inlineHtmlForLink("http://www.google.com", "<img style='width: 111; height: 10'>") + "<div> " + inlineHtmlForLink("http://www.google1.com", "<img style='width: 122; height: 20'>") + "</div>" + "</div>");
    printSinglePage(canvas);
    const Vector<MockCanvas::Operation>& operations = canvas.recordedOperations();
    ASSERT_EQ(2u, operations.size());
    EXPECT_EQ(MockCanvas::DrawRect, operations[0].type);
...
TEST_F(PrintContextTest, LinkTargetContainingABlock)
{
    MockCanvas canvas;
    setBodyInnerHTML(
        "<div style='padding-top: 50px'>" + inlineHtmlForLink("http://www.google2.com", "<div style='width:133; height: 30'>BLOCK</div>") + "</div>");
    printSinglePage(canvas);
    const Vector<MockCanvas::Operation>& operations = canvas.recordedOperations();
...
TEST_F(PrintContextTest, LinkTargetUnderInInlines)
{
    MockCanvas canvas;
    setBodyInnerHTML(
        "<span><b><i><img style='width: 40px; height: 40px'><br>" + inlineHtmlForLink("http://www.google3.com", "<img style='width: 144px; height: 40px'>") + "</i></b></span>");
    printSinglePage(canvas);
    const Vector<MockCanvas::Operation>& operations = canvas.recordedOperations();
    ASSERT_EQ(1u, operations.size());
    EXPECT_EQ(MockCanvas::DrawRect, operations[0].type);
...
TEST_F(PrintContextTest, EmptyLinkedTarget)
{
    MockCanvas canvas;
    document().setBaseURLOverride(KURL(ParsedURLString, "http://a.com/"));
    setBodyInnerHTML(absoluteBlockHtmlForLink(50, 60, 70, 80, "#fragment") + htmlForAnchor(250, 260, "fragment", ""));
    printSinglePage(canvas);
...
TEST_F(PrintContextFrameTest, WithSubframe)
{
    ...
    setBodyInnerHTML(
        ...
        "left: 90px'></iframe>");
    setChildFrameHTML(
        absoluteBlockHtmlForLink(50, 60, 70, 80, "#fragment") + absoluteBlockHtmlForLink(150, 160, 170, 180, "http://www.google.com") + absoluteBlockHtmlForLink(250, 260, 270, 280, "http://www.google.com#fragment"));
...
TEST_F(PrintContextFrameTest, WithScrolledSubframe)
{
    ...
    setBodyInnerHTML(
        ...
        "left: 90px'></iframe>");
    setChildFrameHTML(
        absoluteBlockHtmlForLink(10, 10, 20, 20, "http://invisible.com") + absoluteBlockHtmlForLink(50, 60, 70, 80, "http://partly.visible.com") + absoluteBlockHtmlForLink(150, 160, 170, 180, "http://www.google.com") + absoluteBlockHtmlForLink(250, 260, 270, 280, "http://www.google.com#fragment") + absoluteBlockHtmlForLink(850, 860, 70, 80, "http://another.invisible.com"));
