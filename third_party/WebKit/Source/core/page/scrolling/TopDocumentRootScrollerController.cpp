...
void TopDocumentRootScrollerController::mainFrameViewResized()
{
    ...
    ScrollableArea* area = RootScrollerUtil::scrollableAreaForRootScroller(rootScroller);
...
IntSize TopDocumentRootScrollerController::rootScrollerVisibleArea() const
{
    ...
    float minimumPageScale = m_frameHost->pageScaleConstraintsSet().finalConstraints().minimumScale;
    int browserControlsAdjustment = ceilf(m_frameHost->visualViewport().browserControlsAdjustment() / minimumPageScale);

    return topDocument()->view()->visibleContentSize(ExcludeScrollbars) + IntSize(0, browserControlsAdjustment);
}
...
Element* TopDocumentRootScrollerController::findGlobalRootScrollerElement()
{
    ...
    Node* effectiveRootScroller = &topDocument()->rootScrollerController().effectiveRootScroller();
    ...
    while (element && element->isFrameOwnerElement()) {
        ...
        effectiveRootScroller = &iframeDocument->rootScrollerController().effectiveRootScroller();
        if (effectiveRootScroller->isDocumentNode())
...
void TopDocumentRootScrollerController::recomputeGlobalRootScroller()
{
    ...
    ScrollableArea* targetScroller = RootScrollerUtil::scrollableAreaForRootScroller(target);
    ...
    ScrollableArea* oldRootScrollerArea = RootScrollerUtil::scrollableAreaForRootScroller(m_globalRootScroller);
...
Document* TopDocumentRootScrollerController::topDocument() const
{
    ...
    if (!m_frameHost->page().mainFrame() || !m_frameHost->page().mainFrame()->isLocalFrame())
        return nullptr;
...
{
    ScrollableArea* area = RootScrollerUtil::scrollableAreaForRootScroller(m_globalRootScroller);
