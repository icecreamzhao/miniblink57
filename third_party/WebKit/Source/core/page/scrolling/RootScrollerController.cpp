...
namespace {
    ...
    bool fillsViewport(const Element& element)
    {
        ...
        return boundingBox.location() == LayoutPoint::zero() && boundingBox.size() == topDocument.layoutViewItem().size();
    }
...
{
    bool rootScrollerValid = m_rootScroller && isValidRootScroller(*m_rootScroller);
