    ...
    const String& message)
{
    for (Frame* frame = mainFrame; frame; frame = frame->tree().traverseNext()) {
        ...
        Document::PageDismissalType dismissal = localFrame.document()->pageDismissalEventBeingDispatched();
...
{
    if (!result.scrollbar() && result.innerNode() && result.innerNode()->document().isDNSPrefetchEnabled())
...
void ChromeClient::setToolTip(LocalFrame& frame, const HitTestResult& result)
{
    ...
    if (result.innerNodeOrImageMapImage() != m_lastMouseOverNode && !m_lastToolTipText.isEmpty() && toolTip == m_lastToolTipText)
        clearToolTip(frame);
