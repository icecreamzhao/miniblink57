...
namespace {
    ...
    bool isVisible(const Element& element)
    {
        ...
        if (!style)
            return false;
        return (style->display() != EDisplay::None && style->visibility() != EVisibility::kHidden && style->opacity() != 0);
    }
    ...
    bool matchAttributes(const Element& element, const Vector<String>& words)
    {
        ...
        for (const String& word : words) {
            if (classes.findIgnoringCase(word) != WTF::kNotFound || id.findIgnoringCase(word) != WTF::kNotFound) {
                return true;
            }
        }
        return false;
        ...
        const Element& element)
    {
        ...
        if (!isVisible(element))
            return false;
        if (features.mozScore >= kMozScoreSaturation && features.mozScoreAllSqrt >= kMozScoreAllSqrtSaturation && features.mozScoreAllLinear >= kMozScoreAllLinearSaturation)
            return false;
