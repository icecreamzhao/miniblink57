...
namespace {
    ...
    TEST_F(ElementVisibilityObserverTest, ObserveElementWithoutDocumentFrame)
    {
        ...
        ElementVisibilityObserver* observer = new ElementVisibilityObserver(element, nullptr);
        observer->start();
        observer->stop();
        // It should not crash.
    }
    ...
    {
        Persistent<RemoteFrame> remoteFrame = RemoteFrame::create(new EmptyRemoteFrameClient(), &frameHost(), nullptr);
        frameLoaderClient()->setParent(remoteFrame);

        Persistent<HTMLElement> element = HTMLDivElement::create(document());
        ElementVisibilityObserver* observer = new ElementVisibilityObserver(element, WTF::bind([](bool) {}));
        observer->start();
