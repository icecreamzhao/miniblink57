    ...
    ExceptionState& exceptionState)
{
    ...
    if (matrix->is2D() && z() == 0 && w() == 1) {
        double transformedX = x() * matrix->m11() + y() * matrix->m12() + matrix->m41();
        double transformedY = x() * matrix->m12() + y() * matrix->m22() + matrix->m42();
        return DOMPoint::create(transformedX, transformedY, 0, 1);
    }

    double transformedX = x() * matrix->m11() + y() * matrix->m21() + z() * matrix->m31() + w() * matrix->m41();
    double transformedY = x() * matrix->m12() + y() * matrix->m22() + z() * matrix->m32() + w() * matrix->m42();
    double transformedZ = x() * matrix->m13() + y() * matrix->m23() + z() * matrix->m33() + w() * matrix->m43();
    double transformedW = x() * matrix->m14() + y() * matrix->m24() + z() * matrix->m34() + w() * matrix->m44();
