    ...
    const String& text)
{
    ...
    bool passesContentSecurityPolicyChecks = shouldBypassMainWorldCSP(element) || csp->allowStyleWithHash(text, ContentSecurityPolicy::InlineType::Block) || csp->allowInlineStyle(&element, document.url(), element.fastGetAttribute(HTMLNames::nonceAttr), m_startPosition.m_line, text);
    ...
    if (isCSS(element, type) && passesContentSecurityPolicyChecks) {
        ...
        if (screenEval.eval(mediaQueries) || printEval.eval(mediaQueries)) {
            m_loading = true;
            TextPosition startPosition = m_startPosition == TextPosition::belowRangePosition()
