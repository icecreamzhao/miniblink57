...
{
    if (Element* nearestInclusiveAncestorAssignedToSlot = SlotScopedTraversal::nearestInclusiveAncestorAssignedToSlot(current))
...
{
    Element* nearestInclusiveAncestorAssignedToSlot = SlotScopedTraversal::nearestInclusiveAncestorAssignedToSlot(current);
    ...
    HTMLSlotElement* slot = nearestInclusiveAncestorAssignedToSlot->assignedSlot();
    DCHECK(slot);
    const HeapVector<Member<Node>>& assignedNodes = slot->assignedNodes();
    size_t currentIndex = assignedNodes.find(*nearestInclusiveAncestorAssignedToSlot);
...
Element* SlotScopedTraversal::previous(const Element& current)
{
    ...
    const HeapVector<Member<Node>>& assignedNodes = nearestInclusiveAncestorAssignedToSlot->assignedSlot()->assignedNodes();
    size_t currentIndex = assignedNodes.reverseFind(*nearestInclusiveAncestorAssignedToSlot);
    DCHECK_NE(currentIndex, kNotFound);
