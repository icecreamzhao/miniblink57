...
namespace {
    ...
    public:
        explicit EmptyMutationCallback(Document& document)
            : m_document(document)
        {
        }
...
TEST(MutationObserverTest, DisconnectCrash)
{
    ...
    DCHECK(head);
    Persistent<MutationObserver> observer = MutationObserver::create(new EmptyMutationCallback(*document));
    MutationObserverInit init;
    init.setCharacterDataOldValue(false);
    observer->observe(head, init, ASSERT_NO_EXCEPTION);

    head->remove();
    Persistent<MutationObserverRegistration> registration = observer->m_registrations.begin()->get();
