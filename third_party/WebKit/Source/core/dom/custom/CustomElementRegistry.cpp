    ...
    const LocalDOMWindow* owner)
{
    ...
    if (V0CustomElementRegistrationContext* v0 = document ? document->registrationContext() : nullptr)
    ...
    ExceptionState& exceptionState)
{
    ...
    if (RuntimeEnabledFeatures::customElementsBuiltinEnabled() && options.hasExtends()) {
        ...
        if (htmlElementTypeForTag(extends) == HTMLElementType::kHTMLUnknownElement) {
            exceptionState.throwDOMException(
    ...
    DefinitionMap::AddResult result = m_definitions.add(descriptor.name(), definition);
    ...
    ExceptionState& exceptionState)
{
    ...
    ScriptPromiseResolver* newResolver = ScriptPromiseResolver::create(scriptState);
