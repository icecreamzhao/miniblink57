...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, PrecedingText)
{
    ...
    // SOT + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRSRF", processSequenceForward(&machine, asCodePoints(kRisU), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // SOT + [U] + [S] + | + 'a' + 'a'
    EXPECT_EQ("RRRRSRF",
    ...
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // U+0000 + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRSRF", processSequenceForward(&machine, asCodePoints(0, kRisU), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // U+0000 + [U] + [S] + | + 'a' + 'a'
    EXPECT_EQ("RRRRSRF",
    ...
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // 'a' + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRSRF", processSequenceForward(&machine, asCodePoints('a', kRisU), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // 'a' + [U] + [S] + | + 'a' + 'a'
    EXPECT_EQ("RRRRSRF",
    ...
    // U+1F441 + | + 'a' + 'a'
    EXPECT_EQ("RSRF", processSequenceForward(&machine, asCodePoints(kEye), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // U+1F441 + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRRSRF",
    ...
    // [Lead] + | + 'a' + 'a'
    EXPECT_EQ("SRF", processSequenceForward(&machine, asCodePoints(kLead), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // [Lead] + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRSRF",
    ...
    // 'a' + [Trail] + | + 'a' + 'a'
    EXPECT_EQ("RSRF", processSequenceForward(&machine, asCodePoints('a', kTrail), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // 'a' + [Trail] + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRRSRF",
    ...
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // 'a' + [Trail] + [U] + [S] + | + 'a' + 'a'
    EXPECT_EQ("RRRRRSRF", processSequenceForward(&machine, asCodePoints('a', kTrail, kRisU, kRisS), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // [Trail] + [Trail] + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRRSRF", processSequenceForward(&machine, asCodePoints(kTrail, kTrail, kRisU), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // [Trail] + [Trail] + [U] + [S] + | + 'a' + 'a'
    EXPECT_EQ("RRRRRSRF",
    ...
    // SOT + [Trail] + | + 'a' + 'a'
    EXPECT_EQ("RSRF", processSequenceForward(&machine, asCodePoints(kTrail), asCodePoints('a', 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // SOT + [Trail] + [U] + | + 'a' + 'a'
    EXPECT_EQ("RRRSRF",
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, BrokenSurrogatePair)
{
    ...
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // SOT + | + [Lead] + [Lead]
    EXPECT_EQ("SRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kLead, kLead)));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    // SOT + | + [Lead] + EOT
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, BreakImmediately_BMP)
{
    ...
    // SOT + | + 'a' + [Lead] + 'a'
    EXPECT_EQ("SRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints('a', kLead, 'a')));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + 'a' + [Lead] + [Lead]
    EXPECT_EQ("SRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints('a', kLead, kLead)));
    EXPECT_EQ(1, machine.finalizeAndGetBoundaryOffset());
    ...
    BreakImmediately_Supplementary)
{
    ...
    // SOT + | + U+1F441 + U+1F441
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kEye)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
    ...
    // SOT + | + U+1F441 + [Trail]
    EXPECT_EQ("SRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kTrail)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + [Lead] + 'a'
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kLead, 'a')));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + [Lead] + [Lead]
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kLead, kLead)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + [Lead] + EOT
    EXPECT_EQ("SRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kLead)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyAfter_BMP_BMP)
{
    ...
    // SOT + | + U+231A + U+FE0F + 'a'
    EXPECT_EQ("SRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kWatch, kVS16, 'a')));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+231A + U+FE0F + U+1F441
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kWatch, kVS16, kEye)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+231A + U+FE0F + EOT
    EXPECT_EQ("SRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kWatch, kVS16)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("SRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kWatch, kVS16, kLead)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyAfter_Supplementary_BMP)
{
    ...
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kVS16, 'a')));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + U+FE0F + U+1F441
    EXPECT_EQ("SRRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kVS16, kEye)));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + U+FE0F + EOT
    EXPECT_EQ("SRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kVS16)));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());

    // SOT + | + U+1F441 + U+FE0F + [Trail]
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kVS16, kTrail)));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("SRRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kEye, kVS16, kLead)));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyAfter_BMP_Supplementary)
{
    ...
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kHanBMP, kVS17, 'a')));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("SRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kHanBMP, kVS17)));
    EXPECT_EQ(3, machine.finalizeAndGetBoundaryOffset());
    ...
    NotBreakImmediatelyAfter_Supplementary_Supplementary)
{
    ...
    EXPECT_EQ("SRRRR", processSequenceForward(&machine, asCodePoints(), asCodePoints(kHanSIP, kVS17)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, MuchLongerCase)
{
    ...
    EXPECT_EQ("SRRRRRRRRRRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan, kLead, 'a')));
    EXPECT_EQ(11, machine.finalizeAndGetBoundaryOffset());
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, singleFlags)
{
    ...
    // SOT + | + [U] + [S]
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());

    // 'a' + | + [U] + [S]
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints('a'), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + | + [U] + [S]
    EXPECT_EQ("RSRRRF", processSequenceForward(&machine, asCodePoints(kEye), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());

    // [Lead] + | + [U] + [S]
    EXPECT_EQ("SRRRF", processSequenceForward(&machine, asCodePoints(kLead), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());
    ...
    // SOT + [Trail] + | + [U] + [S]
    EXPECT_EQ("RSRRRF", processSequenceForward(&machine, asCodePoints(kTrail), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());
}
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, twoFlags)
{
    ...
    // 'a' + [Trail] + [U] + [S] + | + [U] + [S]
    EXPECT_EQ("RRRRRSRRRF", processSequenceForward(&machine, asCodePoints('a', kTrail, kRisU, kRisS), asCodePoints(kRisU, kRisS)));
    EXPECT_EQ(4, machine.finalizeAndGetBoundaryOffset());
...
TEST_F(ForwardGraphemeBoundaryStatemachineTest, oddNumberedFlags)
{
    ...
    // SOT + [U] + | + [S] + [S]
    EXPECT_EQ("RRSRRRF", processSequenceForward(&machine, asCodePoints(kRisU), asCodePoints(kRisS, kRisU)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
    ...
    // [Trail] + [Trail] + [U] + | + [S] + [S]
    EXPECT_EQ("RRRSRRRF", processSequenceForward(&machine, asCodePoints(kTrail, kTrail, kRisU), asCodePoints(kRisS, kRisU)));
    EXPECT_EQ(2, machine.finalizeAndGetBoundaryOffset());
