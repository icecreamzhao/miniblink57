...
TEST_F(BackwardGraphemeBoundaryStatemachineTest, BreakImmediately_BMP)
{
    ...
    // [Trail] + [Trail] + 'a'
    EXPECT_EQ("RRF", processSequenceBackward(&machine, asCodePoints(kTrail, kTrail, 'a')));
    EXPECT_EQ(-1, machine.finalizeAndGetBoundaryOffset());
    ...
    BreakImmediately_SupplementaryPlane)
{
    ...
    // [Trail] + [Trail] + U+1F441
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kTrail, kEye)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());
    ...
    NotBreakImmediatelyBefore_BMP_BMP)
{
    ...
    // 'a' + U+231A + U+FE0F
    EXPECT_EQ("RRF", processSequenceBackward(&machine, asCodePoints('a', kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + U+231A + U+FE0F
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kEye, kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());
    ...
    // [Lead] + U+231A + U+FE0F
    EXPECT_EQ("RRF", processSequenceBackward(&machine, asCodePoints(kLead, kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + U+231A + U+FE0F
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // [Trail] + [Trail] + U+231A + U+FE0F
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kTrail, kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // SOT + [Trail] + U+231A + U+FE0F
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kWatch, kVS16)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyBefore_Supplementary_BMP)
{
    ...
    // [Lead] + U+1F441 + U+FE0F
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kLead, kEye, kVS16)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + U+1F441 + U+FE0F
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kEye, kVS16)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());

    // [Trail] + [Trail] + U+1F441 + U+FE0F
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kTrail, kEye, kVS16)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());

    // SOT + [Trail] + U+1F441 + U+FE0F
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kEye, kVS16)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyBefore_BMP_Supplementary)
{
    ...
    // 'a' + U+845B + U+E0100
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints('a', kHanBMP, kVS17)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + U+845B + U+E0100
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kEye, kHanBMP, kVS17)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());
    ...
    // [Lead] + U+845B + U+E0100
    EXPECT_EQ("RRRF", processSequenceBackward(&machine, asCodePoints(kLead, kHanBMP, kVS17)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + U+845B + U+E0100
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kHanBMP, kVS17)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kHanBMP, kVS17)));
    EXPECT_EQ(-3, machine.finalizeAndGetBoundaryOffset());
}
    ...
    NotBreakImmediatelyBefore_Supplementary_Supplementary)
{
    ...
    // 'a' + U+20000 + U+E0100
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints('a', kHanSIP, kVS17)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + U+20000 + U+E0100
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints(kEye, kHanSIP, kVS17)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kLead, kHanSIP, kVS17)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + U+20000 + U+E0100
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kHanSIP, kVS17)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
    ...
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kHanSIP, kVS17)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
}
...
TEST_F(BackwardGraphemeBoundaryStatemachineTest, MuchLongerCase)
{
    ...
    EXPECT_EQ("RRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints('a', kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + ZWJ Emoji Sequence
    EXPECT_EQ("RRRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints(kEye, kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // SOT + ZWJ Emoji Sequence
    EXPECT_EQ(
        "RRRRRRRRRRRF",
        processSequenceBackward(&machine, asCodePoints(kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // [Lead] + ZWJ Emoji Sequence
    EXPECT_EQ("RRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints(kLead, kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + ZWJ Emoji Sequence
    EXPECT_EQ("RRRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints('a', kTrail, kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // [Trail] + [Trail] + ZWJ Emoji Sequence
    EXPECT_EQ("RRRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints(kTrail, kTrail, kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());

    // SOT + [Trail] + ZWJ Emoji Sequence
    EXPECT_EQ("RRRRRRRRRRRRF",
        processSequenceBackward(
            &machine, asCodePoints(kTrail, kMan, kZwj, kHeart, kVS16, kZwj, kKiss, kZwj, kMan)));
    EXPECT_EQ(-11, machine.finalizeAndGetBoundaryOffset());
}
...
TEST_F(BackwardGraphemeBoundaryStatemachineTest, Flags_singleFlag)
{
    ...
    // U+1F441 + [U] + [S]
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints(kEye, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
    ...
    // [Lead] + [U] + [S]
    EXPECT_EQ("RRRRF", processSequenceBackward(&machine, asCodePoints(kLead, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());

    // 'a' + [Trail] + [U] + [S]
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
    ...
    // SOT + [Trail] + [U] + [S]
    EXPECT_EQ("RRRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
}
...
TEST_F(BackwardGraphemeBoundaryStatemachineTest, Flags_twoFlags)
{
    ...
    // 'a' + [Trail] + [U] + [S] + [U] + [S]
    EXPECT_EQ("RRRRRRRRRF", processSequenceBackward(&machine, asCodePoints('a', kTrail, kRisU, kRisS, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());

    // [Trail] + [Trail] + [U] + [S] + [U] + [S]
    EXPECT_EQ("RRRRRRRRRF", processSequenceBackward(&machine, asCodePoints(kTrail, kTrail, kRisU, kRisS, kRisU, kRisS)));
    EXPECT_EQ(-4, machine.finalizeAndGetBoundaryOffset());
...
TEST_F(BackwardGraphemeBoundaryStatemachineTest, Flags_oddNumberedRIS)
{
    ...
    // 'a' + [U] + [S] + [U]
    EXPECT_EQ("RRRRRRF", processSequenceBackward(&machine, asCodePoints('a', kRisU, kRisS, kRisU)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // U+1F441 + [U] + [S] + [U]
    EXPECT_EQ("RRRRRRRF", processSequenceBackward(&machine, asCodePoints(kEye, kRisU, kRisS, kRisU)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // SOT + [U] + [S] + [U]
    EXPECT_EQ("RRRRRRF", processSequenceBackward(&machine, asCodePoints(kRisU, kRisS, kRisU)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());

    // [Lead] + [U] + [S] + [U]
    EXPECT_EQ("RRRRRRF", processSequenceBackward(&machine, asCodePoints(kLead, kRisU, kRisS, kRisU)));
    EXPECT_EQ(-2, machine.finalizeAndGetBoundaryOffset());
