...
ForwardGraphemeBoundaryStateMachine::feedPrecedingCodeUnit(UChar codeUnit)
{
    ...
    case InternalState::CountRISWaitLeadSurrogate:
        ...
        if (U16_IS_LEAD(codeUnit)) {
            const UChar32 codePoint = U16_GET_SUPPLEMENTARY(codeUnit, m_pendingCodeUnit);
            m_pendingCodeUnit = 0;
...
ForwardGraphemeBoundaryStateMachine::feedFollowingCodeUnit(UChar codeUnit)
{
    ...
    case InternalState::SearchWaitTrailSurrogate:
        ...
        {
            const UChar32 codePoint = U16_GET_SUPPLEMENTARY(m_pendingCodeUnit, codeUnit);
            m_pendingCodeUnit = 0;
            if (Character::isRegionalIndicator(m_prevCodePoint) && Character::isRegionalIndicator(codePoint)) {
...
{
    DCHECK(m_internalState == InternalState::CountRIS || m_internalState == InternalState::CountRISWaitLeadSurrogate)
