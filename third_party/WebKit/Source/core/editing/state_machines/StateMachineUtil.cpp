...
namespace {
    ...
    {
        return codePoint == WTF::Unicode::boyCharacter || codePoint == WTF::Unicode::girlCharacter || codePoint == WTF::Unicode::manCharacter || codePoint == WTF::Unicode::womanCharacter;
    }
...
bool isGraphemeBreak(UChar32 prevCodePoint, UChar32 nextCodePoint)
{
    ...
    // Rule GB3, CR x LF
    if (prevCodePoint == 0xD && nextCodePoint == 0xA)
        return false;

    // Rule GB4, (Control | CR | LF) ÷
    if (prevCodePoint == 0xD || prevCodePoint == 0xA)
        return true;
