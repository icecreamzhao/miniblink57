...
void InputMethodControllerTest::createHTMLWithCompositionInputEventListeners()
{
    ...
    Element* editable = insertHTMLElement("<div id='sample' contenteditable></div>", "sample");
...
{
    HTMLInputElement* input = toHTMLInputElement(insertHTMLElement("<input id='sample'>", "sample"));
...
{
    Element* div = insertHTMLElement("<div id='sample' contenteditable></div>", "sample");
...
{
    Element* inputA = insertHTMLElement("<input id='a' /><br><input type='tel' id='b' />", "a");
    ...
    CompositionUnderlineAppearsCorrectlyAfterNewline)
{
    ...
    ASSERT_EQ(0u, document().markers().markersInRange(PlainTextRange(0, 5).createRange(*div), DocumentMarker::AllMarkers()).size());
    ASSERT_EQ(1u, document().markers().markersInRange(PlainTextRange(6, 11).createRange(*div), DocumentMarker::AllMarkers()).size());
...
TEST_F(InputMethodControllerTest, SelectionWhenFocusChangeFinishesComposition)
{
    ...
    EXPECT_EQ(0, controller().compositionRange()->startOffset());
    EXPECT_EQ(3, controller().compositionRange()->endOffset());
    EXPECT_EQ(3, frame().selection().start().computeOffsetInContainerNode());
