...
TEST_F(SpellCheckerTest, SpellCheckDoesNotCauseUpdateLayout)
{
    ...
    HTMLInputElement* input = toHTMLInputElement(document().querySelector("input"));
    input->focus();
    ...
    document().frame()->selection().setSelection(
        ...
        FrameSelection::CloseTyping | FrameSelection::ClearTypingStyle | FrameSelection::DoNotUpdateAppearance);
