...
namespace {
        ...
        const String& newText)
    {
        ...
        if (!selectionStart.computeContainerNode() || !selectionStart.computeContainerNode()->parentNode())
            return 0;

        const ContainerNode* selectionNode = selectionStart.computeContainerNode()->parentNode();

        // For grapheme cluster, we should adjust it for grapheme boundary.
        const EphemeralRange& range = PlainTextRange(0, selectionOffset + commonPrefixLength)
        ...
        const String& newText)
    {
        ...
        // For grapheme cluster, we should adjust it for grapheme boundary.
        const EphemeralRange& range = PlainTextRange(0, selectionOffset + oldText.length() - commonSuffixLength)
                                          .createRange(*selectionNode);
        if (range.isNull())
            return 0;
        const Position& position = range.endPosition();
        ...
        const bool isDirectional)
    {
        ...
        const EphemeralRange& rangeForInsertion = charIt.calculateCharacterSubrange(offset, length);
        const VisibleSelection& selection = createVisibleSelection(SelectionInDOMTree::Builder()
...
void InsertIncrementalTextCommand::doApply(EditingState* editingState)
{
    ...
    m_text = computeTextForInsertion(m_text, commonPrefixLength, commonSuffixLength);

    const int offset = static_cast<int>(commonPrefixLength);
    const int length = static_cast<int>(oldTextLength - commonPrefixLength - commonSuffixLength);
