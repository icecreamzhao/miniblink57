...
{
    if ((m_permittedState == WebBrowserControlsShown && pendingDelta.height() > 0) || (m_permittedState == WebBrowserControlsHidden && pendingDelta.height() < 0))
        return pendingDelta;
    ...
    bool animate)
{
    ...
    DCHECK(!(constraints == WebBrowserControlsShown && current == WebBrowserControlsHidden));
    DCHECK(!(constraints == WebBrowserControlsHidden && current == WebBrowserControlsShown));
    ...
    if (constraints == WebBrowserControlsBoth && current == WebBrowserControlsBoth)
        return;

    if (constraints == WebBrowserControlsHidden || current == WebBrowserControlsHidden)
        setShownRatio(0.f);
    else
        setShownRatio(1.f);
}
