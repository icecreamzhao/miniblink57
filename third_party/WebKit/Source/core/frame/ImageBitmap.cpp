...
namespace {
        ...
        IntSize sourceSize)
    {
        ...
        if (options.colorSpaceConversion() != "none") {
            if (!RuntimeEnabledFeatures::experimentalCanvasFeaturesEnabled() || !RuntimeEnabledFeatures::colorCorrectRenderingEnabled()) {
                ...
                if (options.colorSpaceConversion() == "default" || options.colorSpaceConversion() == "srgb") {
                    parsedOptions.dstColorSpace = SkColorSpace::MakeNamed(SkColorSpace::kSRGB_Named);
                    parsedOptions.dstColorType = SkColorType::kN32_SkColorType;
...
static sk_sp<SkImage> flipSkImageVertically(
    SkImage* input,
    AlphaPremultiplyEnforcement premultiplyEnforcement = DontEnforceAlphaPremultiply,
    const ParsedOptions& options = defaultOptions())
{
    unsigned width = static_cast<unsigned>(input->width());
    unsigned height = static_cast<unsigned>(input->height());
    SkAlphaType alphaType = ((premultiplyEnforcement == EnforceAlphaPremultiply) || options.premultiplyAlpha)
        ? kPremul_SkAlphaType
    ...
    ParsedOptions& options)
{
    ...
    if (options.latestColorSpace) {
        ...
        if (SkColorTypeBytesPerPixel(options.dstColorType) != SkColorTypeBytesPerPixel(options.latestColorType)) {
            size = image->width() * image->height() * SkColorTypeBytesPerPixel(options.latestColorType);
        ...
        SkImageInfo dstInfo = SkImageInfo::Make(image->width(), image->height(), options.dstColorType,
    ...
    ExceptionState& exceptionState)
{
    if (!sourceWidth || !sourceHeight) {
        exceptionState.throwDOMException(
            IndexSizeError, String::format("The source %s provided is 0.", sourceWidth ? "height" : "width"));
        return false;
    }
    return true;
}
    ...
    ColorBehavior colorBehavior)
{
    ...
    if (srcRect.isEmpty() && !parsedOptions.premultiplyAlpha) {
        SkImageInfo info = SkImageInfo::Make(parsedOptions.resizeWidth, parsedOptions.resizeHeight,
    ...
    sk_sp<SkImage> skiaImage = image->imageForCurrentFrame(ColorBehavior::transformToGlobalTarget());
    // Attempt to get raw unpremultiplied image data, executed only when skiaImage
    // is premultiplied.
    if ((((!parsedOptions.premultiplyAlpha && !skiaImage->isOpaque()) || !skiaImage) && image->data() && imageFormat == PremultiplyAlpha) || colorBehavior.isIgnore()) {
    ...
    const ImageBitmapOptions& options)
{
    ...
    // TODO(ccameron): Canvas should operate in sRGB and not display space.
    // https://crbug.com/667431
    sk_sp<SkImage> skImage = m_image->imageForCurrentFrame(ColorBehavior::transformToGlobalTarget());
    SkPixmap pixmap;
    ...
    const ImageBitmapOptions& options)
{
    ...
    ParsedOptions parsedOptions = parseOptions(options, cropRect, IntSize(input->width(), input->height()));
    if (dstBufferSizeHasOverflow(parsedOptions))
        return;
    ...
    const ImageBitmapOptions& options)
{
    ...
    if (!parsedOptions.premultiplyAlpha) {
        ...
        unsigned dstPixelBytesPerRow = bytesPerPixel * parsedOptions.cropRect.width();
        ...
        if (parsedOptions.cropRect == IntRect(IntPoint(), data->size())) {
            ...
            RefPtr<ArrayBuffer> dstBuffer = ArrayBuffer::createOrNull(
                static_cast<unsigned>(parsedOptions.cropRect.height()) * parsedOptions.cropRect.width(),
                bytesPerPixel);
            if (!dstBuffer)
                return;
            ...
            if (!srcRect.isEmpty()) {
                ...
                for (int i = 0; i < copyHeight; i++) {
                    unsigned srcStartCopyPosition = (i + srcPoint.y()) * srcPixelBytesPerRow + srcPoint.x() * bytesPerPixel;
                    unsigned srcEndCopyPosition = srcStartCopyPosition + copyWidth * bytesPerPixel;
...
ScriptPromise ImageBitmap::createImageBitmap(ScriptState* scriptState,
    ...
    ExceptionState& exceptionState)
{
    if ((cropRect && !isSourceSizeValid(cropRect->width(), cropRect->height(), exceptionState)) || !isSourceSizeValid(width(), height(), exceptionState))
