    ...
    const Resource& resource)
{
    ...
    if (!resource.isEligibleForIntegrityCheck(document.getSecurityOrigin())) {
        ...
        logErrorToConsole("Subresource Integrity: The resource '" + resourceUrl.elidedString() + "' has an integrity attribute, but the resource "
    ...
    String& errorMessage)
{
    ...
    if (computeDigest(HashAlgorithmSha256, content, size, digest)) {
        ...
        errorMessage = "Failed to find a valid digest in the 'integrity' attribute for "
                       "resource '"
            + resourceUrl.elidedString() + "' with computed SHA-256 integrity '" + digestToString(digest) + "'. The resource has been blocked.";
    } else {
    ...
    ExecutionContext* executionContext)
{
    ...
    while (position < end) {
        ...
        if (parseResult == AlgorithmUnknown) {
            ...
            if (executionContext) {
                logErrorToConsole("Error parsing 'integrity' attribute ('" + attribute + "'). The specified hash algorithm must be one of "
        ...
        if (parseResult == AlgorithmUnparsable) {
            ...
            skipUntil<UChar, isASCIISpace>(position, end);
            if (executionContext) {
                logErrorToConsole("Error parsing 'integrity' attribute ('" + attribute + "'). The hash algorithm must be one of 'sha256', "
        ...
        if (skipExactly<UChar>(position, end, '?')) {
            ...
            if (begin != position && executionContext) {
                logErrorToConsole(
                    "Ignoring unrecogized 'integrity' attribute option '" + String(begin, position - begin) + "'.",
                    *executionContext);
            }
        }
