...
void FrameViewAutoSizeInfo::configureAutoSizeMode(const IntSize& minSize,
...
void FrameViewAutoSizeInfo::autoSizeIfNeeded()
{
    ...
    // allow it to grow.
    if (!m_didRunAutosize)
        m_frameView->resize(m_frameView->frameRect().width(),
            m_minAutoSize.height());
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (m_didRunAutosize && size.height() <= m_maxAutoSize.height() && size.width() <= m_maxAutoSize.width() && !m_frameView->frame().document()->loadEventFinished() && (newSize.height() < size.height() || newSize.width() < size.width()))
            break;
