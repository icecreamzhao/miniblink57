...
FrameView::FrameView(LocalFrame& frame)
    ...
    m_textAutosizerCheckTimer(this, &FrameView::checkTextAutosizerTimerFired)
    ,
#endif
...
void FrameView::setupRenderThrottling()
{
    ...
    m_visibilityObserver = new ElementVisibilityObserver(
        targetElement, [frameView](bool isVisible) {
            if (!frameView)
                return;
            frameView->updateRenderThrottlingStatus(
                !isVisible, frameView->m_subtreeThrottled);
    ...
    LocalFrame*& customScrollbarFrame) const
{
    ...
    if (Settings* settings = m_frame->settings()) {
        if (!settings->getAllowCustomScrollbarInMainFrame() && m_frame->isMainFrame())
            return false;
    }
...
#ifdef TENCENT_FITSCREEN_FRAEME_FLATTEN
        && m_frame->settings() && !m_frame->settings()->frameFlatteningEnabled() && !m_frame->document()->needFitScreenLayout()
#endif
    )
        RETURN_SCROLLBAR_MODE(ScrollbarAlwaysOff);
...
bool FrameView::usesCompositedScrolling() const
{
    ...
    if (layoutView.isNull())
        return false;
    if (m_frame->settings() && m_frame->settings()->getPreferCompositingToLCDTextEnabled())
...
void FrameView::performPreLayoutTasks()
{
    ...
    bool mainFrameRotation = m_frame->isMainFrame() && m_frame->settings() && m_frame->settings()->getMainFrameResizesAreOrientationChanges();
    if ((wasResized && document->styleEngine().mediaQueryAffectedByViewportChange()) || (wasResized && mainFrameRotation && document->styleEngine().mediaQueryAffectedByDeviceChange())) {
...
{
    return RuntimeEnabledFeatures::scrollAnchoringEnabled() && !RuntimeEnabledFeatures::rootLayerScrollingEnabled() && m_scrollAnchor.hasScroller() && layoutBox()->style()->overflowAnchor() != EOverflowAnchor::None && !m_frame->document()->finishingOrIsPrinting();
}
...
#ifdef TENCENT_FITSCREEN_FRAEME_FLATTEN
            || inChildFrameLayoutWithFrameFlattening
#endif
            )) {
...
void FrameView::layout()
{
    ...
    if (inChildFrameLayoutWithFrameFlattening) {
        if (doLayoutWithFrameFlattening())
            return;
    }
#endif
    ...
    FontCachePurgePreventer fontCachePurgePreventer;
    {
        ...
        if (!inSubtreeLayout) {
...
#ifdef TENCENT_FITSCREEN_FRAEME_FLATTEN
                    && m_frame->settings() && !m_frame->settings()->frameFlatteningEnabled()
                    && !document->needFitScreenLayout()
#endif
                ) {
            ...
            if (oldSize != m_size && !m_firstLayout) {
                LayoutBox* rootLayoutObject = document->documentElement()
...
#ifdef TENCENT_CHANGES
    if (m_textAutosizerCheckTimer.isActive())
        m_textAutosizerCheckTimer.stop();
    m_textAutosizerCheckTimer.startOneShot(0, BLINK_FROM_HERE);
#endif
}

#ifdef TENCENT_CHANGES
void FrameView::checkTextAutosizerTimerFired(TimerBase*)
{
    if (!m_frame.get())
        return;
    Document* document = m_frame->document();
    if (!document || !document->textAutosizer())
        return;

    if (document->textAutosizer()->checkTextAutoSizerConsistent()) {
        setNeedsLayout();
        scheduleRelayout();
    }
}
...
FloatSize FrameView::viewportSizeForViewportUnits() const
{
    ...
    if (RuntimeEnabledFeatures::inertTopControlsEnabled() && browserControls.permittedState() != WebBrowserControlsHidden) {
...
AtomicString FrameView::mediaType() const
{
    ...
    if (m_frame->settings() && !m_frame->settings()->getMediaTypeOverride().isEmpty())
...
void FrameView::addViewportConstrainedObject(LayoutObject* object)
{
    if (!m_viewportConstrainedObjects) {
        m_viewportConstrainedObjects = WTF::wrapUnique(new ViewportConstrainedObjectSet);
    }
...
void FrameView::viewportSizeChanged(bool widthChanged, bool heightChanged)
{
    ...
    bool rootLayerScrollingEnabled = RuntimeEnabledFeatures::rootLayerScrollingEnabled();
    ...
    if (RuntimeEnabledFeatures::inertTopControlsEnabled() && layoutView() && m_frame->isMainFrame() && m_frame->host()->browserControls().height()) {
...
{
    Page* page = frame().page();
    return page && page->visibilityState() != PageVisibilityStateHidden && !m_frame->eventHandler().isMousePositionUnknown() && page->focusController().isActive();
}
...
void FrameView::didUpdateElasticOverscroll()
{
    ...
    if (horizontalScrollbar()) {
        float delta = elasticOverscroll.width() - horizontalScrollbar()->elasticOverscroll();
        if (delta != 0) {
    ...
    if (verticalScrollbar()) {
        float delta = elasticOverscroll.height() - verticalScrollbar()->elasticOverscroll();
        if (delta != 0) {
    ...
    CompositedSelection& selection)
{
    ...
    if (selection.isEditable) {
        if (TextControlElement* enclosingTextControlElement = enclosingTextControl(visibleSelection.rootEditableElement())) {
            selection.isEmptyTextControl = enclosingTextControlElement->value().isEmpty();
        }
    }
    selection.start.isTextDirectionRTL |= primaryDirectionOf(*visibleSelection.start().anchorNode()) == TextDirection::kRtl;
    selection.end.isTextDirectionRTL |= primaryDirectionOf(*visibleSelection.end().anchorNode()) == TextDirection::kRtl;
...
void FrameView::updateCompositedSelectionIfNeeded()
{
    ...
    LocalFrame* localFrame = (focusedFrame && (focusedFrame->localFrameRoot() == m_frame->localFrameRoot()))
        ? focusedFrame
        : nullptr;
...
static inline void removeFloatingObjectsForSubtreeRoot(LayoutObject& root)
{
    ...
    if (LayoutBlock* cb = root.containingBlock()) {
        if ((cb->normalChildNeedsLayout() || cb->selfNeedsLayout()) && cb->isLayoutBlockFlow())
...
static bool prepareOrthogonalWritingModeRootForLayout(LayoutObject& root)
{
    ...
    if (!root.needsLayout() || root.isOutOfFlowPositioned() || root.isColumnSpanAll() || !root.styleRef().logicalHeight().isIntrinsicOrAuto())
        return false;
...
#ifdef TENCENT_FITSCREEN_FRAEME_FLATTEN
    if (m_frame->ownerLayoutObject() && isInChildFrameWithFrameFlattening())
        m_frame->ownerLayoutObject()->setNeedsLayout(LayoutInvalidationReason::ChildChanged);
#endif
...
{
    if (!m_frame || !parent() || !m_frame->deprecatedLocalOwner())
        return false;

    // Frame flattening applies when the owner element is either in a frameset or
    // an iframe with flattening parameters.
    if (m_frame->deprecatedLocalOwner()->hasTagName(iframeTag)) {
        LayoutIFrame* iframeRenderer = toLayoutIFrame(m_frame->deprecatedLocalOwner()->layoutPart());
        if (iframeRenderer->flattenFrame())
            return true;
    }

    Document* document = m_frame->document();
    bool needFitScreenLayout = document && document->needFitScreenLayout();
    if (!m_frame->settings() || !m_frame->settings()->frameFlatteningEnabled() || !needFitScreenLayout)
        return false;

    if (m_frame->deprecatedLocalOwner()->hasTagName(frameTag))
        return true;

    return false;
}

bool FrameView::doLayoutWithFrameFlattening()
{
...
bool FrameView::needsLayout() const
{
    ...
    LayoutViewItem layoutViewItem = this->layoutViewItem();
    return layoutPending() || (!layoutViewItem.isNull() && layoutViewItem.needsLayout()) || isSubtreeLayout();
}
...
void FrameView::didAttachDocument()
{
    ...
    if (m_frame->isMainFrame()) {
        ...
        RootFrameViewport* rootFrameViewport = RootFrameViewport::create(visualViewport, *layoutViewport);
...
void FrameView::scheduleVisualUpdateForPaintInvalidationIfNeeded()
{
    ...
    if (localFrameRoot->view()->m_currentUpdateLifecyclePhasesTargetState < DocumentLifecycle::PaintInvalidationClean || lifecycle().state() >= DocumentLifecycle::PrePaintClean) {
    ...
    DocumentLifecycle::LifecycleState targetState)
{
    ...
    DCHECK(targetState == DocumentLifecycle::LayoutClean || targetState == DocumentLifecycle::CompositingClean || targetState == DocumentLifecycle::PrePaintClean || targetState == DocumentLifecycle::PaintClean);
    ...
    if (LayoutViewItem view = layoutViewItem()) {
        ...
        if (targetState == DocumentLifecycle::PaintClean) {
            ...
            DCHECK((m_frame->document()->printing() && lifecycle().state() == DocumentLifecycle::PrePaintClean) || lifecycle().state() == DocumentLifecycle::PaintClean);
        }
...
void FrameView::setTracksPaintInvalidations(bool trackPaintInvalidations)
{
    ...
    for (Frame* frame = m_frame->tree().top(); frame;
        ...
        if (LayoutViewItem layoutView = toLocalFrame(frame)->contentLayoutItem()) {
            layoutView.frameView()->m_trackedObjectPaintInvalidations = WTF::wrapUnique(trackPaintInvalidations
...
bool FrameView::visualViewportSuppliesScrollbars()
{
    ...
    if (!m_frame->settings() || !m_frame->settings()->getViewportEnabled() || !m_frame->document() || !m_frame->host())
        return false;

    const TopDocumentRootScrollerController& controller = m_frame->host()->globalRootScrollerController();
...
void FrameView::setLayoutSizeInternal(const IntSize& size)
{
    ...
    String out = String::format("FrameView::setLayoutSizeInternal: %d\n", m_layoutSize.width());
...
IntSize FrameView::maximumScrollOffsetInt() const
{
    ...
    // can be made const.
    const ScrollableArea* layoutViewport = const_cast<FrameView*>(this)->layoutViewportScrollableArea();
    TopDocumentRootScrollerController& controller = host->globalRootScrollerController();
    if (layoutViewport == controller.rootScrollerArea())
        visibleSize = controller.rootScrollerVisibleArea();

    IntSize maximumOffset = toIntSize(-scrollOrigin() + (contentBounds - visibleSize));
...
{
    Scrollbar* scrollbar = ((orientation == HorizontalScrollbar) ? horizontalScrollbar()
                                                                 : verticalScrollbar());
...
{
    return (horizontalScrollbar() && horizontalScrollbar()->isOverlayScrollbar()) || (verticalScrollbar() && verticalScrollbar()->isOverlayScrollbar());
}
...
{
    if ((m_frame->settings() && m_frame->settings()->getHideScrollbars()) || visualViewportSuppliesScrollbars()) {
    ...
    bool attemptToRemoveScrollbars = (option == FirstPass && docSize.width() <= fullVisibleSize.width() && docSize.height() <= fullVisibleSize.height());
...
{
    bool forceDisabled = ScrollbarTheme::theme().shouldDisableInvisibleScrollbars() && scrollbarsHidden();

    if (horizontalScrollbar()) {
        horizontalScrollbar()->setEnabled(contentsWidth() > visibleWidth() && !forceDisabled);
    }
    if (verticalScrollbar()) {
        verticalScrollbar()->setEnabled(contentsHeight() > visibleHeight() && !forceDisabled);
    }
}
...
void FrameView::updateScrollbarGeometry()
{
    ...
    if (verticalScrollbar()) {
        ...
        IntRect vBarRect(
            ...
            thickness,
            height() - (horizontalScrollbar() ? horizontalScrollbar()->height() : 0));
...
bool FrameView::needsScrollbarReconstruction() const
{
    ...
    bool hasCustom = (horizontalScrollbar() && horizontalScrollbar()->isCustomScrollbar()) || (verticalScrollbar() && verticalScrollbar()->isCustomScrollbar());
...
{
    return m_frame->settings()->getIgnoreMainFrameOverflowHiddenQuirk() && m_frame->isMainFrame();
}
...
{
    if (horizontalScrollbar() && horizontalScrollbar()->shouldParticipateInHitTesting() && horizontalScrollbar()->frameRect().contains(pointInFrame))
        return horizontalScrollbar();
    if (verticalScrollbar() && verticalScrollbar()->shouldParticipateInHitTesting() && verticalScrollbar()->frameRect().contains(pointInFrame))
...
{
    return (horizontalScrollbar() && width() - horizontalScrollbar()->width() > 0) || (verticalScrollbar() && height() - verticalScrollbar()->height() > 0);
}
    ...
    bool subtreeThrottled)
{
    ...
    HTMLFrameOwnerElement* ownerElement = m_frame->deprecatedLocalOwner();
    m_hiddenForThrottling = hidden && !frameRect().isEmpty() && (ownerElement && ownerElement->layoutObject());
...
void FrameView::maybeRecordLoadReason()
{
    ...
    if (frame().document()->frame()) {
        if (!parent) {
            ...
            // visibility.
            bool parentLoaded = !parent->parentFrameView() || parent->frame().document()->wouldLoadReason() > Created;
...
{
    return canThrottleRendering() && m_frame->document() && lifecycle().throttlingAllowed();
}
...
bool FrameView::hasVisibleSlowRepaintViewportConstrainedObjects() const
{
    ...
    for (const LayoutObject* layoutObject : *viewportConstrainedObjects()) {
        ...
        DCHECK(layoutObject->style()->position() == FixedPosition || layoutObject->style()->position() == StickyPosition);
        PaintLayer* layer = toLayoutBoxModelObject(layoutObject)->layer();
    ...
    MainThreadScrollingReasons parentReason)
{
    ...
    if (WebLayer* scrollLayer = toLocalFrame(frame).view()->layerForScrolling()->platformLayer()) {
        if (reasons) {
    ...
    const
{
    ...
    const bool mayBeScrolledByScript = mayBeScrolledByInput || (scrollingReasons == NotScrollableExplicitlyDisabled);
    ...
    if (mayBeScrolledByScript && hasVisibleSlowRepaintViewportConstrainedObjects()) {
    ...
    MapCoordinatesFlags mode)
{
    ...
    if (frame().localFrameRoot() == frame().tree().top() || (ancestor && ancestor->frame()->localFrameRoot() == frame().localFrameRoot())) {
        FrameView* ancestorView = (ancestor ? ancestor->frameView()
