    ...
    ResourceRequest::RedirectStatus redirectStatus) const
{
    ...
    if (isSchemeOnly())
        return true;
    bool pathsMatch = (redirectStatus == RedirectStatus::FollowedRedirect) || pathMatches(url.path());
    return hostMatches(url.host()) && portMatches(url.port(), url.protocol()) && pathsMatch;
}
...
bool CSPSource::portMatches(int port, const String& protocol) const
{
    ...
    if (m_port == 80 && (port == 443 || (port == 0 && defaultPortForProtocol(protocol) == 443)))
        return true;
...
bool CSPSource::subsumes(CSPSource* other) const
{
    ...
    if ((m_hostWildcard == NoWildcard && other->m_hostWildcard == HasWildcard) || (m_portWildcard == NoWildcard && other->m_portWildcard == HasWildcard)) {
        return false;
    }

    bool hostSubsumes = (m_host == other->m_host || hostMatches(other->m_host));
    bool portSubsumes = (m_portWildcard == HasWildcard) || portMatches(other->m_port, other->m_scheme);
...
{
    bool schemesMatch = schemeMatches(other->m_scheme) || other->schemeMatches(m_scheme);
    if (!schemesMatch || isSchemeOnly() || other->isSchemeOnly())
        return schemesMatch;
    bool hostsMatch = (m_host == other->m_host) || hostMatches(other->m_host) || other->hostMatches(m_host);
    bool portsMatch = (other->m_portWildcard == HasWildcard) || portMatches(other->m_port, other->m_scheme) || other->portMatches(m_port, m_scheme);
...
CSPSource* CSPSource::intersect(CSPSource* other) const
{
    ...
    String path = m_path.length() > other->m_path.length() ? m_path : other->m_path;
    ...
    int port = (other->m_portWildcard == HasWildcard || !other->m_port || m_scheme.length() > other->m_scheme.length())
        ? m_port
        : other->m_port;
    WildcardDisposition hostWildcard = (m_hostWildcard == HasWildcard) ? other->m_hostWildcard : m_hostWildcard;
    WildcardDisposition portWildcard = (m_portWildcard == HasWildcard) ? other->m_portWildcard : m_portWildcard;
