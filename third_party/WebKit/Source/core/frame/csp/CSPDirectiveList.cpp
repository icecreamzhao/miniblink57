    ...
    ContentSecurityPolicyHeaderSource source)
{
    ...
    if (!directives->checkEval(
        ...
        String message = "Refused to evaluate a string as JavaScript because 'unsafe-eval' is "
    ...
    WebURLRequest::RequestContext context) const
{
    ...
    if ((m_requireSRIFor & RequireSRIForToken::Script) && (context == WebURLRequest::RequestContextScript || context == WebURLRequest::RequestContextImport || context == WebURLRequest::RequestContextServiceWorker || context == WebURLRequest::RequestContextSharedWorker || context == WebURLRequest::RequestContextWorker)) {
        return false;
    }
    ...
    ResourceRequest::RedirectStatus redirectStatus) const
{
    ...
    reportViolation(ContentSecurityPolicy::getDirectiveName(
        ...
        ContentSecurityPolicy::DirectiveType::RequireSRIFor,
        "Refused to load the " + resourceType + " '" + url.elidedString() + "' because 'require-sri-for' directive requires "
    ...
    ContentSecurityPolicy::ExceptionStatus exceptionStatus) const
{
    ...
    if (directive == m_defaultSrc)
        suffix = " Note that 'script-src' was not explicitly set, so 'default-src' is "
                 "used as a fallback.";
    ...
    const String& consoleMessage) const
{
    ...
    if (typeAttribute.isEmpty())
        message = message + " When enforcing the 'plugin-types' directive, the plugin's "
    ...
    const String& hashValue) const
{
    ...
    if (directive->allowInline() && directive->isHashOrNoncePresent()) {
        ...
        suffix = " Note that 'unsafe-inline' is ignored if either a hash or nonce value "
        ...
        if (directive == m_defaultSrc)
            suffix = suffix + " Note also that '" + String(isScript ? "script" : "style") + "-src' was not explicitly set, so 'default-src' is used as a "
                                                                                            "fallback.";
    }
    ...
    ResourceRequest::RedirectStatus redirectStatus) const
{
    ...
    if (directive == m_defaultSrc)
        suffix = suffix + " Note that '" + ContentSecurityPolicy::getDirectiveName(effectiveType) + "' was not explicitly set, so 'default-src' is used as a fallback.";

    reportViolation(directive->text(), effectiveType,
        prefix + url.elidedString() + "' because it violates the following Content Security "
                                      "Policy directive: \""
    ...
    const KURL& url) const
{
    ...
    reportViolationWithFrame(directive->text(),
        ...
        "Refused to display '" + url.elidedString() + "' in a frame because an ancestor violates the "
    ...
    const String& content) const
{
    ...
    if (isMatchingNoncePresent(directive, nonce))
        return true;
    if (element && isHTMLScriptElement(element) && !toHTMLScriptElement(element)->loader()->isParserInserted() && allowDynamic()) {
        return true;
    }
    if (reportingStatus == ContentSecurityPolicy::SendReport) {
        return checkInlineAndReportViolation(
    ...
    ContentSecurityPolicy::ReportingStatus reportingStatus) const
{
    return reportingStatus == ContentSecurityPolicy::SendReport
        ? checkMediaTypeAndReportViolation(
            ...
            "Refused to load '" + url.elidedString() + "' (MIME type '" + typeAttribute + "') because it violates the following Content Security "
                                                                                          "Policy Directive: ")
...
bool CSPDirectiveList::allowBaseURI(
    ...
    ContentSecurityPolicy::ReportingStatus reportingStatus) const
{
    bool result = reportingStatus == ContentSecurityPolicy::SendReport
        ? checkSourceAndReportViolation(
            m_baseURI.get(), url,
            ContentSecurityPolicy::DirectiveType::BaseURI, redirectStatus)
        : checkSource(m_baseURI.get(), url, redirectStatus);

    if (result && !checkSource(operativeDirective(m_baseURI.get()), url, redirectStatus)) {
    ...
    Member<CSPDirectiveType>& directive)
{
    ...
    // https://www.w3.org/TR/CSP2/#delivery-html-meta-element.
    if (m_headerSource == ContentSecurityPolicyHeaderSourceMeta && ContentSecurityPolicy::getDirectiveType(name) == ContentSecurityPolicy::DirectiveType::FrameAncestors) {
        m_policy->reportInvalidDirectiveInMeta(name);
        return;
    }
...
void CSPDirectiveList::addDirective(const String& name, const String& value)
{
    ...
    ContentSecurityPolicy::DirectiveType type = ContentSecurityPolicy::getDirectiveType(name);
    ...
    } else if (type == ContentSecurityPolicy::DirectiveType::WorkerSrc && m_policy->experimentalFeaturesEnabled()) {
    ...
    } else if (type == ContentSecurityPolicy::DirectiveType::UpgradeInsecureRequests) {
        enableInsecureRequestsUpgrade(name, value);
    } else if (type == ContentSecurityPolicy::DirectiveType::BlockAllMixedContent) {
    ...
    } else if (type == ContentSecurityPolicy::DirectiveType::TreatAsPublicAddress) {
        treatAsPublicAddress(name, value);
    } else if (type == ContentSecurityPolicy::DirectiveType::RequireSRIFor && m_policy->experimentalFeaturesEnabled()) {
...
bool CSPDirectiveList::subsumes(const CSPDirectiveListVector& other)
{
    ...
    for (const auto& directive : directives) {
        ...
        // Embedding-CSP.
        SourceListDirectiveVector requiredList = getSourceVector(directive, CSPDirectiveListVector(1, this));
