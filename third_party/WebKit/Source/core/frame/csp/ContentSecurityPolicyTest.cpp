...
TEST_F(ContentSecurityPolicyTest, NonceSinglePolicy)
{
    ...
    for (const auto& test : cases) {
        ...
        policy->didReceiveHeader(test.policy,
            ContentSecurityPolicyHeaderTypeEnforce,
            ContentSecurityPolicyHeaderSourceHTTP);
        EXPECT_EQ(test.allowed, policy->allowScriptFromSource(resource, String(test.nonce), ParserInserted));
...
TEST_F(ContentSecurityPolicyTest, NonceMultiplePolicy)
{
    ...
    for (const auto& test : cases) {
        ...
        policy->didReceiveHeader(test.policy2,
            ContentSecurityPolicyHeaderTypeReport,
            ContentSecurityPolicyHeaderSourceHTTP);
        EXPECT_EQ(test.allowed1, policy->allowScriptFromSource(resource, String(test.nonce), ParserInserted));
        EXPECT_EQ(expectedReports, policy->m_violationReportsSent.size());
        ...
        policy->didReceiveHeader(test.policy2,
            ContentSecurityPolicyHeaderTypeEnforce,
            ContentSecurityPolicyHeaderSourceHTTP);
        EXPECT_EQ(test.allowed2, policy->allowScriptFromSource(resource, String(test.nonce), ParserInserted));
        EXPECT_EQ(expectedReports, policy->m_violationReportsSent.size());
...
TEST_F(ContentSecurityPolicyTest, DirectiveType)
{
    ...
    for (const auto& test : cases) {
        const String& nameFromType = ContentSecurityPolicy::getDirectiveName(test.type);
        ContentSecurityPolicy::DirectiveType typeFromName = ContentSecurityPolicy::getDirectiveType(test.name);
