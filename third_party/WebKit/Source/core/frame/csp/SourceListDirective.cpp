    ...
    ResourceRequest::RedirectStatus redirectStatus) const
{
    ...
    KURL effectiveURL = m_policy->selfMatchesInnerURL() && SecurityOrigin::shouldUseInnerURL(url)
...
{
    return !m_list.size() && !m_allowSelf && !m_allowStar && !m_allowInline && !m_allowHashedAttributes && !m_allowEval && !m_allowDynamic && !m_nonces.size() && !m_hashes.size();
}
    ...
    while (position < end) {
        ...
        if (parseSource(beginSource, position, scheme, host, port, path,
            ...
            if (scheme.isEmpty() && host.isEmpty())
                continue;
            if (ContentSecurityPolicy::getDirectiveType(host) != ContentSecurityPolicy::DirectiveType::Undefined)
    ...
    CSPSource::WildcardDisposition& portWildcard)
{
    ...
    if (position < end && *position == ':') {
        ...
        if (position[1] == '/') {
            // scheme://host || scheme://
            //       ^                ^
            if (!parseScheme(begin, position, scheme) || !skipExactly<UChar>(position, end, ':') || !skipExactly<UChar>(position, end, '/') || !skipExactly<UChar>(position, end, '/'))
                return false;
            if (position == end)
                return false;
            beginHost = position;
...
{
    const ContentSecurityPolicy::DirectiveType& type = ContentSecurityPolicy::getDirectiveType(m_directiveName);
    if (type != ContentSecurityPolicy::DirectiveType::DefaultSrc && type != ContentSecurityPolicy::DirectiveType::StyleSrc && type != ContentSecurityPolicy::DirectiveType::ScriptSrc) {
        return false;
    }
    return m_allowInline && !isHashOrNoncePresent() && (type != ContentSecurityPolicy::DirectiveType::ScriptSrc || !m_allowDynamic);
}
    ...
    const HeapVector<Member<SourceListDirective>>& other) const
{
    ...
    HeapVector<Member<CSPSource>> normalizedB = other[0]->getSources(other[0]->m_policy->getSelfSource());
    for (size_t i = 1; i < other.size(); i++) {
        ...
        allowHashedAttributesOther = allowHashedAttributesOther && other[i]->m_allowHashedAttributes;
    ...
    const ContentSecurityPolicy::DirectiveType type = ContentSecurityPolicy::getDirectiveType(m_directiveName);
    if (type == ContentSecurityPolicy::DirectiveType::ScriptSrc || type == ContentSecurityPolicy::DirectiveType::StyleSrc) {
        if (!m_allowEval && allowEvalOther)
            return false;
        if (!m_allowHashedAttributes && allowHashedAttributesOther)
            return false;
        bool allowAllInlineOther = allowInlineOther && !isHashOrNoncePresentOther && (type != ContentSecurityPolicy::DirectiveType::ScriptSrc || !allowDynamicOther);
        if (!allowAllInline() && allowAllInlineOther)
            return false;
    }

    if (type == ContentSecurityPolicy::DirectiveType::ScriptSrc && (m_allowDynamic || allowDynamicOther)) {
    ...
    HeapVector<Member<CSPSource>> normalizedA = getSources(other[0]->m_policy->getSelfSource());
    ...
    const HeapVector<Member<CSPSource>>& other) const
{
    ...
    for (const auto& it : schemesMap) {
        ...
        if ((it.key != "https" || !schemesMap.contains("http")) && (it.key != "wss" || !schemesMap.contains("ws"))) {
            normalized.push_back(it.value);
        }
    }

    HeapVector<Member<CSPSource>> thisVector = getSources(m_policy->getSelfSource());
