...
TEST_F(CSPDirectiveListTest, AllowScriptFromSourceNoNonce)
{
    ...
    for (const auto& test : cases) {
        ...
        // Report-only
        Member<CSPDirectiveList> directiveList = createList(test.list, ContentSecurityPolicyHeaderTypeReport);
        EXPECT_EQ(test.expected, directiveList->allowScriptFromSource(scriptSrc, String(), ParserInserted, ResourceRequest::RedirectStatus::NoRedirect, ContentSecurityPolicy::SuppressReport));
...
TEST_F(CSPDirectiveListTest, AllowFromSourceWithNonce)
{
    ...
    for (const auto& test : cases) {
        ...
        Member<CSPDirectiveList> directiveList = createList(String("script-src ") + test.list,
            ContentSecurityPolicyHeaderTypeReport);
        EXPECT_EQ(test.expected, directiveList->allowScriptFromSource(resource, String(test.nonce), ParserInserted, ResourceRequest::RedirectStatus::NoRedirect, ContentSecurityPolicy::SuppressReport));
...
TEST_F(CSPDirectiveListTest, workerSrc)
{
    ...
    for (const auto& test : cases) {
        ...
        Member<CSPDirectiveList> directiveList = createList(test.list, ContentSecurityPolicyHeaderTypeEnforce);
...
TEST_F(CSPDirectiveListTest, SubsumesBasedOnCSPSourcesOnly)
{
    ...
    CSPDirectiveList* emptyA = createList("", ContentSecurityPolicyHeaderTypeEnforce);
...
TEST_F(CSPDirectiveListTest, SubsumesIfNoneIsPresent)
{
    ...
    for (const auto& test : cases) {
        CSPDirectiveList* A = createList(test.policyA, ContentSecurityPolicyHeaderTypeEnforce);
...
TEST_F(CSPDirectiveListTest, OperativeDirectiveGivenType)
{
    ...
    CSPDirectiveList* empty = createList("", ContentSecurityPolicyHeaderTypeEnforce);
    ...
    for (const auto& test : cases) {
        ...
        // should be returned.
        HeapVector<Member<CSPSource>> sources = allDirectivesList->operativeDirective(test.directive)->m_list;
        ...
        for (const auto& subtest : cases) {
            if (subtest.directive == test.directive)
                continue;
            const char* directiveName = ContentSecurityPolicy::getDirectiveName(subtest.directive);
        ...
        CSPDirectiveList* allExceptChildSrcAndThisList = createList(allExceptChildSrcAndThis.str().c_str(),
            ContentSecurityPolicyHeaderTypeEnforce);
        ...
        case ChildAndDefault:
            ...
            sources = allExceptChildSrcAndThisList->operativeDirective(test.directive)
                          ->m_list;
            EXPECT_EQ(sources.size(), 1u);
            EXPECT_EQ(sources[0]->m_host, "default-src.com");
            break;
...
TEST_F(CSPDirectiveListTest, GetSourceVector)
{
    ...
    HeapVector<Member<SourceListDirective>> result = CSPDirectiveList::getSourceVector(
    ...
    for (const auto& test : cases) {
        ...
        HeapVector<Member<SourceListDirective>> result = CSPDirectiveList::getSourceVector(test.directive, policyVector);
        ...
        policyVector.push_back(createList("default-src https://default-src.com;",
            ContentSecurityPolicyHeaderTypeEnforce));
        size_t udpatedTotal = test.type != NoDefault ? test.expectedTotal + 1 : test.expectedTotal;
        EXPECT_EQ(
            CSPDirectiveList::getSourceVector(test.directive, policyVector).size(),
            udpatedTotal);
        size_t expectedChildSrc = test.directive == ContentSecurityPolicy::DirectiveType::ChildSrc ? 5u
                                                                                                   : 4u;
        ...
        policyVector.push_back(createList("child-src http://child-src.com;",
            ContentSecurityPolicyHeaderTypeEnforce));
        udpatedTotal = test.type == ChildAndDefault || test.directive == ContentSecurityPolicy::DirectiveType::ChildSrc
            ? udpatedTotal + 1
            : udpatedTotal;
        EXPECT_EQ(
            CSPDirectiveList::getSourceVector(test.directive, policyVector).size(),
