...
TEST_F(SourceListDirectiveTest, GetIntersectCSPSources)
{
    ...
    for (const auto& test : cases) {
        ...
        HeapVector<Member<CSPSource>> normalized = sourceList.getIntersectCSPSources(secondList.m_list);
...
TEST_F(SourceListDirectiveTest, GetIntersectCSPSourcesSchemes)
{
    ...
    for (const auto& test : cases) {
        ...
        HeapVector<Member<CSPSource>> normalized = listA.getIntersectCSPSources(listB.m_list);
...
{
    KURL base;
    String requiredSources = "http://example1.com/foo/ http://*.example2.com/bar/ "
                             "http://*.example3.com:*/bar/";
    ...
    for (const auto& test : cases) {
        ...
        for (const auto& sources : test.sourcesVector) {
            SourceListDirective* member = new SourceListDirective("script-src", sources, csp.get());
...
TEST_F(SourceListDirectiveTest, SubsumesWithSelf)
{
    ...
    for (const auto& test : cases) {
        ...
        for (const auto& sources : test.sourcesB) {
            SourceListDirective* member = new SourceListDirective("script-src", sources, cspB);
...
TEST_F(SourceListDirectiveTest, SubsumesAllowAllInline)
{
    ...
    for (const auto& test : cases) {
        ...
        ContentSecurityPolicy* cspB = SetUpWithOrigin("https://another.test/image.png");
...
TEST_F(SourceListDirectiveTest, GetSources)
{
    ...
    for (const auto& test : cases) {
        ...
        HeapVector<Member<CSPSource>> normalized = list.getSources(csp.get()->getSelfSource());
