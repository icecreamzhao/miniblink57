    ...
    const LayoutRect& borderRect) const
{
    ...
    // outer stripe
    const LayoutRectOutsets outerThirdInsets = doubleStripeInsets(m_edges, BorderEdge::DoubleBorderStripeOuter);
    ...
    // inner stripe
    const LayoutRectOutsets innerThirdInsets = doubleStripeInsets(m_edges, BorderEdge::DoubleBorderStripeInner);
    ...
    float effectiveOpacity) const
{
    ...
    // possibility).
    bool needsLayer = group.alpha != 255 && (includesAdjacentEdges(group.edgeFlags) || (index + 1 < borderInfo.opacityGroups.size()));
    ...
    BorderEdgeFlags completedEdges) const
{
    ...
    switch (side) {
    case BSTop: {
        bool usePath = m_isRounded && (borderStyleHasInnerDetail(edge.borderStyle()) || borderWillArcInnerEdge(m_inner.getRadii().topLeft(), m_inner.getRadii().topRight()));
        if (usePath)
    ...
    case BSBottom: {
        bool usePath = m_isRounded && (borderStyleHasInnerDetail(edge.borderStyle()) || borderWillArcInnerEdge(m_inner.getRadii().bottomLeft(), m_inner.getRadii().bottomRight()));
        if (usePath)
            path = &borderInfo.roundedBorderPath;
        else
    ...
    case BSLeft: {
        bool usePath = m_isRounded && (borderStyleHasInnerDetail(edge.borderStyle()) || borderWillArcInnerEdge(m_inner.getRadii().bottomLeft(), m_inner.getRadii().topLeft()));
        if (usePath)
            path = &borderInfo.roundedBorderPath;
        else
    ...
    case BSRight: {
        bool usePath = m_isRounded && (borderStyleHasInnerDetail(edge.borderStyle()) || borderWillArcInnerEdge(m_inner.getRadii().bottomRight(), m_inner.getRadii().topRight()));
        if (usePath)
            path = &borderInfo.roundedBorderPath;
        else
    ...
    bool antialias)
{
    ...
    shouldClip = shouldClip || ((miter1 != NoMiter || miter2 != NoMiter) && styleRequiresClipPolygon(style));
