    ...
    const LayoutPoint& paintOffset)
{
    ...
    if (paintPhase != PaintPhaseSelfOutlineOnly) {
        ...
        if (m_layoutTable.collapseBorders() && shouldPaintDescendantBlockBackgrounds(paintPhase) && m_layoutTable.style()->visibility() == EVisibility::kVisible) {
...
void TablePainter::paintBoxDecorationBackground(
    ...
    const LayoutPoint& paintOffset)
{
    if (!m_layoutTable.hasBoxDecorationBackground() || m_layoutTable.style()->visibility() != EVisibility::kVisible)
        return;
...
void TablePainter::paintMask(const PaintInfo& paintInfo,
    const LayoutPoint& paintOffset)
{
    if (m_layoutTable.style()->visibility() != EVisibility::kVisible || paintInfo.phase != PaintPhaseMask)
        return;
