...
namespace {
    ...
    static CompositingQueryMode gCompositingQueryMode = CompositingQueriesAreOnlyAllowedInCertainDocumentLifecyclePhases;
...
void PaintLayer::dirtyAncestorChainHasSelfPaintingLayerDescendantStatus()
{
    for (PaintLayer* layer = this; layer; layer = layer->parent()) {
        ...
        if (layer->isSelfPaintingLayer()) {
            DCHECK(!parent() || parent()->m_hasSelfPaintingLayerDescendantDirty || parent()->m_hasSelfPaintingLayerDescendant);
            break;
        }
    }
}
...
{
    if (layoutObject()->style()->position() != FixedPosition && layoutObject()->style()->position() != StickyPosition)
        return false;
    ...
    return (layoutObject()->style()->position() == FixedPosition && layoutObject()->containerForFixedPosition() == layoutObject()->view()) || (layoutObject()->style()->position() == StickyPosition && (!ancestorScrollingLayer() || ancestorScrollingLayer() == root()));
}
    ...
    LayoutRect& rect)
{
    ...
    // |paintInvalidationContainer| may have a local 2D transform on it, so take
    // that into account when mapping into the space of the transformed ancestor.
    rect = LayoutRect(paintInvalidationContainer
...
void PaintLayer::updateDescendantDependentFlags()
{
    if (m_needsDescendantDependentFlagsUpdate) {
        bool oldHasNonIsolatedDescendantWithBlendMode = m_hasNonIsolatedDescendantWithBlendMode;
        m_hasVisibleDescendant = false;
        m_hasNonIsolatedDescendantWithBlendMode = false;
        m_hasDescendantWithClipPath = false;
        m_hasRootScrollerAsDescendant = false;
        ...
        for (PaintLayer* child = firstChild(); child;
            ...
            m_hasNonIsolatedDescendantWithBlendMode |= (!child->stackingNode()->isStackingContext() && child->hasNonIsolatedDescendantWithBlendMode()) || child->layoutObject()->styleRef().hasBlendMode();

            m_hasDescendantWithClipPath |= child->hasDescendantWithClipPath() || child->layoutObject()->hasClipPath();

            m_hasRootScrollerAsDescendant |= child->hasRootScrollerAsDescendant() || (child == child->layoutObject()->document().rootScrollerController().rootScrollerPaintLayer());
        }

        if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled() && oldHasNonIsolatedDescendantWithBlendMode != static_cast<bool>(m_hasNonIsolatedDescendantWithBlendMode))
    ...
    if (layoutObject()->style()->visibility() == EVisibility::kVisible) {
        ...
        while (r) {
            if (r->style()->visibility() == EVisibility::kVisible && (!r->hasLayer() || !r->enclosingLayer()->isSelfPaintingLayer())) {
                m_hasVisibleContent = true;
                break;
            }
...
void PaintLayer::update3DTransformedDescendantStatus()
{
    ...
    while (PaintLayerStackingNode* node = iterator.next()) {
        ...
        if (childLayer.preserves3D() && (childLayer.has3DTransform() || childLayer.has3DTransformedDescendant()))
            childHas3D = true;
...
void PaintLayer::updateLayerPosition()
{
    ...
    if (!layoutObject()->isOutOfFlowPositioned() && !layoutObject()->isColumnSpanAll()) {
    ...
    if (PaintLayer* containingLayer = layoutObject()->isOutOfFlowPositioned()
        ...
        if (containingLayer->layoutObject()->isInFlowPositioned() && containingLayer->layoutObject()->isLayoutInline()) {
            LayoutSize offset = toLayoutInline(containingLayer->layoutObject())
...
PaintLayer* PaintLayer::compositingContainer() const
{
    if (!stackingNode()->isStacked()) {
        ...
        if (!isSelfPaintingLayer() && m_layoutObject->isFloating() && m_layoutObject->parent() && !m_layoutObject->parent()->isLayoutBlockFlow())
...
{
    m_ancestorDependentCompositingInputs = WTF::makeUnique<AncestorDependentCompositingInputs>(compositingInputs);
...
{
    SquashingDisallowedReasons oldReasons = m_rareData ? m_rareData->squashingDisallowedReasons
    ...
    GlobalPaintFlags globalPaintFlags)
{
    ...
    if (rootLayer != layer && ((transparencyBehavior == PaintingTransparencyClipBox && layer->paintsWithTransform(globalPaintFlags)) || (transparencyBehavior == HitTestingTransparencyClipBox && layer->hasTransformRelatedProperty()))) {
        ...
        // children.
        const PaintLayer* paginationLayer = transparencyMode == DescendantsOfTransparencyClipBox
    ...
    LayoutPoint& location)
{
    ...
    bool isOutOfFlowPositioned = position == AbsolutePosition || position == FixedPosition;
    if (isOutOfFlowPositioned ||
        ...
        if (!isOutOfFlowPositioned) {
            ...
            while (auto* container = object->container(ancestorLayer ? &*skipInfo : nullptr)) {
    ...
    const LayoutRect* layerBoundingBox)
{
    ...
    LayoutRect layerBoundingBoxInFlowThread = layerBoundingBox ? *layerBoundingBox
    ...
    dirtyRectInMulticolContainer.move(enclosingFlowThread->physicalLocation() - offsetOfPaginationLayerFromRoot);
    ...
    if (!insideLayer && isRootLayer()) {
        ...
        if (fallback) {
            layoutObject()->updateHitTestResult(
                result, toLayoutView(layoutObject())->flipForWritingMode(hitTestLocation.point()));
            insideLayer = this;
    ...
    double* zOffset)
{
    ...
    ClipRectsCacheSlot clipRectsCacheSlot = result.hitTestRequest().ignoreClipping()
    ...
    if (appliedTransform) {
        ...
        localTransformState = createLocalTransformState(rootLayer, containerLayer, hitTestRect,
    ...
    bool& insideClipRect) const
{
    ...
    for (int i = layerFragments.size() - 1; i >= 0; --i) {
        ...
        if ((hitTestFilter == HitTestSelf && !fragment.backgroundRect.intersects(hitTestLocation)) || (hitTestFilter == HitTestDescendants && !fragment.foregroundRect.intersects(hitTestLocation)))
            continue;
        insideClipRect = true;
    ...
    ClipRectsCacheSlot clipRectsCacheSlot)
{
    ...
    for (int i = enclosingPaginationFragments.size() - 1; i >= 0; --i) {
        ...
        if (parent() != enclosingPaginationLayer()) {
            ...
            LayoutRect parentClipRect = clipper()
    ...
    const LayoutPoint& translationOffset)
{
    ...
    RefPtr<HitTestingTransformState> newTransformState = createLocalTransformState(rootLayer, containerLayer, hitTestRect,
        hitTestLocation, transformState,
        translationOffset);
    ...
    HitTestFilter hitTestFilter) const
{
    ...
    if (!layoutObject()->hitTest(
        ...
        DCHECK(!result.innerNode() || (result.hitTestRequest().listBased() && result.listBasedTestResult().size()));
        return false;
    }
    ...
    const HitTestLocation& hitTestLocation) const
{
    ...
    if (!targetNode)
        return false;
    const ReferenceClipPathOperation& referenceClipPathOperation = toReferenceClipPathOperation(*clipPathOperation);
    SVGElement* element = referenceClipPathOperation.findElement(targetNode->treeScope());
...
{
    DCHECK(stackingNode()->isStackingContext() || !stackingNode()->hasPositiveZOrderList());
    ...
    CalculateBoundsOptions options) const
{
    ...
    if (this != compositedLayer && !hasVisibleContent() && !hasVisibleDescendant())
        return LayoutRect();
    ...
    if (transform() && (options == IncludeTransformsAndCompositedChildLayers || ((paintsWithTransform(GlobalPaintNormalPhase) && (this != compositedLayer || options == MaybeIncludeTransformForAncestorLayer)))))
...
GraphicsLayer* PaintLayer::graphicsLayerBacking(const LayoutObject* obj) const
{
    ...
    default:
        return (obj != layoutObject() && compositedLayerMapping()->scrollingContentsLayer())
...
{
    BackgroundPaintLocation location = isRootLayer() || !scrollsOverflow()
...
void PaintLayer::ensureCompositedLayerMapping()
{
    ...
    ensureRareData().compositedLayerMapping = WTF::wrapUnique(new CompositedLayerMapping(*this));
...
{
    if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled()) {
        return transform() && ((globalPaintFlags & GlobalPaintFlattenCompositingLayers) || compositingState() != PaintsIntoOwnBacking);
    }
    ...
    const LayoutRect& localRect) const
{
    ...
    if (!RuntimeEnabledFeatures::compositeOpaqueFixedPositionEnabled() && layoutObject()->style()->position() == FixedPosition && compositingState() != PaintsIntoOwnBacking)
        return false;
...
bool PaintLayer::shouldBeSelfPaintingLayer() const
{
    ...
    return layoutObject()->layerTypeRequired() == NormalPaintLayer || (m_scrollableArea && m_scrollableArea->hasOverlayScrollbars()) || needsCompositedScrolling();
}
    ...
    const ComputedStyle& newStyle)
{
    ...
    ClipPathOperation* oldClipOperation = oldStyle ? oldStyle->clipPath() : nullptr;
...
void showLayerTree(const blink::PaintLayer* layer)
{
    ...
    if (blink::LocalFrame* frame = layer->layoutObject()->frame()) {
        WTF::String output = externalRepresentation(
            frame,
            blink::LayoutAsTextShowAllLayers | blink::LayoutAsTextShowLayerNesting | blink::LayoutAsTextShowCompositedLayers | blink::LayoutAsTextShowAddresses | blink::LayoutAsTextShowIDAndClass | blink::LayoutAsTextDontUpdateLayout | blink::LayoutAsTextShowLayoutState,
            layer);
