    ...
    InvalidateDescendantMode invalidateDescendants)
{
    ...
    for (const auto& floatingObject : m_blockFlow.floatingObjects()->set()) {
        ...
        if (m_blockFlow.isOverhangingFloat(*floatingObject) && !floatingObject->layoutObject()->hasSelfPaintingLayer() && (floatingObject->shouldPaint() || (invalidateDescendants == InvalidateDescendants && floatingObject->layoutObject()->isDescendantOf(&m_blockFlow)))) {
    ...
    PaintInvalidationReason reason)
{
    ...
    // selection.
    if (reason == PaintInvalidationRectangle || reason == PaintInvalidationSelection)
        return;
    ...
    if (m_blockFlow.isAnonymous()) {
        ...
        if (LayoutInline* inlineElementContinuation = m_blockFlow.inlineElementContinuation()) {
