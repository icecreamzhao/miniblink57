...
namespace {
        ...
        const InlineTextBox& textBox)
    {
        const unsigned startOffset = marker.startOffset() > textBox.start()
            ...
            : 0U;
        const unsigned endOffset = std::min(marker.endOffset() - textBox.start(), textBox.len());
...
{
    FloatPoint p1(m_startPoint + FloatPoint(0, m_doubleOffset * m_wavyOffsetFactor));
    FloatPoint p2(m_startPoint + FloatPoint(m_width, m_doubleOffset * m_wavyOffsetFactor));
    ...
    const LayoutPoint& paintOffset)
{
    ...
    DCHECK(!shouldPaintSelfOutline(paintInfo.phase) && !shouldPaintDescendantOutlines(paintInfo.phase));
    ...
    const ComputedStyle& styleToUse = m_inlineTextBox.getLineLayoutItem().styleRef(
...
bool InlineTextBoxPainter::shouldPaintTextBox(const PaintInfo& paintInfo)
{
    ...
    if (m_inlineTextBox.getLineLayoutItem().style()->visibility() != EVisibility::kVisible || m_inlineTextBox.truncation() == cFullTruncation || !m_inlineTextBox.len())
        return false;
    return true;
}
    ...
    const CompositionUnderline& underline)
{
    ...
    unsigned paintStart = std::max(m_inlineTextBox.start(), underline.startOffset());
    // Cap the maximum paint start to (if no truncation) the last character,
    // else the last character before the truncation ellipsis.
    return std::min(paintStart, (m_inlineTextBox.truncation() == cNoTruncation) ? m_inlineTextBox.end() : m_inlineTextBox.start() + m_inlineTextBox.truncation() - 1);
}
    ...
    int endPos)
{
    ...
    int deltaY = (m_inlineTextBox.getLineLayoutItem().style()->isFlippedLinesWritingMode()
    ...
    DocumentMarkerPaintPhase markerPaintPhase)
{
    ...
    DocumentMarkerVector markers = m_inlineTextBox.getLineLayoutItem().document().markers().markersFor(
    ...
    bool grammar)
{
    ...
    if (!markerSpansWholeBox || grammar) {
        ...
        std::tie(startPosition, endPosition) = GetMarkerPaintOffsets(marker, m_inlineTextBox);
    ...
    const SimpleFontData* fontData = m_inlineTextBox.getLineLayoutItem()
    ...
    LayoutTextCombine* combinedText)
{
    ...
    TextRun textRun = m_inlineTextBox.constructTextRun(
        style, string, m_inlineTextBox.getLineLayoutItem().textLength() - m_inlineTextBox.start(),
    ...
    // there.
    if (m_inlineTextBox.hasWrappedSelectionNewline() && m_inlineTextBox.truncation() == cNoTruncation && !m_inlineTextBox.isLineBreak())
    ...
    const Vector<AppliedTextDecoration>& decorations)
{
    ...
    bool skipIntercepts = styleToUse.getTextDecorationSkip() & TextDecorationSkipInk;
        ...
        if ((lines & TextDecorationUnderline) && fontData) {
            ...
            if (skipIntercepts) {
                textPainter.clipDecorationsStripe(
                    -baseline + decorationPainter.decorationBounds().y() - FloatPoint(localOrigin).y(),
    ...
    const CompositionUnderline& underline)
{
    ...
    float start = paintStart == m_inlineTextBox.start()
        ? 0
    ...
    bool ltr = m_inlineTextBox.isLeftToRightDirection();
    bool flowIsLTR = m_inlineTextBox.getLineLayoutItem().style()->isLeftToRightDirection();
    if (paintStart == m_inlineTextBox.start() && paintEnd == m_inlineTextBox.end() + 1) {
        width = m_inlineTextBox.logicalWidth().toFloat();
    } else {
        unsigned paintFrom = ltr == flowIsLTR ? paintStart : paintEnd;
        unsigned paintLength = ltr == flowIsLTR
            ? paintEnd - paintStart
