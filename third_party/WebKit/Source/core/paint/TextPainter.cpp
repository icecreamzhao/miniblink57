    ...
    TextEmphasisPosition position)
{
    ...
    if (!fontData || emphasisMark.isNull()) {
        ...
        m_emphasisMarkOffset = -fontData->getFontMetrics().ascent() - m_font.emphasisMarkDescent(emphasisMark);
    } else {
        DCHECK(position == TextEmphasisPositionUnder);
        m_emphasisMarkOffset = fontData->getFontMetrics().descent() + m_font.emphasisMarkAscent(emphasisMark);
    }
}
    ...
    const TextPainter::Style& textStyle)
{
    ...
    if (haveSelection) {
        ...
        if (const ComputedStyle* pseudoStyle = layoutObject.getCachedPseudoStyle(PseudoIdSelection)) {
            selectionStyle.strokeColor = usesTextAsClip ? Color::black
...
void TextPainter::paintEmphasisMarkForCombinedText()
{
    ...
    FloatPoint emphasisMarkTextOrigin(m_textBounds.x().toFloat(),
        m_textBounds.y().toFloat() + fontData->getFontMetrics().ascent() + m_emphasisMarkOffset);
