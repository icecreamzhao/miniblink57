...
public:
    ...
    const PropertyTreeState* contentsProperties() const
    {
        if (!m_contentsProperties) {
            ...
            std::unique_ptr<PropertyTreeState> oldProperties = std::move(m_contentsProperties);
            updateContentsProperties();
    ...
    std::unique_ptr<ObjectPaintProperties> clone() const
    {
        ...
        if (m_svgLocalToBorderBoxTransform) {
            cloned->m_svgLocalToBorderBoxTransform = m_svgLocalToBorderBoxTransform->clone();
        }
        ...
        if (m_localBorderBoxProperties) {
            cloned->m_localBorderBoxProperties = WTF::wrapUnique(new PropertyTreeState(*m_localBorderBoxProperties));
        }
        return cloned;
    }
#endif
