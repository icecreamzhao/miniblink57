...
void ViewPainter::paintBoxDecorationBackground(const PaintInfo& paintInfo)
{
    ...
    if (!RuntimeEnabledFeatures::rootLayerScrollingEnabled() || BoxPainter::isPaintingBackgroundOfPaintContainerIntoScrollingContentsLayer(&m_layoutView, paintInfo)) {
    ...
    bool shouldClearCanvas = paintsBaseBackground && (document.settings() && document.settings()->getShouldClearDocumentBackground());
    ...
    const LayoutObject* rootObject = document.documentElement() ? document.documentElement()->layoutObject()
                                                                : nullptr;
    ...
    bool forceBackgroundToWhite = BoxPainter::shouldForceWhiteBackgroundForPrintEconomy(
    ...
    bool shouldDrawBackgroundInSeparateBuffer = BoxPainter(m_layoutView)
    ...
    Color combinedBackgroundColor = shouldDrawBackgroundInSeparateBuffer
        ? rootBackgroundColor
