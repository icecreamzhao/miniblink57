    ...
    , m_fill(ninePieceImage.fill())
{
    ...
    m_top.width = computeEdgeWidth(ninePieceImage.borderSlices().top(), borderWidths.top(),
...
void NinePieceImageGrid::setDrawInfoMiddle(NinePieceDrawInfo& drawInfo) const
{
    ...
    IntSize destinationSize = m_borderImageArea.size() - IntSize(m_left.width + m_right.width, m_top.width + m_bottom.width);
    ...
    float imageScaleFactor) const
{
    ...
    drawInfo.isCornerPiece = piece == TopLeftPiece || piece == TopRightPiece || piece == BottomLeftPiece || piece == BottomRightPiece;
