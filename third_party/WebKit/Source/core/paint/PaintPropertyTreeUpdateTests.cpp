...
TEST_P(PaintPropertyTreeUpdateTest, BuildingStopsAtThrottledFrames)
{
    ...
    auto* iframeTransform = childDocument().getElementById("iframeTransform")->layoutObject();
...
TEST_P(PaintPropertyTreeUpdateTest, ClipChangesUpdateOverflowClip)
{
    ...
    document().view()->updateAllLifecyclePhases();
    EXPECT_TRUE(!div->layoutObject()->paintProperties() || !div->layoutObject()->paintProperties()->overflowClip());
}
    ...
    TransformNodeWithAnimationLosesNodeWhenAnimationRemoved)
{
    ...
    const ObjectPaintProperties* properties = target->layoutObject()->paintProperties();
