    ...
    ItemToPaint itemToPaint)
{
    ...
    if (paintInfo.cullRect().m_rect.y() > paginationOffset.y()) {
        paginationOffset.move(LayoutUnit(), pageHeight * ((paintInfo.cullRect().m_rect.y() - paginationOffset.y()) / pageHeight).toInt());
    }
    ...
    LayoutUnit totalHeightOfRows = sectionsRect.height() - table->vBorderSpacing();
    LayoutUnit bottomBound = std::min(LayoutUnit(paintInfo.cullRect().m_rect.maxY()),
    ...
    const CollapsedBorderValue& currentBorderValue)
{
    ...
    LayoutRect tableAlignedRect = m_layoutTableSection.logicalRectForWritingModeAndDirection(
        localVisualRect);
    ...
    for (unsigned r = dirtiedRows.end(); r > dirtiedRows.start(); r--) {
        ...
        for (unsigned c = dirtiedColumns.end(); c > dirtiedColumns.start(); c--) {
            unsigned col = c - 1;
            const LayoutTableCell* cell = m_layoutTableSection.primaryCellAt(row, col);
    ...
    const LayoutPoint& paintOffset)
{
    ...
    LayoutTable::ColAndColGroup colAndColGroup = m_layoutTableSection.table()->colElementAtAbsoluteColumn(
