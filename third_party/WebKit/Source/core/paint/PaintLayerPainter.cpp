...
{
    return (paintFlags & PaintLayerPaintingOverflowContents || (paintFlags & PaintLayerPaintingChildClippingMaskPhase && layoutObject->hasClipPath()))
        ? IgnoreOverflowClip
        : RespectOverflowClip;
}
    ...
    const PaintLayerPaintingInfo& paintingInfo)
{
    ...
    if (RuntimeEnabledFeatures::slimmingPaintV2Enabled() && m_paintLayer.layoutObject()->styleRef().opacity())
        return false;
    ...
    if (m_paintLayer.paintsWithTransparency(paintingInfo.getGlobalPaintFlags())) {
        if (m_paintLayer.layoutObject()->styleRef().opacity() < kMinimumVisibleOpacity) {
            return true;
        }
    }
    return false;
    ...
    PaintLayerFlags paintFlags)
{
    ...
    if (m_paintLayer.paintsWithTransform(paintingInfo.getGlobalPaintFlags()) && !(paintFlags & PaintLayerAppliedTransform))
...
{
    DCHECK(m_paintLayer.isSelfPaintingLayer() || m_paintLayer.hasSelfPaintingLayerDescendant());
    ...
    PaintLayerFlags paintFlags)
{
    ...
    if (!paintLayer.stackingNode()->isStackingContext() && !paintLayer.layoutObject()->isSVGRoot())
        return false;
    ...
    bool& shouldClearEmptyPaintPhaseFlags)
{
    ...
    if (paintLayer.previousPaintResult() == MayBeClippedByPaintDirtyRect && paintLayer.previousPaintDirtyRect() != paintingInfo.paintDirtyRect) {
        needsRepaint = true;
        shouldClearEmptyPaintPhaseFlags = true;
    }
    ...
    FragmentPolicy fragmentPolicy)
{
    ...
    if (RuntimeEnabledFeatures::slimmingPaintV2Enabled() && RuntimeEnabledFeatures::rootLayerScrollingEnabled() && m_paintLayer.layoutObject() && m_paintLayer.layoutObject()->isLayoutView()) {
        const auto* objectPaintProperties = m_paintLayer.layoutObject()->paintProperties();
        DCHECK(objectPaintProperties && objectPaintProperties->localBorderBoxProperties());
        PaintChunkProperties properties(
            context.getPaintController().currentPaintChunkProperties());
        properties.propertyTreeState = *objectPaintProperties->localBorderBoxProperties();
    ...
    bool isPaintingCompositedDecoration = paintFlags & PaintLayerPaintingCompositingDecorationPhase;
    ...
    bool shouldPaintSelfOutline = isSelfPaintingLayer && !isPaintingOverlayScrollbars && (isPaintingCompositedDecoration || !isPaintingScrollingContent) && m_paintLayer.layoutObject()->styleRef().hasOutline();
    ...
    LayoutSize subpixelAccumulation = m_paintLayer.compositingState() == PaintsIntoOwnBacking
    ...
    bool shouldCompositeForBlendMode = m_paintLayer.stackingNode()->isStackingContext() && m_paintLayer.hasNonIsolatedDescendantWithBlendMode();
    ...
    { // Begin block for the lifetime of any filter.
        ...
        if (RuntimeEnabledFeatures::slimmingPaintV2Enabled() && !scopedPaintChunkProperties.has_value()) {
            ...
            DCHECK(!(RuntimeEnabledFeatures::rootLayerScrollingEnabled() && m_paintLayer.layoutObject() && m_paintLayer.layoutObject()->isLayoutView()));
            const auto* objectPaintProperties = m_paintLayer.layoutObject()->paintProperties();
            DCHECK(objectPaintProperties && objectPaintProperties->localBorderBoxProperties());
            PaintChunkProperties properties(
                context.getPaintController().currentPaintChunkProperties());
...
inline bool PaintLayerPainter::isFixedPositionObjectInPagedMedia()
{
    ...
    return object->styleRef().position() == FixedPosition && object->container() == view && view->pageLogicalHeight() &&
    ...
    PaintLayerFlags paintFlags)
{
    ...
    PaintLayerFragments layerFragments;
    bool isFixedPositionObjectInPagedMedia = this->isFixedPositionObjectInPagedMedia();
    ...
    const LayoutPoint& fragmentTranslation)
{
    ...
    PaintLayerPaintingInfo transformedPaintingInfo(
        &m_paintLayer, LayoutRect(enclosingIntRect(transform.inverse().mapRect(paintingInfo.paintDirtyRect))),
    ...
    PaintLayerFlags paintFlags)
{
    ...
    for (; child; child = iterator.next()) {
        ...
        if (childPainter.paint(context, childPaintingInfo, paintFlags) == MayBeClippedByPaintDirtyRect)
            result = MayBeClippedByPaintDirtyRect;
    }
    ...
    PaintLayerFlags paintFlags)
{
    ...
    return m_paintLayer.compositingState() == NotComposited || (globalPaintFlags & GlobalPaintFlattenCompositingLayers);
}
