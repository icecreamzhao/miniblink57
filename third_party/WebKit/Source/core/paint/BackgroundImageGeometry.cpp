...
namespace {
        ...
        const LayoutSize& positioningAreaSize)
    {
        ...
        case Cover: {
            float horizontalScaleFactor = imageIntrinsicSize.width()
                ? positioningAreaSize.width().toFloat() / imageIntrinsicSize.width()
                : 1.0f;
            float verticalScaleFactor = imageIntrinsicSize.height()
                ? positioningAreaSize.height().toFloat() / imageIntrinsicSize.height()
                : 1.0f;
            ...
            if (type == Contain) {
                ...
                return LayoutSize(LayoutUnit(std::max(1.0f, imageIntrinsicSize.width() * verticalScaleFactor)),
    ...
    {
        LayoutSize snappedSize = LayoutSize(size.width().fraction() == destination.width().fraction()
    ...
    LayoutUnit extraOffset)
{
    ...
    setPhaseX(actualWidth
            ? LayoutUnit(roundf(
                actualWidth - fmodf((computedXPosition + extraOffset), actualWidth)))
            : LayoutUnit());
}
    ...
    LayoutUnit extraOffset)
{
    ...
    setPhaseY(actualHeight
            ? LayoutUnit(roundf(
                actualHeight - fmodf((computedYPosition + extraOffset), actualHeight)))
            : LayoutUnit());
}
    ...
    const LayoutRect& paintRect)
{
    ...
    if (!fixedAttachment) {
        ...
        if (isLayoutView) {
            ...
            positioningAreaSize = rootBox->size() - LayoutSize(left + right, top + bottom),
            rootBox->location();
    ...
    positioningAreaSize = LayoutSize(snapSizeToPixel(positioningAreaSize.width(), m_destRect.x()),
    ...
    if (backgroundRepeatX == RoundFill && positioningAreaSize.width() > LayoutUnit() && fillTileSize.width() > LayoutUnit()) {
        ...
        setPhaseX(tileSize().width()
                ? LayoutUnit(roundf(
                    tileSize().width() - fmodf((computedXPosition + left), tileSize().width())))
                : LayoutUnit());
    ...
    if (backgroundRepeatY == RoundFill && positioningAreaSize.height() > LayoutUnit() && fillTileSize.height() > LayoutUnit()) {
        ...
        setPhaseY(tileSize().height()
                ? LayoutUnit(roundf(
                    tileSize().height() - fmodf((computedYPosition + top), tileSize().height())))
                : LayoutUnit());
        setSpaceSize(LayoutSize());
    }
