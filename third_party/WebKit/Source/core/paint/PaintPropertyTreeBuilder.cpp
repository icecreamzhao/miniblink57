...
PaintPropertyTreeBuilder::setupInitialContext()
{
    ...
    context.current.clip = context.absolutePosition.clip = context.fixedPosition.clip = ClipPaintPropertyNode::root();
    context.currentEffect = EffectPaintPropertyNode::root();
    context.inputClipOfCurrentEffect = ClipPaintPropertyNode::root();
    context.current.transform = context.absolutePosition.transform = context.fixedPosition.transform = TransformPaintPropertyNode::root();
    context.current.scroll = context.absolutePosition.scroll = context.fixedPosition.scroll = ScrollPaintPropertyNode::root();
    return context;
}
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (frameView.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        ...
        context.forceSubtreeUpdate |= updateContentClip(frameView, context.current.clip,
        ...
        if (frameView.isScrollable() || !scrollOffset.isZero()) {
            ...
            auto ancestorReasons = context.current.scroll->mainThreadScrollingReasons();
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (RuntimeEnabledFeatures::rootLayerScrollingEnabled() && object.isLayoutView()) {
    ...
    } else if (object.isBoxModelObject() && context.current.paintOffset != LayoutPoint()) {
        PaintLayer* layer = toLayoutBoxModelObject(object).layer();
        if (layer && layer->paintsWithTransform(GlobalPaintFlattenCompositingLayers))
            usesPaintOffsetTranslation = true;
    }
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        ...
        if (object.isBox()) {
            ...
            CompositingReasons compositingReasons = compositingReasonsForTransform(box);

            if (style.hasTransform() || style.preserves3D() || compositingReasons != CompositingReasonNone) {
                ...
                CompositorElementId compositorElementId = style.hasCurrentTransformAnimation()
    ...
    if (properties && properties->transform()) {
        ...
        if (object.styleRef().preserves3D()) {
            context.current.renderingContextId = properties->transform()->renderingContextId();
            context.current.shouldFlattenInheritedTransform = false;
        } else {
            context.current.renderingContextId = 0;
            context.current.shouldFlattenInheritedTransform = true;
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    const bool isCSSIsolatedGroup = object.isBoxModelObject() && style.isStackingContext();
    ...
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        ...
        CompositingReasons compositingReasons = CompositingReasonFinder::requiresCompositingForEffectAnimation(style);
        if (compositingReasons != CompositingReasonNone)
            effectNodeNeeded = true;

        CompositorElementId compositorElementId = (style.hasCurrentOpacityAnimation() || style.hasCurrentFilterAnimation() || style.hasCurrentBackdropFilterAnimation())
            ? createDomNodeBasedCompositorElementId(object)
            : CompositorElementId();
    ...
    if (properties && properties->effect()) {
        ...
        if (!properties->effect()->filter().isEmpty()) {
            ...
            const ClipPaintPropertyNode* inputClip = properties->effect()->outputClip();
            context.inputClipOfCurrentEffect = context.current.clip = context.absolutePosition.clip = context.fixedPosition.clip = inputClip;
        }
    }
}
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        ...
        context.forceSubtreeUpdate |= properties.updateOverflowClip(currentClip, context.current.transform,
            FloatRoundedRect(FloatRect(clipRect)));
    }
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        AffineTransform transformToBorderBox = SVGRootPainter(toLayoutSVGRoot(object))
                                                   .transformToPixelSnappedBorderBox(context.current.paintOffset);
        if (!transformToBorderBox.isIdentity()) {
            auto& properties = object.getMutableForPainting().ensurePaintProperties();
            context.forceSubtreeUpdate |= properties.updateSvgLocalToBorderBoxTransform(
                context.current.transform, transformToBorderBox, FloatPoint3D());
        } else {
            if (auto* properties = object.getMutableForPainting().paintProperties()) {
                context.forceSubtreeUpdate |= properties->clearSvgLocalToBorderBoxTransform();
            }
        }
    }
    ...
    PaintPropertyTreeBuilderContext& context)
{
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        ...
        if (object.hasOverflowClip()) {
            ...
            if (scrollNodeNeededForMainThreadReasons || !scrollOffset.isZero() || scrollableArea->scrollsOverflow()) {
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (object.isLayoutView()) {
        ...
        context.fixedPosition = context.current;
    } else if (object.getMutableForPainting().paintProperties() && object.paintProperties()->cssClip()) {
        ...
        if (context.fixedPosition.clip == cssClip->parent()) {
            ...
            if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
                ...
                context.forceSubtreeUpdate |= properties.updateCssClipFixedPosition(
                    context.fixedPosition.clip, const_cast<TransformPaintPropertyNode*>(cssClip->localTransformSpace()),
                    cssClip->clipRect());
            }
            const auto* properties = object.paintProperties();
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    case AbsolutePosition: {
        DCHECK(context.containerForAbsolutePosition == boxModelObject.container());
        ...
        if (container && container->isInFlowPositioned() && container->isLayoutInline()) {
            DCHECK(object.isBox());
            context.current.paintOffset += toLayoutInline(container)->offsetForInFlowPositionedInline(
                toLayoutBox(object));
        }
        break;
    }
    ...
    PaintPropertyTreeBuilderContext& context)
{
    ...
    if (object.needsPaintPropertyUpdate() || context.forceSubtreeUpdate) {
        if (RuntimeEnabledFeatures::slimmingPaintV2Enabled() && object.paintOffset() != context.current.paintOffset) {
