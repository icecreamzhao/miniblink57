...
{
    if (!scrollbar.isCustomScrollbar() && !(orientation == HorizontalScrollbar ? layerForHorizontalScrollbar() : layerForVerticalScrollbar()))
    ...
    const PaintInvalidatorContext& context)
{
    ...
    // transform node).
    if (!visualRect.isEmpty() && !RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
    ...
    const PaintInvalidatorContext& context)
{
    ...
    if (scrollbar && !(graphicsLayer && isOverlay)) {
        newVisualRect = scrollControlVisualRect(scrollbar->frameRect(), box, context);
    }
        ...
        const PaintInvalidatorContext& context)
{
    ...
    LayoutRect scrollCornerVisualRect = scrollControlVisualRect(scrollCornerAndResizerRect(), box, context);
    const LayoutBoxModelObject& paintInvalidationContainer = *context.paintInvalidationContainer;
    ...
    ScrollbarOrientation orientation)
{
    ...
    case ForwardButtonEndPart:
        scrollbarUseUMA = (orientation == VerticalScrollbar
