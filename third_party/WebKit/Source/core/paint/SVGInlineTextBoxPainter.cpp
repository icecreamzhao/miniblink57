...
void SVGInlineTextBoxPainter::paint(const PaintInfo& paintInfo,
    const LayoutPoint& paintOffset)
{
    DCHECK(paintInfo.phase == PaintPhaseForeground || paintInfo.phase == PaintPhaseSelection);
    DCHECK(m_svgInlineTextBox.truncation() == cNoTruncation);

    if (m_svgInlineTextBox.getLineLayoutItem().style()->visibility() != EVisibility::kVisible || !m_svgInlineTextBox.len())
        return;
    ...
    if (!DrawingRecorder::useCachedDrawingIfPossible(
        ...
        bool includeSelectionRect = paintInfo.phase != PaintPhaseSelection && (haveSelection || InlineTextBoxPainter::paintsMarkerHighlights(textLayoutObject));
...
void SVGInlineTextBoxPainter::paintSelectionBackground(
    const PaintInfo& paintInfo)
{
    if (m_svgInlineTextBox.getLineLayoutItem().style()->visibility() != EVisibility::kVisible)
        return;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        case PT_STROKE:
            if (svgDecorationStyle.hasVisibleStroke()) {
                ...
                float strokeScaleFactor = svgDecorationStyle.vectorEffect() == VE_NON_SCALING_STROKE
                    ? 1 / scalingFactor
                    : 1;
    ...
    SkPaint& paint)
{
    ...
    if (!SVGPaintContext::paintForLayoutObject(
            paintInfo, style, parentInlineLayoutObject(), resourceMode, paint,
    ...
    if (resourceMode == ApplyToStrokeMode) {
        ...
        float strokeScaleFactor = style.svgStyle().vectorEffect() != VE_NON_SCALING_STROKE ? scalingFactor
                                                                                           : 1;
        StrokeData strokeData;
        SVGLayoutSupport::applyStrokeStyleToStrokeData(
    ...
    bool shouldPaintSelection)
{
    ...
    if (shouldPaintSelection) {
        m_svgInlineTextBox.selectionStartEnd(startPosition, endPosition);
        shouldPaintSelection = m_svgInlineTextBox.mapStartEndPositionsIntoFragmentCoordinates(
    ...
    const DocumentMarker& marker) const
{
    ...
    int markerEndPosition = std::min<int>(marker.endOffset() - m_svgInlineTextBox.start(),
    ...
    const Font& font)
{
    ...
    for (const SVGTextFragmentWithRange& textMatchInfo : textMatchInfoList) {
        ...
        paintText(paintInfo, textRun, fragment, textMatchInfo.startPosition,
            textMatchInfo.endPosition, fillPaint);
