    ...
    IntRect resizerCornerRect)
{
    ...
    if (getScrollableArea()
        ...
        context.translate(resizerCornerRect.x() + cornerResizerSize.width(),
            resizerCornerRect.y() + resizerCornerRect.height() - cornerResizerSize.height());
    ...
    bool paintingOverlayControls)
{
    ...
    if (getScrollableArea().hasOverlayScrollbars() && !paintingOverlayControls) {
        ...
        PaintLayer* paintingRoot = getScrollableArea().layer()->enclosingLayerWithCompositedLayerMapping(
            IncludeSelf);
        if (!paintingRoot)
...

    {
        ...
        if (RuntimeEnabledFeatures::slimmingPaintV2Enabled()) {
            const auto* objectProperties = getScrollableArea().box().paintProperties();
