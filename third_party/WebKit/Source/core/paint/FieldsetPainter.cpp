    ...
    const LayoutPoint& paintOffset)
{
    ...
    if (m_layoutFieldset.style()->isHorizontalWritingMode()) {
        ...
        LayoutUnit clipHeight = max(static_cast<LayoutUnit>(m_layoutFieldset.style()->borderTopWidth()),
            legend->size().height() - ((legend->size().height() - m_layoutFieldset.borderTop()) / 2));
...
void FieldsetPainter::paintMask(const PaintInfo& paintInfo,
    const LayoutPoint& paintOffset)
{
    if (m_layoutFieldset.style()->visibility() != EVisibility::kVisible || paintInfo.phase != PaintPhaseMask)
        return;
    ...
    // https://bugs.webkit.org/show_bug.cgi?id=47236
    if (m_layoutFieldset.style()->isHorizontalWritingMode()) {
        LayoutUnit yOff = (legend->location().y() > LayoutUnit())
            ? LayoutUnit()
            : (legend->size().height() - m_layoutFieldset.borderTop()) / 2;
        paintRect.expand(LayoutUnit(), -yOff);
        paintRect.move(LayoutUnit(), yOff);
    } else {
        LayoutUnit xOff = (legend->location().x() > LayoutUnit())
            ? LayoutUnit()
            : (legend->size().width() - m_layoutFieldset.borderLeft()) / 2;
