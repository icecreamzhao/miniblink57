...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithBorderInvalidation)
{
    ...
    const auto* transformedProperties = transformedElement->layoutObject()->paintProperties();
...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithOpacityInvalidation)
{
    ...
    const auto* transparentProperties = transparentElement->layoutObject()->paintProperties();
