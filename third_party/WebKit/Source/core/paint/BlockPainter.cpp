...
void BlockPainter::paintOverflowControlsIfNeeded(
    ...
    const LayoutPoint& paintOffset)
{
    if (m_layoutBlock.hasOverflowClip() && m_layoutBlock.style()->visibility() == EVisibility::kVisible && shouldPaintSelfBlockBackground(paintInfo.phase) && !paintInfo.paintRootBackgroundOnly()) {
    ...
    const LayoutPoint& paintOffset)
{
    ...
    if (inlineBox.parent()
        ...
        childPoint = LineLayoutAPIShim::layoutObjectFrom(inlineBox.getLineLayoutItem())
                         ->containingBlock()
    ...
    const LayoutPoint& paintOffset)
{
    ...
    if (shouldPaintSelfBlockBackground(paintPhase)) {
        if (m_layoutBlock.style()->visibility() == EVisibility::kVisible && m_layoutBlock.hasBoxDecorationBackground())
    ...
    if (paintPhase == PaintPhaseMask && m_layoutBlock.style()->visibility() == EVisibility::kVisible) {
        m_layoutBlock.paintMask(paintInfo, paintOffset);
        return;
    }

    if (paintPhase == PaintPhaseClippingMask && m_layoutBlock.style()->visibility() == EVisibility::kVisible) {
