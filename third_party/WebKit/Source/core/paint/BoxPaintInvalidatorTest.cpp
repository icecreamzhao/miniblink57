...
TEST_P(BoxPaintInvalidatorTest, IncrementalInvalidationExpand)
{
    ...
    const auto& rasterInvalidations = getRasterInvalidationTracking()->trackedRasterInvalidations;
...
TEST_P(BoxPaintInvalidatorTest, SubpixelVisualRectChagne)
{
    ...
    document().view()->updateAllLifecyclePhases();
    rasterInvalidations = &getRasterInvalidationTracking()->trackedRasterInvalidations;
    ASSERT_EQ(2u, rasterInvalidations->size());
...
TEST_P(BoxPaintInvalidatorTest, CompositedLayoutViewResize)
{
    ...
    if (RuntimeEnabledFeatures::rootLayerScrollingEnabled()) {
        ...
        //                  ->graphicsLayerBacking()
        //                  ->getRasterInvalidationTracking());
        EXPECT_EQ(1u, layoutView().layer()->graphicsLayerBacking()->getRasterInvalidationTracking()->trackedRasterInvalidations.size());
    } else {
        const auto& rasterInvalidations = getRasterInvalidationTracking()->trackedRasterInvalidations;
        // TODO(wangxianzhu): Temporary for crbug.com/680745.
        // ASSERT_EQ(1u, rasterInvalidations.size());
...
TEST_P(BoxPaintInvalidatorTest, CompositedBackgroundAttachmentLocalResize)
{
    ...
    document().view()->updateAllLifecyclePhases();
    LayoutBoxModelObject* targetObj = toLayoutBoxModelObject(target->layoutObject());
    GraphicsLayer* containerLayer = targetObj->layer()->graphicsLayerBacking(targetObj);
    ...
    const auto& contentsRasterInvalidations = contentsLayer->getRasterInvalidationTracking()
                                                  ->trackedRasterInvalidations;
    ...
    const auto& containerRasterInvalidations = containerLayer->getRasterInvalidationTracking()
                                                   ->trackedRasterInvalidations;
