    ...
    if (!m_layoutImage.imageResource()->hasImage()) {
        ...
        if (cWidth > 2 && cHeight > 2) {
            ...
            IntRect paintRect = pixelSnappedIntRect(
                LayoutRect(paintOffset.x() + m_layoutImage.borderLeft() + m_layoutImage.paddingLeft(),
                    paintOffset.y() + m_layoutImage.borderTop() + m_layoutImage.paddingTop(),
                    cWidth, cHeight));
    ...
    const LayoutRect& contentRect)
{
    ...
    InterpolationQuality interpolationQuality = BoxPainter::chooseInterpolationQuality(
