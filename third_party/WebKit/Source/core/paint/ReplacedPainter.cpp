    ...
    const LayoutPoint& paintOffset)
{
    ...
    if (shouldPaintSelfBlockBackground(paintInfo.phase)) {
        if (m_layoutReplaced.style()->visibility() == EVisibility::kVisible && m_layoutReplaced.hasBoxDecorationBackground()) {
...

    {
        ...
        if (m_layoutReplaced.style()->hasBorderRadius()) {
            if (borderRect.isEmpty()) {
                ...
                FloatRoundedRect roundedInnerRect = m_layoutReplaced.style()->getRoundedInnerBorderFor(
                    borderRect,
                    LayoutRectOutsets(-(m_layoutReplaced.paddingTop() + m_layoutReplaced.borderTop()),
    ...
    if (drawSelectionTint && !LayoutObjectDrawingRecorder::useCachedDrawingIfPossible(paintInfo.context, m_layoutReplaced, DisplayItem::kSelectionTint)) {
...
{
    if (paintInfo.phase != PaintPhaseForeground && !shouldPaintSelfOutline(paintInfo.phase) && paintInfo.phase != PaintPhaseSelection && paintInfo.phase != PaintPhaseMask && paintInfo.phase != PaintPhaseClippingMask && !shouldPaintSelfBlockBackground(paintInfo.phase))
        return false;
    ...
    // later.
    if (!m_layoutReplaced.isSVGRoot() && m_layoutReplaced.style()->visibility() != EVisibility::kVisible)
        return false;
