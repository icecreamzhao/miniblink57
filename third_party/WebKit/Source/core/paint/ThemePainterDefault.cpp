    ...
    WebThemeEngine::ExtraParams& extraParams)
{
    ...
    if (useMockTheme()) {
        ...
        float arrowSize = std::min(arrowBoxWidth,
                              static_cast<float>(rect.height() - box.borderTop() - box.borderBottom()))
            - 2 * extraPadding;
        // |arrowX| is the middle position for mock theme engine.
        extraParams.menuList.arrowX = (box.styleRef().direction() == TextDirection::kRtl)
