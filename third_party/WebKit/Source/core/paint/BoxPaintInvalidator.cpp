...
{
    DCHECK(box.hasPreviousBoxGeometries() == previousBoxGeometriesMap().contains(&box));
...
{
    PaintInvalidationReason reason = ObjectPaintInvalidatorWithContext(m_box, m_context)
    ...
    if ((style.backgroundLayers().thisOrNextLayersUseContentBox() || style.maskLayers().thisOrNextLayersUseContentBox() || style.boxSizing() == EBoxSizing::kBorderBox) && previousContentBoxRect() != m_box.contentBoxRect())
    ...
    // - visual overflows.
    if (m_context.oldVisualRect != LayoutRect(m_context.oldLocation, oldBorderBoxSize) || m_context.newVisualRect != LayoutRect(m_context.newLocation, newBorderBoxSize)) {
    ...
    if (style.hasVisualOverflowingEffect() || style.hasAppearance() || style.hasFilterInducingProperty() || style.resize() != RESIZE_NONE || style.hasMask())
...
{
    return !m_box.isDocumentElement() && !m_box.backgroundStolenForBeingBody() && m_box.styleRef().backgroundLayers().thisOrNextLayersHaveLocalAttachment();
}
    ...
    const LayoutRect& newLayoutOverflow)
{
    ...
    if (newLayoutOverflow.location() != oldLayoutOverflow.location())
        return true;
    if (newLayoutOverflow.width() != oldLayoutOverflow.width() && m_box.mustInvalidateFillLayersPaintOnHeightChange(m_box.styleRef().backgroundLayers()))
        return true;
    if (newLayoutOverflow.height() != oldLayoutOverflow.height() && m_box.mustInvalidateFillLayersPaintOnHeightChange(m_box.styleRef().backgroundLayers()))
        return true;
    return false;
}
...
{
    bool paintsOntoScrollingContentsLayer = backgroundPaintsOntoScrollingContentsLayer();
    if (!paintsOntoScrollingContentsLayer && !backgroundGeometryDependsOnLayoutOverflowRect())
        return;
    ...
    if (m_box.backgroundChangedSinceLastPaintInvalidation()) {
        ...
        if (!m_box.hasPreviousBoxGeometries() || newLayoutOverflow == oldLayoutOverflow)
            return;
        if (!paintsOntoScrollingContentsLayer) {
        ...
        shouldFullyInvalidateOnScrollingContentsLayer = shouldFullyInvalidateBackgroundOnLayoutOverflowChange(
            oldLayoutOverflow, newLayoutOverflow);
    }
...
PaintInvalidationReason BoxPaintInvalidator::invalidatePaintIfNeeded()
{
    ...
    if (reason == PaintInvalidationIncremental) {
        bool invalidated;
        if (m_box.isLayoutView() && !RuntimeEnabledFeatures::rootLayerScrollingEnabled()) {
...
bool BoxPaintInvalidator::needsToSavePreviousBoxGeometries()
{
    ...
    // crbug.com/490533
    if (style.backgroundLayers().thisOrNextLayersUseContentBox() || style.maskLayers().thisOrNextLayersUseContentBox() || backgroundGeometryDependsOnLayoutOverflowRect() || backgroundPaintsOntoScrollingContentsLayer())
        return true;
...
{
    DCHECK(m_box.hasPreviousBoxGeometries() == previousBoxGeometriesMap().contains(&m_box));
