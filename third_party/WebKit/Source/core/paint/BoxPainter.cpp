...
{
    return paintInfo.paintFlags() & PaintLayerPaintingOverflowContents && !(paintInfo.paintFlags() & PaintLayerPaintingCompositingBackgroundPhase) && obj == paintInfo.paintContainer();
}
...
{
    bool paintingOverflowContents = isPaintingBackgroundOfPaintContainerIntoScrollingContentsLayer(
        &m_layoutBox, paintInfo);
    ...
    const DisplayItemClient& displayItemClient = paintingOverflowContents ? static_cast<const DisplayItemClient&>(
                                                     *m_layoutBox.layer()
    ...
    bool shouldPaintBackground = !themePainted && (!paintInfo.skipRootBackground() || paintInfo.paintContainer() != &m_layoutBox);
    ...
    if (!paintingOverflowContents) {
        ...
        // border.
        if (boxDecorationData.hasBorderDecoration && (!boxDecorationData.hasAppearance || (!themePainted && LayoutTheme::theme().painter().paintBorderOnly(m_layoutBox, paintInfo, snappedPaintRect))) && !(m_layoutBox.isTable() && toLayoutTable(&m_layoutBox)->collapseBorders())) {
    ...
    const FillLayer& fillLayer)
{
    ...
    for (auto currentLayer = &fillLayer; currentLayer;
        ...
        if (currentLayer->clipOccludesNextLayers() && currentLayer->imageOccludesNextLayers(m_layoutBox)) {
...
{
    FillLayerOcclusionOutputList reversedPaintList;
    bool shouldDrawBackgroundInSeparateBuffer = calculateFillLayerOcclusionCulling(reversedPaintList, fillLayer);
...
namespace {
    ...
    public:
        ...
        {
            SkBlendMode bgOp = WebCoreCompositeToSkiaComposite(layer.composite(), layer.blendMode());
        ...
        Optional<BackgroundImageGeometry>& geometry)
    {
        ...
        // calls.
        GraphicsContext& context = paintInfo.context;
        FloatRoundedRect border = info.isRoundedFill ? backgroundRoundedRectAdjustedForBleedAvoidance(
        ...
        const FloatRect srcRect = Image::computeSubsetForTile(imageTile, border.rect(), intrinsicTileSize);
    ...
    const LayoutObject* backgroundObject)
{
    ...
    case ContentFillBox: {
        ...
        LayoutRect clipRect(
            scrolledPaintRect.x() + bLeft + (includePadding ? pLeft : LayoutUnit()),
    ...
    if (bgLayer.clip() == TextFillBox) {
        ...
        if (box) {
            ...
            box->paint(info, LayoutPoint(scrolledPaintRect.x() - box->x(), scrolledPaintRect.y() - box->y()),
...
void BoxPainter::paintMask(const PaintInfo& paintInfo,
    const LayoutPoint& paintOffset)
{
    if (m_layoutBox.style()->visibility() != EVisibility::kVisible || paintInfo.phase != PaintPhaseMask)
        return;
...
{
    return document.printing() && style.printColorAdjust() == EPrintColorAdjust::kEconomy && (!document.settings() || !document.settings()->getShouldPrintBackgrounds());
}
