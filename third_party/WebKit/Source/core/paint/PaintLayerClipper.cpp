    ...
    ClipRects& clipRects)
{
    ...
    DCHECK(box.hasClipRelatedProperty() || box.hasControlClip() || (box.isSVGRoot() && toLayoutSVGRoot(&box)->shouldApplyViewportClip()));
    ...
    if (box.hasOverflowClip() || (box.isSVGRoot() && toLayoutSVGRoot(&box)->shouldApplyViewportClip()) || box.styleRef().containsPaint() || box.hasControlClip()) {
    ...
    const PaintLayer* clippingRootLayer) const
{
    ...
    if (m_geometryMapper) {
        LayoutRect premappedRect = applyOverflowClipToBackgroundRectWithGeometryMapper(
        ...
        const auto* layerTransform = m_layer.layoutObject()
                                         ...
                                         ->localBorderBoxProperties()
                                         ->transform();
        FloatRect clippedRectInLocalSpace = m_geometryMapper->sourceToDestinationRect(
    ...
    const LayoutPoint* offsetFromRoot) const
{
    ...
    if (shouldClipOverflow(context)) {
        LayoutRect overflowOrControlClipRect = overflowOrControlClip(toLayoutBox(layoutObject), offset,
        ...
        LayoutRect layerBoundsWithVisualOverflow = layoutObject.isLayoutView()
    ...
    bool isForeground) const
{
    ...
    const auto* ancestorProperties = context.rootLayer->layoutObject()->paintProperties();
    DCHECK(ancestorProperties && ancestorProperties->localBorderBoxProperties());
    PropertyTreeState destinationPropertyTreeState = *ancestorProperties->localBorderBoxProperties();
    ...
    if (&m_layer == context.rootLayer) {
        ...
        if (isForeground && context.respectOverflowClip == RespectOverflowClip && properties->overflowClip())
        ...
        if (context.respectOverflowClip == IgnoreOverflowClip && ancestorProperties->overflowClip())
    ...
    FloatRect clippedRectInRootLayerSpace = m_geometryMapper->sourceToDestinationVisualRect(
...
#ifdef CHECK_CLIP_RECTS
        ClipRect testBackgroundClipRect = PaintLayerClipper(m_layer, false).backgroundClipRect(context);
    ...
    const ClipRectsContext& context) const
{
    ...
    return box.hasOverflowClip() || box.styleRef().containsPaint() || box.hasControlClip() || (box.isSVGRoot() && toLayoutSVGRoot(&box)->shouldApplyViewportClip());
}
