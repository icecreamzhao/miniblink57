    ...
    const PaintInvalidatorContext& context)
{
    ...
    if (context.forcedSubtreeInvalidationFlags & PaintInvalidatorContext::ForcedSubtreeSlowPathRect) {
        ...
        const auto* containerContentsProperties = context.paintInvalidationContainer->paintProperties()
                                                      ->contentsProperties();
        if (context.treeBuilderContext.current.transform == containerContentsProperties->transform() && context.treeBuilderContext.current.clip == containerContentsProperties->clip()) {
            result = LayoutRect(rect);
        } else {
    ...
    const PaintInvalidatorContext& context)
{
    ...
    if (object != context.paintInvalidationContainer) {
        ...
        const auto* containerTransform = context.paintInvalidationContainer->paintProperties()
                                             ->contentsProperties()
                                             ->transform();
    ...
    PaintInvalidatorContext& context)
{
    ...
    } else if (object.isColumnSpanAll() || (object.isFloating() && !object.parent()->isLayoutBlock())) {
    ...
    PaintInvalidatorContext& context)
{
    ...
    if (object.isPaintInvalidationContainer()) {
        ...
        context.paintInvalidationContainer = &object.containerForPaintInvalidation();
        ...
        object.hasLayer() && context.paintInvalidationContainer != context.paintInvalidationContainerForStackedContents) {
    ...
    PaintInvalidatorContext& context)
{
    ...
    DCHECK(context.treeBuilderContext.current.paintOffset == object.paintOffset());
    ...
    case PaintInvalidationSVGResourceChange:
        context.forcedSubtreeInvalidationFlags |= PaintInvalidatorContext::ForcedSubtreeSVGResourceChange;
        break;
    default:
        break;
    }
    ...
    if (!RuntimeEnabledFeatures::slimmingPaintV2Enabled() && context.oldVisualRect != context.newVisualRect
        // Note that isLayoutView() below becomes unnecessary after the launch of
        // root layer scrolling.
        && (object.hasOverflowClip() || object.isLayoutView()) && !toLayoutBox(object).usesCompositedScrolling())
        context.forcedSubtreeInvalidationFlags |= PaintInvalidatorContext::ForcedSubtreeInvalidationRectUpdate;
}
