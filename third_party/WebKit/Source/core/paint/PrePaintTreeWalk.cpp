    ...
    const PrePaintTreeWalkContext& parentContext)
{
    ...
    if (!object.needsPaintPropertyUpdate() && !object.descendantNeedsPaintPropertyUpdate() && !context.treeBuilderContext.forceSubtreeUpdate && !context.paintInvalidatorContext.forcedSubtreeInvalidationFlags && !object.shouldCheckForPaintInvalidationRegardlessOfPaintInvalidationState()) {
    ...
    if (object.isLayoutPart()) {
        ...
        if (widget && widget->isFrameView()) {
            context.treeBuilderContext.current.paintOffset += layoutPart.replacedContentRect().location() - widget->frameRect().location();
            context.treeBuilderContext.current.paintOffset = roundedIntPoint(context.treeBuilderContext.current.paintOffset);
