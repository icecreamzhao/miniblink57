...
{
    DCHECK(object.hasPreviousSelectionVisualRect() == selectionVisualRectMap().contains(&object));
    if (rect.isEmpty()) {
...
void ObjectPaintInvalidator::objectWillBeDestroyed(const LayoutObject& object)
{
    ...
    DCHECK(object.hasPreviousLocationInBacking() == locationInBackingMap().contains(&object));
    ...
    const LayoutObjectTraversalFunctor& functor)
{
    ...
    // details.
    DCHECK(object.isPaintInvalidationContainer() && (!object.styleRef().isStackingContext() || !object.isLayoutBlock()));
    ...
    while (descendant) {
        if (!descendant->hasLayer() || !descendant->styleRef().isStacked()) {
            ...
            traverseNonCompositingDescendantsInPaintOrder(*descendant, functor);
            descendant = descendant->nextInPreOrderAfterChildren(&object);
        } else if (descendant->styleRef().isStackingContext() && descendant->isLayoutBlock()) {
    ...
    const LayoutObject& object)
{
    ...
    } else if (object.isFloating() && object.parent() && !object.parent()->isLayoutBlock()) {
    ...
    const IntRect& dirtyRect)
{
    ...
    DCHECK(paintInvalidationContainer.isLayoutView() && paintInvalidationContainer.layer()->compositingState() == NotComposited);
    ...
    PaintInvalidationReason invalidationReason)
{
    ...
    DCHECK(gDisablePaintInvalidationStateAsserts || m_object.document().lifecycle().state() == (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled() ? DocumentLifecycle::InPrePaint : DocumentLifecycle::InPaintInvalidation));
...
{
    DCHECK(m_object.hasPreviousLocationInBacking() == locationInBackingMap().contains(&m_object));
    ...
    PaintInvalidationReason reason)
{
    ...
    DCHECK(m_object.hasPreviousSelectionVisualRect() == selectionVisualRectMap().contains(&m_object));
    ...
    PaintInvalidationReason reason)
{
    ...
    case PaintInvalidationNone:
        ...
        if (!RuntimeEnabledFeatures::slimmingPaintV2Enabled() && (m_context.forcedSubtreeInvalidationFlags & PaintInvalidatorContext::ForcedSubtreeInvalidationChecking) && !m_object.isSVGChild()) {
        ...
        if (m_object.isSVG() && (m_context.forcedSubtreeInvalidationFlags & PaintInvalidatorContext::ForcedSubtreeSVGResourceChange)) {
