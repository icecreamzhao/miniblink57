...
bool PaintLayerScrollableArea::shouldUseIntegerScrollOffset() const
{
    ...
    if (frame->settings() && !frame->settings()->getPreferCompositingToLCDTextEnabled())
        return true;
    ...
    const IntRect& parentRect) const
{
    ...
    IntRect rect = view->frameView()->convertToLayoutItem(LayoutBoxItem(&box()), parentRect);
    rect.move(-scrollbarOffset(scrollbar));
    return rect;
}
...
{
    IntSize scrollDimensions = maximumScrollOffsetInt() - minimumScrollOffsetInt();
    ...
    ScrollType scrollType)
{
    ...
    if (box().view()->compositor()->inCompositingMode()) {
        bool onlyScrolledCompositedLayers = scrollsOverflow() && layer()->isAllScrollingContentComposited() && box().style()->backgroundLayers().attachment() != LocalBackgroundAttachment;
    ...
    if (RuntimeEnabledFeatures::slimmingPaintInvalidationEnabled()) {
        ...
        if (!RuntimeEnabledFeatures::rootLayerScrollingEnabled() && layer()->isRootLayer()) {
    ...
    if (RuntimeEnabledFeatures::scrollAnchoringEnabled() && scrollType != AnchoringScroll && scrollType != ClampingScroll)
        scrollAnchor()->clear();
}
...
IntSize PaintLayerScrollableArea::maximumScrollOffsetInt() const
{
    ...
    IntSize visibleSize = pixelSnappedIntRect(box().overflowClipRect(box().location())).size();
    ...
    IncludeScrollbarsInRect scrollbarInclusion) const
{
    ...
    if (scrollbarInclusion == IncludeScrollbars) {
        verticalScrollbarWidth = (verticalScrollbar() && !verticalScrollbar()->isOverlayScrollbar())
            ? verticalScrollbar()->scrollbarThickness()
            : 0;
        horizontalScrollbarHeight = (horizontalScrollbar() && !horizontalScrollbar()->isOverlayScrollbar())
    ...
    ScrollbarOrientation orientation) const
{
    ...
    EOverflow overflowStyle = (orientation == HorizontalScrollbar)
        ? box().style()->overflowX()
        : box().style()->overflowY();
    return (overflowStyle == EOverflow::Scroll || overflowStyle == EOverflow::Auto || overflowStyle == EOverflow::Overlay);
}
...
{
    bool forceDisable = ScrollbarTheme::theme().shouldDisableInvisibleScrollbars() && scrollbarsHidden();
...
void PaintLayerScrollableArea::updateAfterLayout()
{
    ...
    bool horizontalScrollbarShouldChange = needsHorizontalScrollbar != hadHorizontalScrollbar;
    bool verticalScrollbarShouldChange = needsVerticalScrollbar != hadVerticalScrollbar;

    bool scrollbarsWillChange = !scrollbarsAreFrozen && (horizontalScrollbarShouldChange || verticalScrollbarShouldChange);
    if (scrollbarsWillChange) {
        ...
        if ((horizontalScrollbarShouldChange && box().style()->overflowX() != EOverflow::Overlay) || (verticalScrollbarShouldChange && box().style()->overflowY() != EOverflow::Overlay)) {
            if ((verticalScrollbarShouldChange && box().isHorizontalWritingMode()) || (horizontalScrollbarShouldChange && !box().isHorizontalWritingMode())) {
...
void PaintLayerScrollableArea::didChangeGlobalRootScroller()
{
    ...
    // scrollbars.
    if (!box().frame()->settings() || !box().frame()->settings()->getViewportEnabled())
        return;
...
{
    return RuntimeEnabledFeatures::scrollAnchoringEnabled() && m_scrollAnchor.hasScroller() && layoutBox()->style()->overflowAnchor() != EOverflowAnchor::None && !box().document().finishingOrIsPrinting();
}
    ...
    const ComputedStyle* oldStyle)
{
    ...
    bool horizontalScrollbarChanged = setHasHorizontalScrollbar(needsHorizontalScrollbar);
    bool verticalScrollbarChanged = setHasVerticalScrollbar(needsVerticalScrollbar);

    if (box().isLayoutBlock() && (horizontalScrollbarChanged || verticalScrollbarChanged)) {
    ...
    if (hasHorizontalScrollbar() && oldStyle && oldStyle->overflowX() == EOverflow::Scroll && box().style()->overflowX() != EOverflow::Scroll) {
        horizontalScrollbar()->setEnabled(true);
    }

    if (hasVerticalScrollbar() && oldStyle && oldStyle->overflowY() == EOverflow::Scroll && box().style()->overflowY() != EOverflow::Scroll) {
...
void PaintLayerScrollableArea::updateAfterOverflowRecalc()
{
    ...
    bool horizontalScrollbarShouldChange = needsHorizontalScrollbar != hasHorizontalScrollbar();
    bool verticalScrollbarShouldChange = needsVerticalScrollbar != hasVerticalScrollbar();

    if ((box().hasAutoHorizontalScrollbar() && horizontalScrollbarShouldChange) || (box().hasAutoVerticalScrollbar() && verticalScrollbarShouldChange)) {
    ...
    const IntRect& borderBoxRect) const
{
    ...
    return IntRect(horizontalScrollbarStart(borderBoxRect.x()),
        borderBoxRect.maxY() - box().borderBottom() - horizontalScrollbar()->scrollbarThickness(),
    ...
    if (Node* node = layoutObject.node()) {
        if (layoutObject.isLayoutView()) {
            ...
            if (Settings* settings = doc.settings()) {
                if (!settings->getAllowCustomScrollbarInMainFrame() && layoutObject.frame() && layoutObject.frame()->isMainFrame())
                    return layoutObject;
            }
    ...
    ComputeScrollbarExistenceOption option) const
{
    ...
    if (visualViewportSuppliesScrollbars() || !canHaveOverflowScrollbars(box()) || box().frame()->settings()->getHideScrollbars()) {
    ...
    if (box().hasAutoHorizontalScrollbar()) {
        ...
        needsHorizontalScrollbar &= box().isRooted() && this->hasHorizontalOverflow() && box().pixelSnappedClientHeight();
    }

    if (box().hasAutoVerticalScrollbar()) {
        if (option == ForbidAddingAutoBars)
            needsVerticalScrollbar &= hasVerticalScrollbar();
        needsVerticalScrollbar &= box().isRooted() && this->hasVerticalOverflow() && box().pixelSnappedClientWidth();
    }
...
void PaintLayerScrollableArea::updateScrollCornerStyle()
{
    ...
    const LayoutObject& actualLayoutObject = layoutObjectForScrollbar(box());
    RefPtr<ComputedStyle> corner = box().hasOverflowClip()
    ...
    const IntPoint& localPoint)
{
    ...
    if (hasVerticalScrollbar() && verticalScrollbar()->shouldParticipateInHitTesting()) {
        LayoutRect vBarRect(verticalScrollbarStart(0, box().size().width().toInt()),
            box().borderTop(),
            verticalScrollbar()->scrollbarThickness(),
            box().size().height().toInt() - (box().borderTop() + box().borderBottom()) - (hasHorizontalScrollbar() ? horizontalScrollbar()->scrollbarThickness() : resizeControlSize));
    ...
    if (hasHorizontalScrollbar() && horizontalScrollbar()->shouldParticipateInHitTesting()) {
        ...
        LayoutRect hBarRect(
            horizontalScrollbarStart(0),
            (box().size().height() - box().borderBottom() - horizontalScrollbar()->scrollbarThickness())
                .toInt(),
            (box().size().width() - (box().borderLeft() + box().borderRight()) - (hasVerticalScrollbar() ? verticalScrollbar()->scrollbarThickness() : resizeControlSize))
                .toInt(),
            horizontalScrollbar()->scrollbarThickness());
...
void PaintLayerScrollableArea::updateResizerStyle()
{
    ...
    const LayoutObject& actualLayoutObject = layoutObjectForScrollbar(box());
    RefPtr<ComputedStyle> resizer = box().hasOverflowClip()
        ? actualLayoutObject.getUncachedPseudoStyle(
    ...
    const LayoutSize& oldOffset)
{
    ...
    LayoutSize difference(
        (currentSize + newOffset - adjustedOldOffset).expandedTo(minimumSize) - currentSize);
    ...
    ScrollType scrollType)
{
    ...
    LayoutRect intersect = localToAbsolute(box(), intersection(layerBounds, localExposeRect));
    if (intersect.isEmpty() && !layerBounds.isEmpty() && !localExposeRect.isEmpty()) {
...
void PaintLayerScrollableArea::updateScrollableAreaSet(bool hasOverflow)
{
    ...
    if (HTMLFrameOwnerElement* owner = frame->deprecatedLocalOwner()) {
        isVisibleToHitTest &= owner->layoutObject() && owner->layoutObject()->style()->visibleToHitTesting();
    }
    ...
    const PaintLayer* layer)
{
    ...
    bool backgroundSupportsLCDText = RuntimeEnabledFeatures::compositeOpaqueScrollersEnabled() && layer->layoutObject()->style()->isStackingContext() && layer->backgroundPaintLocation() & BackgroundPaintInScrollingContents && layer->backgroundIsKnownToBeOpaqueInRect(toLayoutBox(layer->layoutObject())->paddingBoxRect()) && !layer->compositesWithTransform() && !layer->compositesWithOpacity();

    if (mode == PaintLayerScrollableArea::ConsiderLCDText && !layer->compositor()->preferCompositingToLCDTextEnabled() && !backgroundSupportsLCDText) {
    ...
    if (layer->layoutObject()->hasClip() || layer->hasDescendantWithClipPath() || layer->hasAncestorWithClipPath()) {
        addStyleRelatedMainThreadScrollingReasons(
    ...
    LCDTextMode mode)
{
    ...
    removeStyleRelatedMainThreadScrollingReasons();
    const bool needsCompositedScrolling = computeNeedsCompositedScrolling(mode, layer());
...
bool PaintLayerScrollableArea::visualViewportSuppliesScrollbars() const
{
    ...
    const TopDocumentRootScrollerController& controller = layoutBox()->document().frameHost()->globalRootScrollerController();
    ...
    ScrollbarOrientation orientation)
{
    ...
    const LayoutObject& actualLayoutObject = layoutObjectForScrollbar(scrollableArea()->box());
...
SubtreeLayoutScope*
    PaintLayerScrollableArea::PreventRelayoutScope::s_layoutScope
    = nullptr;
...
PersistentHeapVector<Member<PaintLayerScrollableArea>>*
    PaintLayerScrollableArea::PreventRelayoutScope::s_needsRelayout
    = nullptr;
