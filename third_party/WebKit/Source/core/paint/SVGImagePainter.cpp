...
{
    if (paintInfo.phase != PaintPhaseForeground || m_layoutSVGImage.style()->visibility() != EVisibility::kVisible || !m_layoutSVGImage.imageResource()->hasImage())
        return;
...
FloatSize SVGImagePainter::computeImageViewportSize() const
{
    ...
    ImageResourceContent* cachedImage = m_layoutSVGImage.imageResource()->cachedImage();
