...
{
    return (!m_inlineFlowBox.prevLineBox() && !m_inlineFlowBox.nextLineBox()) || !m_inlineFlowBox.parent();
}

inline bool InlineFlowBoxPainter::includeLogicalLeftEdgeForBoxShadow() const
{
    return shouldForceIncludeLogicalEdges() || m_inlineFlowBox.includeLogicalLeftEdge();
}

inline bool InlineFlowBoxPainter::includeLogicalRightEdgeForBoxShadow() const
{
    return shouldForceIncludeLogicalEdges() || m_inlineFlowBox.includeLogicalRightEdge();
}
    ...
    const LayoutRect& cullRect)
{
    ...
    if (m_inlineFlowBox.getLineLayoutItem().style()->visibility() != EVisibility::kVisible)
        return;
...
void InlineFlowBoxPainter::paintMask(const PaintInfo& paintInfo,
    const LayoutPoint& paintOffset)
{
    if (m_inlineFlowBox.getLineLayoutItem().style()->visibility() != EVisibility::kVisible || paintInfo.phase != PaintPhaseMask)
        return;
