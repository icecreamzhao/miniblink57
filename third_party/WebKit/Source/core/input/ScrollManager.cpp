    ...
    std::deque<int>& scrollChain)
{
    ...
    while (curBox) {
        ...
        if (curElement) {
            ...
            if (isViewportScrollingElement(*curElement) || curElement->isSameNode(documentElement))
                break;
        }
    ...
    Node* mousePressNode)
{
    ...
    while (curBox) {
        ScrollDirectionPhysical physicalDirection = toPhysicalDirection(direction, curBox->isHorizontalWritingMode(),
    ...
    const WebGestureEvent& gestureEvent)
{
    ...
    if (!m_scrollGestureHandlingNode || !m_scrollGestureHandlingNode->layoutObject())
        return WebInputEventResult::NotHandled;
    ...
    m_currentScrollChain.clear();
    std::unique_ptr<ScrollStateData> scrollStateData = WTF::makeUnique<ScrollStateData>();
    ...
    scrollStateData->is_direct_manipulation = gestureEvent.sourceDevice == WebGestureDeviceTouchscreen;
    ...
    const WebGestureEvent& gestureEvent)
{
    ...
    scrollStateData->is_in_inertial_phase = gestureEvent.inertialPhase() == WebGestureEvent::MomentumPhase;
    scrollStateData->is_direct_manipulation = gestureEvent.sourceDevice == WebGestureDeviceTouchscreen;
    scrollStateData->from_user_input = true;
    scrollStateData->delta_consumed_for_scroll_sequence = m_deltaConsumedForScrollSequence;
    ScrollState* scrollState = ScrollState::create(std::move(scrollStateData));
    ...
    m_previousGestureScrolledElement = scrollState->currentNativeScrollingElement();
    ...
    if ((!m_previousGestureScrolledElement || !isViewportScrollingElement(*m_previousGestureScrolledElement)) && frameHost())
