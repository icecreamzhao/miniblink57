...
enum class DragInitiator { Mouse,
    Touch };
...
WebInputEventResult MouseEventManager::dispatchMouseEvent(
    ...
    bool checkForListener)
{
    if (target && target->toNode() && (!checkForListener || target->hasEventListeners(mouseEventType))) {
        Node* targetNode = target->toNode();
        int clickCount = 0;
        if (mouseEventType == EventTypeNames::mouseup || mouseEventType == EventTypeNames::mousedown || mouseEventType == EventTypeNames::click || mouseEventType == EventTypeNames::auxclick || mouseEventType == EventTypeNames::dblclick) {
    ...
    const MouseEventWithHitTestResults& mev)
{
    ...
    bool contextMenuEvent = !RuntimeEnabledFeatures::auxclickEnabled() && mev.event().pointerProperties().button == WebPointerProperties::Button::Right;
#if OS(MACOSX)
    ...
    // the implementations.
    if (mev.event().pointerProperties().button == WebPointerProperties::Button::Left && mev.event().getModifiers() & PlatformEvent::CtrlKey)
        contextMenuEvent = true;
#endif

    WebInputEventResult clickEventResult = WebInputEventResult::NotHandled;
    const bool shouldDispatchClickEvent = m_clickCount > 0 && !contextMenuEvent && mev.innerNode() && m_clickNode && mev.innerNode()->canParticipateInFlatTree() && m_clickNode->canParticipateInFlatTree() && !(m_frame->eventHandler().selectionController().hasExtendedSelection() && isLinkSelection(mev));
    if (shouldDispatchClickEvent) {
        ...
        if (clickTargetNode) {
            clickEventResult = dispatchMouseEvent(
                clickTargetNode, !RuntimeEnabledFeatures::auxclickEnabled() || (mev.event().pointerProperties().button == WebPointerProperties::Button::Left) ? EventTypeNames::click : EventTypeNames::auxclick,
                mev.event(), nullptr);
        }
    }
    ...
    const PlatformMouseEvent& platformMouseEvent)
{
    ...
    PaintLayer* layerForLastNode = EventHandlingUtil::layerForNode(lastNodeUnderMouse);
    PaintLayer* layerForNodeUnderMouse = EventHandlingUtil::layerForNode(m_nodeUnderMouse.get());
    ...
    if (lastNodeUnderMouse && (!m_nodeUnderMouse || m_nodeUnderMouse->document() != m_frame->document())) {
        ...
        if (ScrollableArea* scrollableAreaForLastNode = EventHandlingUtil::associatedScrollableArea(layerForLastNode))
    ...
    if (m_nodeUnderMouse && (!lastNodeUnderMouse || lastNodeUnderMouse->document() != m_frame->document())) {
        ...
        // The mouse has moved between layers.
        if (ScrollableArea* scrollableAreaForNodeUnderMouse = EventHandlingUtil::associatedScrollableArea(layerForNodeUnderMouse))
    ...
    const MouseEventWithHitTestResults& event)
{
    ...
    m_mouseDownMayStartAutoscroll = m_frame->eventHandler().selectionController().mouseDownMayStartSelect() || (m_mousePressNode && m_mousePressNode->layoutBox() && m_mousePressNode->layoutBox()->canBeProgramaticallyScrolled());
...
{
    if (m_frame->settings() && m_frame->settings()->getTouchDragDropEnabled() && m_frame->view()) {
        ...
        MouseEventWithHitTestResults mev = EventHandlingUtil::performMouseEventHitTest(m_frame, request,
            mouseDragEvent);
    ...
    const MouseEventWithHitTestResults& event)
{
    ...
    if (layoutObject && m_mouseDownMayStartAutoscroll && !m_scrollManager->middleClickAutoscrollInProgress() && !m_frame->selection().selectedHTMLForClipboard().isEmpty()) {
    ...
    DragInitiator initiator)
{
    ...
    if (m_mouseDownMayStartDrag) {
        ...
        if (node) {
            DragController::SelectionDragPolicy selectionDragPolicy = event.event().timestamp() - m_mouseDownTimestamp < kTextDragDelay
    ...
    if (!m_mouseDownMayStartDrag) {
        return initiator == DragInitiator::Mouse && !m_frame->eventHandler().selectionController().mouseDownMayStartSelect() && !m_mouseDownMayStartAutoscroll;
    }
    ...
    if (initiator == DragInitiator::Mouse && !dragThresholdExceeded(event.event().position())) {
        dragState().m_dragSrc = nullptr;
        return true;
    }
    ...
    const MouseEventWithHitTestResults& event)
{
    ...
    m_mouseDownMayStartDrag = dispatchDragSrcEvent(EventTypeNames::dragstart, m_mouseDown) == WebInputEventResult::NotHandled && !m_frame->selection().isInPasswordField() && dragState().m_dragSrc;
    ...
    DataTransfer* dataTransfer)
{
    ...
    const bool cancelable = eventType != EventTypeNames::dragleave && eventType != EventTypeNames::dragend;
    ...
    const IntPoint& dragLocationInRootFrame) const
{
    ...
    return abs(delta.width()) >= kDragThresholdX || abs(delta.height()) >= kDragThresholdY;
}
