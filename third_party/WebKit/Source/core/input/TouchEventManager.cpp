    ...
    bool allTouchesReleased)
{
    ...
    for (auto touchInfo : touchInfos) {
        ...
        if (pointState != PlatformTouchPoint::TouchStationary && touchInfo.knownTarget) {
            ...
            changedTouches[pointState].m_pointerType = point.pointerProperties().pointerType;
        }
    }
    ...
    for (unsigned state = 0; state != PlatformTouchPoint::TouchStateEnd;
        ...
        for (const auto& eventTarget : changedTouches[state].m_targets) {
            ...
            if (event.touchStartOrFirstTouchMove() && touchInfos.size() == 1 && m_frame->isMainFrame()) {
                ...
                int64_t latencyInMicros = (TimeTicks::Now() - event.timestamp()).InMicroseconds();
                if (event.cancelable()) {
    ...
    HeapVector<TouchInfo>& touchInfos)
{
    for (auto& touchInfo : touchInfos) {
        ...
        if (touchInfo.point.state() == PlatformTouchPoint::TouchPressed) {
            ...
            if (m_touchSequenceDocument && (!touchInfo.touchNode || &touchInfo.touchNode->document() != m_touchSequenceDocument)) {
            ...
            TouchAction effectiveTouchAction = TouchActionUtil::computeEffectiveTouchAction(*touchInfo.touchNode);
    ...
    HeapVector<TouchInfo>& touchInfos)
{
    for (auto& touchInfo : touchInfos) {
        ...
        FloatPoint pagePoint = targetFrame->view()->rootFrameToContents(touchInfo.point.pos());
