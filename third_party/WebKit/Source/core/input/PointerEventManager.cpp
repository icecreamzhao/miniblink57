    ...
    bool checkForListener)
{
    ...
    if ((eventType == EventTypeNames::pointerout || eventType == EventTypeNames::pointerover) && m_nodeUnderPointer.contains(pointerId)) {
    ...
    EventTarget* target)
{
    if (m_nodeUnderPointer.contains(pointerEvent->pointerId())) {
        EventTargetAttributes node = m_nodeUnderPointer.get(pointerEvent->pointerId());
        if (!target) {
            m_nodeUnderPointer.remove(pointerEvent->pointerId());
        } else if (target != m_nodeUnderPointer.get(pointerEvent->pointerId()).target) {
    ...
    const Vector<PlatformTouchEvent>& coalescedEvents)
{
    ...
    if (event.type() == PlatformEvent::TouchEnd && !m_inCanceledStateForPointerTypeTouch && !touchInfos.isEmpty() && touchInfos[0].targetFrame) {
        possibleGestureToken = DocumentUserGestureToken::create(touchInfos[0].targetFrame->document());
    }
    ...
    HeapVector<TouchEventManager::TouchInfo>& touchInfos)
{
    for (const auto& touchPoint : event.touchPoints()) {
        ...
        int pointerId = m_pointerEventFactory.getPointerEventId(touchPoint.pointerProperties());
        ...
        if (touchInfo.point.state() == PlatformTouchPoint::TouchPressed || !m_pendingPointerCaptureTarget.contains(pointerId)) {
            ...
            if (node) {
                ...
                if (isHTMLCanvasElement(node)) {
                    HitTestCanvasResult* hitTestCanvasResult = toHTMLCanvasElement(node)->getControlAndIdIfHitRegionExists(
    ...
    HeapVector<TouchEventManager::TouchInfo>& touchInfos)
{
    ...
    for (auto touchInfo : touchInfos) {
        ...
        if (touchInfo.touchNode && touchInfo.targetFrame && touchPoint.state() != PlatformTouchPoint::TouchStationary && !m_inCanceledStateForPointerTypeTouch) {
            ...
            if (result != WebInputEventResult::NotHandled && pointerEvent->type() == EventTypeNames::pointerdown && pointerEvent->isPrimary()) {
    ...
    PointerEvent* pointerEvent)
{
    ...
    if (pointerEvent->type() == EventTypeNames::pointerup || pointerEvent->type() == EventTypeNames::pointercancel) {
        releasePointerCapture(pointerEvent->pointerId());
...
{
    PointerEvent* pointerEvent = m_pointerEventFactory.create(mouseEventType, mouseEvent, coalescedEvents,
        m_frame->document()->domWindow());

    // This is for when the mouse is released outside of the page.
    if (pointerEvent->type() == EventTypeNames::pointermove && !pointerEvent->buttons()) {
        releasePointerCapture(pointerEvent->pointerId());
    ...
    WebInputEventResult result = dispatchPointerEvent(effectiveTarget, pointerEvent);
    ...
    if (pointerEvent->isPrimary() && !m_preventMouseEventForPointerType[toPointerTypeIndex(mouseEvent.pointerProperties().pointerType)]) {
        ...
        result = EventHandlingUtil::mergeEventResult(
            result, m_mouseEventManager->dispatchMouseEvent(mouseTarget, mouseEventType, mouseEvent, nullptr));
    }
...
{
    if (m_pointerEventFactory.getPointerType(pointerId) != WebPointerProperties::PointerType::Touch)
        return false;
    Node* lastNodeReceivingEvent = m_nodeUnderPointer.contains(pointerId)
        ? m_nodeUnderPointer.get(pointerId).target->toNode()
        : nullptr;
    return lastNodeReceivingEvent && lastNodeReceivingEvent->document().frame() == frame;
}
