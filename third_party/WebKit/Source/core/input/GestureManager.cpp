...
WebInputEventResult GestureManager::handleGestureTapDown(
    const GestureEventWithHitTestResults& targetedEvent)
{
    m_suppressMouseEventsFromGestures = m_pointerEventManager->primaryPointerdownCanceled(
    ...
    const GestureEventWithHitTestResults& targetedEvent)
{
    ...
    if (!m_suppressMouseEventsFromGestures) {
        ...
        mouseDownEventResult = m_mouseEventManager->setMousePositionAndDispatchMouseEvent(
        ...
        if (mouseDownEventResult == WebInputEventResult::NotHandled) {
            mouseDownEventResult = m_mouseEventManager->handleMouseFocus(
                currentHitTest, m_frame->document()->domWindow()->getInputDeviceCapabilities()->firesTouchEvents(true));
        }
        if (mouseDownEventResult == WebInputEventResult::NotHandled) {
    ...
    if (tappedNonTextNode) {
        if (currentHitTest.innerNode()) {
            ...
            clickEventResult = m_mouseEventManager->setMousePositionAndDispatchMouseEvent(
    ...
    const GestureEventWithHitTestResults& targetedEvent)
{
    ...
    if (innerNode && innerNode->layoutObject() && m_selectionController->handleGestureLongPress(gestureEvent, hitTestResult)) {
