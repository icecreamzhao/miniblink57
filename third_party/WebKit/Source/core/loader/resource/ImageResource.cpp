...
public:
    ImageResourceInfoImpl(ImageResource* resource)
        : m_resource(resource)
    {
        DCHECK(m_resource);
    }
...
private:
    ...
    {
        return m_resource->m_isSchedulingReload || m_resource->shouldReloadBrokenPlaceholder();
    }
...
public:
        ...
        const String&) const override
    {
        return new ImageResource(request, options, ImageResourceContent::create(),
            m_fetchRequest->placeholderImageRequestType() == FetchRequest::AllowPlaceholder);
    }
    ...
    ResourceFetcher* fetcher)
{
    ...
    if (resource && request.placeholderImageRequestType() != FetchRequest::AllowPlaceholder && resource->m_isPlaceholder) {
...
{
    DCHECK((m_multipartParser && isLoading()) || !data() || getContent()->hasImage());
...
void ImageResource::destroyDecodedDataIfPossible()
{
    ...
    if (getContent()->hasImage() && !isPreloaded() && getContent()->isRefetchableDataFromDiskCache()) {
...
void ImageResource::allClientsAndObserversRemoved()
{
    ...
    if (!ThreadHeap::willObjectBeLazilySwept(this)) {
        Platform::current()->currentThread()->getWebTaskRunner()->postTask(
            BLINK_FROM_HERE, WTF::bind(&ImageResourceContent::doResetAnimation, wrapWeakPersistent(getContent())));
    } else {
    ...
    std::unique_ptr<WebDataConsumerHandle> handle)
{
    ...
    if (RuntimeEnabledFeatures::clientHintsEnabled()) {
        m_devicePixelRatioHeaderValue = this->response()
...
{
    if (!(resource.resourceRequest().previewsState() & WebURLRequest::ServerLoFiOn)) {
        return false;
    }
    return !resource.isLoaded() || resource.response().httpHeaderField("chrome-proxy-content-transform").contains("empty-image");
}
        ...
        doesCurrentFrameHasSingleSecurityOrigin) const
{
    ...
    if (doesCurrentFrameHasSingleSecurityOrigin != ImageResourceInfo::HasSingleSecurityOrigin)
        return false;
