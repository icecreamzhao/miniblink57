...
void ImageResourceContent::addObserver(ImageResourceObserver* observer)
{
    ...
    if (isLoaded() && m_observers.contains(observer) && !m_info->schedulingReloadOrShouldReloadBrokenPlaceholder()) {
...
bool ImageResourceContent::shouldUpdateImageImmediately() const
{
    ...
    // images.
    return m_sizeAvailable == Image::SizeUnavailable || (m_image && m_image->maybeAnimated());
}
    ...
    SizeType sizeType)
{
    ...
    if (sizeType == IntrinsicCorrectedToDPR && hasDevicePixelRatioHeaderValue() && devicePixelRatioHeaderValue() > 0)
    ...
    const IntRect* changeRect)
{
    ...
    for (auto* observer : m_observers.asVector()) {
        if (m_observers.contains(observer)) {
            observer->imageChanged(this, changeRect);
            if (notifyingFinishOption == ShouldNotifyFinish && m_observers.contains(observer) && !m_info->schedulingReloadOrShouldReloadBrokenPlaceholder()) {
                markObserverFinished(observer);
                observer->imageNotifyFinished(this);
            }
        }
    ...
    bool allDataReceived)
{
    ...
    case ClearAndUpdateImage:
        ...
        if (m_info->isPlaceholder() && allDataReceived && m_image && !m_image->isNull()) {
...
bool ImageResourceContent::isAccessAllowed(SecurityOrigin* securityOrigin)
{
    return m_info->isAccessAllowed(
        securityOrigin, getImage()->currentFrameHasSingleSecurityOrigin() ? ImageResourceInfo::HasSingleSecurityOrigin : ImageResourceInfo::HasMultipleSecurityOrigin);
}
...
{
    return getStatus() == ResourceStatus::LoadError || getStatus() == ResourceStatus::DecodeError;
}
