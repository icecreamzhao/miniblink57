    ...
    ReferrerPolicy referrerPolicy)
{
    ...
    if (!url.isNull()) {
        ...
        if (updateBehavior != UpdateForcedReload && document.settings() && document.settings()->getFetchImagePlaceholders()) {
    ...
    ReferrerPolicy referrerPolicy)
{
    ...
    if (m_loadingImageDocument && updateBehavior != UpdateForcedReload) {
        ImageResource* imageResource = ImageResource::create(imageSourceToKURL(m_element->imageSourceURL()));
