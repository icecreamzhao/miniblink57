    ...
    , m_parserContext(o.m_parserContext)
{
    ...
    for (unsigned i = 0; i < m_namespaceRules.size(); ++i) {
        m_namespaceRules[i] = static_cast<StyleRuleNamespace*>(o.m_namespaceRules[i]->copy());
    }
    ...
    const SecurityOrigin* securityOrigin)
{
    ...
    CSSStyleSheetResource::MIMETypeCheck mimeTypeCheck = isQuirksModeBehavior(m_parserContext->mode()) && isSameOriginRequest
    ...
    const CSSParserContext* context = CSSParserContext::createWithStyleSheetContents(parserContext(), this);
