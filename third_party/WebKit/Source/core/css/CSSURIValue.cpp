...
{
    return m_isLocal || equalIgnoringFragmentIdentifier(absoluteUrl(), document.url());
}
...
bool CSSURIValue::equals(const CSSURIValue& other) const
{
    ...
    if (m_isLocal != other.m_isLocal)
        return false;
    return (m_isLocal && m_relativeUrl == other.m_relativeUrl) || m_absoluteUrl == other.m_absoluteUrl;
}
