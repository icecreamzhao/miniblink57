...
namespace {
    ...
    {
        return pseudo == CSSSelector::PseudoAny || pseudo == CSSSelector::PseudoCue || pseudo == CSSSelector::PseudoHost || pseudo == CSSSelector::PseudoHostContext || pseudo == CSSSelector::PseudoNot || pseudo == CSSSelector::PseudoSlotted;
    }
...
void RuleFeatureSet::updateInvalidationSets(const RuleData& ruleData)
{
    ...
    const CSSSelector* lastInCompound = extractInvalidationSetFeaturesFromCompound(ruleData.selector(), features,
        Subject);
    ...
    InvalidationSetFeatures& descendantFeatures)
{
    ...
    bool selectorListContainsUniversal = simpleSelector.getPseudoType() == CSSSelector::PseudoNot || simpleSelector.getPseudoType() == CSSSelector::PseudoHostContext;
    ...
    InvalidationSetFeatures& descendantFeatures)
{
    ...
    while (compound) {
        const CSSSelector* lastInCompound = addFeaturesToInvalidationSetsForCompoundSelector(
    ...
    for (const CSSSelector* current = &selector; current;
        ...
        case CSSSelector::PseudoHostContext:
            if (!foundHostPseudo && relation == CSSSelector::SubSelector)
                return SelectorNeverMatches;
            if (!current->isLastInTagHistory() && current->tagHistory()->match() != CSSSelector::PseudoElement && !current->tagHistory()->isHostPseudoClass()) {
                return SelectorNeverMatches;
            }
        ...
        } else if (maxDirectAdjacentSelectors && ((relation != CSSSelector::SubSelector) || current->isLastInTagHistory())) {
...
{
    return !classes.isEmpty() || !attributes.isEmpty() || !ids.isEmpty() || !tagNames.isEmpty() || customPseudoElement;
}

bool RuleFeatureSet::InvalidationSetFeatures::hasTagIdClassOrAttribute() const
{
    return !classes.isEmpty() || !attributes.isEmpty() || !ids.isEmpty() || !tagNames.isEmpty();
}
