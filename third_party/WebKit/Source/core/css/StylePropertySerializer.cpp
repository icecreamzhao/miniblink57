    ...
    for (unsigned i = 0; i < m_propertySet->propertyCount(); ++i) {
        ...
        if (CSSProperty::isAffectedByAllProperty(property.id())) {
            ...
            if (static_cast<unsigned>(m_allIndex) >= i)
                continue;
            if (property.value().equals(allProperty.value()) && property.isImportant() == allProperty.isImportant())
                continue;
    ...
    StringBuilder& result) const
{
    ...
    // expanded.
    if (propertyID == CSSPropertyFontVariantLigatures && val->isIdentifierValue() && toCSSIdentifierValue(val)->getValueID() == CSSValueNone) {
        value = "no-common-ligatures no-discretionary-ligatures "
...
String StylePropertySerializer::fontValue() const
{
    ...
    if ((ligaturesValue->isIdentifierValue() && toCSSIdentifierValue(ligaturesValue)->getValueID() != CSSValueNormal) || ligaturesValue->isValueList() || (numericValue->isIdentifierValue() && toCSSIdentifierValue(numericValue)->getValueID() != CSSValueNormal) || numericValue->isValueList())
        return emptyString();
    ...
    if (val->isIdentifierValue() && (toCSSIdentifierValue(val)->getValueID() != CSSValueSmallCaps && toCSSIdentifierValue(val)->getValueID() != CSSValueNormal))
        return emptyString();
    ...
    for (size_t layer = 0; layer < numLayers; layer++) {
        ...
        for (unsigned propertyIndex = 0; propertyIndex < size; propertyIndex++) {
            ...
            if (property == CSSPropertyBackgroundRepeatX || property == CSSPropertyWebkitMaskRepeatX) {
                DCHECK(shorthand.properties()[propertyIndex + 1] == CSSPropertyBackgroundRepeatY || shorthand.properties()[propertyIndex + 1] == CSSPropertyWebkitMaskRepeatY);
    ...
    const CSSValue& repeatYCSSValue)
{
    ...
    const CSSIdentifierValue& repeatX = repeatXCSSValue.isInitialValue() ? initialRepeatValue
                                                                         : toCSSIdentifierValue(repeatXCSSValue);
    const CSSIdentifierValue& repeatY = repeatYCSSValue.isInitialValue() ? initialRepeatValue
