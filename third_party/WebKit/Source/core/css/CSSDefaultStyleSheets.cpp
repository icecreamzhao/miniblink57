...
#if ENABLE_WML
#include "core/wml/WMLUserAgentStyleSheet.h"
#endif
...
{
    StyleSheetContents* sheet = StyleSheetContents::create(CSSParserContext::create(UASheetMode));
...
#if ENABLE_WML
    m_wmlDefaultStyleSheet = nullptr;
#endif
}
...
#if ENABLE_WML
StyleSheetContents* CSSDefaultStyleSheets::ensureWMLStyleSheetsForElement()
{
    if (!m_wmlDefaultStyleSheet) {
        String wmlDefaultRules = String(wmlUserAgentStyleSheet, sizeof(wmlUserAgentStyleSheet)) + LayoutTheme::theme().extraFullscreenStyleSheet();
        m_wmlDefaultStyleSheet = parseUASheet(wmlDefaultRules);
    }
    m_defaultStyle->addRulesFromSheet(wmlDefaultStyleSheet(), screenEval());
    m_defaultQuirksStyle->addRulesFromSheet(wmlDefaultStyleSheet(), screenEval());

    ASSERT(!m_defaultStyle->features().hasIdsInSelectors());
    ASSERT(m_defaultStyle->features().siblingRules().isEmpty());
    return m_wmlDefaultStyleSheet;
}
#endif
    ...
    const Element& element)
{
    ...
    if (element.namespaceURI() == MathMLNames::mathmlNamespaceURI && !m_mathmlStyleSheet) {
...
#if ENABLE_WML
    visitor->trace(m_wmlDefaultStyleSheet);
#endif
}
