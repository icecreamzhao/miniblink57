...
namespace {
    ...
    public:
        ...
        StyleAttributeMutationScope(AbstractPropertySetCSSStyleDeclaration* decl)
        {
            ...
            m_mutationRecipients = MutationObserverInterestGroup::createForAttributesMutation(
                *s_currentDecl->parentElement(), HTMLNames::styleAttr);
            bool shouldReadOldValue = (m_mutationRecipients && m_mutationRecipients->isOldValueRequested()) || definitionIfStyleChangedCallback(s_currentDecl->parentElement());
...
void InlineCSSStyleDeclaration::didMutate(MutationType type)
{
    ...
    m_parentElement->setNeedsStyleRecalc(
        LocalStyleChange, StyleChangeReasonForTracing::create(StyleChangeReason::InlineCSSStyleMutated));
