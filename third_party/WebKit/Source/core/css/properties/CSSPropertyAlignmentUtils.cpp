...
namespace {
        ...
        CSSParserTokenRange& range)
    {
        ...
        if (id == CSSValueStart || id == CSSValueEnd || id == CSSValueCenter || id == CSSValueSelfStart || id == CSSValueSelfEnd || id == CSSValueFlexStart || id == CSSValueFlexEnd || id == CSSValueLeft || id == CSSValueRight)
    ...
    CSSParserTokenRange& range)
{
    ...
    CSSIdentifierValue* overflowPosition = CSSPropertyParserHelpers::consumeIdent<CSSValueUnsafe, CSSValueSafe>(
        range);
    CSSIdentifierValue* selfPosition = consumeSelfPositionKeyword(range);
    if (!selfPosition)
        return nullptr;
    if (!overflowPosition) {
        overflowPosition = CSSPropertyParserHelpers::consumeIdent<CSSValueUnsafe, CSSValueSafe>(
            range);
    }
    if (overflowPosition) {
