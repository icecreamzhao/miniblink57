    ...
    const CSSParserContext* context)
{
    ...
    if (token.type() == IdentToken) {
        ...
        zoom = CSSPropertyParserHelpers::consumePercent(range, ValueRangeNonNegative);
        if (!zoom) {
            zoom = CSSPropertyParserHelpers::consumeNumber(range, ValueRangeNonNegative);
        }
    }
    if (zoom && context->isUseCounterRecordingEnabled()) {
        if (!(token.id() == CSSValueNormal || (token.type() == NumberToken && toCSSPrimitiveValue(zoom)->getDoubleValue() == 1) || (token.type() == PercentageToken && toCSSPrimitiveValue(zoom)->getDoubleValue() == 100)))
