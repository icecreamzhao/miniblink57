...
static bool computationallyIndependent(const CSSValue& value)
{
    ...
    if (value.isPrimitiveValue()) {
        ...
        for (size_t i = 0; i < lengthArray.values.size(); i++) {
            if (lengthArray.typeFlags.get(i) && i != CSSPrimitiveValue::UnitTypePixels && i != CSSPrimitiveValue::UnitTypePercentage)
                return false;
        }
        return true;
    }
    ...
    ExceptionState& exceptionState)
{
    ...
    InterpolationTypes interpolationTypes = interpolationTypesForSyntax(atomicName, syntaxDescriptor);
    ...
    if (descriptor.hasInitialValue()) {
        ...
        initial = &StyleBuilderConverter::convertRegisteredPropertyInitialValue(*initial);
