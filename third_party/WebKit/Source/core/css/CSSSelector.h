...
{
    return m_pseudoType == PseudoHover || m_pseudoType == PseudoActive || m_pseudoType == PseudoFocus || m_pseudoType == PseudoDrag;
}

inline bool CSSSelector::isIdClassOrAttributeSelector() const
{
    return isAttributeSelector() || match() == CSSSelector::Id || match() == CSSSelector::Class;
}
