    ...
    , m_isInterventionTriggered(false)
{
    ...
    if (shouldTriggerWebFontsIntervention()) {
        ...
        m_fontSelector->document()->addConsoleMessage(ConsoleMessage::create(
            ...
            "Slow network is detected. Fallback font will be used while loading: " + m_font->url().elidedString()));
    }
}
...
bool RemoteFontFaceSource::shouldTriggerWebFontsIntervention()
{
    ...
    bool networkIsSlow = isV2Enabled ? isEffectiveConnectionTypeSlowFor(m_fontSelector->document())
    ...
    const FontDescription& fontDescription)
{
    ...
    SimpleFontData* temporaryFont = FontCache::fontCache()->getNonRetainedLastResortFallbackFont(
        fontDescription);
    if (!temporaryFont) {
        ASSERT_NOT_REACHED();
        return nullptr;
    ...
    RefPtr<CSSCustomFontData> cssFontData = CSSCustomFontData::create(
        this, m_period == BlockPeriod ? CSSCustomFontData::InvisibleFallback : CSSCustomFontData::VisibleFallback);
...
void RemoteFontFaceSource::beginLoadIfNeeded()
{
    if (m_fontSelector->document() && m_font->stillNeedsLoad()) {
        if (!m_font->url().protocolIsData() && !m_font->isLoaded() && m_display == FontDisplayAuto && m_font->isLowPriorityLoadingAllowedForRemoteFont()) {
...
{
    if (!m_isLongLimitExceeded && m_fontDisplay == FontDisplayAuto && !isCorsFailed && !loadError) {
