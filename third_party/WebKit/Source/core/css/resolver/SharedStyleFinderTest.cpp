...
protected:
    ...
    ShadowRoot& attachShadow(Element& host)
    {
        ShadowRootInit init;
        init.setMode("open");
        ShadowRoot* shadowRoot = host.attachShadow(ScriptState::forMainWorld(document().frame()), init,
            ASSERT_NO_EXCEPTION);
    ...
    {
        StyleRuleBase* newRule = CSSParser::parseRule(CSSParserContext::create(HTMLStandardMode),
