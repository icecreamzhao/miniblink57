...
namespace CSSPropertyParserHelpers {
    ...
    public:
        ...
        CSSPrimitiveValue* consumeNumber()
        {
            if (!m_calcValue)
                return nullptr;
            m_sourceRange = m_range;
            CSSPrimitiveValue::UnitType unitType = m_calcValue->isInt() ? CSSPrimitiveValue::UnitType::Integer
    ...
    {
        return value == 0 || cssParserMode == SVGAttributeMode || (cssParserMode == HTMLQuirksMode && unitless == UnitlessQuirk::Allow);
    }
    ...
    CSSPrimitiveValue* consumeResolution(CSSParserTokenRange& range)
    {
        ...
        if (token.type() != DimensionToken)
            return nullptr;
        CSSPrimitiveValue::UnitType unit = token.unitType();
        if (unit == CSSPrimitiveValue::UnitType::DotsPerPixel || unit == CSSPrimitiveValue::UnitType::DotsPerInch || unit == CSSPrimitiveValue::UnitType::DotsPerCentimeter) {
            return CSSPrimitiveValue::create(
        ...
        bool acceptQuirkyColors)
    {
        ...
        if (token.type() == HashToken) {
            ...
            if (token.type() == NumberToken || token.type() == DimensionToken) {
                if (token.numericValueType() != IntegerValueType || token.numericValue() < 0. || token.numericValue() >= 1000000.)
                    return false;
    ...
    static bool parseColorFunction(CSSParserTokenRange& range, RGBA32& result)
    {
        ...
        if ((functionId <= CSSValueRgba && !parseRGBParameters(colorRange, result, functionId == CSSValueRgba)) || (functionId >= CSSValueHsl && !parseHSLParameters(colorRange, result, functionId == CSSValueHsla)))
            return false;
        range = colorRange;
        return true;
    }
    ...
    static void positionFromTwoValues(CSSValue* value1,
        ...
        CSSValue*& resultY)
    {
        bool mustOrderAsXY = isHorizontalPositionKeywordOnly(*value1) || isVerticalPositionKeywordOnly(*value2) || !value1->isIdentifierValue() || !value2->isIdentifierValue();
        ...
        CSSValue*& resultY)
    {
        ...
        if (value1->isIdentifierValue() && value2->isIdentifierValue() != (range.peek().type() == IdentToken) && (value2->isIdentifierValue() ? toCSSIdentifierValue(value2)->getValueID() : toCSSIdentifierValue(value1)->getValueID()) != CSSValueCenter)
        ...
        CSSGradientRepeat repeating)
    {
        ...
        CSSIdentifierValue* sizeKeyword = consumeIdent<CSSValueClosestSide, CSSValueClosestCorner,
        ...
        CSSGradientRepeat repeating)
    {
        ...
        if (!verticalSize && horizontalSize && horizontalSize->isPercentage())
            return nullptr;
        if ((horizontalSize && horizontalSize->isCalculatedPercentageWithLength()) || (verticalSize && verticalSize->isCalculatedPercentageWithLength()))
            return nullptr;
        ...
        ConsumeGeneratedImagePolicy generatedImage)
    {
        ...
        if (range.peek().type() == FunctionToken) {
            ...
            if (generatedImage == ConsumeGeneratedImagePolicy::Allow && isGeneratedImage(id)) {
    ...
    {
        return equalIgnoringASCIICase(keyword, "initial") || equalIgnoringASCIICase(keyword, "inherit") || equalIgnoringASCIICase(keyword, "unset") || equalIgnoringASCIICase(keyword, "default");
    }
