...
TEST_F(CSSLazyParsingTest, DontLazyParseBeforeAfter)
{
    ...
    String sheetText = "p::before { content: 'foo' } p .class::after { content: 'bar' } ";
    CSSParser::parseSheet(context, styleSheet, sheetText, true /* lazy parse */);
...
{
    std::unique_ptr<DummyPageHolder> dummyHolder = DummyPageHolder::create(IntSize(500, 500));
    ...
    {
        CSSStyleSheet* sheet = CSSStyleSheet::create(m_cachedContents, dummyHolder->document());
        DCHECK(sheet);
    ...
    std::unique_ptr<DummyPageHolder> dummyHolder2 = DummyPageHolder::create(IntSize(500, 500));
    CSSStyleSheet* sheet2 = CSSStyleSheet::create(m_cachedContents, dummyHolder2->document());
