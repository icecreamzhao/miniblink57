    ...
    StyleRule::RuleType ruleType)
{
    ...
    if (ruleType == StyleRule::Viewport) {
        parseSuccess = (RuntimeEnabledFeatures::cssViewportEnabled() || isUASheetBehavior(context->mode())) && parser.parseViewportDescriptor(resolvedProperty, important);
...
public:
    ...
    enum class ParseResult { ConsumedValue,
        DisallowedValue,
        UnknownValue };
...
static CSSValue* consumeFontVariantNumeric(CSSParserTokenRange& range)
{
    ...
    do {
        if (numericParser.consumeNumeric(range) != FontVariantNumericParser::ParseResult::ConsumedValue)
            return nullptr;
    } while (!range.atEnd());
...
static CSSValue* consumeCubicBezier(CSSParserTokenRange& range)
{
    ...
    double x1, y1, x2, y2;
    if (consumeNumberRaw(args, x1) && x1 >= 0 && x1 <= 1 && consumeCommaIncludingWhitespace(args) && consumeNumberRaw(args, y1) && consumeCommaIncludingWhitespace(args) && consumeNumberRaw(args, x2) && x2 >= 0 && x2 <= 1 && consumeCommaIncludingWhitespace(args) && consumeNumberRaw(args, y2) && args.atEnd()) {
        range = rangeCopy;
...
{
    CSSValueID id = range.peek().id();
    if (id == CSSValueEase || id == CSSValueLinear || id == CSSValueEaseIn || id == CSSValueEaseOut || id == CSSValueEaseInOut || id == CSSValueStepStart || id == CSSValueStepEnd || id == CSSValueStepMiddle)
        return consumeIdent(range);
    ...
    do {
        ...
        do {
            ...
            for (size_t i = 0; i < longhandCount; ++i) {
                ...
                if (CSSValue* value = consumeAnimationValue(shorthand.properties()[i], m_range,
            ...
            if (parsedValue && filterType != CSSValueSaturate && filterType != CSSValueContrast) {
                ...
                if (toCSSPrimitiveValue(parsedValue)->getDoubleValue() > maxAllowed) {
                    parsedValue = CSSPrimitiveValue::create(
                        maxAllowed, isPercentage ? CSSPrimitiveValue::UnitType::Percentage : CSSPrimitiveValue::UnitType::Number);
                }
            }
        }
    }
...
static CSSValue* consumeTextDecorationLine(CSSParserTokenRange& range)
{
    ...
    CSSValueList* list = CSSValueList::createSpaceSeparated();
    while (true) {
        CSSIdentifierValue* ident = consumeIdent<CSSValueBlink, CSSValueUnderline, CSSValueOverline,
...
static CSSValue* consumePath(CSSParserTokenRange& range)
{
    ...
    if (buildByteStreamFromString(pathString, *byteStream) != SVGParseStatus::NoError || !functionArgs.atEnd())
        return nullptr;
    ...
    bool isMotionPath)
{
    ...
    if (context->isUseCounterRecordingEnabled() && value && !value->isIdentifierValue()) {
        if (isMotionPath) {
...
bool CSSPropertyParser::consumeOffsetShorthand(bool important)
{
    ...
    const CSSValue* offsetDistance = consumeLengthOrPercent(m_range, m_context->mode(), ValueRangeAll);
...
static bool consumePerspective(CSSParserTokenRange& args,
    ...
    bool useLegacyParsing)
{
    CSSPrimitiveValue* parsedValue = consumeLength(args, context->mode(), ValueRangeNonNegative);
    ...
    bool useLegacyParsing)
{
    ...
    CSSValueList* list = CSSValueList::createSpaceSeparated();
    do {
        CSSValue* parsedTransformValue = consumeTransformValue(range, context, useLegacyParsing);
    ...
    CSSPropertyID unresolvedProperty)
{
    if (range.peek().id() == CSSValueNone)
        return consumeIdent(range);
    CSSPrimitiveValue* parsedValue = consumeLength(range, context->mode(), ValueRangeAll);
    ...
    const CSSParserContext* context)
{
    ...
    if (CSSValue* shapeValue = CSSPropertyShapeUtils::consumeBasicShape(range, context)) {
...
static CSSValue* consumeBorderImageWidth(CSSParserTokenRange& range)
{
    ...
    for (size_t index = 0; index < 4; ++index) {
        value = consumeNumber(range, ValueRangeNonNegative);
        if (!value)
            value = consumeLengthOrPercent(range, HTMLStandardMode, ValueRangeNonNegative,
                UnitlessQuirk::Forbid);
        if (!value)
...
static CSSValue* consumeReflect(CSSParserTokenRange& range,
    const CSSParserContext* context)
{
    CSSIdentifierValue* direction = consumeIdent<CSSValueAbove, CSSValueBelow, CSSValueLeft, CSSValueRight>(
        range);
    if (!direction)
        return nullptr;
...
enum TrackListType { GridTemplate,
    ...
    CSSPropertyID currentShorthand)
{
    ...
    // statement.
    const CSSPropertyDescriptor& cssPropertyDesc = CSSPropertyDescriptor::get(property);
    ...
    case CSSPropertyBorderTopColor: {
        bool allowQuirkyColors = inQuirksMode() && (currentShorthand == CSSPropertyInvalid || currentShorthand == CSSPropertyBorderColor);
    ...
    case CSSPropertyBorderTopWidth: {
        bool allowQuirkyLengths = inQuirksMode() && (currentShorthand == CSSPropertyInvalid || currentShorthand == CSSPropertyBorderWidth);
    ...
    case CSSPropertyOffsetPath:
        return consumeOffsetPath(
            m_range, m_context,
            currentShorthand == CSSPropertyMotion || unresolvedProperty == CSSPropertyAliasMotionPath);
...
bool CSSPropertyParser::consumeFontVariantShorthand(bool important)
{
    ...
    do {
        ...
        if (ligaturesParseResult == FontVariantLigaturesParser::ParseResult::ConsumedValue || numericParseResult == FontVariantNumericParser::ParseResult::ConsumedValue)
            continue;

        if (ligaturesParseResult == FontVariantLigaturesParser::ParseResult::DisallowedValue || numericParseResult == FontVariantNumericParser::ParseResult::DisallowedValue)
            return false;
...
bool CSSPropertyParser::consumeBorderSpacing(bool important)
{
    ...
    if (!m_range.atEnd()) {
        verticalSpacing = consumeLength(m_range, m_context->mode(), ValueRangeNonNegative,
            UnitlessQuirk::Allow);
    }
            ...
            for (size_t i = 0; i < longhandCount; ++i) {
                ...
                if (property == CSSPropertyBackgroundRepeatX || property == CSSPropertyWebkitMaskRepeatX) {
                    ...
                    if (!value || !parsedLonghand[i - 1]) // Position must have been
                        ...
                        return false;
                } else if (property == CSSPropertyBackgroundPositionY || property == CSSPropertyBackgroundRepeatY || property == CSSPropertyWebkitMaskPositionY || property == CSSPropertyWebkitMaskRepeatY) {
                    continue;
                } else {
