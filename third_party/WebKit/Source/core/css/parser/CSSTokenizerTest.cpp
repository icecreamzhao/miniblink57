...
{
    return CSSParserToken(AtKeywordToken, string);
}
static CSSParserToken string(const String& string)
{
    return CSSParserToken(StringToken, string);
}
static CSSParserToken function(const String& string)
{
    return CSSParserToken(FunctionToken, string);
}
static CSSParserToken url(const String& string)
{
    return CSSParserToken(UrlToken, string);
}
static CSSParserToken hash(const String& string, HashTokenType type)
{
    return CSSParserToken(type, string);
}
static CSSParserToken delim(char c)
{
    return CSSParserToken(DelimiterToken, c);
}
...
TEST(CSSTokenizerTest, NumberToken)
{
    ...
    TEST_TOKENS("+ 5", delim('+'), whitespace(),
        number(IntegerValueType, 5, NoSign));
    ...
    TEST_TOKENS("1.e2", number(IntegerValueType, 1, NoSign), delim('.'),
        ident("e2"));
    TEST_TOKENS("2e3.5", number(NumberValueType, 2000, NoSign),
        number(NumberValueType, 0.5, NoSign));
    TEST_TOKENS("2e3.", number(NumberValueType, 2000, NoSign), delim('.'));
    TEST_TOKENS("1000000000000000000000000",
        number(IntegerValueType, 1e24, NoSign));
}
...
TEST(CSSTokenizerTest, DimensionToken)
{
    ...
    TEST_TOKENS("2e.5", dimension(IntegerValueType, 2, "e"),
        number(NumberValueType, 0.5, NoSign));
    TEST_TOKENS("2e+.5", dimension(IntegerValueType, 2, "e"),
        number(NumberValueType, 0.5, PlusSign));
}
...
TEST(CSSTokenizerTest, UnicodeRangeToken)
{
    ...
    TEST_TOKENS("u+222+111", unicodeRange(0x222, 0x222),
        number(IntegerValueType, 111, PlusSign));
    TEST_TOKENS("u+12345678", unicodeRange(0x123456, 0x123456),
        number(IntegerValueType, 78, NoSign));
