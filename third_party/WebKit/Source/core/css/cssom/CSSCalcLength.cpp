...
{
    std::unique_ptr<UnitData> unitData = UnitData::fromExpressionNode(value.cssCalcValue()->expressionNode());
    if (unitData)
    ...
    const CSSCalcExpressionNode* expressionNode)
{
    ...
    DCHECK(category == CalculationCategory::CalcLength || category == CalculationCategory::CalcPercentLength || category == CalculationCategory::CalcPercent);
    ...
    for (unsigned i = 0; i < CSSLengthValue::kNumSupportedUnits; ++i) {
        ...
        if (node) {
            node = CSSCalcValue::createExpressionNode(
                node, CSSCalcValue::createExpressionNode(CSSPrimitiveValue::create(std::abs(value), unitFromIndex(i))),
