...
{
    const CSSValue* cssValue = m_ownerElement->ensureMutableInlineStyle().getPropertyCSSValue(
        propertyID);
    if (!cssValue)
...
InlineStylePropertyMap::getIterationEntries()
{
    ...
    for (unsigned i = 0; i < inlineStyleSet.propertyCount(); i++) {
        ...
        if (propertyID == CSSPropertyVariable) {
            ...
            CSSStyleValueVector styleValueVector = StyleValueFactory::cssValueToStyleValueVector(
