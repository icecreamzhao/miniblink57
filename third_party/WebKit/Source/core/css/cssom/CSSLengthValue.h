...
public:
    ...
    {
        return (CSSPrimitiveValue::isLength(unit) || unit == CSSPrimitiveValue::UnitType::Percentage) && unit != CSSPrimitiveValue::UnitType::QuirkyEms && unit != CSSPrimitiveValue::UnitType::UserUnits;
    }
...
DEFINE_TYPE_CASTS(CSSLengthValue,
    CSSStyleValue,
    value,
    (value->type() == CSSStyleValue::SimpleLengthType || value->type() == CSSStyleValue::CalcLengthType),
    (value.type() == CSSStyleValue::SimpleLengthType || value.type() == CSSStyleValue::CalcLengthType));
