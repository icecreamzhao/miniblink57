...
namespace {
    ...
    {
        if (value.isCSSWideKeyword() || value.isIdentifierValue() || value.isCustomIdentValue())
    ...
    CSSStyleValue* createStyleValueWithProperty(CSSPropertyID propertyID,
        const CSSValue& value)
    {
        CSSStyleValue* styleValue = createStyleValueWithPropertyInternal(propertyID, value);
        if (styleValue)
            return styleValue;
    ...
    const CSSValue& cssValue)
{
    ...
    CSSStyleValue* styleValue = createStyleValueWithProperty(propertyID, cssValue);
    if (styleValue) {
