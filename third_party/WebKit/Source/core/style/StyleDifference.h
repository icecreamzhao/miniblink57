...
public:
    ...
    {
        bool result = m_paintInvalidationType || m_layoutType || m_propertySpecificDifferences;
        // m_recomputeOverflow, m_scrollAnchorDisablingPropertyChanged and
        // are never set without other flags set.
        DCHECK(result || (!m_recomputeOverflow && !m_scrollAnchorDisablingPropertyChanged));
        return result;
    }
