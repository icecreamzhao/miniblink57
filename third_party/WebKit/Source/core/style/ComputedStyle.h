...
protected:
    ...
    struct InheritedData {
        ...
        {
            return (m_hasSimpleUnderline == other.m_hasSimpleUnderline) && (m_cursorStyle == other.m_cursorStyle) && (m_insideLink == other.m_insideLink);
        }
    ...
    struct NonInheritedData {
        ...
        {
            return m_effectiveDisplay == other.m_effectiveDisplay && m_originalDisplay == other.m_originalDisplay && m_overflowAnchor == other.m_overflowAnchor && m_overflowX == other.m_overflowX && m_overflowY == other.m_overflowY && m_verticalAlign == other.m_verticalAlign && m_clear == other.m_clear && m_position == other.m_position && m_tableLayout == other.m_tableLayout &&
                // hasViewportUnits
                m_breakBefore == other.m_breakBefore && m_breakAfter == other.m_breakAfter && m_breakInside == other.m_breakInside;
            // styleType
            // pseudoBits
            // explicitInheritance
            // unique
    ...
    void setBitDefaults()
    {
        ...
        m_inheritedData.m_insideLink = static_cast<unsigned>(EInsideLink::kNotInsideLink);

        m_nonInheritedData.m_effectiveDisplay = m_nonInheritedData.m_originalDisplay = static_cast<unsigned>(initialDisplay());
        m_nonInheritedData.m_overflowAnchor = static_cast<unsigned>(initialOverflowAnchor());
        ...
        m_nonInheritedData.m_verticalAlign = static_cast<unsigned>(initialVerticalAlign());
...
public:
...
#ifdef TENCENT_FITSCREEN
    void setFitScreenLayout(bool isFitScreen)
    {
        m_styleInheritedData.access()->m_isFitScreenLayoutStyle = isFitScreen;
    }

    bool getFitScreenLayout() { return m_styleInheritedData->m_isFitScreenLayoutStyle; }

    void setFitScreenLayoutMaxWidth(int width)
    {
        m_styleInheritedData.access()->m_defaultMaxWidth = width;
    }

    int getDefaultMaxWidth() const
    {
        return m_styleInheritedData->m_defaultMaxWidth;
    }

    bool isBackgroundImageCleared() const
    {
        return (*m_background).isImageCleared();
    }

    void setBackgroundImageCleared(bool c)
    {
        m_background.access()->setImageCleared(c);
    }

    Color backgroundColorForFitScreen() const
    {
    ...
    {
        m_rareNonInheritedData.access()->m_multiCol = parent.m_rareNonInheritedData->m_multiCol;
    }

    // Flex utility functions.
    bool isColumnFlexDirection() const
    {
        return flexDirection() == FlowColumn || flexDirection() == FlowColumnReverse;
    }
    bool isReverseFlexDirection() const
    {
        return flexDirection() == FlowRowReverse || flexDirection() == FlowColumnReverse;
    }
    ...
    bool hasMask() const
    {
        return m_rareNonInheritedData->m_mask.hasImage() || m_rareNonInheritedData->m_maskBoxImage.hasImage();
    }
    ...
    {
        return (m_rareNonInheritedData->m_grid->m_gridAutoFlow & InternalAutoFlowDirectionRow) == InternalAutoFlowDirectionRow;
    }
    bool isGridAutoFlowDirectionColumn() const
    {
        return (m_rareNonInheritedData->m_grid->m_gridAutoFlow & InternalAutoFlowDirectionColumn) == InternalAutoFlowDirectionColumn;
    }
    bool isGridAutoFlowAlgorithmSparse() const
    {
        return (m_rareNonInheritedData->m_grid->m_gridAutoFlow & InternalAutoFlowAlgorithmSparse) == InternalAutoFlowAlgorithmSparse;
    }
    bool isGridAutoFlowAlgorithmDense() const
    {
        return (m_rareNonInheritedData->m_grid->m_gridAutoFlow & InternalAutoFlowAlgorithmDense) == InternalAutoFlowAlgorithmDense;
    }
    ...
    {
        return visibility() == EVisibility::kVisible && pointerEvents() != EPointerEvents::kNone;
    }

    // Animation utility functions.
    bool shouldCompositeForCurrentAnimations() const
    {
        return hasCurrentOpacityAnimation() || hasCurrentTransformAnimation() || hasCurrentFilterAnimation() || hasCurrentBackdropFilterAnimation();
    }
    bool isRunningAnimationOnCompositor() const
    {
        return isRunningOpacityAnimationOnCompositor() || isRunningTransformAnimationOnCompositor() || isRunningFilterAnimationOnCompositor() || isRunningBackdropFilterAnimationOnCompositor();
    }
    ...
    {
        return hasTransformOperations() || hasOffset() || hasCurrentTransformAnimation() || translate() || rotate() || scale();
    }
    ...
    {
        return m_rareNonInheritedData->m_transform == otherStyle.m_rareNonInheritedData->m_transform;
    }
    bool preserves3D() const
    {
        return usedTransformStyle3D() != TransformStyle3DFlat;
    }
    enum ApplyTransformOrigin { IncludeTransformOrigin,
        ExcludeTransformOrigin };
    enum ApplyMotionPath { IncludeMotionPath,
        ExcludeMotionPath };
    ...
    {
        return hasOpacity() || hasWillChangeOpacityHint() || hasCurrentOpacityAnimation();
    }
    ...
    {
        return !isOverflowVisible() || hasFilterInducingProperty() || hasNonInitialOpacity();
    }
    ...
    {
        return hasTransform() || preserves3D() || hasPerspective() || hasWillChangeTransformHint();
    }
    ...
    {
        return whiteSpace() == EWhiteSpace::kPreWrap || getLineBreak() == LineBreakAfterWhiteSpace;
    }

    bool breakWords() const
    {
        return (wordBreak() == BreakWordBreak || overflowWrap() == BreakOverflowWrap) && whiteSpace() != EWhiteSpace::kPre && whiteSpace() != EWhiteSpace::kNowrap;
    }
    ...
    {
        return hasBorderDecoration() || hasBorderRadius() || hasOutline() || hasAppearance() || boxShadow() || hasFilterInducingProperty() || hasBackdropFilter() || resize() != RESIZE_NONE;
    }
...
private:
    ...
    {
        return display == EDisplay::InlineBlock || display == EDisplay::WebkitInlineBox || display == EDisplay::InlineFlex || display == EDisplay::InlineTable || display == EDisplay::InlineGrid;
    }
    ...
    {
        return display == EDisplay::Table || display == EDisplay::InlineTable || display == EDisplay::TableRowGroup || display == EDisplay::TableHeaderGroup || display == EDisplay::TableFooterGroup || display == EDisplay::TableRow || display == EDisplay::TableColumnGroup || display == EDisplay::TableColumn || display == EDisplay::TableCell || display == EDisplay::TableCaption;
    }
