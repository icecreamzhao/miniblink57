...
enum ColumnFill { ColumnFillBalance,
    ColumnFillAuto };

enum ColumnSpan { ColumnSpanNone = 0,
    ColumnSpanAll };
...
enum OutlineIsAuto { OutlineIsAutoOff = 0,
    OutlineIsAutoOn };
...
enum EBoxDecorationBreak { BoxDecorationBreakSlice,
    BoxDecorationBreakClone };

// Box attributes. Not inherited.

enum class EBoxSizing : unsigned { kContentBox,
    kBorderBox };

// Random visual rendering model attributes. Not inherited.

enum class EOverflowAnchor : unsigned { Visible,
    None,
    Auto };
...
enum EClear { ClearNone = 0,
    ClearLeft = 1,
    ClearRight = 2,
    ClearBoth = 3 };

enum ETableLayout { TableLayoutAuto,
    TableLayoutFixed };

enum TextCombine { TextCombineNone,
    TextCombineAll };
...
enum EFillRepeat { RepeatFill,
    NoRepeatFill,
    RoundFill,
    SpaceFill };

enum EFillLayerType { BackgroundFillLayer,
    MaskFillLayer };

// CSS3 Background Values
enum EFillSizeType { Contain,
    Cover,
    SizeLength,
    SizeNone };

// CSS3 Background Position
enum BackgroundEdgeOrigin { TopEdge,
    RightEdge,
    BottomEdge,
    LeftEdge };

// CSS Mask Source Types
enum EMaskSourceType { MaskAlpha,
    MaskLuminance };

// Deprecated Flexible Box Properties

enum EBoxPack { BoxPackStart,
    BoxPackCenter,
    BoxPackEnd,
    BoxPackJustify };
enum EBoxAlignment { BSTRETCH,
    BSTART,
    BCENTER,
    BEND,
    BBASELINE };
enum EBoxOrient { HORIZONTAL,
    VERTICAL };
enum EBoxLines { SINGLE,
    MULTIPLE };

// CSS3 Flexbox Properties

enum EFlexDirection { FlowRow,
    FlowRowReverse,
    FlowColumn,
    FlowColumnReverse };
enum EFlexWrap { FlexNoWrap,
    FlexWrap,
    FlexWrapReverse };

enum ETextSecurity { TSNONE,
    TSDISC,
    TSCIRCLE,
    TSSQUARE };
...
enum EUserModify { READ_ONLY,
    READ_WRITE,
    READ_WRITE_PLAINTEXT_ONLY };

// CSS3 User Drag Values

enum EUserDrag { DRAG_AUTO,
    DRAG_NONE,
    DRAG_ELEMENT };

// CSS3 User Select Values

enum EUserSelect { SELECT_NONE,
    SELECT_TEXT,
    SELECT_ALL };
...
enum EOverflowWrap { NormalOverflowWrap,
    BreakOverflowWrap };
...
enum EResize { RESIZE_NONE,
    RESIZE_BOTH,
    RESIZE_HORIZONTAL,
    RESIZE_VERTICAL };

enum QuoteType { OPEN_QUOTE,
    CLOSE_QUOTE,
    NO_OPEN_QUOTE,
    NO_CLOSE_QUOTE };

enum EAnimPlayState { AnimPlayStatePlaying,
    AnimPlayStatePaused };
...
{
    return TextDecorationSkip(static_cast<unsigned>(a) | static_cast<unsigned>(b));
}
inline TextDecorationSkip& operator|=(TextDecorationSkip& a,
    TextDecorationSkip b)
{
...
enum ETransformStyle3D { TransformStyle3DFlat,
    TransformStyle3DPreserve3D };

enum OffsetRotationType { OffsetRotationAuto,
    OffsetRotationFixed };
...
enum TextEmphasisFill { TextEmphasisFillFilled,
...
enum TextOverflow { TextOverflowClip = 0,
...
enum RubyPosition { RubyPositionBefore,
    RubyPositionAfter };
...
enum ItemPositionType { NonLegacyPosition,
    LegacyPosition };
...
enum TextIndentLine { TextIndentFirstLine,
    TextIndentEachLine };
enum TextIndentType { TextIndentNormal,
    TextIndentHanging };
...
enum AutoRepeatType { NoAutoRepeat,
    AutoFill,
    AutoFit };
