    ...
    , m_autoRepeatTotalTracks(autoRepeatTracksCount)
{
    ...
    const NamedGridLinesMap& gridLineNames = isRowAxis ? gridContainerStyle.namedGridColumnLines()
                                                       : gridContainerStyle.namedGridRowLines();
    const NamedGridLinesMap& autoRepeatGridLineNames = isRowAxis ? gridContainerStyle.autoRepeatNamedGridColumnLines()
    ...
    m_insertionPoint = isRowAxis ? gridContainerStyle.gridAutoRepeatColumnsInsertionPoint()
                                 : gridContainerStyle.gridAutoRepeatRowsInsertionPoint();

    m_autoRepeatTrackListLength = isRowAxis ? gridContainerStyle.gridAutoRepeatColumns().size()
    ...
    size_t autoRepeatTracksCount)
{
    return std::min<size_t>(
        std::max(gridContainerStyle.gridTemplateColumns().size() + autoRepeatTracksCount,
    ...
    size_t autoRepeatTracksCount)
{
    ...
    if (finalPosition.shouldBeResolvedAgainstOppositePosition()) {
        ...
        // case).
        int startLine = resolveGridPositionFromStyle(gridContainerStyle, initialPosition,
