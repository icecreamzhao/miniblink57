...
void HTMLFrameElementBase::openURL(bool replaceCurrentItem)
{
    ...
    if (protocolIsJavaScript(m_URL)) {
        ...
        if (ContentSecurityPolicy::shouldBypassMainWorld(&document()) || document().contentSecurityPolicy()->allowJavaScriptURLs(this, document().url(), OrdinalNumber::first())) {
            scriptURL = url;
        } else {
            if (contentFrame())
                return;
...
{
    return attribute.name() == longdescAttr || attribute.name() == srcAttr || HTMLFrameOwnerElement::isURLAttribute(attribute);
}
