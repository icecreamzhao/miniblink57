    ...
    const CSSStyleSheetResource* cachedStyleSheet)
{
    ...
    if (!cachedStyleSheet->errorOccurred() && !m_owner->fastGetAttribute(HTMLNames::integrityAttr).isEmpty() && !cachedStyleSheet->integrityMetadata().isEmpty()) {
        ResourceIntegrityDisposition disposition = cachedStyleSheet->integrityDisposition();

        if (disposition == ResourceIntegrityDisposition::NotChecked && !cachedStyleSheet->loadFailedOrCanceled()) {
            bool checkResult;
    ...
    CSSParserContext* parserContext = CSSParserContext::create(m_owner->document(), baseURL, charset);
    ...
    if (StyleSheetContents* restoredSheet = const_cast<CSSStyleSheetResource*>(cachedStyleSheet)
