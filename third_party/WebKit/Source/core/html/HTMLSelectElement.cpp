...
void HTMLSelectElement::saveLastSelection()
{
    ...
    for (auto& element : listItems())
        m_lastOnChangeSelection.push_back(isHTMLOptionElement(*element) && toHTMLOptionElement(element)->selected());
}
    ...
    SelectOptionFlags flags)
{
    ...
    if (element) {
        ...
        if (!m_activeSelectionAnchor || !isMultiple() || flags & DeselectOtherOptions)
...
{
    if (!isMultiple() && usesMenuList() && m_lastOnChangeOption && m_lastOnChangeOption != excludeElement) {
    ...
    KeyboardEvent* keyEvent)
{
    ...
    return ((layoutTheme.popsMenuByArrowKeys() && (key == "ArrowDown" || key == "ArrowUp")) || (layoutTheme.popsMenuByAltDownUpOrF4Key() && (key == "ArrowDown" || key == "ArrowUp") && keyEvent->altKey()) || (layoutTheme.popsMenuByAltDownUpOrF4Key() && (!keyEvent->altKey() && !keyEvent->ctrlKey() && key == "F4")));
}
...
bool HTMLSelectElement::shouldOpenPopupForKeyPressEvent(KeyboardEvent* event)
{
    ...
    return ((layoutTheme.popsMenuBySpaceKey() && event->keyCode() == ' ' && !m_typeAhead.hasActiveSession(event)) || (layoutTheme.popsMenuByReturnKey() && keyCode == '\r'));
}
...
void HTMLSelectElement::menuListDefaultEventHandler(Event* event)
{
    if (event->type() == EventTypeNames::keydown) {
        ...
        int ignoreModifiers = PlatformEvent::ShiftKey | PlatformEvent::CtrlKey | PlatformEvent::AltKey | PlatformMouseEvent::MetaKey;
    ...
    if (event->type() == EventTypeNames::mousedown && event->isMouseEvent() && toMouseEvent(event)->button() == static_cast<short>(WebPointerProperties::Button::Left)) {
        InputDeviceCapabilities* sourceCapabilities = document().domWindow()->getInputDeviceCapabilities()->firesTouchEvents(
...
void HTMLSelectElement::listBoxDefaultEventHandler(Event* event)
{
    if (event->type() == EventTypeNames::gesturetap && event->isGestureEvent()) {
        ...
        if (endOption && handled) {
            ...
            bool selectNewItem = !m_isMultiple || toKeyboardEvent(event)->shiftKey() || !isSpatialNavigationEnabled(document().frame());
            if (selectNewItem)
                m_activeSelectionState = true;
            // If the anchor is unitialized, or if we're going to deselect all
            // other options, then set the anchor index equal to the end index.
            bool deselectOthers = !m_isMultiple || (!toKeyboardEvent(event)->shiftKey() && selectNewItem);
        ...
        if (keyCode == '\r') {
            if (form())
                form()->submitImplicitly(event, false);
            event->setDefaultHandled();
        } else if (m_isMultiple && keyCode == ' ' && isSpatialNavigationEnabled(document().frame())) {
...
void HTMLSelectElement::selectOptionByAccessKey(HTMLOptionElement* option)
{
    ...
    // selected index.
    SelectOptionFlags flags = DispatchInputAndChangeEvent | (isMultiple() ? 0 : DeselectOtherOptions);
    if (option->selected()) {
        if (usesMenuList())
