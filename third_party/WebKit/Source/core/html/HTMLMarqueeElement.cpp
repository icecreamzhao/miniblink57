...
void HTMLMarqueeElement::setLoop(int value, ExceptionState& exceptionState)
{
    if (value <= 0 && value != -1) {
        exceptionState.throwDOMException(
            IndexSizeError, "The provided value (" + String::number(value) + ") is neither positive nor -1.");
        return;
    }
...
void HTMLMarqueeElement::start()
{
    ...
    RequestAnimationFrameCallback* callback = new RequestAnimationFrameCallback(this);
...
{
    if (attr == HTMLNames::bgcolorAttr || attr == HTMLNames::heightAttr || attr == HTMLNames::hspaceAttr || attr == HTMLNames::vspaceAttr || attr == HTMLNames::widthAttr) {
        return true;
    }
...
{
    StyleSheetContents* styleSheetContents = m_mover->document().elementSheet().contents();
...
void HTMLMarqueeElement::continueAnimation()
{
    ...
    if (scrollDelay < kMinimumScrollDelayMS && !fastHasAttribute(HTMLNames::truespeedAttr))
    ...
    KeyframeEffect* keyframeEffect = KeyframeEffect::create(m_mover, effectModel, timing);
...
{
    Metrics metrics;
    CSSStyleDeclaration* marqueeStyle = document().domWindow()->getComputedStyle(this, String());
    ...
    // inline marquees.
    if (marqueeStyle->getPropertyValue("width") == "auto" && marqueeStyle->getPropertyValue("height") == "auto") {
    ...
    CSSStyleDeclaration* moverStyle = document().domWindow()->getComputedStyle(m_mover, String());
...
HTMLMarqueeElement::getAnimationParameters()
{
    ...
    case kAlternate:
        switch (getDirection()) {
        case kRight:
            parameters.transformBegin = createTransform(innerWidth >= 0 ? 0 : innerWidth);
            parameters.transformEnd = createTransform(innerWidth >= 0 ? innerWidth : 0);
            parameters.distance = std::abs(innerWidth);
            break;
        case kUp:
            parameters.transformBegin = createTransform(innerHeight >= 0 ? innerHeight : 0);
            parameters.transformEnd = createTransform(innerHeight >= 0 ? 0 : innerHeight);
            parameters.distance = std::abs(innerHeight);
            break;
        case kDown:
            parameters.transformBegin = createTransform(innerHeight >= 0 ? 0 : innerHeight);
            parameters.transformEnd = createTransform(innerHeight >= 0 ? innerHeight : 0);
            parameters.distance = std::abs(innerHeight);
            break;
        case kLeft:
        default:
            parameters.transformBegin = createTransform(innerWidth >= 0 ? innerWidth : 0);
            parameters.transformEnd = createTransform(innerWidth >= 0 ? 0 : innerWidth);
            parameters.distance = std::abs(innerWidth);
        }
...
AtomicString HTMLMarqueeElement::createTransform(double value) const
{
    ...
    return String::format("translate%c(", axis) + String::numberToStringECMAScript(value) + "px)";
}
