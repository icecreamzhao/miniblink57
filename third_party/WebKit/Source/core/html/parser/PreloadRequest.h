...
public:
    ...
    static std::unique_ptr<PreloadRequest> createIfNeeded(
        ...
        const FetchRequest::ResourceWidth& resourceWidth = FetchRequest::ResourceWidth(),
        const ClientHintsPreferences& clientHintsPreferences = ClientHintsPreferences(),
        RequestType requestType = RequestTypePreload)
    {
        ...
        if (resourceURL.isEmpty() || resourceURL.startsWith("#") || protocolIs(resourceURL, "data")) {
            return nullptr;
        }
