...
bool HTMLElement::ieForbidsInsertHTML() const
{
    ...
    if (hasTagName(areaTag) || hasTagName(baseTag) || hasTagName(basefontTag) || hasTagName(brTag) || hasTagName(colTag) || hasTagName(embedTag) || hasTagName(frameTag) || hasTagName(hrTag) || hasTagName(imageTag) || hasTagName(imgTag) || hasTagName(inputTag) || hasTagName(keygenTag) || hasTagName(linkTag) || (RuntimeEnabledFeatures::contextMenuEnabled() && hasTagName(menuitemTag)) || hasTagName(metaTag) || hasTagName(paramTag) || hasTagName(sourceTag) || hasTagName(trackTag) || hasTagName(wbrTag))
        return true;
    return false;
}
...
{
    return element.hasTagName(colTag) || element.hasTagName(colgroupTag) || element.hasTagName(framesetTag) || element.hasTagName(headTag) || element.hasTagName(htmlTag) || element.hasTagName(tableTag) || element.hasTagName(tbodyTag) || element.hasTagName(tfootTag) || element.hasTagName(theadTag) || element.hasTagName(trTag);
}
    ...
    ExceptionState& exceptionState)
{
    if (equalIgnoringCase(enabled, "true"))
        ...
        exceptionState.throwDOMException(SyntaxError,
            "The value provided ('" + enabled + "') is not one of 'true', 'false', "
    ...
    Node** strongDirectionalityTextNode) const
{
    ...
    while (node) {
        ...
        if (equalIgnoringCase(node->nodeName(), "bdi") || isHTMLScriptElement(*node) || isHTMLStyleElement(*node) || (node->isElementNode() && toElement(node)->isTextControl()) || (node->isElementNode() && toElement(node)->shadowPseudoId() == "-webkit-input-placeholder")) {
        ...
        for (; elementToAdjust;
            ...
            if (elementAffectsDirectionality(elementToAdjust)) {
                elementToAdjust->setNeedsStyleRecalc(
                    LocalStyleChange, StyleChangeReasonForTracing::create(StyleChangeReason::WritingModeChange));
                return;
            }
        }
    }
    ...
    AllowPercentage allowPercentage)
{
    ...
    if (!parseDimensionValue(value, dimension))
        return;
    if (propertyID == CSSPropertyWidth && (dimension.isPercentage() || dimension.isRelative())) {
    ...
    if (dimension.isPercentage() && allowPercentage != AllowPercentageValues)
        return;
    CSSPrimitiveValue::UnitType unit = dimension.isPercentage() ? CSSPrimitiveValue::UnitType::Percentage
                                                                : CSSPrimitiveValue::UnitType::Pixels;
