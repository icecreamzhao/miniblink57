...
void AutoplayUmaHelper::onAutoplayInitiated(AutoplaySource source)
{
    ...
    if (m_element->isHTMLVideoElement() && m_element->muted() && RuntimeEnabledFeatures::autoplayMutedVideosEnabled()) {
        bool dataSaverEnabled = m_element->document().settings() && m_element->document().settings()->getDataSaverEnabled();
...
void AutoplayUmaHelper::maybeStartRecordingMutedVideoPlayMethodBecomeVisible()
{
    ...
    m_mutedVideoPlayMethodVisibilityObserver = new ElementVisibilityObserver(
        ...
        [self](bool isVisible) {
            self->onVisibilityChangedForMutedVideoPlayMethodBecomeVisible(isVisible);
        });
...
void AutoplayUmaHelper::maybeStartRecordingMutedVideoOffscreenDuration()
{
    ...
    m_isVisible = false;
    AutoplayUmaHelper* self = this;
    m_mutedVideoOffscreenDurationVisibilityObserver = new ElementVisibilityObserver(
        m_element,
        //           WTF::bind(&AutoplayUmaHelper::
        //                         onVisibilityChangedForMutedVideoOffscreenDuration,
        //                     wrapWeakPersistent(this))
        [self](bool isVisible) {
            self->onVisibilityChangedForMutedVideoOffscreenDuration(isVisible);
        });
...
bool AutoplayUmaHelper::shouldRecordUserPausedAutoplayingCrossOriginVideo()
    const
{
    return m_element->isInCrossOriginFrame() && m_element->isHTMLVideoElement() && m_source != AutoplaySource::NumberOfSources && !m_recordedCrossOriginAutoplayResults.count(CrossOriginAutoplayResult::UserPaused);
}
