...
{
    m_cachedSelectionDirection = doc.frame() && doc.frame()->editor().behavior().shouldConsiderSelectionAsDirectional()
...
{
    return supportsPlaceholder() && isEmptyValue() && isEmptySuggestedValue() && !isPlaceholderEmpty();
}
    ...
    ExceptionState& exceptionState)
{
    if (start > end) {
        exceptionState.throwDOMException(
            IndexSizeError, "The provided start value (" + String::number(start) + ") is larger than the provided end value (" + String::number(end) + ").");
        return;
    }
    if (openShadowRoot())
        return;
    ...
    TextFieldSelectionDirection direction)
{
    ...
    frame->selection().setSelection(
        newSelection,
        FrameSelection::DoNotAdjustInFlatTree | FrameSelection::CloseTyping | FrameSelection::ClearTypingStyle | FrameSelection::DoNotSetFocus);
    return true;
}
    ...
    ExceptionState& exceptionState)
{
    ...
    if (newValue < 0) {
        exceptionState.throwDOMException(
            IndexSizeError, "The value provided (" + String::number(newValue) + ") is not positive or 0.");
    } else if (min >= 0 && newValue < min) {
        exceptionState.throwDOMException(
            IndexSizeError, ExceptionMessages::indexExceedsMinimumBound("maxLength", newValue, min));
    } else {
    ...
    ExceptionState& exceptionState)
{
    ...
    if (newValue < 0) {
        ...
        exceptionState.throwDOMException(
            IndexSizeError, ExceptionMessages::indexExceedsMaximumBound("minLength", newValue, max));
    } else {
...
{
    DCHECK(position.isNull() || position.isOffsetInAnchor() || position.computeContainerNode() || !position.anchorNode()->ownerShadowHost() || (position.anchorNode()->parentNode() && position.anchorNode()->parentNode()->isShadowRoot()));
...
TextControlElement* enclosingTextControl(const Node* container)
{
    ...
    return ancestor && isTextControlElement(*ancestor) && container->containingShadowRoot()->type() == ShadowRootType::UserAgent
...
{
    const TextControlElement& sourceElement = static_cast<const TextControlElement&>(source);
