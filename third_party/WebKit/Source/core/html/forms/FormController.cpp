    ...
    size_t& index)
{
    ...
    if (!itemCount)
        return nullptr;
    std::unique_ptr<SavedFormState> savedFormState = WTF::wrapUnique(new SavedFormState);
    while (itemCount--) {
...
Vector<String> SavedFormState::getReferencedFilePaths() const
{
    ...
    for (const auto& formControl : m_stateForNewFormElements) {
        ...
        const Deque<FormControlState>& queue = formControl.value;
        for (const FormControlState& formControlState : queue) {
            const Vector<FileChooserFileInfo>& selectedFiles = HTMLInputElement::filesFromFileInputFormControlState(
                formControlState);
...
Vector<String> DocumentState::toStateVector()
{
    ...
    std::unique_ptr<SavedFormStateMap> stateMap = WTF::wrapUnique(new SavedFormStateMap);
