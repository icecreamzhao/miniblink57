...
Float32ImageData* Float32ImageData::create(const IntSize& size)
{
    if (!Float32ImageData::validateConstructorArguments(kParamSize, &size))
        return nullptr;
    DOMFloat32Array* dataArray = Float32ImageData::allocateAndValidateFloat32Array(4 * size.width() * size.height());
    ...
    ExceptionState& exceptionState)
{
    if (!Float32ImageData::validateConstructorArguments(
            ...
            &exceptionState))
        return nullptr;
    DOMFloat32Array* dataArray = Float32ImageData::allocateAndValidateFloat32Array(4 * width * height,
        &exceptionState);
