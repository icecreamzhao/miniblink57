...
{
    pointerEventInit.setBubbles(type != EventTypeNames::pointerenter && type != EventTypeNames::pointerleave);
    pointerEventInit.setCancelable(type != EventTypeNames::pointerenter && type != EventTypeNames::pointerleave && type != EventTypeNames::pointercancel && type != EventTypeNames::gotpointercapture && type != EventTypeNames::lostpointercapture);
...
PointerEvent* PointerEventFactory::create(
    ...
    LocalDOMWindow* view)
{
    DCHECK(mouseEventName == EventTypeNames::mousemove || mouseEventName == EventTypeNames::mousedown || mouseEventName == EventTypeNames::mouseup);
    ...
    if (pointerEventName == EventTypeNames::pointerdown || pointerEventName == EventTypeNames::pointerup) {
        WebPointerProperties::Button button = mouseEvent.pointerProperties().button;
        // TODO(mustaq): Fix when the spec starts supporting hovering erasers.
        if (mouseEvent.pointerProperties().pointerType == WebPointerProperties::PointerType::Eraser && button == WebPointerProperties::Button::Left)
    ...
    if ((pointerEventName == EventTypeNames::pointerdown && (buttons & ~buttonToButtonsBitfield(mouseEvent.pointerProperties().button)) != 0) || (pointerEventName == EventTypeNames::pointerup && buttons != 0))
    ...
    DOMWindow* view)
{
    ...
    const AtomicString& type = pointerEventNameForTouchPointState(touchPoint.state());

    bool pointerReleasedOrCancelled = pointState == PlatformTouchPoint::TouchReleased || pointState == PlatformTouchPoint::TouchCancelled;
    bool pointerPressedOrReleased = pointState == PlatformTouchPoint::TouchPressed || pointState == PlatformTouchPoint::TouchReleased;
...
PointerEvent* PointerEventFactory::createPointerCaptureEvent(
    ...
    const AtomicString& type)
{
    DCHECK(type == EventTypeNames::gotpointercapture || type == EventTypeNames::lostpointercapture);
...
PointerEvent* PointerEventFactory::createPointerBoundaryEvent(
    ...
    EventTarget* relatedTarget)
{
    DCHECK(type == EventTypeNames::pointerout || type == EventTypeNames::pointerleave || type == EventTypeNames::pointerover || type == EventTypeNames::pointerenter);
...
{
    return m_pointerIdMapping.contains(pointerId) && m_pointerIdMapping.get(pointerId).isActiveButtons;
}
