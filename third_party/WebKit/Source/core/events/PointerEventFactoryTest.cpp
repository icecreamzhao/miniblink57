...
PointerEvent* PointerEventFactoryTest::createAndCheckTouchCancel(
    ...
    bool isPrimary)
{
    PointerEvent* pointerEvent = m_pointerEventFactory.createPointerCancelEvent(uniqueId, pointerType);
...
{
    PointerEvent* clonePointerEvent = m_pointerEventFactory.createPointerBoundaryEvent(pointerEvent, type,
        nullptr);
    ...
    size_t coalescedEventCount)
{
    ...
    EXPECT_EQ(uniqueId, pointerEvent->pointerId());
    EXPECT_EQ(isPrimary, pointerEvent->isPrimary());
    const char* expectedPointerType = pointerTypeNameForWebPointPointerType(pointerType);
