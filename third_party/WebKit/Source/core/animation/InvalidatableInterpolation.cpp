    ...
    InterpolationEnvironment& environment) const
{
    ...
    if ((startValue && startValue->isInheritedValue()) || (endValue && endValue->isInheritedValue())) {
    ...
    InterpolationEnvironment& environment)
{
    ...
    if (firstInterpolation.dependsOnUnderlyingValue()) {
        ...
        const TypedInterpolationValue* firstValue = firstInterpolation.ensureValidConversion(environment,
            underlyingValueOwner);
    ...
    for (size_t i = startingIndex; i < interpolations.size(); i++) {
        ...
        const TypedInterpolationValue* currentValue = currentInterpolation.ensureValidConversion(environment,
            underlyingValueOwner);
        if (!currentValue)
            continue;
        shouldApply = true;
