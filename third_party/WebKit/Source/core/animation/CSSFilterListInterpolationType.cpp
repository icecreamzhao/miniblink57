...
namespace {
        ...
        double zoom)
    {
        ...
        for (size_t i = 0; i < length; i++) {
            InterpolationValue filterResult = FilterInterpolationFunctions::maybeConvertFilter(
...
InterpolationValue CSSFilterListInterpolationType::maybeConvertInherit(
    ...
    ConversionCheckers& conversionCheckers) const
{
    const FilterOperations& inheritedFilterOperations = FilterListPropertyFunctions::getFilterList(cssProperty(),
        *state.parentStyle());
    ...
    InterpolationValue&& end) const
{
    ...
    for (size_t i = 0; i < longerLength; i++) {
        if (i < shorterLength)
            ...
            extendedInterpolableList->set(
                i, FilterInterpolationFunctions::createNoneValue(*longerNonInterpolableList.get(i)));
    }
