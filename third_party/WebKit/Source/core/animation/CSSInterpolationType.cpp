...
private:
        ...
        const InterpolationValue& underlying) const final
    {
        ...
        const CSSValue* resolvedValue = CSSVariableResolver::resolveVariableReferences(
...
private:
    ...
    {
        const CSSValue* inheritedValue = environment.state().parentStyle()->getRegisteredVariable(
    ...
    ConversionCheckers& conversionCheckers) const
{
    ...
    if (result && keyframe.composite() != EffectModel::CompositeOperation::CompositeReplace) {
    ...
    ConversionCheckers& conversionCheckers) const
{
    ...
    const PropertyRegistry::Registration* registration = getRegistration(state, name);
    ...
    if (!declaration.value()) {
        ...
        DCHECK(declaration.isInitial(isInheritedProperty) || declaration.isInherit(isInheritedProperty));
        ...
        const CSSValue* inheritedValue = state.parentStyle()->getRegisteredVariable(name, isInheritedProperty);
        if (!inheritedValue) {
    ...
    const InterpolationEnvironment& environment) const
{
    ...
    const CSSValue* underlyingValue = state.style()->getRegisteredVariable(name, registration->inherits());
    if (!underlyingValue) {
    ...
    StyleResolverState& state) const
{
    ...
    const CSSValue* cssValue = createCSSValue(interpolableValue, nonInterpolableValue, state);
    ...
    const PropertyRegistry::Registration* registration = getRegistration(state, propertyName);
    DCHECK(registration);
