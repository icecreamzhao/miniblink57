...
protected:
    ...
    PassRefPtr<LegacyStyleInterpolation> interpolateLists(
        ...
        double progress)
    {
        RefPtr<LegacyStyleInterpolation> i = SampleInterpolation::create(std::move(listA), std::move(listB));
        i->interpolate(0, progress);
        return i;
    }
};
...
TEST_F(AnimationInterpolableValueTest, SimpleList)
{
    ...
    RefPtr<LegacyStyleInterpolation> i = interpolateLists(std::move(listA), std::move(listB), 0.3);
...
TEST_F(AnimationInterpolableValueTest, NestedList)
{
    ...
    RefPtr<LegacyStyleInterpolation> i = interpolateLists(std::move(listA), std::move(listB), 0.5);
    InterpolableList* outList = toInterpolableList(interpolationValue(*i.get()));
