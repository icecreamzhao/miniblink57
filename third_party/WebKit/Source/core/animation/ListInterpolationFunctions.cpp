...
static void repeatToLength(InterpolationValue& value, size_t length)
{
    ...
    for (size_t i = length; i-- > 0;) {
        newInterpolableList->set(
            i, i < currentLength ? std::move(interpolableList.getMutable(i)) : interpolableList.get(i % currentLength)->clone());
...
{
    size_t underlyingLength = toInterpolableList(*underlyingValueOwner.value().interpolableValue)
                                  .length();
