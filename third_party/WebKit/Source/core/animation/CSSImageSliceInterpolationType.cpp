...
InterpolationValue CSSImageSliceInterpolationType::maybeConvertInherit(
    ...
    ConversionCheckers& conversionCheckers) const
{
    const ImageSlice& inheritedImageSlice = ImageSlicePropertyFunctions::getImageSlice(cssProperty(),
        *state.parentStyle());
...
{
    const SliceTypes& underlyingTypes = toCSSImageSliceNonInterpolableValue(
