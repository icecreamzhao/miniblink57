...
void WorkerBackingThread::initialize()
{
    ...
    std::unique_ptr<V8IsolateInterruptor> interruptor = WTF::makeUnique<V8IsolateInterruptor>(m_isolate);
    ...
    if (RuntimeEnabledFeatures::v8IdleTasksEnabled())
        V8PerIsolateData::enableIdleTasks(
            m_isolate, WTF::wrapUnique(new V8IdleTaskRunner(backingThread().platformThread().scheduler())));
    if (m_isOwningThread)
