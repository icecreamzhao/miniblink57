...
public:
            ...
            nullptr /* workerClients */)
    {
        ...
        m_mockWorkerLoaderProxyProvider = WTF::makeUnique<MockWorkerLoaderProxyProvider>();
    ...
    void startWithSourceCode(const String& source)
    {
        ...
        std::unique_ptr<Vector<CSPHeaderAndType>> headers = WTF::makeUnique<Vector<CSPHeaderAndType>>();
...
TEST_F(DedicatedWorkerTest, PendingActivity_SetInterval)
{
    ...
    const String sourceCode = "var id = setInterval(function() {}, 50);"
...
TEST_F(DedicatedWorkerTest, PendingActivity_SetTimeoutOnMessageEvent)
{
    ...
    const String sourceCode = "addEventListener('message', function(event) {"
...
TEST_F(DedicatedWorkerTest, PendingActivity_SetIntervalOnMessageEvent)
{
    ...
    // message is received. The timer needs a non-zero delay or else worker
    // activities would not run.
    const String sourceCode = "var count = 0;"
                              "var id;"
