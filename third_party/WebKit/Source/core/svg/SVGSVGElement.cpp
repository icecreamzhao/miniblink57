...
{
    if ((name == SVGNames::widthAttr || name == SVGNames::heightAttr) && !isOutermostSVGSVGElement())
        return false;
...
void SVGSVGElement::svgAttributeChanged(const QualifiedName& attrName)
{
    ...
    if (widthOrHeightChanged || attrName == SVGNames::xAttr || attrName == SVGNames::yAttr) {
    ...
    if (updateRelativeLengthsOrViewBox || SVGZoomAndPan::isKnownAttribute(attrName)) {
...
static bool intersectsAllowingEmpty(const FloatRect& r1, const FloatRect& r2)
{
    ...
    return r1.x() < r2.maxX() && r2.x() < r1.maxX() && r1.y() < r2.maxY() && r2.y() < r1.maxY();
}
...
{
    return layoutObject->isSVGShape() || layoutObject->isSVGText() || layoutObject->isSVGImage() || isSVGUseElement(*layoutObject->node());
}
    ...
    GeometryMatchingMode mode) const
{
    ...
    if (!layoutObject || layoutObject->style()->pointerEvents() == EPointerEvents::kNone)
        return false;
    ...
    FloatRect mappedRepaintRect = ctm.mapRect(layoutObject->visualRectInLocalSVGCoordinates());
...
{
    return layoutObject() && layoutObject()->isSVGRoot() && toLayoutSVGRoot(layoutObject())->isEmbeddedThroughSVGImage();
}
