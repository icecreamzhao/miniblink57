...
public:
    ...
    void contentChanged()
    {
        DCHECK(lifecycle().state() <= DocumentLifecycle::CompositingClean || lifecycle().state() >= DocumentLifecycle::PaintClean);
...
void SVGElementProxy::addClient(SVGResourceClient* client)
{
    ...
    auto& scopeObserver = m_observers.add(clientScope, nullptr).storedValue->value;
    if (!scopeObserver)
...
SVGElement* SVGElementProxy::findElement(TreeScope& treeScope)
{
    ...
    if (Element* targetElement = lookupScope->getElementById(m_id)) {
        SVGElementProxySet* proxySet = targetElement->isSVGElement()
