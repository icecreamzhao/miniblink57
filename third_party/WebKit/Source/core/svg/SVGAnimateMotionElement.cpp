...
namespace {
    ...
    bool targetCanHaveMotionTransform(const SVGElement& target)
    {
        ...
        // cause problems.
        return isSVGGElement(target) || isSVGDefsElement(target) || isSVGUseElement(target) || isSVGImageElement(target) || isSVGSwitchElement(target) || isSVGPathElement(target) || isSVGRectElement(target) || isSVGCircleElement(target) || isSVGEllipseElement(target) || isSVGLineElement(target) || isSVGPolylineElement(target) || isSVGPolygonElement(target) || isSVGTextElement(target) || isSVGClipPathElement(target) || isSVGMaskElement(target) || isSVGAElement(target) || isSVGForeignObjectElement(target);
    }
}
...
{
    return SVGAnimationElement::hasValidTarget() && targetCanHaveMotionTransform(*targetElement());
}
