...
void SVGUseElement::updateTargetReference()
{
    ...
    if (m_elementIdentifier.isEmpty() || (m_resource && equalIgnoringFragmentIdentifier(resolvedUrl, m_resource->url())))
        return;
...
void SVGUseElement::buildShadowAndInstanceTree(SVGElement& target)
{
    ...
    m_targetElementInstance = toSVGElementOrDie(shadowTreeRootElement->firstChild());
...
{
    return isSVGPathElement(element) || isSVGRectElement(element) || isSVGCircleElement(element) || isSVGEllipseElement(element) || isSVGPolygonElement(element) || isSVGPolylineElement(element) || isSVGTextElement(element);
}
    ...
    SVGElement& target)
{
    ...
    // chain.
    SVGUseElement* useElement = isSVGUseElement(target) ? toSVGUseElement(&target)
                                                        : Traversal<SVGUseElement>::firstWithin(target);
    for (; useElement;
         useElement = Traversal<SVGUseElement>::nextSkippingChildren(*useElement, &target))
...
{
    if (m_x->currentValue()->isRelative() || m_y->currentValue()->isRelative() || m_width->currentValue()->isRelative() || m_height->currentValue()->isRelative())
        return true;
...
{
    return m_resource && m_resource->isLoaded() && !m_resource->errorOccurred() && m_resource->document();
}
