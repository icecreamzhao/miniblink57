...
namespace {
    ...
    String computeCSSPropertyValue(SVGElement* element, CSSPropertyID id)
    {
        ...
        String value = CSSComputedStyleDeclaration::create(element)->getPropertyValue(id);
        ...
        const String& value)
    {
        ...
        if (value.isEmpty() || value != inherit || !SVGElement::isAnimatableCSSProperty(attributeName))
...
void SVGAnimateElement::resolveTargetProperty()
{
    ...
    DCHECK(m_type != AnimatedPoint && m_type != AnimatedStringList && m_type != AnimatedTransform && m_type != AnimatedTransformList);
}
    ...
    SVGSMILElement* resultElement)
{
    ...
    SVGPropertyBase* toAtEndOfDurationValue = m_toAtEndOfDurationProperty ? m_toAtEndOfDurationProperty : m_toProperty;
    SVGPropertyBase* fromValue = getAnimationMode() == ToAnimation ? animatedValue : m_fromProperty.get();
    ...
    const String& toAtEndOfDurationString)
{
    if (toAtEndOfDurationString.isEmpty())
        return false;
    m_toAtEndOfDurationProperty = createPropertyForAnimation(toAtEndOfDurationString);
    return true;
}
