...
void reportWMLError(Document* doc, WMLErrorCode error)
{
    ...
    if (parser && error != WMLErrorDeckNotAccessible) {
        ...
        if (error == WMLErrorInvalidVariableName || error == WMLErrorInvalidVariableReference) {
            parser->handleError(XMLErrors::ErrorTypeWarning, errorMessage.latin1().data(), parser->textPosition());
        } else {
        ...
        // 			return;
        //
        // 		LocalDOMWindow* domWindow = frame->domWindow();
        // 		if (!domWindow)
        // 			return;
        //
        ...
        // 			return;
        //
        // 		console->addMessage(WMLMessageSource, LogMessageType, ErrorMessageLevel, errorMessage, String(), 0);
        // 		console->reportMessageToClient(ErrorMessageLevel, errorMessage, 0);
    }
}
