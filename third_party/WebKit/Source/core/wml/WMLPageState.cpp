...
void WMLPageState::dump()
{
    ...
    for (; it != end; ++it)
        fprintf(stderr, "\t-> name: '%s'\tvalue: '%s'\n", (*it).key.latin1().data(), (*it).value.latin1().data());
}
#endif
...
void WMLPageState::collectSelectElementVariables()
{
    WMLDocument* wmlDocument = 0;
    if (page() && page()->mainFrame() && page()->mainFrame()->domWindow() && page()->mainFrame()->domWindow()->document()) {
        if (!page()->mainFrame()->domWindow()->document()->isWMLDocument())
            return;
        wmlDocument = (WMLDocument*)(page()->mainFrame()->domWindow()->document());
    } else
        return;

    for (Node* node = wmlDocument; node; node = NodeTraversal::next(*node, wmlDocument)) {
        if (!node->isElementNode() || !isWMLSelectElement((Element*)node))
            continue;
        WMLSelectElement* selectElement = toWMLSelectElement((Element*)node);
        selectElement->selectInitialOptions();
    }
}

/*@author: totrit.
 * Because we use HTMLInputElement rather than WMLInputElement, we have to collect page-state's variables before post.
 */
void WMLPageState::collectInputElementVariables(void)
{
    if (m_inuptCollected)
        return;
    else
        m_inuptCollected = true;

    //totrit: If the document of the main-frame is not a wml-document, cancel the collection.
    //	We temporarily don't support dealing with wml-document that is in a sub-frame, and the main-frame is a html document.
    WMLDocument* wmlDocument = 0;
    if (page() && page()->mainFrame() && page()->mainFrame()->domWindow() && page()->mainFrame()->domWindow()->document()) {
        if (!page()->mainFrame()->domWindow()->document()->isWMLDocument())
            return;
    ...
    HeapVector<Member<WMLInputElement>>& inputElements = wmlDocument->m_inputElements;
    const size_t size = inputElements.size();
    for (size_t i = 0; i < size; i++) {
        WMLInputElement* input = inputElements[i];
        String name = input->name();
        const String& value = input->value();
