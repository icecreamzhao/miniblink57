...
#define DEFINE_RESOURCE_HISTOGRAM(prefix)                        \
    ...
    void addRedirectsToTimingInfo(Resource* resource, ResourceTimingInfo* info)
    {
        ...
        for (size_t i = 0; i < responses.size(); ++i) {
            ...
            bool crossOrigin = !SecurityOrigin::areSameSchemeHostPort(responses[i].url(), newURL);
    ...
    const FetchRequest& request)
{
    ...
    if (resource && resource->loader() && resource->isLoadEventBlockingResourceType() && m_nonBlockingLoaders.contains(resource->loader()) && resource->isLinkPreload() && !request.forPreload()) {
    ...
    const ResourceFactory& factory)
{
    ...
    if (factory.type() != Resource::MainResource && request.options().dataBufferingPolicy != DoNotBufferData) {
        memoryCache()->add(resource);
    }
    return resource;
}
    ...
    Resource* resource)
{
    ...
    if (isMainResource) {
        ...
        m_navigationTimingInfo = ResourceTimingInfo::create(fetchInitiator, startTime, isMainResource);
    }
    ...
    bool isStaticData) const
{
    ...
    // example).
    if (cachePolicy == CachePolicyRevalidate || existingResource->mustRevalidateDueToCacheHeaders() || request.cacheControlContainsNoCache()) {
    ...
    for (const auto& resource : *m_preloads) {
        if (resource && resource->isLinkPreload() && resource->getPreloadResult() == Resource::PreloadNotReferenced) {
            context().addConsoleMessage(
                "The resource " + resource->url().getString() + " was preloaded using link preload but not used within a few "
...
DEFINE_TRACE(ResourceFetcher)
{
    ...
    //visitor->trace(m_archive); DebugBreak();
