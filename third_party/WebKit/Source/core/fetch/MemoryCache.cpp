...
bool MemoryCache::contains(const Resource* resource) const
{
    if (!resource || resource->url().isEmpty())
        return false;
    const ResourceMap* resources = m_resourceMaps.get(resource->cacheIdentifier());
    if (!resources)
        return false;
    KURL url = removeFragmentIdentifierIfNeeded(resource->url());
    ...
    const String& cacheIdentifier) const
{
    ...
    if (!resources)
        return nullptr;
    MemoryCacheEntry* entry = resources->get(removeFragmentIdentifierIfNeeded(resourceURL));
    if (!entry)
        return nullptr;
    ...
    for (const auto& resourceMapIter : m_resourceMaps) {
        for (const auto& resourceIter : *resourceMapIter.value) {
            ...
            if (resource->isLoaded() && resource->decodedSize()) {
                ...
                double elapsedTime = m_pruneFrameTimeStamp - resourceIter.value->m_lastDecodedAccessTime;
