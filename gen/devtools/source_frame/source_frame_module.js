...
this._diff.updateDiffMarkersImmediately();super.onTextEditorContentSet();for(var message of this._allMessages())
this._addMessageToSource(message);this._decorateAllTypes();}
_allMessages(){return this._persistenceBinding?this._persistenceBinding.network.messages().concat(this._persistenceBinding.fileSystem.messages()):this._uiSourceCode.messages();}
onTextChanged(oldRange,newRange){if(this._diff)
this._diff.updateDiffMarkersWhenPossible();super.onTextChanged(oldRange,newRange);this._errorPopoverHelper.hidePopover();if(this._isSettingContent)
...
this.doResize();}
dispose(){this.textEditor.dispose();Common.moduleSetting('textEditorAutocompletion').removeChangeListener(this._updateAutocomplete,this);this.detach();}
...
Workspace.UISourceCode.Message._messageLevelPriority[b.level()];};;SourceFrame.SourceCodeDiff=class{constructor(diffBaseline,textEditor){this._textEditor=textEditor;this._decorations=[];this._textEditor.installGutter(SourceFrame.SourceCodeDiff.DiffGutterType,true);this._diffBaseline=diffBaseline;this._animatedLines=[];}
updateDiffMarkersWhenPossible(){if(this._updateTimeout)
clearTimeout(this._updateTimeout);this._updateTimeout=setTimeout(this.updateDiffMarkersImmediately.bind(this),SourceFrame.SourceCodeDiff.UpdateTimeout);}
...
return;var diff=this._computeDiff(oldContent,newContent);var changedLines=[];for(var i=0;i<diff.length;++i){var diffEntry=diff[i];if(diffEntry.type===SourceFrame.SourceCodeDiff.GutterDecorationType.Delete)
