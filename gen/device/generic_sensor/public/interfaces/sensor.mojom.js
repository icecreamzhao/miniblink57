...
define("device/generic_sensor/public/interfaces/sensor.mojom", [
  ...
  SensorType.LAST = SensorType.PRESSURE;
  ...
  SensorConfiguration.kMaxAllowedFrequency = 60.0;
  ...
  function Sensor_RemoveConfiguration_ResponseParams(values) {
    this.initDefaults_();
    this.initFields_(values);
  }


  Sensor_RemoveConfiguration_ResponseParams.prototype.initDefaults_ = function() {
    this.success = false;
  };
  Sensor_RemoveConfiguration_ResponseParams.prototype.initFields_ = function(fields) {
    for(var field in fields) {
        if (this.hasOwnProperty(field))
          this[field] = fields[field];
    }
  ...
  Sensor_RemoveConfiguration_ResponseParams.validate = function(messageValidator, offset) {
    var err;
    err = messageValidator.validateStructHeader(offset, codec.kStructHeaderSize);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  Sensor_RemoveConfiguration_ResponseParams.encodedSize = codec.kStructHeaderSize + 8;

  Sensor_RemoveConfiguration_ResponseParams.decode = function(decoder) {
    var packed;
    var val = new Sensor_RemoveConfiguration_ResponseParams();
    var numberOfBytes = decoder.readUint32();
    var version = decoder.readUint32();
    packed = decoder.readUint8();
    val.success = (packed >> 0) & 1 ? true : false;
  ...
  Sensor_RemoveConfiguration_ResponseParams.encode = function(encoder, val) {
    var packed;
    encoder.writeUint32(Sensor_RemoveConfiguration_ResponseParams.encodedSize);
    encoder.writeUint32(0);
    packed = 0;
    packed |= (val.success & 1) << 0
    encoder.writeUint8(packed);
  ...
  SensorProxy.prototype.removeConfiguration = function(configuration) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      this.receiver_.acceptAndExpectResponse(message).then(function(message) {
        var reader = new codec.MessageReader(message);
        var responseParams =
            reader.decodeStruct(Sensor_RemoveConfiguration_ResponseParams);
        resolve(responseParams);
      }).catch(function(result) {
        reject(Error("Connection error: " + result));
      });
  ...
  SensorStub.prototype.acceptWithResponder =
    ...
    case kSensor_GetDefaultConfiguration_Name:
      ...
      return this.getDefaultConfiguration().then(function(response) {
        var responseParams =
    ...
    case kSensor_AddConfiguration_Name:
      ...
      return this.addConfiguration(params.configuration).then(function(response) {
        var responseParams =
    ...
    case kSensor_RemoveConfiguration_Name:
      ...
      return this.removeConfiguration(params.configuration).then(function(response) {
        var responseParams =
            new Sensor_RemoveConfiguration_ResponseParams();
        responseParams.success = response.success;
        var builder = new codec.MessageWithRequestIDBuilder(
            kSensor_RemoveConfiguration_Name,
            codec.align(Sensor_RemoveConfiguration_ResponseParams.encodedSize),
            codec.kMessageIsResponse, reader.requestID);
        builder.encodeStruct(Sensor_RemoveConfiguration_ResponseParams,
                             responseParams);
        var message = builder.finish();
        responder.accept(message);
      });
  ...
  function validateSensorResponse(messageValidator) {
   ...
   switch (message.getName()) {
      ...
      case kSensor_RemoveConfiguration_Name:
        if (message.isResponse())
          paramsClass = Sensor_RemoveConfiguration_ResponseParams;
        break;
    }
    if (paramsClass === null)
      return validator.validationError.NONE;
