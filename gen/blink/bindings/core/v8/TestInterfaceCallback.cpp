...
#include "TestInterfaceCallback.h"
...
// static
TestInterfaceCallback* TestInterfaceCallback::create(ScriptState* scriptState, v8::Local<v8::Value> callback){
  if (isUndefinedOrNull(callback))
    return nullptr;
  return new TestInterfaceCallback(scriptState, v8::Local<v8::Function>::Cast(callback));
}

TestInterfaceCallback::TestInterfaceCallback(ScriptState* scriptState, v8::Local<v8::Function> callback)
    : m_scriptState(scriptState),
    m_callback(scriptState->isolate(), this, callback) {
  DCHECK(!m_callback.isEmpty());
}

DEFINE_TRACE(TestInterfaceCallback) {}

DEFINE_TRACE_WRAPPERS(TestInterfaceCallback) {
  visitor->traceWrappers(m_callback.cast<v8::Value>());
}

bool TestInterfaceCallback::call(ScriptWrappable* scriptWrappable, HTMLDivElement* divElement) {
