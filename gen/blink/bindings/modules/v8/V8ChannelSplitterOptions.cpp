...
// clang-format on
#include "V8ChannelSplitterOptions.h"
...
void V8ChannelSplitterOptions::toImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, ChannelSplitterOptions& impl, ExceptionState& exceptionState)
{
    ...
    if (!v8Object->Get(isolate->GetCurrentContext(), v8String(isolate, "numberOfOutputs")).ToLocal(&numberOfOutputsValue)) {
        exceptionState.rethrowV8Exception(block.Exception());
        return;
    }
    if (numberOfOutputsValue.IsEmpty() || numberOfOutputsValue->IsUndefined()) {
        ...
        unsigned numberOfOutputs = toUInt32(isolate, numberOfOutputsValue, NormalConversion, exceptionState);
        if (exceptionState.hadException())
            return;
        impl.setNumberOfOutputs(numberOfOutputs);
    }
}
...
bool toV8ChannelSplitterOptions(const ChannelSplitterOptions& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate)
{
    ...
    if (impl.hasNumberOfOutputs()) {
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "numberOfOutputs"), v8::Integer::NewFromUnsigned(isolate, impl.numberOfOutputs()))))
            return false;
    } else {
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "numberOfOutputs"), v8::Integer::NewFromUnsigned(isolate, 6u))))
            return false;
    }
