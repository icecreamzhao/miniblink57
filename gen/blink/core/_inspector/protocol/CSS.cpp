    ...
    namespace CSS {
        ...
        std::unique_ptr<LayoutTreeNode> LayoutTreeNode::fromValue(protocol::Value* value, ErrorSupport* errors)
        {
            ...
            if (inlineTextNodesValue) {
                ...
                result->m_inlineTextNodes = ValueConversions<protocol::Array<protocol::CSS::InlineTextBox>>::fromValue(inlineTextNodesValue, errors);
            }
        ...
        std::unique_ptr<protocol::DictionaryValue> LayoutTreeNode::toValue() const
        {
            ...
            if (m_inlineTextNodes.isJust())
                result->setValue("inlineTextNodes", ValueConversions<protocol::Array<protocol::CSS::InlineTextBox>>::toValue(m_inlineTextNodes.fromJust()));
        ...
        std::unique_ptr<ComputedStyle> ComputedStyle::fromValue(protocol::Value* value, ErrorSupport* errors)
        {
            ...
            result->m_properties = ValueConversions<protocol::Array<protocol::CSS::CSSComputedStyleProperty>>::fromValue(propertiesValue, errors);
            errors->pop();
            if (errors->hasErrors())
                return nullptr;
            return result;
        ...
        {
            std::unique_ptr<protocol::DictionaryValue> result = DictionaryValue::create();
            result->setValue("properties", ValueConversions<protocol::Array<protocol::CSS::CSSComputedStyleProperty>>::toValue(m_properties.get()));
            return result;
        }
        ...
        public:
                ...
                , m_fallThroughForNotFound(fallThroughForNotFound)
            {
                ...
                m_dispatchMap["CSS.getLayoutTreeAndStyles"] = &DispatcherImpl::getLayoutTreeAndStyles;
        ...
        protected:
            ...
            DispatchResponse::Status getLayoutTreeAndStyles(int callId, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport*);
        ...
        public:
            EnableCallbackImpl(std::unique_ptr<DispatcherBase::WeakPtr> backendImpl, int callId, int callbackId)
        ...
        DispatchResponse::Status DispatcherImpl::enable(int callId, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport* errors)
        {
            ...
            std::unique_ptr<EnableCallbackImpl> callback(new EnableCallbackImpl(weakPtr(), callId, nextCallbackId()));
        ...
        DispatchResponse::Status DispatcherImpl::getBackgroundColors(int callId, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport* errors)
        {
            ...
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
            DispatchResponse response = m_backend->getBackgroundColors(in_nodeId, &out_backgroundColors);
            if (response.status() == DispatchResponse::kFallThrough)
                return response.status();
            std::unique_ptr<protocol::DictionaryValue> result = DictionaryValue::create();
            if (response.status() == DispatchResponse::kSuccess) {
        ...
        DispatchResponse::Status DispatcherImpl::getLayoutTreeAndStyles(int callId, std::unique_ptr<DictionaryValue> requestMessageObject, ErrorSupport* errors)
        {
            // Prepare input parameters.
            protocol::DictionaryValue* object = DictionaryValue::cast(requestMessageObject->get("params"));
            errors->push();
            ...
            // Declare output parameters.
            std::unique_ptr<protocol::Array<protocol::CSS::LayoutTreeNode>> out_layoutTreeNodes;
            std::unique_ptr<protocol::Array<protocol::CSS::ComputedStyle>> out_computedStyles;

            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
            DispatchResponse response = m_backend->getLayoutTreeAndStyles(std::move(in_computedStyleWhitelist), &out_layoutTreeNodes, &out_computedStyles);
            if (response.status() == DispatchResponse::kFallThrough)
                return response.status();
            std::unique_ptr<protocol::DictionaryValue> result = DictionaryValue::create();
            if (response.status() == DispatchResponse::kSuccess) {
                result->setValue("layoutTreeNodes", ValueConversions<protocol::Array<protocol::CSS::LayoutTreeNode>>::toValue(out_layoutTreeNodes.get()));
                result->setValue("computedStyles", ValueConversions<protocol::Array<protocol::CSS::ComputedStyle>>::toValue(out_computedStyles.get()));
            }
            if (weak->get())
                weak->get()->sendResponse(callId, response, std::move(result));
            return response.status();
