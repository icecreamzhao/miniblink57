...
bool WriteFakeIndexFileV5(const base::FilePath& cache_path)
{
    ...
    return sizeof(data) == base::WriteFile(file_name, reinterpret_cast<const char*>(&data), sizeof(data));
}
...
TEST(SimpleVersionUpgradeTest, UpgradeV5V6IndexMustDisappear)
{
    ...
    for (uint64_t entry_hash = 0; entry_hash < kEntries; ++entry_hash) {
        for (int index = 0; index < 3; ++index) {
            std::string file_name = base::StringPrintf("%016" PRIx64 "_%1d", entry_hash, index);
            std::string entry_contents = file_contents + base::StringPrintf(" %" PRIx64, static_cast<uint64_t>(entry_hash));
    ...
    for (uint64_t entry_hash = 0; entry_hash < kEntries; ++entry_hash) {
        for (int index = 0; index < 3; ++index) {
            std::string file_name = base::StringPrintf("%016" PRIx64 "_%1d", entry_hash, index);
            std::string expected_contents = file_contents + base::StringPrintf(" %" PRIx64, static_cast<uint64_t>(entry_hash));
