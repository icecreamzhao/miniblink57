...
namespace {
    ...
    nickname_collision(SECItem* old_nick, PRBool* cancel, void* wincx)
    {
        ...
        if (old_nick && old_nick->data && old_nick->len && PORT_Strlen(nick) == old_nick->len && !PORT_Strncmp((char*)old_nick->data, nick, old_nick->len)) {
            PORT_Free(nick);
    ...
    // cleanup necessary.
    static base::LazyInstance<PKCS12InitSingleton>::Leaky g_pkcs12_init_singleton = LAZY_INSTANCE_INITIALIZER;
