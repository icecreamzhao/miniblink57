    ...
    net::NSSCertDatabase::ImportCertFailureList* not_imported)
{
    ...
    for (size_t i = 0; i < certificates.size(); i++) {
        ...
        if (CERT_VerifyCert(CERT_GetDefaultCertDB(), cert->os_cert_handle(),
                PR_TRUE, certUsageVerifyCA, now, NULL, NULL)
            != SECSuccess) {
...
int ImportUserCert(const net::CertificateList& certificates)
{
    ...
    // Mozilla uses CERT_ImportCerts, which doesn't take a slot arg.  We use
    // PK11_ImportCert instead.
    SECStatus srv = PK11_ImportCert(slot.get(), cert->os_cert_handle(), key,
        net::x509_util::GetUniqueNicknameForSlot(
...
bool SetCertTrust(const net::X509Certificate* cert,
    ...
    net::NSSCertDatabase::TrustBits trustBits)
{
    const unsigned kSSLTrustBits = net::NSSCertDatabase::TRUSTED_SSL | net::NSSCertDatabase::DISTRUSTED_SSL;
    const unsigned kEmailTrustBits = net::NSSCertDatabase::TRUSTED_EMAIL | net::NSSCertDatabase::DISTRUSTED_EMAIL;
    const unsigned kObjSignTrustBits = net::NSSCertDatabase::TRUSTED_OBJ_SIGN | net::NSSCertDatabase::DISTRUSTED_OBJ_SIGN;
    if ((trustBits & kSSLTrustBits) == kSSLTrustBits || (trustBits & kEmailTrustBits) == kEmailTrustBits || (trustBits & kObjSignTrustBits) == kObjSignTrustBits) {
