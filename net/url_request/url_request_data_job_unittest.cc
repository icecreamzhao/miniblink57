...
TEST(BuildResponseTest, InvalidMimeType)
{
    ...
    EXPECT_EQ(OK, URLRequestDataJob::BuildResponse(GURL("data:f(o/b)r,test"), &mime_type, &charset, &data, headers.get()));
...
TEST(BuildResponseTest, InvalidCharset)
{
    ...
    EXPECT_EQ(ERR_INVALID_URL, URLRequestDataJob::BuildResponse(GURL("data:text/html;charset=(),test"), &mime_type, &charset, &data, headers.get()));
}
