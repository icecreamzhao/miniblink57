...
TEST_F(CookieMonsterTest, TestDomainTree)
{
    ...
    for (std::vector<std::string>::const_iterator it = domain_list.begin();
        ...
        GURL gurl("https://" + *it + "/");
        const std::string cookie = base::StringPrintf(domain_cookie_format_tree, it->c_str());
...
TEST_F(CookieMonsterTest, TestDomainLine)
{
    ...
    for (int i = 0; i < 8; i++) {
        for (std::vector<std::string>::const_iterator it = domain_list.begin();
             it != domain_list.end(); it++) {
            GURL gurl("https://" + *it + "/");
            const std::string cookie = base::StringPrintf(domain_cookie_format_line, i, it->c_str());
            setCookieCallback.SetCookie(cm.get(), gurl, cookie);
        }
    }
