...
public:
    ...
    void Update(T new_sample, QuicTime new_time)
    {
        ...
        if (estimates_[0].sample == zero_value_ || Compare()(new_sample, estimates_[0].sample) || new_time.Subtract(estimates_[2].time) > window_length_) {
        ...
        if (estimates_[1].sample == estimates_[0].sample && new_time.Subtract(estimates_[1].time) > window_length_ >> 2) {
        ...
        if (estimates_[2].sample == estimates_[1].sample && new_time.Subtract(estimates_[2].time) > window_length_ >> 1) {
    ...
    {
        estimates_[0] = estimates_[1] = estimates_[2] = Sample(new_sample, new_time);
    }
