...
TcpCubicSenderBytes::TcpCubicSenderBytes(
    ...
    , initial_tcp_congestion_window_(initial_tcp_congestion_window * kDefaultTCPMSS)
    , initial_max_tcp_congestion_window_(max_congestion_window * kDefaultTCPMSS)
    ...
    QuicTime::Delta rtt)
{
    ...
    if (FLAGS_quic_no_lower_bw_resumption_limit) {
        ...
        congestion_window_ = max(min(new_congestion_window,
                                     kMaxResumptionCongestionWindow * kDefaultTCPMSS),
    ...
    QuicByteCount bytes_in_flight)
{
    ...
    if (packet_number <= largest_sent_at_last_cutback_) {
        if (last_cutback_exited_slowstart_) {
            ...
            if (slow_start_large_reduction_) {
                ...
                congestion_window_ = max(congestion_window_ - lost_bytes, min_slow_start_exit_window_);
                slowstart_threshold_ = congestion_window_;
            }
        }
    ...
    if (slow_start_large_reduction_ && InSlowStart()) {
        ...
        if (FLAGS_quic_sslr_limit_reduction && congestion_window_ >= 2 * initial_tcp_congestion_window_) {
    ...
    QuicByteCount bytes_in_flight)
{
    ...
    if (reno_) {
        ...
        if (num_acked_packets_ * num_connections_ >= congestion_window_ / kDefaultTCPMSS) {
            congestion_window_ += kDefaultTCPMSS;
            num_acked_packets_ = 0;
        }
        ...
        congestion_window_ = min(max_congestion_window_,
