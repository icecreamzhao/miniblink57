...
namespace {
    ...
    const uint64_t kCubeFactor = (UINT64_C(1) << kCubeScale) / kCubeCongestionWindowScale;
    ...
    QuicTime::Delta delay_min)
{
    ...
    if (!epoch_.IsInitialized()) {
        ...
        if (FLAGS_shift_quic_cubic_epoch_when_app_limited && app_limited_start_time_ != QuicTime::Zero()) {
    ...
    QuicPacketCount delta_congestion_window = (kCubeCongestionWindowScale * offset * offset * offset) >> kCubeScale;

    QuicPacketCount target_congestion_window = origin_point_congestion_window_ - delta_congestion_window;
