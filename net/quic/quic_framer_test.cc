...
namespace test {
    ...
    {
        return kConnectionIdOffset + connection_id_length + (include_version ? kQuicVersionSize : 0);
    }
    ...
    {
        return kConnectionIdOffset + connection_id_length + (include_version ? kQuicVersionSize : 0) + (include_path_id ? kQuicPathIdSize : 0);
    }
    ...
    const size_t kPublicResetPacketMessageTagOffset = kConnectionIdOffset + PACKET_8BYTE_CONNECTION_ID;
    ...
    public:
            ...
            QuicPacket* packet)
        {
            ...
            if (packet->AssociatedData(framer_.version()) != encrypter_->associated_data_) {
            ...
            bool include_version)
        {
            ...
            for (size_t i = kQuicFrameTypeSize; i < GetMinStreamFrameSize(); ++i) {
                ...
                } else if (i < kQuicFrameTypeSize + stream_id_size + kQuicMaxStreamOffsetSize) {
                ...
                CheckProcessingFails(
                    packet,
                    i + GetPacketHeaderSize(framer_.version(), PACKET_8BYTE_CONNECTION_ID, include_version, !kIncludePathId, !kIncludeDiversificationNonce, PACKET_6BYTE_PACKET_NUMBER),
  ...
  unsigned char packet_34[] = {
    ...
    0xBC, 0x9A, 0x78, 0x56, 0x34, 0x12,
        ...
        for (size_t i = 0;
            ...
            if (i < kConnectionIdOffset) {
                expected_error = "Unable to read public flags.";
            } else if (i < GetPathIdOffset(PACKET_8BYTE_CONNECTION_ID, !kIncludeVersion)) {
                expected_error = "Unable to read ConnectionId.";
            } else if (i < GetPacketNumberOffset(!kIncludeVersion, kIncludePathId)) {
  ...
  unsigned char packet_34[] = {
    ...
    'r',  'l',  'd',  '!',
        ...
        CheckProcessingFails(
            packet, GetPacketHeaderSize(framer_.version(), PACKET_8BYTE_CONNECTION_ID, !kIncludeVersion, !kIncludePathId, !kIncludeDiversificationNonce, PACKET_6BYTE_PACKET_NUMBER),
  ...
  unsigned char packet[] = {
    ...
    0x00,
        ...
        const size_t kLargestObservedOffset = kReceivedEntropyOffset + kQuicEntropyHashSize;
        const size_t kMissingDeltaTimeOffset = kLargestObservedOffset + PACKET_6BYTE_PACKET_NUMBER;
        const size_t kNumTimestampsOffset = kMissingDeltaTimeOffset + kQuicDeltaTimeLargestObservedSize;
        const size_t kTimestampDeltaLargestObserved1 = kNumTimestampsOffset + kQuicNumTimestampsSize;
        const size_t kTimestampTimeDeltaLargestObserved1 = kTimestampDeltaLargestObserved1 + 1;
        const size_t kTimestampDeltaLargestObserved2 = kTimestampTimeDeltaLargestObserved1 + 4;
        const size_t kTimestampTimeDeltaLargestObserved2 = kTimestampDeltaLargestObserved2 + 1;
        const size_t kNumMissingPacketOffset = kTimestampTimeDeltaLargestObserved2 + 2;
        const size_t kMissingPacketsOffset = kNumMissingPacketOffset + kNumberOfNackRangesSize;
        const size_t kMissingPacketsRange = kMissingPacketsOffset + PACKET_1BYTE_PACKET_NUMBER;
        const size_t kRevivedPacketsLength = kMissingPacketsRange + PACKET_1BYTE_PACKET_NUMBER;
        // Now test framing boundaries.
        const size_t ack_frame_size = kRevivedPacketsLength + PACKET_1BYTE_PACKET_NUMBER;
        for (size_t i = kQuicFrameTypeSize; i < ack_frame_size; ++i) {
            ...
            if (i < kLargestObservedOffset) {
                ...
                expected_error = "Unable to read incremental time delta in received packets.";
            ...
            CheckProcessingFails(
                packet,
                i + GetPacketHeaderSize(framer_.version(), PACKET_8BYTE_CONNECTION_ID, !kIncludeVersion, !kIncludePathId, !kIncludeDiversificationNonce, PACKET_6BYTE_PACKET_NUMBER),
  ...
  unsigned char packet[] = {
    ...
    0x00,
        ...
        if (framer_.version() <= QUIC_VERSION_31 || framer_.version() > QUIC_VERSION_33) {
            return;
        }
  ...
  unsigned char packet[] = {
      ...
      0x00,
        ...
        const size_t kMissingDeltaTimeOffset = kLargestObservedOffset + PACKET_6BYTE_PACKET_NUMBER;
        const size_t kNumTimestampsOffset = kMissingDeltaTimeOffset + kQuicDeltaTimeLargestObservedSize;
        const size_t kTimestampDeltaLargestObserved = kNumTimestampsOffset + kQuicNumTimestampsSize;
        const size_t kTimestampTimeDeltaLargestObserved = kTimestampDeltaLargestObserved + 1;
  ...
  unsigned char packet[] = {
      ...
      0x00,
        ...
        const size_t kMissingDeltaTimeOffset = kLargestObservedOffset + PACKET_6BYTE_PACKET_NUMBER;
        const size_t kNumTimestampsOffset = kMissingDeltaTimeOffset + kQuicDeltaTimeLargestObservedSize;
        const size_t kNumMissingPacketOffset = kNumTimestampsOffset + kQuicNumTimestampsSize;
        const size_t kMissingPacketsOffset = kNumMissingPacketOffset + kNumberOfNackRangesSize;
        const size_t kMissingPacketsRange = kMissingPacketsOffset + PACKET_1BYTE_PACKET_NUMBER;
        const size_t kRevivedPacketsLength = kMissingPacketsRange + PACKET_1BYTE_PACKET_NUMBER;
        // Now test framing boundaries.
  ...
  unsigned char packet[] = {
      ...
      0x00,
        ...
        const size_t kLargestAckedDeltaTimeOffset = kLargestAckedOffset + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kFirstAckBlockLengthOffset = kLargestAckedDeltaTimeOffset + kQuicDeltaTimeLargestObservedSize;
        const size_t kNumTimestampsOffset = kFirstAckBlockLengthOffset + PACKET_2BYTE_PACKET_NUMBER;
        // Now test framing boundaries.
        const size_t ack_frame_size = kFirstAckBlockLengthOffset + PACKET_2BYTE_PACKET_NUMBER;
        for (size_t i = kQuicFrameTypeSize; i < ack_frame_size; ++i) {
            string expected_error;
  ...
  unsigned char packet[] = {
      ...
      0x10, 0x32,
        ...
        const size_t kLargestAckedOffset = kQuicFrameTypeSize;
        const size_t kLargestAckedDeltaTimeOffset = kLargestAckedOffset + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kNumberOfAckBlocksOffset = kLargestAckedDeltaTimeOffset + kQuicDeltaTimeLargestObservedSize;
        const size_t kFirstAckBlockLengthOffset = kNumberOfAckBlocksOffset + kNumberOfAckBlocksSize;
        const size_t kGapToNextBlockOffset1 = kFirstAckBlockLengthOffset + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kAckBlockLengthOffset1 = kGapToNextBlockOffset1 + 1;
        const size_t kGapToNextBlockOffset2 = kAckBlockLengthOffset1 + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kAckBlockLengthOffset2 = kGapToNextBlockOffset2 + 1;
        const size_t kGapToNextBlockOffset3 = kAckBlockLengthOffset2 + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kAckBlockLengthOffset3 = kGapToNextBlockOffset3 + 1;
        const size_t kGapToNextBlockOffset4 = kAckBlockLengthOffset3 + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kAckBlockLengthOffset4 = kGapToNextBlockOffset3 + 1;
        const size_t kNumTimestampsOffset = kAckBlockLengthOffset4 + PACKET_2BYTE_PACKET_NUMBER;
        const size_t kTimestampDeltaLargestObserved1 = kNumTimestampsOffset + kQuicNumTimestampsSize;
        const size_t kTimestampTimeDeltaLargestObserved1 = kTimestampDeltaLargestObserved1 + 1;
        const size_t kTimestampDeltaLargestObserved2 = kTimestampTimeDeltaLargestObserved1 + 4;
        const size_t kTimestampTimeDeltaLargestObserved2 = kTimestampDeltaLargestObserved2 + 1;
        ...
        // Now test framing boundaries.
        const size_t ack_frame_size = kAckBlockLengthOffset4 + PACKET_2BYTE_PACKET_NUMBER;
        for (size_t i = kQuicFrameTypeSize; i < ack_frame_size; ++i) {
            string expected_error;
            if (i < kLargestAckedDeltaTimeOffset) {
                expected_error = "Unable to read largest acked.";
  ...
  unsigned char packet[] = {
      ...
      0x00,
        ...
        const size_t kMissingPacketsRange = kMissingPacketsOffset + PACKET_1BYTE_PACKET_NUMBER;
        const size_t kRevivedPacketsLength = kMissingPacketsRange + PACKET_1BYTE_PACKET_NUMBER;
        const size_t kRevivedPacketSequenceNumberLength = kRevivedPacketsLength + PACKET_1BYTE_PACKET_NUMBER;
        // Now test framing boundaries.
        const size_t ack_frame_size = kRevivedPacketSequenceNumberLength + PACKET_6BYTE_PACKET_NUMBER;
        for (size_t i = kQuicFrameTypeSize; i < ack_frame_size; ++i) {
            string expected_error;
    ...
    0x01, 0x00, 0x00, 0x00,
        ...
        for (size_t i = kQuicFrameTypeSize; i < QuicFramer::GetRstStreamFrameSize();
            ...
            if (i < kQuicFrameTypeSize + kQuicMaxStreamIdSize) {
                expected_error = "Unable to read stream_id.";
            } else if (i < kQuicFrameTypeSize + kQuicMaxStreamIdSize + kQuicMaxStreamOffsetSize) {
                expected_error = "Unable to read rst stream sent byte offset.";
            } else if (i < kQuicFrameTypeSize + kQuicMaxStreamIdSize + kQuicMaxStreamOffsetSize + kQuicErrorCodeSize) {
  ...
  unsigned char packet_34[] = {
    ...
    'n',
        ...
        for (size_t i = kQuicFrameTypeSize;
            ...
            } else if (i < kQuicFrameTypeSize + kQuicErrorCodeSize + kQuicMaxStreamIdSize) {
  ...
  unsigned char packet_34[kMaxPacketSize] = {
    ...
    0x00, 0x00, 0x00, 0x00
        ...
        uint64_t header_size = GetPacketHeaderSize(
            ...
            !kIncludePathId, !kIncludeDiversificationNonce,
            PACKET_6BYTE_PACKET_NUMBER);
        memset((framer_.version() <= QUIC_VERSION_33 ? packet : packet_34) + header_size + 1,
...
TEST_P(QuicFramerTest, BuildStopWaitingPacket) {
  ...
  unsigned char packet_34[] = {
    ...
    'm',  'n',  'o',  'p',
        ...
        size_t encrypted_length = framer_.EncryptPayload(ENCRYPTION_NONE, kDefaultPathId, packet_number,
  ...
  unsigned char packet_34[] = {
    ...
    {
        return frame.stream_id == kTestQuicStreamId && !frame.fin && frame.offset == 0 && string(frame.data_buffer, frame.data_length) == kTestString;
