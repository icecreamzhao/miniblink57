...
{
    DCHECK_LT(bytes, static_cast<uint64_t>(kQuicInfiniteBandwidth / (8 * kNumMicrosPerSecond)));
    int64_t bytes_per_second = (bytes * kNumMicrosPerSecond) / delta.ToMicroseconds();
    return QuicBandwidth(bytes_per_second * 8);
}
...
{
    return ToBytesPerSecond() * time_period.ToMicroseconds() / kNumMicrosPerSecond;
}

int64_t QuicBandwidth::ToKBytesPerPeriod(QuicTime::Delta time_period) const
{
    return ToKBytesPerSecond() * time_period.ToMicroseconds() / kNumMicrosPerSecond;
}
...
QuicTime::Delta QuicBandwidth::TransferTime(QuicByteCount bytes) const
{
    ...
    return QuicTime::Delta::FromMicroseconds(bytes * 8 * kNumMicrosPerSecond / bits_per_second_);
}
