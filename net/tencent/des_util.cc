 ...
 * @shift: bits to roll
 */
const int DesUtil::BUF_SIZE = 1024;
 ...
 *
 */
unsigned long DesUtil::des_ekey(u32* pe, const u8* k)
{
 ...
 *
 */
void DesUtil::dkey(u32* pe, const u8* k)
{
    /* K&R: long is at least 32 bits */
...
int DesUtil::des_setkey(struct des_ctx* ctx, const u8* key,
    unsigned int keylen)
{
    ...
    /* Copy to output */
    memcpy(ctx->expkey, tmp, sizeof(ctx->expkey));

    return 0;
}

void DesUtil::des_encrypt(struct des_ctx* ctx, u8* dst, const u8* src)
{
...
void DesUtil::des_decrypt(struct des_ctx* ctx, u8* dst, const u8* src)
{
 ...
 *
 */
int DesUtil::des3_ede_setkey(struct des3_ede_ctx* dctx, const u8* key,
    unsigned int keylen)
{
    const u32* K = (const u32*)key;
    u32* expkey = dctx->expkey;
    if (0 == (!((K[0] ^ K[2]) | (K[1] ^ K[3])) || !((K[2] ^ K[4]) | (K[3] ^ K[5])))) {
        return -EINVAL;
    }
...
void DesUtil::des3_ede_encrypt(struct des3_ede_ctx* dctx, u8* dst, const u8* src)
{
...
void DesUtil::des3_ede_decrypt(struct des3_ede_ctx* dctx, u8* dst, const u8* src)
{
...
//使用ECB PKCS5Padding的加密方式
int DesUtil::encrypt_str(struct des_ctx* ctx, const unsigned char* src,
    unsigned int src_len, unsigned char* dest)
{
    unsigned int i = 0;
    unsigned char decrypt[DES_BLOCK_SIZE] = { 0 };

    unsigned int block = src_len / DES_BLOCK_SIZE;
    unsigned int left = src_len % DES_BLOCK_SIZE;
    unsigned int padding = DES_BLOCK_SIZE - left;

    //使用DES ECB分段进行加密
    for (i = 0; i < block; i++) {
        des_encrypt(ctx, dest + i * DES_BLOCK_SIZE, src + i * DES_BLOCK_SIZE);
    }

    //使用PKCS5Padding
    if (left > 0) {
        memcpy(decrypt, src + block * DES_BLOCK_SIZE, left);
    }

    for (unsigned int j = 0; j < padding; j++) {
        decrypt[left + j] = padding;
    }

    des_encrypt(ctx, dest + i * DES_BLOCK_SIZE, decrypt);

    block += 1;

    return block * DES_BLOCK_SIZE;
}

int DesUtil::decrypt_str(struct des_ctx* ctx, const unsigned char* src,
    unsigned int src_len, unsigned char* dest)
{
    unsigned int i = 0;
    unsigned int block = src_len / DES_BLOCK_SIZE;

    if (src_len % DES_BLOCK_SIZE != 0 || block < 1) {
        return -1;
    }

    //分段进行解密
    for (i = 0; i < block; i++) {
        des_decrypt(ctx, dest + i * DES_BLOCK_SIZE, src + i * DES_BLOCK_SIZE);
    }

    unsigned int padding = dest[src_len - 1];

    if (padding > DES_BLOCK_SIZE) {
        return -1;
    }

    dest[block * DES_BLOCK_SIZE - padding] = 0;

    return block * DES_BLOCK_SIZE - padding;
}

int DesUtil::CharToValue(char ch)
{
    if (ch >= '0' && ch <= '9') {
        return ch - '0';
    } else if (ch >= 'a' && ch <= 'f') {
...
//将每二个字符转化为无符号字符
int DesUtil::StrToUChar(const char* src, unsigned char* dst)
{
    int len = strlen(src) / 2;

    for (int i = 0; i < len; i++) {
        dst[i] = CharToValue(src[i * 2]) * 16 + CharToValue(src[i * 2 + 1]);
    }
...
//将无符号字符转化为字符串
int DesUtil::UCharToStr(const unsigned char* src, unsigned int len, char* dst)
{
    int ret = 0;

    for (unsigned int i = 0; i < len; i++) {
        ret += sprintf(dst + ret, "%02x", src[i]);
    }

    return ret;
}

DesUtil::DesUtil(const char* encriptKey)
{
    encrypt_buf_ = new unsigned char[BUF_SIZE];
    decrypt_buf_ = new unsigned char[BUF_SIZE];
    des_setkey(&ctx_, (u8*)encriptKey, strlen(encriptKey));
}
...
int DesUtil::encrypt_domain(const char* v, char** outData, int outLen)
{
    memset(encrypt_buf_, 0, BUF_SIZE);
    int len = encrypt_str(&ctx_, (u8*)v, strlen(v), encrypt_buf_);
    int ret = UCharToStr(encrypt_buf_, len, *outData);
    return ret;
}

int DesUtil::decrypt_result(const char* results, char** outData, int outLen)
{
    memset(decrypt_buf_, 0, BUF_SIZE);
    int len = StrToUChar(results, decrypt_buf_);
    len = decrypt_str(&ctx_, (u8*)decrypt_buf_, len, (u8*)*outData);
    return len;
}
...
{
     struct des_ctx G_ctx;
     char key[] = "rocksnzhang";
     char v[1024] = "www.google.com";
     unsigned char encrypt[1024] = {0};

     des_setkey(&G_ctx, (u8 *)key, strlen(key) );

     int len = encrypt_str(&G_ctx, (u8*)v, strlen(v), encrypt);
     ...
     char encrypt2[1024] = {0};
     len = decrypt_str(&G_ctx, (u8*)encrypt, len, (u8*)encrypt2);
     printf("%s\n", encrypt2);
