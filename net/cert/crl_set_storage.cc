...
static base::DictionaryValue* ReadHeader(base::StringPiece* data)
{
    ...
    std::unique_ptr<base::Value> header = base::JSONReader::Read(header_bytes, base::JSON_ALLOW_TRAILING_COMMAS);
    if (header.get() == NULL)
        return NULL;
...
#error assumes little endian
    ...
    int version;
    if (!header_dict->GetInteger("Version", &version) || version != kCurrentFileVersion) {
        return false;
    }
    ...
    for (size_t crl_index = 0; !data.empty(); crl_index++) {
        ...
        std::pair<std::string, std::vector<std::string>>* const back_pair = &crl_set->crls_.back();
    ...
    scoped_refptr<CRLSet>* out_crl_set)
{
    ...
    if (!header_dict->GetInteger("Sequence", &sequence) || !header_dict->GetInteger("DeltaFrom", &delta_from) || delta_from < 0 || static_cast<uint32_t>(delta_from) != in_crl_set->sequence_) {
        return false;
    }
...
std::string CRLSetStorage::Serialize(const CRLSet* crl_set)
{
    ...
    for (std::vector<std::string>::const_iterator i = crl_set->blocked_spkis_.begin();
