...
CRLSet::Result CRLSet::CheckSPKI(const base::StringPiece& spki_hash) const
{
    for (std::vector<std::string>::const_iterator i = blocked_spkis_.begin();
        ...
        if (spki_hash.size() == i->size() && memcmp(spki_hash.data(), i->data(), i->size()) == 0) {
            return REVOKED;
        }
    }
    ...
    const base::StringPiece& issuer_spki_hash) const
{
    ...
    std::unordered_map<std::string, size_t>::const_iterator crl_index = crls_index_by_issuer_.find(issuer_spki_hash.as_string());
