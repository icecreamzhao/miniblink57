...
namespace {
        ...
        CertVerifyResult* verify_result)
    {
        ...
        if (!verified_chain.empty()) {
            ...
            scoped_refptr<X509Certificate> verified_cert = X509Certificate::CreateFromDERCertChain(verified_chain_pieces);
        ...
        const X509Certificate::OSCertHandles& intermediates = verify_result->verified_cert->GetIntermediateCertificates();
        ...
        size_t correction_for_root = (status == android::CERT_VERIFY_STATUS_ANDROID_OK) ? 1 : 0;
        for (size_t i = 0; i < chain.size() - correction_for_root; ++i) {
            ...
            } else if (sig_alg == NID_md5WithRSAEncryption || sig_alg == NID_md5WithRSA) {
                verify_result->has_md5 = true;
            } else if (sig_alg == NID_sha1WithRSAEncryption || sig_alg == NID_dsaWithSHA || sig_alg == NID_dsaWithSHA1 || sig_alg == NID_dsaWithSHA1_2 || sig_alg == NID_sha1WithRSA || sig_alg == NID_ecdsa_with_SHA1) {
        ...
        std::vector<std::string>* chain_bytes)
    {
        ...
        X509Certificate::OSCertHandles cert_handles = cert->GetIntermediateCertificates();
        ...
        for (X509Certificate::OSCertHandles::const_iterator it = cert_handles.begin(); it != cert_handles.end(); ++it) {
