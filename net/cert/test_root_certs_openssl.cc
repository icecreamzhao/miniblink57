...
bool TestRootCerts::Add(X509Certificate* certificate)
{
    if (!X509_STORE_add_cert(X509Certificate::cert_store(),
        ...
        if (ERR_GET_LIB(error_code) != ERR_LIB_X509 || ERR_GET_REASON(error_code) != X509_R_CERT_ALREADY_IN_HASH_TABLE) {
...
{
    for (std::vector<scoped_refptr<X509Certificate>>::const_iterator it = temporary_roots_.begin();
