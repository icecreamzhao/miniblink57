...
public:
    ...
    virtual void run() override
    {
        ...
        if ((job->m_asynWkeNetSetData || job->m_diskCacheItem) && kNormalCancelled != job->m_cancelledReason) { // ������didReceiveResponse�ﱻcancel
...
//         m_manager->handleDidReceiveResponse(job);
// 
//         if (job->m_asynWkeNetSetData && kNormalCancelled != job->m_cancelledReason) { // ������didReceiveResponse�ﱻcancel
...
public:
    ...
    static void cancel(WebURLLoaderInternal* job, int jobId)
    {
        ...
        if (!job->isCancelled()) { // ������didReceiveResponse�ﱻcancel
