...
public:
    ...
    virtual void run() override
    {
        ...
        CString urlStr = urlString.utf8();
        job->m_url = std::string(urlStr.data(), urlStr.length());
...
private:
    ...
    void dispatchUrlEndHook(WebURLLoaderInternal* job, const char* data, size_t size)
    {
        ...
        if (1 == job->m_isHookRequest && loadUrlEndCallback)
            loadUrlEndCallback(page->wkeWebView(), loadUrlEndCallbackParam, job->m_url.c_str(), job, (void*)data, size);
    }
