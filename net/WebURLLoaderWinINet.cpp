...
public:
    ...
    virtual void run() OVERRIDE
    {
        ...
        WebURLLoaderWinINet* loader = m_loader; // fileLoadImpl������cancel����m_loader���
...
public:
    ...
    virtual void visitHeader(const blink::WebString& name, const blink::WebString& value) override
    {
        ...
        // ��Ų�������û�е����Ĳ���дreferr���ɴ�Ҳ�ӵ�headBuffer��
    ...
    blink::WebURLError& error, blink::WebData& data)
{
    ...
    bool b = start(request, &loader, false); // ִ������౻������
    ASSERT(!b);
...
//                 if (!ActivatingObjCheck::inst()->isActivating((intptr_t)this)) // �п�����didReceiveResponse��������������
//                     return false;
//             }
//         }
// 
...
void WebURLLoaderWinINet::cancel()
{
    ...
    if (m_requestHandle) {
        ...
        // ����InternetCloseHandle��ϵͳ�������oncomplete�������ֶ�����
    } else
