    ...
    CFStringRef port_key)
{
    ...
    CFStringRef host_ref = base::mac::GetValueFromDictionary<CFStringRef>(dict, host_key);
    if (!host_ref) {
    ...
    CFNumberRef port_ref = base::mac::GetValueFromDictionary<CFNumberRef>(dict, port_key);
    int port;
    if (port_ref) {
