...
namespace {
    ...
    const char kSimplePacScript[] = "function FindProxyForURL(url, host) {\n"
                                    "  return 'PROXY foo:1234';\n"
                                    "}";
    const char kDnsResolvePacScript[] = "function FindProxyForURL(url, host) {\n"
                                        "  if (dnsResolveEx('example.com') != '1.2.3.4')\n"
                                        "    return 'DIRECT';\n"
                                        "  return 'QUIC bar:4321';\n"
                                        "}";
    const char kThrowingPacScript[] = "function FindProxyForURL(url, host) {\n"
                                      "  alert('alert: ' + host);\n"
        ...
        const base::string16& error)
    {
        ...
        EXPECT_TRUE(base::UTF16ToUTF8(error).find("error: http://foo") != std::string::npos);
    }
    ...
    void CheckCapturedNetLogEntries(const TestNetLogEntry::List& entries)
    {
        ...
        // expected event type.
        while (i < entries.size() && entries[i].type != NetLog::TYPE_PAC_JAVASCRIPT_ALERT) {
            i++;
        }
        ...
        while (i < entries.size() && entries[i].type != NetLog::TYPE_PAC_JAVASCRIPT_ERROR) {
            i++;
        }
        message.clear();
        ASSERT_TRUE(entries[i].GetStringValue("message", &message));
...
protected:
    void SetUp() override
    {
        ...
        proxy_service_ = CreateProxyServiceUsingMojoFactory(
            this, base::WrapUnique(new ProxyConfigServiceFixed(ProxyConfig::CreateFromCustomPacURL(GURL(kPacUrl)))),
...
TEST_F(ProxyServiceMojoTest, DnsResolution)
{
    ...
    EXPECT_EQ(1, std::count_if(entries.begin(), entries.end(), [](const TestNetLogEntry& entry) {
        return entry.type == NetLog::TYPE_HOST_RESOLVER_IMPL_JOB;
    }));
}
