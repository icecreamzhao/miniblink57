...
{
    std::unique_ptr<Backend::Iterator> entry_iterator = cache_backend->CreateIterator();
    Entry* entry = nullptr;
    net::TestCompletionCallback cb;
...
bool UpdateRawResponseHeaders(Backend* cache_backend, const std::string& key)
{
    ...
    response_info.headers = new net::HttpResponseHeaders(raw_headers_stream.str());
...
int main(int argc, char* argv[])
{
    ...
    if (subcommand == "delete_key" && args.size() == 4) {
        ...
        if (index < 0)
            return 1;
        successful_command = DeleteStreamForKey(cache_backend.get(), args[3], index);
