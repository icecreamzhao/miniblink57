...
{
    net::SHA256HashValue hash = net::X509Certificate::CalculateFingerprint256(cert_handle);
...
std::string SubjectFromOSCertHandle(
    net::X509Certificate::OSCertHandle cert_handle)
{
    scoped_refptr<net::X509Certificate> cert = net::X509Certificate::CreateFromHandle(
    ...
    const base::FilePath& dump_prefix_path)
{
    ...
    scoped_refptr<net::X509Certificate> x509_target_and_intermediates = net::X509Certificate::CreateFromDERCertChain(der_cert_chain);
    ...
    for (const auto& cert : root_der_certs) {
        scoped_refptr<net::X509Certificate> x509_root = net::X509Certificate::CreateFromBytes(cert.der_cert.data(),
            cert.der_cert.size());
    ...
    int flags = net::CertVerifier::VERIFY_EV_CERT | net::CertVerifier::VERIFY_CERT_IO_ENABLED;

    scoped_refptr<net::CertVerifyProc> cert_verify_proc = net::CertVerifyProc::CreateDefault();
    if (!x509_additional_trust_anchors.empty() && !cert_verify_proc->SupportsAdditionalTrustAnchors()) {
