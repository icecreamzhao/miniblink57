...
int main(int argc, const char* argv[])
{
    ...
    if (command_line->HasSwitch("ssl-cert")) {
        ...
        if (cert_option == "ok") {
            ssl_options.server_certificate = net::SpawnedTestServer::SSLOptions::CERT_OK;
        } else if (cert_option == "mismatched-name") {
            ssl_options.server_certificate = net::SpawnedTestServer::SSLOptions::CERT_MISMATCHED_NAME;
        } else if (cert_option == "expired") {
            ssl_options.server_certificate = net::SpawnedTestServer::SSLOptions::CERT_EXPIRED;
        } else {
    ...
    if (!base::DirectoryExists(test_server->document_root())) {
        printf("Error: invalid doc root: \"%s\" does not exist!\n",
            base::UTF16ToUTF8(
                test_server->document_root().LossyDisplayName())
                .c_str());
        return -1;
    }
