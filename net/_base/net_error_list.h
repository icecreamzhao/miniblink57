...
/******** NOTICE: 这里的状态码如果已有的应该和[TBS2.x]上保持一致，后台要统计 **********/
// A network read timed out.
NET_ERROR(READ_TIMED_OUT, -1001)

// A stream has already closed, request return with error.
NET_ERROR(STREAM_CLOSED, -1002)

// A proxy connection request switch to direct impatiently and it works well. so we treat it as an proxy-error.
NET_ERROR(PROXY_IMPATIENT_SWITCH_DIRECT, -1003)

//DNS hosts file resolver error , 127.0.0.1
NET_ERROR(DNS_HOSTS_FILE_ERROR, -1004)

// Wifi authentication page open failed.
NET_ERROR(WIFI_AUTH_PAGE_OPEN_FAIL, -1005)

// QProxy report id for retrieve data success.
NET_ERROR(QPROXY_RETRIEVE_SUCCESS, -1006)

NET_ERROR(EXCEPTION_SEND_REQUEST, -1007)

// custom provider . (Coomy)
NET_ERROR(CUSTOM_PROXY_PROVIDER_EMPTY, -2001)
NET_ERROR(CUSTOM_PROXY_PROVIDER_ERROR, -2002)
NET_ERROR(CUSTOM_PROXY_TYPE_EVENT_STREAM, -2003)
