    ...
    std::string* charset, std::string* data)
{
    ...
    std::vector<std::string> meta_data = base::SplitString(base::StringPiece(after_colon, comma), ";",
    ...
    for (; iter != meta_data.end(); ++iter) {
        if (!base64_encoded && *iter == kBase64Tag) {
            base64_encoded = true;
        } else if (charset->empty() && iter->compare(0, kCharsetTagLength, kCharsetTag) == 0) {
    ...
    if (base64_encoded) {
        temp_data = UnescapeURLComponent(
            temp_data, UnescapeRule::SPACES | UnescapeRule::PATH_SEPARATORS | UnescapeRule::URL_SPECIAL_CHARS_EXCEPT_PATH_SEPARATORS | UnescapeRule::SPOOFING_AND_CONTROL_CHARS);
    }

    // Strip whitespace.
    if (base64_encoded || !(mime_type->compare(0, 5, "text/") == 0 || mime_type->find("xml") != std::string::npos)) {
    ...
    if (base64_encoded) {
        ...
        // padding.
        if ((padding_needed == 1 || padding_needed == 2) && temp_data[length - 1] != '=') {
