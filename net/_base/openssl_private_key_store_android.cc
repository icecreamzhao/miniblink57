...
bool OpenSSLPrivateKeyStore::StoreKeyPair(const GURL& url, EVP_PKEY* pkey)
{
    ...
    size_t public_len;
    crypto::AutoCBB cbb;
    if (!CBB_init(cbb.get(), 0) || !EVP_marshal_public_key(cbb.get(), pkey) || !CBB_finish(cbb.get(), &public_key, &public_len)) {
        return false;
    }
    ...
    size_t private_len;
    cbb.Reset();
    if (!CBB_init(cbb.get(), 0) || !EVP_marshal_private_key(cbb.get(), pkey) || !CBB_finish(cbb.get(), &private_key, &private_len)) {
        return false;
    }
