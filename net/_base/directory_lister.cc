...
namespace {
        ...
        DirectoryLister::ListingType listing_type)
    {
        ...
        } else if (listing_type != DirectoryLister::NO_SORT && listing_type != DirectoryLister::NO_SORT_RECURSIVE) {
            NOTREACHED();
        }
    }
...
void DirectoryLister::Core::Start()
{
    ...
    if (!base::DirectoryExists(dir_)) {
        origin_task_runner_->PostTask(
            FROM_HERE, base::Bind(&Core::DoneOnOriginThread, this, base::Passed(std::move(directory_list)), ERR_FILE_NOT_FOUND));
        return;
    }
    ...
    */
    ...
    origin_task_runner_->PostTask(
        FROM_HERE, base::Bind(&Core::DoneOnOriginThread, this, base::Passed(std::move(directory_list)), OK));
}
