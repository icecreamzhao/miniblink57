...
namespace gles2 {
        ...
        , log_synthesized_gl_errors_(true)
    {
        ...
        this_in_hex_ = std::string("GroupMarkerNotSet(crbug.com/242999)!:") + base::HexEncode(&this_temp, sizeof(this_temp));
    }
    ...
    {
        if (log_message_count_ < kMaxLogMessages || base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableGLErrorLimit)) {
            ...
            if (log_synthesized_gl_errors_) {
                ::logging::LogMessage(
                    filename, line, ::logging::LOG_ERROR)
                        .stream()
                    << prefixed_msg;
            }
