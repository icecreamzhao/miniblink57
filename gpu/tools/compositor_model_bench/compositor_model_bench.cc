...
public:
    ...
    void QueueTest(const base::FilePath& path)
    {
        ...
        if (spec.simulation_name.empty()) {
            LOG(WARNING) << "Simulation for path " << path.LossyDisplayName() << " will have a blank simulation name, since the file name isn't ASCII";
        }
...
private:
    ...
    bool InitializeNextTest()
    {
        ...
        LOG(INFO) << "Initializing test for " << spec.simulation_name << "(" << ModelToString(spec.model_under_test) << ")";
        const base::FilePath& path = spec.input_path;

        RenderNode* root = NULL;
        if (!(root = BuildRenderTreeFromFile(path))) {
            LOG(ERROR) << "Couldn't parse test configuration file " << path.LossyDisplayName();
            return false;
        }
    ...
    bool UpdateTestStatus()
    {
        ...
        if (sims_remaining_.size() && sims_remaining_.front().simulation_start_time.is_null()) {
