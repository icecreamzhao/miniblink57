...
bool RunGLES2ConformTest(const char* path)
{
    ...
    base::FilePath test_expectations_path = src_path.Append(FILE_PATH_LITERAL("gpu")).Append(FILE_PATH_LITERAL("gles2_conform_support")).Append(FILE_PATH_LITERAL("gles2_conform_test_expectations.txt"));
    ...
    if ((config_os & gpu::GPUTestConfig::kOsWin) != 0) {
        std::string angle_renderer = current_cmd_line->HasSwitch("use-angle")
    ...
    } else if ((config_os & gpu::GPUTestConfig::kOsMac) != 0 || config_os == gpu::GPUTestConfig::kOsLinux) {
        bot_config.set_api(gpu::GPUTestConfig::kAPIGLDesktop);
    } else if (config_os == gpu::GPUTestConfig::kOsChromeOS || config_os == gpu::GPUTestConfig::kOsAndroid) {
        bot_config.set_api(gpu::GPUTestConfig::kAPIGLES);
    } else {
        bot_config.set_api(gpu::GPUTestConfig::kAPIUnknown);
    }
    ...
    int32 expectation = test_expectations.GetTestExpectation(test_name, bot_config);
    ...
    if (success) {
        ...
        success = (success_index != std::string::npos) && (failed_index == std::string::npos);
    }
    if (!success) {
        LOG(ERROR) << output;
    }
