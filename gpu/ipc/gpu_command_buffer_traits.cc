...
{
    int32 temp;
    if (ReadParam(m, iter, &p->get_offset) && ReadParam(m, iter, &p->token) && ReadParam(m, iter, &temp) && ReadParam(m, iter, &p->generation)) {
...
bool ParamTraits<gpu::MailboxHolder>::Read(const Message* m,
    ...
    param_type* p)
{
    if (!ReadParam(m, iter, &p->mailbox) || !ReadParam(m, iter, &p->texture_target) || !ReadParam(m, iter, &p->sync_point))
        return false;
    return true;
}
