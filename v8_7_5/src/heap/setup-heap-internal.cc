...
#undef DATA_HANDLER_ELEMENT
        ...
        int inobject_properties)
    {
        ...
        DCHECK_IMPLIES(is_js_object && !Map::CanHaveFastTransitionableElementsKind(instance_type),
        ...
        // go in RO_SPACE.
        AllocationResult allocation = AllocateRaw(Map::kSize, is_js_object ? AllocationType::kMap : AllocationType::kReadOnly);
    ...
    {
        Object result;
        AllocationResult allocation = AllocateRaw(Map::kSize, AllocationType::kReadOnly);
        if (!allocation.To(&result))
            return allocation;
    ...
    AllocationResult Heap::Allocate(Map map, AllocationType allocation_type)
    {
        ...
        WriteBarrierMode write_barrier_mode = allocation_type == AllocationType::kYoung ? SKIP_WRITE_BARRIER
...
#define ALLOCATE_PARTIAL_MAP(instance_type, size, field_name)      \
    {                                                              \
        Map map;                                                   \
        if (!AllocatePartialMap((instance_type), (size)).To(&map)) \
            return false;                                          \
...
#undef ALLOCATE_PARTIAL_MAP
        ...
        {
            AllocationResult alloc = AllocateRaw(FixedArray::SizeFor(0), AllocationType::kReadOnly);
            if (!alloc.To(&obj))
                return false;
        ...
        {
            AllocationResult alloc = AllocateRaw(WeakFixedArray::SizeFor(0), AllocationType::kReadOnly);
            if (!alloc.To(&obj))
                return false;
        ...
        {
            AllocationResult allocation = Allocate(roots.null_map(), AllocationType::kReadOnly);
            if (!allocation.To(&obj))
                return false;
        }
        ...
        {
            AllocationResult allocation = Allocate(roots.undefined_map(), AllocationType::kReadOnly);
            if (!allocation.To(&obj))
                return false;
        }
        ...
        {
            AllocationResult allocation = Allocate(roots.the_hole_map(), AllocationType::kReadOnly);
            if (!allocation.To(&obj))
                return false;
        }
        ...
        for (unsigned i = 0; i < arraysize(struct_table); i++) {
            ...
            Map map;
            if (!AllocatePartialMap(entry.type, entry.size).To(&map))
                return false;
        ...
        {
            AllocationResult allocation = Allocate(roots.enum_cache_map(), AllocationType::kReadOnly);
            if (!allocation.To(&obj))
                return false;
        }
        ...
        // Allocate the empty descriptor array.
        {
            ...
            if (!AllocateRaw(size, AllocationType::kReadOnly).To(&obj))
                return false;
...
#define ALLOCATE_MAP(instance_type, size, field_name)     \
    {                                                     \
        Map map;                                          \
        if (!AllocateMap((instance_type), size).To(&map)) \
            return false;                                 \
        set_##field_name##_map(map);                      \
    }
    ...
    {                                                           \
        ...
        roots.field_name##_map()->SetConstructorFunctionIndex(  \
            ...
            for (unsigned i = 0; i < arraysize(string_type_table); i++) {
                ...
                // maps during GC.
                if (StringShape(entry.type).IsCons())
                    map->mark_unstable();
                roots_table()[entry.index] = map->ptr();
            }
            ...
            { // Create a separate external one byte string map for native sources.
                Map map;
                AllocationResult allocation = AllocateMap(UNCACHED_EXTERNAL_ONE_BYTE_STRING_TYPE,
                    ExternalOneByteString::kUncachedSize);
                if (!allocation.To(&map))
                    return false;
                map->SetConstructorFunctionIndex(Context::STRING_FUNCTION_INDEX);
...
#undef ALLOCATE_MAP
...

        {
            ...
            AllocationResult alloc = AllocateRaw(FixedArray::SizeFor(1), AllocationType::kReadOnly);
            if (!alloc.To(&obj))
                return false;
        ...
        {
            AllocationResult allocation = Allocate(roots.boolean_map(), AllocationType::kReadOnly);
            if (!allocation.To(&obj))
                return false;
        }
...
#undef ALLOCATE_EMPTY_FIXED_TYPED_ARRAY
    ...
    void Heap::CreateApiObjects()
    {
        ...
        Handle<InterceptorInfo> info = Handle<InterceptorInfo>::cast(isolate->factory()->NewStruct(
    ...
    void Heap::CreateInitialObjects()
    {
        ...
        for (unsigned i = 0; i < arraysize(constant_string_table); i++) {
            Handle<String> str = factory->InternalizeUtf8String(constant_string_table[i].contents);
...
#undef SYMBOL_INIT
        ...
        Handle<FeedbackCell> many_closures_cell = factory->NewManyClosuresCell(factory->undefined_value());
        set_many_closures_cell(*many_closures_cell);

        {
            Handle<FixedArray> empty_sloppy_arguments_elements = factory->NewFixedArray(2, AllocationType::kReadOnly);
        ...
        Handle<FeedbackMetadata> empty_feedback_metadata = factory->NewFeedbackMetadata(0, 0, AllocationType::kReadOnly);
