...
#define SScanF sscanf // NOLINT
    ...
    class S390Debugger {
    public:
        explicit S390Debugger(Simulator* sim)
            : sim_(sim)
        {
        }
            ...
            if (line == nullptr) {
                ...
                if ((strcmp(cmd, "si") == 0) || (strcmp(cmd, "stepi") == 0)) {
                    ...
                    if (argc == 2 || (argc == 3 && strcmp(arg2, "fp") == 0)) {
                        ...
                        } else if (arg1[0] == 'r' && (arg1[1] >= '0' && arg1[1] <= '2' && (arg1[2] == '\0' || (arg1[2] >= '0' && arg1[2] <= '5' && arg1[3] == '\0')))) {
                    ...
                    intptr_t value;
                    intptr_t stop_pc = sim_->get_pc() - (sizeof(FourByteInstr) + kPointerSize);
                    Instruction* stop_instr = reinterpret_cast<Instruction*>(stop_pc);
                    Instruction* msg_address = reinterpret_cast<Instruction*>(stop_pc + sizeof(FourByteInstr));
...
// make sure our register type can hold exactly 4/8 bytes
        ...
        // some buffer below.
        registers_[sp] = reinterpret_cast<intptr_t>(stack_) + stack_size - stack_protection_size_;
 ...
 */
    ...
    void Simulator::SoftwareInterrupt(Instruction* instr)
    {
        ...
        case kCallRtRedirected: {
            ...
            bool stack_aligned = (get_register(sp) & (::v8::internal::FLAG_sim_stack_alignment - 1)) == 0;
            Redirection* redirection = Redirection::FromInstruction(instr);
            const int kArgCount = 9;
            const int kRegisterArgCount = 5;
            int arg0_regnum = 2;
            intptr_t result_buffer = 0;
            bool uses_result_buffer = redirection->type() == ExternalReference::BUILTIN_CALL_PAIR && !ABI_RETURNS_OBJECTPAIR_IN_REGS;
            ...
            for (int i = kRegisterArgCount; i < kArgCount; i++) {
                arg[i] = stack_pointer[(kCalleeRegisterSaveAreaSize / kPointerSize) + (i - kRegisterArgCount)];
            }
            ...
            *reinterpret_cast<intptr_t*>(get_register(sp) + kStackFrameRASlot * kPointerSize) = get_register(r14);
            ...
            if (fp_call) {
                ...
                target(arg[0], arg[1]);
            } else if (redirection->type() == ExternalReference::PROFILING_GETTER_CALL) {
                if (::v8::internal::FLAG_trace_sim || !stack_aligned) {
                    PrintF("Call to host function at %p args %08" V8PRIxPTR
                ...
                if (redirection->type() == ExternalReference::BUILTIN_CALL_PAIR) {
                    SimulatorRuntimePairCall target = reinterpret_cast<SimulatorRuntimePairCall>(external);
                    ObjectPair result = target(arg[0], arg[1], arg[2], arg[3], arg[4], arg[5]);
                    intptr_t x;
                    intptr_t y;
...
// Check that the non-volatile registers have been preserved.
        ...
        const intptr_t* arguments)
    {
        ...
        uintptr_t entry_stack = (original_stack - (kCalleeRegisterSaveAreaSize + stack_arg_count * sizeof(intptr_t)));
        if (base::OS::ActivationFrameAlignment() != 0) {
            entry_stack &= -base::OS::ActivationFrameAlignment();
        }

        // Store remaining arguments on stack, from low to high memory.
        intptr_t* stack_argument = reinterpret_cast<intptr_t*>(entry_stack + kCalleeRegisterSaveAreaSize);
...
#define GET_ADDRESS(index_reg, base_reg, offset) \
    (((index_reg) == 0) ? 0 : get_register(index_reg)) + (((base_reg) == 0) ? 0 : get_register(base_reg)) + offset
    ...
    EVALUATE(RISBG)
    {
        ...
        uint64_t rotated_val = (src_val << shift_amount) | (src_val >> (64 - shift_amount));
    ...
    EVALUATE(M)
    {
        ...
        int64_t product = static_cast<int64_t>(r1_val) * static_cast<int64_t>(mem_val);
        int32_t high_bits = product >> 32;
        r1_val = high_bits;
        int32_t low_bits = product & 0x00000000FFFFFFFF;
        set_low_register(r1, high_bits);
    ...
    EVALUATE(BRXH)
    {
        ...
        r1_val += r3_val;
        int32_t compare_val = r3 % 2 == 0 ? get_low_register<int32_t>(r3 + 1) : r3_val;
    ...
    EVALUATE(SLDL)
    {
        ...
        uint64_t alu_out = (static_cast<uint64_t>(r1_val) << 32) | (static_cast<uint64_t>(r1_next_val));
        alu_out <<= shiftBits;
    ...
    EVALUATE(MSRKC)
    {
        ...
        int64_t result64 = static_cast<int64_t>(r2_val) * static_cast<int64_t>(r3_val);
    ...
    EVALUATE(TRAP4)
    {
        ...
        for (int i = 0; i < kCalleeRegisterSaveAreaSize / kPointerSize; ++i) {
            ...
            if (i != 14)
                (reinterpret_cast<intptr_t*>(sp_addr))[i] = 0xDEADBABE;
        }
    ...
    EVALUATE(MSGRKC)
    {
        ...
        bool isOF = ((r2_val == -1 && result64 == (static_cast<int64_t>(1L) << 63)) || (r2_val != 0 && result64 / r2_val != r3_val));
    ...
    EVALUATE(MLR)
    {
        ...
        uint32_t r2_val = get_low_register<uint32_t>(r2);
        uint64_t product = static_cast<uint64_t>(r1_val) * static_cast<uint64_t>(r2_val);
        int32_t high_bits = product >> 32;
        int32_t low_bits = product & 0x00000000FFFFFFFF;
        set_low_register(r1, high_bits);
        set_low_register(r1 + 1, low_bits);
...
#error unsupport __builtin_popcount
    ...
    EVALUATE(MSGF)
    {
        ...
        int64_t x2_val = (x2 == 0) ? 0 : get_register(x2);
        intptr_t d2_val = d2;
        int64_t mem_val = static_cast<int64_t>(ReadW(b2_val + d2_val + x2_val, instr));
        int64_t r1_val = get_register(r1);
    ...
    EVALUATE(MSC)
    {
        ...
        int32_t mem_val = ReadW(b2_val + d2_val + x2_val, instr);
        int32_t r1_val = get_low_register<int32_t>(r1);
        int64_t result64 = static_cast<int64_t>(r1_val) * static_cast<int64_t>(mem_val);
        int32_t result32 = static_cast<int32_t>(result64);
        bool isOF = (static_cast<int64_t>(result32) != result64);
        SetS390ConditionCode<int32_t>(result32, 0);
        SetS390OverflowCode(isOF);
    ...
    EVALUATE(AHY)
    {
        ...
        int64_t x2_val = (x2 == 0) ? 0 : get_register(x2);
        intptr_t d2_val = d2;
        int32_t mem_val = static_cast<int32_t>(ReadH(b2_val + d2_val + x2_val, instr));
        int32_t alu_out = 0;
        bool isOF = false;
        alu_out = r1_val + mem_val;
        isOF = CheckOverflowForIntAdd(r1_val, mem_val, int32_t);
    ...
    EVALUATE(ML)
    {
        ...
        uint32_t mem_val = ReadWU(b2_val + x2_val + d2, instr);
        uint32_t r1_val = get_low_register<uint32_t>(r1 + 1);
        uint64_t product = static_cast<uint64_t>(r1_val) * static_cast<uint64_t>(mem_val);
    ...
    EVALUATE(DL)
    {
        ...
        uint32_t mem_val = ReadWU(b2_val + x2_val + d2, instr);
        uint32_t r1_val = get_low_register<uint32_t>(r1 + 1);
        uint64_t quotient = static_cast<uint64_t>(r1_val) / static_cast<uint64_t>(mem_val);
        uint64_t remainder = static_cast<uint64_t>(r1_val) % static_cast<uint64_t>(mem_val);
        set_low_register(r1, remainder);
        set_low_register(r1 + 1, quotient);
        return length;
    }
...
#define ATOMIC_LOAD_AND_UPDATE_WORD64(op)                                            \
    ...
    int64_t r1_val = op(reinterpret_cast<int64_t*>(addr), r3_val, __ATOMIC_SEQ_CST); \
    set_register(r1, r1_val);
