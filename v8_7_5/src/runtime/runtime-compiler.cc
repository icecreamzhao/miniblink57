...
namespace internal {
    ...
    RUNTIME_FUNCTION(Runtime_FunctionFirstExecution)
    {
        ...
        LOG(isolate, FunctionEvent("first-execution", Script::cast(sfi->script())->id(), 0, sfi->StartPosition(), sfi->EndPosition(), sfi->DebugName()));
    ...
    RUNTIME_FUNCTION(Runtime_InstantiateAsmJs)
    {
        ...
        DCHECK(function->code() == isolate->builtins()->builtin(Builtins::kInstantiateAsmJs));
    ...
    RUNTIME_FUNCTION(Runtime_CompileForOnStackReplacement)
    {
        ...
        Handle<Code> result;
        if (maybe_result.ToHandle(&result) && result->kind() == Code::OPTIMIZED_FUNCTION) {
            DeoptimizationData data = DeoptimizationData::cast(result->deoptimization_data());
        ...
        int eval_scope_position, int eval_position)
    {
        ...
        if (native_context->allow_code_gen_from_strings()->IsFalse(isolate) && !Compiler::CodeGenerationFromStringsAllowed(isolate, native_context, source)) {
