...
static bool LookupPredicate(const int32_t* table, uint16_t size, uchar chr)
{
    ...
    while (high != low) {
        ...
        if ((current_value <= value) && (mid + 1 == size || GetEntry(TableGet<kEntryDist>(table, mid + 1)) > value)) {
            low = mid;
            break;
    ...
    bool* allow_caching_ptr)
{
    ...
    while (high != low) {
        ...
        if ((current_value <= key) && (mid + 1 == size || GetEntry(TableGet<kEntryDist>(table, mid + 1)) > key)) {
            low = mid;
            break;
    ...
    if (found) {
        ...
        if (value == 0) {
            ...
            if (allow_caching_ptr)
                *allow_caching_ptr = false;
            ...
            for (length = 0; length < kW; length++) {
                ...
                if (mapped == MultiCharacterSpecialCase<kW>::kEndOfEncoding)
                    break;
                if (ranges_are_linear) {
