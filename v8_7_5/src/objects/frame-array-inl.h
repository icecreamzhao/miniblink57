...
#define DEFINE_FRAME_ARRAY_ACCESSORS(name, type)                                        \
    ...
    {                                                                                   \
        Object obj = get(kFirstIndex + frame_ix * kElementsPerFrame + k##name##Offset); \
...
#undef DEFINE_FRAME_ARRAY_ACCESSORS
    ...
    {
        return IsWasmFrame(frame_ix) || IsWasmInterpretedFrame(frame_ix) || IsAsmJsWasmFrame(frame_ix);
    }
