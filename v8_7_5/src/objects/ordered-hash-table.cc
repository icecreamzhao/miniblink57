...
namespace internal {
        ...
        Isolate* isolate, Handle<Derived> table)
    {
        ...
        Handle<Derived> new_table = Allocate(isolate, kMinCapacity,
        ...
        Isolate* isolate, Handle<OrderedHashSet> table, GetKeysConversion convert)
    {
        ...
        int const kMaxStringTableEntries = isolate->heap()->MaxNumberToStringCacheSize();
        ...
        Isolate* isolate, Handle<Derived> table, int new_capacity)
    {
        ...
        Handle<Derived> new_table = Derived::Allocate(isolate, new_capacity,
            Heap::InYoungGeneration(*table) ? AllocationType::kYoung
                                            : AllocationType::kOld);
        int nof = table->NumberOfElements();
        int nod = table->NumberOfDeletedElements();
    ...
    {
        Handle<OrderedNameDictionary> new_table = OrderedHashTable<OrderedNameDictionary, 3>::Rehash(isolate, table,
            new_capacity);
        ...
        Handle<Object> value)
    {
        ...
        // Walk the chain of the bucket and try finding the key.
        {
            ...
            while (entry != kNotFound) {
                Object candidate_key = table->KeyAt(entry);
                // Do not add if we have the key already
                if (candidate_key->SameValueZero(raw_key))
                    return table;
                entry = table->NextChainEntry(entry);
            }
        }
        ...
        Isolate* isolate, Object key)
    {
        ...
        while (entry != kNotFound) {
            Object candidate_key = KeyAt(entry);
            DCHECK(candidate_key->IsTheHole() || Name::cast(candidate_key)->IsUniqueName());
    ...
    Handle<OrderedNameDictionary> OrderedNameDictionary::Allocate(
        Isolate* isolate, int capacity, AllocationType allocation)
    {
        Handle<OrderedNameDictionary> table = OrderedHashTable<OrderedNameDictionary, 3>::Allocate(isolate, capacity,
            allocation);
    ...
    {
        if (table->HasKey(isolate, key))
            return table;

        if (table->UsedCapacity() >= table->Capacity()) {
            MaybeHandle<SmallOrderedHashSet> new_table = SmallOrderedHashSet::Grow(isolate, table);
        ...
        Handle<Object> value)
    {
        ...
        if (table->UsedCapacity() >= table->Capacity()) {
            MaybeHandle<SmallOrderedHashMap> new_table = SmallOrderedHashMap::Grow(isolate, table);
            if (!new_table.ToHandle(&table)) {
    ...
    bool SmallOrderedHashMap::Delete(Isolate* isolate, SmallOrderedHashMap table, // weolar
        Object key)
    {
        ...
        Handle<Name> key, Handle<Object> value, PropertyDetails details)
    {
        ...
        if (table->UsedCapacity() >= table->Capacity()) {
            MaybeHandle<SmallOrderedNameDictionary> new_table = SmallOrderedNameDictionary::Grow(isolate, table);
            if (!new_table.ToHandle(&table)) {
    ...
    Handle<SmallOrderedNameDictionary> SmallOrderedNameDictionary::Rehash(
        ...
        int new_capacity)
    {
        Handle<SmallOrderedNameDictionary> new_table = SmallOrderedHashTable<SmallOrderedNameDictionary>::Rehash(isolate, table,
            new_capacity);
        new_table->SetHash(table->Hash());
        return new_table;
    }
    ...
    template <class SmallTable, class LargeTable>
    bool OrderedHashTableHandler<SmallTable, LargeTable>::Delete(Isolate* isolate, Handle<HeapObject> table, Handle<Object> key)
    {
    ...
    Handle<OrderedHashMap> OrderedHashMapHandler::AdjustRepresentation(
        Isolate* isolate, Handle<SmallOrderedHashMap> table)
    {
        Handle<OrderedHashMap> new_table = OrderedHashMap::Allocate(isolate, OrderedHashTableMinSize);
        int nof = table->NumberOfElements();
        int nod = table->NumberOfDeletedElements();
    ...
    Handle<OrderedHashSet> OrderedHashSetHandler::AdjustRepresentation(
        Isolate* isolate, Handle<SmallOrderedHashSet> table)
    {
        Handle<OrderedHashSet> new_table = OrderedHashSet::Allocate(isolate, OrderedHashTableMinSize);
        int nof = table->NumberOfElements();
        int nod = table->NumberOfDeletedElements();
    ...
    OrderedNameDictionaryHandler::AdjustRepresentation(
        Isolate* isolate, Handle<SmallOrderedNameDictionary> table)
    {
        Handle<OrderedNameDictionary> new_table = OrderedNameDictionary::Allocate(isolate, OrderedHashTableMinSize);
        int nof = table->NumberOfElements();
        int nod = table->NumberOfDeletedElements();
        ...
        for (int entry = 0; entry < (nof + nod); ++entry) {
            ...
            new_table = OrderedNameDictionary::Add(isolate, new_table, key, value, details);
        }
        ...
        Handle<Object> value)
    {
        if (table->IsSmallOrderedHashMap()) {
            Handle<SmallOrderedHashMap> small_map = Handle<SmallOrderedHashMap>::cast(table);
            MaybeHandle<SmallOrderedHashMap> new_map = SmallOrderedHashMap::Add(isolate, small_map, key, value);
            if (!new_map.is_null())
        ...
        Handle<Object> key)
    {
        if (table->IsSmallOrderedHashSet()) {
            Handle<SmallOrderedHashSet> small_set = Handle<SmallOrderedHashSet>::cast(table);
            MaybeHandle<SmallOrderedHashSet> new_set = SmallOrderedHashSet::Add(isolate, small_set, key);
            if (!new_set.is_null())
        ...
        PropertyDetails details)
    {
        if (table->IsSmallOrderedNameDictionary()) {
            Handle<SmallOrderedNameDictionary> small_dict = Handle<SmallOrderedNameDictionary>::cast(table);
            MaybeHandle<SmallOrderedNameDictionary> new_dict = SmallOrderedNameDictionary::Add(isolate, small_dict, key, value,
                details);
            if (!new_dict.is_null())
                return new_dict.ToHandleChecked();

            // We couldn't add to the small table, let's migrate to the
            // big table.
            table = OrderedNameDictionaryHandler::AdjustRepresentation(isolate, small_dict);
        }
        ...
        Isolate* isolate, Handle<HeapObject> table)
    {
        ...
        Handle<OrderedNameDictionary> large_dict = Handle<OrderedNameDictionary>::cast(table);
