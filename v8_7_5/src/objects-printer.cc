...
#undef MAKE_STRUCT_CASE
    ...
    namespace {
        ...
        template <class T>
        void DoPrintElements(std::ostream& os, Object object)
        { // NOLINT
...
#undef PRINT_ELEMENTS
    ...
    { // NOLINT
        ...
        if (is_suspended()) {
            ...
            if (fun_info->HasSourceCode()) {
                ...
                // allocate memory.
                if (fun_info->HasBytecodeArray() && fun_info->GetBytecodeArray()->HasSourcePositionTable()) {
                    os << source_position();
                    os << " (";
    ...
    namespace {
        ...
        {
            // Print in array notation for non-sparse arrays.
            MaybeObject previous_value = array->length() > 0 ? array->Get(0) : MaybeObject(kNullAddress);
            MaybeObject value;
            int previous_index = 0;
            int i;
            for (i = 1; i <= array->length(); i++) {
    ...
    void JSValue::JSValuePrint(std::ostream& os)
    { // NOLINT
    ...
    void ModuleInfoEntry::ModuleInfoEntryPrint(std::ostream& os)
    { // NOLINT
    ...
    void WasmDebugInfo::WasmDebugInfoPrint(std::ostream& os)
    { // NOLINT
    ...
    void StackTraceFrame::StackTraceFramePrint(std::ostream& os)
    { // NOLINT
    ...
    static void PrintBitMask(std::ostream& os, uint32_t value)
    { // NOLINT
        for (int i = 0; i < 32; i++) {
            if ((i & 7) == 0)
                os << " ";
    ...
    void LayoutDescriptor::Print(std::ostream& os)
    { // NOLINT
...
inline i::Object GetObjectFromRaw(void* object)
{
    ...
    if (RoundDown<i::kPtrComprIsolateRootAlignment>(object_ptr) == i::kNullAddress) {
...
V8_EXPORT_PRIVATE extern void _v8_internal_Print_Code(void* object)
{
    ...
    i::wasm::WasmCode* wasm_code = isolate->wasm_engine()->code_manager()->LookupCode(address);
    if (wasm_code) {
        i::StdoutStream os;
    ...
    if (!isolate->heap()->InSpaceSlow(address, i::CODE_SPACE) && !isolate->heap()->InSpaceSlow(address, i::LO_SPACE) && !i::InstructionStream::PcIsOffHeap(isolate, address)) {
        i::PrintF(
