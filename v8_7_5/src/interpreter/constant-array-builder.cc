...
#undef INIT_SINGLETON_ENTRY_FIELD
                zone_(zone)
        {
            idx_slice_[0] = new (zone) ConstantArraySlice(zone, 0, k8BitCapacity, OperandSize::kByte);
        ...
        Handle<FixedArray> ConstantArrayBuilder::ToFixedArray(Isolate* isolate)
        {
            ...
            for (const ConstantArraySlice* slice : idx_slice_) {
                ...
                for (size_t i = 0; i < slice->size(); ++i) {
                    Handle<Object> value = slice->At(slice->start_index() + i).ToHandle(isolate);
        ...
        size_t ConstantArrayBuilder::Insert(const AstRawString* raw_string)
        {
            return constants_map_
                .LookupOrInsert(
                    reinterpret_cast<intptr_t>(raw_string),
                    raw_string->Hash(),
        ...
        size_t ConstantArrayBuilder::Insert(AstBigInt bigint)
        {
            return constants_map_
                .LookupOrInsert(
                    reinterpret_cast<intptr_t>(bigint.c_str()),
        ...
        size_t ConstantArrayBuilder::Insert(const Scope* scope)
        {
            return constants_map_
                .LookupOrInsert(
                    reinterpret_cast<intptr_t>(scope),
