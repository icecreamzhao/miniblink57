...
namespace internal {
    ...
    void StringStream::PrintObject(Object o)
    {
        ...
        if (o->IsHeapObject() && object_print_mode_ == kPrintObjectVerbose) {
            ...
            DebugObjectCache* debug_object_cache = isolate->string_stream_debug_object_cache();
    ...
    {
        if (object_print_mode_ == kPrintObjectConcise)
            return;
        DebugObjectCache* debug_object_cache = isolate->string_stream_debug_object_cache();
        Add("==== Key         ============================================\n\n");
        for (size_t i = 0; i < debug_object_cache->size(); i++) {
            ...
            if (printee->IsJSObject()) {
                ...
                if (printee->IsJSArray()) {
                    ...
                    if (array->HasObjectElements()) {
                        ...
                        unsigned int length = static_cast<uint32_t>(JSArray::cast(array)->length()->Number());
                        if (length < limit)
                            limit = length;
        ...
        if (receiver->IsNullOrUndefined(isolate) || receiver->IsTheHole(isolate) || receiver->IsJSProxy()) {
            ...
            for (PrototypeIterator iter(isolate, JSObject::cast(receiver),
                ...
                if (!key->IsUndefined(isolate)) {
                    if (!name->IsString() || !key->IsString() || !String::cast(name)->Equals(String::cast(key))) {
                        print_name = true;
                    }
