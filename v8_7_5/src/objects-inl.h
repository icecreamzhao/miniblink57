...
namespace internal {
    ...
    {
        return map()->instance_type() == JS_GENERATOR_OBJECT_TYPE || IsJSAsyncFunctionObject() || IsJSAsyncGeneratorObject();
    }
...
#undef IS_TYPE_FUNCTION_DEF
    ...
    bool HeapObject::IsSeqOneByteString() const
    {
        if (!IsString())
            return false;
        return StringShape(String::cast(*this)).IsSequential() && String::cast(*this)->IsOneByteRepresentation();
    }
    ...
    bool HeapObject::IsSeqTwoByteString() const
    {
        if (!IsString())
            return false;
        return StringShape(String::cast(*this)).IsSequential() && String::cast(*this)->IsTwoByteRepresentation();
    }
    ...
    bool HeapObject::IsExternalOneByteString() const
    {
        if (!IsString())
            return false;
        return StringShape(String::cast(*this)).IsExternal() && String::cast(*this)->IsOneByteRepresentation();
    }
    ...
    bool HeapObject::IsExternalTwoByteString() const
    {
        if (!IsString())
            return false;
        return StringShape(String::cast(*this)).IsExternal() && String::cast(*this)->IsTwoByteRepresentation();
    }
    ...
    {
        return map() == GetReadOnlyRoots().array_list_map() || *this == GetReadOnlyRoots().empty_fixed_array();
    }
    ...
    bool HeapObject::IsTemplateList() const
    {
        ...
        // There's actually no way to see the difference between a fixed array and
        // a template list.
        if (FixedArray::cast(*this)->length() < 1)
            return false;
        return true;
    }
    ...
    {
        return IsSmallOrderedHashSet() || IsSmallOrderedHashMap() || IsSmallOrderedNameDictionary();
    }
    ...
    Maybe<bool> Object::IsArray(Handle<Object> object)
    {
        ...
        if (heap_object->IsJSArray())
            return Just(true);
        if (!heap_object->IsJSProxy())
            return Just(false);
...
#undef MAKE_STRUCT_PREDICATE
    ...
    {
        return this->IsHeapNumber() && i::IsMinusZero(HeapNumber::cast(*this)->value());
    }
    ...
    bool Object::FilterKey(PropertyFilter filter)
    {
        ...
        if (filter == PRIVATE_NAMES_ONLY) {
            ...
            if (Symbol::cast(*this)->is_private())
                return true;
        } else {
    ...
    MaybeHandle<Object> Object::ToPropertyKey(Isolate* isolate,
        Handle<Object> value)
    {
        if (value->IsSmi() || HeapObject::cast(*value)->IsName())
            return value;
    ...
    AllocationAlignment HeapObject::RequiredAlignment(Map map)
    {
        ...
        if (instance_type == FIXED_FLOAT64_ARRAY_TYPE || instance_type == FIXED_DOUBLE_ARRAY_TYPE) {
