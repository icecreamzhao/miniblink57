...
namespace internal {
    ...
    void TurboAssembler::Subu(Register rd, Register rs, const Operand& rt)
    {
        if (rt.is_reg()) {
            ...
            } else if (!(-rt.immediate() & kHiMask) && !MustUseReg(rt.rmode())) { // Use load
    ...
    void TurboAssembler::Ulw(Register rd, const MemOperand& rs)
    {
        ...
        if (IsMipsArchVariant(kMips32r6)) {
            ...
            DCHECK(IsMipsArchVariant(kMips32r2) || IsMipsArchVariant(kMips32r1) || IsMipsArchVariant(kLoongson));
...
#define BRANCH_ARGS_CHECK(cond, rs, rt) \
    ...
    {
        BlockTrampolinePoolScope block_trampoline_pool(this);
        if (IsMipsArchVariant(kMips32r6) && bd == PROTECT && !is_int16(offset.immediate())) {
    ...
    {
        if (IsMipsArchVariant(kMips32r6) && bdslot == PROTECT && (!L->is_bound() || is_near_r6(L))) {
            BranchShortHelperR6(0, L);
        } else {
