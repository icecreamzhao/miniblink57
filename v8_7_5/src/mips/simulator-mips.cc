...
#undef XSTR
    ...
    CachePage* Simulator::GetCachePage(base::CustomMatcherHashMap* i_cache,
        void* page)
    {
        base::CustomMatcherHashMap::Entry* entry = i_cache->LookupOrInsert(page, ICacheHash(page));
    ...
    void Simulator::set_fpu_register_hi_word(int fpureg, int32_t value)
    {
        ...
        // TODO(plind): big endian issue.
        DCHECK((fpureg >= 0) && (fpureg < kNumFPURegisters));
        int32_t* phiword = (reinterpret_cast<int32_t*>(&FPUregisters_[fpureg * 2])) + 1;
        *phiword = value;
    }
    ...
    void Simulator::TraceRegWr(int32_t value, TraceType t)
    {
        if (::v8::internal::FLAG_trace_sim) {
            ...
            switch (t) {
            case WORD:
                SNPrintF(trace_buf_, "%08" PRIx32 "    (%" PRIu64 ")    int32:%" PRId32 " uint32:%" PRIu32,
    ...
    void Simulator::TraceRegWr(int64_t value, TraceType t)
    {
        if (::v8::internal::FLAG_trace_sim) {
            ...
            switch (t) {
            case DWORD:
                SNPrintF(trace_buf_, "%016" PRIx64 "    (%" PRIu64 ")    int64:%" PRId64 " uint64:%" PRIu64,
                    value, icount_, value, value);
                break;
            case DOUBLE:
    ...
    void Simulator::TraceMemRd(int32_t addr, int32_t value, TraceType t)
    {
        if (::v8::internal::FLAG_trace_sim) {
            ...
            switch (t) {
            case WORD:
                SNPrintF(trace_buf_, "%08" PRIx32 " <-- [%08" PRIx32 "]    (%" PRIu64 ")    int32:%" PRId32 " uint32:%" PRIu32,
    ...
    void Simulator::TraceMemRd(int32_t addr, int64_t value, TraceType t)
    {
        if (::v8::internal::FLAG_trace_sim) {
            ...
            switch (t) {
            case DWORD:
                SNPrintF(trace_buf_, "%016" PRIx64 " <-- [%08" PRIx32 "]    (%" PRIu64 ")    int64:%" PRId64 " uint64:%" PRIu64,
                    v.fmt_int64, addr, icount_, v.fmt_int64, v.fmt_int64);
                break;
            case DOUBLE:
                SNPrintF(trace_buf_, "%016" PRIx64 " <-- [%08" PRIx32 "]    (%" PRIu64 ")    dbl:%e",
                    v.fmt_int64, addr, icount_, v.fmt_double);
                break;
            case FLOAT_DOUBLE:
                SNPrintF(trace_buf_, "%08" PRIx32 " <-- [%08" PRIx32 "]    (%" PRIu64 ")    flt:%e dbl:%e",
