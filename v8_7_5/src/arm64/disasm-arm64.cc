...
namespace internal {
    ...
    void DisassemblingDecoder::VisitAddSubExtended(Instruction* instr)
    {
        ...
        const char* form = ((mode == UXTX) || (mode == SXTX)) ? "'Rds, 'Rns, 'Xm'Ext" : "'Rds, 'Rns, 'Wm'Ext";
        const char* form_cmp = ((mode == UXTX) || (mode == SXTX)) ? "'Rns, 'Xm'Ext" : "'Rns, 'Wm'Ext";
    ...
    {
        DCHECK((reg_size == kXRegSizeInBits) || ((reg_size == kWRegSizeInBits) && (value <= 0xFFFFFFFF)));

        // Test for movz: 16-bits set at positions 0, 16, 32 or 48.
        if (((value & 0xFFFFFFFFFFFF0000UL) == 0UL) || ((value & 0xFFFFFFFF0000FFFFUL) == 0UL) || ((value & 0xFFFF0000FFFFFFFFUL) == 0UL) || ((value & 0x0000FFFFFFFFFFFFUL) == 0UL)) {
            return true;
        }

        // Test for movn: NOT(16-bits set at positions 0, 16, 32 or 48).
        if ((reg_size == kXRegSizeInBits) && (((value & 0xFFFFFFFFFFFF0000UL) == 0xFFFFFFFFFFFF0000UL) || ((value & 0xFFFFFFFF0000FFFFUL) == 0xFFFFFFFF0000FFFFUL) || ((value & 0xFFFF0000FFFFFFFFUL) == 0xFFFF0000FFFFFFFFUL) || ((value & 0x0000FFFFFFFFFFFFUL) == 0x0000FFFFFFFFFFFFUL))) {
            return true;
        }
        if ((reg_size == kWRegSizeInBits) && (((value & 0xFFFF0000) == 0xFFFF0000) || ((value & 0x0000FFFF) == 0x0000FFFF))) {
            return true;
        }
        return false;
    }
    ...
    void DisassemblingDecoder::VisitBitfield(Instruction* instr)
    {
        ...
        unsigned rd_size_minus_1 = ((instr->SixtyFourBits() == 1) ? kXRegSizeInBits : kWRegSizeInBits) - 1;
        const char* mnemonic = "";
        const char* form = "";
...
#undef FORMAT
    ...
    void DisassemblingDecoder::VisitNEON3Same(Instruction* instr)
    {
        ...
        if (instr->Mask(NEON3SameLogicalFMask) == NEON3SameLogicalFixed) {
            ...
            unsigned index = (instr->Bits(15, 11) << 2) | (instr->Bit(23) << 1) | instr->Bit(29);
    ...
    void DisassemblingDecoder::VisitNEON2RegMisc(Instruction* instr)
    {
        ...
        if (instr->Mask(NEON2RegMiscOpcode) <= NEON_NEG_opcode) {
            ...
            case NEON_FCMLT_zero:
                ...
                break;
            default:
                if ((NEON_XTN_opcode <= instr->Mask(NEON2RegMiscOpcode)) && (instr->Mask(NEON2RegMiscOpcode) <= NEON_UQXTN_opcode)) {
        ...
        Instruction* instr)
    {
        ...
        const char* form_4v = "{'Vt.%1$s, 'Vt2.%1$s, 'Vt3.%1$s, 'Vt4.%1$s}, ['Xns], 'Xmr4";
        NEONFormatDecoder nfd(instr, NEONFormatDecoder::LoadStoreFormatMap());
    ...
    void DisassemblingDecoder::VisitNEONLoadStoreSingleStruct(Instruction* instr)
    {
        ...
        case NEON_ST4_s:
            DCHECK(allocated);
            allocated = (instr->NEONLSSize() <= 1) && ((instr->NEONLSSize() == 0) || (instr->NEONS() == 0));
            break;
        case NEON_LD1R:
        case NEON_LD2R:
        case NEON_LD3R:
    ...
    void DisassemblingDecoder::VisitNEONShiftImmediate(Instruction* instr)
    {
        ...
        if (instr->ImmNEONImmh()) { // immh has to be non-zero.
            ...
            case NEON_SSHLL:
                ...
                if (instr->ImmNEONImmb() == 0 && CountSetBits(instr->ImmNEONImmh(), 32) == 1) { // sxtl variant.
                    form = form_xtl;
            ...
            case NEON_USHLL:
                nfd.SetFormatMap(0, &map_shift_ta);
                if (instr->ImmNEONImmb() == 0 && CountSetBits(instr->ImmNEONImmh(), 32) == 1) { // uxtl variant.
                    form = form_xtl;
    ...
    void DisassemblingDecoder::VisitNEONTable(Instruction* instr)
    {
        ...
        const char form_4v[] = "'Vd.%%s, {'Vn.16b, v%d.16b, v%d.16b, v%d.16b}, 'Vm.%%s";
        static const NEONFormatMap map_b = { { 30 }, { NF_8B, NF_16B } };
        NEONFormatDecoder nfd(instr, &map_b);
        ...
        const char* format)
    {
        ...
        if ((reg_type == CPURegister::kRegister) && (reg_num == kZeroRegCode) && (format[2] == 's')) {
        ...
        const char* format)
    {
        ...
        case 'L': {
            ...
            case 'L': { // ILLiteral - Immediate Load Literal.
                AppendToOutput("pc%+" PRId32, instr->ImmLLiteral() << kLoadLiteralScaleLog2);
                return 9;
            }
        ...
        case 'S': { // IS - Test and branch bit.
            AppendToOutput("#%d", (instr->ImmTestBranchBit5() << 5) | instr->ImmTestBranchBit40());
            return 2;
        }
        ...
        case 'V': { // Immediate Vector.
            ...
            case 'M': { // Modified Immediate cases.
                if (strncmp(format, "IVMIImmFPSingle", strlen("IVMIImmFPSingle")) == 0) {
        ...
        const char* format)
    {
        ...
        if (((instr->Rd() == kZeroRegCode) || (instr->Rn() == kZeroRegCode)) && (((instr->ExtendMode() == UXTW) && (instr->SixtyFourBits() == 0)) || (instr->ExtendMode() == UXTX))) {
