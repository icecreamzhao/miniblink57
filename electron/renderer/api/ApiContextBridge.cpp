...
bool IsPlainObject(const v8::Local<v8::Value>& object)
{
    ...
    return !(object->IsNull() || object->IsUndefined() || object->IsDate() ||
    ...
    BridgeErrorTarget error_target)
{
    ...
    if (value->IsString() || value->IsNumber() || value->IsNull() || value->IsUndefined() ||
...
//         auto then_cb = [proxied_promise, isolate, std::move(global_then_source_context), std::move(global_then_destination_context)](const v8::FunctionCallbackInfo<v8::Value>& info) {
//             v8::Local<v8::Value> result;
//             if (global_source_context.IsEmpty() || global_destination_context.IsEmpty())
//                 return;
// 
//             context_bridge::ObjectCache object_cache;
//             auto val = PassValueToOtherContext(global_source_context.Get(isolate), global_destination_context.Get(isolate), result, &object_cache, false, 0);
//             if (!val.IsEmpty())
...
//         auto catch_cb = ([](std::shared_ptr<gin::Promise<v8::Local<v8::Value>>> proxied_promise,
...
// 
//         ignore_result(source_promise->Then(source_context,
//             gin::ConvertToV8(destination_context->GetIsolate(), then_cb).As<v8::Function>(), 
//             gin::ConvertToV8(destination_context->GetIsolate(), catch_cb).As<v8::Function>()));
// 
...

    {
        ...
        for (uint32_t i = 0; i < length; i++) {
            ...
            if (support_dynamic_properties) {
                ...
                v8::MaybeLocal<v8::Value> maybe_desc = api.GetHandle()->GetOwnPropertyDescriptor(source_context, key.As<v8::String>());
...
static v8::Local<v8::Context> GetContextByV8Object(v8::Isolate* isolate, v8::Local<v8::Object> obj, WorldIDs id)
{
    ...
    if (!s_mbGetContextByV8Object(isolate, &global, 0, &context))
...
//     v8::Local<v8::Context> main_context = frame->MainWorldScriptContext();
...

    {
        ...
        v8::Local<v8::Value> getter_proxy;
        v8::Local<v8::Value> setter_proxy;
        if (!(getter->IsNull() || getter->IsUndefined())) {
        ...
        if (!(setter->IsNull() || setter->IsUndefined()) && setter->IsObject()) {
