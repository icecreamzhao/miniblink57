...
HMODULE s_hMod = nullptr;

void initNapiStub()
{
    s_hMod = LoadLibraryW(L"node.dll");
}
...
//         napi_module_registerPtr = (napi_module_registerFn)GetProcAddress(s_hMod, "napi_module_register");
...
#define DEFINE_FUN3_EX(name, p1, p2, p3)                                     \
    ...
    {                                                                        \
        ...
        if (!funcPtr)                                                        \
            funcPtr = (name##FN)GetProcAddress(s_hMod, #name);               \
