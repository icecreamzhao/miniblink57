...
static napi_env GetEnv(v8::Local<v8::Context> context)
{
    ...
    if (value->IsExternal()) {
        ...
        CHECK(global->SetPrivate(context, NAPI_PRIVATE_KEY(context, env), external).FromJust());
...
public:
    ...
    napi_status Init()
    {
        ...
        if (uv_async_init(m_env->loop, &m_async, AsyncCb) == 0) {
            ...
            if ((m_max_queue_size == 0 || m_cond.get() != nullptr) &&
                uv_idle_init(m_env->loop, &m_idle) == 0) {
                return napi_ok;
            }
    ...
    void DispatchOne()
    {
        ...
        if (popped_value || idle_stop_failed) {
            ...
            if (idle_stop_failed) {
                CHECK(napi_throw_error(m_env, "ERR_NAPI_TSFN_STOP_IDLE_LOOP", "Failed to stop the idle loop") == napi_ok);
            } else {
