...
#define NODE_CLANG_AT_LEAST(major, minor, patch) \
    (NODE_MAKE_VERSION(major, minor, patch) <= NODE_MAKE_VERSION(__clang_major__, __clang_minor__, __clang_patchlevel__))
#else
...
#define NODE_GNUC_AT_LEAST(major, minor, patch) \
    (NODE_MAKE_VERSION(major, minor, patch) <= NODE_MAKE_VERSION(__GNUC__, __GNUC_MINOR__, __GNUC_PATCHLEVEL__))
#else
...
NODE_EXTERN void CleanNodeEnv(void* env);
...
#define NODE_DEFINE_CONSTANT(target, constant)                                                                         \
    do {                                                                                                               \
        ...
        v8::Local<v8::String> constant_name = v8::String::NewFromUtf8(isolate, #constant);                             \
...
NODE_DEPRECATED(
    ...
    inline v8::Local<v8::Value> Encode(
        ...
        v8::Isolate* isolate = v8::Isolate::GetCurrent();
        if (encoding == UCS2) {
            //NODE_ASSERT(reinterpret_cast<uintptr_t>(buf) % sizeof(uint16_t) == 0 && "UCS2 buffer must be aligned on two-byte boundary.");
...
#define NODE_MODULE_CONTEXT_AWARE_X(modname, regfunc, priv, flags) \
    ...
    extern "C" NODE_C_CTOR(_register_##modname)                               \
    {                                                              \
        node_module_register(&_module);                            \
    }                                                              \
...
#define NODE_MODULE_BUILTIN_SCRIPT_DECLARE_IN_MAIN(modname) \
    extern "C" void __cdecl _register_##modname(void);

#define NODE_MODULE_BUILTIN_SCRIPT_DEFINDE_IN_MAIN(modname) \
    _register_##modname();
