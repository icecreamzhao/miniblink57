...
private:
    ...
    NODEJS_DISALLOW_COPY_AND_ASSIGN(FSReqWrap);
};

#define NODE_ASSERT_PATH(path) \
    if (*path == nullptr)      \
...
static void After(uv_fs_t* req)
{
    ...
    NODE_CHECK_EQ(req_wrap->req(), req);
    ...
    if (req->result < 0) {
        ...
        default:
            NODE_CHECK(0 && "Unhandled eio response");
        }
    }
...
private:
    NODEJS_DISALLOW_COPY_AND_ASSIGN(fs_req_wrap);
};

#define ASYNC_DEST_CALL(func, request, dest, encoding, ...)         \
    Environment* env = Environment::GetCurrent(args);               \
    NODE_CHECK(request->IsObject());                                \
...
#undef X
    ...
    // Dates.
#define X(name)                                                                                                     \
    Local<Value> name##_msec = Number::New(env->isolate(),                                                          \
        (static_cast<double>(s->st_##name.tv_sec) * 1000) + (static_cast<double>(s->st_##name.tv_nsec / 1000000))); \
                                                                                                                    \
...
#undef X
    ...
    Local<Value> stats = env->fs_stats_constructor_function()->NewInstance(
                                                                 env->context(),
                                                                 arraysize(argv),
                                                                 argv)
...
static void InternalModuleReadFile(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (;;) {
        ...
        uv_fs_t read_req;
        const ssize_t numchars = uv_fs_read(loop, &read_req, fd, &buf, 1, offset, nullptr);
        uv_fs_req_cleanup(&read_req);

        NODE_CHECK_GE(numchars, 0);
    ...
    uv_fs_t close_req;
    NODE_CHECK_EQ(0, uv_fs_close(loop, &close_req, fd, nullptr));
...
static void Link(const FunctionCallbackInfo<Value>& args)
{
    ...
    NODE_ASSERT_PATH(src)

    BufferValue dest(env->isolate(), args[1]);
    NODE_ASSERT_PATH(dest)
...
static void Rename(const FunctionCallbackInfo<Value>& args)
{
    ...
    NODE_ASSERT_PATH(old_path)
    BufferValue new_path(env->isolate(), args[1]);
    NODE_ASSERT_PATH(new_path)
...
static void FTruncate(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!len_v->IsUndefined() && !len_v->IsNull() && !IsInt64(len_v->NumberValue())) {
...
static void GetLongPathSync(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    const int argc = args.Length();
    ...
    std::string longPathUtf8;
#if defined(WIN32)
    std::wstring pathW = utf8ToUtf16(std::string(path.out(), path.length()));

    std::vector<WCHAR> longPath(MAX_PATH + 1);
    DWORD result = GetLongPathNameW(pathW.c_str(), longPath.data(), longPath.size());
    if (0 == result) {
        args.GetReturnValue().Set(args[0]);
    }
    longPathUtf8 = utf16ToUtf8(longPath.data());
#else
    longPathUtf8 = std::string(path.out(), path.length());
#endif

    Local<Value> rc = StringBytes::Encode(env->isolate(), longPathUtf8.c_str(), UTF8);
    args.GetReturnValue().Set(rc);
}
...
static void CopyFile(const FunctionCallbackInfo<Value>& args)
{
    ...
    //CHECK_NOT_NULL(*src);
#if defined(WIN32)
    std::wstring srcW = utf8ToUtf16(std::string(src.out(), src.length()));

    BufferValue dest(isolate, args[1]);
    //CHECK_NOT_NULL(*dest);
    std::wstring destW = utf8ToUtf16(std::string(dest.out(), dest.length()));

    //CHECK(args[2]->IsInt32());
    //const int flags = args[2].As<Int32>()->Value();

    BOOL b = ::CopyFile(srcW.c_str(), destW.c_str(), FALSE);
...
static void ReadDir(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (callback->IsObject()) {
        ...
        NODE_CHECK_GE(SYNC_REQ.result, 0);
        int r;
        Local<Array> names = Array::New(env->isolate(), 0);
        Local<Function> fn = env->push_values_to_array_function();
...
static void WriteBuffer(const FunctionCallbackInfo<Value>& args)
{
    ...
    NODE_CHECK(Buffer::HasInstance(args[1]));
...
static void WriteString(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!req->IsObject()) {
        ...
        struct Delete {
            inline explicit Delete(char* pointer)
                : pointer_(pointer)
            {
            }
    ...
    FSReqWrap* req_wrap = FSReqWrap::New(env, req.As<Object>(), "write", buf, UTF8, ownership);
...
void FSInitialize(const FunctionCallbackInfo<Value>& args)
{
    ...
    NODE_CHECK(stats_constructor->IsFunction());
    ...
    void* priv)
{
    ...
    env->SetMethod(target, "getLongPathSync", GetLongPathSync);
