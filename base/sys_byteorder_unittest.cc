...
TEST(ByteOrderTest, ByteSwapUintPtrT)
{
    ...
    const uintptr_t swapped_test_data = static_cast<uintptr_t>(k64BitSwappedTestData);
#elif defined(ARCH_CPU_32_BITS)
    const uintptr_t test_data = static_cast<uintptr_t>(k32BitTestData);
    const uintptr_t swapped_test_data = static_cast<uintptr_t>(k32BitSwappedTestData);
#else
