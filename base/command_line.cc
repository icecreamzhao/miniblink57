...
bool CommandLine::HasSwitch(const base::StringPiece& switch_string) const
{
    ...
    return switches_by_stringpiece_.find(switch_string) != switches_by_stringpiece_.end();
}
...
CommandLine::StringVector CommandLine::GetArgs() const
{
    ...
    StringVector::iterator switch_terminator = std::find(args.begin(), args.end(), kSwitchTerminator);
