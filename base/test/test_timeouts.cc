...
#elif defined(ADDRESS_SANITIZER) || defined(THREAD_SANITIZER) || defined(SYZYASAN)
...
void InitializeTimeout(const char* switch_name, int min_value, int* value)
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switch_name)) {
        std::string string_value(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switch_name));
        int timeout;
