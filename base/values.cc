...
namespace {
    ...
    static_assert(arraysize(kTypeNames) == static_cast<size_t>(Value::Type::LIST) + 1,
...
bool DictionaryValue::GetDictionaryWithoutPathExpansion(
    ...
    DictionaryValue** out_value)
{
    const DictionaryValue& const_this = static_cast<const DictionaryValue&>(*this);
...
std::unique_ptr<DictionaryValue> DictionaryValue::DeepCopyWithoutEmptyChildren()
    const
{
    std::unique_ptr<DictionaryValue> copy = CopyDictionaryWithoutEmptyChildren(*this);
    if (!copy)
        copy.reset(new DictionaryValue);
    return copy;
}
...
bool DictionaryValue::Equals(const Value* other) const
{
    ...
    const DictionaryValue* other_dict = static_cast<const DictionaryValue*>(other);
    Iterator lhs_it(*this);
    Iterator rhs_it(*other_dict);
    while (!lhs_it.IsAtEnd() && !rhs_it.IsAtEnd()) {
        if (lhs_it.key() != rhs_it.key() || !lhs_it.value().Equals(&rhs_it.value())) {
            return false;
        }
        lhs_it.Advance();
        rhs_it.Advance();
...
bool ListValue::Equals(const Value* other) const
{
    ...
    const ListValue* other_list = static_cast<const ListValue*>(other);
...
{
    if (static_cast<int>(type) < 0 || static_cast<size_t>(type) >= arraysize(kTypeNames))
