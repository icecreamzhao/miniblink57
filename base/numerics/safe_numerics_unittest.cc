...
Dst GetMaxConvertibleToFloat()
{
    ...
    if (SrcLimits::digits <= DstLimits::digits && MaxExponent<Src>::value <= MaxExponent<Dst>::value)
...
static void TestArithmetic(const char* dst, int line)
{
    ...
    EXPECT_EQ(static_cast<Dst>(1),
        CheckedNumeric<Dst>(CheckedNumeric<Dst>(DstLimits::max()) * DstLimits::max()).ValueOrDefault(1));
...
struct TestNumericConversion<Dst, Src, SIGN_TO_UNSIGN_NARROW> {
    static void Test(const char* dst, const char* src, int line)
    {
        typedef numeric_limits<Src> SrcLimits;
        typedef numeric_limits<Dst> DstLimits;
        static_assert((DstLimits::is_integer && SrcLimits::is_iec559) || (sizeof(Dst) < sizeof(Src)),
            "Destination must be narrower than source.");
        static_assert(SrcLimits::is_signed, "Source must be signed.");
        static_assert(!DstLimits::is_signed, "Destination must be unsigned.");
...
#pragma warning(disable : 4756)
    ...
    float not_a_number = std::numeric_limits<float>::infinity() - std::numeric_limits<float>::infinity();
