...
#if defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_32_BITS) && defined(COMPILER_GCC) && !defined(OS_NACL) && __OPTIMIZE__
...
{
    const int kIntMaxForLayoutUnit = std::numeric_limits<int>::max() >> fractional_shift;

    const int kIntMinForLayoutUnit = std::numeric_limits<int>::min() >> fractional_shift;
...
{
    const unsigned kIntMaxForLayoutUnit = std::numeric_limits<int>::max() >> fractional_shift;
