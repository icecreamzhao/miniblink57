...
bool BaseInitLoggingImpl(const LoggingSettings& settings)
{
    ...
    // vlog switches.
    if (command_line->HasSwitch(switches::kV) || command_line->HasSwitch(switches::kVModule)) {
        ...
        g_vlog_info = new VlogInfo(command_line->GetSwitchValueASCII(switches::kV),
...
int GetVlogLevelHelper(const char* file, size_t N)
{
    ...
    return vlog_info ? vlog_info->GetVlogLevel(base::StringPiece(file, N - 1)) : GetVlogVerbosity();
}
...
LogMessage::~LogMessage()
{
    ...
    if ((g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0) {
        ...
        if (log_via_asl) {
            ...
            std::string asl_facility = main_bundle_id_cf ? base::SysCFStringRefToUTF8(main_bundle_id_cf)
...
#undef ASL_LEVEL_STR_X
    ...
    if (severity_ == LOG_FATAL) {
        ...
        base::debug::GlobalActivityTracker* tracker = base::debug::GlobalActivityTracker::Get();
        if (tracker)
...
BASE_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    if (len) {
        ...
        return base::CollapseWhitespaceASCII(msgbuf, true) + base::StringPrintf(" (0x%X)", error_code);
    }
