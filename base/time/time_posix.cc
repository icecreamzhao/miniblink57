...
base::LazyInstance<base::Lock>::Leaky
    g_sys_time_to_time_struct_lock
...
#if (defined(OS_POSIX) && defined(_POSIX_MONOTONIC_CLOCK) && _POSIX_MONOTONIC_CLOCK >= 0) || defined(OS_BSD) || defined(OS_ANDROID)
...
Time Time::Now()
{
    ...
    return Time((tv.tv_sec * kMicrosecondsPerSecond + tv.tv_usec) + kWindowsEpochDeltaMicroseconds);
}
...
{
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || defined(OS_ANDROID)
...
Time Time::FromTimeVal(struct timeval t)
{
    ...
    if (t.tv_usec == 0 && t.tv_sec == 0)
        return Time();
    if (t.tv_usec == static_cast<suseconds_t>(Time::kMicrosecondsPerSecond) - 1 && t.tv_sec == std::numeric_limits<time_t>::max())
        return Max();
    return Time((static_cast<int64_t>(t.tv_sec) * Time::kMicrosecondsPerSecond) + t.tv_usec + kTimeTToMicrosecondsOffset);
}
