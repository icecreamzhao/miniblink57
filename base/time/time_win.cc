...
base::LazyInstance<base::Lock>::Leaky g_high_res_lock = LAZY_INSTANCE_INITIALIZER;
...
bool Time::FromExploded(bool is_local, const Exploded& exploded, Time* time)
{
    ...
    if (is_local) {
        SYSTEMTIME utc_st;
        success = TzSpecificLocalTimeToSystemTime(nullptr, &st, &utc_st) && SystemTimeToFileTime(&utc_st, &ft);
    } else {
...
void Time::Explode(bool is_local, Exploded* exploded) const
{
    ...
    if (is_local) {
        ...
        success = FileTimeToSystemTime(&utc_ft, &utc_st) && SystemTimeToTzSpecificLocalTime(nullptr, &utc_st, &st);
    } else {
...
TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value)
{
    ...
    return TimeDelta::FromMicroseconds(
        (whole_seconds * Time::kMicrosecondsPerSecond) + ((leftover_ticks * Time::kMicrosecondsPerSecond) / g_qpc_ticks_per_second));
}
...
{
    static bool is_supported = base::CPU().has_non_stop_time_stamp_counter() && !IsBuggyAthlon(base::CPU());
    return is_supported;
}
