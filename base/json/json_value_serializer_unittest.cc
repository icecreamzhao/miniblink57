...
namespace {
    ...
    TEST(JSONValueDeserializerTest, ReadProperJSONFromString)
    {
        ...
        std::unique_ptr<Value> value = str_deserializer.Deserialize(&error_code, &error_message);
        ASSERT_TRUE(value);
    ...
    TEST(JSONValueDeserializerTest, ReadProperJSONFromFile)
    {
        ...
        int error_code = 0;
        std::string error_message;
        std::unique_ptr<Value> value = file_deserializer.Deserialize(&error_code, &error_message);
        ASSERT_TRUE(value);
        ASSERT_EQ(0, error_code);
        ASSERT_TRUE(error_message.empty());
        // Verify if the same JSON is still there.
    ...
    {
        static const char kOriginalSerialization[] = "{\"bool\":true,\"double\":3.14,\"int\":42,\"list\":[1,2],\"null\":null}";
        JSONStringValueDeserializer deserializer(kOriginalSerialization);
        std::unique_ptr<DictionaryValue> root_dict = DictionaryValue::From(deserializer.Deserialize(nullptr, nullptr));
...
#define JSON_NEWLINE "\n"
#endif
        const std::string pretty_serialization = "{" JSON_NEWLINE
...
#undef JSON_NEWLINE
    ...
    TEST(JSONValueSerializerTest, StringEscape)
    {
        ...
        // uneval(s).replace(/\\/g, "\\\\");
        std::string all_chars_expected = "\\u0001\\u0002\\u0003\\u0004\\u0005\\u0006\\u0007\\b\\t\\n\\u000B\\f\\r"
                                         "\\u000E\\u000F\\u0010\\u0011\\u0012\\u0013\\u0014\\u0015\\u0016\\u0017"
                                         "\\u0018\\u0019\\u001A\\u001B\\u001C\\u001D\\u001E\\u001F !\\\"#$%&'()*+,"
    ...
    TEST(JSONValueSerializerTest, UnicodeStrings)
    {
        ...
        std::unique_ptr<Value> deserial_root = deserializer.Deserialize(nullptr, nullptr);
        ASSERT_TRUE(deserial_root);
        DictionaryValue* dict_root = static_cast<DictionaryValue*>(deserial_root.get());
        string16 web_value;
    ...
    TEST(JSONValueSerializerTest, JSONReaderComments)
    {
        ...
        std::unique_ptr<ListValue> list = ListValue::From(JSONReader::Read("[\"// ok\\n /* foo */ \"]"));
        ASSERT_TRUE(list);
        ASSERT_EQ(1U, list->GetSize());
        Value* elt = nullptr;
        ASSERT_TRUE(list->Get(0, &elt));
    ...
    TEST_F(JSONFileValueSerializerTest, Roundtrip)
    {
        ...
        // Now try writing.
        const FilePath written_file_path = temp_dir_.GetPath().AppendASCII("test_output.js");
    ...
    TEST_F(JSONFileValueSerializerTest, RoundtripNested)
    {
        FilePath original_file_path;
        ASSERT_TRUE(PathService::Get(DIR_TEST_DATA, &original_file_path));
        original_file_path = original_file_path.AppendASCII("serializer_nested_test.json");
        ...
        // Now try writing.
        FilePath written_file_path = temp_dir_.GetPath().AppendASCII("test_output.json");
    ...
    TEST_F(JSONFileValueSerializerTest, NoWhitespace)
    {
        ...
        source_file_path = source_file_path.AppendASCII("serializer_test_nowhitespace.json");
