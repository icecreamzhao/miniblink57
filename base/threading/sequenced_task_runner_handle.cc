...
scoped_refptr<SequencedTaskRunner> SequencedTaskRunnerHandle::Get()
{
    ...
    scoped_refptr<base::SequencedTaskRunner> task_runner = SequencedWorkerPool::GetSequencedTaskRunnerForCurrentThread();
    if (task_runner) {
        DCHECK(!base::ThreadTaskRunnerHandle::IsSet());
        return task_runner;
    }
...
{
    return lazy_tls_ptr.Pointer()->Get() || SequencedWorkerPool::GetWorkerPoolForCurrentThread() || base::ThreadTaskRunnerHandle::IsSet();
}
