...
namespace android {
    ...
    TEST_F(PathUtilsTest, TestGetNativeLibraryDirectory)
    {
        ...
        EXPECT_TRUE(
            base::PathExists(path.Append("libbase_unittests.so")) || base::PathExists(path.Append("libbase_unittests.cr.so")) || base::PathExists(path.Append("lib_base_unittests__library.so")) || base::PathExists(path.Append("lib_base_unittests__library.cr.so")));
    }
