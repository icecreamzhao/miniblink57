...
#if !defined(ADDRESS_SANITIZER) && !defined(MEMORY_SANITIZER) && !defined(THREAD_SANITIZER) && !defined(LEAK_SANITIZER) && !BUILDFLAG(USE_EXPERIMENTAL_ALLOCATOR_SHIM)
...
#elif defined(MEMORY_TOOL_REPLACES_ALLOCATOR) || (!defined(LIBC_GLIBC) && !defined(USE_TCMALLOC))
    *result = malloc(size);
