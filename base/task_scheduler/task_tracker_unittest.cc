    ...
    namespace {
        ...
        public:
            ThreadRunningTask(TaskTracker* tracker, const Task* task)
                : SimpleThread("ThreadRunningTask")
                , tracker_(tracker)
                , task_(task)
            {
            }
        ...
        protected:
            ...
            void CallShutdownAsync()
            {
                ...
                while (!tracker_.IsShuttingDownForTesting() && !tracker_.shutdown_completed()) {
...
#define VERIFY_ASYNC_SHUTDOWN_IN_PROGRESS() \
    ...
    {
        const bool can_use_singletons = (GetParam() != TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN);
        ...
        SequencedTaskRunnerHandleIsSetOnSequenced)
    {
        ...
        // being set to it.
        std::unique_ptr<Task> verify_task(
            new Task(FROM_HERE, Bind(&VerifySequencedTaskRunnerHandle, base::Unretained(test_task_runner.get())),
                TaskTraits().WithShutdownBehavior(GetParam()), TimeDelta()));
        ...
        ThreadTaskRunnerHandleIsSetOnSingleThreaded)
    {
        ...
        // being set on it.
        std::unique_ptr<Task> verify_task(
            new Task(FROM_HERE, Bind(&VerifyThreadTaskRunnerHandle, base::Unretained(test_task_runner.get())),
                TaskTraits().WithShutdownBehavior(GetParam()), TimeDelta()));
