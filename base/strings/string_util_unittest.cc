...
TEST(StringUtilTest, TruncateUTF8ToByteSize)
{
    ...
    EXPECT_TRUE(Truncated("a\xc2\x81\xe1\x80\xbf\xf1"
                          "a"
                          "\x80\xa0",
        10, &output));
    EXPECT_EQ(output.compare("a\xc2\x81\xe1\x80\xbf\xf1"
                             "a"),
        0);
    EXPECT_FALSE(Truncated("\xef\xbb\xbf"
                           "abc",
        6, &output));
...
TEST(StringUtilTest, IsStringUTF8)
{
    ...
    EXPECT_TRUE(IsStringUTF8("\xef\xbb\xbf"
                             "abc")); // UTF-8 BOM
...
TEST(StringUtilTest, ReplaceStringPlaceholdersOneDigit)
{
    ...
    string16 formatted = ReplaceStringPlaceholders(ASCIIToUTF16(" $16 "), subst, nullptr);
...
TEST(StringUtilTest, ReplaceStringPlaceholdersInvalidPlaceholder)
{
    std::vector<string16> subst;
    subst.push_back(ASCIIToUTF16("1a"));
    string16 formatted = ReplaceStringPlaceholders(ASCIIToUTF16("+$-+$A+$1+"), subst, nullptr);
