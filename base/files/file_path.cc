...
namespace {
    ...
    StringPieceType::size_type FindDriveLetter(StringPieceType path)
    {
        ...
        if (path.length() >= 2 && path[1] == L':' && ((path[0] >= L'A' && path[0] <= L'Z') || (path[0] >= L'a' && path[0] <= L'z'))) {
            return 1;
        }
    ...
    StringType::size_type ExtensionSeparatorPosition(const StringType& path)
    {
        ...
        const StringType::size_type penultimate_dot = path.rfind(FilePath::kExtensionSeparator, last_dot - 1);
        const StringType::size_type last_separator = path.find_last_of(FilePath::kSeparators, last_dot - 1,
            FilePath::kSeparatorsLength - 1);

        if (penultimate_dot == StringType::npos || (last_separator != StringType::npos && penultimate_dot < last_separator)) {
            return last_dot;
        }
        ...
        for (size_t i = 0; i < arraysize(kCommonDoubleExtensionSuffixes); ++i) {
            if (LowerCaseEqualsASCII(extension, kCommonDoubleExtensionSuffixes[i])) {
                if ((last_dot - penultimate_dot) <= 5U && (last_dot - penultimate_dot) > 1U) {
                    return penultimate_dot;
                }
            }
        }
    ...
    {
        // Special cases "", ".", and ".."
        if (path.empty() || path == FilePath::kCurrentDirectory || path == FilePath::kParentDirectory) {
            return true;
        }
    ...
    FilePath* path) const
{
    ...
    if (parent_components.empty() || parent_components.size() >= child_components.size())
        return false;
    ...
    if ((FindDriveLetter(*parent_comp) != StringType::npos) && (FindDriveLetter(*child_comp) != StringType::npos)) {
...
FilePath FilePath::DirName() const
{
    ...
    StringType::size_type last_separator = new_path.path_.find_last_of(kSeparators, StringType::npos,
        kSeparatorsLength - 1);
...
bool FilePath::ReferencesParent() const
{
    ...
    for (; it != components.end(); ++it) {
        ...
        if (component.find_first_not_of(FILE_PATH_LITERAL(". \n\r\t")) == std::string::npos && component.find(kParentDirectory) != std::string::npos) {
            return true;
        }
    }
    return false;
...
{
    static decltype(::CharUpperW)* const char_upper_api = reinterpret_cast<decltype(::CharUpperW)*>(
    ...
    for (; i1 != string1end && i2 != string2end; ++i1, ++i2) {
        wchar_t c1 = (wchar_t)LOWORD(char_upper_api((LPWSTR)(DWORD_PTR)MAKELONG(*i1, 0)));
        wchar_t c2 = (wchar_t)LOWORD(char_upper_api((LPWSTR)(DWORD_PTR)MAKELONG(*i2, 0)));
        if (c1 < c2)
            return -1;
        if (c1 > c2)
            return 1;
