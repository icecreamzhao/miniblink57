...
const base::FilePath::CharType PersistentMemoryAllocator::kFileExtension[] = FILE_PATH_LITERAL(".pma");
...
PersistentMemoryAllocator::Iterator::Iterator(
    ...
    : allocator_(allocator)
    , last_record_(starting_after)
    , record_count_(0)
{
    // Ensure that the starting point is a valid, iterable block (meaning it can
    // be read and has a non-zero "next" pointer).
    const volatile BlockHeader* block = allocator_->GetBlock(starting_after, 0, 0, false, false);
...
PersistentMemoryAllocator::Iterator::GetNext(uint32_t* type_return)
{
    ...
    Reference next;
    while (true) {
        const volatile BlockHeader* block = allocator_->GetBlock(last, 0, 0, true, false);
    ...
    const uint32_t max_records = freeptr / (sizeof(BlockHeader) + kAllocAlignment);
...
bool PersistentMemoryAllocator::IsMemoryAcceptable(const void* base,
    ...
    bool readonly)
{
    return ((base && reinterpret_cast<uintptr_t>(base) % kAllocAlignment == 0) && (size >= sizeof(SharedMetadata) && size <= kSegmentMaxSize) && (size % kAllocAlignment == 0 || readonly) && (page_size == 0 || size % page_size == 0 || readonly));
}
...
//     CHECK(corrupt_.is_lock_free());
    OutputDebugStringA("PersistentMemoryAllocator::PersistentMemoryAllocator is not impl\n");
    *(int*)1 = 1;
    ...
    if (shared_meta()->cookie != kGlobalCookie) {
        ...
        if (shared_meta()->size == 0 || shared_meta()->version == 0 || shared_meta()->freeptr.load(std::memory_order_relaxed) == 0 || shared_meta()->tailptr == 0 || shared_meta()->queue.cookie == 0 || shared_meta()->queue.next.load(std::memory_order_relaxed) == 0) {
            SetCorrupt();
        }
        if (!readonly) {
    ...
    uint32_t type_id)
{
    ...
    /* const */ uint32_t freeptr = shared_meta()->freeptr.load(std::memory_order_acquire);
    ...
    for (;;) {
        ...
        if (block->size != 0 || block->cookie != kBlockCookieFree || block->type_id.load(std::memory_order_relaxed) != 0 || block->next.load(std::memory_order_relaxed) != 0) {
            SetCorrupt();
            return kReferenceNull;
        }
...
{
    if (corrupt_.load(std::memory_order_relaxed) || CheckFlag(&shared_meta()->flags, kFlagCorrupt)) {
    ...
    bool free_ok) const
{
    ...
    if (!free_ok) {
        ...
        if (ref + size > freeptr)
            return nullptr;
        const volatile BlockHeader* const block = reinterpret_cast<volatile BlockHeader*>(mem_base_ + ref);
        if (block->size < size)
            return nullptr;
        ...
        if (ref != kReferenceQueue && block->cookie != kBlockCookieAllocated)
            return nullptr;
        if (type_id != 0 && block->type_id.load(std::memory_order_relaxed) != type_id) {
            return nullptr;
        }
    }
...
{
    DCHECK(size > 0);
    const volatile BlockHeader* block = GetBlock(ref, type_id, size, false, false);
    if (!block)
        return nullptr;
