...
base::LazyInstance<base::StatisticsRecorder>::Leaky g_statistics_recorder_ = LAZY_INSTANCE_INITIALIZER;
    ...
    , include_persistent_(include_persistent)
{
    ...
    if (!include_persistent_ && iter_ != histograms_->end() && (iter_->second->flags() & HistogramBase::kIsPersistent)) {
...
StatisticsRecorder::HistogramIterator::operator++()
{
    ...
    for (;;) {
        ...
        if (iter_ == histograms_end)
            break;
        if (!include_persistent_ && (iter_->second->flags() & HistogramBase::kIsPersistent)) {
            continue;
        }
        break;
    }
