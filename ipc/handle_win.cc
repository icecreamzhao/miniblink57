    ...
    param_type* r)
{
    ...
    if (attachment->GetType() != MessageAttachment::TYPE_BROKERABLE_ATTACHMENT)
        return false;
    BrokerableAttachment* brokerable_attachment = static_cast<BrokerableAttachment*>(attachment);
    if (brokerable_attachment->GetBrokerableType() != BrokerableAttachment::WIN_HANDLE) {
        return false;
    }
    IPC::internal::HandleAttachmentWin* handle_attachment = static_cast<IPC::internal::HandleAttachmentWin*>(brokerable_attachment);
