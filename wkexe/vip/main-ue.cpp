...
#include "E:\\mycode\\mtmb\\mtmb\\core\\mb.h"
...
void TestCaseSetClipData(HWND hWnd)
{//这里将导致：当在ueditor进行粘贴的时候 getAsFile出现空指针~~
...
void createSimpleMb()
{
    ...
    ::mbLoadURL(view, "file:///G:/test/web_test/ueditor/temp.htm");

    mbOnJsQuery(view, onJsQuery, (void*)1);

    mbShowWindow(view, TRUE);

	TestCaseSetClipData(hWnd);//测试样例~~
}
