...
int CWndBrowser::m_globalCount = 0;
...
LRESULT CWndBrowser::hideWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void CWndBrowser::onMoreCommand()
{
    POINT pt = { 0 };
    ::GetCursorPos(&pt);
    if (m_hMenu) {
...
void CWndBrowser::onShowDevtools()
{
...
bool CWndBrowser::onCommand(UINT id) {
    switch (id) {
...
void CWndBrowser::onCanGoForwardCallback(mbWebView webView, void* param, MbAsynRequestState state, BOOL b)
{
    if (kMbAsynRequestStateOk != state)
        return;
    CWndBrowser* self = (CWndBrowser*)param;
    ::EnableWindow(self->m_forwardHwnd, b);
}

void CWndBrowser::onCanGoBackCallback(mbWebView webView, void* param, MbAsynRequestState state, BOOL b)
{
    if (kMbAsynRequestStateOk != state)
        return;
    CWndBrowser* self = (CWndBrowser*)param;
...
void CWndBrowser::onUrlChangedCallback(mbWebView webView, void* param, const utf8* url, BOOL canGoBack, BOOL canGoForward)
{
    CWndBrowser* self = (CWndBrowser*)param;
...
void CWndBrowser::onDocumentReady(mbWebView webView, void* param, mbWebFrameHandle frameId)
{
...
void CWndBrowser::onLoadingFinish(mbWebView webView, void* param, mbWebFrameHandle frameId, const utf8* url, mbLoadingResult result, const utf8* failedReason)
{
    if (mbIsMainFrame(webView, frameId))
...
mbWebView CWndBrowser::onCreateView(mbWebView parentWebviwe, void* param, mbNavigationType navType, const utf8* url, const mbWindowFeatures* features)
{
    CWndBrowser* rootWin = new CWndBrowser();
    mbWebView newWindow = rootWin->createWnd();
...
void CWndBrowser::onPaint()
{

}

void CWndBrowser::onSize(bool minimized)
{
    RECT rect;
...
void CWndBrowser::onWheel(WPARAM wParam, LPARAM lParam)
{
    int x = LOWORD(lParam);
    int y = HIWORD(lParam);
...
LRESULT CALLBACK rootWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    CWndBrowser* self = getUserDataPtr<CWndBrowser*>(hWnd);
    if (!self)
...
LRESULT CALLBACK CWndBrowser::editWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message) {
    case WM_CHAR:
        if (wParam == VK_RETURN) {
            HWND parent = GetParent(hWnd);
            CWndBrowser* self = getUserDataPtr<CWndBrowser*>(parent);
...
mbWebView CWndBrowser::createWnd()
{
    RECT startRect;
    startRect.left = 200;
    startRect.top = 100;
    ...
    const std::wstring& window_class = L"MBCWndBrowser";
...
void CWndBrowser::createChildControl(HWND parentHwnd)
{
    HINSTANCE hInstance = GetModuleHandle(NULL);
