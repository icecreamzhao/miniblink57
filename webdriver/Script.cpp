...
Script::Script(IHTMLDocument2* document, std::string script_source)
{
    std::wstring wide_script = StringUtilities::ToWString(script_source);
    this->Initialize(document, wide_script, 0);
}

Script::Script(IHTMLDocument2* document, std::wstring script_source)
{
...
int Script::Execute()
{
    ...
    hr = function_dispatch->InvokeEx(
        call_member_id, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &call_parameters, &result, &exception, custom_exception_service_provider);
...
int Script::Execute(const IECommandExecutor& command_executor, const Json::Value& args, Json::Value* result)
{
    IECommandExecutor& mutable_executor = const_cast<IECommandExecutor&>(command_executor);
    int status_code = this->AddArguments(mutable_executor.element_manager(), args);
    status_code = this->Execute();
    status_code = this->ConvertResultToJsonValue(command_executor, result);
    return status_code;
}

int Script::ExecuteAsync(const IECommandExecutor& command_executor, const Json::Value& args, HWND* async_executor_handle)
{
    return this->ExecuteAsync(command_executor, args, 0, async_executor_handle);
}

int Script::ExecuteAsync(
    const IECommandExecutor& command_executor, const Json::Value& args, const int timeout_override_in_milliseconds, HWND* async_executor_handle)
{
    ...
    return_code = this->CreateAsyncScriptExecutor(command_executor.window_handle(), serialized_args, async_executor_handle);
    ...
    if (this->polling_source_code_.size() > 0) {
        ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_SET_POLLING_SCRIPT, NULL, reinterpret_cast<LPARAM>(this->polling_source_code_.c_str()));
    }

    std::string required_element_list = "";
    ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_GET_REQUIRED_ELEMENT_LIST, NULL, reinterpret_cast<LPARAM>(&required_element_list));
    Json::Value required_elements;
    std::string parse_errors;
    std::stringstream json_stream;
    json_stream.str(required_element_list);
    Json::parseFromStream(Json::CharReaderBuilder(), json_stream, &required_elements, &parse_errors);

    for (Json::UInt i = 0; i < required_elements.size(); ++i) {
        std::string element_id = required_elements[i].asString();
        this->SetAsyncScriptElementArgument(*async_executor_handle, command_executor, element_id);
    }
    ...
    bool is_execution_ready = ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_IS_EXECUTION_READY, NULL, NULL) != 0;
    while (!is_execution_ready && --retry_counter > 0) {
        ::Sleep(SCRIPT_WAIT_TIME_IN_MILLISECONDS);
        is_execution_ready = ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_IS_EXECUTION_READY, NULL, NULL) != 0;
    }
    ...
    if (timeout_override_in_milliseconds > 0) {
        ...
        bool is_execution_finished = ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_IS_EXECUTION_COMPLETE, NULL, NULL) != 0;
        while (!is_execution_finished && --retry_counter > 0) {
            ::Sleep(SCRIPT_WAIT_TIME_IN_MILLISECONDS);
            is_execution_finished = ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_IS_EXECUTION_COMPLETE, NULL, NULL) != 0;
        }
        ...
        if (is_execution_finished) {
            ...
            int status_code
                = static_cast<int>(::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_GET_RESULT, NULL, reinterpret_cast<LPARAM>(&script_result)));
            return status_code;
        } else {
            LOG(TRACE) << "Async script execution not completed after timeout, detaching listener";
            ::SendMessage(*async_executor_handle, WD_ASYNC_SCRIPT_DETACH_LISTENTER, NULL, NULL);
        }
    }
    return WD_SUCCESS;
}

int Script::CreateAsyncScriptExecutor(HWND element_repository_handle, const std::wstring& serialized_args, HWND* async_executor_handle)
{
    ...
    if (event_handle != NULL) {
        ...
        result.Clear();
        result.vt = VT_BSTR;
        error_description = L"Couldn't create an event for synchronizing the creation of the thread. This generally means that you were trying to click on an "
                            L"option in two different instances.";
        result.bstrVal = error_description;
        this->result_.Copy(&result);
        return EUNEXPECTEDJSERROR;
    }
    ...
    if (event_handle == NULL || ::GetLastError() == ERROR_ALREADY_EXISTS) {
        if (event_handle == NULL) {
            ...
            error_description = L"Couldn't create an event for synchronizing the creation of the thread. This is an internal failure at the Windows OS level, "
                                L"and is generally not due to an error in the IE driver.";
        } else {
            ::CloseHandle(event_handle);
            LOG(WARN) << "Synchronization event is already created in another instance.";
            error_description = L"Couldn't create an event for synchronizing the creation of the thread. This generally means that you were trying to click on "
                                L"an option in multiple different instances.";
        }
        result.Clear();
        result.vt = VT_BSTR;
        result.bstrVal = error_description;
    ...
    HANDLE thread_handle
        = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, AsyncScriptExecutor::ThreadProc, reinterpret_cast<void*>(&thread_context), 0, &thread_id));
...
int Script::SetAsyncScriptDocument(HWND async_executor_handle)
{
    ...
    ::SendMessage(async_executor_handle, WD_ASYNC_SCRIPT_SET_DOCUMENT, NULL, reinterpret_cast<LPARAM>(document_stream));
    return WD_SUCCESS;
}

int Script::SetAsyncScriptElementArgument(HWND async_executor_handle, const IECommandExecutor& command_executor, const std::string& element_id)
{
    ...
    if (return_code != WD_SUCCESS) {
        LOG(WARN) << "Element requested with id " << element_id << " does not exist.";
        return return_code;
    }
    HRESULT hr = ::CoMarshalInterThreadInterfaceInStream(IID_IHTMLElement, element_wrapper->element(), &info->element_stream);
    if (FAILED(hr)) {
        CComVariant result;
        CComBSTR error_description = L"";
    ...
    ::PostMessage(async_executor_handle, WD_ASYNC_SCRIPT_SET_ELEMENT_ARGUMENT, NULL, reinterpret_cast<LPARAM>(info));
    return return_code;
}
...
int Script::ConvertResultToJsonValue(IElementManager* element_manager, Json::Value* value)
{
    LOG(TRACE) << "Entering Script::ConvertResultToJsonValue";
    int status_code = VariantUtilities::VariantAsJsonValue(element_manager, this->result_, value);
    if (status_code != WD_SUCCESS) {
        ...
        Script stringify_script_wrapper(this->script_engine_host_, json_stringify_script, 1);
        ...
        if (status_code == WD_SUCCESS) {
            ...
            if (result.vt == VT_BSTR) {
                ...
                bool successful_parse = Json::parseFromStream(Json::CharReaderBuilder(), json_stream, &interim_value, &parse_errors);
                if (successful_parse) {
                    *value = interim_value;
                }
            }
...
bool Script::CreateAnonymousFunction(VARIANT* result)
{
    ...
    bool get_result_success = VariantUtilities::GetVariantObjectPropertyValue(this->script_engine_host_, L"__webdriver_script_fn", result);
...
int Script::WalkArray(IElementManager* element_manager, const Json::Value& array_value)
{
...
int Script::WalkObject(IElementManager* element_manager, const Json::Value& object_value)
{
