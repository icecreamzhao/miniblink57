...
Browser::Browser(IWebBrowser2* browser, HWND hwnd, HWND session_handle, bool is_edge_chromium)
    : DocumentHost(hwnd, session_handle)
{
...
void __stdcall Browser::BeforeNavigate2(
    IDispatch* pObject, VARIANT* pvarUrl, VARIANT* pvarFlags, VARIANT* pvarTargetFrame, VARIANT* pvarData, VARIANT* pvarHeaders, VARIANT_BOOL* pbCancel)
{
...
void __stdcall Browser::OnQuit()
{
    ...
    if (!this->is_explicit_close_requested_) {
        if (this->is_awaiting_new_process()) {
            ...
            this->DetachEvents();
            this->browser_ = NULL;
            ::PostMessage(this->executor_handle(), WD_BROWSER_REATTACH, NULL, reinterpret_cast<LPARAM>(info));
            return;
        } else {
            LOG(WARN) << "This instance of Internet Explorer (" << this->browser_id() << ") is exiting without an explicit request to close it. "
...
void __stdcall Browser::NewProcess(DWORD lCauseFlag, IDispatch* pWB2, VARIANT_BOOL* pbCancel)
{
...
void __stdcall Browser::NewWindow3(IDispatch** ppDisp, VARIANT_BOOL* pbCancel, DWORD dwFlags, BSTR bstrUrlContext, BSTR bstrUrl)
{
    ...
    if (this->is_edge_chromium()) {
        ...
        LRESULT create_result = ::SendMessage(this->executor_handle(), WD_BROWSER_NEW_WINDOW, NULL, reinterpret_cast<LPARAM>(&info));
        if (create_result != 0) {
        ...
        HRESULT hr = ::CoGetInterfaceAndReleaseStream(info.browser_stream, IID_IWebBrowser2, reinterpret_cast<void**>(&browser));
        if (FAILED(hr)) {
    ...
    ::PostMessage(this->executor_handle(), WD_AFTER_NEW_WINDOW, param_flag, reinterpret_cast<LPARAM>(ie_window_handles));
}
...
void Browser::InitiateBrowserReattach()
{
    ...
    WindowUtilities::GetProcessesByName(L"iexplore.exe", &this->known_process_ids_);
...
void Browser::GetDocument(const bool force_top_level_document, IHTMLDocument2** doc)
{
...
void Browser::Close()
{
    ...
    if (this->is_edge_chromium()) {
        ...
        ::SendMessage(this->executor_handle(), WD_ADD_CHROMIUM_WINDOW_HANDLE, reinterpret_cast<WPARAM>(top_level_window_handle), NULL);
    }
...
int Browser::NavigateToUrl(const std::string& url, std::string* error_message)
{
    ...
    if (FAILED(hr)) {
        ...
        _com_error error(hr);
        std::wstring formatted_message = StringUtilities::Format(L"Received error: 0x%08x ['%s']", hr, error.ErrorMessage());
...
int Browser::NavigateBack()
{
    ...
    HANDLE thread_handle = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &Browser::GoBackThreadProc, (void*)stream, 0, &thread_id));
...
unsigned int WINAPI Browser::GoBackThreadProc(LPVOID param)
{
    ...
    hr = ::CoGetInterfaceAndReleaseStream(message_payload, IID_IWebBrowser2, reinterpret_cast<void**>(&browser));
    if (browser != NULL) {
        hr = browser->GoBack();
    }
    return 0;
...
int Browser::NavigateForward()
{
    ...
    HRESULT hr = ::CoMarshalInterThreadInterfaceInStream(IID_IWebBrowser2, this->browser_, &stream);
    unsigned int thread_id = 0;
    HANDLE thread_handle = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &Browser::GoForwardThreadProc, (void*)stream, 0, &thread_id));
    if (thread_handle != NULL) {
        ::CloseHandle(thread_handle);
    }
...
bool Browser::IsDocumentNavigating(const std::string& page_load_strategy, IHTMLDocument2* doc)
{
    ...
    if (FAILED(hr) || is_navigating) {
        ...
        if ((ready_state == L"complete") || (page_load_strategy == EAGER_PAGE_LOAD_STRATEGY && ready_state == L"interactive")) {
            is_navigating = false;
        } else {
            if (page_load_strategy == EAGER_PAGE_LOAD_STRATEGY) {
