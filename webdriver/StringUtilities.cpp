...
std::string StringUtilities::toLowerASCII(const std::string& str)
{
...
bool StringUtilities::compareStringsIgnoreCase(const std::string& str1, const std::string& str2)
{
...
std::wstring StringUtilities::ToWString(const std::string& input)
{
    ...
    bool convert_failed = (0 == ::MultiByteToWideChar(CP_UTF8, 0, input.c_str(), input_string_byte_count, &output_buffer[0], wide_string_length));
    if (convert_failed) {
        if (::GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            ...
            wide_string_length = ::MultiByteToWideChar(CP_UTF8, 0, input.c_str(), input_string_byte_count, NULL, 0);
            output_buffer.resize(wide_string_length);
            convert_failed = (0 == ::MultiByteToWideChar(CP_UTF8, 0, input.c_str(), input_string_byte_count, &output_buffer[0], wide_string_length));
            if (!convert_failed) {
...
std::string StringUtilities::ToString(const std::wstring& input)
{
    ...
    bool convert_failed = (0 == ::WideCharToMultiByte(CP_UTF8, 0, input.c_str(), wide_string_length, &string_buffer[0], output_string_byte_count, NULL, NULL));
    if (convert_failed) {
        if (::GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            // Buffer wasn't big enough. Call WideCharToMultiByte again with
            // NULL values to determine how big the buffer should be.
            output_string_byte_count = ::WideCharToMultiByte(CP_UTF8, 0, input.c_str(), wide_string_length, NULL, 0, NULL, NULL);
            string_buffer.resize(output_string_byte_count);
            convert_failed
                = (0 == ::WideCharToMultiByte(CP_UTF8, 0, input.c_str(), wide_string_length, &string_buffer[0], output_string_byte_count, NULL, NULL));
            if (!convert_failed) {
...
void StringUtilities::Split(const std::string& input, const std::string& delimiter, std::vector<std::string>* tokens)
{
...
void StringUtilities::Split(const std::wstring& input, const std::wstring& delimiter, std::vector<std::wstring>* tokens)
{
...
// }
//
// void StringUtilities::NormalizeUnicodeString(NORM_FORM normalization_form, std::wstring* input)
// {
//     if (FALSE == ::IsNormalizedString(normalization_form, input->c_str(), -1)) {
//         int required = ::NormalizeString(normalization_form, input->c_str(), -1, NULL, 0);
//         std::vector<wchar_t> buffer(required);
//         ::NormalizeString(normalization_form, input->c_str(), -1, &buffer[0], static_cast<int>(buffer.size()));
//         *input = &buffer[0];
//     }
// }
