...
bool Element::IsObscured(LocationInfo* click_location, long* obscuring_element_index, std::string* obscuring_element_description)
{
    ...
    *click_location = this->CalculateClickPoint(element_location, false);
    ...
    hr = elements_doc->elementsFromPoint(static_cast<float>(x), static_cast<float>(y), &elements_hit);
    if (SUCCEEDED(hr) && elements_hit != NULL) {
        ...
        for (long index = 0; index < element_count; ++index) {
            ...
            Element list_element_wrapper(element_in_list, this->containing_window_handle_);
            status_code = list_element_wrapper.IsDisplayed(false, &is_list_element_displayed);
            if (is_list_element_displayed) {
                ...
                bool found_element_not_in_tree = is_child != VARIANT_TRUE && is_ancestor != VARIANT_TRUE;
...
int Element::GetClickLocation(const ElementScrollBehavior scroll_behavior, LocationInfo* element_location, LocationInfo* click_location)
{
...
int Element::GetAttributeValue(const std::string& attribute_name, VARIANT* attribute_value)
{
...
int Element::GetPropertyValue(const std::string& property_name, VARIANT* property_value)
{
    ...
    HRESULT hr = this->element_->GetIDsOfNames(IID_NULL, &property_name_pointer, 1, LOCALE_USER_DEFAULT, &dispid_property);
    if (FAILED(hr)) {
        LOGHR(WARN, hr) << "Unable to get dispatch ID (dispid) for property " << property_name;
    ...
    hr = this->element_->Invoke(dispid_property, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, &no_args_dispatch_parameters, property_value, NULL, NULL);
    if (FAILED(hr)) {
        LOGHR(WARN, hr) << "Unable to get result for property " << property_name;
        property_value->vt = VT_EMPTY;
        return WD_SUCCESS;
    }
...
int Element::GetCssPropertyValue(const std::string& property_name, std::string* property_value)
{
...
int Element::GetLocationOnceScrolledIntoView(const ElementScrollBehavior scroll, LocationInfo* location, std::vector<LocationInfo>* frame_locations)
{
    ...
    if (result != WD_SUCCESS || !this->IsLocationInViewPort(click_location, document_contains_frames)
        || this->IsHiddenByOverflow(element_location, click_location) || !this->IsLocationVisibleInFrames(click_location, *frame_locations)) {
    ...
    LOG(DEBUG) << "(x, y, w, h): " << element_location.x << ", " << element_location.y << ", " << element_location.width << ", " << element_location.height;
...
bool Element::IsHiddenByOverflow(const LocationInfo element_location, const LocationInfo click_location)
{
...
bool Element::IsLocationVisibleInFrames(const LocationInfo location, const std::vector<LocationInfo> frame_locations)
{
    ...
    for (; iterator != frame_locations.end(); ++iterator) {
        if (location.x < iterator->x || location.y < iterator->y || location.x > iterator->x + iterator->width || location.y > iterator->y + iterator->height) {
            return false;
        }
    }
    return true;
        ...
        for (long i = 0; i < frame_count; ++i) {
            ...
            if (frame_doc.IsEqualObject(owner_doc)) {
                ...
                if (frame_base) {
                    ...
                    bool doc_dimensions_success = DocumentHost::GetDocumentDimensions(frame_doc, &frame_doc_info);
                    ...
                    status_code = element_wrapper.GetLocation(&frame_location, frame_locations);
...
int Element::GetDocumentFromWindow(IHTMLWindow2* parent_window, IHTMLDocument2** parent_doc)
{
