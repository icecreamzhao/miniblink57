...
HWND WindowUtilities::GetChildWindow(HWND parent_window_handle, std::wstring window_class)
{
    std::vector<wchar_t> class_name_buffer(window_class.size() + 1);
    HWND hwndtmp = GetWindow(parent_window_handle, GW_CHILD);
    while (hwndtmp != NULL) {
        ::GetClassName(hwndtmp, &class_name_buffer[0], static_cast<int>(class_name_buffer.size()));
...
void WindowUtilities::GetProcessesByName(const std::wstring& process_name, std::vector<DWORD>* process_ids)
{
    int max_process_id_count = 1024;
    std::vector<DWORD> all_process_ids(max_process_id_count);
    DWORD bytes_needed = 0;
    BOOL processes_enumerated = ::EnumProcesses(&all_process_ids[0], static_cast<DWORD>(all_process_ids.size()) * sizeof(DWORD), &bytes_needed);
    ...
    while (process_id_count == max_process_id_count) {
        ...
        bytes_needed = 0;
        processes_enumerated = ::EnumProcesses(&all_process_ids[0], static_cast<DWORD>(all_process_ids.size()) * sizeof(DWORD), &bytes_needed);
    ...
    for (size_t index = 0; index < process_id_count; ++index) {
        ...
        if (process_handle != NULL) {
            ...
            DWORD chars_copied = ::GetProcessImageFileName(process_handle, &image_path_buffer[0], MAX_PATH);
            std::wstring image_path(&image_path_buffer[0]);
            std::transform(image_path.begin(), image_path.end(), image_path.begin(), ::towlower);
