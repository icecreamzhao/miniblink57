...
Alert::Alert(std::shared_ptr<DocumentHost> browser, HWND handle)
{
    ...
    if (strcmp(&window_class[0], SECURITY_DIALOG_WINDOW_CLASS) == 0) {
        ...
        ::EnumChildWindows(this->alert_handle_, &Alert::FindTextBoxes, reinterpret_cast<LPARAM>(&text_boxes));
...
int Alert::SendKeysInternal(const std::string& keys, TextBoxFindInfo* text_box_find_info)
{
    ...
    while ((text_box_find_info->textbox_handle == NULL) && --max_wait) {
        ::EnumChildWindows(this->alert_handle_, &Alert::FindTextBox, reinterpret_cast<LPARAM>(text_box_find_info));
    ...
    if (text_box_find_info->textbox_handle == NULL) {
        ...
        ::SendMessage(text_box_find_info->textbox_handle, WM_SETTEXT, NULL, reinterpret_cast<LPARAM>(text.c_str()));
    }
    return WD_SUCCESS;
}
...
std::string Alert::GetStandardDialogText()
{
    ...
    while ((info.label_handle == NULL) && --max_wait) {
        ::EnumChildWindows(this->alert_handle_, &Alert::FindTextLabel, reinterpret_cast<LPARAM>(&info));
    ...
    ::EnumChildWindows(this->alert_handle_, &Alert::FindTextBox, reinterpret_cast<LPARAM>(&textbox_find_info));
...
std::string Alert::GetDirectUIDialogText()
{
    ...
    HRESULT hr = ::AccessibleObjectFromWindow(direct_ui_child_handle, OBJID_WINDOW, IID_IAccessible, reinterpret_cast<void**>(&window_object));
    if (FAILED(hr)) {
    ...
    // the window object.
    CComPtr<IAccessible> pane_object = this->GetChildWithRole(window_object, ROLE_SYSTEM_PANE, 0);
    if (!pane_object) {
    ...
    CComPtr<IAccessible> message_text_object = this->GetChildWithRole(pane_object, ROLE_SYSTEM_STATICTEXT, child_index);
...
HWND Alert::GetDirectUIChild()
{
    ...
    ::EnumChildWindows(this->alert_handle_, &Alert::FindDirectUIChild, reinterpret_cast<LPARAM>(&direct_ui_child));
...
int Alert::ClickAlertButton(DialogButtonInfo button_info)
{
    ...
    if (this->is_standard_control_alert_) {
        ::SendMessage(this->alert_handle_, WM_COMMAND, button_info.button_control_id, NULL);
    } else {
        if (button_info.use_accessibility) {
            ...
            ::SendMessage(::GetParent(button_info.button_handle), WM_COMMAND, MAKEWPARAM(0, BN_CLICKED), reinterpret_cast<LPARAM>(button_info.button_handle));
        }
    }
    ...
    LOG(DEBUG) << "IsWindow() for alert handle 0x" << this->alert_handle_ << ": " << is_alert_handle_valid ? "true" : "false";
    return WD_SUCCESS;
}
...
Alert::DialogButtonInfo Alert::GetDialogButton(BUTTON_TYPE button_type)
{
    ...
    if (this->is_standard_alert_ || !this->is_security_alert_) {
        ...
        while ((button_find_info.button_handle == NULL) && --max_wait) {
            ::EnumChildWindows(this->alert_handle_, &Alert::FindDialogButton, reinterpret_cast<LPARAM>(&button_find_info));
...
int Alert::ClickAlertButtonUsingAccessibility(const std::string& automation_id)
{
    ...
    HRESULT hr = ::CoCreateInstance(CLSID_CUIAutomation, NULL, CLSCTX_INPROC_SERVER, IID_IUIAutomation, reinterpret_cast<void**>(&ui_automation));
    ...
    hr = ui_automation->ElementFromHandle(this->alert_handle_, &parent_window);
    if (FAILED(hr)) {
    ...
    hr = ui_automation->CreatePropertyCondition(UIA_AutomationIdPropertyId, button_automation_id, &button_condition);
    if (FAILED(hr)) {
    ...
    hr = parent_window->FindFirst(TreeScope::TreeScope_Children, button_condition, &button);
    if (FAILED(hr)) {
    ...
    hr = button->GetCurrentPatternAs(UIA_InvokePatternId, IID_PPV_ARGS(&button_invoke_pattern));
    if (FAILED(hr)) {
...
BOOL CALLBACK Alert::FindTextLabel(HWND hwnd, LPARAM arg)
{
    ...
    if (text_length > 0) {
        if (find_info->excluded_control_id == 0 || control_id != find_info->excluded_control_id) {
