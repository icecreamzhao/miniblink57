...
int ElementRepository::GetManagedElement(const std::string& element_id, ElementHandle* element_wrapper) const
{
...
bool ElementRepository::AddManagedElement(BrowserHandle current_browser, IHTMLElement* element, ElementHandle* element_wrapper)
{
    ...
    if (!element_already_managed) {
        ...
        ElementHandle new_wrapper(new Element(element, containing_window_handle));
...
bool ElementRepository::IsElementManaged(IHTMLElement* element, ElementHandle* element_wrapper)
{
...
void ElementRepository::ClearCache()
{
    ...
    LOG(DEBUG) << "Refreshing managed element cache. Found " << bad_elements.size() << " to remove from cache.";
