...
bool DocumentHost::IsCrossZoneUrl(std::string url)
{
    ...
    HRESULT hr = ::CreateUri(target_url.c_str(), Uri_CREATE_IE_SETTINGS, 0, &parsed_url);
    if (FAILED(hr)) {
...
bool DocumentHost::SetFocusToBrowser()
{
    ...
    if (foreground_window != top_level_window_handle) {
        LOG(TRACE) << "Top-level IE window is " << top_level_window_handle << " foreground window is " << foreground_window;
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = ui_automation->ElementFromHandle(top_level_window_handle, &parent_window);
            if (SUCCEEDED(hr)) {
                CComPtr<IUIAutomationWindowPattern> window_pattern;
                hr = parent_window->GetCurrentPatternAs(UIA_WindowPatternId, IID_PPV_ARGS(&window_pattern));
                if (SUCCEEDED(hr) && window_pattern != nullptr) {
                    ...
                    if (visual_state == WindowVisualState::WindowVisualState_Maximized || visual_state == WindowVisualState::WindowVisualState_Normal) {
    ...
    if (foreground_window != top_level_window_handle) {
        ...
        DWORD process_id = 0;
        DWORD thread_id = ::GetWindowThreadProcessId(top_level_window_handle, &process_id);
        ...
        if (current_thread_id != thread_id) {
            ...
            hook.Initialize(hook_settings);
            ::SendMessage(content_window_handle, WD_ALLOW_SET_FOREGROUND, NULL, NULL);
            hook.Dispose();
        }
