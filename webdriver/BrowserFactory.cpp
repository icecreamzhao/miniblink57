...
#define PROTECTED_MODE_SETTING_ERROR_MESSAGE                                                                                                                   \
...
#define CREATEPROCESS_REGISTRY_ERROR_MESSAGE                                                                                                                   \
    "Unable to use CreateProcess() API. To use CreateProcess() with Internet Explorer 8 or higher, the value of registry setting in "                          \
...
DWORD BrowserFactory::LaunchBrowserProcess(std::string* error_message)
{
    ...
    LOG(DEBUG) << "Ignoring Protected Mode Settings: " << this->ignore_protected_mode_settings_;
    ...
    LOG(DEBUG) << "Has Valid Protected Mode Settings: " << has_valid_protected_mode_settings;
...
void BrowserFactory::LaunchBrowserUsingIELaunchURL(PROCESS_INFORMATION* proc_info, std::string* error_message)
{
    LOG(TRACE) << "Entering BrowserFactory::LaunchBrowserUsingIELaunchURL";
    LOG(DEBUG) << "Starting IE using the IELaunchURL API";
    HRESULT launch_result = ::IELaunchURL(this->initial_browser_url_.c_str(), proc_info, NULL);
    if (FAILED(launch_result)) {
        ...
        *error_message = StringUtilities::Format(
            IELAUNCHURL_ERROR_MESSAGE, launch_result, StringUtilities::ToString(hresult_msg).c_str(), this->initial_browser_url().c_str());
    }
}
...
bool BrowserFactory::IsCreateProcessApiAvailable()
{
    ...
    if (this->ie_major_version_ >= 8) {
        ...
        if (RegistryUtilities::GetRegistryValue(HKEY_CURRENT_USER, IE_TABPROCGROWTH_REGISTRY_KEY, L"TabProcGrowth", &tab_proc_growth)) {
...
void BrowserFactory::LaunchBrowserUsingCreateProcess(PROCESS_INFORMATION* proc_info, std::string* error_message)
{
    ...
    BOOL create_process_result = ::CreateProcess(NULL, command_line, NULL, NULL, FALSE, 0, NULL, NULL, &start_info, proc_info);
    if (!create_process_result) {
        *error_message = StringUtilities::Format(CREATEPROCESS_ERROR_MESSAGE, StringUtilities::ToString(command_line));
    }
    delete[] command_line;
}
...
bool BrowserFactory::CreateUniqueTempDir(std::wstring& temp_dir)
{
    ...
    for (int i = 0; i < 10; i++) {
        std::wstring output = temp_path + L"IEDriver-" + StringUtilities::CreateGuid();
...
void BrowserFactory::LaunchEdgeInIEMode(PROCESS_INFORMATION* proc_info, std::string* error_message)
{
    ...
    LOG(TRACE) << "Edge in IE Mode starting command line is: '" << LOGWSTRING(executable_and_url) << "'.";
...
bool BrowserFactory::AttachToBrowser(ProcessWindowInfo* process_window_info, std::string* error_message)
{
    ...
    if (!this->force_shell_windows_api_) {
        ...
        attached = this->AttachToBrowserUsingActiveAccessibility(process_window_info, error_message);
        if (!attached) {
    ...
    if (!attached) {
        ...
        attached = this->AttachToBrowserUsingShellWindows(process_window_info, error_message);
    }
    ...
    if (attached) {
        ...
        if (zoom_level != 100) {
            std::string zoom_level_error = StringUtilities::Format(ZOOM_SETTING_ERROR_MESSAGE, zoom_level);
...
bool BrowserFactory::AttachToBrowserUsingActiveAccessibility(ProcessWindowInfo* process_window_info, std::string* error_message)
{
    ...
    while (process_window_info->hwndBrowser == NULL) {
        ...
        if (!this->edge_ie_mode_) {
            ...
            if (this->ignore_process_match_) {
                ...
                ::EnumWindows(&BrowserFactory::FindEdgeWindowIgnoringProcessMatch, reinterpret_cast<LPARAM>(process_window_info));
            } else {
                LOG(TRACE) << "Finding windonw handle for IE Mode on Edge";
                ::EnumWindows(&BrowserFactory::FindEdgeWindow, reinterpret_cast<LPARAM>(process_window_info));
            }
        }
    ...
    if (process_window_info->hwndBrowser == NULL) {
        *error_message = StringUtilities::Format(ATTACH_TIMEOUT_ERROR_MESSAGE, process_window_info->dwProcessId, this->browser_attach_timeout_);
        return false;
    } else {
        LOG(DEBUG) << "Found window handle " << process_window_info->hwndBrowser << " for window with class 'Internet Explorer_Server' belonging"
...
bool BrowserFactory::AttachToBrowserUsingShellWindows(ProcessWindowInfo* process_window_info, std::string* error_message)
{
    ...
    while (process_window_info->hwndBrowser == NULL) {
        ...
        enumerator->Reset();
        for (CComVariant shell_window_variant; enumerator->Next(1, &shell_window_variant, NULL) == S_OK; shell_window_variant.Clear()) {
            ...
            hr = IUnknown_QueryService(shell_window_variant.pdispVal, SID_STopLevelBrowser, IID_PPV_ARGS(&shell_browser));
            if (shell_browser) {
                HWND hwnd;
                hr = shell_browser->GetWindow(&hwnd);
                if (SUCCEEDED(hr)) {
                    ::EnumChildWindows(hwnd, &BrowserFactory::FindChildWindowForProcess, reinterpret_cast<LPARAM>(process_window_info));
                    if (process_window_info->hwndBrowser != NULL) {
                        LOG(DEBUG) << "Found window handle " << process_window_info->hwndBrowser << " for window with class 'Internet Explorer_Server'"
                                   << " belonging to process with id " << process_window_info->dwProcessId;
                        CComPtr<IWebBrowser2> browser;
        ...
        if (process_window_info->hwndBrowser == NULL || process_window_info->pBrowser == NULL) {
            ::Sleep(250);
        }
    }
...
IWebBrowser2* BrowserFactory::CreateBrowser(bool is_protected_mode)
{
    ...
    if (is_protected_mode) {
        hr = ::CoCreateInstance(CLSID_InternetExplorer, NULL, context, IID_IWebBrowser2, reinterpret_cast<void**>(&browser));
    } else {
        hr = ::CoCreateInstance(CLSID_InternetExplorerMedium, NULL, context, IID_IWebBrowser2, reinterpret_cast<void**>(&browser));
    }
    ...
    while (FAILED(hr) && HRESULT_CODE(hr) == ERROR_SHUTDOWN_IS_SCHEDULED && clock() < timeout) {
        ::Sleep(500);
        hr = ::CoCreateInstance(CLSID_InternetExplorer, NULL, context, IID_IWebBrowser2, reinterpret_cast<void**>(&browser));
    }
    if (FAILED(hr) && HRESULT_CODE(hr) != ERROR_SHUTDOWN_IS_SCHEDULED) {
...
bool BrowserFactory::CreateLowIntegrityLevelToken(HANDLE* process_token_handle, HANDLE* mic_token_handle, PSID* sid)
{
    ...
    result = ::OpenProcessToken(process_handle, MAXIMUM_ALLOWED, process_token_handle);

    if (result) {
        result = ::DuplicateTokenEx(*process_token_handle, MAXIMUM_ALLOWED, NULL, SecurityImpersonation, TokenPrimary, mic_token_handle);
        if (!result) {
    ...
    if (result) {
        result = ::SetTokenInformation(*mic_token_handle, TokenIntegrityLevel, &tml, sizeof(tml) + ::GetLengthSid(*sid));
        if (!result) {
...
void BrowserFactory::InvokeClearCacheUtility(bool use_low_integrity_level)
{
    ...
    if (!use_low_integrity_level || this->CreateLowIntegrityLevelToken(&process_token, &mic_token, &sid)) {
        if (0 != system_path_size && system_path_size <= static_cast<int>(system_path_buffer.size())) {
            if (::PathCombine(&rundll_exe_path_buffer[0], &system_path_buffer[0], RUNDLL_EXE_NAME)
                && ::PathCombine(&inetcpl_path_buffer[0], &system_path_buffer[0], INTERNET_CONTROL_PANEL_APPLET_NAME)) {
                ...
                args = StringUtilities::Format(CLEAR_CACHE_COMMAND_LINE_ARGS, &inetcpl_path_buffer[0], CLEAR_CACHE_OPTIONS);
            } else {
        ...
        if (can_create_process) {
            ...
            if (use_low_integrity_level) {
                is_process_created
                    = CreateProcessAsUser(mic_token, &rundll_exe_path_buffer[0], &args_buffer[0], NULL, NULL, FALSE, 0, NULL, NULL, &start_info, &process_info);
            } else {
                is_process_created = CreateProcess(&rundll_exe_path_buffer[0], &args_buffer[0], NULL, NULL, FALSE, 0, NULL, NULL, &start_info, &process_info);
            }
...
BOOL CALLBACK BrowserFactory::FindBrowserWindow(HWND hwnd, LPARAM arg)
{
    ...
    if (strcmp(IE_FRAME_WINDOW_CLASS, name) != 0 && strcmp(SHELL_DOCOBJECT_VIEW_WINDOW_CLASS, name) != 0 && strcmp(ANDIE_FRAME_WINDOW_CLASS, name) != 0) {
        return TRUE;
    }
...
BOOL CALLBACK BrowserFactory::FindEdgeWindow(HWND hwnd, LPARAM arg)
{
    ...
    if (strcmp(ANDIE_FRAME_WINDOW_CLASS, name) != 0)
        return TRUE;
...
BOOL CALLBACK BrowserFactory::FindDialogWindowForProcess(HWND hwnd, LPARAM arg)
{
    ...
    if (strcmp(ALERT_WINDOW_CLASS, name) != 0 && strcmp(HTML_DIALOG_WINDOW_CLASS, name) != 0 && strcmp(SECURITY_DIALOG_WINDOW_CLASS, name) != 0) {
        return TRUE;
    } else {
        ...
        if ((window_long_style & WS_DISABLED) != 0 || (window_long_style & WS_VISIBLE) == 0 || (window_long_style & WS_CAPTION) == 0) {
            return TRUE;
        }
        DWORD process_id = NULL;
        ::GetWindowThreadProcessId(hwnd, &process_id);
...
void BrowserFactory::GetIEExecutableLocation()
{
    ...
    if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE, IE_REDIRECT, L"{1FD49718-1D00-4B19-AF5F-070AF6D5D54C}", &redirection)) {
    ...
    if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE, IE_CLSID_REGISTRY_KEY, L"", &class_id)) {
        ...
        if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE, location_key, L"", true, &executable_location)) {
...
void BrowserFactory::GetEdgeExecutableLocation()
{
    ...
    if (RegistryUtilities::GetRegistryValue(HKEY_LOCAL_MACHINE, EDGE_REGISTRY_KEY, L"", true, &edge_executable_location)) {
        // If the executable location in the registry has an environment
        // variable in it, expand the environment variable to an absolute
        // path.
...
void BrowserFactory::GetIEVersion()
{
    ...
    if (ie_version.size() == 0) {
        ...
        LOG(WARN) << "Couldn't find IE version for executable " << LOGWSTRING(this->ie_executable_location_) << ", falling back to " << this->ie_major_version_;
        return;
    }
...
bool BrowserFactory::ProtectedModeSettingsAreValid()
{
    ...
    LOG(DEBUG) << "Detected IE version: " << this->ie_major_version_ << ", Windows version supports Protected Mode: " << IsWindowsVistaOrGreater() ? "true"
                                                                                                                                                   : "false";
    ...
    if (this->ie_major_version_ >= 7 && IsWindowsVistaOrGreater()) {
        ...
        if (ERROR_SUCCESS == ::RegOpenKeyEx(HKEY_CURRENT_USER, IE_SECURITY_ZONES_REGISTRY_KEY, 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &key_handle)) {
            ...
            LOG(WARN) << "RegOpenKeyEx for zone settings registry key " << LOGWSTRING(registry_key_string) << " in HKEY_CURRENT_USER failed";
        }
    }
...
int BrowserFactory::GetZoneProtectedModeSetting(const HKEY key_handle, const std::wstring& zone_subkey_name)
{
    ...
    if (ERROR_SUCCESS == ::RegOpenKeyEx(key_handle, zone_subkey_name.c_str(), 0, KEY_QUERY_VALUE, &subkey_handle)) {
        ...
        if (ERROR_SUCCESS
            == ::RegQueryValueEx(subkey_handle, IE_PROTECTED_MODE_SETTING_VALUE_NAME, NULL, NULL, reinterpret_cast<LPBYTE>(&value), &value_length)) {
            LOG(DEBUG) << "Found Protected Mode setting value of " << value << " for zone " << LOGWSTRING(zone_subkey_name);
            protected_mode_value = value;
        } else {
            LOG(DEBUG) << "RegQueryValueEx failed for getting Protected Mode setting for a zone: " << LOGWSTRING(zone_subkey_name);
        }
        ...
        if (zone_subkey_name == ZONE_INTERNET || zone_subkey_name == ZONE_RESTRICTED_SITES
            || (zone_subkey_name == ZONE_LOCAL_INTRANET && this->ie_major_version_ == 7)) {
            protected_mode_value = 0;
        }
        LOG(DEBUG) << "Protected Mode zone setting value does not exist for zone " << LOGWSTRING(zone_subkey_name) << ". Using default value of "
                   << protected_mode_value;
    }
    return protected_mode_value;
}

bool BrowserFactory::IsWindowsVersionOrGreater(unsigned short major_version, unsigned short minor_version, unsigned short service_pack)
{
