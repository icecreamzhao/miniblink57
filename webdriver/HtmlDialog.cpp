...
HtmlDialog::HtmlDialog(IHTMLWindow2* window, HWND hwnd, HWND session_handle)
...
void HtmlDialog::GetDocument(const bool force_top_level_document, IHTMLDocument2** doc)
{
...
int HtmlDialog::NavigateToUrl(const std::string& url, std::string* error_message)
{
...
BOOL CALLBACK HtmlDialog::FindChildDialogWindow(HWND hwnd, LPARAM arg)
{
    ...
    if (strcmp(ALERT_WINDOW_CLASS, &window_class_name[0]) != 0 && strcmp(HTML_DIALOG_WINDOW_CLASS, &window_class_name[0]) != 0) {
        return TRUE;
    } else {
        ...
        if ((::GetWindowLong(hwnd, GWL_STYLE) & WS_DISABLED) != 0 || (::GetWindowLong(hwnd, GWL_STYLE) & WS_VISIBLE) == 0) {
            return TRUE;
        }
    }
