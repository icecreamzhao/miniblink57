...
LRESULT IECommandExecutor::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnSetCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (this->current_command_.command_type() == CommandType::NoCommand || requested_command.command_type() == CommandType::Quit) {
...
LRESULT IECommandExecutor::OnExecCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnGetResponseLength(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnWait(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    LOG(DEBUG) << "Wait complete. Setting serialized response to deferred value " << deferred_response;
    this->serialized_response_ = deferred_response;
    this->is_waiting_ = false;
    return 0;
}

LRESULT IECommandExecutor::OnBeforeNewWindow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnAfterNewWindow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    LOG(TRACE) << "Entering IECommandExecutor::OnAfterNewWindow";
    if (wParam > 0) {
        LOG(DEBUG) << "Creating thread and reposting message.";
        this->CreateDelayPostMessageThread(static_cast<DWORD>(wParam), this->m_hWnd, WD_AFTER_NEW_WINDOW);
        if (lParam > 0) {
            ...
            std::vector<HWND>* current_window_handles = reinterpret_cast<std::vector<HWND>*>(lParam);
            std::unordered_set<HWND> current_window_set(current_window_handles->begin(), current_window_handles->end());
            ...
            while (diff.size() == 0 && clock() < end) {
                ...
                ::EnumWindows(&BrowserFactory::FindEdgeBrowserHandles, reinterpret_cast<LPARAM>(&edge_window_handles));

                std::vector<HWND> new_ie_window_handles;
                for (auto& edge_window_handle : edge_window_handles) {
                    std::vector<HWND> child_window_handles;
                    ::EnumChildWindows(edge_window_handle, &BrowserFactory::FindIEBrowserHandles, reinterpret_cast<LPARAM>(&child_window_handles));
            ...
            if (diff.size() == 0) {
                ...
                for (int i = diff.size() - 1; i >= 0; i--) {
                    ...
                    if (process_id) {
                        ...
                        if (attach_flag) {
                            BrowserHandle new_window_wrapper(new Browser(info.pBrowser, NULL, this->m_hWnd, this->is_edge_chromium_));
...
LRESULT IECommandExecutor::OnBrowserNewWindow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    std::string new_browser_id = this->OpenNewBrowsingContext(WINDOW_WINDOW_TYPE, target_url);
    ...
    HRESULT hr = ::CoMarshalInterThreadInterfaceInStream(IID_IWebBrowser2, browser, &(info->browser_stream));
    if (FAILED(hr)) {
...
LRESULT IECommandExecutor::OnBrowserCloseWait(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (found_iterator != this->managed_browsers_.end()) {
        HWND alert_handle;
        bool is_alert_active = this->IsAlertActive(found_iterator->second, &alert_handle);
        if (is_alert_active) {
        ...
        if (!is_alert_active) {
            ...
            ::PostMessage(this->m_hWnd, WD_BROWSER_CLOSE_WAIT, NULL, reinterpret_cast<LPARAM>(message_payload));
            return 0;
        }
    } else {
...
LRESULT IECommandExecutor::OnSessionQuitWait(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (this->managed_browsers_.size() > 0) {
        ...
        for (; it != managed_browsers_.end(); ++it) {
            ...
            HWND alert_handle;
            bool is_alert_active = this->IsAlertActive(it->second, &alert_handle);
            if (is_alert_active) {
                // If there's an alert window active, the browser's Quit event does
                // not fire until any alerts are handled. Note that OnBeforeUnload
                // alerts must be handled here; the driver contains the ability to
        ...
        ::PostMessage(this->m_hWnd, WD_SESSION_QUIT_WAIT, NULL, NULL);
    } else {
...
LRESULT IECommandExecutor::OnAddChromiumWindowHandle(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    if (wParam != NULL) {
...
LRESULT IECommandExecutor::OnBeforeBrowserReattach(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnBrowserReattach(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (new_process_ids.size() == 0) {
        ...
        this->PostBrowserReattachMessage(current_process_id, browser_id, known_process_ids);
    }
    ...
    if (!this->factory_->IsBrowserProcessInitialized(new_process_id)) {
        ...
        // repost the message
        LOG(DEBUG) << "Browser process " << new_process_id << " not initialized, rescheduling reattach";
        this->PostBrowserReattachMessage(current_process_id, browser_id, known_process_ids);
        return 0;
    }
...
LRESULT IECommandExecutor::OnIsSessionValid(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnNewHtmlDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandles)
{
...
LRESULT IECommandExecutor::OnQuit(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (this->edge_temp_dir_ != L"") {
        for (int i = 0; i < 100; i++) {
            ...
            if (BrowserFactory::DeleteDirectory(edge_temp_dir_)) {
                ...
                LOG(TRACE) << "Failed to delete Edge temporary user data directory " << LOGWSTRING(edge_temp_dir_) << ", retrying " << i + 1 << "...";
            } else {
...
LRESULT IECommandExecutor::OnGetQuitStatus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    return this->is_quitting_ && this->managed_browsers_.size() > 0 ? 1 : 0;
}

LRESULT IECommandExecutor::OnScriptWait(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
        ...
        if (this->async_script_timeout_ >= 0 && this->wait_timeout_ < clock()) {
            ...
            if (this->is_waiting_) {
                ...
                HANDLE thread_handle
                    = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &IECommandExecutor::ScriptWaitThreadProc, (void*)this->m_hWnd, 0, &thread_id));
                ...
                ::SendMessage(browser->script_executor_handle(), WD_ASYNC_SCRIPT_DETACH_LISTENTER, NULL, NULL);
                int status_code = static_cast<int>(
                    ::SendMessage(browser->script_executor_handle(), WD_ASYNC_SCRIPT_GET_RESULT, NULL, reinterpret_cast<LPARAM>(&script_result)));
                if (status_code != WD_SUCCESS) {
...
LRESULT IECommandExecutor::OnRefreshManagedElements(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    this->managed_elements_->ClearCache();
    return 0;
}

LRESULT IECommandExecutor::OnHandleUnexpectedAlerts(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT IECommandExecutor::OnTransferManagedElement(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    ::CoGetInterfaceAndReleaseStream(element_stream, IID_IHTMLElement, reinterpret_cast<void**>(&element));
    delete info;
    ElementHandle element_handle;
    this->managed_elements_->AddManagedElement(browser_handle, element, &element_handle);
    ...
    ::PostMessage(browser_handle->script_executor_handle(), WD_ASYNC_SCRIPT_NOTIFY_ELEMENT_TRANSFERRED, NULL, reinterpret_cast<LPARAM>(return_info));
    return WD_SUCCESS;
}

LRESULT IECommandExecutor::OnScheduleRemoveManagedElement(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
unsigned int WINAPI IECommandExecutor::WaitThreadProc(LPVOID lpParameter)
{
    ...
    strcpy_s(message_payload, deferred_response.size() + 1, deferred_response.c_str());

    ::Sleep(WAIT_TIME_IN_MILLISECONDS);
    ::PostMessage(window_handle, WD_WAIT, static_cast<WPARAM>(deferred_response.size()), reinterpret_cast<LPARAM>(message_payload));
    ...
    if (!this->command_handlers_->IsValidCommand(this->current_command_.command_type())) {
            ...
            if (status_code == WD_SUCCESS) {
                ...
                if (alert_is_active) {
                    if (this->IsCommandValidWithAlertPresent()) {
                        ...
                        std::string alert_text;
                        bool is_notify_unexpected_alert = this->HandleUnexpectedAlert(browser, alert_handle, is_quit_command, &alert_text);
                        if (!is_quit_command) {
        ...
        if (status_code == WD_SUCCESS) {
            if (browser->is_closing() && !this->is_quitting_) {
                ...
                strcpy_s(message_payload, browser->browser_id().size() + 1, browser->browser_id().c_str());
...
{
    std::string command_type = this->current_command_.command_type();
    if (command_type == webdriver::CommandType::GetAlertText || command_type == webdriver::CommandType::SendKeysToAlert
        || command_type == webdriver::CommandType::AcceptAlert || command_type == webdriver::CommandType::DismissAlert
        || command_type == webdriver::CommandType::SetAlertCredentials || command_type == webdriver::CommandType::GetTimeouts
        || command_type == webdriver::CommandType::SetTimeouts || command_type == webdriver::CommandType::Screenshot
        || command_type == webdriver::CommandType::ElementScreenshot || command_type == webdriver::CommandType::GetCurrentWindowHandle
        || command_type == webdriver::CommandType::GetWindowHandles || command_type == webdriver::CommandType::SwitchToWindow) {
        return true;
    }
    return false;
}
...
void IECommandExecutor::CreateWaitThread(const std::string& deferred_response, const bool is_deferred_command_execution)
{
    // If we are still waiting, we need to wait a bit then post a message to
    // ourselves to run the wait again. However, we can't wait using Sleep()
    // on this thread. This call happens in a message loop, and we would be
    ...
    strcpy_s(thread_context->deferred_response, deferred_response.size() + 1, deferred_response.c_str());

    unsigned int thread_id = 0;
    HANDLE thread_handle
        = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &IECommandExecutor::WaitThreadProc, reinterpret_cast<void*>(thread_context), 0, &thread_id));
    if (thread_handle != NULL) {
        ::CloseHandle(thread_handle);
    } else {
        LOGERR(DEBUG) << "Unable to create waiter thread";
...
void IECommandExecutor::CreateDelayPostMessageThread(const DWORD delay_time, const HWND window_handle, const UINT message_to_post)
{
    ...
    unsigned int thread_id = 0;
    HANDLE thread_handle
        = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, &IECommandExecutor::DelayPostMessageThreadProc, reinterpret_cast<void*>(context), 0, &thread_id));
    if (thread_handle != NULL) {
        ::CloseHandle(thread_handle);
    } else {
        LOGERR(DEBUG) << "Unable to create waiter thread";
...
bool IECommandExecutor::HandleUnexpectedAlert(BrowserHandle browser, HWND alert_handle, bool force_use_dismiss, std::string* alert_text)
{
    ...
    *alert_text = dialog.GetText();
    ...
    if (this->unexpected_alert_behavior_ == ACCEPT_UNEXPECTED_ALERTS || this->unexpected_alert_behavior_ == ACCEPT_AND_NOTIFY_UNEXPECTED_ALERTS) {
        LOG(DEBUG) << "Automatically accepting the alert";
        dialog.Accept();
    } else if (this->unexpected_alert_behavior_.size() == 0 || this->unexpected_alert_behavior_ == DISMISS_UNEXPECTED_ALERTS
        || this->unexpected_alert_behavior_ == DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS || force_use_dismiss) {
    ...
    bool is_notify_unexpected_alert = this->unexpected_alert_behavior_.size() == 0 || this->unexpected_alert_behavior_ == IGNORE_UNEXPECTED_ALERTS
        || this->unexpected_alert_behavior_ == DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS || this->unexpected_alert_behavior_ == ACCEPT_AND_NOTIFY_UNEXPECTED_ALERTS;
    is_notify_unexpected_alert = is_notify_unexpected_alert && dialog.is_standard_alert();
    return is_notify_unexpected_alert;
}

void IECommandExecutor::PostBrowserReattachMessage(const DWORD current_process_id, const std::string& browser_id, const std::vector<DWORD>& known_process_ids)
{
    ...
    ::PostMessage(this->m_hWnd, WD_BROWSER_REATTACH, NULL, reinterpret_cast<LPARAM>(repost_info));
}

void IECommandExecutor::GetNewBrowserProcessIds(std::vector<DWORD>* known_process_ids, std::vector<DWORD>* new_process_ids)
{
    ...
    std::vector<DWORD>::iterator end_iterator = std::set_difference(
        all_ie_process_ids.begin(), all_ie_process_ids.end(), known_process_ids->begin(), known_process_ids->end(), temp_new_process_ids.begin());
...
std::string IECommandExecutor::OpenNewBrowsingContext(const std::string& window_type, const std::string& url)
{
...
std::string IECommandExecutor::OpenNewBrowserWindow(const std::wstring& url)
{
    ...
    BrowserHandle new_window_wrapper(new Browser(browser, NULL, this->m_hWnd, this->is_edge_chromium_));
...
std::string IECommandExecutor::OpenNewBrowserTab(const std::wstring& url)
{
    ...
    if (this->is_edge_chromium_) {
        ...
        std::string window_class = WindowUtilities::GetWindowClass(top_level_handle);
        if (window_class != "Chrome_WidgetWin_1") {
            std::vector<HWND> edge_handles;
            ::EnumWindows(&BrowserFactory::FindEdgeBrowserHandles, reinterpret_cast<LPARAM>(&edge_handles));
            for (auto& edge_handle : edge_handles) {
                std::vector<HWND> ie_mode_handles;
                ::EnumChildWindows(edge_handle, &BrowserFactory::FindIEBrowserHandles, reinterpret_cast<LPARAM>(&ie_mode_handles));
    ...
    ::EnumChildWindows(top_level_handle, &BrowserFactory::FindIEBrowserHandles, reinterpret_cast<LPARAM>(&original_handles));
    ...
    browser_wrapper->browser()->Navigate2(&url_variant, &flags, NULL, NULL, NULL);
    ::Sleep(500);

    HWND new_tab_window = NULL;
    std::vector<HWND> new_handles;
    ::EnumChildWindows(top_level_handle, &BrowserFactory::FindIEBrowserHandles, reinterpret_cast<LPARAM>(&new_handles));
    ...
    if (browser_wrapper->is_edge_chromium()) {
        ...
        while ((new_handles.size() == 0 || new_handles[0] == original_handles[0]) && clock() < end_time) {
        ...
        while (new_handles.size() <= original_handles.size() && clock() < end_time) {
            ::Sleep(50);
            ::EnumChildWindows(top_level_handle, &BrowserFactory::FindIEBrowserHandles, reinterpret_cast<LPARAM>(&new_handles));
        }
        ...
        std::vector<HWND>::iterator it
            = std::set_difference(new_handles.begin(), new_handles.end(), original_handles.begin(), original_handles.end(), diff.begin());
        ...
        if (diff.size() > 1) {
            ...
            LOG(DEBUG) << "Found more than one new window handles! Found " << diff.size() << "windows [" << handle_list << "]";
        }
    ...
    BrowserHandle new_tab_wrapper(new Browser(info.pBrowser, NULL, this->m_hWnd, this->is_edge_chromium_));
    // Force a wait cycle to make sure the browser is finished initializing.
...
int IECommandExecutor::CreateNewBrowser(std::string* error_message)
{
    ...
    this->proxy_manager_->SetProxySettings(process_window_info.hwndBrowser);
    BrowserHandle wrapper(new Browser(process_window_info.pBrowser, process_window_info.hwndBrowser, this->m_hWnd, this->is_edge_chromium_));
...
bool IECommandExecutor::AddManagedElement(IHTMLElement* element, ElementHandle* element_wrapper)
{
...
int IECommandExecutor::LocateElement(
    const ElementHandle parent_wrapper, const std::string& mechanism, const std::string& criteria, Json::Value* found_element) const
{
...
int IECommandExecutor::LocateElements(
    const ElementHandle parent_wrapper, const std::string& mechanism, const std::string& criteria, Json::Value* found_elements) const
{
