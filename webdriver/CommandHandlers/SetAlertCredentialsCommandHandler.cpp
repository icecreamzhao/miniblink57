...
void SetAlertCredentialsCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (alert_handle == NULL) {
        ...
        if (status_code != WD_SUCCESS) {
            response->SetErrorResponse(status_code, "Could not set user name");
            return;
        }
        status_code = dialog.SetPassword(password);
        if (status_code != WD_SUCCESS) {
            response->SetErrorResponse(status_code, "Could not set password");
            return;
        }
