...
void FullScreenWindowCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (!supports_full_screen) {
        response->SetErrorResponse(ERROR_UNSUPPORTED_OPERATION, "This version of Internet Explorer does not support setting to full screen");
    }
