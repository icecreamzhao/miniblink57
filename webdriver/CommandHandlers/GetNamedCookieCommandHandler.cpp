...
void GetNamedCookieCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    status_code = browser_wrapper->cookie_manager()->GetCookies(browser_wrapper->GetCurrentUrl(), &cookies);
