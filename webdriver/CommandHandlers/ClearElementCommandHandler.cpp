...
void ClearElementCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (id_parameter_iterator == command_parameters.end()) {
        ...
        if (status_code == WD_SUCCESS) {
            ...
            if (!element_wrapper->IsEditable() || !element_wrapper->IsEnabled()) {
                response->SetErrorResponse(ERROR_INVALID_ELEMENT_STATE, "Element must not be read-only or disabled");
                return;
            }
            if (!element_wrapper->IsInteractable()) {
                response->SetErrorResponse(
                    ERROR_ELEMENT_NOT_INTERACTABLE, "Element is not interactable, it must not be hidden and it must be able to receive focus");
                return;
            }
            ...
            status_code = script_wrapper.ExecuteAsync(executor, args, ASYNC_SCRIPT_EXECUTION_TIMEOUT_IN_MILLISECONDS, &async_executor_handle);
            if (status_code != WD_SUCCESS) {
                ...
                response->SetErrorResponse(EELEMENTNOTENABLED,
                    "A JavaScript error was encountered clearing the element. The driver assumes this is because the element is hidden, disabled or read-only, "
                    "and it must not be to clear the element.");
                return;
            }
