...
class GetCurrentWindowHandleCommandHandler : public MBCommandHandler {
public:
    GetCurrentWindowHandleCommandHandler(void);
    virtual ~GetCurrentWindowHandleCommandHandler(void);

protected:
    void ExecuteInternal(const MBCommandExecutor& executor, const ParametersMap& parameters, Response* response);
};
