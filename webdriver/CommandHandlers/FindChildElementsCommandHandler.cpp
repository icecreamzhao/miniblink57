...
void FindChildElementsCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (status_code == WD_SUCCESS) {
        ...
        if (status_code == WD_SUCCESS) {
            ...
            response->SetErrorResponse(status_code, "Finding elements with " + mechanism + " == " + value + "returned an unexpected error");
        }
    } else {
        if (status_code == EOBSOLETEELEMENT) {
            response->SetErrorResponse(ERROR_STALE_ELEMENT_REFERENCE, "Specified parent element is no longer attached to the DOM");
        } else {
