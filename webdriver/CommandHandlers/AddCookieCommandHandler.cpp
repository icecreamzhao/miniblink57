...
void AddCookieCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (last_path_slash_index != std::string::npos) {
        ...
        if (last_path_segment.size() > 1 && last_path_segment.find(".") != std::string::npos) {
    ...
    status_code = browser_wrapper->cookie_manager()->SetCookie(browser_wrapper->GetCurrentUrl(), cookie);
