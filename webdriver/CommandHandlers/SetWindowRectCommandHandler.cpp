...
void SetWindowRectCommandHandler::ExecuteInternal(const MBCommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    bool is_width_defined = this->IsParameterDefined(command_parameters, "width", &width_parameter);

    if (is_width_defined) {
        if (!this->GetNumericParameter("width", true, width_parameter, &width, &argument_error_message)) {
    ...
    bool is_height_defined = this->IsParameterDefined(command_parameters, "height", &height_parameter);
    if (is_height_defined) {
        if (!this->GetNumericParameter("height", true, height_parameter, &height, &argument_error_message)) {
            response->SetErrorResponse(ERROR_INVALID_ARGUMENT, argument_error_message);
            return;
        }
    }
    ...
    bool is_x_defined = this->IsParameterDefined(command_parameters, "x", &x_parameter);
    if (is_x_defined) {
        if (!this->GetNumericParameter("x", false, x_parameter, &x, &argument_error_message)) {
            response->SetErrorResponse(ERROR_INVALID_ARGUMENT, argument_error_message);
            return;
        }
    }
    ...
    bool is_y_defined = this->IsParameterDefined(command_parameters, "y", &y_parameter);

    if (is_y_defined) {
        if (!GetNumericParameter("y", false, y_parameter, &y, &argument_error_message)) {
            response->SetErrorResponse(ERROR_INVALID_ARGUMENT, argument_error_message);
            return;
        }
    }
...
bool SetWindowRectCommandHandler::IsParameterDefined(const ParametersMap& command_parameters, const std::string& parameter_name, Json::Value* parameter_value)
{
