...
void DeleteCookieCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    browser_wrapper->cookie_manager()->DeleteCookie(browser_wrapper->GetCurrentUrl(), cookie);
    if (status_code != WD_SUCCESS) {
