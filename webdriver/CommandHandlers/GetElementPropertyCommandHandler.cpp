...
void GetElementPropertyCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (id_parameter_iterator == command_parameters.end()) {
        ...
        if (status_code == WD_SUCCESS) {
            CComVariant value;
            status_code = element_wrapper->GetPropertyValue(name, &value);
            if (status_code != WD_SUCCESS) {
