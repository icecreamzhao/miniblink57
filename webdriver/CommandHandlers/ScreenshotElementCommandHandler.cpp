...
void ScreenshotElementCommandHandler::ExecuteInternal(const IECommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (status_code != WD_SUCCESS) {
        response->SetErrorResponse(status_code, "Could not get element of which to take a screenshot");
        return;
    }
    ...
    status_code = element_wrapper->GetLocationOnceScrolledIntoView(executor.input_manager()->scroll_behavior(), &location, &frame_locations);
