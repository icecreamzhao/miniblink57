...
#include "webdriver/CommandHandlers/NewSessionCommandHandler.h"
...
void NewSessionCommandHandler::ExecuteInternal(const MBCommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    ...
    if (it != command_parameters.end()) {
        WDLOG(DEBUG) << "Found W3C capabilities structure";
...
Json::Value NewSessionCommandHandler::GetCapability(
    const Json::Value& capabilities, const std::string& capability_name, const Json::ValueType& expected_capability_type, const Json::Value& default_value)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::GetCapability "
    ...
    if (!this->IsEquivalentType(capability_value.type(), expected_capability_type)) {
        WDLOG(WARN) << "Invalid capability setting: " << capability_name << " is type " << this->GetJsonTypeDescription(capability_value.type()) << " instead of "
                  << this->GetJsonTypeDescription(expected_capability_type) << ". Default value will be used: " << default_value.toStyledString();
        return default_value;
    }
    return capability_value;
}

bool NewSessionCommandHandler::IsEquivalentType(const Json::ValueType& actual_type, const Json::ValueType& expected_type)
{
...
std::string NewSessionCommandHandler::GetJsonTypeDescription(const Json::ValueType& type)
{
    switch (type) {
...
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::GetUnexpectedAlertBehaviorValue";
    std::string value = DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS;
    if (desired_value == DISMISS_UNEXPECTED_ALERTS || desired_value == ACCEPT_UNEXPECTED_ALERTS || desired_value == ACCEPT_AND_NOTIFY_UNEXPECTED_ALERTS
        || desired_value == DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS || desired_value == IGNORE_UNEXPECTED_ALERTS) {
        value = desired_value;
    } else {
        WDLOG(WARN) << "Desired value of " << desired_value << " for " << UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY << " is not"
...
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::GetPageLoadStrategyValue";
    std::string value = NORMAL_PAGE_LOAD_STRATEGY;
    if (desired_value == NORMAL_PAGE_LOAD_STRATEGY || desired_value == EAGER_PAGE_LOAD_STRATEGY || desired_value == NONE_PAGE_LOAD_STRATEGY) {
        value = desired_value;
    } else {
        WDLOG(WARN) << "Desired value of " << desired_value << " for " << PAGE_LOAD_STRATEGY_CAPABILITY << " is not"
                  << " a valid value. Using default of " << value;
    }
    return value;
}
...
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::ValidateArguments";
    ...
    if (capabilities.isMember("alwaysMatch")) {
        WDLOG(DEBUG) << "Found alwaysMatch in capabilities";
    ...
    WDLOG(DEBUG) << "Validating alwaysMatch capability set";
        ...
        if (!first_match_candidates.isArray()) {
            *error_message = "firstMatch must be a JSON list";
            ...
            for (size_t i = 0; i < first_match_candidates.size(); ++i) {
                WDLOG(DEBUG) << "Validating firstMatch capability set with index " << i;
...
Json::Value NewSessionCommandHandler::ProcessCapabilities(const MBCommandExecutor& executor, const Json::Value& capabilities, std::string* error_message)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::ProcessCapabilities";
...
void NewSessionCommandHandler::SetBrowserFactorySettings(const MBCommandExecutor& executor, const Json::Value& capabilities)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::SetBrowserFactorySettings";
...
// 
//         Json::Value browser_command_line_switches
//             = this->GetCapability(capabilities, BROWSER_COMMAND_LINE_SWITCHES_CAPABILITY, Json::stringValue, Json::Value(Json::stringValue));
...
//         Json::Value attach_to_edgechrome = this->GetCapability(capabilities, ATTACH_TO_EDGE_CHROME, Json::booleanValue, false);
...
//         Json::Value ignore_process_match_ie_mode = this->GetCapability(capabilities, IGNORE_PROCESS_MATCH, Json::booleanValue, false);
...
void NewSessionCommandHandler::SetProxySettings(const MBCommandExecutor& executor, const Json::Value& proxy_capability, const bool use_per_process_proxy)
{
    DebugBreak();
//     WDLOG(TRACE) << "Entering NewSessionCommandHandler::SetProxySettings";
...
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::CreateReturnedCapabilities";
    Json::Value capabilities;
    capabilities[BROWSER_NAME_CAPABILITY] = "chrome";//"internet explorer";
    capabilities[BROWSER_VERSION_CAPABILITY] = "108.0.5359.125";//std::to_string(static_cast<long long>(executor.browser_factory()->browser_version()));
#ifdef _WIN32
    capabilities[PLATFORM_NAME_CAPABILITY] = "windows";
#else
    capabilities[PLATFORM_NAME_CAPABILITY] = "linux"; 
#endif
...
//     capabilities[STRICT_FILE_INTERACTABILITY_CAPABILITY] = executor.use_strict_file_interactability();
    ...
    capabilities["networkConnectionEnabled"] = false;
    capabilities["pageLoadStrategy"] = "normal";
    capabilities["setWindowRect"] = true;
    capabilities["strictFileInteractability"] = false;

    capabilities["unhandledPromptBehavior"] = "dismiss and notify";
    capabilities["webauthn:extension:credBlob"] = true;
    ...
    capabilities["browserVersion"] = "108.0.5359.125";
    Json::Value chromeOptions;
    chromeOptions["chromedriverVersion"] = "108.0.5359.125 (04b0d1dbeaf2ace7f88a791efa178fc34bf57eab-refs/branch-heads/5359@{#1180})";
    //chromeOptions["userDataDir"] = "C:\\Users\\wang\\AppData\\Local\\Temp\\scoped_dir14192_1512798666";
    capabilities["chrome"] = chromeOptions;

//     Json::Value googchromeOptions;
//     ie_options[INITIAL_BROWSER_URL_CAPABILITY] = executor.settings().initial_browser_url;
//     ie_options[BROWSER_COMMAND_LINE_SWITCHES_CAPABILITY] = executor.settings().browser_command_line_switches;
...
//     }
    ...
    capabilities[PROXY_CAPABILITY] = Json::Value(Json::objectValue);
    capabilities[IE_DRIVER_EXTENSIONS_CAPABILITY] = Json::Value(Json::objectValue);
    return capabilities;
}

bool NewSessionCommandHandler::MatchCapabilities(const MBCommandExecutor& executor, const Json::Value& merged_capabilities, std::string* error_message)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::MatchCapabilities";
    ...
    for (; name_iterator != capability_names.end(); ++name_iterator) {
        ...
        if (capability_name == BROWSER_NAME_CAPABILITY && merged_capabilities[BROWSER_NAME_CAPABILITY].asString() != "chrome") {
            *error_message = "browserName must be 'chrome', but was '" + merged_capabilities[BROWSER_NAME_CAPABILITY].asString() + "'";
            return false;
        }

        if (capability_name == PLATFORM_NAME_CAPABILITY && merged_capabilities[PLATFORM_NAME_CAPABILITY].asString() != "windows") {
            *error_message = "platformName must be 'windows', but was '" + merged_capabilities[PLATFORM_NAME_CAPABILITY].asString() + "'";
            return false;
        }
        ...
        if (capability_name == BROWSER_VERSION_CAPABILITY) {
            ...
            if (available_browser_version != requested_browser_version) {
                *error_message = "requested browserVersion value was '" + requested_browser_version_value + "', but the installed version of IE is "
                    + std::to_string(available_browser_version) + " (note: only exact matches on major version " + "number are supported)";
                return false;
            }
        }
#endif
        if (capability_name == ACCEPT_INSECURE_CERTS_CAPABILITY && merged_capabilities[ACCEPT_INSECURE_CERTS_CAPABILITY].asBool()) {
        ...
        if (capability_name.find(":") != std::string::npos
            && (capability_name != IE_DRIVER_EXTENSIONS_CAPABILITY && capability_name.find("test:") == std::string::npos)) {
...
bool NewSessionCommandHandler::MergeCapabilities(
    const Json::Value& primary_capabilities, const Json::Value& secondary_capabilities, Json::Value* merged_capabilities, std::string* error_message)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::MergeCapabilities";
    ...
    for (size_t i = 0; i < secondary_property_names.size(); ++i) {
        ...
        if (merged_capabilities->isMember(property_name)) {
            *error_message = "Cannot merge capabilities: " + property_name + " is already specified";
            return false;
        }
...
bool NewSessionCommandHandler::ValidateCapabilities(const Json::Value& capabilities, const std::string& capability_set_name, std::string* error_message)
{
    WDLOG(TRACE) << "Entering NewSessionCommandHandler::ValidateCapabilities";
    WDLOG(DEBUG) << "Validating capabilities object";
    ...
    for (; name_iterator != capability_names.end(); ++name_iterator) {
        ...
        if (capability_name == ACCEPT_INSECURE_CERTS_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << ACCEPT_INSECURE_CERTS_CAPABILITY << " capability."
                       << " Validating value type is boolean.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::booleanValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            }
            continue;
        }
        ...
        if (capability_name == STRICT_FILE_INTERACTABILITY_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << STRICT_FILE_INTERACTABILITY_CAPABILITY << " capability."
                       << " Validating value type is boolean.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::booleanValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
        ...
        if (capability_name == BROWSER_NAME_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << BROWSER_NAME_CAPABILITY << " capability."
                       << " Validating value type is string.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::stringValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            }
            continue;
        ...
        if (capability_name == BROWSER_VERSION_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << BROWSER_VERSION_CAPABILITY << " capability."
                       << " Validating value type is string.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::stringValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
        ...
        if (capability_name == PLATFORM_NAME_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << PLATFORM_NAME_CAPABILITY << " capability."
                       << " Validating value type is string.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::stringValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
        ...
        if (capability_name == UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY << " capability."
                       << " Validating value type is string.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::stringValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            } else {
                WDLOG(DEBUG) << "Validating " << UNHANDLED_PROMPT_BEHAVIOR_CAPABILITY << " capability"
                           << " is a valid value.";
                std::string unhandled_prompt_behavior = capabilities[capability_name].asString();
                if (unhandled_prompt_behavior != ACCEPT_UNEXPECTED_ALERTS && unhandled_prompt_behavior != DISMISS_UNEXPECTED_ALERTS
                    && unhandled_prompt_behavior != ACCEPT_AND_NOTIFY_UNEXPECTED_ALERTS && unhandled_prompt_behavior != DISMISS_AND_NOTIFY_UNEXPECTED_ALERTS
                    && unhandled_prompt_behavior != IGNORE_UNEXPECTED_ALERTS) {
                    *error_message = "Invalid capabilities in " + capability_set_name + ": " + "unhandledPromptBehavior is " + unhandled_prompt_behavior
        ...
        if (capability_name == PAGE_LOAD_STRATEGY_CAPABILITY) {
            ...
            WDLOG(DEBUG) << "Found " << PAGE_LOAD_STRATEGY_CAPABILITY << " capability."
                       << " Validating value type is string.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::stringValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            } else {
                WDLOG(DEBUG) << "Validating " << PAGE_LOAD_STRATEGY_CAPABILITY << " capability"
                           << " is a valid value.";
                page_load_strategy = capabilities[capability_name].asString();
                if (page_load_strategy != NONE_PAGE_LOAD_STRATEGY && page_load_strategy != EAGER_PAGE_LOAD_STRATEGY
                    && page_load_strategy != NORMAL_PAGE_LOAD_STRATEGY) {
                    *error_message = "Invalid capabilities in " + capability_set_name + ": " + "pageLoadStrategy is " + page_load_strategy
        ...
        if (capability_name == TIMEOUTS_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << TIMEOUTS_CAPABILITY << " capability."
                       << " Validating value type is object.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::objectValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            } else {
                WDLOG(DEBUG) << "Validating " << TIMEOUTS_CAPABILITY << " capability"
                ...
                for (; timeout_name_iterator != timeout_names.end(); ++timeout_name_iterator) {
                    ...
                    if (timeout_name != PAGE_LOAD_TIMEOUT_NAME && timeout_name != IMPLICIT_WAIT_TIMEOUT_NAME && timeout_name != SCRIPT_TIMEOUT_NAME) {
                        *error_message = "Invalid capabilities in " + capability_set_name + ": " + "a timeout named " + timeout_name
                            + " is specified, but timeout names must be " + "'implicit', 'pageLoad', or 'script'";
                        return false;
                    }
                    ...
                    if (timeout_name != SCRIPT_TIMEOUT_NAME || !timeout_value.isNull()) {
                        if (!timeout_value.isNumeric() || !timeout_value.isIntegral()) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "timeout " + timeout_name + "must be an integer";
                            return false;
                        }
                        if (!timeout_value.isInt64()) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "timeout " + timeout_name
        ...
        if (capability_name == PROXY_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << PROXY_CAPABILITY << " capability."
                       << " Validating value type is object.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::objectValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
            } else {
                WDLOG(DEBUG) << "Validating " << PROXY_CAPABILITY << "capability"
                           << " object structure.";
                ...
                for (; proxy_setting_iterator != proxy_setting_names.end(); ++proxy_setting_iterator) {
                    ...
                    if (proxy_setting == "proxyType") {
                        if (!this->ValidateCapabilityType(proxy, proxy_setting, Json::ValueType::stringValue, &proxy_error)) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "proxy setting " + proxy_error;
                            return false;
                        }
                        std::string proxy_type = proxy[proxy_setting].asString();
                        if (proxy_type != "pac" && proxy_type != "direct" && proxy_type != "autodetect" && proxy_type != "system" && proxy_type != "manual") {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "a proxy type named " + proxy_type
                                + " is specified, but proxy type must be " + "'pac', 'direct', 'autodetect', 'system', " + "or 'manual'";
                            return false;
                        }
                        continue;
                    }

                    if (proxy_setting == "proxyAutoconfigUrl" || proxy_setting == "ftpProxy" || proxy_setting == "httpProxy" || proxy_setting == "sslProxy"
                    ...
                    if (proxy_setting == "noProxy") {
                        if (!this->ValidateCapabilityType(proxy, proxy_setting, Json::ValueType::arrayValue, &proxy_error)) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "proxy setting " + proxy_error;
                            return false;
                        }
                        continue;
                    ...
                    if (proxy_setting == "socksVersion") {
                        if (!this->ValidateCapabilityType(proxy, proxy_setting, Json::ValueType::intValue, &proxy_error)) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "proxy setting " + proxy_error;
                            return false;
                        }
                        int socks_version = proxy[proxy_setting].asInt();
                        if (socks_version < 0 || socks_version > 255) {
                            *error_message = "Invalid capabilities in " + capability_set_name + ": " + "SOCKS version must be between 0 and 255.";
                            return false;
                        }
                        continue;
                    }

                    *error_message = "Invalid capabilities in " + capability_set_name + ": " + "unknown proxy setting named " + proxy_error;
                    return false;
                }
            }
            continue;
        ...
        if (capability_name == IE_DRIVER_EXTENSIONS_CAPABILITY) {
            WDLOG(DEBUG) << "Found " << IE_DRIVER_EXTENSIONS_CAPABILITY << " capability."
                       << " Validating value type is object.";
            if (!this->ValidateCapabilityType(capabilities, capability_name, Json::ValueType::objectValue, &capability_error_message)) {
                *error_message = "Invalid capabilities in " + capability_set_name + ": " + capability_error_message;
                return false;
        ...
        if (capability_name.find(":") != std::string::npos) {
            WDLOG(DEBUG) << "Found extension capability named " << capability_name << "."
                       << " Nothing further to validate.";
            continue;
        }

        *error_message = "Invalid capabilities in " + capability_set_name + ": " + "unknown capability named " + capability_name;
        return false;
    }
...
bool NewSessionCommandHandler::ValidateCapabilityType(
    const Json::Value& capabilities, const std::string& capability_name, const Json::ValueType& expected_capability_type, std::string* error_message)
{
    Json::Value capability_value = capabilities[capability_name];
    if (!this->IsEquivalentType(capability_value.type(), expected_capability_type)) {
        *error_message = capability_name + " is type " + this->GetJsonTypeDescription(capability_value.type()) + " instead of "
