...
bool VariantUtilities::VariantIsArray(VARIANT value)
{
    ...
    if (type_name == L"JScriptTypeInfo" || type_name == L"") {
        ...
        HRESULT hr = value.pdispVal->GetIDsOfNames(IID_NULL, &length_property_name, 1, LOCALE_USER_DEFAULT, &dispid_length);
        if (SUCCEEDED(hr)) {
            return true;
        }
    }
...
int VariantUtilities::VariantAsJsonValue(IElementManager* element_manager, VARIANT variant_value, Json::Value* value)
{
...
bool VariantUtilities::HasSelfReferences(VARIANT current_object, std::vector<IDispatch*>* visited)
{
    ...
    if (VariantIsArray(current_object) || VariantIsObject(current_object)) {
        ...
        if (VariantIsArray(current_object)) {
            ...
            status_code = GetPropertyNameList(current_object.pdispVal, &property_names);
        }

        visited->push_back(current_object.pdispVal);
        for (size_t i = 0; i < property_names.size(); ++i) {
            CComVariant property_value;
            GetVariantObjectPropertyValue(current_object.pdispVal, property_names[i], &property_value);
...
int VariantUtilities::ConvertVariantToJsonValue(IElementManager* element_manager, VARIANT variant_value, Json::Value* value)
{
    int status_code = WD_SUCCESS;
    if (VariantIsString(variant_value)) {
        ...
        *value = Json::Value::null;
        ...
        if (VariantIsArray(variant_value) || VariantIsElementCollection(variant_value)) {
            ...
            for (long i = 0; i < length; ++i) {
                ...
                int array_item_status = GetArrayItem(variant_value.pdispVal, i, &array_item);
                if (array_item_status != WD_SUCCESS) {
                    LOG(WARN) << "Did not successfully get item with index " << i << " from array.";
                    return EUNEXPECTEDJSERROR;
                }
                Json::Value array_item_result;
                ConvertVariantToJsonValue(element_manager, array_item, &array_item_result);
            ...
            *value = result_array;
            ...
            if (ExecuteToJsonMethod(variant_value, &json_serialized)) {
                ...
                int property_enum_status = GetAllVariantObjectPropertyValues(element_manager, variant_value, &result_object);
            ...
            *value = result_object;
            ...
            if (FAILED(hr)) {
                ...
                if (ExecuteToJsonMethod(variant_value, &json_serialized_variant)) {
                    ...
                    ConvertVariantToJsonValue(element_manager, json_serialized_variant, &interim_value);
                    *value = interim_value;
                    return WD_SUCCESS;
                }
                ...
                if (typeinfo_count != 0) {
                    ...
                    int property_enum_status = GetAllVariantObjectPropertyValues(element_manager, variant_value, &final_result_object);
                    if (property_enum_status != WD_SUCCESS) {
                        return EUNEXPECTEDJSERROR;
                    }
...
bool VariantUtilities::ExecuteToJsonMethod(VARIANT object_to_serialize, VARIANT* json_object_variant)
{
    CComVariant to_json_method;
    bool has_to_json_property = GetVariantObjectPropertyValue(object_to_serialize.pdispVal, L"toJSON", &to_json_method);
    ...
    HRESULT hr = to_json_method.pdispVal->GetIDsOfNames(IID_NULL, &call_member_name, 1, LOCALE_USER_DEFAULT, &call_member_id);
    if (FAILED(hr)) {
    ...
    hr = to_json_method.pdispVal->Invoke(call_member_id, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &call_parameters, json_object_variant, &exception, 0);
...
bool VariantUtilities::GetVariantObjectPropertyValue(IDispatch* variant_object_dispatch, std::wstring property_name, VARIANT* property_value)
{
    LPOLESTR property_name_pointer = reinterpret_cast<LPOLESTR>(const_cast<wchar_t*>(property_name.data()));
    DISPID dispid_property;
    HRESULT hr = variant_object_dispatch->GetIDsOfNames(IID_NULL, &property_name_pointer, 1, LOCALE_USER_DEFAULT, &dispid_property);
    if (FAILED(hr)) {
        ...
        // want to accomplish.
        LOGHR(DEBUG, hr) << "Unable to get dispatch ID (dispid) for property " << StringUtilities::ToString(property_name);
        return false;
    }

    // get the value of eval result
    DISPPARAMS no_args_dispatch_parameters = { 0 };
    hr = variant_object_dispatch->Invoke(
        dispid_property, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, &no_args_dispatch_parameters, property_value, NULL, NULL);
    if (FAILED(hr)) {
        LOGHR(WARN, hr) << "Unable to get result for property " << StringUtilities::ToString(property_name);
        return false;
    }
    return true;
}
...
std::wstring VariantUtilities::GetVariantObjectTypeName(VARIANT value)
{
    ...
    if (value.vt == VT_DISPATCH && value.pdispVal) {
        ...
        HRESULT get_type_info_result = value.pdispVal->GetTypeInfo(0, LOCALE_USER_DEFAULT, &typeinfo);
        TYPEATTR* type_attr;
        CComBSTR name_bstr;
        if (SUCCEEDED(get_type_info_result) && SUCCEEDED(typeinfo->GetTypeAttr(&type_attr)) && SUCCEEDED(typeinfo->GetDocumentation(-1, &name_bstr, 0, 0, 0))) {
...
int VariantUtilities::GetPropertyNameList(IDispatch* object_dispatch, std::vector<std::wstring>* property_names)
{
...
int VariantUtilities::GetArrayLength(IDispatch* array_dispatch, long* length)
{
    ...
    bool get_length_success = GetVariantObjectPropertyValue(array_dispatch, L"length", &length_result);
...
int VariantUtilities::GetArrayItem(IDispatch* array_dispatch, long index, VARIANT* item)
{
    LOG(TRACE) << "Entering Script::GetArrayItem";
    std::wstring index_string = std::to_wstring(static_cast<long long>(index));
    CComVariant array_item_variant;
    bool get_array_item_success = GetVariantObjectPropertyValue(array_dispatch, index_string, item);
    ...
    if (!get_array_item_success) {
        ...
        DISPID dispid_item;
        HRESULT hr = array_dispatch->GetIDsOfNames(IID_NULL, &item_method_pointer, 1, LOCALE_USER_DEFAULT, &dispid_item);
        if (FAILED(hr)) {
            return EUNEXPECTEDJSERROR;
        }
        ...
        hr = array_dispatch->Invoke(dispid_item, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &call_parameters, item, NULL, NULL);
        if (FAILED(hr)) {
            return EUNEXPECTEDJSERROR;
        }
    }
...
int VariantUtilities::GetAllVariantObjectPropertyValues(IElementManager* element_manager, VARIANT variant_value, Json::Value* value)
{
    std::vector<std::wstring> property_names;
    ...
    for (size_t i = 0; i < property_names.size(); ++i) {
        ...
        bool property_value_retrieved = GetVariantObjectPropertyValue(variant_value.pdispVal, property_names[i], &property_value_variant);
        if (!property_value_retrieved) {
            LOG(WARN) << "Did not successfully get value for property '" << StringUtilities::ToString(property_names[i]) << "' from object.";
            return EUNEXPECTEDJSERROR;
        }

        Json::Value property_value;
        ConvertVariantToJsonValue(element_manager, property_value_variant, &property_value);
