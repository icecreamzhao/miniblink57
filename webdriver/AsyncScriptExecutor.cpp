...
LRESULT AsyncScriptExecutor::OnInit(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::OnInit";
    return 0;
}

LRESULT AsyncScriptExecutor::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (serialized_args.size() > 0) {
        ...
        Json::parseFromStream(Json::CharReaderBuilder(), json_stream, &this->script_args_, &parse_errors);
...
LRESULT AsyncScriptExecutor::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnSetDocument(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::OnSetDocument";
    CComPtr<IHTMLDocument2> doc;
    LPSTREAM initializer_payload = reinterpret_cast<LPSTREAM>(lParam);
    HRESULT hr = ::CoGetInterfaceAndReleaseStream(initializer_payload, IID_IHTMLDocument2, reinterpret_cast<void**>(&this->script_host_));
    return 0;
}

LRESULT AsyncScriptExecutor::OnSetElementArgument(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    std::vector<std::string>::iterator item = std::find(this->element_id_list_.begin(), this->element_id_list_.end(), element_id);
    if (item == this->element_id_list_.end()) {
        LOG(WARN) << "Invalid element ID sent from main repository: " << element_id;
    }
    CComPtr<IHTMLElement> element;
    ::CoGetInterfaceAndReleaseStream(info->element_stream, IID_IHTMLElement, reinterpret_cast<void**>(&element));
    delete info;
...
LRESULT AsyncScriptExecutor::OnGetRequiredElementList(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnIsExecutionReady(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::OnIsExecutionReady";
    return this->element_id_list_.size() == 0 ? 1 : 0;
}

LRESULT AsyncScriptExecutor::OnSetPollingScript(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnExecuteScript(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::OnExecuteScript";
    Script script_to_execute(this->script_host_, this->script_source_code_, this->script_argument_count_);
    this->status_code_ = script_to_execute.AddArguments(this, this->script_args_);
    ...
    if (!this->is_listener_attached_) {
        ...
        if (this->status_code_ == WD_SUCCESS) {
            if (this->polling_script_source_code_.size() > 0) {
                ...
                this->status_code_ = script_to_execute.ConvertResultToJsonValue(this, &this->script_result_);
            }
            ...
            if (script_to_execute.ResultIsString()) {
                script_to_execute.ConvertResultToJsonValue(this, &this->script_result_);
            }
        }
    }
    this->is_execution_completed_ = true;
...
LRESULT AsyncScriptExecutor::OnDetachListener(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnIsExecutionComplete(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    return this->is_execution_completed_ ? 1 : 0;
}

LRESULT AsyncScriptExecutor::OnGetResult(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT AsyncScriptExecutor::OnNotifyElementTransferred(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    std::vector<std::string>::const_iterator item = std::find(this->element_id_list_.begin(), this->element_id_list_.end(), original_element_id);
    this->element_id_list_.erase(item);
    if (this->element_id_list_.size() == 0) {
        this->is_execution_completed_ = true;
    }
...
void AsyncScriptExecutor::ReplaceTransferredElementResult(std::string original_element_id, std::string element_id, Json::Value* result)
{
    if (result->isArray()) {
        for (Json::ArrayIndex i = 0; i < result->size(); ++i) {
            this->ReplaceTransferredElementResult(original_element_id, element_id, &((*result)[i]));
        }
    } else if (result->isObject()) {
        if (result->isMember(JSON_ELEMENT_PROPERTY_NAME) && (*result)[JSON_ELEMENT_PROPERTY_NAME] == original_element_id) {
            ...
            for (; it != member_names.end(); ++it) {
                this->ReplaceTransferredElementResult(original_element_id, element_id, &((*result)[*it]));
            }
        }
    }
}

int AsyncScriptExecutor::GetManagedElement(const std::string& element_id, ElementHandle* element_wrapper) const
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::GetManagedElement";
    return this->element_repository_->GetManagedElement(element_id, element_wrapper);
}

bool AsyncScriptExecutor::AddManagedElement(IHTMLElement* element, ElementHandle* element_wrapper)
{
    LOG(TRACE) << "Entering AsyncScriptExecutor::AddManagedElement";
    bool is_new_element = this->element_repository_->AddManagedElement(NULL, element, element_wrapper);
    if (is_new_element) {
...
void AsyncScriptExecutor::RemoveManagedElement(const std::string& element_id)
{
    ...
    ::PostMessage(this->main_element_repository_handle_, WD_ASYNC_SCRIPT_SCHEDULE_REMOVE_MANAGED_ELEMENT, NULL, reinterpret_cast<LPARAM>(info));
}
...
void AsyncScriptExecutor::TransferReturnedElements()
{
    ...
    for (; it != this->element_id_list_.end(); ++it) {
        ...
        this->element_repository_->GetManagedElement(element_id, &element_handle);
        ElementInfo* info = new ElementInfo;
        info->element_id = element_id.c_str();
        ::CoMarshalInterThreadInterfaceInStream(IID_IHTMLElement, element_handle->element(), &info->element_stream);
        ::PostMessage(this->main_element_repository_handle_, WD_ASYNC_SCRIPT_TRANSFER_MANAGED_ELEMENT, NULL, reinterpret_cast<LPARAM>(info));
    }
}
