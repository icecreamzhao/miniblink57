...
public:
    ...
    bool AddManagedElement(IHTMLElement* element, ElementHandle* element_wrapper);
    ...
    IElementManager* element_manager(void)
    {
        return this;
    }
    ...
    ElementFinder* element_finder(void) const
    {
        return this->element_finder_;
    }
    InputManager* input_manager(void) const
    {
        return this->input_manager_;
    }
    ProxyManager* proxy_manager(void) const
    {
        return this->proxy_manager_;
    }
    BrowserFactory* browser_factory(void) const
    {
...
private:
    ...
    void CreateWaitThread(const std::string& deferred_response, const bool is_deferred_command_execution);
    void CreateDelayPostMessageThread(const DWORD delay_time, const HWND window_handle, const UINT message_to_post);
    bool IsCommandValidWithAlertPresent(void);
    bool IsAlertActive(BrowserHandle browser, HWND* alert_handle);
    bool HandleUnexpectedAlert(BrowserHandle browser, HWND alert_handle, bool force_use_dismiss, std::string* alert_text);

    void PostBrowserReattachMessage(const DWORD current_process_id, const std::string& browser_id, const std::vector<DWORD>& known_process_ids);
    void GetNewBrowserProcessIds(std::vector<DWORD>* known_process_ids, std::vector<DWORD>* new_process_ids);

    std::string OpenNewBrowsingContext(const std::string& window_type, const std::string& url);
