...
int CookieManager::GetCookies(const std::string& url, std::vector<BrowserCookie>* all_cookies)
{
    ...
    if (supports_advanced_api) {
        ...
        for (cookie_iterator = cookies.begin(); cookie_iterator != cookies.end(); ++cookie_iterator) {
        ...
        for (file_iterator = files.begin(); file_iterator != files.end(); ++file_iterator) {
...
void CookieManager::ParseCookieList(const std::string& cookie_file_contents, const bool include_secure_cookies, std::map<std::string, BrowserCookie>* cookies)
{
    ...
    for (cookie_string_iterator = persistent_cookie_strings.begin(); cookie_string_iterator != persistent_cookie_strings.end(); ++cookie_string_iterator) {
...
LRESULT CALLBACK CookieWndProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (WM_COPYDATA == call_window_proc_struct->message) {
    ...
    } else if (WD_GET_HTTPONLY_COOKIES == call_window_proc_struct->message || WD_GET_SCRIPTABLE_COOKIES == call_window_proc_struct->message
        ...
        if ((WD_GET_SECURE_COOKIES == call_window_proc_struct->message || WD_GET_SCRIPTABLE_COOKIES == call_window_proc_struct->message)
        ...
        if (success) {
            ...
            ::InternetGetCookieEx(
                parsed_uri.c_str(), NULL, reinterpret_cast<LPTSTR>(webdriver::HookProcessor::GetDataBufferAddress()), &buffer_size, get_cookie_flags, NULL);
