...
UriInfo::UriInfo(const std::string& uri, const std::string& verb, const std::string& command_name)
{
    ParseUri(uri, &this->fragments_, &this->parameter_indexes_);
    this->AddHttpVerb(verb, command_name);
}

void UriInfo::ParseUri(const std::string& uri, std::vector<std::string>* fragments, std::vector<size_t>* parameter_indexes)
{
    ...
    do {
        ...
        std::string token = uri.substr(current_position, token_end_position - current_position);
...
bool UriInfo::IsUriMatch(const std::vector<std::string>& uri_fragments, std::vector<std::string>* uri_param_names, std::vector<std::string>* uri_param_values)
{
    ...
    for (size_t index = 0; index < this->fragments_.size(); ++index) {
        if (uri_fragments[index] != this->fragments_[index]) {
            ...
            if (next_parameter_index < this->parameter_indexes_.size() && this->parameter_indexes_[next_parameter_index] != index) {
                return false;
            } else {
                if (!this->IsValidParameterValue(
                        this->fragments_[this->parameter_indexes_[next_parameter_index]], uri_fragments[this->parameter_indexes_[next_parameter_index]])) {
                    return false;
                }
...
bool UriInfo::HasHttpVerb(const std::string& http_verb, std::string* command_name)
{
...
void UriInfo::AddHttpVerb(const std::string& http_verb, const std::string& command_name)
{
    this->verb_map_[http_verb] = command_name;
}

bool UriInfo::IsValidParameterValue(const std::string& param_name, const std::string& param_value)
{
