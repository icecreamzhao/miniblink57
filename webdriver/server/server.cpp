...
#include "webdriver/server/uri_info.h"
#include "webdriver/server/wd_logging.h"

#define SERVER_DEFAULT_PAGE                                                                                                                                    \
...
Server::Server(const int port, const std::string& host, const std::string& log_level, const std::string& log_file)
{
    this->Initialize(port, host, log_level, log_file, "");
}

Server::Server(const int port, const std::string& host, const std::string& log_level, const std::string& log_file, const std::string& acl)
{
...
void Server::Initialize(const int port, const std::string& host, const std::string& log_level, const std::string& log_file, const std::string& acl)
{
    LoggerLOG::Level(log_level);
    LoggerLOG::File(log_file);
    WDLOG(INFO) << "Starting WebDriver server on port: '" << port << "' on host: '" << host << "'";
    this->port_ = port;
    this->host_ = host;
    if (acl.size() > 0) {
...
std::string Server::GetListeningPorts(const bool use_ipv6)
{
    ...
    int formatted_string_size = snprintf(NULL, 0, port_format_string.c_str(), this->host_.c_str(), this->port_, this->port_) + 1;
    std::vector<char> listening_ports_buffer(formatted_string_size);
    snprintf(&listening_ports_buffer[0], formatted_string_size, port_format_string.c_str(), this->host_.c_str(), this->port_, this->port_);
...
std::string Server::GetAccessControlList()
{
    ...
    if (this->whitelist_.size() > 0) {
        ...
        for (std::vector<std::string>::const_iterator it = this->whitelist_.begin(); it < this->whitelist_.end(); ++it) {
            acl.append(",+").append(*it);
        }
        WDLOG(DEBUG) << "Civetweb ACL is " << acl;
    }
    return acl;
}
...
bool Server::Start()
{
    WDLOG(TRACE) << "Entering Server::Start";
    ...
    if (context_ == NULL) {
        ...
        if (listening_port_option == ipv4_port_option) {
            ...
            WDLOG(WARN) << "Failed to start Civetweb";
            return false;
        } else {
            ...
            WDLOG(INFO) << "Failed first attempt to start Civetweb. Attempt start with IPv4 only";
            this->options_["listening_ports"] = listening_port_option;
            options.clear();
            this->GenerateOptionsList(&options);
            context_ = mg_start(&callbacks, this, &options[0]);
...
void Server::Stop()
{
    WDLOG(TRACE) << "Entering Server::Stop";
    if (context_) {
        mg_stop(context_);
        context_ = NULL;
    }
...
int Server::ProcessRequest(struct mg_connection* conn, const struct mg_request_info* request_info)
{
    ...
    WDLOG(TRACE) << "Process request with:"
               << " URI: " << request_info->local_uri << " HTTP verb: " << http_verb << std::endl
               << "body: " << request_body;

    if (strcmp(request_info->local_uri, "/") == 0) {        
        this->SendHttpOk(conn, request_info, SERVER_DEFAULT_PAGE, HTML_CONTENT_TYPE);
        ...
        std::string serialized_response = this->DispatchCommand(request_info->local_uri, http_verb, request_body);
        http_response_code = this->SendResponseToClient(conn, request_info, serialized_response);
    }

    return http_response_code;
}

void Server::AddCommand(const std::string& url, const std::string& http_verb, const std::string& command_name)
{
    if (this->commands_.find(url) == this->commands_.end()) {
        this->commands_[url] = std::shared_ptr<UriInfo>(new UriInfo(url, http_verb, command_name));
    } else {
...
{
    WDLOG(TRACE) << "Entering Server::ShutDownSession";
    ...
    if (it != this->sessions_.end()) {
        ...
        WDLOG(DEBUG) << "Shutdown session is not found";
    }
}

std::string Server::ReadRequestBody(struct mg_connection* conn, const struct mg_request_info* request_info)
{
    WDLOG(TRACE) << "Entering Server::ReadRequestBody";
    ...
    if (content_length != 0) {
        ...
        while (bytes_read < content_length) {
            bytes_read += mg_read(conn, &buffer[bytes_read], content_length - bytes_read);
        }
...
std::string Server::DispatchCommand(const std::string& uri, const std::string& http_verb, const std::string& command_body)
{
    ...
    std::string command = this->LookupCommand(uri, http_verb, &session_id, &locator_parameters);
    WDLOG(DEBUG) << "Command: " << http_verb << " " << uri << " " << command_body;
    ...
    if (command == webdriver::CommandType::NoCommand) {
        ...
        if (locator_parameters.size() > 0) {
            ...
            invalid_command_response.SetErrorResponse(ERROR_UNKNOWN_METHOD, unknown_method_body);
            ...
            invalid_command_response.SetErrorResponse(ERROR_UNKNOWN_COMMAND, unknown_command_body);
        }
        ...
        if (command != webdriver::CommandType::NewSession && !this->LookupSession(session_id, &session_handle)) {
            if (command == webdriver::CommandType::Quit) {
                ...
                invalid_session_id_response.SetErrorResponse(ERROR_INVALID_SESSION_ID, invalid_session_message);
                serialized_response = invalid_session_id_response.Serialize();
            }
        } else {
            if (command == webdriver::CommandType::NewSession && this->sessions_.size() > 0) {
                ...
                session_exists_response.SetErrorResponse(ERROR_SESSION_NOT_CREATED, session_exists_message);
    ...
    WDLOG(DEBUG) << "Response: " << serialized_response;
    return serialized_response;
}

std::string Server::ListSessions()
{
    WDLOG(TRACE) << "Entering Server::ListSessions";

    // Manually construct the serialized command for getting
    // session capabilities.
    std::string get_caps_command = "{ \"name\" : \"" + webdriver::CommandType::GetSessionCapabilities + "\"" + ", \"locator\" : {}, \"parameters\" : {} }";
...
int Server::SendResponseToClient(struct mg_connection* conn, const struct mg_request_info* request_info, const std::string& serialized_response)
{
    ...
    if (serialized_response.size() > 0) {
        ...
        if (return_code == 0) {
            this->SendHttpOk(conn, request_info, serialized_response, HTML_CONTENT_TYPE);
            return_code = 200;
        } else if (return_code == 200) {
            this->SendHttpOk(conn, request_info, serialized_response, JSON_CONTENT_TYPE);
            ...
            this->SendHttpMethodNotAllowed(conn, request_info, allowed_verbs, serialized_response);
            return_code = 405;
        } else if (return_code == 501) {
            this->SendHttpNotImplemented(conn, request_info, "");
            return_code = 501;
        } else {
...
void Server::SendHttpBadRequest(struct mg_connection* const connection, const struct mg_request_info* request_info, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpBadRequest, " << body.size();
...
void Server::SendHttpInternalError(struct mg_connection* connection, const struct mg_request_info* request_info, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpInternalError, " << body.size();
...
void Server::SendHttpNotFound(struct mg_connection* const connection, const struct mg_request_info* request_info, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpNotFound, " << body.size();
...
void Server::SendHttpMethodNotAllowed(
    struct mg_connection* connection, const struct mg_request_info* request_info, const std::string& allowed_methods, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpMethodNotAllowed, " << body.size();
...
void Server::SendHttpTimeout(struct mg_connection* connection, const struct mg_request_info* request_info, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpTimeout, " << body.size();
...
void Server::SendHttpNotImplemented(struct mg_connection* connection, const struct mg_request_info* request_info, const std::string& body)
{
    WDLOG(TRACE) << "Entering Server::SendHttpNotImplemented";
...
void Server::SendHttpSeeOther(struct mg_connection* connection, const struct mg_request_info* request_info, const std::string& location)
{
    WDLOG(TRACE) << "Entering Server::SendHttpSeeOther";
...
std::string Server::LookupCommand(const std::string& uri, const std::string& http_verb, std::string* session_id, std::string* locator)
{
    ...
    for (; it != this->commands_.end(); ++it) {
        ...
        if (it->second->IsUriMatch(url_fragments, &locator_param_names, &locator_param_values)) {
            if (it->second->HasHttpVerb(http_verb, &value)) {
                std::string param = this->ConstructLocatorParameterJson(locator_param_names, locator_param_values, session_id);
...
std::string Server::ConstructLocatorParameterJson(
    std::vector<std::string> locator_param_names, std::vector<std::string> locator_param_values, std::string* session_id)
{
    std::string param = "{";
...
{
    WDLOG(TRACE) << "Entering Server::PopulateCommandRepository";
