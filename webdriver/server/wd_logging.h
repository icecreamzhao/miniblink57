...
public:
    ...
    enum LogLevel { logFATAL = 0, logERROR, logWARN, logINFO, logDEBUG, logTRACE };
    ...
    static std::string Time()
    {
        ...
        char time[26];
        size_t length = strftime(time, sizeof(time), "%Y-%m-%d %H:%M:%S:", localtime(reinterpret_cast<const time_t*>(&tb.time)));
...
class LoggerLOG : public Logger<LoggerLOG> {
public:
    static void File(const std::string& name, const char* openMode = "w")
    {
        const std::string& file = Name(name);
        if (file == "stdout") {
            LoggerLOG::File() = stdout;
        } else if (file == "stderr") {
            LoggerLOG::File() = stderr;
        } else {
            LoggerLOG::File() = fopen(file.c_str(), openMode);
        }
    }

    static void Limit(off_t size)
    {
        LoggerLOG::Limit() = size;
    }
...
//         }
        ...
        OutputDebugStringToFile(str.c_str());
    }

    friend class Logger<LoggerLOG>;
};
...
// #define WDLOG(LEVEL)                                                                                                                                             \
//     if (LoggerLOG::log##LEVEL > LoggerLOG::Level())                                                                                                                        \
//         ;                                                                                                                                                      \
//     else                                                                                                                                                       \
//         LoggerLOG().Stream(LoggerLOG::log##LEVEL) << __FILE__ << "(" << __LINE__ << ") " /* << stuff here */

#define WDLOG(LEVEL) \
    LoggerLOG().Stream(LoggerLOG::log##LEVEL) << __FILE__ << "(" << __LINE__ << ") "

#ifdef _WIN32
#define LOGHR(LEVEL, HR) WDLOG(##LEVEL) << HR << " [" << (_bstr_t(_com_error((DWORD)HR).ErrorMessage())) << "]: "
#define LOGERR(LEVEL) WDLOG(##LEVEL) << " [Windows Error " << (::GetLastError()) << "]: "
#define LOGWSTRING(STR) _bstr_t((##STR).c_str())
#endif

#define WDCHECK(condition) \
    do { \
        if (!(condition)) \
        *(int*)1 = 1; \
    } while (0); \
