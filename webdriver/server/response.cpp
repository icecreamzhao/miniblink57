...
Response::Response(void)
    ...
    , additional_data_(Json::Value::null)
{
}
...
void Response::Deserialize(const std::string& json)
{
    ...
    Json::parseFromStream(Json::CharReaderBuilder(), json_stream, &response_object, &parse_errors);
...
std::string trimJson(const std::string& json)
{
    std::string result;
...
std::string Response::Serialize(void)
{
    ...
    return trimJson(output);
}

void Response::SetSuccessResponse(const Json::Value& response_value)
{
    WDLOG(TRACE) << "Entering Response::SetSuccessResponse";
    this->SetResponse("", response_value);
}

void Response::SetResponse(const std::string& error, const Json::Value& response_value)
{
    WDLOG(TRACE) << "Entering Response::SetResponse";
    this->error_ = error;
...
void Response::AddAdditionalData(const std::string& data_name, const std::string& data_value)
{
    WDLOG(TRACE) << "Entering Response::AddAdditionalData";
...
int Response::GetHttpResponseCode(void)
{
    ...
    if (this->error_ == ERROR_ELEMENT_CLICK_INTERCEPTED || this->error_ == ERROR_ELEMENT_NOT_SELECTABLE || this->error_ == ERROR_ELEMENT_NOT_INTERACTABLE
        || this->error_ == ERROR_INSECURE_CERTIFICATE || this->error_ == ERROR_INVALID_ARGUMENT || this->error_ == ERROR_INVALID_COOKIE_DOMAIN
        || this->error_ == ERROR_INVALID_COORDINATES || this->error_ == ERROR_INVALID_ELEMENT_STATE || this->error_ == ERROR_INVALID_SELECTOR) {
        response_code = 400;
    } else if (this->error_ == ERROR_INVALID_SESSION_ID || this->error_ == ERROR_NO_SUCH_COOKIE || this->error_ == ERROR_NO_SUCH_ALERT
        || this->error_ == ERROR_NO_SUCH_ELEMENT || this->error_ == ERROR_NO_SUCH_FRAME || this->error_ == ERROR_NO_SUCH_WINDOW
        || this->error_ == ERROR_STALE_ELEMENT_REFERENCE || this->error_ == ERROR_UNKNOWN_COMMAND) {
        response_code = 404;
    } else if (this->error_ == ERROR_UNKNOWN_METHOD) {
        response_code = 405;
    } else if (this->error_ == ERROR_JAVASCRIPT_ERROR || this->error_ == ERROR_MOVE_TARGET_OUT_OF_BOUNDS || this->error_ == ERROR_SCRIPT_TIMEOUT
        || this->error_ == ERROR_SESSION_NOT_CREATED || this->error_ == ERROR_UNABLE_TO_SET_COOKIE || this->error_ == ERROR_UNABLE_TO_CAPTURE_SCREEN
        || this->error_ == ERROR_UNEXPECTED_ALERT_OPEN || this->error_ == ERROR_UNKNOWN_ERROR || this->error_ == ERROR_UNSUPPORTED_OPERATION
