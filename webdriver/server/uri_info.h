...
class UriInfo {
public:
    UriInfo(const std::string& uri, const std::string& verb, const std::string& command_name);

    static void ParseUri(const std::string& uri, std::vector<std::string>* fragments, std::vector<size_t>* parameter_indexes);

    bool IsUriMatch(const std::vector<std::string>& uri_fragments, std::vector<std::string>* uri_param_names, std::vector<std::string>* uri_param_values);
    bool HasHttpVerb(const std::string& http_verb, std::string* command_name);
    std::string GetSupportedVerbs(void);
    void AddHttpVerb(const std::string& http_verb, const std::string& command_name);

private:
    typedef std::map<std::string, std::string> HttpVerbMap;

    bool IsValidParameterValue(const std::string& param_name, const std::string& param_value);
