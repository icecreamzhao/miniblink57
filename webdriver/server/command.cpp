...
Command::Command()
    : command_type_(webdriver::CommandType::NoCommand)
    , session_id_("")
{
}
...
void Command::Deserialize(const std::string& json)
{
    ...
    WDLOG(DEBUG) << "Raw JSON command: " << json;

    Json::Value root;
    std::string parse_errors;
    std::stringstream json_stream;
    json_stream.str(json);
    bool successful_parse = Json::parseFromStream(Json::CharReaderBuilder(), json_stream, &root, &parse_errors);

    if (!successful_parse) {
        // report to the user the failure and their locations in the document.
        WDLOG(WARN) << "Failed to parse configuration due to " << parse_errors << std::endl << "JSON command: '" << json << "'";
    }
    ...
    if (this->command_type_ != webdriver::CommandType::NoCommand) {
        ...
        WDLOG(DEBUG) << "Command type is zero, no 'name' attribute in JSON object";
    }
}

std::string Command::Serialize()
{
    WDLOG(TRACE) << "Entering Command::Serialize";
