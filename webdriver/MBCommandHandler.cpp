...
//#include "Element.h"
#include "MBCommandExecutor.h"

namespace webdriver {

MBCommandHandler::MBCommandHandler()
{
}
...
void MBCommandHandler::ExecuteInternal(const MBCommandExecutor& executor, const ParametersMap& command_parameters, Response* response)
{
    //LOG(TRACE) << "Entering MBCommandHandler::ExecuteInternal";
    response->SetErrorResponse(501, "Command not implemented");
}

int MBCommandHandler::GetElement(const MBCommandExecutor& executor, const std::string& element_id, ElementHandle* element_wrapper)
{
    DebugBreak();
    return EOBSOLETEELEMENT;

//     LOG(TRACE) << "Entering MBCommandHandler::GetElement";
...
Json::Value MBCommandHandler::RecreateJsonParameterObject(const ParametersMap& command_parameters)
{
    Json::Value result;
