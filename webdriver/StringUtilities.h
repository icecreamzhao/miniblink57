...
//     static void DecomposeUnicodeString(std::wstring* input);
    ...
    static void Split(const std::string& input, const std::string& delimiter, std::vector<std::string>* tokens);
    static void Split(const std::wstring& input, const std::wstring& delimiter, std::vector<std::wstring>* tokens);

private:
    //static void NormalizeUnicodeString(NORM_FORM normalization_form, std::wstring* input);
};
