...
#define MOVE_ERROR_TEMPLATE                                                                                                                                    \
    "The requested mouse movement to (%d, %d) would be outside the bounds of the current view port (left: %d, right: %d, top: %d, bottom: %d)"
...
int InputManager::PerformInputSequence(BrowserHandle browser_wrapper, const Json::Value& sequences, std::string* error_info)
{
    ...
    if (this->inputs_.size() > 0) {
        ...
        this->action_simulator_->SimulateActions(browser_wrapper, this->inputs_, &this->current_input_state_);
    }
...
int InputManager::GetTicks(const Json::Value& sequences, Json::Value* ticks)
{
    for (size_t i = 0; i < sequences.size(); ++i) {
        ...
        for (size_t j = 0; j < actions.size(); ++j) {
            ...
            if (action.isMember("type") && action["type"].isString() && action["type"].asString() == "pause") {
                if (action.isMember("duration") && (action["duration"].type() != Json::ValueType::intValue || action["duration"].asInt() < 0)) {
                    return EINVALIDARGUMENT;
                }
                if (device_type == "key") {
...
void InputManager::Reset(BrowserHandle browser_wrapper)
{
    ...
    for (; it != local_pressed_keys.rend(); ++it) {
        ...
        if (*it == WD_MOUSE_LBUTTON) {
            ...
            this->AddKeyboardInput(browser_window_handle, key_value, true, &current_input_state);
        }
    }
    ...
    if (this->current_input_state_.mouse_x > 0 || this->current_input_state_.mouse_y > 0) {
...
int InputManager::PointerMoveTo(BrowserHandle browser_wrapper, const Json::Value& move_to_action, InputState* input_state)
{
    ...
    if (this->action_simulator_->UseExtraInfo()) {
        ...
        if (origin == "viewport") {
            ...
            if (offset_specified) {
                ...
                if (temp_x > INT_MAX || temp_x < INT_MIN) {
                    input_state->error_info = StringUtilities::Format(MOVE_OVERFLOW_ERROR_TEMPLATE, "x coordinate", temp_x);
                ...
                if (temp_y > INT_MAX || temp_y < INT_MIN) {
                    input_state->error_info = StringUtilities::Format(MOVE_OVERFLOW_ERROR_TEMPLATE, "y coordinate", temp_y);
                    return EMOVETARGETOUTOFBOUNDS;
                }
        ...
        LOG(DEBUG) << "Queueing SendInput structure for mouse move (origin: " << origin << ", x: " << end_x << ", y: " << end_y << ")";
        HWND browser_window_handle = browser_wrapper->GetContentWindowHandle();
        RECT window_rect;
        ...
        if (click_point.x < window_rect.left || click_point.x > window_rect.right || click_point.y < window_rect.top || click_point.y > window_rect.bottom) {
            input_state->error_info
                = StringUtilities::Format(MOVE_ERROR_TEMPLATE, end_x, end_y, window_rect.left, window_rect.right, window_rect.top, window_rect.bottom);
            LOG(WARN) << input_state->error_info;
            return EMOVETARGETOUTOFBOUNDS;
        }
        if (end_x == input_state->mouse_x && end_y == input_state->mouse_y) {
            LOG(DEBUG) << "Omitting SendInput structure for mouse move; no movement required (x: " << end_x << ", y: " << end_y << ")";
        } else {
...
bool InputManager::IsSingleKey(const std::wstring& input)
{
    ...
    if (composed_input.size() > 1) {
        ...
        BOOL get_type_success = ::GetStringTypeW(CT_CTYPE3, input.c_str(), static_cast<int>(input.size()), &char_types[0]);
        if (get_type_success) {
...
void InputManager::AddKeyboardInput(HWND window_handle, std::wstring key, bool key_up, InputState* input_state)
{
    ...
    LOG(DEBUG) << "Queueing SendInput structure for " << log_event << " (key: " << LOGWSTRING(log_key) << ")";
    ...
    if (character == WD_KEY_NULL) {
        ...
        for (; it != modifier_key_codes.end(); ++it) {
            ...
            this->CreateKeyboardInputItem(modifier_key_info, KEYEVENTF_SCANCODE, true);
        }
        return;
    }
    ...
    if (this->IsModifierKey(character)) {
        ...
        if (character == WD_KEY_SHIFT || character == WD_KEY_R_SHIFT) {
            ...
            modifier_key_info.scan_code = scan_code;
            this->CreateKeyboardInputItem(modifier_key_info, KEYEVENTF_SCANCODE, input_state->is_shift_pressed);
            if (input_state->is_shift_pressed) {
                input_state->is_shift_pressed = false;
            } else {
        ...
        if (character == WD_KEY_CONTROL || character == WD_KEY_R_CONTROL) {
            ...
            modifier_key_info.scan_code = scan_code;
            this->CreateKeyboardInputItem(modifier_key_info, KEYEVENTF_SCANCODE, input_state->is_control_pressed);
            if (input_state->is_control_pressed) {
                input_state->is_control_pressed = false;
            } else {
        ...
        if (character == WD_KEY_ALT || character == WD_KEY_R_ALT) {
            ...
            modifier_key_info.scan_code = scan_code;
            this->CreateKeyboardInputItem(modifier_key_info, KEYEVENTF_SCANCODE, input_state->is_alt_pressed);
            if (input_state->is_alt_pressed) {
                input_state->is_alt_pressed = false;
            } else {
        ...
        if (character == WD_KEY_META || character == WD_KEY_R_META) {
    ...
    if (modifier_key_info != 0) {
        ...
        bool is_shift_required = (modifier_key_info & MODIFIER_KEY_SHIFT) != 0 && !input_state->is_shift_pressed;
        bool is_control_required = (modifier_key_info & MODIFIER_KEY_CTRL) != 0 && !input_state->is_control_pressed;
        bool is_alt_required = (modifier_key_info & MODIFIER_KEY_ALT) != 0 && !input_state->is_alt_pressed;
        if (!key_up) {
...
{
    return std::find(this->pressed_keys_.begin(), this->pressed_keys_.end(), character) != this->pressed_keys_.end();
}
...
void InputManager::CreateKeyboardInputItem(KeyInfo key_info, DWORD initial_flags, bool is_generating_key_up)
{
    INPUT input_element;
    input_element.type = INPUT_KEYBOARD;
...
{
    return character == WD_KEY_SHIFT || character == WD_KEY_CONTROL || character == WD_KEY_ALT || character == WD_KEY_META || character == WD_KEY_R_SHIFT
        || character == WD_KEY_R_CONTROL || character == WD_KEY_R_ALT || character == WD_KEY_R_META || character == WD_KEY_NULL;
}
