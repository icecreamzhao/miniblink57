...
void ProxyManager::Initialize(ProxySettings settings)
{
    ...
    std::transform(this->proxy_type_.begin(), this->proxy_type_.end(), this->proxy_type_.begin(), ::tolower);
    ...
    if (this->proxy_type_ == WD_PROXY_TYPE_SYSTEM || this->proxy_type_ == WD_PROXY_TYPE_DIRECT || this->proxy_type_ == WD_PROXY_TYPE_MANUAL) {
...
void ProxyManager::RestoreProxySettings()
{
    ...
    if (settings_restored) {
        BOOL notify_success = ::InternetSetOption(NULL, INTERNET_OPTION_PROXY_SETTINGS_CHANGED, NULL, 0);
        if (!notify_success) {
...
void ProxyManager::SetPerProcessProxySettings(HWND browser_window_handle)
{
    ...
    hook.PushData(proxy);
    LRESULT result = ::SendMessage(browser_window_handle, WD_CHANGE_PROXY, NULL, NULL);

    if (this->socks_proxy_.size() > 0 && (this->socks_user_name_.size() > 0 || this->socks_password_.size() > 0)) {
        LOG(WARN) << "Windows APIs provide no way to set proxy user name and "
                  << "password on a per-process basis. Setting global setting.";

        this->SetProxyAuthentication(StringUtilities::ToWString(this->socks_user_name_), StringUtilities::ToWString(this->socks_password_));
        this->is_proxy_authorization_modified_ = true;

        // Notify WinINet clients that the proxy options have changed.
        BOOL success = ::InternetSetOption(NULL, INTERNET_OPTION_PROXY_SETTINGS_CHANGED, NULL, 0);
        if (!success) {
            LOGERR(WARN) << "InternetSetOption failed setting INTERNET_OPTION_PROXY_SETTINGS_CHANGED";
        }
    }
...
void ProxyManager::SetGlobalProxySettings()
{
    ...
    StringUtilities::Split(proxy_settings_string, _TEXT(BYPASS_PROXY_MARKER), &proxy_settings);
...
void ProxyManager::SetProxyAuthentication(const std::wstring& user_name, const std::wstring& password)
{
    ...
    BOOL success = ::InternetSetOption(NULL, INTERNET_OPTION_PROXY_USERNAME, const_cast<wchar_t*>(user_name.c_str()), static_cast<int>(user_name.size()) + 1);
    if (!success) {
        LOGERR(WARN) << "InternetSetOption failed setting INTERNET_OPTION_PROXY_USERNAME";
    }

    success = ::InternetSetOption(NULL, INTERNET_OPTION_PROXY_PASSWORD, const_cast<wchar_t*>(password.c_str()), static_cast<int>(password.size()) + 1);
    if (!success) {
...
LRESULT CALLBACK SetProxyWndProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (WM_COPYDATA == call_window_proc_struct->message) {
        ...
        if (proxy == L"direct") {
            ...
            ::WideCharToMultiByte(CP_UTF8, 0, proxy.c_str(), -1, &multibyte_buffer[0], static_cast<int>(multibyte_buffer.size()), NULL, NULL);
            proxy_info.dwAccessType = INTERNET_OPEN_TYPE_PROXY;
            proxy_info.lpszProxy = reinterpret_cast<LPCTSTR>(&multibyte_buffer[0]);
        }

        ::WideCharToMultiByte(CP_UTF8, 0, proxy_bypass.c_str(), -1, &multibyte_buffer_bypass[0], static_cast<int>(multibyte_buffer_bypass.size()), NULL, NULL);
        proxy_info.lpszProxyBypass = reinterpret_cast<LPCTSTR>(&multibyte_buffer_bypass[0]);
        DWORD proxy_info_size = sizeof(proxy_info);
        HRESULT hr = ::UrlMkSetSessionOption(INTERNET_OPTION_PROXY, reinterpret_cast<void*>(&proxy_info), proxy_info_size, 0);
    }
