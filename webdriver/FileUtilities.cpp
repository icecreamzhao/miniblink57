...
std::string FileUtilities::GetFileVersion(const std::wstring& file_name)
{
    ...
    ::GetFileVersionInfo(file_name.c_str(), 0, /* ignored */
        length, &version_buffer[0]);

    UINT page_count;
    BOOL query_result = ::VerQueryValue(&version_buffer[0], FILE_LANGUAGE_INFO, reinterpret_cast<void**>(&language_info), &page_count);

    std::vector<wchar_t> sub_block(MAX_PATH);
    _snwprintf_s(&sub_block[0], MAX_PATH, MAX_PATH, FILE_VERSION_INFO, language_info->language, language_info->code_page);
    std::wstring sub_block_string = &sub_block[0];

    void* value = NULL;
    unsigned int size;
    query_result = ::VerQueryValue(&version_buffer[0], sub_block_string.c_str(), &value, &size);
