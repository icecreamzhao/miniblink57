...
public:
    ...
    int GetCookies(const std::string& url, std::vector<BrowserCookie>* all_cookies);
...
private:
    std::wstring SendGetCookieMessage(const std::wstring& url, const unsigned int message, HookProcessor* hook);
    void ParseCookieString(const std::wstring& cookie_string, std::map<std::string, std::string>* cookies);
    BrowserCookie ParseSingleCookie(const std::string& cookie);
    void ParseCookieList(const std::string& cookie_file_contents, const bool include_secure_cookies, std::map<std::string, BrowserCookie>* cookies);
