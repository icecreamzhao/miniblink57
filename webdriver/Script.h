...
public:
    ...
    Script(IHTMLDocument2* document, std::string script_source);
    Script(IHTMLDocument2* document, std::wstring script_source);
    ~Script(void);

    std::wstring source_code() const
    {
        return this->source_code_;
    }
    VARIANT result()
    {
        return this->result_;
    ...
    int Execute(void);
    int Execute(const IECommandExecutor& command_executor, const Json::Value& args, Json::Value* result);
    int ExecuteAsync(const IECommandExecutor& command_executor, const Json::Value& args, HWND* async_executor_handle);
    int ExecuteAsync(
    ...
    int ConvertResultToJsonValue(IElementManager* element_manager, Json::Value* value);

    std::wstring polling_source_code(void) const
    {
        return this->polling_source_code_;
    }
    void set_polling_source_code(const std::wstring& value)
    {
        this->polling_source_code_ = value;
    }

private:
    bool CreateAnonymousFunction(VARIANT* result);
    int CreateAsyncScriptExecutor(HWND element_repository_handle, const std::wstring& serialized_args, HWND* async_executor_handle);
    int SetAsyncScriptDocument(HWND async_executor_handle);
    int SetAsyncScriptElementArgument(HWND async_executor_handle, const IECommandExecutor& command_executor, const std::string& element_id);
    void Initialize(IHTMLDocument2* document, const std::wstring& script_source, const unsigned long argument_count);

    int AddArgument(IElementManager* element_manager, const Json::Value& arg);
    int WalkArray(IElementManager* element_manager, const Json::Value& array_value);
    int WalkObject(IElementManager* element_manager, const Json::Value& object_value);
