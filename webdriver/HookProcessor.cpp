...
public:
    ...
    LRESULT CopyData(HWND destination_window_handle, int data_size, void* pointer_to_data)
    {
        ...
        LRESULT result = ::SendMessage(destination_window_handle, WM_COPYDATA, reinterpret_cast<WPARAM>(this->m_hWnd), reinterpret_cast<LPARAM>(&data));
        return result;
    }
};
...
void HookProcessor::Initialize(const std::string& hook_procedure_name, const int hook_procedure_type)
{
...
void HookProcessor::Initialize(const HookSettings& settings)
{
    ...
    bool is_hook_installed = this->InstallWindowsHook(settings.hook_procedure_name, settings.hook_procedure_type);
}
...
bool HookProcessor::CanSetWindowsHook(HWND window_handle)
{
    ...
    if (driver_bitness != browser_bitness) {
        LOG(WARN) << "Unable to set Windows hook procedure. Driver is a " << driver_bitness << "-bit process, but browser is a " << browser_bitness
                  << "-bit process.";
    }
...
bool HookProcessor::InstallWindowsHook(const std::string& hook_proc_name, const int hook_proc_type)
{
    LOG(TRACE) << "Entering HookProcessor::InstallWindowsHook";

    HINSTANCE instance_handle = _AtlBaseModule.GetModuleInstance();

    FARPROC hook_procedure_address = ::GetProcAddress(instance_handle, hook_proc_name.c_str());
    if (hook_procedure_address == NULL) {
        LOGERR(WARN) << "Unable to get address of hook procedure named " << hook_proc_name;
        return false;
    }
    ...
    this->hook_procedure_handle_ = ::SetWindowsHookEx(hook_proc_type, hook_procedure, instance_handle, thread_id);
...
void HookProcessor::CreateReturnPipe()
{
    ...
    std::wstring pipe_name = StringUtilities::Format(PIPE_NAME_TEMPLATE, ::GetCurrentProcessId());
    ...
    BOOL descriptor_created = ::ConvertStringSecurityDescriptorToSecurityDescriptor(LOW_INTEGRITY_SDDL_SACL, SDDL_REVISION_1, &pointer_to_descriptor, NULL);
    ...
    this->pipe_handle_ = ::CreateNamedPipe(pipe_name.c_str(), PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
        PIPE_UNLIMITED_INSTANCES, 0, NAMED_PIPE_BUFFER_SIZE, PIPE_CONNECTION_TIMEOUT_IN_MILLISECONDS, security_attributes_pointer);
...
void HookProcessor::Dispose()
{
    ...
    if (this->pipe_handle_ != INVALID_HANDLE_VALUE && this->pipe_handle_ != NULL) {
...
bool HookProcessor::PushData(int data_size, void* pointer_to_data)
{
    ...
    LRESULT result = holder.CopyData(this->window_handle_, data_size, pointer_to_data);
...
int HookProcessor::PullData(std::vector<char>* data)
{
    ...
    if (is_connected) {
        ...
        BOOL is_read_successful = ::ReadFile(this->pipe_handle_, &buffer[0], NAMED_PIPE_BUFFER_SIZE, &bytes_read, NULL);
        while (!is_read_successful && ERROR_MORE_DATA == ::GetLastError()) {
            data->insert(data->end(), buffer.begin(), buffer.begin() + bytes_read);
            is_read_successful = ::ReadFile(this->pipe_handle_, &buffer[0], NAMED_PIPE_BUFFER_SIZE, &bytes_read, NULL);
        }
...
void HookProcessor::CopyDataToBuffer(int source_data_size, void* source)
{
    ...
    memcpy_s(data_buffer, data_buffer_size, source, data_buffer_size);
}
...
void HookProcessor::WriteBufferToPipe(const int process_id)
{
    ...
    BOOL is_pipe_available = ::WaitNamedPipe(pipe_name.c_str(), PIPE_CONNECTION_TIMEOUT_IN_MILLISECONDS);
    if (is_pipe_available) {
        pipe_handle = ::CreateFile(pipe_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    }
    ...
    if (INVALID_HANDLE_VALUE != pipe_handle) {
        ...
        if (::SetNamedPipeHandleState(pipe_handle, &pipe_mode, NULL, NULL)) {
            ...
            BOOL is_write_successful = ::WriteFile(pipe_handle, GetDataBufferAddress(), GetDataBufferSize(), &bytes_written, NULL);
