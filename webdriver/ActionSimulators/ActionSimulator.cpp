...
void ActionSimulator::UpdateInputState(INPUT current_input, InputState* input_state)
{
    if (current_input.type == INPUT_MOUSE) {
        if (current_input.mi.dwFlags & MOUSEEVENTF_MOVE) {
            ...
            if (input_state->is_left_button_pressed && input_state->mouse_x == current_input.mi.dx && input_state->mouse_y == current_input.mi.dy) {
