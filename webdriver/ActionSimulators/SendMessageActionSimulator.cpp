...
int SendMessageActionSimulator::SimulateActions(BrowserHandle browser_wrapper, std::vector<INPUT> inputs, InputState* input_state)
{
    ...
    for (; input_iterator != inputs.end(); ++input_iterator) {
        ...
        if (current_input.type == INPUT_MOUSE) {
            if (current_input.mi.dwFlags & MOUSEEVENTF_MOVE) {
                this->SendMouseMoveMessage(window_handle, *input_state, current_input.mi.dx, current_input.mi.dy);
            } else if (current_input.mi.dwFlags & MOUSEEVENTF_LEFTDOWN) {
                bool is_double_click = this->IsInputDoubleClick(current_input, *input_state);
                this->SendMouseDownMessage(window_handle, *input_state, WD_CLIENT_LEFT_MOUSE_BUTTON, current_input.mi.dx, current_input.mi.dy, is_double_click);
            } else if (current_input.mi.dwFlags & MOUSEEVENTF_LEFTUP) {
                this->SendMouseUpMessage(window_handle, *input_state, WD_CLIENT_LEFT_MOUSE_BUTTON, current_input.mi.dx, current_input.mi.dy);
            } else if (current_input.mi.dwFlags & MOUSEEVENTF_RIGHTDOWN) {
                bool is_double_click = this->IsInputDoubleClick(current_input, *input_state);
                this->SendMouseDownMessage(
                    window_handle, *input_state, WD_CLIENT_RIGHT_MOUSE_BUTTON, current_input.mi.dx, current_input.mi.dy, is_double_click);
            } else if (current_input.mi.dwFlags & MOUSEEVENTF_RIGHTUP) {
                this->SendMouseUpMessage(window_handle, *input_state, WD_CLIENT_RIGHT_MOUSE_BUTTON, current_input.mi.dx, current_input.mi.dy);
            }
            ...
            if (current_input.ki.dwFlags & KEYEVENTF_KEYUP) {
                this->SendKeyUpMessage(
                    window_handle, *input_state, current_input.ki.wVk, current_input.ki.wScan, extended, unicode, layout, &this->keyboard_state_buffer_);
            } else {
                this->SendKeyDownMessage(
                    window_handle, *input_state, current_input.ki.wVk, current_input.ki.wScan, extended, unicode, layout, &this->keyboard_state_buffer_);
            }
...
bool SendMessageActionSimulator::IsInputDoubleClick(INPUT current_input, InputState input_state)
{
    ...
    if (!button_pressed && input_state.mouse_x == current_input.mi.dx && input_state.mouse_y == current_input.mi.dy
...
void SendMessageActionSimulator::SendKeyDownMessage(
    HWND window_handle, InputState input_state, int key_code, int scan_code, bool extended, bool unicode, HKL layout, std::vector<BYTE>* keyboard_state)
{
    LPARAM lparam = 0;
...
void SendMessageActionSimulator::SendKeyUpMessage(
    HWND window_handle, InputState input_state, int key_code, int scan_code, bool extended, bool unicode, HKL layout, std::vector<BYTE>* keyboard_state)
{
    LPARAM lparam = 0;
...
void SendMessageActionSimulator::SendMouseMoveMessage(HWND window_handle, InputState input_state, int x, int y)
{
...
void SendMessageActionSimulator::SendMouseDownMessage(HWND window_handle, InputState input_state, int button, int x, int y, bool is_double_click)
{
...
void SendMessageActionSimulator::SendMouseUpMessage(HWND window_handle, InputState input_state, int button, int x, int y)
{
