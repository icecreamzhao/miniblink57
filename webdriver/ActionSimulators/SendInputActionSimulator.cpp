...
int SendInputActionSimulator::SimulateActions(BrowserHandle browser_wrapper, std::vector<INPUT> inputs, InputState* input_state)
{
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        ...
        if (current_input.type == INPUT_HARDWARE && current_input.hi.uMsg > 0) {
            ...
            this->GetNormalizedCoordinates(window_handle, current_input.mi.dx, current_input.mi.dy, &normalized_x, &normalized_y);
            ...
            // SendInput.
            unsigned long normalized_flags = this->NormalizeButtons(is_button_swapped, current_input.mi.dwFlags);
    ...
    for (; it != sleep_indexes.end(); ++it) {
        ...
        if (number_of_inputs > 0) {
            this->SendInputToBrowser(browser_wrapper, inputs, static_cast<int>(next_input_index), static_cast<int>(number_of_inputs));
        }
    ...
    if (last_inputs > 0) {
        this->SendInputToBrowser(browser_wrapper, inputs, static_cast<int>(next_input_index), static_cast<int>(last_inputs));
    }
...
void SendInputActionSimulator::SendInputToBrowser(BrowserHandle browser_wrapper, std::vector<INPUT> inputs, int start_index, int input_count)
{
    if (input_count > 0) {
...
void SendInputActionSimulator::GetNormalizedCoordinates(HWND window_handle, int x, int y, int* normalized_x, int* normalized_y)
{
...
unsigned long SendInputActionSimulator::NormalizeButtons(bool is_button_swapped, unsigned long input_flags)
{
...
bool SendInputActionSimulator::WaitForInputEventProcessing(int input_count)
{
    ...
    LOG(DEBUG) << "Requested waiting for " << input_count << " events, processed " << processed_event_count << " events,"
               << " timed out after " << total_timeout_in_milliseconds << " milliseconds";
