...
public:
    ...
    int SimulateActions(BrowserHandle browser_wrapper, std::vector<INPUT> inputs, InputState* input_state);

private:
    void SendKeyDownMessage(
    ...
    void SendMouseMoveMessage(HWND window_handle, InputState input_state, int x, int y);
    void SendMouseUpMessage(HWND window_handle, InputState input_state, int button, int x, int y);
    void SendMouseDownMessage(HWND window_handle, InputState input_state, int button, int x, int y, bool is_double_click);
