...
namespace draw_property_utils {
    ...
    namespace {
        ...
        static void ValidateRenderSurfaceForLayer(LayerImpl* layer)
        {
            ...
            EffectNode* effect_node = layer->layer_tree_impl()->property_trees()->effect_tree.Node(
            ...
            gfx::RectF* clip_rect_in_target_space)
        {
            ...
            const EffectNode* target_effect_node = ContentsTargetEffectNode(layer->effect_tree_index(), effect_tree);
            gfx::Transform clip_to_target;
            // We use the local clip for clip rect calculation and combined clip for
            // visible rect calculation.
            gfx::RectF clip_from_clip_node = for_visible_rect_calculation ? clip_node->combined_clip_in_target_space
            ...
            if (clip_node->target_transform_id > target_node_id) {
                ...
                if (property_trees->GetToTarget(clip_node->target_transform_id,
                    ...
                    const EffectNode* source_node = effect_tree.Node(clip_node->target_effect_id);
                    ConcatInverseSurfaceContentsScale(source_node, &clip_to_target);
                    *clip_rect_in_target_space = MathUtil::MapClippedRect(clip_to_target, clip_from_clip_node);
                } else {
                    return false;
                }
            } else {
                if (property_trees->GetFromTarget(
                    ...
                    *clip_rect_in_target_space = MathUtil::ProjectClippedRect(clip_to_target, clip_from_clip_node);
                } else {
                    return false;
                }
            }
            ...
            int target_effect_id)
        {
            ...
            gfx::Vector2dF surface_contents_scale = effect_tree.Node(target_effect_id)->surface_contents_scale;
            // The viewport clip should not be scaled.
            if (surface_contents_scale.x() > 0 && surface_contents_scale.y() > 0 && clip_node->transform_id != TransformTree::kRootNodeId)
            ...
            gfx::RectF* accumulated_clip)
        {
            ...
            case ClipNode::ClipType::APPLIES_LOCAL_CLIP: {
                ...
                *accumulated_clip = gfx::IntersectRects(*accumulated_clip, current_clip.clip_rect);
                return true;
            }
            case ClipNode::ClipType::EXPANDS_CLIP: {
                ...
                const EffectNode* expanding_effect_node = property_trees->effect_tree.Node(
                    clip_node->clip_expander->target_effect_id());
                ConditionalClip accumulated_clip_in_expanding_space = ConvertRectBetweenSurfaceSpaces(
                ...
                // Do the expansion.
                gfx::RectF expanded_clip_in_expanding_space = gfx::RectF(clip_node->clip_expander->MapRectReverse(
                    gfx::ToEnclosingRect(
                        accumulated_clip_in_expanding_space.clip_rect),
                    property_trees));

                // Put the expanded clip back into the original target space.
                ConditionalClip expanded_clip_in_target_space = ConvertRectBetweenSurfaceSpaces(
        ...
        static ConditionalClip ComputeAccumulatedClip(
            ...
            int target_id)
        {
            DCHECK(!include_viewport_clip || target_id == EffectTree::kContentsRootNodeId);
            ...
            while (target_node->clip_id > clip_node->id || target_node->has_unclipped_descendants) {
            ...
            while (clip_node->clip_type != ClipNode::ClipType::APPLIES_LOCAL_CLIP && parent_chain.size() > 0) {
                clip_node = parent_chain.top();
                parent_chain.pop();
            }
            ...
            return ConditionalClip { true /* is_clipped */, accumulated_clip.IsEmpty() ? gfx::RectF() : accumulated_clip };
        }
            ...
            bool non_root_surfaces_enabled)
        {
            ...
            for (auto& layer : visible_layer_list) {
                ...
                bool layer_needs_clip_rect = non_root_surfaces_enabled
            ...
            bool non_root_surfaces_enabled)
        {
            ...
            for (auto& layer : visible_layer_list) {
                ...
                int effect_ancestor_with_copy_request = effect_tree.ClosestAncestorWithCopyRequest(layer->effect_tree_index());
                if (effect_ancestor_with_copy_request > EffectTree::kContentsRootNodeId) {
                    ...
                    gfx::RectF accumulated_clip_in_copy_request_space = accumulated_clip_rect.clip_rect;

                    const EffectNode* copy_request_effect_node = effect_tree.Node(effect_ancestor_with_copy_request);
                ...
                if (!non_root_surfaces_enabled) {
                    ...
                    if (transform_node->ancestors_are_invertible) {
                        gfx::RectF combined_clip_rect_in_target_space = clip_node->combined_clip_in_target_space;
                ...
                // the viewport clip.
                const bool fully_visible = !clip_node->layers_are_clipped && !effect_tree.Node(clip_node->target_effect_id)->surface_is_clipped;
                ...
                if (clip_node->target_transform_id != target_node_id) {
                    ...
                    combined_clip_rect_in_target_space = clip_node->combined_clip_in_target_space;
                }
                ...
                // visible.
                if (!layer_content_bounds_in_target_space.IsEmpty() && combined_clip_rect_in_target_space.Contains(layer_content_bounds_in_target_space)) {
                    layer->set_visible_layer_rect(gfx::Rect(layer_bounds));
                    continue;
                }
        ...
        static bool IsLayerBackFaceVisible(LayerType* layer,
            ...
            const PropertyTrees* property_trees)
        {
            const TransformNode* node = property_trees->transform_tree.Node(transform_tree_index);
            ...
            const PropertyTrees* property_trees)
        {
            ...
            if (layer->should_check_backface_visibility()) {
                int backface_transform_id = TransformTreeIndexForBackfaceVisibility(layer, tree);
                // A layer with singular transform is not drawn. So, we can assume that its
                // backface is not visible.
                if (TransformToScreenIsKnown(layer, backface_transform_id, tree) && !HasSingularTransform(backface_transform_id, tree) && IsLayerBackFaceVisible(layer, backface_transform_id, property_trees))
                    return false;
            }
            ...
            std::vector<LayerImpl*>* visible_layer_list)
        {
            ...
            for (auto* layer_impl : *layer_tree_impl) {
                if (!IsRootLayer(layer_impl) && LayerShouldBeSkipped(layer_impl, transform_tree, effect_tree))
                    continue;

                bool layer_is_drawn = effect_tree.Node(layer_impl->effect_tree_index())->is_drawn;
        ...
        const EffectTree& effect_tree)
    {
        ...
        if (effect_node->has_render_surface && effect_node->num_copy_requests_in_subtree > 0)
            return false;
        // If the layer transform is not invertible, it should be skipped.
        // TODO(ajuma): Correctly process subtrees with singular transform for the
        // case where we may animate to a non-singular transform and wish to
        // pre-raster.
        return !transform_node->node_and_ancestors_are_animated_or_invertible || effect_node->hidden_by_backface_visibility || !effect_node->is_drawn;
    }
        ...
        LayerList* update_layer_list)
    {
        ...
        for (auto* layer : *layer_tree) {
            if (!IsRootLayer(layer) && LayerShouldBeSkipped(layer, transform_tree, effect_tree))
                continue;

            bool layer_is_drawn = effect_tree.Node(layer->effect_tree_index())->is_drawn;
        ...
        bool non_root_surfaces_enabled)
    {
        ...
        for (int i = ClipTree::kViewportNodeId;
            ...
            const TransformTree& transform_tree = property_trees->transform_tree;
            const EffectTree& effect_tree = property_trees->effect_tree;
            const TransformNode* transform_node = transform_tree.Node(clip_node->transform_id);
            ClipNode* parent_clip_node = clip_tree->parent(clip_node);

            bool target_is_clipped = effect_tree.Node(clip_node->target_effect_id)->surface_is_clipped;

            gfx::Transform parent_to_current;
            const TransformNode* parent_target_transform_node = transform_tree.Node(parent_clip_node->target_transform_id);
            bool success = true;
            ...
            gfx::RectF parent_combined_clip_in_target_space = parent_clip_node->combined_clip_in_target_space;
            gfx::RectF parent_clip_in_target_space = parent_clip_node->clip_in_target_space;
            if (parent_target_transform_node && parent_target_transform_node->id != clip_node->target_transform_id && non_root_surfaces_enabled) {
                ...
                if (clip_node->clip_type == ClipNode::ClipType::EXPANDS_CLIP) {
                    parent_combined_clip_in_target_space = gfx::RectF(clip_node->clip_expander->MapRectReverse(
            ...
            if (clip_node->resets_clip && non_root_surfaces_enabled) {
                if (clip_node->clip_type == ClipNode::ClipType::APPLIES_LOCAL_CLIP) {
                    ...
                    clip_node->clip_in_target_space = MathUtil::MapClippedRect(to_target, clip_node->clip);
                    ResetIfHasNanCoordinate(&clip_node->clip_in_target_space);
                    clip_node->combined_clip_in_target_space = gfx::IntersectRects(clip_node->clip_in_target_space,
                        parent_combined_clip_in_target_space);
                } else {
                    DCHECK(!target_is_clipped);
                    DCHECK(!clip_node->layers_are_clipped);
                    clip_node->combined_clip_in_target_space = parent_combined_clip_in_target_space;
                }
                ResetIfHasNanCoordinate(&clip_node->combined_clip_in_target_space);
                continue;
            }
            bool use_only_parent_clip = clip_node->clip_type != ClipNode::ClipType::APPLIES_LOCAL_CLIP;
            if (use_only_parent_clip) {
                clip_node->combined_clip_in_target_space = parent_combined_clip_in_target_space;
                if (!non_root_surfaces_enabled) {
                    clip_node->clip_in_target_space = parent_clip_node->clip_in_target_space;
                ...
                } else if (transform_tree.ContentTargetId(transform_node->id) == clip_node->target_transform_id) {
                    property_trees->GetToTarget(clip_node->transform_id,
                ...
                gfx::RectF source_clip_in_target_space = MathUtil::MapClippedRect(source_to_target, clip_node->clip);
                ...
                bool layer_clipping_uses_only_local_clip = non_root_surfaces_enabled
    ...
    static void ComputeClipsWithEffectTree(PropertyTrees* property_trees)
    {
        ...
        const RenderSurfaceImpl* root_render_surface = root_effect_node->render_surface;
        gfx::Rect root_clip = gfx::ToEnclosingRect(clip_tree->Node(root_effect_node->clip_id)->clip);
        ...
        for (int i = 2; i < static_cast<int>(effect_tree->size()); ++i) {
            ...
            if (render_surface && render_surface->is_clipped()) {
                DCHECK(gfx::ToEnclosingRect(accumulated_clip) == render_surface->clip_rect())
        ...
        const LayerImpl* layer)
    {
        ...
        const ClipNode* clip_node = clip_tree->Node(layer->clip_tree_index());
        const EffectNode* effect_node = effect_tree->Node(layer->effect_tree_index());
        const EffectNode* target_node = effect_node->has_render_surface
            ? effect_node
            : effect_tree->Node(effect_node->target_id);
        ...
        bool is_clipped_from_clip_tree = property_trees->non_root_surfaces_enabled
            ? clip_node->layers_are_clipped
            : clip_node->layers_are_clipped_when_surfaces_disabled;
    ...
    static void ComputeVisibleRectsInternal(
        ...
        std::vector<LayerImpl*>* visible_layer_list)
    {
        if (property_trees->non_root_surfaces_enabled != can_render_to_separate_surface) {
        ...
        LayerImplList* visible_layer_list)
    {
        for (auto* layer : *root_layer->layer_tree_impl()) {
            ...
            EffectNode* node = property_trees->effect_tree.Node(layer->effect_tree_index());
    ...
    gfx::Rect ComputeLayerVisibleRectDynamic(const PropertyTrees* property_trees,
        const LayerImpl* layer)
    {
        int effect_ancestor_with_copy_request = property_trees->effect_tree.ClosestAncestorWithCopyRequest(
            layer->effect_tree_index());
        bool non_root_copy_request = effect_ancestor_with_copy_request > EffectTree::kContentsRootNodeId;
        ...
        if (non_root_copy_request) {
            ...
            accumulated_clip_in_root_space = ComputeAccumulatedClipInRootSpaceForVisibleRect(
                property_trees, layer->clip_tree_index());
        }

        const EffectNode* root_effect_node = non_root_copy_request
        ...
        ConditionalClip accumulated_clip_in_layer_space = ComputeTargetRectInLocalSpace(
    ...
    {
        for (auto layer : layer_list) {
            gfx::Rect visible_rect_dynamic = ComputeLayerVisibleRectDynamic(property_trees, layer);
        ...
        RenderSurfaceImpl* render_surface)
    {
        const TransformTree& transform_tree = property_trees->transform_tree;
        const EffectTree& effect_tree = property_trees->effect_tree;
        const TransformNode* transform_node = transform_tree.Node(render_surface->TransformTreeIndex());
        const EffectNode* effect_node = effect_tree.Node(render_surface->EffectTreeIndex());
        ...
        const EffectNode* target_effect_node = effect_tree.Node(effect_node->target_id);
        ...
        RenderSurfaceImpl* render_surface)
    {
        ...
        const TransformTree& transform_tree = property_trees->transform_tree;
        const TransformNode* transform_node = transform_tree.Node(render_surface->TransformTreeIndex());
        if (transform_tree.TargetId(transform_node->id) == parent_clip_node->target_transform_id) {
        ...
        if (parent_clip_node->target_transform_id < transform_tree.TargetId(transform_node->id)) {
    ...
    static float LayerDrawOpacity(const LayerImpl* layer, const EffectTree& tree)
    {
        ...
        const EffectNode* target_node = tree.Node(layer->render_target()->EffectTreeIndex());
        ...
        const PropertyTrees* property_trees)
    {
        ...
        layer->draw_properties().screen_space_transform = ScreenSpaceTransformInternal(layer, property_trees->transform_tree);
        layer->draw_properties().target_space_transform = DrawTransform(
            layer, property_trees->transform_tree, property_trees->effect_tree);
        layer->draw_properties().screen_space_transform_is_animating = transform_node->to_screen_is_potentially_animated;

        layer->draw_properties().opacity = LayerDrawOpacity(layer, property_trees->effect_tree);
        if (property_trees->non_root_surfaces_enabled) {
        ...
        const PropertyTrees* property_trees)
    {
        ...
        mask_layer->draw_properties().screen_space_transform = ScreenSpaceTransformInternal(mask_layer,
            property_trees->transform_tree);
        mask_layer->draw_properties().visible_layer_rect = gfx::Rect(mask_layer->bounds());
    }

    void ComputeSurfaceDrawProperties(const PropertyTrees* property_trees,
        RenderSurfaceImpl* render_surface)
    {
        const EffectNode* effect_node = property_trees->effect_tree.Node(render_surface->EffectTreeIndex());
