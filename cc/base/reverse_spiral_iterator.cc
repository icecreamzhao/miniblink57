    ...
    , vertical_step_count_(0)
{
    ...
    max_distance = std::max(max_distance, consider_index_rect_.bottom() - around_index_rect_.bottom());
    max_distance = std::max(
    ...
    horizontal_step_count_ = around_index_rect_.num_indices_x() + 2 * max_distance;
...
ReverseSpiralIterator& ReverseSpiralIterator::operator++()
{
    while (!around_index_rect_.Contains(index_x_, index_y_)) {
        ...
        if (around_index_rect_.Contains(index_x_, index_y_)) {
            ...
            switch (direction_) {
            case UP:
                if (consider_index_rect_.valid_column(index_x_) && consider_index_rect_.bottom() < index_y_)
                    steps_to_take = index_y_ - consider_index_rect_.bottom() - 1;
                break;
            case LEFT:
                if (consider_index_rect_.valid_row(index_y_) && consider_index_rect_.right() < index_x_)
                    steps_to_take = index_x_ - consider_index_rect_.right() - 1;
                break;
            case DOWN:
                if (consider_index_rect_.valid_column(index_x_) && consider_index_rect_.top() > index_y_)
                    steps_to_take = consider_index_rect_.top() - index_y_ - 1;
                break;
            case RIGHT:
                if (consider_index_rect_.valid_row(index_y_) && consider_index_rect_.left() > index_x_)
