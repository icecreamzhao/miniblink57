...
namespace {
    ...
    public:
        void SetUp() override
        {
            impl_task_runner_provider_ = base::WrapUnique(new FakeImplTaskRunnerProvider);
    ...
    TEST_F(MicroBenchmarkControllerTest, SendMessage)
    {
        ...
        bool message_handled = layer_tree_host_->SendMessageToMicroBenchmark(0, std::move(message));
        ...
        message->SetBoolean("can_handle", true);
        message_handled = layer_tree_host_->SendMessageToMicroBenchmark(id, std::move(message));
