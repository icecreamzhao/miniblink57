    ...
    cc::DrawQuad* out)
{
    ...
    quad->resources.ids[cc::RenderPassDrawQuad::kMaskResourceIdIndex] = data.mask_resource_id();
    ...
    if (!quad->render_pass_id)
        return false;
    return data.ReadMaskUvScale(&quad->mask_uv_scale) && data.ReadMaskTextureSize(&quad->mask_texture_size) && data.ReadFiltersScale(&quad->filters_scale) && data.ReadFiltersOrigin(&quad->filters_origin);
}
    ...
    cc::DrawQuad* out)
{
    ...
    quad->resources.ids[cc::StreamVideoDrawQuad::kResourceIdIndex] = data.resource_id();
    ...
    cc::DrawQuad* out)
{
    ...
    quad->resources.ids[cc::TextureDrawQuad::kResourceIdIndex] = data.resource_id();
    ...
    if (!data.ReadUvTopLeft(&quad->uv_top_left) || !data.ReadUvBottomRight(&quad->uv_bottom_right)) {
        return false;
    }
    ...
    cc::DrawQuad* out)
{
    ...
    if (!data.ReadTexCoordRect(&quad->tex_coord_rect) || !data.ReadTextureSize(&quad->texture_size)) {
        return false;
    }
    ...
    cc::DrawQuad* out)
{
    ...
    if (!data.ReadYaTexCoordRect(&quad->ya_tex_coord_rect) || !data.ReadUvTexCoordRect(&quad->uv_tex_coord_rect) || !data.ReadYaTexSize(&quad->ya_tex_size) || !data.ReadUvTexSize(&quad->uv_tex_size)) {
        return false;
    }
    quad->resources.ids[cc::YUVVideoDrawQuad::kYPlaneResourceIdIndex] = data.y_plane_resource_id();
    quad->resources.ids[cc::YUVVideoDrawQuad::kUPlaneResourceIdIndex] = data.u_plane_resource_id();
    quad->resources.ids[cc::YUVVideoDrawQuad::kVPlaneResourceIdIndex] = data.v_plane_resource_id();
    quad->resources.ids[cc::YUVVideoDrawQuad::kAPlaneResourceIdIndex] = data.a_plane_resource_id();
    static_assert(cc::YUVVideoDrawQuad::kAPlaneResourceIdIndex == cc::DrawQuad::Resources::kMaxResourceIdCount - 1,
    ...
    if (quad->bits_per_channel < cc::YUVVideoDrawQuad::kMinBitsPerChannel || quad->bits_per_channel > cc::YUVVideoDrawQuad::kMaxBitsPerChannel) {
        return false;
    }
    return true;
}
...
bool StructTraits<cc::mojom::DrawQuadDataView, cc::DrawQuad>::Read(
    ...
    cc::DrawQuad* out)
{
    if (!data.ReadRect(&out->rect) || !data.ReadOpaqueRect(&out->opaque_rect) || !data.ReadVisibleRect(&out->visible_rect)) {
        return false;
    }
