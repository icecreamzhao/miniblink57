...
int FakeLayerTreeHostImpl::RecursiveUpdateNumChildren(LayerImpl* layer)
{
    ...
    for (size_t i = 0; i < layer->test_properties()->children.size(); ++i) {
        num_children_that_draw_content += RecursiveUpdateNumChildren(layer->test_properties()->children[i]);
    }
    layer->test_properties()->num_descendants_that_draw_content = num_children_that_draw_content;
