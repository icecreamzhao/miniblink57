...
{
    scoped_refptr<PictureLayer> layer = PictureLayer::Create(content_layer_client);
    layer->SetLayerIdForTesting(engine_layer_id);
    return layer;
}
...
RemoteClientLayerFactory::CreateSolidColorScrollbarLayer(
    ...
    int scroll_layer_id)
{
    scoped_refptr<SolidColorScrollbarLayer> layer = SolidColorScrollbarLayer::Create(
...
scoped_refptr<PictureLayer> RemoteClientLayerFactory::CreateFakePictureLayer(
    ...
    ContentLayerClient* content_layer_client)
{
    scoped_refptr<PictureLayer> layer = FakePictureLayer::Create(content_layer_client);
    layer->SetLayerIdForTesting(engine_layer_id);
    return layer;
}
...
RemoteClientLayerFactory::CreatePushPropertiesCountingLayer(
    int engine_layer_id)
{
    scoped_refptr<PushPropertiesCountingLayer> layer = PushPropertiesCountingLayer::Create();
    layer->SetLayerIdForTesting(engine_layer_id);
    return layer;
}
