    ...
    const gfx::Vector2dF& offset)
{
    ...
    for (size_t i = 0; i < filters.size(); ++i) {
        ...
        case FilterOperation::COLOR_MATRIX:
            image_filter = CreateMatrixImageFilter(op.matrix(), std::move(image_filter));
            break;
        case FilterOperation::ZOOM: {
            ...
            gfx::Vector2dF center = gfx::Vector2dF((size.width() + std::abs(offset.x())) / 2,
                (size.height() + std::abs(offset.y())) / 2);

            // The dimensions of the source content. This shrinks as the texture
            // rectangle gets clipped.
            gfx::Vector2d src_dimensions = gfx::Vector2d((size.width() + std::abs(offset.x())) / op.amount(),
