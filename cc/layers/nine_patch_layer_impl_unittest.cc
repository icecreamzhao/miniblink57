...
namespace {
        ...
        size_t expected_quad_size)
    {
        ...
        std::unique_ptr<NinePatchLayerImpl> layer = NinePatchLayerImpl::Create(host_impl.active_tree(), 1);
        ...
        size_t expected_quad_size)
    {
        ...
        float ratio_left = border_left == 0 ? 0 : (aperture_rect.x() / border.x()),
            ...
            ? 0
            : ((bitmap_size.width() - aperture_rect.right()) / (border.width() - border.x())),
              ratio_bottom = border_bottom == 0
            ? 0
            : ((bitmap_size.height() - aperture_rect.bottom()) / (border.height() - border.y()));
