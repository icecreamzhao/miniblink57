...
namespace {
    ...
    TEST(RenderSurfaceLayerImplTest, AppendQuadsWithScaledMask)
    {
        ...
        LayerTestCommon::LayerImplTest impl;
        std::unique_ptr<LayerImpl> root = LayerImpl::Create(impl.host_impl()->active_tree(), 2);
        root->SetHasRenderSurface(true);
        std::unique_ptr<LayerImpl> surface = LayerImpl::Create(impl.host_impl()->active_tree(), 3);
        ...
        std::unique_ptr<LayerImpl> child = LayerImpl::Create(impl.host_impl()->active_tree(), 5);
        ...
        const RenderPassDrawQuad* quad = RenderPassDrawQuad::MaterialCast(render_pass->quad_list.front());
