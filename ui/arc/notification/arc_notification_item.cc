    ...
    const ArcNotificationData& data)
{
    ...
    case ArcNotificationType::PROGRESS:
        ...
        rich_data.timestamp = base::Time::UnixEpoch() + base::TimeDelta::FromMilliseconds(data.time);
        rich_data.progress = std::max(
            0, std::min(100, static_cast<int>(std::round(static_cast<float>(data.progress_current) / data.progress_max * 100))));
        break;
    }
