...
{
    std::string rtf = "{\\rtf1\\ansi{\\fonttbl\\f0\\fswiss Helvetica;}\\f0\\pard\n"
...
{
    const ui::Clipboard::FormatType kFormat = ui::Clipboard::GetFormatType("chromium/x-test-format");
...
{
    const ui::Clipboard::FormatType kFormat1 = ui::Clipboard::GetFormatType("chromium/x-test-format1");
    std::string payload1("test string1");
    base::Pickle write_pickle1;
    write_pickle1.WriteString(payload1);

    const ui::Clipboard::FormatType kFormat2 = ui::Clipboard::GetFormatType("chromium/x-test-format2");
...
{
    const uint64_t first_sequence_number = this->clipboard().GetSequenceNumber(CLIPBOARD_TYPE_COPY_PASTE);
    ...
    const uint64_t second_sequence_number = this->clipboard().GetSequenceNumber(CLIPBOARD_TYPE_COPY_PASTE);
