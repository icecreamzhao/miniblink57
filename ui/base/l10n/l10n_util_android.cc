...
bool IsLayoutRtl()
{
    ...
    if (!is_layout_rtl_cached) {
        ...
        layout_rtl_cache = static_cast<bool>(Java_LocalizationUtils_isLayoutRtl(env));
    }
...
{
    JNIEnv* env = base::android::AttachCurrentThread();
    ScopedJavaLocalRef<jobject> java_locale = NewJavaLocale(env, locale);
    ScopedJavaLocalRef<jobject> java_display_locale = NewJavaLocale(env, display_locale);
...
ScopedJavaLocalRef<jstring> GetDurationString(JNIEnv* env,
    ...
    jlong timeInMillis)
{
    ScopedJavaLocalRef<jstring> jtime_remaining = base::android::ConvertUTF16ToJavaString(
        env,
