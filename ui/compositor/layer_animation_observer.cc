...
{
    LayerAnimationElement::AnimatableProperties properties = sequence->properties();
    for (unsigned i = LayerAnimationElement::FIRST_PROPERTY;
         i != LayerAnimationElement::SENTINEL;
         i = i << 1) {
        if (i & properties) {
            LayerAnimationElement::AnimatableProperty property = static_cast<LayerAnimationElement::AnimatableProperty>(i);
...
ImplicitAnimationObserver::AnimationStatusForProperty(
    LayerAnimationElement::AnimatableProperty property) const
{
    PropertyAnimationStatusMap::const_iterator iter = property_animation_status_.find(property);
    return iter == property_animation_status_.end() ? ANIMATION_STATUS_UNKNOWN : iter->second;
}
