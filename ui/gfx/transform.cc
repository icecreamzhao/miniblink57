...
bool Transform::IsIdentityOrIntegerTranslation() const
{
    ...
    bool no_fractional_translation = static_cast<int>(matrix_.get(0, 3)) == matrix_.get(0, 3) && static_cast<int>(matrix_.get(1, 3)) == matrix_.get(1, 3) && static_cast<int>(matrix_.get(2, 3)) == matrix_.get(2, 3);
...
bool Transform::IsBackFaceVisible() const
{
    ...
    double cofactor_part_1 = matrix_.get(0, 0) * matrix_.get(1, 1) * matrix_.get(3, 3);

    double cofactor_part_2 = matrix_.get(0, 1) * matrix_.get(1, 3) * matrix_.get(3, 0);

    double cofactor_part_3 = matrix_.get(0, 3) * matrix_.get(1, 0) * matrix_.get(3, 1);

    double cofactor_part_4 = matrix_.get(0, 0) * matrix_.get(1, 3) * matrix_.get(3, 1);

    double cofactor_part_5 = matrix_.get(0, 1) * matrix_.get(1, 0) * matrix_.get(3, 3);

    double cofactor_part_6 = matrix_.get(0, 3) * matrix_.get(1, 1) * matrix_.get(3, 0);

    double cofactor33 = cofactor_part_1 + cofactor_part_2 + cofactor_part_3 - cofactor_part_4 - cofactor_part_5 - cofactor_part_6;
...
bool Transform::Preserves2dAxisAlignment() const
{
    ...
    bool has_x_or_y_perspective = matrix_.get(3, 0) != 0 || matrix_.get(3, 1) != 0;
    ...
    return num_non_zero_in_row_0 <= 1 && num_non_zero_in_row_1 <= 1 && num_non_zero_in_col_0 <= 1 && num_non_zero_in_col_1 <= 1 && !has_x_or_y_perspective;
}
...
{
    return matrix_.get(2, 0) == 0.0 && matrix_.get(2, 1) == 0.0 && matrix_.get(0, 2) == 0.0 && matrix_.get(1, 2) == 0.0 && matrix_.get(2, 2) == 1.0 && matrix_.get(3, 2) == 0.0 && matrix_.get(2, 3) == 0.0;
}
...
bool Transform::Blend(const Transform& from, double progress)
{
    ...
    if (!DecomposeTransform(&to_decomp, *this) || !DecomposeTransform(&from_decomp, from))
        return false;
...
bool Transform::ApproximatelyEqual(const gfx::Transform& transform) const
{
    ...
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 4; col++) {
            const float delta = std::abs(matrix().get(row, col) - transform.matrix().get(row, col));
