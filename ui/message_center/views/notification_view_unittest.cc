...
protected:
    ...
    gfx::Size GetImagePaintSize(ProportionalImageView* view)
    {
        ...
        while (rect.width() > 0 && bitmap.getColor(rect.x(), kHalfHeight) != kBitmapColor)
            rect.Inset(1, 0, 0, 0);
        while (rect.height() > 0 && bitmap.getColor(kHalfWidth, rect.y()) != kBitmapColor)
            rect.Inset(0, 1, 0, 0);
        while (rect.width() > 0 && bitmap.getColor(rect.right() - 1, kHalfHeight) != kBitmapColor)
            rect.Inset(0, 0, 1, 0);
        while (rect.height() > 0 && bitmap.getColor(kHalfWidth, rect.bottom() - 1) != kBitmapColor)
            rect.Inset(0, 0, 0, 1);
...
{
    scoped_refptr<NotificationSettingsDelegate> delegate = new NotificationSettingsDelegate();
...
TEST_F(NotificationViewTest, UpdateButtonCountTest)
{
    ...
    ui::MouseEvent move(ui::ET_MOUSE_MOVED, cursor_location, cursor_location,
        ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ui::EventDispatchDetails details = views::test::WidgetTest::GetEventProcessor(widget())->OnEventFromSource(&move);
...
TEST_F(NotificationViewTest, SettingsButtonTest)
{
    ...
    ui::MouseEvent move(ui::ET_MOUSE_MOVED, cursor_location, cursor_location,
        ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    widget()->OnMouseEvent(&move);
    ui::EventDispatchDetails details = views::test::WidgetTest::GetEventProcessor(widget())
...
TEST_F(NotificationViewTest, FormatContextMessageTest)
{
    ...
    const std::string kVeryLongContextText = "VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY"
                                             "VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY"
                                             "VERY VERY VERY VERY Long Long Long Long Long Long Long Long context";

    const std::string kVeryLongElidedContextText = "VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERY VERYVERY VERY "
    ...
    const std::string kLongUrlContext = "https://"
    ...
    base::string16 result = notification_view()->FormatContextMessage(notification1);
    ...
    EXPECT_TRUE(base::UTF16ToUTF8(result).find(
                    ".veryveryveyrylong.chromium.org")
        != std::string::npos);
