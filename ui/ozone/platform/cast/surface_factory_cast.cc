...
void SurfaceFactoryCast::TerminateDisplay()
{
    ...
    EGLGetDisplayFn get_display = reinterpret_cast<EGLGetDisplayFn>(dlsym(egl_lib_handle, "eglGetDisplay"));
    EGLTerminateFn terminate = reinterpret_cast<EGLTerminateFn>(dlsym(egl_lib_handle, "eglTerminate"));
    DCHECK(get_display);
    DCHECK(terminate);
...
void SurfaceFactoryCast::OnSwapBuffers()
{
    ...
    } else if (overlay_count_ == previous_frame_overlay_count_ && overlay_bounds_ != previous_frame_overlay_bounds_) {
    ...
    gfx::BufferUsage usage)
{
    ...
    public:
        CastPixmap(SurfaceFactoryCast* parent)
            : parent_(parent)
        {
        }
