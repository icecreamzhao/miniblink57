...
public:
    ...
    void Init()
    {
        base::Callback<void(IPC::Message*)> sender = base::Bind(&DispatchToGpuPlatformSupportTaskOnIO, gpu_task_runner_);
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& gpu_task_runner)
{
    ...
    io_helper_thread_->task_runner()->PostTask(
        FROM_HERE, base::Bind(&FakeGpuProcess::InitOnIO, base::Unretained(fake_gpu_process_.get())));
