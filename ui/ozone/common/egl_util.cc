    ...
    const char* gles_library_name)
{
    ...
    base::NativeLibrary gles_library = base::LoadNativeLibrary(base::FilePath(gles_library_name), &error);
    if (!gles_library) {
        LOG(WARNING) << "Failed to load GLES library: " << error.ToString();
        return false;
    }

    base::NativeLibrary egl_library = base::LoadNativeLibrary(base::FilePath(egl_library_name), &error);
    if (!egl_library) {
    ...
    SurfaceFactoryOzone::GLGetProcAddressProc get_proc_address = reinterpret_cast<SurfaceFactoryOzone::GLGetProcAddressProc>(
