    ...
    const gfx::Rect& requested_bounds)
{
    ...
    if (controller_->keyboard_mode() == FULL_WIDTH) {
        const gfx::Rect& window_bounds = controller_->GetContainerWindow()->GetRootWindow()->bounds();
    ...
    ui::LayerAnimator* animator = controller_->GetContainerWindow()->layer()->GetAnimator();
    ...
    if (old_bounds.height() == 0 && child->bounds().height() != 0 && controller_->show_on_resize()) {
