...
namespace {
    ...
    {
        return (window->parent() && window->parent()->GetProperty(kChildWindowVisibilityChangesAnimatedKey)) || window->GetProperty(kWindowVisibilityChangesAnimatedKey);
    }
...
void VisibilityController::UpdateLayerVisibility(aura::Window* window,
    bool visible)
{
    bool animated = window->type() != ui::wm::WINDOW_TYPE_CONTROL && window->type() != ui::wm::WINDOW_TYPE_UNKNOWN && ShouldAnimateWindow(window);
    animated = animated && CallAnimateOnChildWindowVisibilityChanged(window, visible);
    ...
    if (!visible && window->layer()->GetAnimator()->IsAnimatingProperty(ui::LayerAnimationElement::VISIBILITY) && !window->layer()->GetTargetVisibility()) {
        return;
    }
