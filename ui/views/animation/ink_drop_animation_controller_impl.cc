    ...
    InkDropState ink_drop_state)
{
    ...
    if (ink_drop_state != views::InkDropState::HIDDEN) {
        SetHoveredInternal(false, base::TimeDelta::FromMilliseconds(kHoverFadeOutBeforeAnimationDurationInMs));
    }

    // Make sure the ink drop starts from the HIDDEN state it was going to auto
    // transition to it.
    if (ink_drop_animation_->ink_drop_state() == InkDropState::HIDDEN || ShouldAnimateToHidden(ink_drop_animation_->ink_drop_state())) {
