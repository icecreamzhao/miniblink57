...
void TooltipManagerAura::UpdateTooltipManagerForCapture(Widget* source)
{
    ...
    screen_position_client = aura::client::GetScreenPositionClient(target->GetRootWindow());
    if (!screen_position_client)
        return;
...
void TooltipManagerAura::UpdateTooltip()
{
    ...
    if (aura::client::GetTooltipClient(root_window)) {
        ...
        gfx::Point view_point = root_window->GetHost()->dispatcher()->GetLastMouseLocationInRoot();
