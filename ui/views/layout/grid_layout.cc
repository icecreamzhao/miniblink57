...
public:
        ...
        for (typename std::vector<T*>::iterator i = elements->begin();
            ...
            if (element->ResizePercent() > 0) {
                ...
                if (--resized == 0) {
                    ...
                    to_give = static_cast<int>(delta * (element->resize_percent_ / total_percent));
                    remaining -= to_give;
                }
    ...
    {
        DCHECK(start >= 0 && length > 0 && start + length <= static_cast<int>(elements->size()));
        int size = 0;
...
struct ViewState {
    ViewState(ColumnSet* column_set, View* view, int start_col, int start_row,
        ...
        , baseline(-1)
    {
        DCHECK(view && start_col >= 0 && start_row >= 0 && col_span > 0 && row_span > 0 && start_col < column_set->num_columns() && (start_col + col_span) <= column_set->num_columns());
    }
...
void ColumnSet::CalculateMasterColumns()
{
    for (std::vector<Column*>::iterator i = columns_.begin();
        ...
        if (same_size_column_index != -1) {
            DCHECK(same_size_column_index >= 0 && same_size_column_index < static_cast<int>(columns_.size()));
            ...
            if (master_column == NULL) {
                ...
                if (same_size_column_master == NULL) {
                    ...
                    // master column.
                    same_size_column->GetLastMasterColumn()->same_size_columns_.push_back(column);
                ...
                if (same_size_column_master == NULL) {
                    ...
                    column->GetLastMasterColumn()->same_size_columns_.push_back(same_size_column);
                } else if (column->GetLastMasterColumn() != same_size_column->GetLastMasterColumn()) {
                    // The two columns are already linked with other columns.
                    std::vector<Column*>* same_size_columns = &(column->GetLastMasterColumn()->same_size_columns_);
                    std::vector<Column*>* other_same_size_columns = &(same_size_column->GetLastMasterColumn()->same_size_columns_);
...
void ColumnSet::AccumulateMasterColumns()
{
    ...
    for (std::vector<Column*>::iterator i = columns_.begin();
        ...
        if (master_column && std::find(master_columns_.begin(), master_columns_.end(), master_column) == master_columns_.end()) {
...
void ColumnSet::DistributeRemainingWidth(ViewState* view_state)
{
    ...
    if (resizable_columns > 0) {
        ...
        for (int i = start_col, resize_i = 0; i < max_col; ++i) {
            if (columns_[i]->IsResizable()) {
                resize_i++;
                int delta = (resize_i == resizable_columns) ? remaining_width : static_cast<int>(width * columns_[i]->ResizePercent() / total_resize);
...
void ColumnSet::CalculateSize()
{
    ...
    std::vector<ViewState*>::iterator view_state_iterator = view_states_.begin();
    for (; view_state_iterator != view_states_.end() && (*view_state_iterator)->col_span == 1; ++view_state_iterator) {
...
void GridLayout::SkipColumns(int col_count)
{
    ...
    DCHECK(current_row_col_set_ && next_column_ <= current_row_col_set_->num_columns());
    SkipPaddingColumns();
}
...
{
    DCHECK(current_row_col_set_ && col_span > 0 && row_span > 0 && (next_column_ + col_span) <= current_row_col_set_->num_columns());
    // We don't support baseline alignment of views spanning rows. Please add if
    // you need it.
    DCHECK(v_align != BASELINE || row_span == 1);
    ViewState* state = new ViewState(current_row_col_set_, view, next_column_, current_row_,
...
void GridLayout::Layout(View* host)
{
    ...
    for (std::vector<ViewState*>::iterator i = view_states_.begin();
        ...
        View* view = (*i)->view;
        DCHECK(view);
        int x = column_set->columns_[view_state->start_col]->Location() + insets_.left();
    ...
    gfx::Size* pref) const
{
    ...
    for (std::vector<ColumnSet*>::iterator i = column_sets_.begin();
        ...
        (*i)->Resize(width - (*i)->LayoutWidth() - insets_.left() - insets_.right());
    ...
    for (std::vector<ViewState*>::iterator i = view_states_.begin();
        ...
        if (view_state->h_align == FILL) {
            ...
            int actual_width = view_state->column_set->GetColumnWidth(view_state->start_col,
                view_state->col_span);
            if (actual_width != view_state->pref_width && !view_state->pref_height_fixed) {
                ...
                view_state->pref_height = view_state->view->GetHeightForWidth(actual_width);
                view_state->remaining_height = view_state->pref_height;
            }
        }
    }
    ...
    for (; view_states_iterator != view_states_.end() && (*view_states_iterator)->row_span == 1; ++view_states_iterator) {
        ViewState* view_state = *view_states_iterator;
        Row* row = rows_[view_state->start_row];
        row->AdjustSize(view_state->remaining_height);
        if (view_state->baseline != -1 && view_state->baseline <= view_state->pref_height) {
    ...
    pref->set_height(rows_[rows_.size() - 1]->Location() + rows_[rows_.size() - 1]->Size() + insets_.height());
...
{
    DCHECK(view_state->view && (view_state->view->parent() == NULL || view_state->view->parent() == host_));
...
void GridLayout::AddRow(Row* row)
{
    ...
    // that.
    DCHECK(remaining_row_span_ <= 0 || row->column_set() == NULL || row->column_set() == GetLastValidColumnSet());
    next_column_ = 0;
    rows_.push_back(row);
...
void GridLayout::SkipPaddingColumns()
{
    if (!current_row_col_set_)
        return;
    while (next_column_ < current_row_col_set_->num_columns() && current_row_col_set_->columns_[next_column_]->is_padding_) {
        next_column_++;
    }
}
