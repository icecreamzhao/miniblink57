...
namespace examples {
    ...
    void ButtonExample::CreateExampleView(View* container)
    {
        ...
        LabelButton* styled_button = new LabelButton(this, ASCIIToUTF16("Styled Button"));
        styled_button->SetStyle(Button::STYLE_BUTTON);
        container->AddChildView(styled_button);

        LabelButton* disabled_button = new LabelButton(this, ASCIIToUTF16("Disabled Button"));
        ...
        if (event.IsControlDown()) {
            if (event.IsShiftDown()) {
                ...
                label_button_->SetImage(Button::STATE_NORMAL,
                    label_button_->GetImage(Button::STATE_NORMAL).isNull() ? *icon_ : gfx::ImageSkia());
            } else {
