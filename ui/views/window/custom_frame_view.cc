...
namespace {
    ...
    {
        static const gfx::FontList title_font_list = internal::NativeWidgetPrivate::GetWindowTitleFontList();
...
gfx::Size CustomFrameView::GetMaximumSize() const
{
    ...
    gfx::Size converted_size = frame_->non_client_view()->GetWindowBoundsForClientBounds(
                                                            gfx::Rect(max_size))
                                   .size();
...
int CustomFrameView::NonClientBorderThickness() const
{
    ...
    return FrameBorderThickness() + (ShouldShowClientEdge() ? kClientEdgeThickness : 0);
}
...
{
    return kTitlebarTopAndBottomEdgeThickness + (ShouldShowClientEdge() ? kClientEdgeThickness : 0);
}
...
gfx::Rect CustomFrameView::IconBounds() const
{
    ...
    int unavailable_px_at_top = frame_->IsMaximized() ? frame_thickness : kTitlebarTopAndBottomEdgeThickness;
    ...
    int y = unavailable_px_at_top + (NonClientTopBorderHeight() - unavailable_px_at_top - size - TitlebarBottomThickness() + 1) / 2;
...
void CustomFrameView::PaintRestoredClientEdge(gfx::Canvas* canvas)
{
    ...
    int shadowed_area_bottom = std::max(shadowed_area_top, shadowed_area_bounds.bottom());
    ...
    const gfx::ImageSkia* bottom_left = rb.GetImageSkiaNamed(IDR_CONTENT_BOTTOM_LEFT_CORNER);
    const gfx::ImageSkia* bottom_center = rb.GetImageSkiaNamed(IDR_CONTENT_BOTTOM_CENTER);
    const gfx::ImageSkia* bottom_right = rb.GetImageSkiaNamed(IDR_CONTENT_BOTTOM_RIGHT_CORNER);
...
void CustomFrameView::LayoutWindowControls()
{
    ...
    int extra_width = is_maximized ? (kFrameBorderThickness - kFrameShadowThickness) : 0;
    ...
    WindowButtonOrderProvider* button_order = WindowButtonOrderProvider::GetInstance();
    const std::vector<views::FrameButton>& leading_buttons = button_order->leading_buttons();
    const std::vector<views::FrameButton>& trailing_buttons = button_order->trailing_buttons();

    ImageButton* button = NULL;
    for (std::vector<views::FrameButton>::const_iterator it = leading_buttons.begin(); it != leading_buttons.end(); ++it) {
    ...
    for (std::vector<views::FrameButton>::const_reverse_iterator it = trailing_buttons.rbegin(); it != trailing_buttons.rend(); ++it) {
        button = GetImageButton(*it);
        if (!button)
            continue;
        gfx::Rect target_bounds(gfx::Point(next_button_x, caption_y),
...
void CustomFrameView::LayoutTitleBar()
{
    ...
    title_bounds_.SetRect(title_x,
        ...
        std::max(0, maximum_title_bar_x_ - kTitleCaptionSpacing - title_x), title_height);
}
